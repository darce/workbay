...
check-review-pipeline: check-disk-space ## Run the deterministic review-pipeline tests (explicit collection; no wildcard)
	@$(REVIEW_PIPELINE_JUDGE_ROOT)/scripts/assert_gate_interpreter.sh "$(REVIEW_PIPELINE_PYTHON)" "check-review-pipeline"
	@JUDGE_ROOT="$(REVIEW_PIPELINE_JUDGE_ROOT)" SYSTEM_PYTHON="$(REVIEW_PIPELINE_PYTHON)" WORKBAY_DISABLE_PYTEST_PATH_GUARD=1 PYTHONPATH="$(REVIEW_PIPELINE_PYTHONPATH)" "$(REVIEW_PIPELINE_PYTHON)" -m pytest -q "$(REVIEW_PIPELINE_JUDGE_ROOT)/packages/workbay-system/workbay_system/payload/scripts/test_review_pipeline.py"
...
dogfood: ## Install latest/TAG git overlay, package (git-mirror uv specs), or worktree into this repo
	...
	if [ "$$source" = "package" ]; then \
	    ...
	    protocol_spec="$(DOGFOOD_PROTOCOL_SPEC)"; \
