	...
	@cd packages/mcp-workbay-orchestrator && \
	    ...
	    if test -x "$$PY" && "$$PY" -c "$$REQ" >/dev/null 2>&1; then :; \
	        ...
	        if test -x "$$PY"; then \
	            ...
	            echo "  (do NOT 'uv sync --extra dev' here: the unpublished mcp-workbay-handoff sibling breaks the resolve)." >&2; \
	        fi; \
	        PY=python; \
	        "$$PY" -c "$$REQ" >/dev/null 2>&1 || { \
	            echo "test-contract: pyenv python ($$PY) also cannot import pytest+fastmcp; cannot run the contract test." >&2; \
	            echo "  Install dev test deps: uv pip install --python packages/mcp-workbay-orchestrator/.venv/bin/python 'pytest>=8' 'hypothesis>=6'" >&2; \
	            exit 1; \
	        }; \
	    fi; \
...
# so `workbay_protocol` resolves inside those subprocesses too. Probe the
