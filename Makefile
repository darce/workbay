...
sync-handoff-embeddings: ## Provision mcp-workbay-handoff[embeddings] into root .venv (implementation note)
	...
	# or the equivalent extra on that env (REV-A-002).
	uv sync --group dev
...
check-all: check-disk-space ## Format+lint+mypy+tests for every package; expensive suites stamp-skip when unchanged (CHECK_ALL_FRESH=1 forces full run)
	...
	$(MAKE) format-all
	$(MAKE) brand-check
	python3 scripts/check_all_cache.py run --key check-handoff --paths packages/mcp-workbay-handoff packages/workbay-protocol/src packages/workbay-protocol/pyproject.toml packages/mcp-workbay-orchestrator/src packages/mcp-workbay-orchestrator/pyproject.toml packages/workbay-codex-bridge/src packages/workbay-codex-bridge/pyproject.toml packages/workbay-system/workbay_system/payload Makefile pyproject.toml uv.lock -- $(MAKE) -C packages/mcp-workbay-handoff check-handoff
	python3 scripts/check_all_cache.py run --key check-orchestrator --fingerprint "pytest-workers=$(PYTEST_WORKERS)" --paths packages/mcp-workbay-orchestrator packages/workbay-protocol/src packages/workbay-protocol/pyproject.toml packages/mcp-workbay-handoff/src packages/mcp-workbay-handoff/pyproject.toml packages/workbay-codex-bridge/src packages/workbay-codex-bridge/pyproject.toml packages/workbay-system/workbay_system/payload Makefile pyproject.toml uv.lock -- $(MAKE) -C packages/mcp-workbay-orchestrator check-orchestrator
	python3 scripts/check_all_cache.py run --key check-bridge --paths packages/workbay-codex-bridge packages/workbay-protocol/src packages/workbay-protocol/pyproject.toml Makefile pyproject.toml uv.lock -- $(MAKE) -C packages/workbay-codex-bridge check-bridge
	python3 scripts/check_all_cache.py run --key check-protocol --paths packages/workbay-protocol packages/workbay-system/workbay_system/payload Makefile pyproject.toml uv.lock -- $(MAKE) check-protocol
	python3 scripts/check_all_cache.py run --key check-system --fingerprint "pytest-workers=$(PYTEST_WORKERS)" --paths packages/workbay-system scripts config .claude docs/workbay Makefile pyproject.toml uv.lock -- $(MAKE) check-system
	python3 scripts/check_all_cache.py run --key check-system-integration --paths packages/workbay-system scripts config .claude docs/workbay Makefile pyproject.toml uv.lock -- $(MAKE) check-system-integration
	$(MAKE) check-heuristics-wiring
	python3 scripts/check_all_cache.py run --key check-workbay --paths packages/workbay Makefile pyproject.toml uv.lock -- $(MAKE) check-workbay
...
.PHONY: heuristics-sync
heuristics-sync: ## Sync darce/heuristics-canon lexicons into payload SSOTs (dry-run default; FLAGS=--execute writes + wires guard/dogfood/drift)
	@python packages/workbay-system/scripts/sync_heuristics_canon.py $(FLAGS)
	@if [ "$(FLAGS)" = "--execute" ]; then \
		$(MAKE) check-heuristics-wiring && \
		$(MAKE) dogfood DOGFOOD_SOURCE=worktree && \
		$(MAKE) check-overlay-drift ; \
	else \
		echo "heuristics-sync: dry-run — pass FLAGS=--execute to write SSOTs + wire (guard -> worktree dogfood -> overlay drift)."; \
		echo "  propagation: self-host updates immediately via worktree-source dogfood; external consumer repos pick it up on their next overlay update from the mirror."; \
	fi
