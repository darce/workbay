...
.PHONY: check-system
check-system: check-disk-space ## Run the workbay-system FAST suite (plugin/skill/Make-target contracts) + scripts/ unit tests, EXCLUDING integration-marked tests (PYTEST_WORKERS, default 4; 0 = serial). Integration tests run via check-system-integration; check-all runs both.
	@PYTHONPATH="$(SYSTEM_PYTHONPATH)" $(SYSTEM_PYTHON) -m pytest packages/workbay-system/tests scripts/tests -m "not integration" -q --durations=25 $(if $(filter-out 0,$(PYTEST_WORKERS)),-n $(PYTEST_WORKERS),)
...
check-telemetry-fresh: ## Advisory: report silent agent_errors/stale MAINT rows without failing local gates
	PYTHONPATH=packages/workbay-bootstrap/src:packages/mcp-workbay-handoff/src .venv/bin/python -m workbay_bootstrap doctor --target . || true
