...
#   DOGFOOD_PROTOCOL_SPEC  — git+ uv spec used when DOGFOOD_SOURCE=package (never a bare dist name).
#                            Required: workbay-bootstrap pins workbay-protocol>=0.3.0 and the
#                            newest workbay-protocol on any index is a stale 0.2.0, so omitting
#                            this resolves to that abandoned upload and the closure fails.
...
DOGFOOD_PROTOCOL_SPEC ?= git+$(DOGFOOD_GIT_URL)@$(DOGFOOD_PACKAGE_REF)\#subdirectory=packages/workbay-protocol
...
check-disk-space: ## Preflight: fail fast if a working volume (tmp, uv cache, or repo) is nearly full
	@rc=0; \
	for label in tmp uv_cache repo; do \
	    case "$$label" in \
	        tmp) vol="$${TMPDIR:-/tmp}" ;; \
	        uv_cache) vol="$$(uv cache dir 2>/dev/null)" ;; \
	        repo) vol="$(CURDIR)" ;; \
	    esac; \
	    if [ -z "$$vol" ] || [ ! -e "$$vol" ]; then \
	        echo "check-disk-space: FAIL volume=$$label path=$$vol reason=path_unavailable" >&2; \
	        rc=1; \
	        continue; \
	    fi; \
	    if ! avail=$$(df -Pm "$$vol" 2>/dev/null | awk 'NR==2{print $$4}'); then \
	        echo "check-disk-space: FAIL volume=$$label path=$$vol reason=df_failed" >&2; \
	        exit 1; \
	    fi; \
	    case "$$avail" in ''|*[!0-9]*) \
	        echo "check-disk-space: FAIL volume=$$label path=$$vol reason=invalid_free_mb value='$$avail'" >&2; \
	        exit 1;; \
	    esac; \
	    if [ "$$avail" -lt "$(DISK_FLOOR_MB)" ]; then \
	        echo "check-disk-space: FAIL volume=$$label free_mb=$$avail floor_mb=$(DISK_FLOOR_MB) path=$$vol" >&2; \
	        rc=1; \
	    else \
	        echo "check-disk-space: OK volume=$$label free_mb=$$avail floor_mb=$(DISK_FLOOR_MB) path=$$vol"; \
	    fi; \
	done; \
	exit $$rc
...
.PHONY: check-system
check-system: check-disk-space check-review-pipeline check-duplicate-test-basenames ## Run the workbay-system FAST suite (plugin/skill/Make-target contracts) + scripts/ unit tests, EXCLUDING integration-marked tests (PYTEST_WORKERS, default 4; 0 = serial; auto-serial when xdist missing). Integration tests run via check-system-integration; check-all runs both.
...
.PHONY: check-duplicate-test-basenames
check-duplicate-test-basenames: ## Fail when check-system's collected test roots share a test-module basename
	@scripts/assert_gate_interpreter.sh "$(SYSTEM_PYTHON)" "check-duplicate-test-basenames"
	@$(SYSTEM_PYTHON) scripts/check_duplicate_test_basenames.py

.PHONY: check-review-pipeline
check-review-pipeline: check-disk-space ## Run the deterministic review-pipeline tests (explicit collection; no wildcard)
	@$(_RP_JUDGE_ROOT)/scripts/assert_gate_interpreter.sh "$(_RP_PYTHON)" "check-review-pipeline"
	@JUDGE_ROOT="$(_RP_JUDGE_ROOT)" SYSTEM_PYTHON="$(_RP_PYTHON)" WORKBAY_DISABLE_PYTEST_PATH_GUARD=1 PYTHONPATH="$(_RP_PYTHONPATH)" "$(_RP_PYTHON)" -m pytest -q "$(_RP_JUDGE_ROOT)/packages/workbay-system/workbay_system/payload/scripts/test_review_pipeline.py"
...
check-hooks: check-disk-space ## Run PreToolUse hook unit tests (root + payload twins as separate invocations)
	...
	@WORKBAY_DISABLE_PYTEST_PATH_GUARD=1 PYTHONPATH="$(SYSTEM_PYTHONPATH)" $(SYSTEM_PYTHON) -m pytest -q \
		...
		scripts/hooks/test_guard_agent_remote_only_implementation.py
	@WORKBAY_DISABLE_PYTEST_PATH_GUARD=1 PYTHONPATH="$(SYSTEM_PYTHONPATH)" $(SYSTEM_PYTHON) -m pytest -q \
		...
		packages/workbay-system/workbay_system/payload/scripts/hooks/test_guard_agent_remote_only_implementation.py
...
.PHONY: lane-dispose
lane-dispose: ## Dry-run terminal-lane disposition scan/backfill/abandon. Pass DISPOSE_ARGS="scan|backfill|abandon ..."; apply flags only through DISPOSE_ARGS.
	.venv/bin/mcp-workbay-orchestrator --workspace-root . lane-dispose $(DISPOSE_ARGS)
...
.PHONY: test-orchestrator test-bridge test-tier-fast test-tier-e2e
...
# WORKBAY_TEST_TIER entry points for scripts/remote_gate.sh (space-free env
# tokens only). Do not change what the four sub-targets do on their own.
# SUBJECTS ?= lets REMOTE_GATE_ENV="WORKBAY_TEST_TIER_SUBJECTS=a,b" feed the
# e2e subject filter; comma-separated, no spaces.
SUBJECTS ?= $(WORKBAY_TEST_TIER_SUBJECTS)

test-tier-fast: ## Run orchestrator/handoff/bootstrap/system suites with WORKBAY_TEST_TIER=fast (deselects e2e + timing)
	WORKBAY_TEST_TIER=fast $(MAKE) test-orchestrator test-handoff check-bootstrap check-system

test-tier-e2e: ## Run orchestrator/handoff/bootstrap/system suites with WORKBAY_TEST_TIER=e2e [SUBJECTS=a,b]
	WORKBAY_TEST_TIER=e2e WORKBAY_TEST_TIER_SUBJECTS=$(SUBJECTS) $(MAKE) test-orchestrator test-handoff check-bootstrap check-system
...
check-all: check-disk-space check-review-pipeline ## Format+lint+mypy+tests for every package; expensive suites stamp-skip when unchanged (CHECK_ALL_FRESH=1 forces full run)
	@# check-system also runs this gate, but the direct prerequisite survives a fresh cache stamp.
...
.PHONY: landing-census
# scripts/landing_census.py answers the two questions the landing order is
# derived from -- what is still unmerged, and (with AGAINST) what each branch
# would still add on top of a candidate. It had no make entry point, so every
# run was an agent hand-assembling an invocation: the ad-hoc measurement this
# work exists to replace.
landing-census: ## Census unmerged branches, read-only. [REPO=.] [INTEGRATION=main] [AGAINST=<rev> residual mode] [MAX_BRANCHES=500] [JSON=1]
	@test -x "$(or $(LANDING_GATE_PYTHON),$(WORKSPACE_PYTHON))" || { \
		echo "landing-census: interpreter not found: $(or $(LANDING_GATE_PYTHON),$(WORKSPACE_PYTHON)) — pass LANDING_GATE_PYTHON=<path>" >&2; \
		exit 2; \
	}
	@"$(or $(LANDING_GATE_PYTHON),$(WORKSPACE_PYTHON))" $(CURDIR)/scripts/landing_census.py \
		--repo "$(or $(REPO),$(CURDIR))" \
		--integration-ref "$(or $(INTEGRATION),main)" \
		$(if $(AGAINST),--against "$(AGAINST)",) \
		$(if $(MAX_BRANCHES),--max-branches $(MAX_BRANCHES),) \
		$(if $(JSON),--json,)

.PHONY: landing-gate
# The three defects that made past landing measurements lie -- a subject
# measured behind the integration ref, a gate run whose imports resolved to a
# different checkout, and a failure-id diff read as "green" when nothing was
# measured -- are encoded as typed outcomes in scripts/landing_gate.py. This
# target is the deterministic entry point; running the gate by hand is how the
# defects went unnoticed in the first place.
#
# Exit codes are distinct on purpose: 0 mergeable, 1 refused with a named
# outcome, 2 the gate could not establish a fact.
landing-gate: ## Refuse a merge whose landing measurement is stale, cross-checkout, truncated, unmeasured, or incomparable. SUBJECT=<branch> [WORKTREE=.] [INTEGRATION=main] [BASELINE_IDS=<file>] [SUBJECT_IDS=<file>] [BASELINE_RAW=<file>] [SUBJECT_RAW=<file>] [BASELINE_SCOPE=<file>] [SUBJECT_SCOPE=<file>] [JSON=1]
	@test -n "$(SUBJECT)" || { echo "landing-gate: SUBJECT=<branch> is required" >&2; exit 2; }
	@test -x "$(or $(LANDING_GATE_PYTHON),$(WORKSPACE_PYTHON))" || { \
		echo "landing-gate: interpreter not found: $(or $(LANDING_GATE_PYTHON),$(WORKSPACE_PYTHON)) — lane worktrees often have no .venv; pass LANDING_GATE_PYTHON=<path>" >&2; \
		exit 2; \
	}
	@"$(or $(LANDING_GATE_PYTHON),$(WORKSPACE_PYTHON))" $(CURDIR)/scripts/landing_gate.py \
		--worktree "$(or $(WORKTREE),$(CURDIR))" \
		--subject-ref "$(SUBJECT)" \
		--integration-ref "$(or $(INTEGRATION),main)" \
		$(if $(BASELINE_IDS),--baseline-ids "$(BASELINE_IDS)",) \
		$(if $(SUBJECT_IDS),--subject-ids "$(SUBJECT_IDS)",) \
		$(if $(BASELINE_RAW),--baseline-raw "$(BASELINE_RAW)",) \
		$(if $(SUBJECT_RAW),--subject-raw "$(SUBJECT_RAW)",) \
		$(if $(BASELINE_SCOPE),--baseline-scope "$(BASELINE_SCOPE)",) \
		$(if $(SUBJECT_SCOPE),--subject-scope "$(SUBJECT_SCOPE)",) \
		$(if $(JSON),--json,)
...
dogfood: ## Install latest/TAG git overlay, package (git-mirror uv specs), or worktree into this repo
	...
	if [ "$$source" = "package" ] && [ -e "$(CURDIR)/packages/workbay-bootstrap/pyproject.toml" ]; then \
	    echo "refuse: package dogfood into the source monorepo is a self-target install." >&2; \
	    exit 2; \
	fi; \
	if [ "$$source" = "package" ]; then \
	    bootstrap_spec=$(call shell-quote,$(DOGFOOD_BOOTSTRAP_SPEC)); \
	    system_spec=$(call shell-quote,$(DOGFOOD_SYSTEM_SPEC)); \
	    protocol_spec=$(call shell-quote,$(DOGFOOD_PROTOCOL_SPEC)); \
	    uvx_flags="$(DOGFOOD_UVX_FLAGS)"; \
	    echo "==> dogfood installing package overlay into $(CURDIR)"; \
	    echo "==> using $$bootstrap_spec with $$system_spec $$protocol_spec"; \
	    uvx $$uvx_flags --from "$$bootstrap_spec" --with "$$system_spec" --with "$$protocol_spec" \
	        workbay-bootstrap install --source package --target "$(CURDIR)" --remote-url $(call shell-quote,$(DOGFOOD_GIT_URL)) --remote-ref $(call shell-quote,$(DOGFOOD_PACKAGE_REF)) $(DOGFOOD_INSTALL_FLAGS) && \
	    uvx $$uvx_flags --from "$$bootstrap_spec" --with "$$system_spec" --with "$$protocol_spec" \
