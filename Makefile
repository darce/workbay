...
sync-handoff-embeddings: ## Provision mcp-workbay-handoff[embeddings] into root .venv (implementation note)
	...
	# After sync: backfill concepts if needed, then operator-owned MCP restart.
	uv sync --group dev
...
.PHONY: lane-reap
lane-reap: ## Dry-run conclusive-dead lane reaper (repo-wide). Pass REAP_ARGS="--apply [--task-ref X]" to apply; --apply only via REAP_ARGS.
