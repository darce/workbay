...
### Changed
- **Re-pinned to `workbay-system` 0.4.11.** This picks up the remote-dispatch cleanup fix and the measured VM admission guidance in the skills.

## [0.4.10] — 2026-09-21

### Changed
- Re-pinned to `workbay-system` 0.4.10 for the standalone-reap debounce fix.

## [0.4.9] — 2026-09-21

### Changed
- Requires workbay-system 0.4.9.

## [0.4.8] — 2026-09-20

### Changed
- Requires workbay-system 0.4.8.

## [0.4.7] — 2026-09-19

### Changed
- Requires workbay-system 0.4.7.

## [0.4.6] — 2026-09-18

### Changed
- Requires workbay-system 0.4.6.

## [0.4.5] — 2026-09-18

### Changed
- Install, update and repair write `codemap_mode: "available"` when the ledger has no explicit value, so the closed codemap-first hook no longer refuses every source query with `policy_missing`; explicit values are preserved.

## [0.4.4] — 2026-09-18

### Changed
- Install/update refuses an invalid `.mcp.json` instead of overwriting it; on-policy `embedding.env` is fully projected and malformed files are refused.
- Embeddings policy migration (E1) and the Claude/Codex install-update-repair environment gap (IO-01) are closed.

## [0.4.3] — 2026-09-16

### Changed
- mcp-sync validates every surface before writing, pins the consumer root once (macOS `F_GETPATH` uses a MAXPATHLEN buffer), and the atomic writer refuses symlinked or swapped config parents.
- Repair skips a refused MCP sync instead of reporting success; package update forwards the persisted gitonly closure URL/ref.
- Self-host protection: dogfood remote-ref defaults, self-host refusal, and shared labels for identical self-host payload surfaces.
- Install and review-pipeline.mk share one ownership model; manifest Claude adapters route through the shared path.
- Added the `WORKBAY_TEST_TIER` pytest plugin and package conftest.

## [0.4.2] — 2026-09-13

### Changed
- Stack pin refresh only: the `workbay-system` floor moves to 0.4.3.

## [0.4.1] — 2026-09-11

### Changed
- Stack pin refresh only.

## [0.4.0] — 2026-09-10

### Added
- Doctor now reports MCP launcher version skew, while installs preserve configured MCP environments and safely recognize identical repository payloads during materialization.

## [0.3.29] — 2026-09-03

### Fixed
- Install no longer promotes `embeddings_mode` out of scope (ILD-01); the setting is left exactly as the operator configured it.
- The tracked install ledger no longer records the run-local `git_writes` log (wall-clock `duration_ms`, host cwd, clone-vs-fetch argv).

## [0.3.28] — 2026-09-02

### Changed
- `stack_source` is validated as a whole and refused cleanly in the CLI, with validation errors caught at the CLI boundary and attestation fields checked.
- Release tags are cut annotated with provenance and guarded on push; `check_release_tag_provenance` verifies the tag being cut.
- Doctor `gitonly_mcp_tool_source` audits live uv receipts with `source_kind` scoping; manifest drift is a refresh note, while missing or unreadable receipts keep error severity and never clobber a diverged sidecar.
- Payload `settings.hooks.json` is adopted into the managed surface; `hook_command_unresolved` downgrades to a warning only on true orphans.

## [0.3.27] — 2026-08-24

### Changed
- Release tags are annotated with provenance and guarded on push; `check_release_tag_provenance` audits them; installs record `stack_source` ref and sha.
- Payload `settings.hooks.json` is adopted into the managed surface; `hook_command_unresolved` downgrades to a warning only on true orphans.
- Doctor `gitonly_mcp_tool_source` audits live uv receipts with `source_kind` scoping; manifest drift is a refresh note; missing or unreadable receipts keep error severity and never clobber a diverged sidecar.

## [0.3.26] — 2026-08-23

### Changed
- Floor-sync: require workbay-system >= 0.3.25 and mcp-workbay-orchestrator >= 0.2.20 (synced MCP pins).

## [0.3.25] — 2026-08-22

### Fixed
- **Git-only MCP install carries the declared FastMCP pin on argv.** `uv tool install --no-sources` does not inherit the handoff `fastmcp` specifier, so the install command now adds `--with <declared-pin>`.
- **Pin resolution now reads git-sourced member specs too (round-3 claude-01).** `--source package --remote-url <git-url> --remote-ref <tag>` installs (and any other git-only install whose member specs are all `git+...` pins) have no local checkout and no installed handoff metadata yet, so the pin used to be unreadable and the install hard-aborted.
- **Pin-unreadable abort now lands a receipt row + abort snapshot (round-3 claude-02).** `_install_gitonly_mcp_tools` re-wraps `DeclaredFastmcpRequirementError` into `InstallExecutionError` before it reaches `_execute_gitonly_mcp_tools_step`, so the step's `except DeclaredFastmcpRequirementError` arm never fired — the abort raised straight past the receipt/snapshot writer, and `repair`/doctor had no failed step to retry.
- **`gitonly_closure` no longer reimplements pin resolution (round-3 claude-04).** `_require_declared_fastmcp_requirement` duplicated the exact member-spec / git-ref / metadata / checkout-walk ordering that `_declared_fastmcp.resolve_declared_fastmcp_requirement` already owns (REF-09 — a future new pin source only needed adding once, but the duplicate meant it had to be added twice, and drifted).
- **Added a static three-way fastmcp specifier parity test (CL0816-MCPSPEC-R4REV-fixint-01).** `mcp-workbay-handoff`, `mcp-workbay-orchestrator`, and the private canvas member each declare their own `fastmcp` dependency specifier; a test now parses all three pyprojects directly and fails, naming every value, if any of them drifts from the other two.
- **Embedding-backfill-gap facet no longer crashes doctor() when `workbay_handoff_mcp` is unimportable (CL0816-SEMRET-R3REV-claude-02).** `_doctor_embedding_env_map`'s `merge_embedding_env` import is now guarded; an unimportable `embedding_env` module routes to the existing `instrument_unavailable` warning finding instead of raising.
- **`_doctor_embedding_artifacts` no longer diverges from the backfill-gap facet on whether the embeddings gate is disabled (CL0816-SEMRET-R3REV-claude-04).** The artifacts facet's disable check now delegates to `embedding_backfill_doctor._doctor_embeddings_disabled` (the same nonempty-process-overlay rule the backfill facet uses via `workbay_handoff_mcp.embedding_env.merge_embedding_env`) instead of its own presence-OR, non-quote-stripping check (`embeddings_disabled() or embeddings_gate_disabled_from_file(target)`).
- **The backfill-gap facet collapsed a genuinely-missing store module and a present-but-incomplete one to the same `instrument_unavailable` warning (CL0816-SEMRET-R3REV-claude-07b).** `_load_store_backfill_contract` now distinguishes `ModuleNotFoundError` (the store module, or a hard dependency like numpy, is not installed — an absent probe instrument, still `instrument_unavailable:` / warning severity) from a plain `ImportError` on an importable module missing an expected name (a real API-drift contract failure between this doctor and the store it pins, now `error` severity, no longer downgraded to a warning).

### Tests
- **The `_doctor_embedding_backfill_gap` facet's `gather_concepts()` branch is now exercised by a facet-level test (CL0816-SEMRET-R3REV-claude-05).** Every prior facet test seeded only `decisions` + `concept_embeddings`, so `_collect_concepts` always took the present-tables fallback; a real handoff.db has all `CONCEPT_BACKFILL_SOURCES` tables and takes the `contract.gather(conn, None)` branch instead.
- **`test_doctor_embedding_backfill_gap.py`'s hermetic-env fixture could leak a direct `os.environ` write past teardown (CL0816-SEMRET-R3REV-claude-06).** `monkeypatch.delenv(key, raising=False)` on an absent key registers no undo action, so a facet under test writing directly to `os.environ` (mirroring `apply_embedding_env`) survived teardown into the next test.

## [0.3.24] — 2026-08-17

### Added
- `--hooks none|recorded|repo|all` install and repair profile, and an `apply-hooks` verb whose `--target` defaults to cwd (as do `doctor`, `status` and `repair`).
- Doctor facet `embedding_backfill_gap` plus an opt-in make target.

### Fixed
- `apply-hooks --hooks none` is a 0-adapter no-op and prints a receipt saying so; `hook_adapter_drift` is skipped for `repair --hooks none`; `recorded` is repair-only on the install parser.
- git-spec credential scrubbing fails closed: bare `file:` URLs, empty-authority file URLs, malformed IPv6 authorities, and path or control-character payloads no longer leak userinfo.
- git-only install: `resolve_tool_python` rejects `/usr/bin/env` and non-Python interpreters, ordinary uv failures soft-defer while a CPE hard-aborts, and repair receipt polarity and non-convergence are surfaced.
- Doctor remediations name the `workbay` front door and `workbay apply-hooks`, and CONSUMER install docs are unified on v0.1.55.

## [0.3.23] — 2026-08-10

### Added
- **Embeddings-runtime drift guard.** `mcp-workbay-handoff[embeddings]` is declared on the root sync path and a doctor facet reports when the installed runtime has drifted from the declaration.

### Fixed
- **The embeddings runtime probe is bounded and fails closed on timeout.** It previously ran through an unbounded external call whose timeout was treated as "probably fine".
- **Harness alias retirement no longer destroys content.** Skill-markdown aliases are unlinked by inode, which fixes the crash on case-insensitive filesystems and stops a retirement pass from clobbering a file it merely shares a name with.
- **Doctor predicates no longer pass vacuously.** A "clean" verdict now requires positive evidence, and the tracked-source ignore-leak check is gated on the file actually being tracked.
- **Git-only first-call offline behaviour defers like prewarm**, with a repair arm, instead of failing the first call outright.

### Removed
- **The doctor public-index query path and `PYPI_URL`**, matching the protocol change above.

## [0.3.22] — 2026-08-02

### Fixed
- **Reinstall preserves receipt extras.** A reinstall dropped extras recorded on the install receipt; a missing reader floor is now read as absence rather than as a downgrade signal.
- **Probe-unavailable is a channel, not silence.** Five sites that swallowed an unavailable probe now thread the unavailable state through, so a degraded probe is distinguishable from a negative result.

## [0.3.21] — 2026-07-28

### Added
- **Doctor facet: codemap index freshness.** A stdlib-only mini-probe reports whether the codemap index matches the tree it indexes.

### Changed
- **Doctor verifies the runtime interpreter, not the front door.** Bootstrap now probes the interpreter the uv-tool console script actually runs.
- The git-only handoff install spec attaches `[embeddings]` as a PEP 508 name-extra on the runtime `--from` spec (opt-in via `package_extras`), so the optional embedding libs are resolvable from a git-only install instead of silently absent.

## [0.3.20] — 2026-07-22

### Changed
- Version-floor + monorepo pin sync for the coordinated release; no behavior change in this package.

## [0.3.19] — 2026-07-17

### Changed
- Install-surface wiring for the check-all cache (0126): seed `/.workbay-cache` in the consumer `.gitignore`, exclude `__pycache__`/`.DS_Store` from carved-child surface enumeration, recapture install goldens, and add a consumer-sim eval gate for the cache macro.

## [0.3.18] — 2026-07-16

### Changed
- Version floor sync for the 0144 release train (stack pins raised to mcp-workbay-orchestrator 0.2.11 / mcp-workbay-handoff 0.2.9 / workbay-system 0.3.17; no behavior change in this package).

## [0.3.17] — 2026-07-14

### Changed
- Raise the `workbay-system` floor to 0.3.16 so a bootstrap install hoists the `harness-protocol.yaml` carrying `orchestrator.host_memory.os_reserve_remote_api_gib`.

## [0.3.16] — 2026-07-14

### Changed
- Raise the `workbay-system` pin floor to `>=0.3.15,<0.4.0` (stale tool/command surfaces retired from canonical skill bodies, internal).

## [0.3.15] — 2026-07-14

### Changed
- `apply-hooks` no longer crashes on a removed dispatch argument.
- Embedding-provision diagnostic distinguishes a genuinely missing package or `[embeddings]` extra from a code/version regression (a renamed or removed export), so the remedy is actionable instead of pointing at a reinstall that never fixes it; the `[embeddings]`-extra remedy no longer cites a nonexistent recipe.
- Add a `check-bootstrap` make target for the bootstrap suite.

## [0.3.14] — 2026-07-13

### Changed
- Reinject provisioning-coverage: cause-specific doctor readiness remedies + operator doc for the opt-in `[embeddings]` extra.
- Fresh-install suite-health fixes (0113); `version_of` drift probe now leaves `sys.path` unchanged (snapshot/restore around `exec_module`).
- Drop an internal plan token from the README that rode into wheel METADATA (privacy packaging gate).

## [0.3.13] — 2026-07-12

### Changed
- Suite-health hardening (0113): contain the `version_of_drift` `sys.path` mutation in `_import_installed_drift` via snapshot/restore around `exec_module` so the doctor probe leaves `sys.path` unchanged (regression test asserts no mutation).
- conftest autouse guard re-binds `workbay_bootstrap` submodules onto the parent package to contain dual-package cross-file monkeypatch pollution (green-in-isolation / red-in-full-suite ordering leak).
- `release_prepare` rederive-install-state docstring refresh (handoff self-heals via sync-on-run; gate/pyenv retired); embeddings gate-status contract + env-leak hermeticity tests; golden recapture.

## [0.3.12] — 2026-07-11

### Changed
- Portable self-host `provenance_key` so bootstrap provenance no longer pins to an absolute checkout path.
- Decouple the telemetry gate from the full doctor run and de-mutate the coherence check.
- Regenerate `.workbay-bootstrap.json` with portable provenance.

## [0.3.11] — 2026-07-10

### Changed
- Overlay-fence reconciliation: the fence reads lifecycle provenance, self-heal hook delivery moves to the `scripts/hooks` seam, and doctor covers Claude settings + dual-register residuals.
- Wire `ensure-agent-surfaces` opt-ins and `{{consumer_root}}` coherence into InstallRequest defaults.
- De-advisory telemetry freshness gate.

## [0.3.10] — 2026-07-08

### Changed
- Re-sync workbay-system floor to 0.3.8 (implementation note coordinated bump).

## [0.3.9] — 2026-07-08

### Changed
- Bump the `workbay-system` dependency floor to `>=0.3.7,<0.4.0` for the coordinated release.

## [0.3.8] — 2026-07-08

### Changed
- Git-write containment (implementation note): guarded git-write choke point, fail-closed guard, cwd binding, mv/timeout/multivar handling, and worktree-config migration hygiene.
- Generated-surface provenance (implementation note): `SURFACE_PROVENANCE` table, ignore entries derived from provenance, and doctor verifying derived surfaces by kind.

## [0.3.7] — 2026-07-07

### Changed
- Git-only package-mode installs now provision the MCP tools and the codex bridge, with a cwd-independent shim launcher.
- Restore profile-all execution for `mcp_servers=None` installs.
- Derive the managed-link clone path; drop the legacy hidden-state-directory guard branch.

## [0.3.6] — 2026-07-04

### Changed
- Carry the `workbay-protocol>=0.2.1` and `workbay-system>=0.3.4` floors for the refreshed git-only stack and current bootstrap install metadata.

## [0.3.5] — 2026-07-01

### Fixed
- `make dogfood` self-target (dogfood into the monorepo) no longer corrupts the shared workspace `.venv` or crashes.

## [0.3.4] — 2026-07-01

### Changed
- Carry the updated `workbay-system>=0.3.3` dependency floor (workbay-system 0.3.3 compaction single-source release).

## [0.3.3] — 2026-06-30

### Fixed
- Git-only install regression (internal): pass `--no-sources` on the remaining git_overlay clone `uv` sites the internal fix missed — the generated serve launcher (`_local_mcp_spec`), the env presync (`_presync_local_mcp_envs`), and the handoff retry command (`_build_local_handoff_retry_cmd`) — so the clone's workspace-source pins no longer break MCP provisioning / init-state.

### Changed
- Raise the `workbay-system` dependency floor to `>=0.3.2`.
