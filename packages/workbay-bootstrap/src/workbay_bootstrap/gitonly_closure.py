...
from workbay_bootstrap._declared_fastmcp import (
    DECLARED_FASTMCP_REQUIREMENT_UNREADABLE,
    DeclaredFastmcpRequirementError,
    require_declared_fastmcp_requirement as _require_declared_fastmcp_requirement_impl,
)
...
def _require_declared_fastmcp_requirement(
    ...
    of the exact same member-spec / git-ref / metadata / checkout-walk
    resolution order that ``_declared_fastmcp.resolve_declared_fastmcp_requirement``
    already owns (REF-09 — one fact, one owner). Delegate to the shared,
    exported resolver instead so a future new pin source only has to be added
    once.
    """
    return _require_declared_fastmcp_requirement_impl(
        member_specs, start=Path(__file__).resolve().parent
    )
...
def build_uv_tool_install_argv(
    *,
    ...
    if force:
        argv.append("--force")
    # Carry the declared FastMCP specifier on the install command itself.
    # ``--with`` (not ``--constraint``) is the existing extra-requirement
    # mechanism on this argv; a constraint file would hide the pin off-argv.
    argv.extend(["--with", _require_declared_fastmcp_requirement(member_specs)])
