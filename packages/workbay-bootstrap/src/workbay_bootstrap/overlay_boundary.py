...
def validate_tracked_overlay_boundary(target: Path) -> list[dict[str, str]]:
    ...
    for rel in TRACKED_OVERLAY_SOURCE_PATHS:
        # Only fire when the path is actually git-tracked. In consumer trees
        # these paths are materialized and correctly gitignored but untracked;
        # the real contradiction is tracked AND ignored (self-host misuse).
        if rel in leaking and _git_path_is_tracked(target, rel):
            findings.append(
                {
