...
def _import_installed_drift(
    ...
    module spec: no ``sys.path`` mutation, and a stray ``version_of_drift``
    elsewhere on ``sys.path`` cannot shadow the real one. Returns ``None`` when
    the detector is unavailable so doctor can degrade instead of crashing.
    """
    if target is None:
        return None
    try:
