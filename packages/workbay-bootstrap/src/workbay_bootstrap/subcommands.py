...
def _repair_deferred_install_steps(
    ...
    if "gitonly_mcp_tools" in deferred:
        # Best-effort twin of prewarm: install_plan defers offline/timeout on
        # the shim path; repair must clear the latch and retry so a deferred
        # receipt is recoverable once connectivity returns.
        import subprocess
        ...
        if member_specs:
            ...
            try:
                ...
                # Still offline / still failing: leave the deferred receipt.
                installed = []
        if installed:
            for step in steps:
...
def _doctor_embeddings_runtime_drift(target: Path) -> list[Finding]:
    ...
    # Intentional opt-out: stay silent (severity warning is forbidden here).
    # Gate-file I/O can raise PermissionError / UnicodeDecodeError / OSError
    # when .workbay/embedding.env is unreadable — degrade to [] so doctor()
    # still completes (SECD-05 fail-safe; no false crash).
    try:
...
def repair(
    *,
    ...
    receipt_repairs = list(mode_repairs) + list(presync_repairs)
