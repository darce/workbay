...
def _embeddings_facet_preamble(target: Path) -> Path | None:
    """Return resolved ``target`` when embeddings doctor facets should run.

    Shared by runtime-drift and backfill-gap so the gate/declare short-circuit
    cannot drift. ``None`` means stay silent (opt-out, undeclared extra,
    unreadable gate, or import failure).
    """
    target = Path(target).resolve()
    try:
...
def _doctor_embeddings_runtime_drift(target: Path) -> list[Finding]:
    ...
    """
    target = _embeddings_facet_preamble(target)
    if target is None:
        return []
