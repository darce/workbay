...
def _doctor_local_source_pypi_staleness(
    ...
    if not check_pypi:
        return []
    stack_distribution = str(manifest.get("stack_distribution") or "workbay-stack")
    try:
...
def _doctor_local_mcp_console_scripts(
    ...
    """
    from workbay_bootstrap.install import _LOCAL_MCP_SERVERS, _MCP_LAUNCH_SHIM
...
def _managed_stop_adapter_drifted(
    ...
    """
    from workbay_bootstrap.install import _render_template
...
def update(
    ...
    * ``package`` — re-installs from the **currently installed**
      ...
      ``pip install --upgrade workbay-stack``); ``remote_ref`` is invalid
    ...
    if source_kind == "package":
        if remote_ref is not None:
            raise ValueError(
                ...
                "wheel (pip install --upgrade workbay-stack) and re-run "
...
def repair(
    *,
    ...
    if _is_selfhost_worktree(target):
        ...
        if receipt.get("ok"):
            return {"repaired": [finding], "skipped": []}
        return {"repaired": [], "skipped": [finding]}
