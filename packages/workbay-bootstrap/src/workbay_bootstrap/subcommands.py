...
def _doctor_embedding_artifacts(target: Path) -> list[Finding]:
    ...
    # A worktree that has deliberately opted out (kill-switch) is not "drifted":
    # provisioning was intentionally skipped, so stay silent rather than nag with
    # the unprovisioned info note (consistent with maybe_provision_embeddings).
    if not env_path.is_file() and embeddings_disabled():
        return findings
