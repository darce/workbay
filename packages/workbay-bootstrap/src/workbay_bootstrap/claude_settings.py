...
def adopt_payload_settings_hooks(
    ...
    *,
    ...
    if (
        dest.is_file()
        and not _sidecar_matches_payload(dest, payload)
        and not _settings_hooks_receipt_known(target)
    ):
        return []
    return [
        write_json(
            dest,
            payload,
            manifest_path=CLAUDE_SETTINGS_HOOKS_PATH.as_posix(),
        )
    ]
...
def write_wholesale_claude_settings(
    ...
    *,
    ...
    entries.extend(adopt_payload_settings_hooks(target, clone, write_json=write_json))
    return entries
