...
- **resolve-every-script** — every ``command`` path named by a rendered hook
  ...
  session). ERROR — except on an *unmanaged* config (the same predicate that
  fires ``orphan_hook_config``): workbay disowns that file, so refusing on it
  leaves no in-band repair path. There the finding is still emitted (audited,
  never dropped) but downgraded to WARNING with the adopt/delete remedy
  (upstream REQUEST 2026-08-23).
...
HOOK_CONFIG_CANDIDATE_GLOBS: tuple[str, ...] = (
    ...
    # internal: Claude project settings carry SessionStart/etc. The
    # Defect 2 unresolved ``.claude/hooks/ensure-agent-surfaces.sh`` path lived
    # here and was invisible to the resolve gate until this glob landed.
    ".claude/settings*.json",
)

# Wholesale-managed Claude settings (``settings.json``, ``settings.hooks.json``)
# resolve *every* hook command. ``settings.local.json`` is user-owned — resolve
# only entries stamped ``_managed_by: workbay-bootstrap`` ([OBS-04]).
_MANAGED_BY_VALUE = "workbay-bootstrap"
...
# The portable ``{{consumer_root}}/`` template token (claude_settings
# ``_CONSUMER_ROOT_TOKEN_RE``) also substitutes to the workspace root, i.e. the
# assessment target. Slice-4 taught coherence to enumerate ``settings.local.json``
# (where bootstrap-managed compact-session/reinject commands carry this token);
# strip it here too so those managed commands resolve instead of false-firing
# ``hook_command_unresolved`` on a literal ``{{consumer_root}}`` path segment.
_CONSUMER_ROOT_ANCHOR_RE = re.compile(r"^\{\{consumer_root\}\}/")
...
def assess_hook_coherence(
    ...
    *,
    ...
    if receipt is not None:
        ...
        loaded = _load_receipt(target)
        # ``None`` means the receipt file is missing or unreadable/corrupt --
        # distinct from "parsed fine and does not mention this path". Only a
        # successfully parsed receipt is evidence that workbay disowns a
        # config; absence of evidence of disowning is not disowning.
        receipt_available = loaded is not None
        receipt = loaded if loaded is not None else {}
    ...
    for config in configs:
        ...
        resolve_findings = _resolve_every_script(target, config, rel)
        # User-owned Claude local settings are intentionally outside the
        # receipt / renderer map; do not orphan-warn them ([OBS-04]). Managed-
        # stamped entries inside them are still resolve-checked at ERROR.
        if _is_user_owned_claude_settings(rel):
            findings.extend(resolve_findings)
            continue
        if _is_receipt_known(rel, known):
            findings.extend(resolve_findings)
        else:
            # Unmanaged config -- the orphan_hook_config predicate. workbay
            # disowns the file (update()/doctor never refresh it), so an
            # unresolved command in it must not fail-close install/update:
            # downgrade to warning and name the in-band remedy, keeping the
            # audit content intact (upstream REQUEST 2026-08-23).
            #
            # The downgrade requires a PARSED receipt that omits the config
            # (a true orphan). A missing/corrupt receipt on an installed
            # target proves nothing about disowning, so the finding keeps
            # error severity there -- doctor/CI must fail-close rather than
            # let the session fail-close at runtime (review r1 W1).
            if receipt_available:
                findings.extend(
                    _downgrade_unmanaged_unresolved(finding)
                    for finding in resolve_findings
                )
            else:
                findings.extend(resolve_findings)
            findings.append(
    ...
    findings.extend(_owner_repo_bare_gate(target))
    return findings
...
def _load_receipt(target: Path) -> dict[str, object] | None:
    """Parsed bootstrap receipt, or ``None`` when the file is missing or
    unreadable/corrupt. Callers must treat ``None`` as "no receipt evidence
    either way" -- never as an empty (config-disowning) receipt."""
...
# The concrete remedy appended to a downgraded unmanaged-config finding.
# ``workbay install`` adopts the payload copy of the sidecar (see
# ``claude_settings.adopt_payload_settings_hooks``), so re-running install is a
# real in-band repair, not aspiration.
_UNMANAGED_UNRESOLVED_REMEDY = (
    " This config is not receipt-managed (see orphan_hook_config), so the "
    "running harness does not load a workbay-owned copy of it: re-run "
    "`workbay install` to adopt the managed copy, or delete the stale file."
)


def _downgrade_unmanaged_unresolved(finding: CoherenceFinding) -> CoherenceFinding:
    """Project a ``hook_command_unresolved`` finding on an unmanaged config
    down to ``warning``, appending the adopt/delete remedy. Other kinds pass
    through unchanged (defensive; the resolve gate emits only this kind)."""
    if finding.kind != "hook_command_unresolved":
        return finding
    return CoherenceFinding(
        kind=finding.kind,
        severity="warning",
        path=finding.path,
        detail=finding.detail + _UNMANAGED_UNRESOLVED_REMEDY,
    )
...
def _is_user_owned_claude_settings(rel: str) -> bool:
    """True for ``.claude/settings.local.json`` (user-owned residual/adapter file)."""
    return Path(rel).name == "settings.local.json"


def _filter_managed_hook_entries(data: object) -> object:
    """Keep only array entries stamped ``_managed_by: workbay-bootstrap``.

    Used for user-owned ``settings.local.json`` so consumer-authored hooks
    (no stamp) are never resolve-checked ([OBS-04]). Nested Claude shape::

        {"hooks": {"SessionStart": [{"_managed_by": "...", "hooks": [...]}]}}

    Flat harness shapes with a top-level ``command`` are kept only when the
    entry itself carries the managed stamp.
    """
    ...
    if not isinstance(hooks, dict):
        return data
    filtered_stages: dict[str, list[object]] = {}
    for stage, entries in hooks.items():
        if not isinstance(entries, list):
            continue
        kept: list[object] = []
        for entry in entries:
            if isinstance(entry, dict) and entry.get("_managed_by") == _MANAGED_BY_VALUE:
                kept.append(entry)
        if kept:
            filtered_stages[stage] = kept
    out = dict(data)
    out["hooks"] = filtered_stages
    return out
...
def _resolve_every_script(
    ...
    # internal managed-scoping: wholesale Claude settings → all
    # commands; user-owned settings.local.json → managed entries only.
    # Other harness configs are unchanged (full resolve).
    if _is_user_owned_claude_settings(rel):
        data = _filter_managed_hook_entries(data)
    findings: list[CoherenceFinding] = []
...
def _command_path_tokens(command: str) -> list[str]:
    ...
    for word in words:
        ...
        word = _CONSUMER_ROOT_ANCHOR_RE.sub("", word)
...
def _provenance_key(target: Path, path: Path, package_root: Path | None) -> str | None:
    ...
    if path.is_symlink():
        ...
        if text.endswith("/" + rel):
            text = text[: -(len(rel) + 1)]
        # Portability: a mount into the checkout's OWN tree (the self-host
        # in-tree payload) is checkout-relative by nature. Recording its
        # absolute path poisons the TRACKED manifest for every other checkout —
        # a worktree dogfood-commit stamps that worktree's path onto main, so a
        # later checkout (or the primary, once the worktree is torn down) reads
        # a dead path. Emit a path relative to `target` so the receipt is
        # checkout-independent; the live cross-surface skew comparison in
        # `_same_snapshot_gate` stays consistent (one relative root per
        # checkout). Genuinely-external mounts keep the absolute form.
        try:
        ...
        if text == target_root:
            text = "."
        elif text.startswith(target_root + "/"):
            text = text[len(target_root) + 1 :]
        return f"link:{text}"
    if not path.exists():
        return None
...
# --- owner-repo bare probe (internal) -------------------------------


def _resolve_owner_repo_root(target: Path) -> Path | None:
    """Return the primary/owner git root for ``target`` (parent of common-dir)."""
    from workbay_bootstrap.external import run_external
    ...
    try:
        ...
        # For a normal repo/worktree the common dir is `<root>/.git`, so the owner
        # root is its parent. For a CANONICAL bare repo (`project.git` is itself the
        # gitdir, no `.git` subdir) rev-parse returns that bare dir directly — taking
        # `.parent` would overshoot to the containing directory and read bareness /
        # the manifest from the wrong (possibly foreign) repo.
        if common.name == ".git":
            return common.parent
        return common
    except (OSError, subprocess.CalledProcessError, subprocess.TimeoutExpired):
        # ``run_external`` raises ``ExternalCallTimeout`` (a ``TimeoutExpired``
        # subclass) on a hung git; fail open so ``doctor`` degrades, never crashes.
        return None


def _is_bare_repository(repo_root: Path) -> bool | None:
    from workbay_bootstrap.external import run_external
    ...
    try:
        ...
        # See ``_resolve_owner_repo_root``: a hung git raises ``TimeoutExpired``.
        return None


def _target_relative_path(target: Path, path: Path) -> str:
    try:
...
# Every bootstrap install kind writes its manifest at a WORKING-TREE root and
# checks a tree out there — bootstrap never installs into a bare repo. So any of
# these markers at a now-bare owner root is the contradiction (not just the
# self-host ``worktree`` kind; a consumer ``git_overlay`` install whose root went
# bare is the dominant field case and equally impossible for bootstrap to produce).
_WORKING_TREE_INSTALL_KINDS = frozenset({"worktree", "git_overlay", "package"})


def _records_worktree_install_at(owner: Path) -> bool:
    """True when the owner-root manifest records a working-tree install kind."""
    manifest = _load_receipt(owner) or {}
    return str(manifest.get("source_kind") or "") in _WORKING_TREE_INSTALL_KINDS


def _owner_repo_bare_gate(target: Path) -> list[CoherenceFinding]:
    """Detect a bare owner/primary repo (implementation note, ``owner_repo_bare``).

    Bare-primary + all-linked-worktrees is legitimate git, so bareness alone is
    a warning with a runbook pointer. Severity **error** only when the owner
    is bare AND the bootstrap manifest at that root records a working-tree install
    (``source_kind`` in worktree/git_overlay/package) — a contradiction bootstrap
    never produces.
    """
    owner = _resolve_owner_repo_root(target)
    if owner is None:
        return []
    if _is_bare_repository(owner) is not True:
        return []
    rel = _target_relative_path(target, owner)
    if _records_worktree_install_at(owner):
        return [
            CoherenceFinding(
                kind="owner_repo_bare",
                severity="error",
                path=rel,
                detail=(
                    "owner/primary repo reports bare while "
                    f"{BOOTSTRAP_MANIFEST_NAME} at that root records a "
                    "working-tree install (source_kind in "
                    "worktree/git_overlay/package); bootstrap never produces this "
                    "state — inspect for a core.bare=true leak in the shared git "
                    "config and restore the owner repo per lifecycle-recovery.md "
                    "(Restore: owner repo wrongly bare)."
                ),
            )
        ]
    return [
        CoherenceFinding(
            ...
            path=rel,
            detail=(
                "owner/primary repo reports bare; a bare primary with "
                "linked worktrees can be legitimate, but if git operations "
                "fail with 'this operation must be run in a work tree' restore "
                "the owner repo per lifecycle-recovery.md "
                "(Restore: owner repo wrongly bare)."
            ),
        )
    ]
