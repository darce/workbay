...
def _advise_optional_mcp_availability(
    ...
    for server_id in configured_optional:
        ...
        try:
            ...
            proc = run_external(
                [sys.executable, str(shim), "--check", server_id],
                call_class="probe",
                check=False,
                capture_output=True,
                text=True,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    _add_hooks_profile(
        p_install, default="none", choices=("none", "repo", "all")
    )
    p_install.add_argument(
    ...
    p_apply_hooks.add_argument(
        ...
        default=None,
        help=(
            "Consumer repository root that was previously installed "
            "(defaults to cwd)."
        ),
    )
    _add_hooks_profile(
        p_apply_hooks, default="none", choices=("none", "repo", "all")
    )
    ...
    repair_remote.add_argument(
        ...
        action="store_true",
        help=(
            "Flip the ledger execution_mode to local_ok (no probe). "
            "Use to undo a prior remote_only policy."
        ),
    )
    p_repair.add_argument(
        "--force-dirty",
    ...
    p_repair.add_argument(
        ...
        action="store_true",
        help=(
            "Replace dangling managed symlinks with payload copies. "
            "Local-precedence surfaces are left untouched."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "install":
        ...
        try:
            manifest = install(
                ...
                install_claude_stop_hook_local=hook_kwargs.get(
                    "install_claude_stop_hook_local", False
                ),
                install_codex_stop_hook=hook_kwargs.get(
                    "install_codex_stop_hook", False
                ),
                install_vscode_stop_hook=hook_kwargs.get(
                    "install_vscode_stop_hook", False
                ),
                install_grok_stop_hook=hook_kwargs.get(
                    "install_grok_stop_hook", False
                ),
                install_claude_reinject_hook_local=hook_kwargs.get(
                ...
                execution_mode=(
                    "remote_only" if args.with_remote else "local_ok"
                ),
                embeddings_mode=(
                    "verified"
                    if args.with_embeddings
                    else ("disabled" if args.no_embeddings else "unspecified")
                ),
            )
    ...
    if args.command == "mcp-sync":
        ...
        _advise_optional_mcp_availability(args.target, servers)
