...
from workbay_system.review_surface_ownership import (
    Conflict as ReviewSurfaceConflict,
)
from workbay_system.review_surface_ownership import (
    Unknown as ReviewSurfaceUnknown,
)
from workbay_system.review_surface_ownership import (
    _payload_identical as _payload_identical,
)
from workbay_system.review_surface_ownership import (
    classify as classify_review_surfaces,
)
from workbay_system.review_surface_ownership import (
    read_recorded_paths as read_review_surface_receipt,
)
...
# Sole derivation of the clone subpath as a posix relpath (``.workbay/remote``).
# Single-sourced from CLONE_SUBDIR so a runtime-root rename is a one-line codemod;
# re-exported here (next to CLONE_SUBDIR) so coherence.py and subcommands.py share
# one derivation site instead of each hardcoding the literal.
CLONE_RELPATH = "/".join(CLONE_SUBDIR)
...
def _build_install_manifest(
    *,
    ...
    stack_source: StackSource | None = None,
    profile: str,
    ...
    ``execution_mode`` is written only when ``"remote_only"`` so default
    git_overlay manifests stay byte-identical (schema defaults cover absence).
    ``embeddings_mode`` is written only when not ``"unspecified"`` for the
    same byte-compat reason (``verified`` / ``disabled`` are explicit).
    """
    ...
    if source_kind == "package":
        ...
        manifest["gitonly_closure_url"] = remote_url
        manifest["gitonly_closure_ref"] = remote_ref
        ...
        if stack_distribution is not None:
            ...
            # Source provenance (REQUEST 2026-08-23): which ref/SHA the stack
            # packages were resolved from, so "is this really vX.Y.Z?" is
            # answerable from the ledger alone. Omitted (None) keeps older
            # manifest shapes byte-identical.
            if stack_source is not None:
                manifest["stack_source"] = stack_source
    ...
    if execution_mode == "remote_only":
        manifest["execution_mode"] = "remote_only"
    if embeddings_mode != "unspecified":
        manifest["embeddings_mode"] = embeddings_mode
    return manifest


_RUN_LOCAL_CONFIG_ACTIONS = frozenset({"already_present", "unchanged"})


def _stabilize_ledger_config_actions(target: Path, configs: list[dict[str, str]]) -> list[dict[str, str]]:
    """Keep durable config actions across a no-op reinstall.

    ``created`` vs ``already_present`` is a run-local outcome, not an install
    FACT. Reinstalling an unchanged tree would otherwise rewrite the tracked
    ledger and train operators to commit noise (RLSE-11).
    """
    path = target / BOOTSTRAP_MANIFEST_NAME
    if not path.is_file():
        return configs
    try:
    ...
    for entry in existing.get("configs") or []:
        if not isinstance(entry, dict):
            continue
        entry_path = entry.get("path")
        action = entry.get("action")
        if isinstance(entry_path, str) and isinstance(action, str):
            prior_actions[entry_path] = action
    ...
    for entry in configs:
        entry_path = entry.get("path")
        action = entry.get("action")
        if entry_path in prior_actions and action in _RUN_LOCAL_CONFIG_ACTIONS and prior_actions[entry_path] != action:
            stabilized.append({**entry, "action": prior_actions[entry_path]})
        else:
            stabilized.append(entry)
    return stabilized


_MANIFEST_REBUILT_KEYS = frozenset(
    {
        "schema_version",
        "source_kind",
        "remote_url",
...
_EMBEDDINGS_OPERATOR_POLICY_KEY = "embeddings_operator_policy"
_EMBEDDINGS_INFERENCE_RECEIPT_KEY = "embeddings_inference_receipt"


def _validate_embeddings_operator_policy(value: object) -> object:
    """Shape-validate a persisted operator policy; refuse rather than drop it."""
    try:
        from workbay_protocol.bootstrap import EmbeddingsOperatorPolicy
    except ImportError:
        if not isinstance(value, dict):
            raise BootstrapManifestValidationError(
                f"refusing to write {BOOTSTRAP_MANIFEST_NAME}: {_EMBEDDINGS_OPERATOR_POLICY_KEY} must be an object"
            )
        return value
    try:
        EmbeddingsOperatorPolicy.model_validate(value)
    except Exception as exc:  # noqa: BLE001
        raise BootstrapManifestValidationError(
            f"refusing to write {BOOTSTRAP_MANIFEST_NAME}: {_EMBEDDINGS_OPERATOR_POLICY_KEY} is invalid: {exc}"
        ) from exc
    return value


def _validate_embeddings_inference_receipt(value: object) -> object:
    """Shape-validate a persisted inference receipt; refuse rather than drop it."""
    try:
        from workbay_protocol.bootstrap import EmbeddingsInferenceReceipt
    except ImportError:
        if not isinstance(value, dict):
            raise BootstrapManifestValidationError(
                f"refusing to write {BOOTSTRAP_MANIFEST_NAME}: {_EMBEDDINGS_INFERENCE_RECEIPT_KEY} must be an object"
            )
        return value
    try:
        EmbeddingsInferenceReceipt.model_validate(value)
    except Exception as exc:  # noqa: BLE001
        raise BootstrapManifestValidationError(
            f"refusing to write {BOOTSTRAP_MANIFEST_NAME}: {_EMBEDDINGS_INFERENCE_RECEIPT_KEY} is invalid: {exc}"
        ) from exc
    return value


def _monotonic_policy_revision(existing: object, incoming: object) -> None:
    """Keep ``policy_revision`` from moving backwards across a rewrite."""
    if not isinstance(existing, dict) or not isinstance(incoming, dict):
        return
    old_rev = existing.get("policy_revision")
    new_rev = incoming.get("policy_revision")
    if isinstance(old_rev, int) and (not isinstance(new_rev, int) or new_rev < old_rev):
        incoming["policy_revision"] = old_rev


def _preserve_existing_manifest_keys(target: Path, manifest: dict[str, object]) -> None:
    """Carry operator policy, inference receipt, and unknown ledger keys forward.

    ``_build_install_manifest`` rebuilds a known key set and historically
    dropped ``embeddings_operator_policy`` / ``embeddings_inference_receipt``
    plus any extra top-level keys. Reinstall and update must not clobber an
    explicit operator opt-out (NW-0227-01R2RV-001).
    """
    path = target / BOOTSTRAP_MANIFEST_NAME
    if not path.is_file():
        return
    ...
    existing_policy = existing.get(_EMBEDDINGS_OPERATOR_POLICY_KEY)
    if existing_policy is not None:
        validated_policy = _validate_embeddings_operator_policy(existing_policy)
        if _EMBEDDINGS_OPERATOR_POLICY_KEY not in manifest:
            manifest[_EMBEDDINGS_OPERATOR_POLICY_KEY] = validated_policy
        else:
            _monotonic_policy_revision(validated_policy, manifest[_EMBEDDINGS_OPERATOR_POLICY_KEY])

    existing_receipt = existing.get(_EMBEDDINGS_INFERENCE_RECEIPT_KEY)
    if existing_receipt is not None and _EMBEDDINGS_INFERENCE_RECEIPT_KEY not in manifest:
        manifest[_EMBEDDINGS_INFERENCE_RECEIPT_KEY] = _validate_embeddings_inference_receipt(existing_receipt)

    for key, value in existing.items():
        if key in manifest or key in _MANIFEST_REBUILT_KEYS:
            continue
        if key in {_EMBEDDINGS_OPERATOR_POLICY_KEY, _EMBEDDINGS_INFERENCE_RECEIPT_KEY}:
            continue
        manifest[key] = value


_CODEMAP_MODE_KEY = "codemap_mode"
_CODEMAP_MODE_DEFAULT = "available"


def _default_codemap_mode(manifest: dict[str, object]) -> None:
    """Fill ``codemap_mode`` with advisory ``available`` when none is explicit.

    Absent and JSON null are not operator choices; the closed codemap-first
    hook maps those to ``policy_missing`` / ``broken_enforcement``. An
    explicit value — valid or invalid — is left untouched so reinstall and
    update cannot clobber ``off``/``enforced`` or silently rewrite a corrupt
    ledger that the reader already reports as ``invalid_value``.
    """
    if manifest.get(_CODEMAP_MODE_KEY) is None:
        manifest[_CODEMAP_MODE_KEY] = _CODEMAP_MODE_DEFAULT


def _persist_install_manifest_bytes(target: Path, manifest: dict[str, object]) -> None:
    (target / BOOTSTRAP_MANIFEST_NAME).write_text(json.dumps(manifest, indent=2) + "\n")


def _write_install_manifest(target: Path, manifest: dict[str, object]) -> None:
    """Persist a success manifest or an intentionally incomplete abort snapshot."""
    from workbay_bootstrap.embedding_provision import _bootstrap_writer_lock

    with _bootstrap_writer_lock(target):
        _preserve_existing_manifest_keys(target, manifest)
        _default_codemap_mode(manifest)
        _persist_install_manifest_bytes(target, manifest)
...
def _finalize_install_manifest(
    ...
    *,
    ...
    with _bootstrap_writer_lock(target):
        _preserve_existing_manifest_keys(target, manifest)
        _default_codemap_mode(manifest)
        # Write-side gate first so a bad ``stack_source`` is refused with the
        # field-level message (not a generic pydantic wrap) BEFORE the ledger is
        # written. The production caller passes ``_stack_source()``, which can
        # only emit legal values, but ``stack_source`` is a caller-supplied
        # parameter of ``_build_install_manifest`` -- so this is reachable (and
        # tested) that way. The return is stored so the narrowed ``StackSource``
        # is what actually gets serialized.
        if "stack_source" in manifest:
            manifest["stack_source"] = validate_stack_source(manifest["stack_source"])
        try:
            ...
            raise BootstrapManifestValidationError(
                f"refusing to write {BOOTSTRAP_MANIFEST_NAME}: workbay_protocol.BootstrapManifest validation failed: {exc}"
            ) from exc

        _enforce_stack_member_convergence(
            manifest,
...
# Thin view over ``surfaces.SURFACE_PROVENANCE`` (implementation note).
SHARED_SURFACES: tuple[str, ...] = surfaces_for_kind("shared")
...
# Compiled-bytecode caches and OS cruft never materialize as per-child surface
# symlinks. Before implementation note no carved surface shipped a Python module, so a
# ``__pycache__`` child could not arise; the check-all engine
# (``scripts/workbay/check_all_cache.py``) is the first, so a stale bytecode
# cache in a dirty source tree would otherwise be enumerated as a bogus
# per-child surface and leak into a consumer's install manifest. ``.DS_Store``
# is the same leak class on macOS (a Finder visit to a carved surface dir mints
# one). This denylist is folded into :func:`_excluded_carved_children`, the
# single exclusion source, so apply and the drift guard stay in agreement.
CARVED_CHILD_IGNORED_NAMES: frozenset[str] = frozenset({"__pycache__", ".DS_Store"})
...
# marketplace pin.
# Thin view over ``surfaces.SURFACE_PROVENANCE`` (implementation note).
GENERATED_SURFACES: tuple[str, ...] = surfaces_for_kind("generated")

from workbay_bootstrap.harnesses import (  # noqa: E402
...
# Thin view over ``surfaces.SURFACE_PROVENANCE`` (implementation note).
GENERATOR_LEDGER_SURFACES: tuple[str, ...] = surfaces_for_kind("generator_input")
...
LIFECYCLE_HOISTS: tuple[tuple[str, str], ...] = (
    ...
    ("scripts/workbay_lifecycle", "scripts/workbay_lifecycle"),
...
# Retired consumer-overlay dirs from the PRE-rename lifecycle profile
# (internal). Before the namespace collapse, the lifecycle runner
# hoisted to ``scripts/workbay/lifecycle`` and the evals harness lived at
# ``scripts/workbay/evals`` — both real directories under ``scripts/workbay``
# that made ``workbay`` an importable package (``workbay.lifecycle`` /
# ``workbay.evals``). The rename hoists the runner to the top-level
# ``scripts/workbay_lifecycle`` instead. A copy/copytree refresh never removes
# the old trees, so an overlay upgrading from a pre-rename install would keep
# both — recreating the dual-occupant namespace collision this branch exists to
# eliminate. On every lifecycle-bearing (re)install these retired dirs are
# pruned so no overlay ever holds both. Only a real (non-symlink) directory is
# removed; sibling shims (``scripts/workbay/update.sh``, ``git-plan-cat.sh``)
# and any consumer-owned symlink are never touched.
RETIRED_LIFECYCLE_HOIST_DESTS: tuple[str, ...] = (
    "scripts/workbay/lifecycle",
    "scripts/workbay/evals",
)
...
PROFILE_ALL = "all"
SUPPORTED_PROFILES: frozenset[str] = frozenset({PROFILE_MINIMAL, PROFILE_LIFECYCLE, PROFILE_ALL})
...
def _build_local_handoff_retry_cmd(target: Path, cmd: list[str]) -> list[str] | None:
    ...
    if cmd[0] == "uvx":
        ...
        len(cmd) >= 5
        and cmd[0] == _MCP_LAUNCH_WRAP_CMD
        and cmd[1] == "-c"
        and "mcp_launch.py" in cmd[2]
        and cmd[4].startswith("workbay-handoff")
    ):
        # cwd-independent ``sh -c '… mcp_launch.py … "$@"' sh <server> [args…]``:
        # ``sh`` is $0, the server id is $1 (cmd[4]), forwarded serve args follow.
        tail = cmd[5:]
    elif (
        len(cmd) >= 3
        ...
        # Legacy cwd-relative shim form (installs predating the sh wrapper).
        tail = cmd[3:]
    else:
        return None
...
# cwd-independent managed launcher (internal S3). A bare
# ``python3 scripts/hooks/mcp_launch.py <server>`` resolves that RELATIVE script
# path against the harness's launch cwd; when the server is started from any cwd
# other than the repo root (a linked worktree, a subdir session) python3 cannot
# find the shim and the server never starts. Wrap the launch in ``sh -c`` that
# resolves the shim against the git repo root at launch — portable (no
# machine-absolute path baked into the managed ``.mcp.json``, so a bootstrap
# resync does not revert it). ``sh`` is $0 and the server id is the sole trailing
# positional forwarded via ``"$@"``: recognizers read it as ``args[-1]`` and the
# shim path stays embedded in ``args[1]``. Must stay byte-identical to the
# ``mcp_servers.yaml`` SSOT (the generated ``_mcp_pins.py`` is rendered from it).
_MCP_LAUNCH_WRAP_CMD = "sh"
# Resolve the shim against the git repo root, falling back to the launch cwd
# (``.``) when git can't (a non-git consumer install, or git absent). This
# mirrors the shim's own ``_git_repo_root() or os.getcwd()`` root resolution, so
# a git-repo install becomes cwd-independent while a non-git install keeps the
# prior cwd-relative behavior (the harness launches from the repo/target root).
_MCP_LAUNCH_WRAP_SCRIPT = (
    f'root="$(git rev-parse --show-toplevel 2>/dev/null || echo .)"; exec python3 "$root/{_MCP_LAUNCH_SHIM}" "$@"'
)


def _mcp_launch_spec_args(server_id: str) -> list[str]:
    """Args for the cwd-independent ``sh`` launcher of one managed MCP server."""
    return ["-c", _MCP_LAUNCH_WRAP_SCRIPT, "sh", server_id]


def _launch_shim_server_id(command: object, args: object) -> str | None:
    """Return the server id when ``(command, args)`` is the managed launch shim.

    Recognizes the current cwd-independent form
    (``sh -c '… mcp_launch.py … "$@"' sh <server>``) and the legacy
    ``python3 scripts/hooks/mcp_launch.py <server>`` form (installs written
    before the cwd-independence fix), so reconcile/presync classify both as the
    managed shim rather than an opaque third-party launcher. Returns ``None`` for
    any other spec (e.g. a published ``uvx`` pin or a ``uv run`` clone launcher).
    """
    if not isinstance(args, list) or not all(isinstance(a, str) for a in args):
        return None
    if command == _MCP_LAUNCH_WRAP_CMD and len(args) >= 4 and args[0] == "-c" and _MCP_LAUNCH_SHIM in args[1]:
        return args[-1]
...
_LOCAL_MCP_PROJECTS: dict[str, str] = {server_id: relpath for server_id, relpath, _console in _LOCAL_MCP_SERVERS}


def _local_mcp_spec(server_id: str, project: str, cli: str) -> dict[str, Any]:
    """Launch spec for one local MCP server.

    In-tree (``packages/<pkg>``) routes through the stdlib shim via the
    cwd-independent ``sh -c`` wrapper (``_mcp_launch_spec_args``): per boot the
    wrapper resolves ``mcp_launch.py`` against the git repo root and the shim
    ...
    if project == f"packages/{cli}":
        return {
            "type": "stdio",
            "command": _MCP_LAUNCH_WRAP_CMD,
            "args": _mcp_launch_spec_args(server_id),
        }
    return {
        "type": "stdio",
        "command": "uv",
...
def _build_local_default_mcp_servers(target: Path, base: Path | None = None) -> dict[str, dict[str, Any]] | None:
    ...
    for server_id, relpath, cli in _LOCAL_MCP_SERVERS:
        ...
        spec = dict(_local_mcp_spec(server_id, project, cli_name))
        pinned_env = DEFAULT_MCP_SERVERS[server_id].get("env")
        if isinstance(pinned_env, dict) and pinned_env:
            spec["env"] = dict(pinned_env)
        servers[server_id] = spec
    return servers


def _split_managed_optional(
    mcp_servers: Mapping[str, Mapping[str, Any]] | None,
) -> tuple[dict[str, Mapping[str, Any]], dict[str, Mapping[str, Any]]]:
    """Partition a resolved managed map into (required_subset, optional_extras).

    Optional entries (e.g. an opted-in codebase-graph server) launch through the
    stdlib shim already and need no local/uvx rewrite, so callers rewrite only
    the required subset and re-merge the optional extras verbatim.
    """
    from workbay_bootstrap._mcp_pins import OPTIONAL_MCP_SERVERS

    optional_keys = set(OPTIONAL_MCP_SERVERS)
    servers = dict(mcp_servers or {})
    required = {k: v for k, v in servers.items() if k not in optional_keys}
    optional = {k: v for k, v in servers.items() if k in optional_keys}
    return required, optional


def _is_pure_default_mcp_map(
    mcp_servers: Mapping[str, Mapping[str, Any]] | None,
) -> bool:
    """True when the REQUIRED subset is exactly the default map.

    Opted-in optional servers (a superset key like codebase-graph) do not
    disqualify the map: the required servers must still get their local/uvx
    rewrite, so the presence of an optional entry must not flip this gate off.
    """
    if mcp_servers is DEFAULT_MCP_SERVERS:
        return True
    if mcp_servers is None:
        return False
    required, _optional = _split_managed_optional(mcp_servers)
    base_keys = set(DEFAULT_MCP_SERVERS)
    if set(required) != base_keys:
        return False
    return all(required[k] == DEFAULT_MCP_SERVERS[k] for k in base_keys)
...
def _resolve_install_mcp_servers(
    ...
    if not _is_pure_default_mcp_map(mcp_servers):
        return mcp_servers
    if _is_release_tag_ref(remote_ref):
        return mcp_servers
    _required, optional = _split_managed_optional(mcp_servers)
...
def _is_release_tag_ref(ref: str | None) -> bool:
    """True for supported public release tag shapes."""
    return bool(ref and (ref.startswith("v") or ref.startswith("workbay-v")))
...
def _resolve_worktree_install_mcp_servers(
    ...
    if not _is_pure_default_mcp_map(mcp_servers):
        return mcp_servers
    _required, optional = _split_managed_optional(mcp_servers)
    local = _build_local_default_mcp_servers(target, base=target)
    if local is None:
        return mcp_servers
    return {**local, **optional}
...
def _local_uv_project_from_spec(
    ...
    implementation note stdlib shim form (worktree base), wrapped in the cwd-independent
    ``sh -c`` launcher (``_launch_shim_server_id`` recognizes it). The shim spec
    carries no project path on its command line, so the project is recovered by
    mapping the launched server id through ``_LOCAL_MCP_PROJECTS`` -- the
    ...
    shim_server_id = _launch_shim_server_id(command, args)
    if shim_server_id is not None:
        relpath = _LOCAL_MCP_PROJECTS.get(shim_server_id)
        if relpath is None:
            return None
...
def _mcp_specs_use_launch_shim(
    ...
    if not mcp_servers:
        return False
    for spec in mcp_servers.values():
        if _launch_shim_server_id(spec.get("command"), spec.get("args")) is not None:
            return True
    return False


class ReceiptSourceWarning(UserWarning):
    """Receipt provenance may differ from the package overlay version."""


from workbay_bootstrap.install_receipt import InstallPreflightError


class PackageSelfHostError(InstallPreflightError):
    """Package payloads cannot replace live self-host source files."""
...
def _resolve_gitonly_member_specs(
    ...
    *,
    ...
    """Resolve the git/path member-spec map for the git-only MCP tool install.

    Order (first match wins):

    1. ``worktree`` — path specs from the monorepo root at *target*.
    2. ``git_overlay`` / ``package`` with an explicit *remote_ref*
       — ref-pinned git specs using the explicit or resolved remote URL.
    3. Existing overlay clone under ``<target>/.workbay/remote`` — path specs.
    4. Front-door ``workbay`` ``uv-receipt.toml`` — grounded git URL+rev (or
       directory) pins recorded when the consumer installed the front door
       tool. Consulted **only** for ``source_kind == "package"`` when the
       operator supplied no ``remote_url`` (D6). Never substitutes a host-local
       pin when the operator explicitly named a remote.

    Returns ``None`` only when no grounded source exists. Callers must fail
    closed rather than silently deferring MCP tool install.
    """
    from workbay_bootstrap.gitonly_closure import (
        member_specs_from_git_ref,
        member_specs_from_repo_root,
        member_specs_from_uv_receipt,
    )

    if source_kind == "worktree":
        return member_specs_from_repo_root(target)
    clone = target.joinpath(*CLONE_SUBDIR)
    if source_kind in {"git_overlay", "package"} and remote_ref:
    ...
    # Receipt fallback is package-mode only, and only when the operator did not
    # name a remote_url. Explicit refs were handled above, so receipt pins
    # can never override an operator-supplied ref (D6).
    if source_kind == "package" and not remote_url:
        receipt_specs = member_specs_from_uv_receipt()
        if receipt_specs:
            warnings.warn(
                "Using front-door uv receipt pins; their revision is not verified "
                "against the package overlay version. Supply --remote-ref to "
                "prevent receipt/version skew.",
                ReceiptSourceWarning,
                stacklevel=2,
            )
...
def _install_gitonly_mcp_tools(
    ...
    *,
    ...
    Each installed entry records the resolved ``from_spec`` alongside the tool
    name so the receipt (and the ``gitonly_mcp_tool_source`` doctor facet) can
    audit whether a server was pinned from a git ref or a local dev path.

    When ``embeddings_mode`` is ``"verified"`` (``install --with-embeddings``),
    the handoff tool is installed with the same-package ``[embeddings]`` extra
    so the runtime uv tool has numpy/etc.; other MCP packages stay bare.
    """
    from workbay_bootstrap.external import run_external
    from workbay_bootstrap.gitonly_closure import (
        GITONLY_MCP_PACKAGES,
        DeclaredFastmcpRequirementError,
        build_uv_tool_install_argv,
        scrub_git_spec_userinfo,
    )
    ...
    for package in GITONLY_MCP_PACKAGES:
        ...
        # AGT-02: embeddings-on signal is install()'s embeddings_mode=="verified"
        # (CLI --with-embeddings), not a global or gate file.
        package_extras: tuple[str, ...] = (
            ("embeddings",) if package == "mcp-workbay-handoff" and embeddings_mode == "verified" else ()
        )
        # Install argv may still carry whatever the resolver produced (private
        # clones that rely on ephemeral URL userinfo). The *recorded* from_spec
        # that lands in .workbay-bootstrap.json is always scrubbed so secrets
        # never reach a tracked manifest (internal / D3). Prefer host
        # credential helpers / netrc / SSH for durable auth.
        try:
        ...
        except DeclaredFastmcpRequirementError as exc:
            raise InstallExecutionError(
                f"gitonly_mcp_tools failed: {exc}",
        ...
        installed.append({"name": package, "from_spec": scrub_git_spec_userinfo(from_spec)})
    return installed
...
StackResolvedFrom = Literal["git", "local", "unknown"]


class _StackSourceOptional(TypedDict, total=False):
    """Optional half of :class:`StackSource`; never instantiated on its own."""

    ref: str
    sha: str


class StackSource(_StackSourceOptional):
    """Install-ledger source provenance for the package stack.

    ``resolved_from`` is REQUIRED, and the requirement is encoded in the type
    rather than deferred to a runtime check: it lives on this total-by-default
    subclass while ``ref`` / ``sha`` stay optional on the ``total=False`` base,
    so a type checker rejects ``{}`` and ``{"ref": "x"}`` statically.

    ``ref`` / ``sha`` are recorded only when known. An UNATTESTED git claim --
    ``{"resolved_from": "git"}`` with neither key -- is legal, and is exactly
    what ``_stack_source_from_direct_url`` emits when ``direct_url.json``
    carries no usable ``requested_revision`` and no 40-hex ``commit_id``. The
    read side must never reject what the derive side legitimately writes.
    """

    resolved_from: StackResolvedFrom


STACK_RESOLVED_FROM_VALUES: frozenset[str] = frozenset(["git", "local", "unknown"])
_STACK_SOURCE_LEGAL_VALUES = "git|local|unknown"
_STACK_SOURCE_KEYS: frozenset[str] = frozenset(["ref", "sha", "resolved_from"])
_STACK_SOURCE_ERROR_PREFIX = "manifest stack_source invalid"
_STACK_SOURCE_REMEDY = f"re-run `workbay-bootstrap install` to regenerate {BOOTSTRAP_MANIFEST_NAME}"


def _stack_source_error(detail: str) -> BootstrapManifestValidationError:
    """Refusal carrying the field context, the offending value, and the remedy."""
    return BootstrapManifestValidationError(f"{_STACK_SOURCE_ERROR_PREFIX}: {detail}; {_STACK_SOURCE_REMEDY}")


def validate_stack_source(value: object) -> StackSource:
    """Refuse a manifest ``stack_source`` object that breaks the wire shape.

    Returns the same mapping, narrowed to :class:`StackSource`. Both call
    sites -- the write-side hook in :func:`_finalize_install_manifest` and
    the read-side hook in ``subcommands._load_manifest`` -- store the return
    so the narrowed object is what is written or handed to callers.

    Validates every recorded field, not only the one never in doubt:

    - ``resolved_from`` present, a ``str``, and one of ``git|local|unknown``;
    - ``ref`` optional, but a non-empty ``str`` when present;
    - ``sha`` optional, but matching ``_STACK_SOURCE_SHA_RE`` when present --
      the SAME predicate the derive path gates on, so read and write agree;
    - no unknown keys.

    ``ref`` / ``sha`` stay OPTIONAL on purpose: an unattested git claim is a
    legal derive-path output (see :class:`StackSource`), so requiring ``sha``
    for ``resolved_from == "git"`` would refuse real installs.
    """
    if not isinstance(value, dict):
        raise _stack_source_error(
            f"expected an object with resolved_from in {_STACK_SOURCE_LEGAL_VALUES}, got {type(value).__name__}"
        )
    unknown = set(value) - _STACK_SOURCE_KEYS
    if unknown:
        raise _stack_source_error(
            "unknown key(s) "
            + ", ".join(sorted(repr(key) for key in unknown))
            + "; legal keys are ref, sha, resolved_from"
        )
    if "resolved_from" not in value:
        raise _stack_source_error(f"missing resolved_from, expected one of {_STACK_SOURCE_LEGAL_VALUES}")
    resolved_from = value["resolved_from"]
    if not isinstance(resolved_from, str) or resolved_from not in STACK_RESOLVED_FROM_VALUES:
        raise _stack_source_error(f"resolved_from={resolved_from!r} not in {_STACK_SOURCE_LEGAL_VALUES}")
    if "ref" in value:
        ref = value["ref"]
        if not isinstance(ref, str) or not ref:
            raise _stack_source_error(f"ref={ref!r} must be a non-empty string")
    if "sha" in value:
        sha = value["sha"]
        if not isinstance(sha, str) or not _STACK_SOURCE_SHA_RE.match(sha):
            raise _stack_source_error(f"sha={sha!r} must be a 40-character lowercase hex commit id")
    return cast(StackSource, value)
...
def _annotate_surface_provenance(
    ...
    *,
    ...
    payload_root = self_host_payload_root(resolved_target)
    for entry in surfaces:
        ...
        # Identical copies may record payload identity as advisory provenance.
        # Keep their source local: install-time equality cannot authorize
        # reaping a copy that an operator may later edit under an ignored root.
        if (
            entry.get("source") in {"local", "shared"}
            and payload_root is not None
            and not path.is_symlink()
            and path.resolve().is_relative_to(resolved_target)
            and (payload_root / rel).resolve().is_relative_to(payload_root)
            and _payload_identical(payload_root / rel, path)
        ):
            entry["provenance_key"] = f"link:{WORKBAY_SYSTEM_PAYLOAD_SUBDIR}"
            continue
...
def _enforce_hook_coherence_gate(
    ...
    *,
    ...
    findings = assess_hook_coherence(target, package_root=package_root, receipt=manifest)
...
def _migrate_legacy_manifest(target: Path) -> Path | None:
    ...
    for legacy_name in (LEGACY_WORKBAY_OVERLAY_MANIFEST_NAME,):
        ...
        if not isinstance(data, dict) or not isinstance(data.get("surfaces"), list):
            continue
        try:
            from workbay_bootstrap.git_write import GitWriteEscapeError, _git_write

            _git_write(target, "mv", legacy_name, BOOTSTRAP_MANIFEST_NAME, cwd=target)
        except (
...
def _resolve_ref_to_sha(clone: Path, remote_ref: str) -> str:
    ...
    for candidate in candidates:
        try:
            ...
            continue
    raise RuntimeError(
        f"could not resolve remote_ref {remote_ref!r} in {clone} (tried remote-tracking branch, tag, and raw ref)"
    )
...
def _materialize_one_symlink(
    ...
    if target_path.is_symlink():
        ...
        if os.path.isabs(raw_target):
            ...
            abs_target_str = os.path.normpath(os.path.join(str(target_path.parent), raw_target))
        try:
            resolved = target_path.resolve(strict=False)
        except OSError:
            resolved = None
        resolves_to_expected = resolved is not None and resolved == remote_path.resolve()
        if resolves_to_expected:
            return {"path": rel, "source": "shared"}
        if _is_repointable_bootstrap_symlink(target_path, abs_target_str, clone_resolved, remote_subtree_prefix):
...
def _points_into_remote_subtree(
    ...
    return abs_target_str == str(clone_resolved) or abs_target_str.startswith(remote_subtree_prefix)
...
def _is_repointable_bootstrap_symlink(
    ...
    """
    if _points_into_remote_subtree(abs_target_str, clone_resolved, remote_subtree_prefix):
        return True
    if target_path.exists():
        try:
        ...
        if resolved_str and _names_clone_subtree(resolved_str):
            if not _points_into_remote_subtree(resolved_str, clone_resolved, remote_subtree_prefix):
                return True
...
def _names_clone_subtree(abs_target_str: str) -> bool:
    ...
    for runtime, remote in (CLONE_SUBDIR,):
        if any(parts[i] == runtime and parts[i + 1] == remote for i in range(len(parts) - 1)):
            return True
    return False
...
def _excluded_carved_children(surface: str) -> frozenset[str]:
    """Child names a carved ``surface`` must never materialize as per-child links.

    The single exclusion source shared by the enumerator
    (:func:`_carved_surface_children`) and the pre-materialize symlink cleanup
    (:func:`_prepare_carved_parent`), so the two can never disagree on what a
    carved surface excludes: the surface's own carve-outs
    (:data:`SURFACE_CHILD_EXCLUSIONS`), its lifecycle-hoisted children (copied as
    real files by the lifecycle pass, so a symlink here would collide), and
    compiled-bytecode / OS cruft (:data:`CARVED_CHILD_IGNORED_NAMES`).
    """
    return SURFACE_CHILD_EXCLUSIONS[surface] | _lifecycle_hoist_children(surface) | CARVED_CHILD_IGNORED_NAMES
...
def _carved_surface_children(surface: str, remote_path: Path) -> list[Path]:
    ...
    Every child of ``remote_path`` (in name order) except those in
    :func:`_excluded_carved_children` — the single carve/exclusion source. Shared
    by the materializer (apply) and the drift guard (``--check``) via
    :func:`iter_expected_surface_targets`.
    """
    excluded = _excluded_carved_children(surface)
    return [child for child in sorted(remote_path.iterdir(), key=lambda p: p.name) if child.name not in excluded]
...
def iter_expected_surface_targets(target: Path, clone: Path) -> list[ExpectedSurfaceTarget]:
    ...
    for surface in SHARED_SURFACES:
        ...
        if surface in SURFACE_CHILD_EXCLUSIONS:
            ...
            results.append(ExpectedSurfaceTarget(surface, remote_path, target / surface))
    return results
    ...
    for expected in iter_expected_surface_targets(target, clone):
        ...
        if parent is not None:
            if parent not in carved_parent_is_foreign:
                local_entry = _prepare_carved_parent(parent, target / parent, clone_resolved, remote_subtree_prefix)
...
def _copy_surface_entry(
    ...
    *,
    ...
    replaced for idempotent reruns. Preexisting destinations identical to the
    current payload are recorded as ``shared`` without rewriting so
    :func:`_annotate_surface_provenance` can attach the real key; divergent
    local files/directories, foreign symlinks, special nodes, and
    executable/traversal-bit mismatches stay local and untouched.
    Unreadable destinations fail closed as local. Path/name similarity is
    not identity.
    """
    ...
    if dest.is_symlink():
        ...
        if not previously_shared:
            if _payload_identical(src, dest):
                return {"path": rel, "source": "shared"}
            return {"path": rel, "source": "local"}
        if dest.is_dir():
            shutil.rmtree(dest)
...
def _carved_parent_has_managed_children(surface: str, previous_sources: Mapping[str, str]) -> bool:
    prefix = surface + "/"
    return any(path.startswith(prefix) and source == "shared" for path, source in previous_sources.items())


class UnownedReviewSurfaceConflict(RuntimeError):
    """A legacy receipt cannot authorize an unknown review gate implementation."""
...
def _materialize_surfaces_copy(
    ...
    *,
    ...
    plain surface already present in the target is left untouched and treated
    as local unless it is identical to the current payload, in which case it
    is recorded as shared so provenance annotation can fire; foreign symlinks
    stay local. A carved parent that is a real directory is **merged** child
    by child (each child gets :func:`_copy_surface_entry`'s per-path
    precedence), matching :func:`_prepare_carved_parent` on the git_overlay
    path.
    """
    materialized: list[dict[str, str]] = []
    previous_sources = previous_sources or {}
    # Check before copying any surface so failed migration preserves the receipt
    # and consumer customizations. Explicit local overrides remain authorized.
    recorded_paths = read_review_surface_receipt(target)
    if isinstance(recorded_paths, ReviewSurfaceUnknown):
        ownership = recorded_paths
    else:
        ownership = classify_review_surfaces(target, source_root, recorded_paths)
    if isinstance(ownership, ReviewSurfaceConflict):
        paths = " ".join(ownership.paths)
        raise UnownedReviewSurfaceConflict(
            f"unowned_review_surface_conflict: {paths}; preserve these files "
            "as explicit local overrides in the receipt, or move them aside "
            "and rerun update to install the shared payload"
        )
    if isinstance(ownership, ReviewSurfaceUnknown):
        raise UnownedReviewSurfaceConflict(f"review_surface_ownership_unknown: {ownership.cause}")
    for surface in SHARED_SURFACES:
        ...
        dest = target / surface
        if surface in SURFACE_CHILD_EXCLUSIONS:
            previously_managed = previous_sources.get(surface) == "shared" or _carved_parent_has_managed_children(
...
_STACK_SOURCE_SHA_RE = re.compile(r"^[0-9a-f]{40}$")


def _stack_source_from_direct_url(data: object) -> StackSource:
    """Derive the manifest ``stack_source`` object from a parsed PEP 610
    ``direct_url.json`` payload (REQUEST 2026-08-23).

    Returns ``{"ref": ..., "sha": ..., "resolved_from": "git|local|unknown"}``
    with ``ref``/``sha`` present only when known/resolvable: a git install
    records the requested revision and the 40-char commit SHA pip/uv already
    resolved; a local (``file://``) install records the source URL; anything
    else — no direct URL (index install), malformed payload — degrades to
    ``{"resolved_from": "unknown"}``. Pure function, never raises.
    """
    if not isinstance(data, Mapping):
        return {"resolved_from": "unknown"}
    url = data.get("url")
    vcs_info = data.get("vcs_info")
    if isinstance(vcs_info, Mapping) and vcs_info.get("vcs") == "git":
        source: StackSource = {"resolved_from": "git"}
        ref = vcs_info.get("requested_revision")
        if isinstance(ref, str) and ref:
            source["ref"] = ref
        sha = vcs_info.get("commit_id")
        if isinstance(sha, str) and _STACK_SOURCE_SHA_RE.match(sha):
            source["sha"] = sha
        return source
    if isinstance(url, str) and url.startswith("file://"):
        return {"resolved_from": "local", "ref": url}
    return {"resolved_from": "unknown"}


def _stack_source() -> StackSource:
    """``stack_source`` for the installed stack anchor (implementation note).

    Reads the anchor distribution's on-disk ``direct_url.json`` — information
    the installer already resolved — so this NEVER makes a network call and
    never raises: any failure degrades to ``{"resolved_from": "unknown"}``.
    """
    try:
        dist = importlib_metadata.distribution(STACK_DISTRIBUTION)
    except importlib_metadata.PackageNotFoundError:
        return {"resolved_from": "unknown"}
    try:
        raw = dist.read_text("direct_url.json")
        data = json.loads(raw) if raw else None
    except Exception:  # noqa: BLE001 - degrade, never fail the install
        data = None
    return _stack_source_from_direct_url(data)


def _stack_member_drift_findings(
    manifest: Mapping[str, object],
    *,
    version_lookup: Any | None = None,
) -> list[dict[str, str]]:
    """Return installed-version drift for the package stack recorded in manifest."""
    ...
    expected: dict[str, str] = {str(stack_distribution): str(manifest.get("stack_version") or "")}
    ...
    for distribution in sorted(expected):
        ...
        try:
            have = version_lookup(distribution)
        except importlib_metadata.PackageNotFoundError:
            have = None
        if have is None:
            findings.append(
                {
                    "kind": "stack_drift",
                    "path": distribution,
                    "detail": (f"{distribution} not installed (stack requires {want})"),
                }
            )
            ...
            findings.append(
                {
                    "kind": "stack_drift",
                    "path": distribution,
                    "detail": (f"installed {have} != stack-required {want}"),
                }
            )
    return findings


def _enforce_stack_member_convergence(
    manifest: Mapping[str, object],
    *,
    allow_member_skew: bool = False,
    version_lookup: Any | None = None,
) -> None:
    """Fail package install/update when installed stack members do not match pins."""
    if allow_member_skew:
        return
    findings = _stack_member_drift_findings(manifest, version_lookup=version_lookup)
    if not findings:
        return
    lines = [
        "refusing to write a mixed workbay package stack; "
        "upgrade/converge the workbay package set and re-run "
        "`make workbay-update` (or `workbay-bootstrap update`). "
        "Use --allow-member-skew only for intentional development skew."
    ]
    lines.extend(f"- {f['path']}: {f['detail']}" for f in findings)
    raise BootstrapManifestValidationError("\n".join(lines))
    ...
    for surface in GENERATED_SURFACES:
        ...
        if target_path.is_symlink():
            try:
                ...
                resolved = None
            points_into_clone = resolved is not None and str(resolved).startswith(str(clone_resolved) + os.sep)
...
def _prepare_plugin_generated_surfaces(target: Path, clone: Path, override_root: Path | None) -> list[dict[str, str]]:
...
def _plugin_override_root_from_manifest(target: Path, manifest: Mapping[str, object] | None) -> Path | None:
    if manifest is None:
        return None
...
def _discover_plugin_override_root(
    ...
    *,
    ...
    if plugin_overrides is not None:
        ...
        if not manifest_path.is_file():
            raise FileNotFoundError(f"plugin override manifest not found: {manifest_path}")
        return candidate
...
def _render_plugin_override_lock(
    ...
    if existing_lock_path.is_file():
        ...
        for entry in existing_payload.get("components", []):
            ...
            name = entry.get("name")
            if isinstance(provenance, dict) and isinstance(kind, str) and isinstance(name, str):
...
def _apply_doc_surface_overrides(target: Path, override_root: Path, clone: Path) -> None:
...
def _run_generator(target: Path, clone: Path, remote_sha: str, override_root: Path | None = None) -> None:
    ...
    run_external(effective_plugin_cmd, call_class="generator", check=True, cwd=str(clone))


def _prune_retired_lifecycle_hoists(target: Path) -> None:
    """Remove pre-rename lifecycle-hoist dirs orphaned by the namespace rename.

    See :data:`RETIRED_LIFECYCLE_HOIST_DESTS`. Idempotent — absent paths are
    skipped. A symlink at a retired path is left untouched (consumer-owned, or
    a stale bootstrap-owned symlink already handled by the surface
    materializer's excluded-child removal); only a stale *real* directory is
    pruned, so the retired ``workbay.lifecycle`` / ``workbay.evals`` occupant
    can never coexist with the renamed ``workbay_lifecycle`` / ``workbay_evals``
    packages at a consumer site.
    """
    for rel in RETIRED_LIFECYCLE_HOIST_DESTS:
        stale = target / rel
        if stale.is_symlink():
            continue
        if stale.is_dir():
...
def _install_lifecycle_profile(target: Path, clone: Path) -> list[dict[str, str]]:
    ...
    Before hoisting, retired pre-rename hoist dirs are pruned (see
    :func:`_prune_retired_lifecycle_hoists`) so an overlay upgrading from a
    pre-rename install never carries both the old ``scripts/workbay/lifecycle``
    tree and the renamed ``scripts/workbay_lifecycle`` tree.
    """
    _prune_retired_lifecycle_hoists(target)
    entries: list[dict[str, str]] = []
...
def _makefile_declares_live_include_directive(text: str) -> bool:
    ...
    return any(line.strip() == LIFECYCLE_INCLUDE_DIRECTIVE and not line.startswith("\t") for line in text.splitlines())
...
def _ensure_consumer_makefile_include(target: Path) -> dict[str, str] | None:
    ...
    block = f"{LIFECYCLE_INCLUDE_SENTINEL_BEGIN}\n{LIFECYCLE_INCLUDE_DIRECTIVE}\n{LIFECYCLE_INCLUDE_SENTINEL_END}\n"
...
def _consumer_gitignore_entries() -> list[str]:
    ...
    Lifecycle provenance (implementation note): ``surfaces_for_kind("lifecycle")`` is
    read so hoist destinations enter the fence, but only surfaces not already
    covered by a parent shared/generated/generator_input line are emitted.
    Nested hoist dests under ``Makefile.d`` / ``scripts/workbay`` stay legible
    in provenance without redundant exact pathspecs that expand self-managed
    false-leaks (Defect 1A needs ``/scripts/workbay_lifecycle`` only).
    """
    # ``.workbay-cache`` is the check-all stamp-skip cache root (implementation note;
    # the engine writes ``.workbay-cache/checkall-stamps/``). Like ``.workbay``
    # and ``.task-state`` it is a runtime-generated local dir, not a materialized
    # surface, so it is seeded here (not derived from a surface kind) and ignored
    # at install time so a consumer's first cached ``check`` run never dirties
    # ``git status``.
    entries = [f"/{RUNTIME_ROOT_DIRNAME}", "/.task-state", "/.workbay-cache"]
    entries += [f"/{surface}" for surface in SHARED_SURFACES]
    entries += [f"/{surface}" for surface in GENERATED_SURFACES]
    entries += [f"/{surface}" for surface in surfaces_for_kind("generator_input")]
    covered = {e.lstrip("/") for e in entries}
    for surface in surfaces_for_kind("lifecycle"):
        if any(surface == root or surface.startswith(f"{root}/") for root in covered):
            continue
        entries.append(f"/{surface}")
...
def _ensure_consumer_gitignore_block(target: Path) -> dict[str, str]:
    ...
    block = GITIGNORE_SENTINEL_BEGIN + "\n" + "\n".join(leaking) + "\n" + GITIGNORE_SENTINEL_END + "\n"
    if existing is None:
...
def _probe_remote_gate_for_install(target: Path | None = None):
    """Resolve the remote-gate host (env, then ``.workbay/remote-gate.env``) and probe it.

    Thin wrapper so tests can monkeypatch
    ``workbay_bootstrap.install._probe_remote_gate_for_install`` without real SSH.
    The config-file fallback exists for non-login-shell harnesses that do not
    inherit the operator's env.
    """
    from workbay_protocol.remote_probe import (
        probe_remote_gate,
        resolve_remote_gate_host,
    )

    return probe_remote_gate(resolve_remote_gate_host(target))


def _verify_embeddings_for_install(target: Path) -> None:
    """Advisory probe of the installer/front-door embeddings environment.

    The published git-only front door intentionally carries the bare handoff
    package.  Its interpreter therefore cannot import the optional embeddings
    stack, even though the runtime handoff tool installed later in this command
    receives ``[embeddings]``.  This probe must never gate installation; the
    post-install runtime-console probe is the sole fail-closed authority.

    Thin seam so tests can monkeypatch
    ``workbay_bootstrap.install._verify_embeddings_for_install`` without real
    model downloads or optional extras.
    """
    from workbay_bootstrap.embedding_provision import _load_model_pin

    try:
        # Useful on editable/worktree installs whose invoking interpreter does
        # carry the extra, but never evidence about the serving interpreter.
        _load_model_pin()
    except Exception as exc:  # noqa: BLE001 - advisory must never gate install
        print(
            f"warning: embeddings front-door probe unavailable for {target}: "
            f"{exc}. Continuing to install and hard-verify the "
            "mcp-workbay-handoff runtime interpreter.",
            file=sys.stderr,
        )


def _verify_embeddings_runtime_console(target: Path) -> None:
    """Provision and prove embeddings in the runtime handoff interpreter.

    This is the interpreter ``mcp_launch.py`` execs at runtime, not the
    front-door ``workbay`` tool venv.  It is the single fail-closed embeddings
    gate: import the optional stack, load the digest pin, provision and verify
    the artifacts, and enable the worktree gate in one runtime-scoped probe.

    R2 (assessment agent-driven-install-experience-eval-2026-07-22):
    ``_verify_embeddings_for_install`` runs in-process and *before*
    ``run_install`` installs the git-only server tool, so it can only prove the
    front door imports — a structural false green (shipped as 0145 S4). This
    runs *after* the runtime tool is installed (with the ``[embeddings]`` extra
    via R1) and probes that interpreter directly, so a green
    ``--with-embeddings`` is impossible while the runtime is broken
    ([AGT-03]/[DBG-01]). Fail-closed; thin patchable seam for tests.
    """
    from workbay_bootstrap.install_receipt import InstallPreflightError
    from workbay_bootstrap.runtime_interp import resolve_tool_python

    repair = (
        f"re-run `workbay install --target {target} --with-embeddings` "
        "(R1 attaches the [embeddings] extra to the runtime tool), or reinstall "
        "`mcp-workbay-handoff[embeddings]` via `uv tool install`"
    )
    runtime_python = resolve_tool_python("mcp-workbay-handoff")
    if not runtime_python:
        raise InstallPreflightError(
            "embeddings runtime-verify failed: could not resolve the "
            f"mcp-workbay-handoff uv-tool interpreter. Repair: {repair}",
            failure_class="application",
        )
    probe = "; ".join(
        (
            ...
            "from workbay_bootstrap.embedding_provision import provision, set_embeddings_gate",
            f"target = Path({str(target)!r})",
            "provision(worktree_root=target)",
    ...
    # Cold import of numpy/ONNX can exceed the shared probe default (30s); keep
    # that default cheap for other probes and bound this heavyweight check at 120s.
    probe_timeout = 120
    try:
        result = run_resolved_interpreter_probe(runtime_python, probe, timeout=probe_timeout)
    except ExternalCallTimeout as exc:
        raise InstallPreflightError(
            "embeddings runtime-verify failed: the embeddings runtime "
            f"verification probe timed out after {exc.timeout} seconds. "
            f"Repair: {repair}",
            failure_class="application",
        ) from exc
    if result.returncode != 0:
        ...
        raise InstallPreflightError(
            "embeddings runtime-verify failed: the mcp-workbay-handoff uv-tool "
            f"interpreter could not import/provision the embeddings stack ({detail}). "
            f"Repair: {repair}",
            failure_class="application",
        )


def _preflight_install_modes(
    target: Path,
    *,
    execution_mode: str,
    embeddings_mode: str,
) -> None:
    """Fail closed on remote-only probe and verified-embeddings preconditions."""
    from workbay_bootstrap.install_receipt import InstallPreflightError

    if execution_mode == "remote_only":
        # Probe before any materialization so a failed gate leaves the target
        # untouched (install --with-remote fail-closed; deferred setup is
        # install without the flag, then repair --with-remote later).
        result = _probe_remote_gate_for_install(target)
        if not result.ok:
            raise InstallPreflightError(
                f"remote gate probe failed ({result.state}): {result.detail}",
                failure_class="application",
            )

    if embeddings_mode == "verified":
        # S45R gate fix: explicit --with-embeddings vs the ambient kill-switch
        # is a contradiction the operator must resolve, not a silent override
        # in either direction ([OBS-08]).
        from workbay_bootstrap.embedding_provision import embeddings_disabled

        if embeddings_disabled():
            raise InstallPreflightError(
                "install --with-embeddings conflicts with "
                "WORKBAY_HANDOFF_EMBEDDINGS_DISABLED in the environment; unset "
                "the variable or install without the flag.",
                failure_class="application",
            )
        # Advisory only: the published front door is deliberately extra-less.
        # The runtime uv tool is installed with [embeddings] later and is the
        # only interpreter whose import/provision result may gate success.
        _verify_embeddings_for_install(target)


def _finish_install_embeddings(
    target: Path,
    *,
    embeddings_mode: str,
    no_embeddings: bool,
) -> None:
    """Runtime embeddings verify when requested; otherwise consent prompts."""
    import sys

    if embeddings_mode == "verified":
        # The runtime tool now exists with [embeddings] via R1. Provision and
        # verify in that interpreter — the same process that will serve MCP.
        _verify_embeddings_runtime_console(target)
        return

    from workbay_bootstrap.install_embeddings_consent import (
        apply_install_embeddings_consent,
    )
    ...
    for line in apply_install_embeddings_consent(
        ...
        no_embeddings=no_embeddings or embeddings_mode == "disabled",
    ):
        print(line, file=sys.stderr)


def _preflight_package_source(
    target: Path,
    source: str,
    remote_url: str | None,
) -> None:
    """Reject credential-bearing package URLs and live self-host targets."""
    if source == "package" and remote_url:
        from urllib.parse import urlsplit

        if urlsplit(remote_url).username is not None:
            raise InstallPreflightError("package remote URL must not contain userinfo")
    if source == "package" and self_host_payload_root(target) is not None:
        tracked = _git("ls-files", cwd=target)
        raise PackageSelfHostError(
            "refusing package install into self-host checkout; protected paths: " + ", ".join(tracked.splitlines()[:12])
        )
    ...
    *,
    ...
    Args:
        ...
        install_claude_reinject_hook_local: When True, write the user-owned,
            ...
            ``.claude/settings.local.json``. Off by default.
        install_codex_ensure_agent_surfaces_hook: When True, register the
            self-heal ``ensure-agent-surfaces`` SessionStart hook for the Codex
            harness (adapter declared under hook ``ensure-agent-surfaces`` in
            portable_commands.json). Off by default. Claude's registration is
            always-on via the base ``settings.hooks`` render.
        install_vscode_ensure_agent_surfaces_hook: When True, register the
            self-heal ``ensure-agent-surfaces`` SessionStart hook for the VS
            Code harness. Off by default.
        install_grok_ensure_agent_surfaces_hook: When True, register the
            self-heal ``ensure-agent-surfaces`` SessionStart hook for the Grok
            harness. Off by default.
        execution_mode: Offload policy to record. ``"remote_only"`` probes the
            remote gate before any materialization and persists that policy in
            the ledger; ``"local_ok"`` (default) omits the field for
            byte-compat with existing manifests.
        embeddings_mode: Semantic-embeddings capability to record.
            ``"verified"`` (``--with-embeddings``) hard-verifies the
            [embeddings] extra and digest pin, then provisions; ``"disabled"``
            (``--no-embeddings``) opts out; ``"unspecified"`` (default) omits
            the field unless provisioned artifacts digest-verify at install,
            in which case the ledger records ``verified`` so semantic features
            do not read silence as "unspecified".
    ...
    Raises:
        ...
        InstallPreflightError: remote-only install probe or embeddings hard-verify failed.
    """
    from workbay_bootstrap.install_plan import InstallRequest, run_install

    _preflight_package_source(target, source, remote_url)
    _preflight_install_modes(
        target,
...
_MCP_CONFIG_SURFACE_PATHS: dict[str, str] = {
    "config_claude": ".mcp.json",
    "config_vscode": ".vscode/mcp.json",
    "config_codex": ".codex/config.toml",
    "config_cursor": ".cursor/mcp.json",
}


class _McpEnvFanout(TypedDict):
    server: str
    env: str
    sources: list[str]
...
def _read_mcp_surface_servers(target: Path, path: str) -> Mapping[str, Any]:
    """Best-effort read of one harness's existing MCP server table."""
    ...
    try:
        if path == ".codex/config.toml":
            ...
            container = "servers" if path == ".vscode/mcp.json" else "mcpServers"
            servers = doc.get(container, {})
    except (
        OSError,
        UnicodeDecodeError,
...
def _mcp_env_override_fanout(
    target: Path,
    mcp_servers: Mapping[str, Mapping[str, Any]],
) -> list[_McpEnvFanout]:
    """Describe non-baseline env values copied from one surface to others.

    A custom env value is not itself suspicious: callers can intentionally
    provide a custom managed map.  Fan-out is specifically the case where the
    same value already lives on at least one config surface and rendering the
    incoming map will add it to another.  Values are deliberately omitted from
    the report because environment overrides can contain credentials.
    """
    existing = {path: _read_mcp_surface_servers(target, path) for path in _MCP_CONFIG_SURFACE_PATHS.values()}
    rows: list[_McpEnvFanout] = []
    for server_name, spec in sorted(mcp_servers.items()):
        env = spec.get("env")
        if not isinstance(env, Mapping):
            continue
        baseline_spec = DEFAULT_MCP_SERVERS.get(server_name, {})
        baseline_env = baseline_spec.get("env")
        if not isinstance(baseline_env, Mapping):
            baseline_env = {}
        for env_name, env_value in sorted(env.items()):
            if not isinstance(env_name, str):
                continue
            if baseline_env.get(env_name) == env_value:
                continue
            sources: list[str] = []
            destinations: list[str] = []
            for path, surface_servers in existing.items():
                surface_spec = surface_servers.get(server_name)
                surface_env = surface_spec.get("env") if isinstance(surface_spec, Mapping) else None
                if isinstance(surface_env, Mapping) and surface_env.get(env_name) == env_value:
                    sources.append(path)
                else:
                ...
                rows.append(
                    {
                        ...
                        "bypass_class": env_name.endswith(("_PREFLIGHT_OK", "_BREAKER_RESET")),
                    }
                )
    return rows


def _report_mcp_env_override_fanout(
    rows: Iterable[_McpEnvFanout],
    *,
    receipt: Any,
    successful_paths: set[str],
) -> None:
    """Persist and print actual (successfully written) override fan-out."""
    for row in rows:
        destinations = [path for path in row["destinations"] if path in successful_paths]
        if not destinations:
            continue
        reason = (
            f"{row['env']} for {row['server']} propagated from {', '.join(row['sources'])} to {', '.join(destinations)}"
        )
        receipt.ok("mcp_env_override_fanout", reason=reason)
        level = "WARN" if row.get("bypass_class") else "NOTICE"
        print(f"{level}: MCP env override fan-out: {reason}", file=sys.stderr)


class InvalidCodexConfigError(ValueError):
    """Existing Codex configuration cannot be safely regenerated."""


class InvalidMcpJsonError(InvalidCodexConfigError):
    """Existing ``.mcp.json`` cannot be merged; original bytes must stay."""

    kind = "mcp_json_invalid"

    def __init__(self, path: Path | str, reason: str) -> None:
        self.path = str(path)
        self.reason = str(reason)
        super().__init__(f"{self.kind}: {self.path}: {self.reason}")


class InvalidManagedMcpSpecError(InvalidCodexConfigError):
    """Incoming managed MCP map cannot be published to any surface."""

    kind = "mcp_env_invalid"

    def __init__(self, reason: str, *, kind: str = "mcp_env_invalid") -> None:
        self.kind = kind
        self.reason = str(reason)
        super().__init__(f"{kind}: {self.reason}")
...
def _write_configs(
    ...
    *,
    ...
    if mcp_servers:
        # Validate the existing Claude document and the complete incoming map
        # once so a typed refusal never publishes a partial harness set
        # (NW-0226-01R2RV-001 / -003).
        _load_claude_mcp_document(target / ".mcp.json")
        _validate_managed_mcp_servers(mcp_servers)
        fanout_rows = _mcp_env_override_fanout(target, mcp_servers) if receipt is not None else []
        ...
        for step_name, writer in writers:
            if receipt is None:
                ...
                continue
            try:
                entry = writer(target, mcp_servers)
                entries.append(entry)
                successful_paths.add(entry["path"])
            except (InvalidMcpJsonError, InvalidManagedMcpSpecError):
                raise
            except Exception as exc:  # bulkhead: classify, record, continue
                receipt.failed(
                    step_name,
                    reason=str(exc),
                    failure_class="system" if isinstance(exc, OSError) else "application",
                    criticality="continue",
                )
            else:
                receipt.ok(step_name)
        _report_mcp_env_override_fanout(
            fanout_rows,
            receipt=receipt,
...
def _run_init_state(
    ...
    *,
    ...
    env_has_state_dir = isinstance(raw_env, Mapping) and "WORKBAY_HANDOFF_STATE_DIR" in raw_env
    ...
    if not any(arg == "--state-dir" or arg.startswith("--state-dir=") for arg in args) and not env_has_state_dir:
    ...
    if raw_env is not None:
        if not isinstance(raw_env, Mapping) or not all(
            ...
            raise ValueError("workbay-handoff-mcp config env must be a mapping[str, str]")
        env.update(raw_env)
...
def _validate_managed_mcp_servers(
    mcp_servers: Mapping[str, Mapping[str, Any]],
) -> Mapping[str, Mapping[str, Any]]:
    """Refuse a managed map that cannot be published identically on every surface.

    Shared composer for install/update/repair/mcp-sync. A scalar or otherwise
    non-object ``env`` used to render on JSON surfaces and raise untyped
    ValueError in the Codex renderer, which published a partial set
    (NW-0226-01R2RV-003).
    """
    if not isinstance(mcp_servers, Mapping):
        raise InvalidManagedMcpSpecError(f"mcp_servers must be a mapping, not {type(mcp_servers).__name__}")
    for name, spec in mcp_servers.items():
        if not isinstance(name, str) or not name:
            raise InvalidManagedMcpSpecError("mcp server names must be non-empty strings")
        if not isinstance(spec, Mapping):
            raise InvalidManagedMcpSpecError(f"{name}: server spec must be an object, not {type(spec).__name__}")
        env = spec.get("env") if "env" in spec else None
        if env is None:
            continue
        if not isinstance(env, Mapping) or not all(
            isinstance(key, str) and isinstance(value, str) for key, value in env.items()
        ):
            raise InvalidManagedMcpSpecError(
                f"{name}: env must be a mapping[str, str], not {type(env).__name__}",
                kind="mcp_env_invalid",
            )
    return mcp_servers
...
def _canonicalize_managed_servers(
    ...
    through unchanged."""
    mcp_servers = _validate_managed_mcp_servers(mcp_servers)
...
_MCP_WIRING_KEYS = frozenset({"command", "args", "url", "type", "transport", "env"})


def _compose_managed_server(previous: Mapping[str, Any], spec: Mapping[str, Any]) -> dict[str, Any]:
    """Replace installer-owned wiring, merge env, keep operator keys.

    ``command`` / ``args`` / transport keys come from the incoming spec so a
    removed upstream key disappears. Operator env values and unrecognized
    keys stay authoritative unless the spec names the same env key.
    """
    composed = {key: value for key, value in previous.items() if key not in _MCP_WIRING_KEYS}
    composed.update(dict(spec))
    previous_env = previous.get("env")
    env = dict(previous_env) if isinstance(previous_env, Mapping) else {}
    spec_env = spec.get("env")
    if isinstance(spec_env, Mapping):
        env.update(spec_env)
    if env:
        composed["env"] = env
    else:
...
def _render_mcp_json(
    ...
    *,
    ...
    (if any), compose managed servers under ``mcpServers``, and return
    ...
    doc: dict[str, Any] = _load_claude_mcp_document(path)
    ...
    for name, spec in mcp_servers.items():
        ...
        if not isinstance(previous, Mapping):
            previous = {}
        servers[name] = _compose_managed_server(previous, spec)
...
def _load_json_or_empty(path: Path) -> dict[str, Any]:
    ...
    Used for VS Code and Cursor MCP surfaces, which keep their existing
    check/refusal contracts. Claude ``.mcp.json`` must not use this
    fallback — see :func:`_load_claude_mcp_document`.
    """
    if not path.exists():
        return {}
    try:
...
def _load_claude_mcp_document(path: Path) -> dict[str, Any]:
    """Load existing ``.mcp.json``, or ``{}`` when the file is absent.

    A present file that is invalid JSON, a non-object root, or a
    non-object ``mcpServers`` is a typed ``mcp_json_invalid`` refusal.
    Never fall back to ``{}`` for a file that exists but does not parse
    (NW-0226-01R2RV-001).
    """
    if not path.exists():
        return {}
    try:
        ...
        raise InvalidMcpJsonError(path, f"invalid JSON ({exc})") from exc
    if not isinstance(loaded, dict):
        raise InvalidMcpJsonError(path, f"root must be a JSON object, not {type(loaded).__name__}")
    if "mcpServers" in loaded and not isinstance(loaded["mcpServers"], dict):
        raise InvalidMcpJsonError(path, "mcpServers must be a JSON object")
    return loaded
...
def _render_vscode_mcp_json(
    ...
    *,
    ...
    file (if any), compose managed servers under ``servers``, and
...
def _load_codex_config(path: Path) -> Any:
    if path.exists():
        try:
            ...
            raise InvalidCodexConfigError(f"Invalid Codex config: {path}") from exc
    else:
        doc = tomlkit.document()

    for key in ("mcp_servers", "marketplaces", "plugins"):
        if key not in doc:
            continue
        if not isinstance(doc[key], Mapping):
            raise InvalidCodexConfigError(f"Invalid Codex config: {path}: {key} must be a table")
        for name, value in doc[key].items():
            if not isinstance(value, Mapping):
                raise InvalidCodexConfigError(f"Invalid Codex config: {path}: {key}.{name} must be a table")
            if key == "mcp_servers" and "env" in value and not isinstance(value["env"], Mapping):
                raise InvalidCodexConfigError(f"Invalid Codex config: {path}: {key}.{name}.env must be a table")
    return doc
...
def _render_codex_config(
    ...
    *,
    ...
    managed server while preserving operator keys and root comments, and
    ...
    contract. Line endings normalize to LF; comments inside replaced managed
    tables are not guaranteed to survive."""
    ...
    for name, spec in mcp_servers.items():
        ...
        # Commands and transport are generated; preferences and unknown keys
        # remain authoritative in the existing project configuration.
        previous = servers_table.get(name, {})
        wiring_keys = {"command", "args", "url", "type", "transport", "env"}
        for key, value in previous.items():
            if key not in wiring_keys:
                new_table[key] = value
        for spec_key, spec_value in spec.items():
            new_table[spec_key] = spec_value
        env = dict(previous.get("env", {}))
...
def _write_codex_config(
    ...
    *,
    ...
    other root keys and tables intact. Line endings normalize to LF; comments
    inside replaced managed tables are not guaranteed to survive."""
    path = target / ".codex" / "config.toml"
    existed = path.exists()
...
def _set_git_hooks_path(
    ...
    *,
    ...
    if dot_git.is_file():
        ...
        from workbay_bootstrap.worktree_config import (
            enable_worktree_config_with_migration,
        )

        enable_worktree_config_with_migration(target, receipt=receipt)
        _git_write(
            target,
            "config",
            "--worktree",
...
def _resolve_dotted_path(doc: dict[str, Any], json_path: str) -> tuple[dict[str, Any], str]:
    ...
    for seg in segments[:-1]:
        ...
        if not isinstance(nxt, dict):
            raise ValueError(f"refusing to merge: {seg!r} along {json_path!r} is not an object")
        parent = nxt
...
def _apply_merge_array_entry(adapter: Mapping[str, Any], *, target: Path) -> dict[str, str]:
    ...
    from .claude_settings import (
        SidecarSymlinkRefusalError,
        merge_adapter_document,
        merge_claude_document,
    )

    target_rel = adapter["target"]
    relative = Path(target_rel)
    if relative.is_absolute() or ".." in relative.parts or not relative.parts:
        raise ValueError(f"refusing adapter target: {target_rel!r}")
    if any(part.casefold() == ".claude" for part in relative.parts) and relative.parts[0] != ".claude":
        raise SidecarSymlinkRefusalError(f"refusing non-canonical Claude adapter target: {target_rel!r}")
    if relative.parts[0] == ".claude":
        return merge_claude_document(
            target,
            relative,
            lambda doc: _merge_array_document(doc, adapter["patch"], target_rel),
        )

    return merge_adapter_document(
        target,
        relative,
        lambda doc: _merge_array_document(doc, adapter["patch"], target_rel),
    )
...
def _merge_array_document(doc, patch, settings_path):
    """Merge in memory; persistence belongs to the destination's writer."""
    if not isinstance(doc, dict):
        raise ValueError(f"refusing to merge hook into {settings_path}: existing JSON document is not an object")

    parent, leaf_key = _resolve_dotted_path(doc, patch["json_path"])
    array_raw = parent.get(leaf_key, [])
    if not isinstance(array_raw, list):
        raise ValueError(f"refusing to merge hook into {settings_path}: {patch['json_path']!r} is not a list")
    ...
    if not isinstance(managed_entry, Mapping) or match_key not in managed_entry:
        raise ValueError(f"adapter entry missing match_key {match_key!r}: {managed_entry!r}")
...
def _walk_hook_adapters(
    *,
    ...
    for hook in hooks:
        ...
        selected = [a for a in adapters if isinstance(a, Mapping) and a.get("opt_in_flag") in active_flags]
        if not selected:
            continue
        ...
        for adapter in selected:
            ...
            if handler is None:
                raise NotImplementedError(f"hook {hook.get('hook_id')!r}: unknown adapter patch operation {op!r}")
            ...
            if adapter.get("harness") == "claude-code" and adapter.get("write_kind") == "user_owned_local":
