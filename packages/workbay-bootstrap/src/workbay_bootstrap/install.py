...
_MCP_LAUNCH_WRAP_SCRIPT = (
    ...
    f'exec python3 "$root/{_MCP_LAUNCH_SHIM}" "$@"'
)
...
def _resolve_gitonly_member_specs(
    ...
    *,
    ...
    2. ``git_overlay`` / ``package`` with a release-tag *remote_ref* + *remote_url*
       — tag-pinned git specs (operator-supplied, grounded).
    ...
    if (
        ...
        and remote_ref
        and _is_release_tag_ref(remote_ref)
        and remote_url
    ):
    ...
    # name a remote_url. An explicit --remote-url with a non-tag ref must not
    # silently install whatever the host's front-door workbay pin says (D6).
...
_STACK_SOURCE_REMEDY = (
    f"re-run `workbay-bootstrap install` to regenerate {BOOTSTRAP_MANIFEST_NAME}"
)
...
def _payload_identical(src: Path, dest: Path) -> bool:
    ...
    Fail closed: OSError, unsupported file type, type mismatch,
    extra/missing children, byte drift, symlink-target mismatch, or
    executable/traversal-bit mismatch is not identical. Supported types
    are regular files, directories, and symlinks; special nodes (FIFO,
    device, socket) are never read. Directory identity uses the same
    ``_EXEC_BITS`` mask as files (traversal), not full directory mode —
    umask write-bit drift is not a payload/reaper signal, while a
    traversal-bit mismatch is a local mode customization and stays
    local. Path/name similarity is not evidence. Uses ``lstat`` so a
    foreign symlink is never treated as the payload file it happens to
    point at.
    """
    try:
...
def _payload_identical_unchecked(src: Path, dest: Path) -> bool:
    ...
    if src_dir or dest_dir:
        ...
        if src_names != dest_names:
            return False
        return all(
            _payload_identical_unchecked(src / name, dest / name) for name in src_names
        )
