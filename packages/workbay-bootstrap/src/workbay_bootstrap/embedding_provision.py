    ...
    try:
        ...
        if not isinstance(exc, ModuleNotFoundError):
            ...
            try:
                ...
                message = (
                    "mcp-workbay-handoff is not importable; cannot read the "
                    "embedding digest pin — skipping provisioning (provider stays "
                    "unconfigured). Install the handoff package (git-only: uv tool "
                    "install the workbay closure per docs/CONSUMER.md)."
                )
            else:
                message = (
...
def run_embeddings_cli(
    worktree_root: Path, *, status: bool, enable: bool, disable: bool
) -> int:
    ...
    **file gate** in ``.workbay/embedding.env`` only — process-env kill-switch
    ...
    if status:
        ...
        raise ValueError(
            "run_embeddings_cli requires exactly one of status/enable/disable"
        )
    root = worktree_root.resolve()
    try:
        if mode == "status":
            print(json.dumps(embeddings_gate_status(root)), file=sys.stdout)
            return 0
        set_embeddings_gate(root, enabled=(mode == "enable"))
