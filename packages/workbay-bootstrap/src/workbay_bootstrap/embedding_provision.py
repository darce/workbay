    ...
    *,
    ...
    try:
        ...
        raise EmbeddingProvisionUnavailable(
            f"failed to download {pin.filename}: {exc}"
        ) from exc
    finally:
...
def _record_embedding_degrade(summary: str, *, worktree_root: Path | str) -> None:
    try:
        ...
        if not result.get("ok"):
            sys.stderr.write(
                f"agent_errors installer capture failed: {result.get('error')}\n"
            )
    except ImportError:
        ...
        sys.stderr.write(
            f"agent_errors installer capture failed: {type(exc).__name__}: {exc}\n"
        )
