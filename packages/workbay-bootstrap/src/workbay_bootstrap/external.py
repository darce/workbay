...
def run_resolved_interpreter_probe(
    ...
    *,
    ...
    """Run a static import probe in a resolved interpreter with a class timeout.

    Narrow gateway for install-time runtime checks that already hold a fully
    resolved interpreter path and a fixed probe string. Bounds the child via
    the ``probe`` call class (default 30s; override with ``timeout`` or
    ``WORKBAY_TIMEOUT_PROBE``). On expiry the process tree is reaped through
    the same kill machinery as :func:`run_external`. Does not trip the offline
    latch. Propagates the same ``subprocess`` exception types as a direct
    ``subprocess.run`` call, plus :class:`ExternalCallTimeout` when the bound
    expires. Callers that need a longer probe must pass an explicit bound;
    there is no unbounded path.

    ``env`` is a full child environment mapping (same contract as
    :func:`run_external` / ``subprocess``). Pass a per-call dict when the probe
    must inject secrets without mutating the parent process environment.
    """
    # Route through run_external so the probe inherits class timeouts, session
    # isolation, and _kill_process_tree reaping — never a bare subprocess.run.
    return run_external(
