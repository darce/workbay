    ...
    *,
    ...
    if _mcp_specs_use_launch_shim(plan.mcp_servers):
        try:
            ...
            # Best-effort contract (matches prewarm): a FIRST-call
            # failure trips the offline latch inside run_external but
            # still raises. With DEFAULT_MCP_SERVERS (shim path) prewarm
            # is a no-op, so gitonly is often the first best-effort
            # external call — it must defer, not abort, on offline /
            # timeout / OSError the same way prewarm does.
