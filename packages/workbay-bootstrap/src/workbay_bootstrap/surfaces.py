...
LEGACY_CLONE_SUBDIR = (".workstate", "remote")
...
def overlay_clone_homes(target: Path) -> tuple[Path, Path]:
    """Canonical and legacy managed clone roots under ``target``."""
    return (
        target.joinpath(*CLONE_SUBDIR),
        target.joinpath(*LEGACY_CLONE_SUBDIR),
    )
