...
Post-install, use `workbay-bootstrap embeddings --status|--enable|--disable --target <path>` as the SSOT toggle (idempotent writes to `.workbay/embedding.env`). Harnesses also ship `/workbay` (Claude → `AskUserQuestion` menu; other harnesses → `/workbay embeddings <on|off|status>`).
...
```text
workbay-bootstrap install --target <path> [--remote-ref <tag>] [--mcp-servers <default|path>] [--no-mcp-servers] [--no-embeddings]
...
workbay-bootstrap embeddings --target <path> --status|--enable|--disable
