...
Consumers run `workbay-bootstrap install --target /path/to/repo` once; it
...
Git-only delivery from the public mirror
[`darce/workbay`](https://github.com/darce/workbay). Pin a released monorepo
tag for reproducibility.

**Preferred consumer path** is the front door (`workbay`) via
`scripts/install-workbay-cli.sh` — see the monorepo
[`README.md`](../../README.md) and [`docs/CONSUMER.md`](../../docs/CONSUMER.md).
That path installs the full runtime closure and exposes `workbay install`,
which delegates here.

### Bootstrap CLI only (narrower tool)

When you specifically want `workbay-bootstrap` on `$PATH` without the front
door, use the manual git closure (escape hatch; kept in
...
# From the consumer root, status/doctor/repair default to cwd.
workbay-bootstrap status
workbay-bootstrap doctor
# workbay install defaults --source package; --hooks none|repo|all (recorded is repair-only)
```
...
```text
workbay install --target /path/to/repo [--with-codebase-graph] [--with-remote] [--with-embeddings|--no-embeddings]   # front door (delegates here)
workbay-bootstrap install --target /path/to/repo [--remote-ref TAG] [--mcp-servers default|PATH] [--no-mcp-servers] [--with-codebase-graph] [--with-remote] [--with-embeddings|--no-embeddings]
workbay-bootstrap update  --target /path/to/repo --remote-ref TAG
workbay-bootstrap status  --target /path/to/repo
workbay-bootstrap clean     --target /path/to/repo [--dry-run] [--yes]
workbay-bootstrap gc        --target /path/to/repo [--dry-run] [--yes]
workbay-bootstrap doctor  --target /path/to/repo [--mcp-servers default|PATH]
workbay-bootstrap provision-embeddings --target /path/to/repo
workbay embeddings --status|--enable|--disable   # preferred (cwd = worktree root)
workbay-bootstrap embeddings --target /path/to/repo --status|--enable|--disable  # deprecated alias
workbay-bootstrap repair  --target /path/to/repo [--force-dirty] [--no-embeddings] [--with-remote|--no-remote] [--mcp-servers default|PATH]
workbay-bootstrap overrides status --target /path/to/repo [--plugin-overrides PATH] [--json]
workbay-bootstrap overrides relock  --target /path/to/repo [--plugin-overrides PATH] [--force]
workbay-bootstrap adopt-worktree [--target /path/to/worktree] [--primary /path/to/primary] [--check] [--json]
```
...
- **Raw `git worktree add` / auto-worktrees** are healed on demand:
  ...
  workbay-bootstrap adopt-worktree --target /path/to/worktree
  # or, as a steady-state guard (exit 1 on drift):
  workbay-bootstrap adopt-worktree --target /path/to/worktree --check
  ```
...
- `--mcp-servers` defaults to the built-in managed map registering
  `mcp-workbay-handoff` and `mcp-workbay-orchestrator` through
  `scripts/hooks/mcp_launch.py` (via a cwd-independent `sh -c` launcher
  that resolves the repo root and execs
  `python3 …/mcp_launch.py <server-id>`), so Codex, VS Code, and Claude
  clients start real MCP stdio servers from the generated config without
  a per-session `uvx` package resolve. Pass a JSON file path to override;
  pass `--no-mcp-servers` to skip the three config writers entirely.
