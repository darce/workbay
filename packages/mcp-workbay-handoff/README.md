...
2. **`.workbay-bootstrap.json`** at the workspace root, via the
   `compaction.thresholds` key (`tokens` and `chars` must be non-negative integers;
   the legacy `.workbay-overlay.json` name is still read as a fallback; the values
   below are illustrative overrides, not required values):
...
make compaction-enable TASK=WORKBAY-99
make compaction-status TASK=WORKBAY-99      # prints the resolved receipt
...
Git-only delivery: WorkBay is installed from the public git mirror
...
```bash
REF=v0.1.55  # a released tag from https://github.com/darce/workbay/tags
# --no-sources is required (the pyproject carries a workspace source); the sole
# workbay dependency (workbay-protocol) is git-sourced via --with, never PyPI.
uv tool install --no-sources \
  --with "git+https://github.com/darce/workbay.git@$REF#subdirectory=packages/workbay-protocol" \
  --from "git+https://github.com/darce/workbay.git@$REF#subdirectory=packages/mcp-workbay-handoff" \
  mcp-workbay-handoff
```
...
The names below are package exports. Three of them are library-only — they
are not served MCP tools. Each such line names the MCP replacement.
...
from workbay_handoff_mcp import (
    ...
    set_handoff_state,
    update_task_status,  # library-only; MCP: set_handoff_state(..., status_only=True)
    get_verified_tests,
    render_handoff,
    record_file_touch,  # library-only; MCP: touched_files(operation="record")
    get_touched_files,  # library-only; MCP: touched_files(operation="list")
)
...
mcp-workbay-handoff --workspace-root /path/to/workspace validate --kind decision_id --decision codex_slice_complete_plan0004_contract-pinning-and-docs --decision-kind slice_complete
mcp-workbay-handoff --workspace-root /path/to/workspace review-findings --operation list
mcp-workbay-handoff --workspace-root /path/to/workspace integrity-check --kind close
```
...
Pinned via git-sourced uvx (reproducible across machines without a global install — resolves the server from a pinned monorepo tag on the git mirror, never PyPI):
...
```json
{
  ...
  "args": [
    "--no-sources",
    "--with",
    "git+https://github.com/darce/workbay.git@v0.1.55#subdirectory=packages/workbay-protocol",
    "--from",
    "git+https://github.com/darce/workbay.git@v0.1.55#subdirectory=packages/mcp-workbay-handoff",
    "mcp-workbay-handoff",
    "--workspace-root",
    "/path/to/workspace",
    "serve-stdio"
...
currently installed; the git-sourced and source-checkout variants above pin a known
release.
...
```bash
mcp-workbay-handoff --workspace-root /path/to/workspace --tool-profile all serve-stdio
```
