...
from .handoff_state import _set_handoff_state_with_conn, set_handoff_state  # noqa: F401
from .handoff_state import get_handoff_state as _get_handoff_state
from .import_export import (  # noqa: F401
    ...
    switch_task,
    tasks_gc,
    update_task_status,  # library re-export; not a served MCP tool
)
from .lanes_api import close_lane, get_lane, list_lanes, open_lane, update_lane  # noqa: F401
from .lanes_recording import (  # noqa: F401
...
# Guards `search_handoff` ranking execution. Defined below the re-export
# block, not above it: nothing outside this module reads it, and both uses
# are inside function bodies, so import order is irrelevant (E402).
_allow_ranking_execute: ContextVar[bool] = ContextVar("_allow_ranking_execute", default=False)
...
def get_handoff_state(
    ...
    """Read handoff state after normalizing sequence-valued section names."""
    if isinstance(sections, (list, tuple)):
        if not all(isinstance(section, str) for section in sections):
            return _envelope(
                ...
                task_ref=task_ref,
                data={"error": "sections must contain only strings."},
            )
        normalized_sections = ",".join(sections)
    else:
        normalized_sections = sections
    return _get_handoff_state(
        task_ref=task_ref,
        branch=branch,
        ...
        decision_lane_id=decision_lane_id,
...
def _handoff_fts_sql(
    *,
    fts_table: str,
    is_decision: bool,
    has_status: bool,
    scoped_task_ref: bool,
    scoped_lane: bool,
) -> str:
    """FTS5 MATCH + ORDER BY rank SQL for one search_handoff record type.

    Extracted so the implementation note lock test can assert the ranking contract without
    scraping the private loop. Not a cosine/embed path (Principle 10).
    """
...
def _guard_search_ranking_execute(conn: object) -> object:
    """Reject MATCH + ORDER BY rank unless ``_execute_handoff_fts`` is active."""
    ...
    class _Wrapped:
        ...
        def execute(self, sql: object, params: object = ()) -> object:
            ...
            if " MATCH ?" in compact and "ORDER BY rank" in compact and not _allow_ranking_execute.get():
                raise RuntimeError("search_handoff ranking must use _execute_handoff_fts")
            return self._inner.execute(sql, params)  # type: ignore[union-attr]

        def __getattr__(self, name: str) -> object:
            return getattr(self._inner, name)

    return _Wrapped(conn)


def _execute_handoff_fts(conn: object, sql: str, params: object = ()) -> object:
    """Run ranking SQL only when it is FTS5 MATCH + ORDER BY rank.

    The helper-string lock is not enough (BR-MEM-01): a cosine port that
    leaves ``_handoff_fts_sql`` unused would still call ``conn.execute``.
    This is the only ranking execute on the search_handoff path.
    """
    compact = " ".join(sql.split())
    if " MATCH ?" not in compact or "ORDER BY rank" not in compact:
        raise RuntimeError("search_handoff ranking must be FTS5 MATCH + ORDER BY rank")
    token = _allow_ranking_execute.set(True)
    try:
        return conn.execute(sql, params)  # type: ignore[union-attr]
    finally:
        _allow_ranking_execute.reset(token)
...
def search_handoff(
    ...
    *,
    ...
    `scope` defaults to ``"task"``, preserving implicit active-task
    resolution when `task_ref` is omitted. Pass ``"all"`` to search across
    task refs without invoking the active-task resolver.
    """
    effective_scope = "task" if scope is None else scope
    if effective_scope not in {"task", "all"}:
        return _envelope(
            ok=False,
            tool="search_handoff",
            data={"error": f"Invalid scope: {scope!r}. Valid: ['all', 'task']."},
        )
    if not queries:
        return _envelope(
    ...
    results: list[dict] = []
    with _get_db_connection() as raw_conn:
        conn = _guard_search_ranking_execute(raw_conn)
        tables_exist = (
            conn.execute(
        ...
        effective_task_ref: str | None = None if effective_scope == "all" else task_ref
        scope_degraded_reason: str | None = None
        if effective_scope == "task" and effective_task_ref is None:
            try:
                ...
                candidate_refs = sorted(
                    ...
                    if isinstance(candidate, dict) and candidate.get("task_ref")
                )
                scope_degraded_reason = "Active-task scope was ambiguous; search widened to scope='all'."
                if candidate_refs:
                    scope_degraded_reason += f" Candidate task_refs: {', '.join(candidate_refs)}."
                effective_scope = "all"
        ...
        scope_applied = "task" if effective_task_ref else "all"
        ...
        for rtype in validated_types:
            ...
            sql = _handoff_fts_sql(
                fts_table=fts_table,
                is_decision=is_decision,
                has_status=has_status,
                scoped_task_ref=bool(effective_task_ref),
                scoped_lane=bool(lane_id),
            )
            try:
                rows = _execute_handoff_fts(conn, sql, (*params, clamped_limit)).fetchall()
    ...
    response_data: dict[str, object] = {
        ...
        "scoped_task_ref": effective_task_ref,
    }
    if scope_degraded_reason is not None:
        response_data["scope_degraded_reason"] = scope_degraded_reason
    return _envelope(
        ok=True,
        tool="search_handoff",
        data=response_data,
...
def search_artifacts(
    ...
    if task_ref:
        ...
        scope["task_ref"] = None
    try:
        outcome = artifact_index.search_artifacts_outcome(
            queries=queries,
    ...
    if include_embed and mutation.get("operation") in {"insert", "update"}:
        ...
        if affected_ids:
            try:
                ...
                outcome = embed_concept_on_write("decision.rationale", affected_ids[0], resolved_task_ref, rationale)
                # Production failures are swallowed inside embed_concept_best_effort
                # and returned as ``failed`` — they never raise ([RES-02]).
                if outcome == "failed":
                    msg = "embed_concept_on_write failed during close_slice: provider/store returned failed"
                    warnings.append(msg)
...
def _apply_close_slice_side_effect_fields(data: dict, side_effects: dict) -> None:
    ...
    if any(f.get("effect") not in ("dashboard", "embed_concept_on_write") for f in failures):
...
_CLOSE_SLICE_ARCHIVED_ERROR = (
    "Cannot close a slice on an archived task. "
    "Switch to it first or use set_handoff_state(..., status_only=True)."
)
    ...
    if has_active_row:
        ...
        if archived is not None:
            return _envelope(
                ...
                tool="close_slice",
                data={
                    "error": _CLOSE_SLICE_ARCHIVED_ERROR,
                    "state_error": _CLOSE_SLICE_ARCHIVED_ERROR,
                    "decision_recorded": False,
                    "state_updated": False,
                    "current_task_md_written": False,
                    "revision_retries": 0,
    ...
    for attempt in range(MAX_OPTIMISTIC_REVISION_ATTEMPTS):
        ...
        if not prep_done:
            if changed_files is None:
                ...
                derived_paths, derive_warning = resolve_omitted_changed_files(repo_root, pre_resolved_ctx.commit_sha)
