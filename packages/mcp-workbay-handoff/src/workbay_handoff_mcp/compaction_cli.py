"""``python3 -m workbay_handoff_mcp.compaction_cli`` — internal.
...
consumer can run ``make compact-now`` against the handoff package that
``workbay-bootstrap`` installs alongside the MCP server (git-only
delivery — no PyPI ``uvx`` / ``pip install`` step).
"""
