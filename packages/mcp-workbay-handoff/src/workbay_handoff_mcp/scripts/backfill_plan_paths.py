...
``python3 -m workbay_handoff_mcp.scripts.backfill_plan_paths`` against the
handoff package that ``workbay-bootstrap`` installs (git-only delivery —
no PyPI ``uvx`` / ``pip install`` step).
"""
