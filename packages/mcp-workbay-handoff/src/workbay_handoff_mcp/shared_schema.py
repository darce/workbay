...
#         records are stamped decision_origin='system' at the schema layer (the public
#         record_decision wrapper takes no decision_origin kwarg). No new column, so
#         _HANDOFF_REQUIRED_COLUMNS is unchanged; the bump is what makes the edited
#         trigger re-install on already-stamped v31 DBs — _apply_handoff_migrations
#         replays every migration and this one DROP/CREATEs the trigger unconditionally.
HANDOFF_SCHEMA_VERSION = 32
...
def _migrate_decisions_decision_origin(conn: sqlite3.Connection) -> None:
    ...
    fts_present = (
        conn.execute(
            "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'decisions_fts' LIMIT 1"
        ).fetchone()
        is not None
    )
    if fts_present:
...
_PURGED_CONCEPT_ENTITY_KINDS: tuple[str, ...] = (
...
def _prepare_handoff_connection(conn: sqlite3.Connection) -> sqlite3.Connection:
    ...
    # Committed by the caller's _get_db_connection context (or explicit commit).
