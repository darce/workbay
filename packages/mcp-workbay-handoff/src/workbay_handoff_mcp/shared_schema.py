...
#         floor stays 37.
#   v40 — additive nullable speed/tier routing columns on worktree_lanes. The
#         reader floor stays 37; older readers safely ignore routing metadata.
HANDOFF_SCHEMA_VERSION = 40
...
def _migrate_add_column_extensions(conn: sqlite3.Connection) -> None:
    ...
    for column in ("model", "backend", "reasoning_effort", "speed", "tier", "test_cmd"):
...
def _rebuild_worktree_lanes_status_check(conn: sqlite3.Connection) -> None:
    ...
    # Preserve additive routing and self-verify columns when present.
    ...
    has_speed = _has_column(conn, "worktree_lanes", "speed")
    has_tier = _has_column(conn, "worktree_lanes", "tier")
    ...
    has_branch_identity = {column: _has_column(conn, "worktree_lanes", column) for column in branch_identity_columns}
    ...
    select_speed = "speed" if has_speed else "NULL"
    select_tier = "tier" if has_tier else "NULL"
    ...
    identity_select = ", ".join(column if has_branch_identity[column] else "NULL" for column in branch_identity_columns)
    conn.execute(
        f"""
        INSERT INTO worktree_lanes (
            id, task_ref, lane_id, title, objective, worktree_path, branch, owner_agent,
            model, backend, reasoning_effort, speed, tier, test_cmd, lane_kind, {identity_target},
            status, notes, created_at, updated_at
        )
        SELECT
            id, task_ref, lane_id, title, objective, worktree_path, branch, owner_agent,
            {select_model}, {select_backend}, {select_effort}, {select_speed}, {select_tier},
            {select_test_cmd}, {select_lane_kind},
            {identity_select},
            status, notes, created_at, updated_at
...
# COLD delete->WAL conversion testing measured failures with waits of 8-23ms
# against a 30000ms busy_timeout: SQLite returns SQLITE_BUSY immediately for
# exclusive journal-mode conversion and never invokes the busy handler, so no
# busy_timeout value can protect this statement
