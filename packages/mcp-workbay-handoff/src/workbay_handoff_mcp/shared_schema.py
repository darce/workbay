...
# Schema 32→33 added ``codemap_reindex_lease`` only, so the floor stays 32.
MIN_COMPATIBLE_READER_VERSION = 32
...
_HANDOFF_REQUIRED_COLUMNS: dict[str, frozenset[str]] = {
    ...
    "worktree_lanes": frozenset({"test_cmd", "lane_kind"}),
...
def _rebuild_worktree_lanes_status_check(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        ...
        INSERT INTO worktree_lanes (
            ...
            model, backend, reasoning_effort, test_cmd, lane_kind, status, notes, created_at, updated_at
        )
        SELECT
            id, task_ref, lane_id, title, objective, worktree_path, branch, owner_agent,
...
def _bootstrap_handoff_schema(conn: sqlite3.Connection) -> None:
    ...
    try:
        ...
        try:
            ...
            # user_version write stays last so a crash mid-bootstrap leaves the
            # DB unstamped and the next open retries.
            conn.execute(
        ...
        try:
            ...
            if action == sqlite3.SQLITE_PRAGMA:
                ...
                if (
                    ...
                    and _schema_is_main_file(
                        _authorizer_target_schema(action, arg1, dbname)
                    )
                ):
                    self._set_holds_write_lock(True)
            elif action in _WRITE_LOCK_AUTHORIZER_ACTIONS:
                if _schema_is_main_file(
                    _authorizer_target_schema(action, arg1, dbname)
                ):
                    self._set_holds_write_lock(True)
...
# (SQLITE_BUSY / SQLITE_LOCKED); other errors propagate unretried. CARD-07: after
...
_SQLITE_JOURNAL_MODE_RETRYABLE_CODES = frozenset(
    {
...
def _is_journal_mode_lock_error(exc: BaseException) -> bool:
    """True for SQLITE_BUSY / SQLITE_LOCKED on journal_mode; false otherwise."""
    if not isinstance(exc, sqlite3.OperationalError):
        return False
    code = getattr(exc, "sqlite_errorcode", None)
    if code is not None:
        return int(code) in _SQLITE_JOURNAL_MODE_RETRYABLE_CODES
...
def _apply_journal_mode_wal(conn: sqlite3.Connection) -> None:
    ...
    Single enforcement point for this factory ([ARCH-13]). Retries only lock
    errors; re-raises the original exception after the attempt ceiling.
    """
    ...
    for attempt in range(JOURNAL_MODE_WAL_MAX_ATTEMPTS):
        try:
            ...
            if not _is_journal_mode_lock_error(exc):
                raise
            last_exc = exc
            if attempt + 1 >= JOURNAL_MODE_WAL_MAX_ATTEMPTS:
                break
            delay_ms = JOURNAL_MODE_WAL_RETRY_DELAYS_MS[
                min(attempt, len(JOURNAL_MODE_WAL_RETRY_DELAYS_MS) - 1)
            ]
            # Bounded backoff [CARD-09] inside this except handler. Must NOT use
            # blocking_sleep: under a held write lock that raises
            # WriteLockHeldAcrossBlockingWorkError from inside the handler,
            # destroying the retry and masking last_exc (the operational error).
...
def connect_handoff_db(db_path, *, read_only: bool = False) -> sqlite3.Connection:
    ...
    *does* install a busy timeout of 5000 ms — the defect is the wrong bound
    ...
    must not be mutated by diagnostic/lease readers). ``PRAGMA busy_timeout`` is
    still applied so readers wait out short writers.
    """
    if read_only:
        from pathlib import Path
...
def _get_db_connection(*, begin_immediate: bool = False) -> Iterator[sqlite3.Connection]:
    ...
    label = (
        "handoff_db_connection begin_immediate=1"
        if begin_immediate
        else "handoff_db_connection"
    )
    try:
        ...
        try:
            ...
            _log.exception(
                "db writer registration failed during connect (continuing unregistered)"
            )
            heartbeat_cm = None
            registered = False
        try:
            ...
            if registered and heartbeat_cm is not None:
                try:
                    ...
                    _log.exception(
                        "db writer unregister failed during connect cleanup"
                    )
    finally:
        conn.close()
