...
from .bounded_call import run_with_daemon_timeout
...
def lookup_related_prior(
    *,
    ...
    Runs through the shared abandonable daemon-call bound. On timeout return a
    ...
    # copy_context so the daemon worker sees configure_runtime() (ContextVar,
    # not inherited by ordinary threads).
    context = copy_context()
    try:
        return run_with_daemon_timeout(
            lambda: context.run(_call),
            timeout_seconds=timeout_seconds,
            timeout_message=f"related_prior lookup timed out after {timeout_seconds}s",
        )
    except TimeoutError:
