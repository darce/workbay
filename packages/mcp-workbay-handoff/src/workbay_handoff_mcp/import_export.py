...
def _import_snapshot(
    ...
    fallback_agent, fallback_branch, fallback_commit = _resolve_import_fallbacks(
        active,
        fallback_agent=_normalize_optional_text(resolve_env_alias("WORKBAY_HANDOFF_DEFAULT_AGENT")) or "codex",
    ...
    for row in decisions:
        ...
        conn.execute(
            "INSERT INTO decisions (task_ref, lane_id, session, decision, rationale, agent, model, model_label, reasoning_level, input_tokens, output_tokens, total_tokens, changed_files_json, slice_number, branch, commit_sha, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(task_ref, decision, session) DO NOTHING",
            (
                task_ref,
                _resolve_import_lane_id(row),
                row.get("session", "import"),
...
def tasks_gc(apply: bool = False) -> dict:
    """Archive ``status=done`` on-main ``MAINT-*`` rows.
