...
def _parse_lane_identity(
    landing_commit_sha: object, branch_tip_sha: object, branch_tip_source: object
) -> tuple[str | None, str | None, str | None, str | None]:
    """Validate identity before any write, including a source supplied alone."""
    landing, landing_error = _parse_full_commit_sha(landing_commit_sha, "landing_commit_sha")
    tip, tip_error = _parse_full_commit_sha(branch_tip_sha, "branch_tip_sha")
    source = None
    source_error = None
    if branch_tip_source is not None:
        if isinstance(branch_tip_source, str):
            source = _normalize_optional_text(branch_tip_source)
        if not isinstance(branch_tip_source, str) or (
            source is not None and source not in WORKTREE_LANE_BRANCH_TIP_SOURCES
        ):
            source_error = "branch_tip_source must be one of: " + ", ".join(WORKTREE_LANE_BRANCH_TIP_SOURCES)
    return landing, tip, source, landing_error or tip_error or source_error
...
def open_lane(
    *,
    ...
    ``model``, ``backend``, ``reasoning_effort``, ``speed``, ``tier``, and
    ``test_cmd``. A caller that
    ...
    """
    parsed_landing, parsed_tip, parsed_source, identity_error = _parse_lane_identity(
        landing_commit_sha, branch_tip_sha, branch_tip_source
    )
    if identity_error is not None:
        return _envelope(
            ok=False,
            tool="open_lane",
            data={"error": identity_error, "error_kind": "invalid_lane_identity"},
            entity="lane",
        )
    ...
    with _get_db_connection() as conn:
        ...
        conn.execute(
            ...
            INSERT INTO worktree_lanes (
                ...
                owner_agent, model, backend, reasoning_effort, speed, tier,
                test_cmd, lane_kind, status, notes,
                landing_commit_sha, branch_tip_sha, branch_tip_source, branch_tip_observed_at,
                created_at, updated_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, COALESCE(?, 'implement'), ?, ?,
                    ?, ?, ?, CASE WHEN ? IS NOT NULL THEN datetime('now') END, datetime('now'), datetime('now'))
            ON CONFLICT(task_ref, lane_id) DO UPDATE SET
                ...
                speed = COALESCE(excluded.speed, worktree_lanes.speed),
                tier = COALESCE(excluded.tier, worktree_lanes.tier),
                ...
                landing_commit_sha = COALESCE(excluded.landing_commit_sha, worktree_lanes.landing_commit_sha),
                branch_tip_sha = COALESCE(excluded.branch_tip_sha, worktree_lanes.branch_tip_sha),
                branch_tip_source = COALESCE(excluded.branch_tip_source, worktree_lanes.branch_tip_source),
                branch_tip_observed_at = CASE
                    WHEN excluded.branch_tip_sha IS NOT NULL
                        AND excluded.branch_tip_sha IS NOT worktree_lanes.branch_tip_sha
                    THEN excluded.branch_tip_observed_at ELSE worktree_lanes.branch_tip_observed_at END,
    ...
    *,
    ...
    with _get_db_connection() as conn:
        ...
        patch: dict[str, object | None] = {
            ...
            "status": status,
            "notes": notes,
            "landing_commit_sha": parsed_landing,
            "branch_tip_sha": parsed_tip,
            "branch_tip_source": parsed_source,
        }
        fields: list[str] = []
        params: list[object] = []
        if parsed_tip is not None:
            fields.append(
                "branch_tip_observed_at = CASE WHEN branch_tip_sha IS NOT ? "
                "THEN datetime('now') ELSE branch_tip_observed_at END"
            )
            params.append(parsed_tip)
