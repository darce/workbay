...
OPENABLE_LANE_STATUSES = frozenset(
    {"planned", "active", "blocked", "review", "closed_stale"}
)
...
CURRENT_TASK_SIDE_EFFECT_PROGRAMMING_ERROR_TYPE = (
    "current_task_side_effect_programming_error"
)
...
def open_lane(
    *,
    ...
    md_written, side_effect_error_type = _write_current_task_md_for_task(
        object(), resolved_task_ref
    )
    if not md_written:
        ...
        return _envelope(
            ...
            data={
                ...
                "error_type": side_effect_error_type
                or CURRENT_TASK_SIDE_EFFECT_ERROR_TYPE,
                "lane": lane_dict,
...
def close_lane(
    *,
    ...
    if not md_written:
        ...
        typed_cause = (
            side_effect_error_type or CURRENT_TASK_SIDE_EFFECT_ERROR_TYPE
        )
...
def latest_reclaim_candidate(*, lane_id: str, task_ref: str) -> dict[str, Any]:
    ...
    with _get_db_connection() as conn:
        decision_id = reclaim_candidate_decision_id(
            task_ref=normalized_task_ref, lane_id=normalized_lane_id
        )
        row = conn.execute(
            """
            SELECT id, task_ref, lane_id, session, decision, branch, commit_sha, agent,
        ...
        # PLAN0181-S2DEADCHK-01: a post-fetch task_ref mismatch guard is
        # unreachable here because WHERE task_ref = ? already constrains the
        # row; do not reintroduce a silent dead check.
        # PLAN0181-S2GATE2-LATEST-IGNORES-LANEID-01: the decision id embeds
        # lane_id, but decisions.lane_id is written from actor and can disagree.
        # Surface that mismatch instead of returning a healthy-looking candidate
        # whose lane_id points at the wrong worktree.
        if row is not None:
            ...
            if stored_lane != normalized_lane_id:
                return _envelope(
                    ...
                    data={
                        "error": (
                            "reclaim-candidate row lane_id does not match "
                            "query lane_id."
                        ),
                        "task_ref": normalized_task_ref,
                        "lane_id": normalized_lane_id,
                        "decision": decision_id,
                        "row_lane_id": (
                            None if row["lane_id"] is None else str(row["lane_id"])
                        ),
                    },
                    task_ref=normalized_task_ref,
                    entity="lane",
        ...
        if after_id is not _UNSET:
            ...
            if after_id is None:
                ...
                fetched = [
                    _row_to_dict(row)
                    for row in conn.execute(
                        f"SELECT * FROM worktree_lanes WHERE {where_sql} AND id < ? "
                        f"ORDER BY id DESC LIMIT ?",
                ...
                return _envelope(
                    ...
                    data={
                        "error": (
                            "after_id must be an int or None; "
                            f"got {type(after_id).__name__}."
                        )
                    },
                    entity="lane",
                )
...
def list_lanes_by_worktree_path(*, worktree_path: str) -> dict[str, Any]:
    ...
    with _get_db_connection() as conn:
        rows = [
            _row_to_dict(row)
            for row in conn.execute(
                "SELECT * FROM worktree_lanes WHERE worktree_path = ? "
                "ORDER BY id DESC",
                (normalized_path,),
            ).fetchall()
        ]
...
_NONTERMINAL_WORKTREE_PATH_SCAN_TERMINALS = frozenset(
    {"closed", "merged", "closed_stale"}
)
