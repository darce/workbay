...
class ReviewFindingsUpdateOp(BaseModel):
    ...
    resolution_notes: Annotated[
        ...
        Field(
            description=(
                f"Optional notes describing how the finding was resolved or dispositioned. "
                f"Max {MAX_RESOLUTION_NOTES_LENGTH} characters (pre-server named rejection)."
            ),
            max_length=MAX_RESOLUTION_NOTES_LENGTH,
        ),
    ] = None
    ...
    verification_evidence: Annotated[
        ...
        Field(
            description=(
                f"Optional verification evidence used when closing a finding as fixed. "
                f"Max {MAX_VERIFICATION_EVIDENCE_LENGTH} characters after trim "
                f"(authoritative pre-server named rejection; pydantic Field maxLength is a "
                f"coarse raw-payload outer bound and may reject trailing whitespace that "
                f"would normalize under the stored cap)."
            ),
            # Coarse raw-payload bound only. Authoritative length is measured on the
            # normalized (stripped) form in _pre_server_verification_evidence_rejection
            # and on the server write path — see CAP-RAW-VS-STRIP-LENGTH-DISAGREE.
            max_length=MAX_VERIFICATION_EVIDENCE_LENGTH,
        ),
    ] = None
...
class ReviewFindingsResolveOp(BaseModel):
    ...
    status: Annotated[
        Literal["fixed", "deferred", "wontfix"] | None,
        Field(
            description=(
                "Optional terminal status for the resolve batch. Default/omitted is the fix-claim path "
                "(classifier + commit ancestry → fixed / resolved_on_branch). "
                "deferred and wontfix are status decisions: ancestry-exempt, rationale-required, "
                "stored verbatim (implementation note S2)."
            ),
        ),
    ] = None
    finding_statuses: Annotated[
        dict[str, Literal["fixed", "deferred", "wontfix"]] | None,
        Field(
            description=(
                "Optional per-finding status overrides (finding_id → fixed|deferred|wontfix). "
                "Use when a single resolve batch mixes fix-claims and status decisions."
            ),
        ),
    ] = None
    resolution_notes: Annotated[
        ...
        Field(
            description=(
                f"Optional human-authored resolution notes to apply when descendant fixes are closed, "
                f"or required rationale for deferred/wontfix status decisions. "
                f"Max {MAX_RESOLUTION_NOTES_LENGTH} characters (pre-server named rejection)."
            ),
            max_length=MAX_RESOLUTION_NOTES_LENGTH,
        ),
    ...
    notes: Annotated[
        ...
        Field(
            description=(
                f"Alias for resolution_notes (status-decision rationale). Max {MAX_RESOLUTION_NOTES_LENGTH} characters."
            ),
            max_length=MAX_RESOLUTION_NOTES_LENGTH,
        ),
    ] = None
    verification_evidence: Annotated[
        ...
        Field(
            description=(
                f"Optional verification evidence applied to each fixed finding in the resolve batch. "
                f"Also accepted as rationale for deferred/wontfix status decisions. "
                f"Max {MAX_VERIFICATION_EVIDENCE_LENGTH} characters after trim "
                f"(authoritative pre-server named rejection; pydantic Field maxLength is a "
                f"coarse raw-payload outer bound and may reject trailing whitespace that "
                f"would normalize under the stored cap)."
...
class ReviewFindingsDispositionOp(BaseModel):
    ...
    resolution_notes: Annotated[
        ...
        Field(
            description=(
                f"Required notes explaining the disposition decision. "
                f"Max {MAX_RESOLUTION_NOTES_LENGTH} characters (pre-server named rejection)."
            ),
            max_length=MAX_RESOLUTION_NOTES_LENGTH,
        ),
    ...
    verified_commit_sha: Annotated[
        ...
        Field(
            description=(
                "Optional commit SHA that verified a fixed disposition. Stored like "
                "resolve/update verified_commit_sha (required for descendant fixed closes)."
            ),
        ),
    ] = None
    actor: ActorParam = None
...
class ReviewFindingsReanchorOp(BaseModel):
    ...
    resolution_notes: Annotated[
        ...
        Field(
            description=(
                f"Optional provenance note (e.g. rename-map evidence). "
                f"Max {MAX_RESOLUTION_NOTES_LENGTH} characters (pre-server named rejection)."
            ),
            max_length=MAX_RESOLUTION_NOTES_LENGTH,
        ),
...
def _pre_server_resolution_notes_rejection(
    notes: str | None,
    *,
    finding_id: str | None = None,
    tool: str = "review_findings",
) -> dict | None:
    """Named pre-server notes-length gate (T9) — structured rejection, not opaque server fail."""
    if notes is None:
        return None
    if len(notes) <= MAX_RESOLUTION_NOTES_LENGTH:
        return None
    return _envelope(
        ok=False,
        tool=tool,
        data=rejection_resolution_notes_max_length(
            actual_length=len(notes),
...
def _pre_server_verification_evidence_rejection(
    evidence: str | None,
    *,
    finding_id: str | None = None,
    tool: str = "review_findings",
) -> dict | None:
    """Named pre-server evidence-length gate — reject before server round-trip.

    Length is measured on the *normalized* (stripped) form — the same value
    the server stores — not the raw payload. Pydantic ``Field(max_length=N)``
    still measures raw strings and cannot strip; that outer bound is coarse
    advertising only. Callers that hit Field validation see pydantic's
    ``String should have at most N characters`` message, not this envelope.
    """
    if evidence is None:
        return None
    normalized = _normalize_optional_text(evidence)
    if normalized is None:
        return None
    if len(normalized) <= MAX_VERIFICATION_EVIDENCE_LENGTH:
        return None
    return _envelope(
        ok=False,
        tool=tool,
...
def review_findings(
    ...
    if isinstance(review_payload, ReviewFindingsUpdateOp):
        notes_reject = _pre_server_resolution_notes_rejection(
            review_payload.resolution_notes,
            finding_id=review_payload.finding_id,
            tool="update_review_finding",
        )
        if notes_reject is not None:
            return notes_reject
        evidence_reject = _pre_server_verification_evidence_rejection(
            review_payload.verification_evidence,
            finding_id=review_payload.finding_id,
            tool="update_review_finding",
        )
    ...
    if isinstance(review_payload, ReviewFindingsResolveOp):
        ...
        if notes_reject is not None:
            return notes_reject
        notes_alias_reject = _pre_server_resolution_notes_rejection(
            review_payload.notes,
            tool="resolve_review_findings",
        )
        if notes_alias_reject is not None:
            return notes_alias_reject
        evidence_reject = _pre_server_verification_evidence_rejection(
            review_payload.verification_evidence,
            tool="resolve_review_findings",
        )
        ...
        return _resolve_review_findings(
            ...
            actor=dump_actor(review_payload.actor),
            status=review_payload.status,
            finding_statuses=review_payload.finding_statuses,
        )
    ...
    if isinstance(review_payload, ReviewFindingsDispositionOp):
        ...
        notes_reject = _pre_server_resolution_notes_rejection(
            composed_notes,
            finding_id=review_payload.finding_id,
            tool="disposition_review_finding",
        )
        if notes_reject is not None:
            return notes_reject
        return _update_review_finding(
