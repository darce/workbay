...
from .current_task_rendering import _write_workspace_current_task_json
...
def _write_current_task_md_for_active_context(conn: sqlite3.Connection, fallback_task_ref: str) -> None:
    """Regenerate CURRENT_TASK.json after a review-findings write.

    The workspace-summary CURRENT_TASK writer treats ``task_ref`` as advisory
    only (see ``_write_workspace_current_task_json``). Do **not** call
    ``_resolve_workspace_handoff_row`` here while a write transaction may be
    open: that path reaches ``_raise_ambiguous`` → ``_classify_live_row`` →
    ``branch_is_merged`` / ``is_ancestor_of_ref`` and forks ``git`` under
    SQLite RESERVED (AUDITHOOK-TP01). On ambiguity the previous code caught
    ``ValueError`` and fell back to ``fallback_task_ref`` anyway, so the
    classification work was discarded — now we use the fallback directly and
    never fork git on this hottest write path.
    """
    _write_workspace_current_task_json(conn, fallback_task_ref)
...
def _canonical_task_ref_from_handoff(conn: sqlite3.Connection, task_ref: str) -> str | None:
    """Return the stored live or archived task_ref, matching case-insensitively.

    Findings and review-runs share this key space. Whitespace is stripped
    before lookup. Returns None when no known task exists.
    """
    ...
    rows = conn.execute(
        """
        SELECT task_ref, 0 AS source_order FROM handoff_state WHERE lower(task_ref) = lower(?)
        UNION ALL
        SELECT task_ref, 1 AS source_order FROM task_archives WHERE lower(task_ref) = lower(?)
    ...
    if not rows:
        return None
    return str(rows[0]["task_ref"])


def _canonical_task_ref(conn: sqlite3.Connection, task_ref: str | None) -> str:
    """Resolve active context, then apply the shared live-first identity map."""
    resolved = _resolve_task_ref(conn, task_ref)
    known = _canonical_task_ref_from_handoff(conn, resolved)
    if known is not None:
        return known
    # Unregistered harvester scopes keep their first stored spelling, but all
    # later writes and reads case-fold through that representative. This gives
    # an unknown task_ref one identity without rewriting user-facing labels.
    row = conn.execute(
        """
        SELECT task_ref FROM (
            SELECT task_ref, id, 0 AS source_order
            FROM review_findings WHERE lower(task_ref) = lower(?)
            UNION ALL
            SELECT task_ref, id, 1 AS source_order
            FROM review_runs WHERE lower(task_ref) = lower(?)
        )
    ...
    return str(row["task_ref"]) if row is not None else resolved.strip()


def _canonicalize_finding_path(raw: str) -> str | None:
    """Return a normalized workspace-relative finding path, or None if unplaceable."""
    ...
    candidate = (workspace / PurePosixPath(*resolved)).resolve(strict=False)
    try:
        return candidate.relative_to(workspace).as_posix()
    except ValueError:
        # An existing symlink component may point outside the workspace.
        return None
