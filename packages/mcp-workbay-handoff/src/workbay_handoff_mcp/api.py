...
def collect_stale_nonscratch_open_findings() -> dict:
    """Report open findings on non-scratch refs with no live row or status=done."""
    return core.collect_stale_nonscratch_open_findings()
...
def semantic_reinjection_packet(
    ...
    return _compaction_module.semantic_reinjection_packet(
        task_ref=task_ref,
...
def render_handoff(
    ...
    if kind == "current_task":
        ...
        if not task_ref:
            return core._envelope(ok=False, tool="render_handoff", data={"error": "task_ref required for plan_checklist"})
...
def _maybe_capture_write_rejection(tool_name: str, result: object, *, sub_operation: str | None = None) -> None:
    ...
    if isinstance(data, dict):
        summary = data.get("error") or data.get("state_error")
...
def _wrap_branch_mismatch_for_mcp(entry: ToolEntry) -> Callable[..., object]:
    ...
    def _wrapped(*args: object, **kwargs: object) -> object:
        try:
            result = _normalize_handler_result(entry.name, handler(*args, **kwargs))
