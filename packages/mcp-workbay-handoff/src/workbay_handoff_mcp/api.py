...
_PUBLISHED_BOOT_ROSTER: ContextVar[BootRoster | None] = ContextVar(
    "handoff_published_boot_roster", default=None
)
...
def filter_unserved_call_shaped_names(
    description: str, served: set[str], names: frozenset[str]
) -> str:
...
def _publishable_tool(entry: ToolEntry, roster: BootRoster) -> FunctionTool:
    ...
    if roster.effective_policy == "skill":
        description = filter_unserved_call_shaped_names(
            description, set(roster.served), HANDOFF_TOOL_NAMES
        )
