...
from workbay_protocol import StructuredSummary, resolve_env_alias
...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "archive": (
        ...
        "optionally close+archive closeable rows (dry-run by default); operation='retention' to "
...
ArchiveParam = Annotated[
    ArchiveOpArchive | ArchiveOpGc | ArchiveOpReap | ArchiveOpRetention | ArchiveOpGet,
    Field(discriminator="operation"),
]

_ARCHIVE_ADAPTER: TypeAdapter[ArchiveOpArchive | ArchiveOpGc | ArchiveOpReap | ArchiveOpRetention | ArchiveOpGet] = (
...
def _lifecycle_tool_entries() -> list[ToolEntry]:
    return [
        ...
        ToolEntry(
            ...
            cli_name="archive",
            cli_args=[
                ArgSpec("--operation", required=True, choices=["archive", "gc", "reap", "retention", "get"]),
                ArgSpec("--task-ref"),
                ArgSpec("--notes"),
...
def build_handoff_mcp(config: RuntimeConfig) -> FastMCP:
    ...
    mcp = FastMCP(
        "WorkBay Handoff MCP",
        instructions=(
            "You are connected to the WorkBay Handoff MCP server. "
