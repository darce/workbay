...
from workbay_protocol.tool_serving_index import HANDOFF_TOOL_NAMES

from . import compaction as _compaction_module
from . import continuation as _continuation_module
from . import core
...
from .config import RuntimeConfig, _resolve_primary_worktree_root
...
from .review_findings_updates import _attach_opportunistic_integrate_signal
from .runtime import configure_runtime as _configure_runtime
from .runtime import get_runtime_config, reset_runtime_config
from .shared_primitives import (
    DEFAULT_HANDOFF_LIMITS,
    HANDOFF_ACTIVE_STATUSES,
    LIVE_ACTIVE_STATUSES,
    WORKBAY_HANDOFF_ACTIVE_TASK_ENV,
...
from .shared_schema import (
    ...
    assert_boot_schema_compatible,
    bounded_resolve_active_task_ref,
    connect_handoff_db,
)
...
from .tool_roster import (
    BootRoster,
    record_boot_roster,
    resolve_handoff_boot_roster,
)
from .write_retry import LOCK_RETRY_WRITE_TOOLS, wrap_mcp_write_with_lock_retry
...
def _implicit_active_task_pin_error(
    *,
    task_ref: str | None,
    tool: str,
) -> dict | None:
    """Return a typed error envelope when an implicit active-task pin is invalid."""
    pinned = os.environ.get(WORKBAY_HANDOFF_ACTIVE_TASK_ENV, "").strip()
    if task_ref is not None or not pinned:
        return None
    try:
        with _get_db_connection() as conn:
            _resolve_workspace_handoff_row(conn, honor_active_task_pin=True)
...
@functools.wraps(core.get_handoff_state)
def get_handoff_state(*args: Any, **kwargs: Any) -> dict:
    """Read handoff state after validating an implicit active-task pin."""
    bound = inspect.signature(core.get_handoff_state).bind_partial(*args, **kwargs)
    pin_error = _implicit_active_task_pin_error(
        task_ref=bound.arguments.get("task_ref"),
        tool="get_handoff_state",
    )
    if pin_error is not None:
        return pin_error
    return core.get_handoff_state(*args, **kwargs)
...
@functools.wraps(core.load_session)
def load_session(*args: Any, **kwargs: Any) -> dict:
    """Load compound session state after validating an implicit task pin."""
    bound = inspect.signature(core.load_session).bind_partial(*args, **kwargs)
    pin_error = _implicit_active_task_pin_error(
        task_ref=bound.arguments.get("task_ref"),
        tool="load_session",
    )
    if pin_error is not None:
        return pin_error
    return core.load_session(*args, **kwargs)
...
def configure_runtime(config: RuntimeConfig) -> RuntimeConfig:
    """Configure the API runtime without materializing state on disk."""
...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "find_related_prior_work": (
        "Find advisory cosine neighbors of one raw phrase or one finding_id. "
        "Exactly one of text or finding_id is required. "
        "task_ref excludes the current task so results are prior work, not siblings. "
        "Do not use this for IDs or tokens — use search_handoff. "
        "Do not invent FTS synonyms first. Results are advisory only: they never "
        "gate, filter, auto-close, or auto-link."
    ),
    ...
    "continuation": (
        "Save or load a typed cross-session continuation packet for a task (and optional lane). "
        "Set operation to 'save' to upsert schema'd sections (done_do_not_redo, next_actions, "
        "verified_anchors, gotchas; at least one required) with supersedes-lineage, or 'load' to "
        "fetch the newest packet for the task/lane key or a specific packet by id. Absent load "
        "returns ok with packet=null. Packets reuse the artifacts sidecar (source_kind=continuation)."
    ),
    "search_handoff": "Search decisions, findings, blockers, actions, and verified tests by keyword with BM25 ranking. The default scope='task' filters to an explicit task_ref or the implicitly resolved active task; pass scope='all' to opt out and search across all task refs. If implicit active-task resolution is ambiguous, search degrades to scope='all' and reports scope_degraded_reason. Every successful payload reports scope_applied and scoped_task_ref so an empty result reveals where the tool searched. Pass detail='summary' to truncate snippets and fields='record_type,snippet' to project per-result fields. Pass decision_fields=['decision','branch','commit_sha','lane_id',...] to merge decision-table columns onto decision rows for discovery use cases (allowlist; non-decision rows unchanged). Pass include_system=True to include decision rows with decision_origin='system' (default False filters them out in FTS before LIMIT; NULL origins remain searchable). Keyword/ID only. On zero hits do not invent synonyms; pass the original phrase to find_related_prior_work.",
    "list_tool_domains": "List this server's declarative tool domains without parameter schemas.",
    "activate_tool_domain": (
        "Persist domain activation onto the live task for the next server boot. "
        "This does not change the in-session tool roster."
    ),
    "list_handoff_rows": (
...
class ReportBlockerEvent(BaseModel):
    ...
    lane_id: Annotated[
        str | None,
        Field(description="Lane attributed to a newly added blocker."),
    ] = None
...
class ReviewRunsRecordOp(BaseModel):
    ...
    max_review_rounds: Annotated[
        ...
        Field(
            description=(
                "Optional tighter bound on verdict-bearing rows per task, "
                "subject, and review mode. Values above the module default "
                "are clamped; a caller can only tighten the bound."
            ),
        ),
    ] = 5
...
class ArtifactsSearchOp(BaseModel):
    ...
    queries: Annotated[
        ...
        Field(
            description=(
                "Optional search queries. Omit or pass empty to list sources. Words within one query are ANDed; "
                "misses are typed via empty_reason: no_match, no_query, corpus_empty, scope_excluded_all, "
                "or query_over_constrained."
            )
        ),
    ] = None
    task_ref: TaskRefParam = None
...
class ContinuationSaveOp(BaseModel):
    operation: Literal["save"]
    task_ref: TaskRefParam = None
    lane_id: Annotated[
        str | None,
        Field(description="Optional lane scope. Packets are upserted per (task_ref, lane_id)."),
    ] = None
    done_do_not_redo: Annotated[
        str | None,
        Field(description="Markdown: completed work the next agent must not re-do."),
    ] = None
    next_actions: Annotated[
        str | None,
        Field(description="Markdown: ordered next actions for the cold-start agent."),
    ] = None
    verified_anchors: Annotated[
        str | None,
        Field(description="Markdown: verified paths, commands, or evidence anchors."),
    ] = None
    gotchas: Annotated[
        str | None,
        Field(description="Markdown: traps, non-obvious constraints, and coordination notes."),
    ] = None


class ContinuationLoadOp(BaseModel):
    operation: Literal["load"]
    task_ref: TaskRefParam = None
    lane_id: Annotated[
        str | None,
        Field(description="Optional lane scope for newest-packet lookup."),
    ] = None
    packet_id: Annotated[
        str | None,
        Field(description="Optional specific packet id. When set, loads that packet (or empty if absent)."),
    ] = None


ContinuationParam = Annotated[
    ContinuationSaveOp | ContinuationLoadOp,
    Field(discriminator="operation"),
]

_CONTINUATION_ADAPTER: TypeAdapter[ContinuationSaveOp | ContinuationLoadOp] = TypeAdapter(ContinuationParam)
...
class ArchiveOpReapDone(BaseModel):
    ...
    task_ref: Annotated[
        ...
        Field(
            description=(
                "Optional task_ref scope; when set classification skips other rows "
                "before bucketing so every reported bucket (closeable/active/ambiguous) "
                "and the reap set include only that row."
            ),
        ),
    ] = None
...
def _validate_continuation(
    payload: ContinuationParam,
) -> ContinuationSaveOp | ContinuationLoadOp:
    return _CONTINUATION_ADAPTER.validate_python(payload)
...
def set_handoff_state(
    ...
    status: Annotated[
        ...
        Field(
            description=(
                "Active handoff status, typically in_progress, blocked, review, or done. "
                "On create, omit to default to in_progress. On update, omit to preserve the stored status."
            )
        ),
    ] = None,
    ...
    if isinstance(result, dict) and result.get("ok"):
        _attach_opportunistic_integrate_signal(result, task_ref)
    return result
...
def update_task_status(
    ...
    """Library-only alias. Not a served MCP tool.

    Delegates to the same producer as ``set_handoff_state(..., status_only=True)``.
    The envelope ``tool`` field is the served identity ``set_handoff_state``.
    """
...
def record_decision(
    ...
    decision_origin: Annotated[
        Literal["agent", "system"] | None,
        Field(
            description=(
                "Optional origin stamp: 'agent' (default via trigger when omitted) or "
                "'system' for machine-written rows reaped by gc_system_decisions."
            )
        ),
    ] = None,
    refresh_rationale_on_conflict: Annotated[
        bool,
        Field(
            description=(
                "When True, a conflict on (task_ref, decision, session) refreshes the "
                "existing row's rationale and any supplied descriptive provenance "
                "(decision_origin); created_at stays frozen. Default False keeps "
                "append-only DO NOTHING idempotency."
            )
        ),
    ] = False,
) -> dict:
    return _core_record_decision(
        ...
        refresh_rationale_on_conflict=refresh_rationale_on_conflict,
    )
...
def record_event(
    ...
    """Record a typed decision, test_result, blocker, or error event.

    The decision path does not accept ``slice_number``, ``decision_origin``, or
    ``refresh_rationale_on_conflict``; those parameters are ``record_decision``-only
    (PLAN0181-S2EVENTKW-01).
    """
    ...
    if isinstance(event_payload, RecordDecisionEvent):
        ...
        result = _core_report_blocker(
            ...
            actor=resolved_actor,
            task_ref=event_payload.task_ref,
            lane_id=event_payload.lane_id,
        )
    if isinstance(result, dict) and result.get("ok"):
        _attach_opportunistic_integrate_signal(result, event_payload.task_ref)
    return result
...
def latest_lane_landing(
    *,
    lane_id: str,
    task_ref: str,
) -> dict:
    """Return the newest lane-landing decision row (commit SHA) recorded for a lane.

    Keyword-only so parameter order cannot silently swap the two opaque strings.
    ``task_ref`` is required: the only intended caller always knows both values.
    An absent landing is ``ok=True`` with ``landing=None``.
    """

    return core.latest_lane_landing(lane_id=lane_id, task_ref=task_ref)


def latest_reclaim_candidate(
    *,
    lane_id: str,
    task_ref: str,
) -> dict:
    """Return the newest reclaim-candidate decision row recorded for a lane.

    Keyword-only so parameter order cannot silently swap the two opaque strings.
    ``task_ref`` is required: the only intended caller always knows both values.
    An absent candidate is ``ok=True`` with ``candidate=None``.
    """

    return core.latest_reclaim_candidate(lane_id=lane_id, task_ref=task_ref)


def latest_branch_reclaim_candidate(
    *,
    lane_id: str,
    task_ref: str,
) -> dict:
    """Return the newest branch-reclaim-candidate decision for a lane.

    An absent candidate is ``ok=True`` with ``candidate=None``.
    """

    return core.latest_branch_reclaim_candidate(lane_id=lane_id, task_ref=task_ref)


def _related_prior_semantic_degrade_reason(
    *,
    embeddings_mode: str,
    status: object,
    note: object,
) -> str | None:
    if embeddings_mode != "verified" or status != "provider_unavailable":
        return None
...
def find_related_prior_work(
    *,
    text: Annotated[
        str | None,
        Field(description="Free-text like-this query. Exactly one of text or finding_id is required."),
    ] = None,
    finding_id: Annotated[
        str | None,
        Field(description="Use the stored embedding for this review finding. No model load at query time."),
    ] = None,
    task_ref: TaskRefParam = None,
    include_task_refs: Annotated[
        list[str] | None,
        Field(
            description="Optional candidate membership filter by task_ref (repeatable). Does not invert task_ref exclude."
        ),
    ] = None,
    include_plan_paths: Annotated[
        list[str] | None,
        Field(description="Optional candidate membership filter by task_plan_path (repeatable)."),
    ] = None,
    include_statuses: Annotated[
        list[str] | None,
        Field(description="Optional candidate membership filter by handoff status (repeatable)."),
    ] = None,
    limit: Annotated[int, Field(description="Maximum advisory hits when status is ok_with_results.")] = 5,
    min_score: Annotated[
        float | None,
        Field(description="Minimum cosine similarity floor (inclusive). None uses the library default."),
    ] = None,
    ...
    """Return prior findings/decisions that resemble the query (advisory context only).

    Keyword-only. Exactly one of ``text`` or ``finding_id`` is required.
    ``finding_id`` mode uses the already-stored embedding (no model load).
    Free-text mode requires a configured provider and degrades to the typed
    empty state ``provider_unavailable`` when absent.

    Results never gate, filter, auto-close, or auto-link anything (CAL-03).
    Always inspect ``status`` — never treat an empty ``results`` list alone as
    "nothing similar" (AXI-5 typed empty states).
    """
    embeddings_mode = _compaction_module._embeddings_mode_from_ledger()
    try:
        result = core.find_related_prior_work(
            text=text,
            finding_id=finding_id,
            task_ref=task_ref,
            include_task_refs=include_task_refs,
            include_plan_paths=include_plan_paths,
            include_statuses=include_statuses,
            limit=limit,
            min_score=min_score,
        )
    except ValueError as exc:
        if "exactly one of" not in str(exc):
            raise
        result = {
            "status": "no_results_above_threshold",
            "results": [],
            "min_score": 0.35 if min_score is None else min_score,
            "query_mode": None,
            "query_model_id": None,
            "note": "exactly_one_of_text_or_finding_id",
        }
    result["ok"] = True
    result["task_ref"] = task_ref
    result["embeddings_mode"] = embeddings_mode
    result["semantic_degrade_reason"] = _related_prior_semantic_degrade_reason(
        embeddings_mode=embeddings_mode,
        status=result.get("status"),
        note=result.get("note"),
    )
    scoped = isinstance(task_ref, str) and task_ref.strip() != ""
    record_orientation_read_once(
        tool="find_related_prior_work",
        task_ref=task_ref.strip() if scoped else "-",
        resolution_outcome="explicit" if scoped else "omitted",
        source=source,
        session=session,
        read_profile="find_related_prior_work",
    )
    return result
...
def continuation(
    payload: Annotated[
        ...
        Field(
            description=(
                "Typed continuation payload. operation selects 'save' (upsert schema'd sections "
                "with supersedes-lineage) or 'load' (newest packet for task/lane, or by packet_id)."
            )
        ),
    ],
) -> dict:
    op = _validate_continuation(payload)
    if isinstance(op, ContinuationSaveOp):
        return _continuation_module.save_continuation(
            task_ref=op.task_ref,
            lane_id=op.lane_id,
            done_do_not_redo=op.done_do_not_redo,
            next_actions=op.next_actions,
            verified_anchors=op.verified_anchors,
            gotchas=op.gotchas,
        )
    return _continuation_module.load_continuation(
        task_ref=op.task_ref,
        lane_id=op.lane_id,
...
def archive(
    ...
    if isinstance(op, ArchiveOpArchive):
        ...
        envelope = core.reap_done_nonscratch_handoff_rows(apply=op.apply, task_ref=op.task_ref)
...
def review_runs(
    ...
    if isinstance(review_payload, ReviewRunsRecordOp):
        return _core_record_review_run(
            ...
            max_review_rounds=review_payload.max_review_rounds,
        )
...
# Actor-field keys accepted inside a rationale <actor> payload (implementation note S2).
_RATIONALE_ACTOR_FIELDS = frozenset(
    {"agent", "model", "model_label", "reasoning_level", "branch", "commit_sha", "lane_id"}
)
_RATIONALE_ACTOR_OPEN_RE = re.compile(r"(?i)<actor>")
_RATIONALE_ACTOR_CLOSE_RE = re.compile(r"(?i)</actor>")
_RATIONALE_ACTOR_NESTED_TAG_RE = re.compile(r"<\s*/?\s*[A-Za-z_]")


def parse_and_apply_rationale_actor(
    rationale: str | None,
    actor: ActorParam | None,
    *,
    task_ref: str | None = None,
) -> tuple[str | None, ActorParam | None, dict[str, Any] | None, dict[str, Any] | None]:
    """Parse optional ``<actor>...</actor>`` from a close_slice rationale.

    Returns ``(rationale, actor, receipt_info, rejection_data)``:

    - No ``<actor>`` tag → inputs unchanged, ``receipt_info`` and
      ``rejection_data`` are ``None``.
    - Well-formed tag + top-level ``actor`` omitted → strip the span, apply the
      payload as the actor override, ``receipt_info`` is the applied actor dict.
    - Well-formed tag + explicit top-level ``actor`` → strip the span, keep the
      explicit param, ``receipt_info`` is a warning note (no silent override).
    - Malformed / nested / unclosed / empty payload → ``rejection_data`` is the
      existing ``rejection_close_slice_rationale_xml`` envelope (fail closed).

    Never silently defaults attribution when a tag is present but unusable.
    """
    if rationale is None:
        return rationale, actor, None, None

    open_match = _RATIONALE_ACTOR_OPEN_RE.search(rationale)
    if open_match is None:
        return rationale, actor, None, None

    from .structured_rejections import rejection_close_slice_rationale_xml  # noqa: PLC0415

    # RG-0146-03: an <actor> tag inside a fenced code block is documentation,
    # not attribution — but silently ignoring it would store the tag verbatim
    # (the pollution class) and silently applying it would misattribute. Fail
    # closed instead: odd number of ``` fences before the open tag = inside a
    # fence → structured rejection, caller rewrites the example un-tagged.
    if rationale.count("```", 0, open_match.start()) % 2 == 1:
        return (
            rationale,
            actor,
            None,
            rejection_close_slice_rationale_xml(rejected_tag="<actor>", task_ref=task_ref),
        )

    def _reject() -> tuple[str | None, ActorParam | None, dict[str, Any] | None, dict[str, Any]]:
        return (
            rationale,
            actor,
            None,
    ...
    close_match = _RATIONALE_ACTOR_CLOSE_RE.search(rationale, open_match.end())
    if close_match is None:
        return _reject()

    payload = rationale[open_match.end() : close_match.start()]
    stripped_rationale = rationale[: open_match.start()] + rationale[close_match.end() :]

    payload_stripped = payload.strip()
    if not payload_stripped:
        return _reject()

    # BR-0146-S2-01: exactly one actor pair is consumable. Any residual
    # <actor>/</actor> token after stripping the first span would survive the
    # (actor-free) anti-pattern scan and pollute the stored rationale — the
    # #1507/#1508 class this parser replaced. Fail closed on multi-tag input.
    if _RATIONALE_ACTOR_OPEN_RE.search(stripped_rationale) or _RATIONALE_ACTOR_CLOSE_RE.search(stripped_rationale):
        return _reject()

    override: dict[str, str] | None = None
    if payload_stripped.startswith("{"):
        try:
            parsed = json.loads(payload_stripped)
        except json.JSONDecodeError:
            return _reject()
        if not isinstance(parsed, dict):
            return _reject()
        # Fail closed on nested XML/tags or unknown / non-string fields.
        if not set(parsed.keys()).issubset(_RATIONALE_ACTOR_FIELDS):
            return _reject()
        if any(value is not None and not isinstance(value, str) for value in parsed.values()):
            return _reject()
        override = {key: value for key, value in parsed.items() if isinstance(value, str) and value}
        if not override:
            return _reject()
    else:
        # Bare non-empty string → agent override. Nested tags fail closed.
        if _RATIONALE_ACTOR_NESTED_TAG_RE.search(payload_stripped):
            return _reject()
        override = {"agent": payload_stripped}

    if actor is not None:
        receipt: dict[str, Any] = {
            "warning": (
                "explicit top-level actor param takes precedence; "
                "rationale <actor> tag was stripped without applying override"
            ),
            "stripped_override": override,
        }
        return stripped_rationale, actor, receipt, None

    return stripped_rationale, override, override, None
...
def close_slice(
    ...
    # implementation note S2: consume well-formed <actor>...</actor> before the
    # anti-pattern scan so attribution can be applied (never silently
    # defaulted). Malformed actor payloads return the existing structured
    # rejection. Remaining scan still hard-rejects <changed_files> tags
    # (silent scope pollution; decisions #1507/#1508 class).
    rationale, actor, rationale_actor_receipt, actor_rejection = parse_and_apply_rationale_actor(
        rationale,
        actor,
        task_ref=task_ref,
    )
    if actor_rejection is not None:
        return core._envelope(
            ok=False,
            tool="close_slice",
            data=actor_rejection,
            task_ref=task_ref,
        )

    if rationale is not None:
        # internal: case-insensitive scan so <CHANGED_FILES>, etc. are
        # caught. Constant lives next to close_slice_requirements() so
        # requirements-on-read and the guard share a single source
        # (internal). Actor tags are handled above.
    ...
    if isinstance(result, dict) and result.get("ok"):
        _attach_opportunistic_integrate_signal(result, task_ref)
        if rationale_actor_receipt is not None:
            data = result.get("data")
            if isinstance(data, dict):
                data["rationale_actor_applied"] = rationale_actor_receipt
            else:
                result["rationale_actor_applied"] = rationale_actor_receipt
    return result
    ...
    return [
        ...
        ToolEntry(
            ...
            cli_args=[
                ...
                ArgSpec("--source-task-ref", dest="source_task_refs", action="append"),
                ArgSpec("--target-task-ref"),
                ArgSpec("--no-retire-sources", dest="retire_sources", action="store_false"),
    ...
    return [
        ...
        ToolEntry(
            ...
            cli_args=[
                ...
                ArgSpec("--task-ref"),
                ArgSpec("--notes"),
                # Deliberate CLI/library default divergence, kept per implementation note
                # [API-09]: store_true means the flagless CLI default is False, so
                # every caller that omits the flag keeps today's semantics and does
                # not reach the DELETE FROM handoff_state in archive_task_state
                # (whose own library default is clear_active_if_matches=True).
                # Do NOT "fix" this by flipping the default — 0178 revision 2 chose
                # option (b): leave the published default alone and have the
                # terminal caller (task_finish._archive) opt in by adding
                # --clear-active-if-matches to its argv. Opting a caller in is the
                # sanctioned change; changing this default is the breaking one.
        ...
        ToolEntry(
            "continuation",
            continuation,
            TOOL_DESCRIPTIONS["continuation"],
            profile="extended",
            cli_name="continuation",
            cli_args=[
                ArgSpec("--operation", required=True, choices=["save", "load"]),
                ArgSpec("--task-ref"),
                ArgSpec("--lane-id"),
                ArgSpec("--packet-id", dest="packet_id"),
                ArgSpec("--done-do-not-redo", dest="done_do_not_redo"),
                ArgSpec("--next-actions", dest="next_actions"),
                ArgSpec("--verified-anchors", dest="verified_anchors"),
                ArgSpec("--gotchas"),
            ],
            surface_class="action",
        ...
        ToolEntry(
            "find_related_prior_work",
            find_related_prior_work,
            TOOL_DESCRIPTIONS["find_related_prior_work"],
            profile="extended",
            cli_name="find-related-prior-work",
            cli_args=[
                ArgSpec("--text"),
                ArgSpec("--finding-id", dest="finding_id"),
                ArgSpec("--task-ref"),
                ArgSpec("--include-task-ref", action="append", dest="include_task_refs"),
                ArgSpec("--include-plan-path", action="append", dest="include_plan_paths"),
                ArgSpec("--include-status", action="append", dest="include_statuses"),
                ArgSpec("--limit", type=int),
                ArgSpec("--min-score", type=float, dest="min_score"),
            ],
            surface_class="query",
            entity_family="handoff_state",
        ),
        ...
        ToolEntry(
            ...
            cli_args=[
                ...
                ArgSpec(
                    "--last-injected-continuation-id",
                    dest="last_injected_continuation_id",
                    help="Dedupe key for data.continuation when the caller already injected this packet.",
                ),
            ],
            surface_class="query",
        ...
        ToolEntry(
            ...
            cli_args=[
                ...
                ArgSpec(
                    ...
                    dest="include_system",
                    help=(
                        "Include decision rows with decision_origin='system' in search results. "
                        "Default omits them (NULL origins still match)."
                    ),
                ),
            ],
        ),
...
_HANDOFF_OPS_TOOL_NAMES = frozenset(
    {
        "archive",
        "audit_decision_ids",
        "collect_stale_nonscratch_open_findings",
...
def _domain_tool_map() -> dict[str, list[str]]:
    """Derive family domains from their registry builders minus the ops overlay."""
    families = (
        ("task_state", _task_state_tool_entries),
        ("review", _review_tool_entries),
        ("lifecycle", _lifecycle_tool_entries),
        ("artifact", _artifact_tool_entries),
    )
    domain_map = {
        name: [entry.name for entry in builder() if entry.name not in _HANDOFF_OPS_TOOL_NAMES]
        for name, builder in families
    }
    domain_map["ops"] = sorted(_HANDOFF_OPS_TOOL_NAMES)
    return domain_map


def list_tool_domains(*, include_ops: bool = False) -> dict:
    domain_map = _domain_tool_map()
    names = list(domain_map) if include_ops else [name for name in domain_map if name != "ops"]
    boot = _PUBLISHED_BOOT_ROSTER.get() or BootRoster("all", None, False, HANDOFF_TOOL_NAMES)
    return core._envelope(
        ok=True,
        tool="list_tool_domains",
        data={
            "domains": [{"name": name, "tools": domain_map[name]} for name in names],
            "effective_policy": boot.effective_policy,
            "resolved_task_ref": boot.resolved_task_ref,
            "floor_taken": boot.floor_taken,
        },
    )


MAX_SKILL_SLUG_LENGTH = 1024
# Keep identical to workbay_orchestrator_mcp.api.MAX_SKILL_SLUG_LENGTH.
...
def activate_tool_domain(
    *, domain: str | None = None, skill_slugs: list[str] | None = None, task_ref: str | None = None
    ...
    if task_ref is not None and (not isinstance(task_ref, str) or not task_ref.strip()):
        return core._envelope(
            ok=False,
            tool="activate_tool_domain",
            data={"error": "invalid_task_ref", "task_ref": task_ref},
    ...
    if domain is None and not skill_slugs:
        return core._envelope(ok=False, tool="activate_tool_domain", data={"error": "empty_write"})
    if domain is not None and domain not in domain_map:
        return core._envelope(
            ok=False,
            tool="activate_tool_domain",
            data={"error": "unknown_domain", "domain": domain, "valid": list(domain_map)},
        )
    ...
    with _get_db_connection(begin_immediate=True) as conn:
        ...
        if resolved_task_ref is None:
            try:
                ...
                resolved_task_ref = resolve_active_task_ref_for_hook(conn, strict=False).task_ref
            except (ValueError, sqlite3.Error):
                resolved_task_ref = None
        live = None
        if resolved_task_ref is not None:
            placeholders = ",".join("?" for _ in LIVE_ACTIVE_STATUSES)
            live = conn.execute(
                f"SELECT 1 FROM handoff_state WHERE task_ref = ? AND status IN ({placeholders})",
                (resolved_task_ref, *LIVE_ACTIVE_STATUSES),
            ).fetchone()
        if live is None or resolved_task_ref is None:
            return core._envelope(ok=False, tool="activate_tool_domain", data={"error": "no_live_task"})
        ...
        if stored_slugs is None:
            return core._envelope(
                ...
                data={"error": "invalid_roster_json", "column": "skill_slugs"},
            )
        ...
        if stored_domains is None:
            return core._envelope(
                ...
                tool="activate_tool_domain",
                task_ref=resolved_task_ref,
                data={"error": "invalid_roster_json", "column": "activated_domains"},
            )
        merged_slugs = _ordered_union(stored_slugs, skill_slugs or [])
        merged_domains = _ordered_union(stored_domains, [domain] if domain is not None else [])
        slug_value = (
            json.dumps(merged_slugs)
            if skill_slugs is not None or (row is not None and row["skill_slugs"] is not None)
            else None
        )
        domain_value = (
            json.dumps(merged_domains)
            if domain is not None or (row is not None and row["activated_domains"] is not None)
            else None
        )
        conn.execute(
            """
            ...
            """,
            (resolved_task_ref, slug_value, domain_value),
        )
        return core._envelope(
            ok=True,
            tool="activate_tool_domain",
...
def _catalog_tool_entries() -> list[ToolEntry]:
    return [
        ToolEntry(
            "list_tool_domains",
            list_tool_domains,
            TOOL_DESCRIPTIONS["list_tool_domains"],
            surface_class="query",
            entity_family="session",
        ),
        ToolEntry(
            "activate_tool_domain",
            activate_tool_domain,
            TOOL_DESCRIPTIONS["activate_tool_domain"],
            surface_class="action",
            entity_family="session",
        ),
    ]
...
def _render_current_task_result(
    ...
    reflects the caller-requested task. An explicitly supplied task_ref may
    legitimately have no handoff_state row yet; that projection is skipped.
    A configured active-task pin remains fail-closed and reports the pin
    diagnostic if its row cannot be resolved.
    """
    ...
    explicit_task_ref = task_ref is not None
    configured_pin = os.environ.get("WORKBAY_HANDOFF_ACTIVE_TASK", "").strip()
    resolved_task_ref = task_ref
    tiebreak_note: str | None = None
    tiebreak_candidates: tuple[str, ...] | None = None
    if resolved_task_ref is None:
        with core._get_db_connection() as conn:
            from .shared_primitives import resolve_active_task_ref_ex  # noqa: PLC0415

            try:
                resolution = resolve_active_task_ref_ex(conn)
            except ValueError:
                if configured_pin:
                    raise
                resolution = None
            if resolution is not None:
                ...
                tiebreak_note = resolution.tiebreak_note
                tiebreak_candidates = resolution.tiebreak_candidates
    ...
    if write_file:
        ...
        if resolved_task_ref is not None:
            try:
                ...
                if configured_pin and not explicit_task_ref:
                    raise ValueError(
                        f"WORKBAY_HANDOFF_ACTIVE_TASK={configured_pin!r} does not match any handoff_state row"
                    ) from exc
                pass
...
def render_handoff(
    ...
    if kind == "current_task":
        try:
            result = _render_current_task_result(
                task_ref=task_ref,
...
# The read tools whose MCP tool-boundary invocations count as orientation
...
def _wrapped_handler_in_orientation_boundary(
    ...
    Only the registered read tools mark the boundary; every other tool
...
_PUBLISHED_BOOT_ROSTER: ContextVar[BootRoster | None] = ContextVar("handoff_published_boot_roster", default=None)

_MCP_CONTENT_CARRIER_ENV = "WORKBAY_HANDOFF_MCP_CONTENT_CARRIER"
_MCP_CONTENT_CARRIER_BOTH = "both"
_MCP_CONTENT_CARRIER_STRUCTURED = "structured"
MCPContentCarrier = Literal["both", "structured"]


def _resolve_mcp_content_carrier(content_carrier: str | None = None) -> MCPContentCarrier:
    """Resolve the registration-time MCP result-carrier policy.

    MCP 2025-11-25, server/tools, says: "For backwards compatibility, a tool
    that returns structured content SHOULD also return the serialized JSON in
    a TextContent block." FastMCP 3.4.2 implements that recommendation in
    ``Tool.convert_result``: ``content = _convert_to_content(raw_value,
    serializer=self.serializer)`` is followed by ``return ToolResult(
    content=content, structured_content=structured)``. Keep that dual carrier
    as the default; the explicit ``structured`` mode is an opt-out for clients
    that already read structured results.
    """
    raw = (
        content_carrier
        if content_carrier is not None
        else resolve_env_alias(_MCP_CONTENT_CARRIER_ENV, default=_MCP_CONTENT_CARRIER_BOTH)
    )
    if not isinstance(raw, str):
        raise ValueError(f"{_MCP_CONTENT_CARRIER_ENV} must be 'both' or 'structured'; got {raw!r}")
    normalized = raw.strip().lower()
    if normalized not in {_MCP_CONTENT_CARRIER_BOTH, _MCP_CONTENT_CARRIER_STRUCTURED}:
        raise ValueError(f"{_MCP_CONTENT_CARRIER_ENV} must be 'both' or 'structured'; got {raw!r}")
    return cast(MCPContentCarrier, normalized)


def _wrap_structured_only_result(handler: Callable[..., Any]) -> Callable[..., Any]:
    """Return an MCP-only wrapper that omits the duplicate text carrier.

    ``ToolResult`` is FastMCP's public result type. Returning it from a
    registration wrapper bypasses the library's automatic ``_convert_to_content``
    call while preserving the handler signature and generated output schema.
    The wrapper remains synchronous so FastMCP keeps its normal worker-thread
    execution policy for synchronous handoff handlers.
    """
...
def _wrap_branch_mismatch_for_mcp(
    entry: ToolEntry, handler_override: Callable[..., Any] | None = None
) -> Callable[..., object]:
    """Return an MCP-only wrapper that normalizes typed connect/write refusals."""

    handler = handler_override or entry.handler
    signature = inspect.signature(handler)
...
def filter_unserved_call_shaped_names(description: str, served: set[str], names: frozenset[str]) -> str:
    """Remove unserved this-server ``name(...)`` spans from a description."""
    result = description
    for match in reversed(list(re.finditer(r"\b([A-Za-z_][A-Za-z0-9_]*)\s*\(", description))):
        if match.group(1) in served or match.group(1) not in names:
            continue
        depth = 1
        cursor = match.end()
        quote: str | None = None
        escaped = False
        while cursor < len(description) and depth:
        ...
        if start and description[start - 1] == "`" and end < len(description) and description[end] == "`":
            start -= 1
            end += 1
...
def _instructions_for_roster(roster: BootRoster) -> str:
    instructions = (
        ...
        "## Key Tool Guidance\n\n"
        "- `load_session`: use at session start to get state + open findings + "
        "newest continuation packet (data.continuation) in one call.\n"
    ...
    if roster.effective_policy == "all":
        return instructions

    served = set(roster.served)
    kept: list[str] = []
    for line in instructions.splitlines(keepends=True):
        mentioned = {
            match.group(1)
            for match in re.finditer(r"`([A-Za-z_][A-Za-z0-9_]*)", line)
            if match.group(1) in HANDOFF_TOOL_NAMES
        }
        if mentioned - served:
            continue
        kept.append(line)
    filtered = filter_unserved_call_shaped_names("".join(kept), served, HANDOFF_TOOL_NAMES)
    if roster.floor_taken:
        filtered += (
            "\n\nThis boot took the bootstrap floor; served tools are the in_progress default union "
            "plus catalog and ALWAYS_SERVE."
        )
    return filtered


def _publishable_tool(
    entry: ToolEntry,
    roster: BootRoster,
    *,
    content_carrier: MCPContentCarrier = _MCP_CONTENT_CARRIER_BOTH,
) -> FunctionTool:
    """Build the sole per-entry object passed to ``mcp.add_tool``."""
    handler_override: Callable[..., Any] | None = None
    if entry.name == "list_tool_domains":

        @functools.wraps(entry.handler)
        def _with_boot_roster(*args: object, **kwargs: object) -> object:
            token = _PUBLISHED_BOOT_ROSTER.set(roster)
            try:
                return entry.handler(*args, **kwargs)
            finally:
                _PUBLISHED_BOOT_ROSTER.reset(token)

        handler_override = _with_boot_roster
    tool = _wrap_branch_mismatch_for_mcp(entry, handler_override)
    if entry.name in LOCK_RETRY_WRITE_TOOLS:
        tool = wrap_mcp_write_with_lock_retry(
            tool,
            tool=entry.name,
            db_path_resolver=lambda: get_runtime_config().db_path,
        )
    # The shipped policy=all path predates explicit FunctionTool construction.
    # FastMCP therefore sourced descriptions from the wrapped handler docstring,
    # which differs from the registry copy for several long-lived tools. Preserve
    # those bytes on the default path; skill policy may use the registry text as
    # the input to its call-shaped-name filter.
    description = (
        (entry.handler.__doc__ or entry.description) if roster.effective_policy == "all" else entry.description
    )
    if entry.deprecated_since is not None:
        description = f"[DEPRECATED since {entry.deprecated_since}] {description}"
    if roster.effective_policy == "skill":
        description = filter_unserved_call_shaped_names(description, set(roster.served), HANDOFF_TOOL_NAMES)
    if content_carrier == _MCP_CONTENT_CARRIER_STRUCTURED:
        tool = _wrap_structured_only_result(tool)
    return FunctionTool.from_function(tool, name=entry.name, description=description)


def build_handoff_mcp(config: RuntimeConfig, *, content_carrier: str | None = None) -> FastMCP:
    resolved_content_carrier = _resolve_mcp_content_carrier(content_carrier)
    # Fail-closed BOOT gate ([OBS-08], [DBG-05]): refuse to build the serving MCP
    # when the on-disk DB is AHEAD of this package (stale copy-editable schema
    # skew) instead of booting into a "connected but every tool errors" state.
    # build_handoff_mcp is the serve-only choke point (cli.py serve-stdio/http);
    # doctor / init-state / migrate never call it, so recovery stays unblocked.
    assert_boot_schema_compatible(config.db_path)
    configure_runtime(config)
    roster = resolve_handoff_boot_roster(
        policy=config.tool_roster_policy,
        db_path=config.db_path,
        domain_tool_map=_domain_tool_map(),
        bounded_resolver=bounded_resolve_active_task_ref,
    )
    record_boot_roster(roster)
    mcp = FastMCP(
        f"{BRAND_NAME} Handoff MCP",
        instructions=_instructions_for_roster(roster),
    )
    _apply_tool_descriptions()
    for entry in _build_tool_registry():
        if entry.name not in roster.served:
            continue
        ...
        # tools that take BEGIN IMMEDIATE before side effects. A busy_timeout
        # miss from a live peer is retried with jittered backoff; exhaustion
        # returns typed db_busy naming the registry holder (write_retry.py).
        mcp.add_tool(_publishable_tool(entry, roster, content_carrier=resolved_content_carrier))
...
def _check_fts_index_health(db_path: Path) -> dict[str, Any]:
    # read_only opens via URI mode=ro, which raises OperationalError when the
    # file is absent (unlike the RW factory, which would create it). Doctor is
    # the first consumer of this check and is exactly the tool operators run
    # when handoff.db is missing — return the same missing-table remediation
    # shape main produced, without opening a connection (so we never create a
    # stray empty database file). [RES-01][OBS-04]
    if not Path(db_path).exists():
        table_counts = {tbl: -1 for tbl in _FTS_TABLES}
        missing = list(_FTS_TABLES)
        return {
            "ok": False,
            "tables": table_counts,
            "remediation": (
    ...
    # closing(): Connection.__exit__ only commits/rolls back; it does not close.
...
def _check_review_findings_finding_id_index_health(db_path: Path) -> dict[str, Any]:
    """Read-only doctor facet: fail when ``idx_review_findings_finding_id`` is absent.

    Does not re-enter ``BEGIN IMMEDIATE`` or call the ensure helper. A
    34-stamped operator file whose btree failed to build twice is otherwise
    indistinguishable from a cured one via schema_version / FTS doctor
    checks. Missing file or missing ``review_findings`` table is the same
    class of failure as a missing btree.
    """
    index_name = _REVIEW_FINDINGS_FINDING_ID_INDEX
    if not Path(db_path).exists():
        return {
            "ok": False,
            "index": index_name,
            ...
            "stat1_named": False,
            "remediation": (
                f"{index_name} is missing because {db_path} does not exist. "
                "Run `mcp-workbay-handoff init-state` or migrate."
            ),
        }
    ...
    try:
        ...
        return {
            ...
            "stat1_named": False,
            "remediation": (
                f"Could not inspect {index_name} on {db_path} ({exc}). "
                "Run `mcp-workbay-handoff migrate` or reopen with 0.2.19+."
            ),
        }
    if not present:
        return {
            ...
            "stat1_named": False,
            "remediation": (
                f"{index_name} is missing on review_findings. "
                "Reopen the database with mcp-workbay-handoff 0.2.19+ "
                "(or run `mcp-workbay-handoff migrate`) so "
                "_ensure_review_findings_finding_id_index can build the btree."
            ),
        }
    result: dict[str, Any] = {
        "ok": True,
    ...
    if not stat1_named:
        result["warning"] = (
            f"{index_name} is present but unnamed in sqlite_stat1; "
            "planner stats are stale (performance, not correctness). "
            "A quiet reopen ANALYZE-heals; vacuum_handoff_db is the "
            "fail-closed refresh."
        )
    return result
...
def _check_embedding_failed_signal(db_path: Path) -> dict[str, Any]:
    """Surface ``embedding_failed`` agent_errors rows for doctor (implementation note S1b).

    Best-effort; never raises. Missing ``agent_errors`` (fresh/pre-feature DB)
    or a hard probe failure still returns a block so doctor exits cleanly, but
    sets ``severity="warn"`` (with a note) so consumers that key on severity do
    not treat a failed probe as clean. ``severity`` is also ``warn`` when any
    embedding_failed row exists; else ``ok`` — signal facet, not a hard gate.
    """
    import sqlite3 as _sqlite3

    block: dict[str, Any] = {
        "ok": True,
        "embedding_failed_rows": 0,
        "embedding_failed_occurrences": 0,
        "last_seen_at": None,
        "severity": "ok",
    }
    try:
        ...
        with closing(connect_handoff_db(db_path, read_only=True)) as conn:
            try:
                row = conn.execute(
                    """
                    SELECT COUNT(*), COALESCE(SUM(occurrence_count), 0), MAX(last_seen_at)
                    FROM agent_errors
                    WHERE error_class = 'embedding_failed'
                    """
                ).fetchone()
            except _sqlite3.OperationalError:
                block["note"] = "agent_errors unavailable"
                block["severity"] = "warn"
                return block
            rows = int(row[0] or 0)
            block["embedding_failed_rows"] = rows
            block["embedding_failed_occurrences"] = int(row[1] or 0)
            block["last_seen_at"] = row[2]
            block["severity"] = "warn" if rows > 0 else "ok"
    except _sqlite3.Error as exc:  # noqa: BLE001 — diagnostic capture
        block["note"] = f"embedding_failed signal probe failed ({exc})"
        block["severity"] = "warn"
    return block


def _worker_daemon_log_dir(config: RuntimeConfig) -> Path:
    """Directory the worker daemon writes ``worker-*.jsonl`` into.

    The producer binds ``log_dir`` to ``orchestrator_root / "logs" /
    "worker-daemon"``. ``for_repo`` collapses a linked worktree to that
    primary root; ``for_workspace`` does not. Resolve the primary git
    worktree when one exists so a doctor run from a lane worktree reads
    the directory the producer actually writes.
    """
    primary = _resolve_primary_worktree_root(config.workspace_root)
    root = primary if primary is not None else config.workspace_root
    return root / "logs" / "worker-daemon"


def _worker_jsonl_has_event(log_dir: Path, event_name: str) -> bool:
    """True when any ``worker-*.jsonl`` record has ``event`` equal to ``event_name``."""
    if not log_dir.exists():
        return False
    for path in log_dir.glob("worker-*.jsonl"):
        try:
...
def run_doctor(config: RuntimeConfig) -> dict[str, Any]:
    ...
    finding_id_index_check = _check_review_findings_finding_id_index_health(config.db_path)
    ...
    worker_log_dir = _worker_daemon_log_dir(config)
    worker_logs_found = any(worker_log_dir.glob("worker-*.jsonl")) if worker_log_dir.exists() else False
    scope_violation_found = _worker_jsonl_has_event(worker_log_dir, WorkerEventName.SCOPE_VIOLATION)
    portable_hook_semantics = [
        ...
        },
        {
            ...
            "trigger": "integrity_check(kind='close') is invoked",
            "durable_output": "CURRENT_TASK.json rewritten from the live workspace summary",
        ...
        },
        {
            ...
            "durable_output": "any worker-*.jsonl under the primary worktree logs/worker-daemon directory",
            "evidence_path": str(worker_log_dir),
            "evidence_found": worker_logs_found,
        },
        {
            "name": "after_task_switch",
            "trigger": "CURRENT_TASK.json is present at the configured path",
            "durable_output": "CURRENT_TASK.json exists at the configured path",
            "evidence_path": str(config.current_task_path),
            "evidence_found": config.current_task_path.exists(),
        },
        {
            "name": "before_review_prompt_build",
            "trigger": "worker JSONL contains a scope_violation event (owned-path check tripped)",
            "durable_output": "scope_violation event in worker-*.jsonl",
            "evidence_path": str(worker_log_dir),
            "evidence_found": scope_violation_found,
        },
    ]
    ...
    # without the doctor itself raising. A missing idx_review_findings_finding_id
    # is the same class of hard structural miss: nested-only reporting would
    # leave top-level ok true whenever either startup probe works.
    stdio_ok = stdio_probe_error is None
    cli_ok = cli_probe_error is None
    overall_ok = (stdio_ok or cli_ok) and bool(finding_id_index_check.get("ok"))
    ...
    stdio_startup_block: dict[str, Any] = {
        ...
        # tool_profile cannot subset the registry (_VALID_TOOL_PROFILES is
        # ("all",) only). These counts are an unfiltered inventory of
        # _build_tool_registry(), not a serving-policy slice.
        "registry_inventory_scope": "unfiltered_build_tool_registry",
    }
    ...
    embedding_failed_check = _check_embedding_failed_signal(config.db_path)
    ...
    mcp_protocol_check = check_mcp_protocol()
    ...
    return {
        ...
        "checks": {
            ...
            "review_findings_finding_id_index": finding_id_index_check,
            ...
            "embedding_failed_signal": embedding_failed_check,
            "src_installed_skew": src_installed_skew_check,
            "mcp_protocol": mcp_protocol_check,
        },
