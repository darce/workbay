...
def _connect(db_path: Path | str) -> sqlite3.Connection:
    # Single owner for authorizer, busy timeout, and statement-cache policy
    # (internal / [ARCH-13]). Factory already sets row_factory=Row and
    # a 30s busy timeout equivalent to the prior bare connect.
    from .shared_schema import _connect_handoff_sqlite
...
def _try_acquire_flock(lock_path: Path) -> int | None:
    ...
    try:
        # Barrier first: never flock-acquire while SQLite RESERVED is held.
        from .shared_write_context import acquire_flock  # noqa: PLC0415

        acquire_flock(fd, fcntl.LOCK_EX | fcntl.LOCK_NB)
...
def _unlock_and_close(lock_fd: int | None) -> None:
    ...
    try:
        ...
        acquire_flock(lock_fd, fcntl.LOCK_UN)
    except OSError:
        pass
    try:
        os.close(lock_fd)
