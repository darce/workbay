...
from .projection_event_dedupe import (
    ...
    fetch_projection_event,
)
from .related_prior_attach import attach_related_prior
...
def record_decision(
    ...
    refresh_rationale_on_conflict: bool = False,
) -> dict:
    ...
    if decision_origin is not None and decision_origin not in {"agent", "system"}:
        return _envelope(
            ...
            data={"error": (f"decision_origin must be 'agent' or 'system' (or omitted); got {decision_origin!r}")},
        )
    ...
    # Insert and DO UPDATE both change the stored rationale and must re-embed;
    # DO NOTHING (operation='noop') must not (PLAN0181-S2REFRESHIDEM-01).
    # Embed the rationale actually persisted on the row, not the caller's
    # argument: COALESCE / blank-as-absent can leave the stored value while
    # the argument is None or blank (PLAN0181-S2GATE2-EMBED-STALE-RATIONALE-01).
    if result.get("ok") and mutation.get("operation") in {"insert", "update"}:
        ...
        if affected_ids:
            ...
            decision_data = data.get("decision") if isinstance(data.get("decision"), dict) else None
            resolved_task_ref = scope.get("task_ref") or (decision_data or {}).get("task_ref")
            if resolved_task_ref is not None:
                persisted_rationale = decision_data.get("rationale") if decision_data is not None else rationale
                # Capture outcome into side_effect_failures + warnings (same shape
                # as close_slice). Primary write stays ok; embed is non-fatal
                # ([DATA-13], [OBS-08], [RES-02]). Production failures return
                # ``failed`` rather than raising ([RES-02]).
                try:
                    ...
                    if outcome == "failed":
                        msg = "embed_concept_on_write failed during record_decision: provider/store returned failed"
                        _surface_side_effect_failure(
                            result,
                    ...
                    msg = f"embed_concept_on_write failed during record_decision: {exc}"
                    _surface_side_effect_failure(
                        result,
                        effect="embed_concept_on_write",
                        detail=msg,
...
def _record_decision_with_conn(
    ...
    *,
    ...
    decision_origin: str | None = None,
    refresh_rationale_on_conflict: bool = False,
    resolved_ctx: ResolvedWriteContext | None = None,
    target_context_warnings: list[str] | None = None,
    ...
    def _idempotent_decision_envelope(existing_event: sqlite3.Row) -> dict:
    ...
    # DURREV-HARM-03: pure-SQL idempotency short-circuit BEFORE actor resolve.
    # A replayed event_id must return ok=True/idempotent without git shell-out
    # (and without BranchMismatchError from a different branch). The hoist still
    # keeps resolve above lock-taking DML (claim_projection_event INSERT).
    if event_id is not None and not refresh_rationale_on_conflict:
        prior_event = fetch_projection_event(conn, event_id)
        if prior_event is not None:
            return _idempotent_decision_envelope(prior_event)

    # [CON-18][internal]: claim_projection_event is INSERT OR IGNORE and
    # takes RESERVED; actor resolution shells out to git and must run first.
    # Callers that already resolved outside the lock (close_slice) pass
    # resolved_ctx / target_context_warnings and short-circuit resolution.
    ctx = resolved_ctx if resolved_ctx is not None else _resolve_write_actor(conn, actor, task_ref=resolved_task_ref)
    if target_context_warnings is not None:
        precomputed_target_warnings = list(target_context_warnings)
    else:
        precomputed_target_warnings = collect_target_context_warnings(
            conn,
            ctx,
            task_ref=resolved_task_ref,
            target_worktree_path=precomputed_target_worktree_path,
        )
    ...
    # Genuine event-id idempotency (no refresh asked) short-circuits. When the
    # caller opted into refresh_rationale_on_conflict, a reused event_id must
    # still reach the UPSERT so newest-wins content is applied — otherwise a
    # successful envelope silently keeps the stale rationale
    # (PLAN0181-S2GATE2-EVENTID-BYPASSES-REFRESH-01).
    # Race: a concurrent claim may land between the pure-SQL pre-check and
    # this claim; keep the post-claim short-circuit for that interleaving.
    if existing_event is not None and not refresh_rationale_on_conflict:
        return _idempotent_decision_envelope(existing_event)
    # Refresh still skips the short-circuit above, but only for *this* decision
    # row. complete_projection_event unconditionally rebinds target_id; without
    # an ownership check a reused event_id with refresh=True rebinds an event
    # owned by decision A onto decision B, and a later A retry then short-
    # circuits to B's row as "idempotent" success
    # (PLAN0181-S2GATE3-EVENTID-REBINDS-TO-OTHER-DECISION-01).
    if existing_event is not None and refresh_rationale_on_conflict:
        intended = conn.execute(
            "SELECT id FROM decisions WHERE task_ref = ? AND decision = ? AND session = ?",
            (resolved_task_ref, decision, session),
        ).fetchone()
        event_target_table = existing_event["target_table"]
        event_target_id = existing_event["target_id"]
        owns_this_row = (
            intended is not None
            and event_target_table == "decisions"
            and event_target_id is not None
            and int(event_target_id) == int(intended["id"])
        )
        if not owns_this_row:
            return _envelope(
                ok=False,
                tool="record_decision",
                data={
                    "error": (
                        "event_id already targets a different decision "
                        f"(target_table={event_target_table!r}, "
                        f"target_id={event_target_id!r}); refusing to rebind "
                        "under refresh_rationale_on_conflict"
                    )
                },
            )
    # ctx and precomputed_target_warnings were resolved above the claim so
    # git/subprocess work never runs under RESERVED ([CON-18], [internal]).
    warnings: list[str] = []
    ...
    warnings.extend(precomputed_target_warnings)
    changed_files_json = json.dumps(changed_files) if changed_files is not None else "[]"
    # Blank/whitespace-only rationale is absent (NULL) for the conflict path:
    # COALESCE(excluded.rationale, decisions.rationale) only preserves on SQL
    # NULL, and size validation already treats blank as absent — binding ''
    # would wipe a stored envelope while reporting ok=True
    # (PLAN0181-S2GATE2-COALESCE-BLANK-WIPES-ENVELOPE-01).
    if rationale is not None and _normalize_optional_text(rationale) is None:
        rationale = None
    # Default is append-only idempotency (DO NOTHING). Callers that need
    # newest-wins content on a stable (task_ref, decision, session) key opt into
    # DO UPDATE of every descriptive column the INSERT supplies — never the
    # conflict key (task_ref, decision, session) or creation facts like
    # created_at (PLAN0181-S2CREATEDAT-01). Omitted nullable args are no-change
    # via COALESCE(excluded.X, decisions.X), not NULL erasure — including
    # rationale (PLAN0181-S2RATIONALEWIPE-01). Blank/whitespace rationale is
    # normalized to NULL above so COALESCE preserves rather than overwriting.
    # A refreshing caller therefore cannot clear a stored rationale back to
    # NULL via this path; that is a deliberate, documented limit matching the
    # changed_files_json note below.
    # changed_files_json is never NULL (bound as '[]' when omitted); treat '[]'
    # as omitted so a refresh that does not mention files cannot wipe a stored
    # list — same deliberate limit, not a path to clear the list
    # (PLAN0181-S2FREEZEREST-01) [CON-05][OBS-12][AGT-10].
    on_conflict = (
        "ON CONFLICT(task_ref, decision, session) DO UPDATE SET "
        # Omitted/blank rationale is no-change; callers cannot clear to NULL here.
        "rationale = COALESCE(excluded.rationale, decisions.rationale), "
        "lane_id = COALESCE(excluded.lane_id, decisions.lane_id), "
        "agent = COALESCE(excluded.agent, decisions.agent), "
        "harness = COALESCE(excluded.harness, decisions.harness), "
        "model = COALESCE(excluded.model, decisions.model), "
        "model_label = COALESCE(excluded.model_label, decisions.model_label), "
        "reasoning_level = COALESCE(excluded.reasoning_level, decisions.reasoning_level), "
        "input_tokens = COALESCE(excluded.input_tokens, decisions.input_tokens), "
        "output_tokens = COALESCE(excluded.output_tokens, decisions.output_tokens), "
        "total_tokens = COALESCE(excluded.total_tokens, decisions.total_tokens), "
        "branch = COALESCE(excluded.branch, decisions.branch), "
        "commit_sha = COALESCE(excluded.commit_sha, decisions.commit_sha), "
        # '[]' means omitted — refreshing callers cannot clear a stored file list.
        "changed_files_json = COALESCE(NULLIF(excluded.changed_files_json, '[]'), decisions.changed_files_json), "
        "slice_number = COALESCE(excluded.slice_number, decisions.slice_number), "
        "decision_origin = COALESCE(excluded.decision_origin, decisions.decision_origin)"
        if refresh_rationale_on_conflict
        else "ON CONFLICT(task_ref, decision, session) DO NOTHING"
    )
    # Pre-read existence for insert-vs-conflict classification. Do NOT use
    # cursor.lastrowid: SQLite retains last_insert_rowid() on DO UPDATE and
    # DO NOTHING (connection-scoped). It can name an unrelated row from any
    # prior INSERT on this connection (migrations, projection_event_dedupe,
    # other tables), so `if cur.lastrowid:` misclassifies conflict as insert
    # and returns a wrong/null decision receipt
    # (PLAN0181-S2GATE2-LASTROWID-FALSE-ORACLE-01). rowcount is statement-
    # scoped: 1 for INSERT or DO UPDATE, 0 for DO NOTHING. Combined with the
    # pre-existence check that is sufficient (PLAN0181-S2REFRESHIDEM-01).
    ...
    decision_row = _row_to_dict(stored)
    affected_id = int(stored["id"]) if stored is not None else None
...
def record_test_result(
    ...
    with _get_db_connection() as conn:
        ...
        def _idempotent_test_envelope(existing_event: sqlite3.Row) -> dict:
            existing_test = None
        ...
        # DURREV-HARM-03: pure-SQL idempotency before resolve (no git on replay).
        if event_id is not None:
            prior_event = fetch_projection_event(conn, event_id)
            if prior_event is not None:
                return _idempotent_test_envelope(prior_event)

        # [CON-18][internal]: claim_projection_event takes RESERVED; resolve
        # actor (git/subprocess) and target-context warnings before the claim.
        ctx = _resolve_write_actor(conn, actor, task_ref=resolved_task_ref)
        warnings = collect_target_context_warnings(conn, ctx, task_ref=resolved_task_ref)
        existing_event = claim_projection_event(
            conn,
        ...
        if existing_event is not None:
            return _idempotent_test_envelope(existing_event)
        task_revision = _current_task_revision(conn, resolved_task_ref)
        cur = conn.execute(
            """
...
def report_blocker(
    ...
    if operation == "add" and result.get("ok"):
        ...
        # Capture outcome into side_effect_failures + warnings (same shape as
        # close_slice). Primary write stays ok; embed is non-fatal
        # ([DATA-13], [OBS-08], [RES-02]).
        try:
            outcome = embed_concept_on_write(
                "blocker.description",
                blocker.get("id"),
                str(blocker.get("task_ref")),
                blocker.get("description"),
            )
            if outcome == "failed":
                msg = "embed_concept_on_write failed during report_blocker: provider/store returned failed"
                _surface_side_effect_failure(
                    result,
                    effect="embed_concept_on_write",
                    detail=msg,
            ...
            msg = f"embed_concept_on_write failed during report_blocker: {exc}"
            _surface_side_effect_failure(
                result,
                effect="embed_concept_on_write",
                detail=msg,
        ...
        # Advisory only: lookup runs after the blocker row (and embed hook)
        # have committed and cannot abort or mask the write result.
        attach_related_prior(
            result,
            query=str(blocker.get("description") or description or ""),
            task_ref=str(blocker.get("task_ref") or result.get("task_ref") or "") or None,
        )
    return result
...
def _report_blocker_impl(
    ...
    with _get_db_connection() as conn:
        ...
        if operation == "add":
            ...
            resolved_lane_id = _normalize_optional_text(lane_id)
            if resolved_lane_id is None:
                resolved_lane_id = ctx.lane_id
            cur = conn.execute(
                """
                ...
                """,
                (resolved_task_ref, resolved_lane_id, description, ctx.agent, ctx.branch, ctx.commit_sha),
            )
...
def _collect_commit_verification_data(
...
) -> tuple[int, int, list, list, str | None, str | None, int | None]:
    """Collect fresh-test count and commit-level slice decisions.

    Fresh-test counts are latest-per-command at ``commit_sha``: group rows by
    normalized command, keep MAX(id), then count passes and fails. A later
    pass of the same command clears an earlier fail at the same SHA.
    ...
    ``(fresh_test_count, failed_test_count, slice_decisions, structured,
    satisfied_by_commit, match_kind, ancestor_distance)`` where
    ``match_kind`` is ``"exact"`, ``"reachable_ancestor"``, or ``None``
    when no structured decision qualifies.
    """
    fresh_test_count = 0
    failed_test_count = 0
    ...
    if require_fresh_tests and commit_sha is not None:
        latest_rows = _latest_fresh_tests_at_commit(conn, task_ref, commit_sha)
        fresh_test_count = sum(1 for row in latest_rows if int(row["passed"]) == 1)
        failed_test_count = sum(1 for row in latest_rows if int(row["passed"]) == 0)
    if require_summary and commit_sha is not None:
        ...
        commit_slice_decisions = exact_rows
        return fresh_test_count, failed_test_count, commit_slice_decisions, [], None, None, None
...
def _latest_fresh_tests_at_commit(
    conn: sqlite3.Connection,
    task_ref: str,
    commit_sha: str,
) -> list[sqlite3.Row]:
    """Return the newest verified_tests row per normalized command at ``commit_sha``.

    ``verified_tests`` is insert-only, so a flake or red-then-green of the same
    command leaves superseded rows in place. Close-check judges the latest row
    per command (MAX(id)) at HEAD; an older fail does not veto a later pass.
    """
    return conn.execute(
        """
        SELECT vt.id, vt.command, vt.passed
        FROM verified_tests AS vt
        WHERE vt.id IN (
            SELECT MAX(id)
            FROM verified_tests
            WHERE task_ref = ? AND commit_sha = ?
            GROUP BY TRIM(COALESCE(command, ''))
        )
        ORDER BY vt.id ASC
        """,
        (task_ref, commit_sha),
...
def _failing_fresh_test_identities(
    conn: sqlite3.Connection,
    task_ref: str,
    commit_sha: str | None,
) -> list[str]:
    """Return identities for commands whose latest HEAD row failed."""
    if commit_sha is None:
        return []
    identities: list[str] = []
    for row in _latest_fresh_tests_at_commit(conn, task_ref, commit_sha):
        if int(row["passed"]) != 0:
            continue
        command = str(row["command"] or "").strip() or "<unnamed command>"
        identities.append(f"{command} (id={int(row['id'])})")
    return identities
...
def _evaluate_close_failures(
    *,
    ...
    failed_test_identities: Sequence[str] | None = None,
) -> list[str]:
    ...
    if require_fresh_tests and failed_test_count > 0:
        named = [str(item).strip() for item in (failed_test_identities or ()) if str(item).strip()]
        if named:
            failures.append(
                "Fresh verification for the current commit includes failing run(s): " + ", ".join(named) + "."
            )
        else:
            failures.append("Fresh verification for the current commit includes failing run(s).")
    elif require_fresh_tests and fresh_test_count == 0:
...
def handoff_close_check(
    ...
    with _get_db_connection() as conn:
        ...
        failing_fresh_test_identities = (
            _failing_fresh_test_identities(conn, resolved_task_ref, normalized_current_commit_sha)
            if require_fresh_tests
            else []
        )
    ...
    failures = _evaluate_close_failures(
        ...
        failed_test_identities=failing_fresh_test_identities,
    ...
    data: dict = {
        ...
        "checks": {
            ...
            "fresh_tests": {
                ...
                "is_violation": bool(require_fresh_tests and (fresh_test_count == 0 or failed_test_count > 0)),
            },
    ...
    if require_fresh_tests and fresh_test_count == 0:
        data["stale_test"] = {
            "current_commit_sha": normalized_current_commit_sha,
            "reason": (
                "Fresh verification exists for the current commit, but every recorded run FAILED."
                if failed_test_count > 0
                else "No verification rows recorded for the current commit."
            ),
        }
    return _envelope(
