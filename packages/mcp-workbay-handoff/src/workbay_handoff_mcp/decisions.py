...
def _collect_commit_verification_data(
    ...
    if require_fresh_tests and commit_sha is not None:
        test_counts = conn.execute(
            """
            SELECT
                COALESCE(SUM(passed = 1), 0) AS passed_count,
                COALESCE(SUM(passed = 0), 0) AS failed_count
            FROM verified_tests
            WHERE task_ref = ? AND commit_sha = ?
            """,
            (task_ref, commit_sha),
        ).fetchone()
        fresh_test_count = int(test_counts["passed_count"])
        failed_test_count = int(test_counts["failed_count"])
    ...
    *,
    ...
    if require_fresh_tests and fresh_test_count == 0:
        if failed_test_count > 0:
            failures.append("Fresh verification exists for the current commit, but every recorded run FAILED.")
        else:
