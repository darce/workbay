    ...
    if result.get("ok") and mutation.get("operation") in {"insert", "update"}:
        ...
        if affected_ids:
            ...
            if resolved_task_ref is not None:
                persisted_rationale = (
                    decision_data.get("rationale") if decision_data is not None else rationale
                )
                ...
                try:
                    ...
                    if outcome == "failed":
                        msg = (
                            "embed_concept_on_write failed during record_decision: "
...
def report_blocker(
    ...
    if operation == "add" and result.get("ok"):
        ...
        try:
            ...
            if outcome == "failed":
                msg = (
                    "embed_concept_on_write failed during report_blocker: "
                    "provider/store returned failed"
                )
                _surface_side_effect_failure(
                    result,
