...
def record_decision(
    ...
    # Embed the rationale after the decisions row has committed (best-effort;
    # no-op without the optional embeddings provider). A derived artifact must
    # not be produced inside the write transaction.
    embed_concept_on_write("decision.rationale", cur.lastrowid, resolved_task_ref, rationale)
    return result
