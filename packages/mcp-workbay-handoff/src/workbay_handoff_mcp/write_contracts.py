...
# G1: set_handoff_state and update_task_status share one status grammar.
# Both write handoff_state.status; update_task_status delegates to
# set_handoff_state(status_only=True). The prior split
# (update: in_progress|done|paused|blocked|abandoned; set: that set plus
# active) was stale-grammar drift, not an intentional product difference.
# Literal so adding a HandoffStatus member cannot silently widen PreToolUse.
# Widened vs prior: review. Narrowed vs prior: paused, abandoned, and
# (set_handoff_state only) active.
HANDOFF_STATUS_GRAMMAR = r"^(in_progress|blocked|review|done)$"

# Inline close_slice prohibition (api.close_slice scans these case-insensitively).
# Well-formed <actor>...</actor> spans are consumed by
# ``api.parse_and_apply_rationale_actor`` *before* this scan; only
# ``changed_files`` tags remain here as hard rejects. Not expressible as a
# registry field_grammar, so it is carried as a first-class ``prohibitions``
# entry on the close_slice contract — surfaced identically by
...
def _close_slice_xml_prohibition() -> dict[str, Any]:
    ...
    return {
        ...
        "remediation": (
            "Do not embed changed_files as XML tags inside rationale; pass "
            "changed_files=[...] as a top-level close_slice parameter. Optional "
            "well-formed <actor>...</actor> in rationale is parsed when top-level "
            "actor is omitted; malformed actor tags still fail closed."
        ),
    }
...
_REVIEW_FINDINGS_RESOLVE_VARIANT = WriteContract(
    ...
    field_grammars={"status": r"^(fixed|deferred|wontfix)$"},
)
...
_REVIEW_FINDINGS_INTEGRATE_VARIANT = WriteContract(
    tool_name="review_findings.operation=integrate",
    optional=["integration_ref", "task_ref", "actor"],
)
_REVIEW_FINDINGS_LIST_VARIANT = WriteContract(
    ...
    field_grammars={
        "status": "^(" + "|".join(sorted({"all", *REVIEW_FINDING_STATUSES})) + ")$",
        "severity": "^(" + "|".join(sorted({"all", *REVIEW_FINDING_SEVERITIES})) + ")$",
        "review_mode": r"^(branch|release_audit|planning)$",
...
_REVIEW_FINDINGS_DISPOSITION_VARIANT = WriteContract(
    ...
    optional=["resolution_notes", "disposition_evidence", "verified_commit_sha", "actor"],
    field_grammars={
...
REGISTRY: dict[str, WriteContract] = {
    ...
    "review_findings": WriteContract(
        ...
        variants={
            ...
            "integrate": _REVIEW_FINDINGS_INTEGRATE_VARIANT,
...
def validate_write(tool_name: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # RG-0146-01: preflight/write parity for the close_slice rationale actor
    # grammar. The authoritative parser lives in api.py; run it here (function-
    # level import — api imports this module at load time) so a malformed
    # <actor> payload that close_slice will fail-close on is never preflight-
    # green. Well-formed single pairs pass (they will be parsed-and-applied).
    if tool_name == "close_slice":
        rationale = payload.get("rationale")
        if isinstance(rationale, str) and "<actor" in rationale.lower():
            from .api import parse_and_apply_rationale_actor  # noqa: PLC0415

            _r, _a, _receipt, rejection = parse_and_apply_rationale_actor(rationale, payload.get("actor"))
            if rejection is not None:
                errors.append(
                    "field 'rationale' for close_slice carries a malformed <actor> "
                    "payload (unclosed/nested/invalid-JSON/multi-pair); close_slice "
                    "will fail closed on it"
                )
    ...
    if contract.variants and contract.selector_field:
        ...
        if tool_name == "review_findings" and variant_selected is not None:
            variant = contract.variants[variant_selected]
            allowed = set(contract.required + contract.optional + variant.required + variant.optional)
            for key in sorted(payload.keys() - allowed):
                errors.append(f"unknown field {key!r} for {variant.tool_name}")
            if variant_selected == "resolve":
                # Match the handler's normalized target selection: empty lists
                # and all_open=False do not select a target.
                finding_ids = payload.get("finding_ids") or []
                has_ids = isinstance(finding_ids, list) and any(str(value).strip() for value in finding_ids)
                if has_ids and payload.get("all_open"):
                    errors.append("Pass either finding_ids or all_open=True, not both, for review_findings.resolve")
                statuses = payload.get("finding_statuses")
                if statuses is not None and (
                    not isinstance(statuses, dict)
                    or any(value not in ("fixed", "deferred", "wontfix") for value in statuses.values())
                ):
                    errors.append("field 'finding_statuses' values must be fixed, deferred, or wontfix")
