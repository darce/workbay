    ...
    *,
    ...
    with _registry_lock, _registry_file_lock(path, blocking=not non_blocking) as acquired:
        ...
        if non_blocking:
            ...
            if (
                last is not None
                and (now_m - last) < float(REAPER_CONNECT_MIN_INTERVAL_SECONDS)
            ):
                skip = {
                    "event": _EVENT_REAPER_SKIPPED,
        ...
        for writer in writers:
            ...
            if alive and stale and kill_stale_live:
                if identity_match is True:
                    ...
                    latest = _read_writer_shard_file(
                        writer_shard_path(db_path, writer.writer_id)
                    )
                    if (
                        latest is not None
                        ...
                        abort = {
                            ...
                            "fresh_heartbeat_age_seconds": round(
                                ts - latest.heartbeat_ts, 3
                            ),
