...
def _print_json(payload: str | dict, *, exit_nonzero_on_not_ok: bool = True) -> None:
    if isinstance(payload, str):
        ...
        parsed = payload
    if exit_nonzero_on_not_ok and isinstance(parsed, dict) and parsed.get("ok") is False:
        raise SystemExit(1)
...
@dataclass
class CliEntry:
    """Registry entry for a single CLI sub-command and its failure policy."""

    name: str
    dispatch: Callable[[argparse.Namespace], Any]
    description: str = ""
    args: list[ArgSpec] = field(default_factory=list)
    exit_nonzero_on_not_ok: bool = True
...
def _add_arg(sub: argparse.ArgumentParser, spec: ArgSpec) -> None:
    ...
    kwargs: dict[str, Any] = {}
    if spec.help:
        # S1-A-006: argparse runs help strings through %-interpolation, so a literal
        # '%' (e.g. in a percentage) raises ValueError on --help. Escape centrally
        # here so every ArgSpec help string is covered, not just parser descriptions.
        kwargs["help"] = spec.help.replace("%", "%%")
    if spec.action:
...
# review-findings shares one subparser across every --operation. Argparse
# therefore accepts the union of all flags; the selected op model only carries
# a subset. Map operation -> model so an explicitly set flag that the model
# cannot carry is refused (SystemExit naming the flag) rather than dropped.
_REVIEW_FINDINGS_OP_MODELS: dict[str, type] = {
    "record": ReviewFindingsRecordOp,
    "batch_record": ReviewFindingsBatchRecordOp,
    "update": ReviewFindingsUpdateOp,
    "resolve": ReviewFindingsResolveOp,
    "repair_provenance": ReviewFindingsRepairProvenanceOp,
    "merge": ReviewFindingsMergeOp,
    "integrate": ReviewFindingsIntegrateOp,
    "disposition": ReviewFindingsDispositionOp,
    "reanchor": ReviewFindingsReanchorOp,
    "list": ReviewFindingsListOp,
}

# CLI dest -> model field when names differ or the dest nests under a parent.
_REVIEW_FINDINGS_CLI_TO_FIELD: dict[str, str] = {
    "line_start": "details",
    "line_end": "details",
    "fix": "details",
    "actor_commit_sha": "actor",
    "actor_branch": "actor",
    "findings_json": "findings",
    "findings_file": "findings",
    "resolve_finding_id": "finding_ids",
}

# Destinations always present on the Namespace because the ArgSpec sets a
# non-None default. Treat as "explicitly set" only when the value differs from
# that default so unpassed flags do not false-positive the guard.
_REVIEW_FINDINGS_ALWAYS_DEFAULTS: dict[str, Any] = {
    "limit": 20,
    "offset": 0,
    "detail": "full",
...
# Destinations owned by the shared review-findings subparser (not operation).
_REVIEW_FINDINGS_CLI_DESTS: tuple[str, ...] = (
    "session",
    "finding_id",
    "resolve_finding_id",
    "severity",
...
def _review_findings_cli_arg_is_set(dest: str, value: Any) -> bool:
    """Return True when the caller explicitly supplied a non-default value."""
    if dest in _REVIEW_FINDINGS_ALWAYS_DEFAULTS:
        return value != _REVIEW_FINDINGS_ALWAYS_DEFAULTS[dest]
    if value is None or value is False or value == []:
        return False
    return True


def _reject_unhonoured_review_findings_flags(args: argparse.Namespace) -> None:
    """Refuse flags the selected operation's pydantic model cannot carry.

    Shared-subparser shape means argparse accepts every review-findings flag on
    every operation. Silently dropping an explicit flag returns ok:true while
    the value never lands (RLSE-05). Drive rejection from the op model field set
    so new flags/ops stay covered without a per-flag blocklist (REF-12).
    """
    model = _REVIEW_FINDINGS_OP_MODELS.get(args.operation)
    if model is None:
        return
    fields = model.model_fields
    for dest in _REVIEW_FINDINGS_CLI_DESTS:
        if not hasattr(args, dest):
            continue
        value = getattr(args, dest)
        if not _review_findings_cli_arg_is_set(dest, value):
            continue
        model_field = _REVIEW_FINDINGS_CLI_TO_FIELD.get(dest, dest)
        if model_field not in fields:
            flag = "--" + dest.replace("_", "-")
            raise SystemExit(f"{flag} is not accepted for --operation={args.operation}")


def _dispatch_review_findings(args: argparse.Namespace) -> Any:
    _reject_unhonoured_review_findings_flags(args)

    payload: dict[str, Any] = {"operation": args.operation}
    if args.task_ref is not None:
        payload["task_ref"] = args.task_ref

    if args.operation in ("resolve", "update", "merge"):
        actor: dict[str, str] = {}
    ...
    if args.operation == "record":
        ...
        payload["source_task_refs"] = args.source_task_refs
        payload["target_task_ref"] = args.target_task_ref
        payload["retire_sources"] = args.retire_sources
...
def _errors_record_exit_nonzero(result: object) -> bool:
    """Fail caller errors; exempt only a failed best-effort spool append."""
    if not isinstance(result, dict) or result.get("ok") is not False:
        return False
    return not str(result.get("error") or "").startswith("spool write failed:")
...
def _dispatch_errors_replay_spool(args: argparse.Namespace) -> dict:
    ...
    # Honour --workspace-root via configure_runtime: replay_agent_error_spool
    # resolves the primary state dir from cwd, so without an explicit cwd the
    # process cwd wins and the flag is inert (internal R1).
...
def _dispatch_terminal_guard_record(args: argparse.Namespace) -> dict:
    """Hook-facing record path for terminal_guard_events (internal)."""
    from .terminal_telemetry import record_terminal_guard_event  # noqa: PLC0415
    ...
    return record_terminal_guard_event(
        ...
        native_tool_hint=args.native_tool_hint,
        command_preview=args.command_preview,
...
def _dispatch_terminal_guard_replay_spool(args: argparse.Namespace) -> dict:
    """Lifecycle reclaimer drain for the terminal_guard JSONL spool."""
    from .terminal_telemetry import replay_terminal_guard_spool  # noqa: PLC0415

    # max_batch accepted for CLI parity with errors-replay-spool; current
    # replay_terminal_guard_spool is whole-spool (see internal).
    _ = getattr(args, "max_batch", None)
    return replay_terminal_guard_spool(
...
def _dispatch_continuation(args: argparse.Namespace) -> Any:
    payload: dict[str, Any] = {"operation": args.operation}
    if args.task_ref is not None:
        payload["task_ref"] = args.task_ref
    if args.lane_id is not None:
    ...
    if args.operation == "save":
        for field in ("done_do_not_redo", "next_actions", "verified_anchors", "gotchas"):
    ...
    return continuation(payload=cast(ContinuationParam, payload))
...
def _dispatch_archive(args: argparse.Namespace) -> Any:
    ...
    if args.operation == "archive":
        ...
        payload["tombstone_findings"] = args.tombstone_findings
...
def _dispatch_findings_triage(args: argparse.Namespace) -> Any:
    """Classify open findings or apply a reviewed manifest (implementation note).

    --manifest: read-only classify write.
    --apply --from-manifest: execute dispositions/re-anchors from a reviewed JSON
    manifest (fixtures/operator-gated; never pass the live DB casually).
    """
    ...
    if apply_flag:
        if not from_manifest:
            raise SystemExit("findings-triage --apply requires --from-manifest PATH (reviewed classification JSON).")
        from .backlog_triage import DEFAULT_APPLY_BATCH_SIZE, apply_reviewed_manifest
        from .shared_schema import _get_db_connection

        src = Path(from_manifest)
        if not src.is_file():
            raise SystemExit(f"--from-manifest not found: {src}")
        loaded = json.loads(src.read_text(encoding="utf-8"))
        if not isinstance(loaded, dict):
            raise SystemExit("--from-manifest must be a JSON object")

        batch_size = getattr(args, "batch_size", None)
        if batch_size is None:
            batch_size = DEFAULT_APPLY_BATCH_SIZE
    ...
    if not manifest_path:
        raise SystemExit("findings-triage requires --manifest PATH (classify) or --apply --from-manifest PATH (apply).")

    rename_map: dict[str, str] = {}
    rename_map_path = getattr(args, "rename_map", None)
    if rename_map_path:
        try:
            raw = Path(rename_map_path).read_text(encoding="utf-8")
        except OSError as exc:
            raise SystemExit(f"--rename-map could not be read ({rename_map_path}): {exc}")
        try:
            loaded = json.loads(raw)
        except json.JSONDecodeError as exc:
            raise SystemExit(f"--rename-map is not valid JSON ({rename_map_path}): {exc}")
        if not isinstance(loaded, dict):
            raise SystemExit("--rename-map must be a JSON object of old_path -> new_path")
        rename_map = {str(k): str(v) for k, v in loaded.items()}

    from .backlog_triage import classify_open_findings
    from .runtime import get_runtime_config
    from .shared_schema import _get_db_connection

    # Pass workspace_root explicitly so any opt-in git shell-out resolves
    # against the configured workspace, not ambient cwd of another checkout.
    workspace_root = get_runtime_config().workspace_root
    use_git_rename_map = bool(getattr(args, "git_rename_map", False))

    with _get_db_connection() as conn:
        result = classify_open_findings(
            conn,
            rename_map,
            workspace_root=workspace_root,
            use_git_rename_map=use_git_rename_map,
        )
    ...
    return {
        ...
        "open_total": result.get("open_total"),
        "counts": result.get("counts"),
        "degrade": result.get("degrade"),
        "rename_map_size": result.get("rename_map_size"),
        "git_rename_map_size": result.get("git_rename_map_size"),
    }
...
_CLI_DISPATCH_OVERRIDES: dict[str, Callable[[argparse.Namespace], Any]] = {
    ...
    "continuation": _dispatch_continuation,
...
def _build_cli_registry() -> list[CliEntry]:
    ...
    # CLI-only extras: artifact variants + findings-triage (internal).
    registry.extend(
        [
            ...
            # implementation note: classify (--manifest) + apply (--apply --from-manifest).
            CliEntry(
                name="findings-triage",
                dispatch=_dispatch_findings_triage,
                description=(
                    "Classify open review findings into triage buckets (--manifest) or "
                    "apply a reviewed manifest (--apply --from-manifest). Apply uses "
                    "sanctioned disposition/reanchor ops only; fixtures first."
                ),
                args=[
                    ArgSpec(
                        "--manifest",
                        help="Write classification JSON to this path (classify mode).",
                    ),
                    ArgSpec(
                        "--rename-map",
                        help="JSON file of old_path -> new_path (implementation note rename map). Empty/absent degrades to live/high only.",
                    ),
                    ArgSpec(
                        "--git-rename-map",
                        action="store_true",
                        help=(
                            "Opt in to composing a git-derived rename map under the "
                            "caller --rename-map (default off; empty caller map is the fail-safe)."
                        ),
                    ),
                    ArgSpec(
                        "--apply",
                        action="store_true",
                        help="Apply dispositions/re-anchors from a reviewed manifest (requires --from-manifest).",
                    ),
                    ArgSpec(
                        "--from-manifest",
                        help="Reviewed manifest path for --apply.",
                    ),
                    ArgSpec(
                        "--batch-size",
                        type=int,
                        default=200,
                        help="Apply batch size (default 200; mirrors apply_stale_findings_gc).",
                    ),
                    ArgSpec(
                        "--dry-run",
                        action="store_true",
                        help="With --apply: report would-write outcomes without mutating.",
                    ),
                ],
            ),
        ]
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    errors_record_entry = CliEntry(
        name="errors-record",
        dispatch=_dispatch_errors_record,
        exit_nonzero_on_not_ok=False,
    )
    errors_record_parser = subparsers.add_parser(
        errors_record_entry.name,
        help=(
    ...
    errors_record_parser.set_defaults(_cli_entry=errors_record_entry)
    ...
    errors_replay_parser.add_argument(
        ...
        default=None,
        help=(
            "Maximum number of spool lines to drain this call ([RES-14]). "
            "Default: unbounded. Lifecycle callers pass 500 so a ~137-line "
            "backlog clears in one make context."
        ),
    )
    # internal: hook-facing terminal-guard record + lifecycle drain.
    # Mirror errors-record / errors-replay-spool (special-cased, not registry).
    terminal_guard_record_parser = subparsers.add_parser(
        "terminal-guard-record",
        help=(
            "Record a non-pass terminal-guard decision (ask|block) into "
            "terminal_guard_events. Used by PreToolUse guards via detached "
            "shell-out; prints the JSON envelope."
        ),
    )
    terminal_guard_record_parser.add_argument(
        "--decision",
        required=True,
        choices=("ask", "block"),
        help="Non-pass guard decision.",
    )
    terminal_guard_record_parser.add_argument("--tool-name", required=True)
    terminal_guard_record_parser.add_argument("--harness", required=True)
    terminal_guard_record_parser.add_argument("--command-preview", required=True)
    terminal_guard_record_parser.add_argument("--policy-version", required=True)
    terminal_guard_record_parser.add_argument("--policy-source", required=True)
    terminal_guard_record_parser.add_argument("--trigger")
    terminal_guard_record_parser.add_argument("--native-tool-hint")
    terminal_guard_record_parser.add_argument("--task-ref")
    terminal_guard_record_parser.add_argument("--worktree-path")
    terminal_guard_record_parser.add_argument("--fallback-source")
    terminal_guard_record_parser.add_argument("--created-at")
    terminal_guard_replay_parser = subparsers.add_parser(
        "terminal-guard-replay-spool",
        help=(
            "Replay spooled terminal-guard events from "
            ".task-state/terminal_guard.jsonl into the durable ledger; "
            "event-key dedupe. Lifecycle reclaimers drain this alongside "
            "agent-errors."
        ),
    )
    terminal_guard_replay_parser.add_argument(
        "--spool-path",
        help=("Explicit spool file to drain (default: runtime .task-state/terminal_guard.jsonl)."),
    )
    terminal_guard_replay_parser.add_argument(
        "--max-batch",
        type=int,
        default=None,
        help=(
            "Reserved for parity with errors-replay-spool / lifecycle "
            "callers (AGENT_ERRORS_REPLAY_MAX_BATCH). Full flock+max_batch "
    ...
    for entry in _build_cli_registry():
        # Tool descriptions are shared with MCP and may contain literal '%'
        # (e.g. the '%-REV-%' LIKE pattern); argparse %-interpolates help
        # strings, so escape or `--help` raises ValueError.
        sub = subparsers.add_parser(entry.name, help=entry.description.replace("%", "%%"))
        sub.set_defaults(_cli_entry=entry)
...
def main() -> None:
    ...
    if args.subcommand == "doctor":
        ...
        return
    entry = getattr(args, "_cli_entry", None)
    if isinstance(entry, CliEntry):
        result = entry.dispatch(args)
        exit_nonzero = entry.exit_nonzero_on_not_ok or (
            entry.name == "errors-record" and _errors_record_exit_nonzero(result)
        )
        _print_json(result, exit_nonzero_on_not_ok=exit_nonzero)
        return
    if args.subcommand == "errors-replay-spool":
        _print_json(_dispatch_errors_replay_spool(args))
        return
    if args.subcommand == "terminal-guard-record":
        _print_json(_dispatch_terminal_guard_record(args))
        return
    if args.subcommand == "terminal-guard-replay-spool":
        _print_json(_dispatch_terminal_guard_replay_spool(args))
        return
