...
def _resolve_primary_worktree_root(start_dir: Path) -> Path | None:
    ...
    The probe is told which repo to look at (``git -C <start_dir>``) rather
    than asking ambient environment state. Ambient ``GIT_DIR`` /
    ``GIT_COMMON_DIR`` / ``GIT_WORK_TREE`` are stripped so an outer git
    invocation cannot redirect discovery to another repository. That strip
    set is shared with ``resolve_lane_python``: the two must change together
    or the parity pin reddens.
...
class RuntimeConfig:
    ...
    def for_workspace(
        ...
        *,
        ...
        tool_roster_policy: str | None = None,
        ...
        # Precedence (ARCH-13, one sanctioned path for all constructors):
        #   1. explicit state_dir= / --state-dir
        #   2. env WORKBAY_HANDOFF_STATE_DIR
        #   3. derived default <workspace_root>/.task-state
        # for_repo delegates here with state_dir=None when callers omit it;
        # from_args already rewrites env into an explicit state_dir before
        # calling for_repo, but honouring the env at this resolver means
        # direct for_repo / for_workspace callers cannot bypass the suite
        # hermetic pin (HERMETICDB-R1-F1 / HARM-01).
        if state_dir is not None:
            resolved_state_dir = _resolve_runtime_path(state_dir, workspace_root=resolved_workspace_root)
        else:
            env_state_dir = resolve_env_alias("WORKBAY_HANDOFF_STATE_DIR")
            if env_state_dir is not None:
                resolved_state_dir = _resolve_runtime_path(env_state_dir, workspace_root=resolved_workspace_root)
            else:
                # Match bootstrap_owned_root below: resolve so symlink / case
                # spellings cannot bypass containment. [SEC-01][WEB-13]
                resolved_state_dir = (resolved_workspace_root / ".task-state").resolve()

        # Refuse state under the bootstrap-owned install root (.workbay). Relative
        # to this workspace_root only — a clone that *is* nested under some outer
        # .workbay remains valid so long as its own state is not under its own
        # .workbay (ARCH-13: make the wrong state unrepresentable).
        #
        # Resolve the comparison root: ``resolved_state_dir`` already follows
        # symlinks via ``_resolve_runtime_path`` / ``Path.resolve()``. Without
        # resolving ``workspace/.workbay`` the same way, a symlink install root
        # makes ``relative_to`` raise ValueError and the guard fails open
        # (internal). Non-strict resolve still yields a usable absolute path
        # when ``.workbay`` does not exist yet.
        bootstrap_owned_root = (resolved_workspace_root / ".workbay").resolve()
        try:
            resolved_state_dir.relative_to(bootstrap_owned_root)
        except ValueError:
            pass
        else:
            # Name the logical default path without resolving it: if
            # workspace/.task-state is a symlink into .workbay, the resolved
            # form is the path we just refused — do not tell the operator to
            # "move state to" that refused location. [SEC-01]
            logical_default = resolved_workspace_root / ".task-state"
            raise ConsumerRootResolutionError(
                f"state_dir {resolved_state_dir} must not sit at or inside the "
                f"bootstrap-owned install root {bootstrap_owned_root}. "
                f"Handoff runtime state must live outside that install root "
                f"(typical layout path: {logical_default}; if that path is a "
                f"symlink into .workbay, replace it with a real directory "
                f"outside the install root)."
            )
        ...
        if tool_roster_policy is None:
            resolved_roster_policy = resolve_env_alias("WORKBAY_TOOL_ROSTER_POLICY") or "all"
        else:
            resolved_roster_policy = tool_roster_policy
        if resolved_roster_policy not in {"all", "skill"}:
            logging.getLogger("workbay_handoff_mcp.roster").warning(
                "policy_coerced", extra={"value": resolved_roster_policy}
            )
            resolved_roster_policy = "all"
        ...
        return cls(
            ...
            tool_roster_policy=resolved_roster_policy,
    ...
    def for_repo(
        ...
        *,
        ...
        if primary_root is None:
            # OBS-08: silence is not success. Non-git fallback must stay available
            # for tests and ad-hoc contexts, but minting a DB outside any git
            # repository without a signal is worse than a loud fallback.
            # Inverse of OBS-08 for volume: warn once per resolved path so daemon
            # loops that call for_repo repeatedly do not bury real signal
            # (internal).
            start_key = str(start)
            if start_key not in _non_git_fallback_warned_paths:
                _non_git_fallback_warned_paths.add(start_key)
                logger.warning(
                    "No git repository found under probed directory %s; falling back "
                    "to that directory as workspace root (%s). A handoff database may "
                    "be created outside any git repository.",
                    start,
                    start,
                )
        ...
        return cls.for_workspace(
            ...
            tool_roster_policy=tool_roster_policy,
    ...
    def from_args(cls, args: object) -> RuntimeConfig:
        ...
        return cls.for_repo(
            ...
            tool_roster_policy=getattr(args, "tool_roster_policy", None),
        )


logger = logging.getLogger(__name__)

# Paths already warned for non-git for_repo fallback (once per resolved path).
_non_git_fallback_warned_paths: set[str] = set()
