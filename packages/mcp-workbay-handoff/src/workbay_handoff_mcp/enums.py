...
class LaneStatus(StrEnum):
    ...
    MERGED = "merged"
    CLOSED = "closed"
    # Reaper terminal status; schema CHECK stores it. close_lane stays
    # closed|merged — that is an operator action, not the reaper CAS path.
...
class WorkerEventName(StrEnum):
    ...
    DORMANT_DEADLINE_EXCEEDED = "dormant_deadline_exceeded"
