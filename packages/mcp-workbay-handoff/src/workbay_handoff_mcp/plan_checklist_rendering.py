...
def render_plan_checklist(task_ref: str, *, plan_path: str | None = None) -> dict[str, Any]:
    ...
    lifecycle_pkg = root / "packages/workbay-system/workbay_system/payload/scripts/workbay_lifecycle"
...
def count_evidenced_unticked_boxes(task_ref: str) -> int | None:
    ...
    return sum(
        1 for item in items if isinstance(item, dict) and item.get("projected") == "tick" and not item.get("doc_ticked")
    )
