...
from .compaction_contract import (
    load_compaction_contract as _load_compaction_contract_from_disk,
)
...
_CompactionContractCacheKey = tuple[Path, str]
_compaction_contract_cache: tuple[_CompactionContractCacheKey, CompactionContract] | None = None
_compaction_contract_cache_lock = Lock()


def _parse_compaction_contract_content(contract_path: Path, content: bytes) -> CompactionContract:
    """Parse a contract from the exact bytes used to derive its cache identity."""
    ...
    for harness_name, harness_payload in discovery_payload.items():
        ...
        if not isinstance(harness_payload, dict):
            raise ValueError(
                f"Compaction contract at {contract_path} has a non-mapping transcript discovery entry "
                f"for {harness_name!r}"
            )
...
def load_compaction_contract(
    workspace_root: str | Path,
    *,
    contract_path: Path | None = None,
    content: bytes | None = None,
) -> CompactionContract:
    """Load from disk normally, or parse a caller's single-read snapshot."""

    if contract_path is None or content is None:
        return _load_compaction_contract_from_disk(workspace_root)
    return _parse_compaction_contract_content(contract_path, content)


def _read_compaction_contract(
    workspace_root: str | Path,
) -> tuple[_CompactionContractCacheKey, bytes]:
    """Read the resolved contract once and return its content identity."""
    ...
    return (resolved_path, hashlib.sha256(content).hexdigest()), content


def _compaction_contract_cache_key(workspace_root: str | Path) -> _CompactionContractCacheKey:
    """Return the content-invalidating identity for the resolved contract."""

    cache_key, _ = _read_compaction_contract(workspace_root)
    return cache_key


def _load_compaction_contract_memoized(
    workspace_root: str | Path,
) -> tuple[CompactionContract, _CompactionContractCacheKey]:
    """Load the contract through a bounded, content-invalidated cache."""

    global _compaction_contract_cache

    cache_key, content = _read_compaction_contract(workspace_root)
    with _compaction_contract_cache_lock:
        cached = _compaction_contract_cache
        if cached is not None and cached[0] == cache_key:
            return cached[1], cache_key
        contract = load_compaction_contract(
            workspace_root,
            contract_path=cache_key[0],
            content=content,
        )
        _compaction_contract_cache = (cache_key, contract)
        return contract, cache_key
...
def compact_session(
    ...
    *,
    ...
    # Capture outcome: failed is surfaced via hook WARNING +
    # agent_errors.embedding_failed ([DATA-13], [OBS-08]).
    _prose_outcome = embed_concept_on_write(
        "compaction.prose_residual", summary.compaction_id, summary.task_ref, summary.prose_residual
    )
    if _prose_outcome == "failed":
        _log.warning(
            "compaction prose residual embed failed for compaction_id=%s task_ref=%s",
            summary.compaction_id,
            summary.task_ref,
        )
    ...
    # Observe AnchorStoreOutcome rather than discarding it ([ARCH-13], [OBS-08]).
    # The store already logs WARNING + increments process-local counters on fail.
    _anchor_outcome = embed_compaction_anchor_on_write(summary.compaction_id, summary.task_ref, anchor_source_text)
    if _anchor_outcome == "failed":
        _log.warning(
            "compaction anchor embed failed for compaction_id=%s task_ref=%s",
            summary.compaction_id,
            summary.task_ref,
        )
...
def _embeddings_mode_from_ledger() -> Literal["unspecified", "verified", "disabled"]:
    """Echo install-time embeddings capability from the bootstrap ledger (implementation note S5).

    Uses the same workspace root as the rest of the handoff runtime. Missing
    runtime / missing ledger / unknown value all read as ``unspecified`` —
    matching ``load_embeddings_mode`` rollback-safety so absence is expected
    under pre-flag installs.
    """
    try:
        from workbay_protocol.bootstrap import load_embeddings_mode

        from .runtime import get_runtime_config

        return load_embeddings_mode(get_runtime_config().workspace_root)
    except Exception:  # noqa: BLE001 - capability echo must never raise
        return "unspecified"


def _semantic_degrade_reason_from_skip(
    *,
    embeddings_mode: str,
    status: object,
    skip_reason: object,
) -> str | None:
    """Derive the native "flag when not available" signal (implementation note S5).

    Single-source with ``skip_reason``: non-null only when the operator
    installed ``--with-embeddings`` (``embeddings_mode == "verified"``) and
    semantic selection fell back to skip/degrade. Never invent a reason
    independent of the existing skip plumbing; under unspecified/disabled
    the field stays null (no behavior change).
    """
    if embeddings_mode != "verified":
        return None
    if status not in ("skipped", "degraded"):
        return None
...
def semantic_reinjection_packet(
    ...
    embeddings_mode = _embeddings_mode_from_ledger()
    try:
        from .embeddings.reinjection import (
        ...
        return {
            ...
            "semantic_degrade_reason": _semantic_degrade_reason_from_skip(
                embeddings_mode=embeddings_mode,
                status=status,
                skip_reason=skip_reason,
            ),
...
class _CompactionAdvisoryPrecomputed:
    """Workspace/env-derived advisory inputs reusable across task rows."""

    workspace_root: Path
    key: tuple[Path, _CompactionContractCacheKey, tuple[tuple[str, str], ...]] | None
    contract_source: dict | None = None
    effective: EffectiveThresholds | None = None
...
def _compaction_advisory_env_key(
    contract: CompactionContract,
    env: Mapping[str, str],
) -> tuple[tuple[str, str], ...]:
    """Fingerprint env values consumed by invariant advisory resolution."""
    ...
    # ``Path.expanduser`` consults the process environment even when callers
    # supply an explicit advisory env mapping.
    values.append(("__process_home__", os.environ.get("HOME", "")))
    values.append(("__process_cwd__", str(Path.cwd().resolve())))
    return tuple(values)


def _precomputed_compaction_advisory_matches(
    precomputed: _CompactionAdvisoryPrecomputed,
    *,
    workspace_root: str | Path,
    env: Mapping[str, str],
) -> bool:
    """Check that a caller-supplied invariant bundle still matches its inputs."""

    requested_root = Path(workspace_root).expanduser().resolve()
    if requested_root != precomputed.workspace_root:
        return False
    if precomputed.key is None:
        return precomputed.skip_warning is not None
    root, contract_key, env_key = precomputed.key
    if requested_root != root:
        return False
    try:
        if _compaction_contract_cache_key(root) != contract_key:
            return False
    except (FileNotFoundError, OSError):
        return False
    current_env_values: list[tuple[str, str]] = []
    for name, _ in env_key:
        if name == "__process_home__":
            value = os.environ.get("HOME", "")
        elif name == "__process_cwd__":
            value = str(Path.cwd().resolve())
        else:
            value = env.get(name, "")
        current_env_values.append((name, value))
    return tuple(current_env_values) == env_key


def _precompute_compaction_advisory(
    *,
    workspace_root: str | Path,
    env: Mapping[str, str] | None = None,
) -> _CompactionAdvisoryPrecomputed:
    """Resolve all workspace/env advisory inputs once for a task collection."""

    source_env: Mapping[str, str] = env if env is not None else os.environ
    resolved_root = Path(workspace_root).expanduser().resolve()
    try:
        contract, contract_key = _load_compaction_contract_memoized(resolved_root)
    except FileNotFoundError as exc:
        return _CompactionAdvisoryPrecomputed(
            workspace_root=resolved_root,
            key=None,
            skip_warning=f"missing compaction contract: {exc}",
        )
    except (ValueError, OSError) as exc:
        return _CompactionAdvisoryPrecomputed(
            workspace_root=resolved_root,
            key=None,
            skip_warning=f"unreadable compaction contract: {exc}",
        )

    invariant_key = (
        resolved_root,
        contract_key,
        _compaction_advisory_env_key(contract, source_env),
    )
    warnings: list[str] = []
    contract_source = build_contract_source_report(contract, workspace_root=resolved_root).model_dump(mode="json")
...
def compute_compaction_advisory(
    *,
    ...
    if invariant is None or not _precomputed_compaction_advisory_matches(
        invariant,
        workspace_root=workspace_root,
        env=source_env,
    ):
        invariant = _precompute_compaction_advisory(workspace_root=workspace_root, env=source_env)
    if invariant.skip_warning is not None:
        return _skip_advisory([invariant.skip_warning])

    contract_source = invariant.contract_source
    effective = invariant.effective
    if contract_source is None or effective is None:  # pragma: no cover - constructor invariant
        return _skip_advisory(["unreadable compaction advisory precomputed inputs"])
    warnings = list(invariant.warnings)
    harness = invariant.harness
    transcript_path = invariant.transcript_path
    transcript_source = invariant.transcript_source
    observed_chars = invariant.observed_chars
