...
def _needs_attention_findings(open_findings: dict[str, list[dict]]) -> list[_NeedsAttentionItem]:
    ...
    for task_ref, findings in open_findings.items():
        flagged = [f for f in findings if str(f.get("severity") or "").lower() in severities]
        if not flagged:
            continue
        counts: dict[str, int] = {}
        ...
        parts = [f"{counts[s]} {s}" for s in sorted(counts.keys())]
        has_high = counts.get("high", 0) > 0
        items.append(
            {
                "task_ref": task_ref,
                "kind": "findings",
                "detail": f"{len(flagged)} open ({', '.join(parts)})",
...
def generate_dashboard_md(write_file: bool = True, *, full: bool = False) -> dict:
    ...
    with _get_db_connection() as conn:
        ...
        open_findings = _collect_all_open_findings(conn, max_per_task=100, include_stale=full)
        ...
        if not full:
            active_ref_set = set(active_task_refs)
            open_findings = _filter_findings_to_task_refs(open_findings, active_ref_set)
            deferred_findings = _filter_findings_to_task_refs(deferred_findings, active_ref_set)
            resolved_findings = _filter_findings_to_task_refs(resolved_findings, active_ref_set)
        needs_attention = _collect_needs_attention(dashboard_rows, open_findings)
