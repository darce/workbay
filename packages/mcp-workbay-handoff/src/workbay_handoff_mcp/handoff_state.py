...
from .read_budget import (
    ...
    apply_bare_call_budget_defaults,
    budget_payload,
    plan_state_read,
    resolve_policy,
)
...
# Merge-safe fields that may update an existing row without expected_revision.
# When the provided (non-None) field set is a non-empty subset of this allowlist
# and expected_revision is omitted, the write auto-adopts the row's current
# revision inside the open transaction. All other updates keep the fence.
REVISION_OPTIONAL_FIELDS = frozenset({"focus", "task_plan_path"})
...
def set_handoff_state(
    ...
    if result.get("ok"):
        ...
        # Identity fields (target_branch / target_worktree_path) and terminal
        # status transitions must flush CURRENT_TASK.json unconditionally so the
        # snapshot cannot advertise a prior task's branch/worktree or a task
        # that has left LIVE_ACTIVE_STATUSES [OBS-08][ARCH-13]. Routine
        # mid-lifecycle (live-to-live) writes stay derive-on-read gated.
        # Terminal gate mirrors import_export.update_task_status.
        if (
            target_branch is not None
            or target_worktree_path is not None
            or (status is not None and status not in LIVE_ACTIVE_STATUSES)
        ):
            _write_workspace_summary_current_task_json(unconditional=True)
        # Embed objective/focus after the row committed (best-effort).
        # None params (e.g. a status-only update) are a no-op, so unchanged text
        # never re-embeds. task_plan_path is deliberately not embedded: it is
        # unresolvable in reinjection and not task-memory prose (implementation note S2).
        # Capture outcomes into side_effect_failures + warnings (same shape as
        # close_slice). Primary write stays ok; embed is non-fatal
        # ([DATA-13], [OBS-08], [RES-02]). Production failures return ``failed``
        # rather than raising ([RES-02]).
        for field_name, text in (
            ("handoff_state.objective", objective),
            ("handoff_state.focus", focus),
        ):
            try:
                outcome = embed_concept_on_write(field_name, task_ref, task_ref, text)
                if outcome == "failed":
                    msg = (
                        "embed_concept_on_write failed during set_handoff_state: "
                        f"provider/store returned failed for {field_name}"
                    )
                    _surface_side_effect_failure(
                        result,
                ...
                msg = f"embed_concept_on_write failed during set_handoff_state for {field_name}: {exc}"
                _surface_side_effect_failure(
                    result,
                    effect="embed_concept_on_write",
                    detail=msg,
...
def _set_handoff_state_with_conn(
    ...
    *,
    ...
    # Callers that already resolved outside a write lock (close_slice) pass
    # resolved_ctx so this path stays DB-only under BEGIN IMMEDIATE ([internal]).
    ...
    if task_row is None:
        ...
        # Create path: omitted status defaults to in_progress (legacy behavior).
        create_status = "in_progress" if status is None else status
    ...
    # Partial-update provided-set: non-None columns among the contract fields.
    provided = {
        name
        for name, value in (
            ("focus", focus),
    ...
    if expected_revision is None:
        # REVISION_OPTIONAL_FIELDS allowlist: focus / task_plan_path only may
        # omit expected_revision; adopt the live row revision in-transaction.
        if provided and provided.issubset(REVISION_OPTIONAL_FIELDS):
    ...
    # None on UPDATE means preserve the stored column (partial-update contract).
    ...
    resolved_status = status if status is not None else str(task_row["status"])
    ...
    # Build UPDATE that SETs only provided (non-None) contract columns plus
    # revision/audit fields. Unprovided columns keep their stored values.
    set_parts: list[str] = []
    set_params: list[object] = []
    if "objective" in provided:
        set_parts.append("objective = ?")
        set_params.append(resolved_objective)
    if "focus" in provided:
        set_parts.append("focus = ?")
        set_params.append(resolved_focus)
    if "status" in provided:
        set_parts.append("status = ?")
        set_params.append(resolved_status)
    if "target_branch" in provided:
        set_parts.append("target_branch = ?")
        set_params.append(resolved_target_branch)
    if "target_worktree_path" in provided:
        set_parts.append("target_worktree_path = ?")
        set_params.append(resolved_target_worktree_path)
    if "task_plan_path" in provided:
        set_parts.append("task_plan_path = ?")
        set_params.append(resolved_task_plan_path)
    # Always bump revision and audit metadata on a successful CAS update.
    set_parts.extend(
        [
            "revision = revision + 1",
    ...
    set_params.extend([ctx.agent, ctx.branch, ctx.commit_sha])
    # Prefer precomputed warnings only when the caller intentionally supplies
    # them. close_slice omits them so collect re-runs pure-SQL branch/cwd checks
    # under BEGIN IMMEDIATE against committed state ([internal], [ARCH-13],
    # [CON-11], [RES-02], [CON-22]). Thread any pre-derived worktree path so
    # git porcelain is not re-run under the write lock.
    if target_context_warnings is not None:
        warnings = list(target_context_warnings)
    else:
        warnings = collect_target_context_warnings(
            ...
            target_worktree_path=(
                precomputed_target_worktree_path
                if precomputed_target_worktree_path is not None
                else resolved_target_worktree_path
            ),
            task_ref=task_ref,
        )
...
def get_handoff_state(
    ...
    #
    # [DATA-14] bare-call default budget: no read_profile, no
    # response_budget_bytes, and no explicit sections= → apply
    # DEFAULT_BARE_CALL_RESPONSE_BUDGET_BYTES with auto_summary so chatty
    # unshaped reads stay bounded. S10-A-02: an explicit sections= selection
    # is already shaped and stays exempt.
    response_budget_bytes, budget_policy, _bare_budget_applied = apply_bare_call_budget_defaults(
    ...
    with _get_db_connection() as conn:
        ...
        # [T23 / DATA-14] write-contract registry is ~13KB and is opt-in.
        # Routine identity/hot_summary reads keep scalar write limits only;
        # pass include_write_schemas=True to restore the full tools block
        # (byte-identical shape to the pre-0108 default).
        if include_write_schemas:
            # internal / BR-04: publish the write-contract registry through
            # limits.write so callers can discover required fields and field
            # grammars without importing internals.
            write_limits["tools"] = _write_contracts_registry_export()
        result["limits"] = {
            **limits.as_payload(),
            "write": write_limits,
        }
