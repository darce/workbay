...
LOCK_RETRY_EXCLUDED_TOOLS: dict[str, str] = {
    "record_event": (
        "identical re-invocation duplicates rows (blockers / verified_tests)"
    ),
    "set_handoff_state": (
    ...
    "archive": (
        "identical re-archive returns ok:true but rewrites snapshot_json "
        "(content length changes) rather than no-opping"
    ),
    "compaction": (
        "identical record re-invocation allocates a new compaction_id and "
        "session_compactions row each call"
    ),
    "import_handoff_state": (
        "identical merge re-import grows next_actions row count"
    ),
    "next_actions": (
        "identical add re-invocation inserts a second next_actions row"
    ),
    ...
    "review_runs": (
        "identical record re-invocation returns ok:false "
        "(review_run_id already exists)"
    ),
    ...
    "touched_files": (
        "identical record re-invocation always INSERT; touched_files row "
        "count grows"
    ),
}
    ...
    if kind is SqliteLockKind.UNKNOWN:
        ...
        if isinstance(boundary, str):
            try:
                ...
                raise ValueError(
                    f"unknown retry boundary {boundary!r}; "
                    f"expected one of {[b.value for b in RetryBoundary]}"
                ) from exc_boundary
...
def _probe_os_lock_holders(
    ...
    *,
    ...
    status, pids = _collect_os_holder_pids(
        db_path, budget_seconds=max(0.05, deadline - time.monotonic())
    )
...
def _registry_notes_fragment(registry_holders: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        f"pid={r0['pid']} label={r0.get('label')!r}"
    )
...
def _format_holder_attribution(
    *,
    ...
    if os_outcome == "named" and os_holders:
        reg_by_pid = {
            int(h["pid"]): h for h in registry_holders if h.get("pid") is not None
        }
        extras: list[str] = []
        for h in os_holders:
            ...
            if reg is not None and reg.get("label"):
                extras.append(
                    f"registry_label={reg['label']!r} writer_id={reg.get('writer_id')!r}"
                )
        ...
        return (
            f"write-lock holder candidates pids=[{pid_list}] source=os"
            f"{extra_suffix}"
        )
    ...
    if os_outcome == "openers" and os_holders:
        pid_list = ",".join(str(h["pid"]) for h in os_holders)
        return (
            f"file-opener candidates (not necessarily lock holders) "
            f"pids=[{pid_list}] source=os_openers"
            f"{reg_notes}"
        )
    ...
    if os_outcome == "skipped_write_lock_held":
        return (
            f"{HOLDER_ATTR_PROBE_UNAVAILABLE_PREFIX}: "
            "write lock held in this process (probe skipped)"
            + reg_notes
        )
    # unavailable:<reason>
...
def describe_lock_holder_report(
    ...
    *,
    ...
    os_outcome, os_holders = _probe_os_lock_holders(
        path, timeout_seconds=probe_timeout_seconds
    )
    ...
    if os_holders:
        ...
        for row in os_holders:
            ...
            if reg is not None:
                if reg.get("label") and (
                    not item.get("label") or str(item.get("label")).startswith("pid:")
                ):
...
class DbBusyError(RuntimeError):
    ...
    def __init__(
        ...
        *,
        ...
        self.database_path = (
            database_path if database_path is not None else DATABASE_PATH_UNAVAILABLE
        )
        ...
        if holder_attribution is not None:
            ...
            if (
                ...
                age_part = (
                    f" heartbeat_age_seconds={age}" if age is not None else ""
                )
                holder_msg = (
                    f"holder pid={primary['pid']} label={primary['label']!r}"
                    f"{age_part} source=os"
                )
    ...
    def as_data(self) -> dict[str, object]:
        ...
        if (
            ...
            data["registry_holder_heartbeat_age_seconds"] = primary[
    ...
    *,
    ...
    while True:
        ...
        try:
            ...
            retryable = is_sqlite_lock_error(
                exc, boundary=RetryBoundary.WHOLE_CALL
            )
    ...
    try:
        kind = (
            classify_sqlite_lock_error(last_exc)
            if last_exc is not None
            else SqliteLockKind.UNKNOWN
        )
    except Exception:
