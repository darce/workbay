...
from .lanes_recording import branch_reclaim_candidate_decision_id as _branch_reclaim_candidate_decision_id
from .lanes_recording import close_lane as _close_lane
from .lanes_recording import get_lane as _get_lane
from .lanes_recording import latest_branch_reclaim_candidate as _latest_branch_reclaim_candidate
...
from .lanes_recording import reclaim_candidate_decision_id as _reclaim_candidate_decision_id
...
LaneStatus = Literal["planned", "active", "blocked", "review", "merged", "closed", "closed_stale"]
# Open/update must not accept closed|merged: those terminals reclaim
# lane_messages only on close_lane. closed_stale stays for reaper CAS
# round-trip via update/open [cs0166-r12-16].
OpenUpdateLaneStatus = Literal["planned", "active", "blocked", "review", "closed_stale"]
...
class LanesOpenOp(BaseModel):
    ...
    speed: Annotated[str | None, Field(description="Optional routing speed.")] = None
    tier: Annotated[str | None, Field(description="Optional routing tier.")] = None
    status: OpenUpdateLaneStatus = "planned"
...
class LanesUpdateOp(BaseModel):
    ...
    speed: Annotated[str | None, Field(description="Optional routing speed.")] = None
    tier: Annotated[str | None, Field(description="Optional routing tier.")] = None
    status: OpenUpdateLaneStatus | None = None
    notes: Annotated[str | None, Field(description="Optional free-form notes.")] = None
    task_ref: TaskRefParam = None
    actor: ActorParam = None
...
class LanesCloseOp(BaseModel):
    ...
    landing_commit_sha: Annotated[
        str | None,
        Field(description="Optional full SHA of the commit where the lane landed."),
    ] = None
    branch_tip_sha: Annotated[
        str | None,
        Field(description="Optional full SHA observed at the lane branch tip."),
    ] = None
    branch_tip_source: Annotated[
        str | None,
        Field(description="Optional provenance for the supplied branch tip SHA."),
    ] = None
...
def dispatch_lanes(payload: LanesOp) -> dict:
    ...
    if isinstance(payload, LanesCloseOp):
        return _close_lane(
            lane_id=payload.lane_id,
            status=payload.status,
            landing_commit_sha=payload.landing_commit_sha,
            branch_tip_sha=payload.branch_tip_sha,
            branch_tip_source=payload.branch_tip_source,
            notes=payload.notes,
            task_ref=payload.task_ref,
            actor=actor,
        )
...
latest_branch_reclaim_candidate = _latest_branch_reclaim_candidate
reclaim_candidate_decision_id = _reclaim_candidate_decision_id
branch_reclaim_candidate_decision_id = _branch_reclaim_candidate_decision_id
