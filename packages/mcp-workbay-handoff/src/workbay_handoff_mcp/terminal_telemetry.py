...
def _resolve_repo_instance_id(
    conn,
    *,
    seen_at: str,
    git_common_dir: str | None,
) -> str:
    """Resolve or create a repo_instances row for the current workspace.

    ``git_common_dir`` must be precomputed **before** the write connection is
    opened (AUDITHOOK-TP02). Pass the soft ``None`` returned by
    :func:`_resolve_git_common_dir` on probe failure — do not re-shell under
    RESERVED.
    """
...
def _prepare_terminal_guard_event(
    ...
    *,
    ...
    # git_common_dir is precomputed outside the DB connection (AUDITHOOK-TP02).
    # Soft None from a failed git probe is preserved; never re-resolve here.
    repo_instance_id = _resolve_repo_instance_id(
        conn,
        seen_at=normalized_created_at,
...
def record_terminal_guard_event(
    *,
    ...
    try:
        # Resolve git common dir once before any DB connection (AUDITHOOK-TP02).
        # Soft None on probe failure is intentional — callers depend on that.
        # Kept inside the try so runtime/resolution failures still spool.
...
def replay_terminal_guard_spool(*, spool_path: str | None = None) -> dict:
    ...
    # Resolve git common dir once per call, before the DB connection
    # (AUDITHOOK-TP02). Soft None on probe failure is intentional.
