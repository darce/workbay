...
from .shared_schema import holds_write_lock, process_holds_write_lock
...
# Audit-hook install counter (observable for idempotence pins). Incremented only
# when a hook is actually registered via sys.addaudithook. Audit hooks cannot be
# removed once added; a second install is a permanent duplicate.
_AUDIT_HOOK_INSTALL_COUNT = 0
...
class WriteLockHeldAcrossBlockingWorkError(RuntimeError):
    """Fail-closed barrier: blocking work attempted while this process holds RESERVED.

    Designs out the Coffman **hold-and-wait** condition ([CON-21]) by refusing
    subprocess, network, sleep, flock acquire, and lane-dispatch work while any
    tracked connection in this process holds a SQLite write lock ([CON-18]).
    Failure mode is raise (not log-and-continue): a guard that only logs is the
    defect that already shipped a multi-minute hold across remote dispatch.
    """


class ExternalResolutionInTransactionError(WriteLockHeldAcrossBlockingWorkError):
    """Raised when git/subprocess resolution is attempted while a write transaction is open.

    Write locks taken via the connection-factory ``BEGIN IMMEDIATE`` path must
    stay database-only ([ARCH-13], [CON-11], [RES-02], [CON-22]). Callers
    precompute actor context and target worktree paths on a deferred connection,
    then pass the resolved values into the write helpers so no external
    resolution runs under the exclusive lock.

    Subclass of :class:`WriteLockHeldAcrossBlockingWorkError` so call sites that
    catch the general barrier still see this specialized message for the
    historical git/subprocess resolution class.
    """
...
def _infer_harness_agent_from_env() -> str | None:
    """Best-effort harness slug from transcript env vars, then identity markers.

    Transcript-path detection (``detect_active_harness``) wins when exactly one
    transcript env var is set. When that returns no harness, fall back to
    presence-only identity markers (``detect_harness_from_identity_markers``),
    e.g. ``CLAUDE_CODE_SESSION_ID`` for Claude Code MCP subprocesses that never
    receive a transcript path. Fail-open: any exception → ``None``.
    """
    try:
        ...
        resolution = detect_active_harness(contract, env=env)
        if resolution.harness is not None:
            return str(resolution.harness)
        # internal: fall back to identity markers ONLY when transcript
        # detection is ABSENT — not AMBIGUOUS. A multiple-transcript conflict must
        # degrade to None (honoring the conflict), never be silently overridden by
        # a lower-confidence marker. This is a hard prerequisite for the Slice-1
        # cross-harness override: the override must never elevate a marker while a
        # genuine transcript conflict exists. NOTE: this returns None (not the
        # marker) even in the no-row case, so a bare write (no row, no explicit
        # actor, no WORKBAY_HANDOFF_DEFAULT_AGENT) under two conflicting transcript
        # vars now fails loud (WriteActorAttributionError) rather than guessing —
        # the intended "honor the conflict" behavior. Also degrades the
        # orientation_reads.resolve_harness() telemetry slug to its CLAUDECODE/
        # unknown fallback under the same conflict (benign).
        if resolution.ambiguous:
            return None
        # MCP subprocesses often lack transcript path vars but still carry
        # harness identity markers (contract.harness_identity_markers).
        marker_resolution = detect_harness_from_identity_markers(contract, env)
        if marker_resolution.harness is None:
            return None
        return str(marker_resolution.harness)
    except Exception:
        return None


def normalize_actor_harness(agent: str | None) -> str:
    """Map free-form writer identity to the coarse provenance harness."""
    normalized = _normalize_optional_text(agent)
    if normalized is None:
        return "unknown"
    slug = normalized.lower().replace("_", "-")
    if slug.startswith("claude"):
        return "claude"
    if slug.startswith("codex") or slug.startswith("gpt-"):
        return "codex"
    if slug in {"cursor", "vscode", "composer"} or slug.startswith("cursor-"):
        return "cursor"
    if slug.startswith("grok"):
        return "grok"
    if "copilot" in slug:
        return "copilot"
    if slug in {"automation", "hook", "mcp", "manual", "workbay-lifecycle", "tasks-gc"}:
        return "automation"
    if slug == "unknown":
        return "unknown"
    return "unknown"
...
def _prefer_live_harness_over_stale_writer(active_agent: str | None, harness_agent: str | None) -> str | None:
    """Cross-harness precedence override (implementation note; resolves finding 5170).

    ``active_agent`` (the handoff row's last-writer) is normally the *better*,
    more-specific signal — a worker continuing its OWN row is that last-writer. But
    when a coordinator on a DIFFERENT harness writes, without an explicit ``actor``,
    to a row last-touched by a lane worker, the stale last-writer wins and the write
    is mis-attributed to that lane. Resolve the live harness marker over the stale
    writer ONLY in that cross-harness-mismatch case; keep ``active_agent`` in every
    other case (zero regression). Pure — no raise.

    - ``harness_agent`` falsy → ``active_agent`` (no live signal; absent OR — via
      implementation note — ambiguous transcript state, both of which yield ``None``).
    - ``active_agent`` falsy → ``harness_agent`` (the no-row case 0107 fixed).
    - same coarse harness → keep ``active_agent`` (worker-continues-own-row; do not
      downgrade a specific identity to a coarse harness slug).
    - cross-harness mismatch → ``harness_agent`` (the coordinator's live identity is
      the correct, more-current signal). **This is the fix.**

    "Cross-harness" is coarse (``normalize_actor_harness``): the override also fires
    when the row's last-writer is an ``automation``/``unknown`` bucket identity and
    a different live harness marker is present — intentionally, since the live
    writer, not the stale automation/foreign last-writer, is doing this write.
    Legitimate automation/GC writers pass an explicit ``actor`` (``explicit_agent``
    wins above this rung), so they are never re-attributed.
    """
    if not harness_agent:
        return active_agent
    if not active_agent:
        return harness_agent
    if normalize_actor_harness(active_agent) == normalize_actor_harness(harness_agent):
        return active_agent
    return harness_agent
...
def assert_no_write_lock_held(reason: str) -> None:
    """Fail closed when *this thread* holds RESERVED and *reason* is about to block.

    Thread-scoped view via :func:`process_holds_write_lock` — chokepoints have
    no ``conn`` in hand. Raises :class:`WriteLockHeldAcrossBlockingWorkError`
    (fail-closed). A log-only guard is not acceptable: the live outage held
    RESERVED across a remote-lane network wait for >5 minutes. A sibling
    thread's RESERVED hold does not refuse a lock-free caller ([CON-21]).

    **Not covered** (a green barrier is not a hold-time guarantee): pure-SQL
    duration of large multi-row DML, filesystem walks (``os.stat`` /
    ``iterdir``), large ``json.dumps`` serialization, and any raw blocking
    call that bypasses the five wrappers (``run_subprocess`` /
    ``popen_subprocess`` / ``blocking_sleep`` / ``acquire_flock`` /
    ``assert_safe_for_network`` / ``assert_safe_for_lane_dispatch``). The
    complementary signal is the non-gating RESERVED-window duration
    observation at write-transaction exit
    (:meth:`_TrackedConnection._set_holds_write_lock` in ``shared_schema``).

    Parameters
    ----------
    reason:
        Short work-class label embedded in the error
        (``subprocess``, ``network``, ``lane_dispatch``, ``sleep``,
        ``fcntl.flock``, ``os_holder_probe``, …).
    """
    if process_holds_write_lock():
        raise WriteLockHeldAcrossBlockingWorkError(
            f"blocking work ({reason}) must not run while a write lock "
            f"(RESERVED) is held by this thread; release the transaction "
            f"before {reason}"
        )
...
def run_subprocess(
    ...
    **kwargs: Any,
    ...
    """``subprocess.run`` gated by :func:`assert_no_write_lock_held`."""
    assert_no_write_lock_held("subprocess")
    return subprocess.run(args, **kwargs)


def popen_subprocess(*args: Any, **kwargs: Any) -> subprocess.Popen[Any]:
    """``subprocess.Popen`` gated by :func:`assert_no_write_lock_held`."""
    assert_no_write_lock_held("subprocess")
    return subprocess.Popen(*args, **kwargs)


def blocking_sleep(seconds: float) -> None:
    """``time.sleep`` gated by :func:`assert_no_write_lock_held`."""
    assert_no_write_lock_held("sleep")
    time.sleep(seconds)


def acquire_flock(fd: int, operation: int) -> None:
    """``fcntl.flock`` gated on acquire (not pure unlock).

    ``LOCK_UN`` alone is a release and must remain callable on a hold path so
    callers can drop a sidecar flock without nesting errors. Exclusive/shared
    acquires (including ``LOCK_EX | LOCK_NB``) assert the barrier first.
    """
    unlocking = bool(operation & fcntl.LOCK_UN) and not bool(
        operation & (fcntl.LOCK_EX | fcntl.LOCK_SH)
    )
    if not unlocking:
        assert_no_write_lock_held("fcntl.flock")
    fcntl.flock(fd, operation)


def assert_safe_for_network(detail: str = "network") -> None:
    """Barrier for HTTP / remote round-trips."""
    assert_no_write_lock_held(detail)


def assert_safe_for_lane_dispatch() -> None:
    """Barrier for orchestrator lane dispatch / offload entry."""
    assert_no_write_lock_held("lane_dispatch")


def _subprocess_write_lock_audit_hook(event: str, _args: tuple[Any, ...]) -> None:
    """CPython audit hook: refuse raw ``subprocess.Popen`` under RESERVED.

    Fires on the ``subprocess.Popen`` audit event (also emitted by
    ``subprocess.run``, which constructs a ``Popen``). Closes finding 14153:
    a module with no write-lock marker, called from inside a RESERVED window,
    can call raw ``subprocess.Popen`` / ``run`` and bypass every wrapper.

    Hot-path discipline: compare the event name **first** and return
    immediately on any other event. Audit events fire for ``open``, ``exec``,
    ``import``, and much more; evaluating the predicate on every event would
    put a weak-set walk under an RLock on the whole process's hot path.

    Exception asymmetry (intentional):
    - Fail-**closed** on a real violation: re-raise
      :class:`WriteLockHeldAcrossBlockingWorkError`.
    - Fail-**open** on a hook-internal error: any other exception class is
      swallowed so a bug in the predicate cannot become a process-wide
      subprocess outage. An exception escaping an audit hook aborts the
      audited operation from an unexpected place.

    Does **not** cover ``time.sleep`` or ``fcntl.flock`` — CPython emits no
    audit event for either. Those stay on the static / wrapper gates.
    """
    # Event-name filter MUST be first: every other audit event is a no-op.
    ...
    try:
        ...
        # Fail-open on hook-internal error only. Real violations re-raised above.
        return


def install_subprocess_write_lock_audit_hook() -> None:
    """Install the subprocess RESERVED backstop exactly once (idempotent).

    Audit hooks cannot be removed once added. Calling this twice must not
    register a second hook. The install counter increments only on the first
    successful registration so tests can pin idempotence without probing
    ``sys`` internals (there is no public list of audit hooks).
    """
    global _AUDIT_HOOK_INSTALL_COUNT, _AUDIT_HOOK_INSTALLED
    if _AUDIT_HOOK_INSTALLED:
        return
    sys.addaudithook(_subprocess_write_lock_audit_hook)
    _AUDIT_HOOK_INSTALLED = True
    _AUDIT_HOOK_INSTALL_COUNT += 1


def audit_hook_install_count() -> int:
    """Return how many times the subprocess audit hook was actually registered."""
    return _AUDIT_HOOK_INSTALL_COUNT


# Install at import time so any importer of the barrier module gets the backstop
# without a separate opt-in call. Idempotent: re-import / re-call is a no-op.
install_subprocess_write_lock_audit_hook()


def _reject_external_resolution_while_in_transaction(conn: sqlite3.Connection) -> None:
    """Fail closed when external (git/subprocess) work would run under a write lock.

    Uses :func:`holds_write_lock` on the supplied *conn* and the thread-scoped
    :func:`process_holds_write_lock` view so another tracked connection on
    *this thread* that holds RESERVED is refused even when *conn* itself is
    clean. A sibling *thread*'s RESERVED hold is not hold-and-wait for this
    caller ([CON-21]). Holding a write lock across git/subprocess resolution
    is the CON-18 violation that produces multi-second (and operator-visible
    >30 s) ``database is locked`` errors; callers must precompute outside the
    lock and pass resolved values in ([CON-18], [CON-11], [ARCH-13], [RES-02]).
    """
    if holds_write_lock(conn) or process_holds_write_lock():
        # Phrase by the lock state that was observed, not the acquisition form:
        # RESERVED may come from BEGIN IMMEDIATE, deferred DML, or an open-path
        # purge — claiming BEGIN IMMEDIATE misleads operators (DURREV-LOCK-3 /
        # [OBS-08]).
        raise ExternalResolutionInTransactionError(
            "external resolution (git/subprocess) must not run while a write "
            "lock (RESERVED) is held; precompute outside the lock and pass "
            "resolved values into the write helper"
        )
...
def collect_target_context_warnings(
    ...
    *,
    ...
    # Pure-SQL branch enforcement runs first so a genuine BranchMismatchError
    # is never masked by a later in-lock worktree-resolution refusal
    # ([internal], [CON-11], [OBS-08]). Branch comparison needs no git.
    ...
    if resolved_target_worktree_path is None and resolved_target_branch is not None:
        if _worktree_derivation_enabled():
            # Git porcelain under a write transaction is structurally forbidden
            # ([internal], [ARCH-13], [CON-11], [RES-02], [CON-22]). Callers
            # under BEGIN IMMEDIATE must precompute the path on a deferred
            # connection and pass it in (not a precomputed warning list — that
            # would skip in-lock pure-SQL branch/cwd enforcement).
            #
            # When prep could not precompute (e.g. target_branch was NULL at
            # prep and a concurrent writer set it before BEGIN IMMEDIATE), the
            # path is unresolvable without leaving the lock. That is not a
            # reason to fail the write: skip resolution and degrade to a
            # warning ([internal], [RES-06], [OBS-08]). The guard itself
            # stays intact for other call sites; resolution is skipped here,
            # not attempted and rescued.
            if holds_write_lock(conn):
                warnings.append(
                    "context_drift: worktree path comparison skipped because the "
                    "target worktree could not be resolved without leaving the "
                    "write lock (target_branch={}).".format(resolved_target_branch)
                )
            else:
                _reject_external_resolution_while_in_transaction(conn)
                try:
                    canonical_fn = _resolve_core_override(
                        "_canonical_worktree_for_task", _canonical_worktree_for_task
                    )
                    ...
                    warnings.append(
                        ...
                        "`git worktree add` for the canonical branch or archive the task.".format(
...
def _resolve_write_actor(
    ...
    *,
    ...
    # Actor resolution can shell out for SHA validation, worktree derivation,
    # and git detect helpers. Never do that work while a write transaction is
    # open ([internal], [internal], [ARCH-13], [CON-11], [RES-02], [CON-22]).
    _reject_external_resolution_while_in_transaction(conn)
    ...
    # implementation note R3: validate + auto-expand the caller-supplied commit_sha at THIS
    # seam — the un-validated path the MCP record surfaces (record_event /
    # close_slice / review_findings(record) / handoff_state / lanes_recording)
    # reach with a raw actor dict, distinct from build_write_actor which already
    # validates (:491). A fabricated full SHA in a resolvable repo hard-rejects
    # (the validator's rev-parse named error); an abbreviation auto-expands to the
    # canonical 40-char form; a context the server cannot resolve to a git repo
    # keeps the validator's silent-skip so foreign/archived-worktree writes are
    # preserved, never blocked. Idempotent for actors already built via
    # build_write_actor (re-validating a full valid SHA returns it unchanged).
    # Bypassed under WORKBAY_HANDOFF_SKIP_SHA_VALIDATION (both test conftests).
    explicit_commit = _validate_and_expand_commit_sha(explicit_commit)
    ...
    # fallback_agent.
    #
    # implementation note refinement: the lowest rung is no longer a flat
    # ``active_agent or harness_agent``. ``active_agent`` (row last-writer) is the
    # better signal when a worker continues its OWN row, but the live harness
    # marker must win when a DIFFERENT harness's coordinator writes to a lane
    # worker's row. ``_prefer_live_harness_over_stale_writer`` applies that override
    # ONLY on cross-harness mismatch (see resolved_agent below).
    ...
    resolved_agent = (
        explicit_identity
        or explicit_agent
        or self_declared_agent
        # implementation note: the flat ``active_agent or harness_agent`` tail trusted the
        # row's stale last-writer over the live harness marker even when a DIFFERENT
        # harness's coordinator was writing. Discriminate the cross-harness case so
        # the live identity wins there, without downgrading the common
        # worker-continues-own-row path (finding 5170 / REV-HARM-A-001).
        or _prefer_live_harness_over_stale_writer(active_agent, harness_agent)
    )
    ...
    return ResolvedWriteContext(
        ...
        harness=normalize_actor_harness(resolved_agent),
