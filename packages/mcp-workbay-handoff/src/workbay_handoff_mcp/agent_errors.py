...
``record_event(event_kind='error')``. Writes a redacted ``agent_errors``
row, reusing ``_dedup_insert_or_bump`` so repeats of the same
``(error_class, summary, task_ref)`` within the 10-minute window bump
...
uses WAL + busy_timeout in one transaction, and a schema-compat
guard refuses to touch a DB whose reader floor exceeds this package
or whose true schema version (via ``handoff_schema_version``) is
behind what this package writes — the redacted event is appended to
``.task-state/agent-errors-spool.jsonl`` instead, for later replay by
a current install.
...
from workbay_protocol import RUNTIME_ROOT_DIRNAME, resolve_env_alias
...
def record_agent_error(
    *,
    ...
    # Precompute outside the DB connection so git never runs under RESERVED.
    git_common_dir = _resolve_git_common_dir(get_runtime_config().workspace_root)

    with _get_db_connection() as conn:
        now = str(conn.execute("SELECT datetime('now')").fetchone()[0])
        repo_instance_id = _resolve_repo_instance_id(conn, seen_at=now, git_common_dir=git_common_dir)
        ...
        row = conn.execute(
            ...
            WHERE error_class = ? AND summary = ? AND task_ref IS ?
            ORDER BY last_seen_at DESC, id DESC
            LIMIT 1
            """,
            (normalized_class, normalized_summary, normalized_task_ref),
        ).fetchone()
    ...
    # Advisory only: lookup runs after the agent_errors row commits and
    # cannot abort or mask the write result. Skip embedding_failed: that
    # class is the degrade from a failed embed-on-write, and looking up
    # prior art would recursively trigger another embed.
    if normalized_class != "embedding_failed":
        attach_related_prior(
            result,
            query=f"{normalized_class} {normalized_summary}",
            task_ref=normalized_task_ref,
        )
    return result
...
def _resolve_primary_state_dir(cwd: Path) -> Path | None:
    ...
    Precedence (ARCH-13 — one sanctioned override everywhere):

    1. ``WORKBAY_HANDOFF_STATE_DIR`` when set (suite hermetic pin, explicit
       operator override). Production launch paths leave this unset, so the
       change is a no-op for operators (HERMETICDB-R1-F2).
    2. ``git rev-parse --path-format=absolute --git-common-dir`` from ``cwd``.
       ``--git-common-dir`` points at the primary ``.git`` even from a linked
       worktree, so hook writes land in the primary repo state and never
       create cwd-local ``.task-state`` directories.
    """
    override = resolve_env_alias("WORKBAY_HANDOFF_STATE_DIR")
...
def record_agent_error_direct(
    *,
    ...
    Spools instead of writing when the DB is missing, its reader floor
    exceeds ``HANDOFF_SCHEMA_VERSION``, its true schema version is
    behind this package, or the write fails operationally.

    related_prior lookup is excluded on this path (typed absent): the
    hook/CLI writer runs outside the configured runtime and may spool.
    Loading an embedding model here would add unbounded latency on an
    error path. Runtime ``record_agent_error`` attaches advisory prior
    art after the row commits.
    """
    normalized_class = _normalize_optional_text(error_class)
    normalized_summary = _redact_text(summary, limit=_SUMMARY_LIMIT)
    if normalized_class is None or not _ERROR_CLASS_RE.match(normalized_class):
    ...
    try:
        # Shared connect path ([ARCH-13], [RES-02]): same busy_timeout as every
        # other handoff writer — do not reintroduce a forked 5s bind here.
    ...
    try:
        ...
        true_version = handoff_schema_version(conn)
        if floor > HANDOFF_SCHEMA_VERSION or true_version < HANDOFF_SCHEMA_VERSION:
            return _spool_agent_error(
                state_dir,
                event,
                reason=(f"schema_version_mismatch: db={true_version} floor={floor} package={HANDOFF_SCHEMA_VERSION}"),
            )
...
def _is_transient_sqlite_error(exc: sqlite3.Error) -> bool:
    ...
    Uses :func:`workbay_handoff_mcp.sqlite_lock_errors.classify_sqlite_lock_error`
    keyed on ``exc.sqlite_errorcode``. Named lock outcomes (``BUSY``,
    ``BUSY_SNAPSHOT``, ``LOCKED``) abort a drain without quarantining lines —
    they are connection-level contention, not per-line defects.

    When ``sqlite_errorcode`` is absent (``UNKNOWN``), degrades to the prior
    substring check so bare fixtures / older interpreters keep working.
    Everything else (constraint, datatype, programming errors) is structural
    and should count toward quarantine.
    """
    if not isinstance(exc, sqlite3.OperationalError):
        return False
    # Local import keeps agent_errors import-light for CLI paths that only
    # need redaction helpers; classification is only needed on the drain path.
    from .sqlite_lock_errors import SqliteLockKind, classify_sqlite_lock_error, is_lock_contention_error

    kind = classify_sqlite_lock_error(exc)
    if kind is SqliteLockKind.UNKNOWN:
        message = str(exc).lower()
        return "locked" in message or "busy" in message
    return is_lock_contention_error(exc)
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            # Shared connect path ([ARCH-13], [RES-02]): same busy_timeout as
            # record_agent_error_direct — one owner for the wait bound.
            conn = _connect_handoff_sqlite(db_path)
            try:
                floor = int(conn.execute("PRAGMA user_version").fetchone()[0])
                true_version = handoff_schema_version(conn)
                if floor > HANDOFF_SCHEMA_VERSION or true_version < HANDOFF_SCHEMA_VERSION:
                    return {
                        "ok": False,
                        "error": (
                            f"schema_version_mismatch: db={true_version} floor={floor} package={HANDOFF_SCHEMA_VERSION}"
                        ),
                        "remaining": sum(1 for line in lines if line.strip()),
                        "failed": 0,
                        "quarantined": 0,
