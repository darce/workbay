...
def record_agent_error(
    *,
    ...
    with _get_db_connection() as conn:
        ...
        repo_instance_id = _resolve_repo_instance_id(
            conn, seen_at=now, git_common_dir=git_common_dir
        )
        cursor = conn.execute(
            """
    ...
    try:
        ...
        if floor > HANDOFF_SCHEMA_VERSION or true_version < HANDOFF_SCHEMA_VERSION:
            return _spool_agent_error(
                ...
                event,
                reason=(
                    f"schema_version_mismatch: db={true_version} floor={floor} "
                    f"package={HANDOFF_SCHEMA_VERSION}"
                ),
            )
