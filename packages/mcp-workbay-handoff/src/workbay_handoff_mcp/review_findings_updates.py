...
def _mark_quarantined(
    ...
    with circuit._lock:
        if key not in circuit.quarantined:
            circuit.quarantined[key] = 0
        # A failed half-open trial restarts the cooldown from now, not just the
        # first trip. Stamping opened_at once makes the elapsed-time predicate
        # permanently true after the first window, so every later call admits a
        # trial and forks git again -- the circuit stops circuiting.
...
def _circuit_note_head_unresolved(circuit: _OpportunisticGitCircuit) -> None:
    ...
    with circuit._lock:
        ...
        if circuit.head_failures >= INTEGRATE_HEAD_RESOLVE_FAILURE_LIMIT:
            ...
            # Refresh on every failed trial, not only the first trip, for the
            # same reason as the quarantine cooldown above: a stamp-once
            # opened_at leaves the 30s time predicate true forever.
...
def _circuit_note_git_probe_failed(circuit: _OpportunisticGitCircuit) -> None:
    ...
    with circuit._lock:
        if not circuit.git_missing:
            return
        circuit.git_missing_half_open = False
        # Last-failure timestamp, not first-open. Stamping once leaves the 30s
        # elapsed-time predicate true forever, so every later host write admits
        # a "half-open" trial and forks git again.
...
def _run_opportunistic_integrate_for_task(
    ...
    try:
        ...
        # Only a *live* cheap read may stand in for HEAD. There is deliberately
        # no cache of the last successful observation to fall back on: treating
        # one as current asserts "HEAD is unchanged" on no evidence. It masks
        # exactly the two cases this stack exists to fix -- a linked-worktree
        # commondir miss, and HEAD advancing after the first latch -- and the
        # latter wedges the trigger permanently, because nothing else ever
        # re-reads HEAD. The circuit carried a write-only ``resolved_heads``
        # dict for exactly this shape; CASC-WHOLE-01 removed it, because a
        # cache no reader consults is a trap waiting for the next edit.
