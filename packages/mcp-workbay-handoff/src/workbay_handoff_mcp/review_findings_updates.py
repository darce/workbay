    ...
    *,
    ...
    for candidate in (resolution_notes, notes, verification_evidence):
        ...
        if (
            ...
            and len(normalized) >= STATUS_DECISION_MIN_RATIONALE_LENGTH
        ):
            return normalized
    return None
...
def resolve_review_findings(
    *,
    ...
    if normalized_global_status is not None and normalized_global_status not in {
        ...
        *STATUS_DECISION_STATUSES,
        ...
        return _envelope(
            ...
            data={
                "error": (
                    f"Invalid resolve status {normalized_global_status!r}. "
                    f"Valid: fixed, deferred, wontfix."
                ),
            },
            task_ref=task_ref,
            entity="finding",
        ...
        for raw_id, raw_status in finding_statuses.items():
            ...
            if st not in {FindingStatus.FIXED.value, *STATUS_DECISION_STATUSES}:
                return _envelope(
                    ...
                    data={
                        "error": (
                            f"Invalid per-finding resolve status {st!r} for {fid}. "
                            f"Valid: fixed, deferred, wontfix."
                        ),
                    },
                    task_ref=task_ref,
    ...
    closing_candidates = [
        ...
        or str(entry["outcome"]) in STATUS_DECISION_STATUSES
    ]
    if (
        ...
        for _, entry in planned_results:
            ...
            if (
                outcome_value == ResolutionOutcomeKind.FIXED.value
                or outcome_value in STATUS_DECISION_STATUSES
            ):
    ...
    for row, entry in planned_results:
        ...
        if outcome_value == ResolutionOutcomeKind.FIXED.value or outcome_value in STATUS_DECISION_STATUSES:
            close_status = (
                outcome_value if outcome_value in STATUS_DECISION_STATUSES else FindingStatus.FIXED.value
            )
            try:
