...
SANCTIONED_RESOLVE_CALL = (
    'review_findings(review={"operation":"resolve", "all_open":true, "verification_evidence":...})'
)
    ...
    *,
    ...
    if normalized_verification_evidence is None and recent_fixed_count + len(fixed_candidates) > BATCH_CLOSE_THRESHOLD:
        batch_guard_reason = (
            ...
            f"and this request would close {len(fixed_candidates)} more. Provide verification_evidence "
            f"or resolve fewer findings via {SANCTIONED_RESOLVE_CALL}."
        )
    ...
    if recent_count >= BATCH_CLOSE_THRESHOLD:
        return {
            ...
            "error": (
                ...
                f"closure is individually verified. Sanctioned call: {SANCTIONED_RESOLVE_CALL}."
            ),
            "false_fix_guard": {
                "finding_id": str(existing["finding_id"]),
