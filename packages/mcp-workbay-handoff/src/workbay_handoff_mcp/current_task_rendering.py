...
Extracted from _shared.py (implementation note of internal). Contains:
  ...
  - write path (_write_workspace_current_task_json, _write_current_task_md_from_state)
...
if TYPE_CHECKING:
    from .compaction import _CompactionAdvisoryPrecomputed

# Handshake size for CURRENT_TASK/export snapshot review_findings buckets.
_FINDINGS_SNAPSHOT_BOUND = 5
...
def _collect_dashboard_rows(
    ...
    rows = conn.execute(
        """
        WITH findings_activity AS (
            SELECT
                task_ref,
                MAX(COALESCE(updated_at, resolved_at, created_at)) AS updated_at,
                CAST(NULL AS TEXT) AS resolved_at,
        ...
        activity AS (
            ...
            UNION ALL
            SELECT task_ref, COALESCE(updated_at, resolved_at, created_at) AS updated_at FROM findings_activity
            UNION ALL
...
def _collect_task_snapshot(conn: sqlite3.Connection, task_ref: str) -> TaskSnapshot:
    ...
    def _snapshot_review_findings() -> list[dict]:
        findings: list[dict] = []
        for row in conn.execute(
            """
            SELECT * FROM (
                SELECT
                    ...
                    ROW_NUMBER() OVER (
                        PARTITION BY CASE
                            ...
                            WHEN status IN ('deferred', 'wontfix') THEN 'deferred'
                            WHEN status IN ('fixed', 'resolved_on_branch', 'integrated') THEN 'resolved'
            ...
            WHERE _rn <= ?
            ORDER BY CASE severity WHEN 'high' THEN 0 WHEN 'medium' THEN 1 WHEN 'low' THEN 2 END,
                     COALESCE(updated_at, created_at) DESC
            """,
            (task_ref, _FINDINGS_SNAPSHOT_BOUND),
        ).fetchall():
            item = dict(row)
            item.pop("_rn", None)
    ...
    roster_row = conn.execute(
        "SELECT task_ref, skill_slugs, activated_domains, updated_at FROM task_tool_roster WHERE task_ref = ?",
        (task_ref,),
    ).fetchone()
    task_tool_roster = dict(roster_row) if roster_row is not None else None
    if task_tool_roster is not None:
        for key in ("skill_slugs", "activated_domains"):
            raw = task_tool_roster.get(key)
            task_tool_roster[key] = json.loads(raw) if isinstance(raw, str) else None
    ...
    return {
        ...
        "review_findings": _snapshot_review_findings(),
        ...
        "task_tool_roster": task_tool_roster,
        "plan_cursors": _rows("SELECT * FROM plan_cursors WHERE task_ref = ? ORDER BY updated_at DESC, id DESC"),
        # Exclude phase='lane_prep' footprint telemetry from the default CURRENT_TASK
        # projection (internal). Prep rows remain queryable via
        # list_turn_metrics(phase='lane_prep'); mixing them here crowds agent-turn evidence.
        "turn_metrics": _rows(
            "SELECT * FROM turn_metrics WHERE task_ref = ? AND COALESCE(phase, '') != 'lane_prep' "
...
def _write_workspace_current_task_json(
    conn: sqlite3.Connection,
    task_ref: str,
    *,
    ...
    Distinct from the lane-SQL dual-write side-effect
    ``lanes_recording._write_current_task_md_for_task`` (which regenerates the
    per-task CURRENT_TASK surface after a committed lane write). This helper
    only refreshes the workspace-summary CURRENT_TASK.json.
...
def _render_workspace_summary_from_per_task_files() -> dict[str, object]:
    ...
    Every returned payload also embeds ``authority_db_path`` and
    ``authority_projection_dir`` so writers and close_check readers share one
    key set [DATA-01].
    ...
    """
    runtime = get_runtime_config()
    # Read version before data [CON-11][DATA-01]: a concurrent write after this
    # stamp and before the payload reads can only under-report freshness (cheap
    # cache miss). Stamp-last would certify content the payload does not carry
    # and serve stale identity until the next handoff_state write [RES-08].
    content_revision = _handoff_content_revision(runtime.db_path)
    target_dir = runtime.per_task_projection_dir
    ...
    if not candidate_payloads:
        result: dict[str, object] = {**_workspace_summary_base(), "shape": "none"}
    else:
        ...
        if not live_payloads:
            result = {**_workspace_summary_base(), "shape": "none"}
        else:
            from .compaction import _precompute_compaction_advisory  # noqa: PLC0415 – circular import guard

            workspace_root = runtime.compaction_config_root
            try:
                precomputed = _precompute_compaction_advisory(workspace_root=workspace_root)
            except Exception as exc:
                _log.warning("Workspace compaction advisory enrichment skipped: %s", exc)
                enriched_payloads = live_payloads
            else:
                enriched_payloads = [_overlay_compaction_advisory(p, precomputed=precomputed) for p in live_payloads]
    ...
    result["authority_db_path"] = str(runtime.db_path)
    result["authority_projection_dir"] = str(runtime.per_task_projection_dir)
    # Fail closed [OBS-08][DATA-01]: never stamp a clean negative while
    # handoff_state still holds rows. Payload comes from projection files;
    # the fingerprint comes from the table. When they disagree about whether
    # a task is live, omit the stamp so the hook falls through rather than
    # certifying the mismatch for the whole staleness window [RES-08].
    # Fingerprint is ``count:sum_rev:max_updated``; limit the split so the
    # ISO timestamp (which itself contains colons) is not fractured.
    if content_revision is not None:
        row_count = int(content_revision.split(":", 2)[0])
        payload_reports_no_task = result.get("shape") in (None, "none")
        if not (payload_reports_no_task and row_count > 0):
            result["authority_content_revision"] = content_revision
    return result


def _overlay_compaction_advisory(
    payload: dict[str, object],
    *,
    precomputed: _CompactionAdvisoryPrecomputed | None = None,
) -> dict[str, object]:
...
def _handoff_content_revision(db_path: Path) -> str | None:
    """Short fingerprint of handoff_state content, or None when unreadable.

    Composite of COUNT(*), SUM(revision), and MAX(updated_at) over
    ``handoff_state``. That triple discriminates an insert, a revision-bump
    update, and a delete. Opened read-only via
    ``connect_handoff_db(..., read_only=True)`` (package connect contract),
    then deliberately narrowed to a 1s busy timeout — see body comment.

    Counterpart: scripts/hooks/_active_task_context._handoff_content_revision
    (mirrored under the workbay-system payload hooks tree) — both copies must
    define the fingerprint string identically. Hooks cannot import this
    package, so the helper is intentionally duplicated rather than shared.
    The twin may still use a bare ``sqlite3.connect`` (hooks tree constraint);
    this in-package copy must not.
    """
    # Open and use under one try/finally so a post-open PRAGMA (or query)
    # failure cannot leak the live handle. MAINREDS-R1-F5: the previous
    # structure wrapped connect+PRAGMA in a try/except that returned None
    # without ever entering the close finally — a genuine widening vs the
    # pre-change code that only wrapped the bare connect (where failure
    # never bound con). Sole caller is the multi-active render loop; a
    # persistent PRAGMA failure would leak one fd per iteration.
    try:
        con = connect_handoff_db(db_path, read_only=True)
    except (sqlite3.Error, OSError, ValueError):
        return None
    try:
        try:
            # Busy-timeout decision (RES-02): the factory applies the package
            # contractual 30s busy_timeout. Sole caller is
            # ``_render_workspace_summary_from_per_task_files`` (CURRENT_TASK
            # workspace-summary render / stamp path). Blocking 30s on every
            # render under multi-writer load is a latency defect on a
            # synchronous hot path. Prefer the historical 1s bound: on lock
            # timeout this helper returns None and the stamp is omitted
            # (fail-closed freshness, OBS-08) rather than stalling render.
            # Documented deviation from HANDOFF_SQLITE_BUSY_TIMEOUT_MS.
            con.execute("PRAGMA busy_timeout=1000")
            row = con.execute(
                "SELECT COUNT(*), COALESCE(SUM(revision), 0), MAX(updated_at) FROM handoff_state"
            ).fetchone()
        except (sqlite3.Error, OSError, ValueError):
            return None
    finally:
    ...
    if row is None:
        return None
    count, sum_rev, max_updated = row
    return f"{int(count)}:{int(sum_rev)}:{max_updated if max_updated is not None else ''}"
...
def _handoff_db_authority_mtime(
    ...
    Counterpart: scripts/hooks/_active_task_context._handoff_db_authority_mtime
    (mirrored under the workbay-system payload hooks tree) — both copies must
    define the timestamp identically. Hooks cannot import this package, so the
    helper is intentionally duplicated rather than shared.
    """
    mtimes: list[float] = []
    for path in (
        db_path,
...
def _write_workspace_summary_current_task_json(*, unconditional: bool = False) -> bool:
    ...
    partial file. Captures the handoff authority timestamp (db + -wal/-shm +
    per-task projection JSON mtimes) *before* the render reads, then stamps the
    snapshot mtime to that captured value after the atomic replace so the
    freshness predicate compares the same quantity the writer observed.
    Embeds ``authority_db_path`` and ``authority_projection_dir`` so readers
    validate against the authority the writer actually used [DATA-01].
    ...
    # Capture before render: content vintage is the authority at read start,
    # not the wall-clock time when the atomic replace finishes.
    authority_mtime = _handoff_db_authority_mtime(runtime.db_path, runtime.per_task_projection_dir)
    ...
    try:
        with os.fdopen(tmp_fd, "w", encoding="utf-8") as handle:
            handle.write(serialized)
        os.replace(tmp_path, target_path)
        if authority_mtime is not None:
            os.utime(target_path, (authority_mtime, authority_mtime))
    except Exception:
        if os.path.exists(tmp_path):
            os.unlink(tmp_path)
        raise
