...
def _load_finding_query_vector(
    ...
    rows = conn.execute(
        f"""
        SELECT entity_kind, entity_id, vector, model_id, text_hash
...
def _rank_against_store(
    ...
    *,
    ...
    sql = "SELECT c.entity_kind, c.entity_id, c.task_ref, c.vector, c.model_id, c.text_hash FROM concept_embeddings c"
    params: list[object] = []
    if needs_join:
