...
def _rank_against_store(
    ...
    *,
    ...
    sql = "SELECT c.entity_kind, c.entity_id, c.task_ref, c.vector, c.model_id FROM concept_embeddings c"
    params: list[object] = []
    if needs_join:
        ...
        sql += (
            " LEFT JOIN handoff_state h ON h.task_ref = c.task_ref"
            " LEFT JOIN task_archives a ON a.task_ref = c.task_ref"
        )
    where: list[str] = []
    ...
    if include_plan_paths is not None:
        ...
        where.append(
            "COALESCE(h.task_plan_path, json_extract(a.snapshot_json, '$.active.task_plan_path'))"
            f" IN ({placeholders})"
        )
    ...
    if include_statuses is not None:
        ...
        where.append(
            "COALESCE(h.status, json_extract(a.snapshot_json, '$.active.status'))"
            f" IN ({placeholders})"
        )
    ...
    for row in conn.execute(sql, params).fetchall():
        ...
        # (BR-MEM2-01 / CARD-05).
        model_match_count += 1
        if entity_kind == _COMPACTION_RESIDUAL_KIND and _compaction_residual_oversized(
            conn, entity_id
        ):
            continue
