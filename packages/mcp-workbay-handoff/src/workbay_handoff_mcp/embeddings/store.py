...
from workbay_handoff_mcp.embeddings.vector_codec import (
...
# ONNX inference measured 45.33s for 8192 chars and 51.46s for 4096 chars.
# A 5s on-write budget deliberately usually defers a real embed so an ordinary
# handoff write stays responsive; the existing resumable backfill can retry it.
DEFAULT_ONWRITE_EMBED_TIMEOUT_SECONDS = 5.0
ENV_ONWRITE_EMBED_TIMEOUT_SECONDS = "WORKBAY_HANDOFF_ONWRITE_EMBED_TIMEOUT_SECONDS"


def resolve_onwrite_embed_timeout_seconds() -> float:
    """Resolve the bounded on-write wait, rejecting unsafe environment values."""
    raw = os.environ.get(ENV_ONWRITE_EMBED_TIMEOUT_SECONDS)
    if raw is None:
        return DEFAULT_ONWRITE_EMBED_TIMEOUT_SECONDS
    try:
        value = float(raw)
    except (TypeError, ValueError) as exc:
        raise ValueError(f"{ENV_ONWRITE_EMBED_TIMEOUT_SECONDS} must be a finite positive number, got {raw!r}") from exc
    if not math.isfinite(value) or value <= 0.0:
        raise ValueError(f"{ENV_ONWRITE_EMBED_TIMEOUT_SECONDS} must be a finite positive number, got {raw!r}")
    return value
...
# vs deferred/failed is distinguishable without re-reading agent_errors.
...
def prefetch_owned_vector_blobs(
    ...
    """
    unique = {(str(text_hash_value), str(model_id)) for text_hash_value, model_id in keys}
    if not unique:
        return {}
    hashes = tuple({text_hash_value for text_hash_value, _model_id in unique})
    ...
    for text_hash_value, model_id, vector in rows:
        key = (str(text_hash_value), str(model_id))
...
def store_concept_embedding(
    ...
    vectors = _provider_embed_batch(
        provider,
        [truncate_embed_text(text)],
        timeout_seconds=resolve_onwrite_embed_timeout_seconds(),
    )
...
# Callers that surface degrade (e.g. close_slice) must treat ``failed`` and
# ``deferred`` as typed non-success signals — never infer success from silence.
...
def _is_sqlite_lock_error(exc: BaseException) -> bool:
    try:
        ...
        return bool(is_sqlite_lock_error(exc))
    except Exception:  # noqa: BLE001 - classifier import/use must not raise
        return False


def _record_best_effort_embedding_failure(
    exc: BaseException,
    *,
    task_ref: str,
    detail: str,
    ...
    """Record a typed, deduped ``embedding_failed`` row. Never raises.

    Reason class comes from ``embedding_unavailable_reason(exc=...)`` and is
    encoded in ``summary`` so it participates in the
    ``(error_class, summary, task_ref)`` dedupe key. SQLITE lock/busy skips
    recording (F-HB-04: do not re-enter the contended DB).
    """
    if _is_sqlite_lock_error(exc):
        return
    try:
        from workbay_handoff_mcp.agent_errors import record_agent_error
        from workbay_handoff_mcp.embeddings.provider import embedding_unavailable_reason

        resolved_reason = reason or embedding_unavailable_reason(exc=exc) or "error"
        record_agent_error(
            error_class="embedding_failed",
            summary=f"embedding_failed: {resolved_reason}",
            detail=detail,
            task_ref=task_ref,
        )
...
def embed_concept_best_effort(
    ...
    can attach a typed partial-failure signal; a provider timeout returns
    ``\"deferred\"`` because the operation may have executed. The resumable
    backfill reconciles both gaps. Named except arms record a deduped
    ``agent_errors.embedding_failed`` row whose ``summary`` encodes the
    reason class from :func:`embedding_unavailable_reason`; timeout telemetry
    uses the explicit ``timeout`` reason. Outcomes increment
    :func:`get_concept_embed_counts`.
    """
    try:
        ...
        # Timeout is UNKNOWN ([DATA-13]): inference may have completed in the
        # orphaned worker, so leave no negative cache row and let backfill retry.
        _record_best_effort_embedding_failure(
            exc,
            task_ref=task_ref,
        ...
        _log.warning(
            "embed-on-write deferred for %s/%s task_ref=%s error_type=TimeoutError: %s",
            entity_kind,
            entity_id,
            task_ref,
            exc,
        )
        return _record_concept_outcome("deferred")
    except (EmbeddingArtifactError, ImportError) as exc:
        # Named cause: artifact / optional runtime missing — not an inference error.
        _record_best_effort_embedding_failure(
            exc,
            task_ref=task_ref,
            detail=f"{entity_kind}/{entity_id}: {exc}",
        ...
        # Residual non-fatal arm: still named via embedding_unavailable_reason
        # (typically "error") so a new exception class cannot impersonate
        # "nothing relevant". SQLITE lock/busy skips recording (F-HB-04).
        if _is_sqlite_lock_error(exc):
            _log.warning(
...
def store_compaction_anchor_best_effort(
    ...
    returns a typed outcome, increments :func:`get_anchor_store_counts`,
    records a deduped ``agent_errors.embedding_failed`` row with a named
    reason class, and logs a WARNING with the exception class so a silent
    permanent failure (e.g. 87/87 anchors NULL) is no longer invisible (AXI-5).
    ``timeout_seconds`` defaults to the same configured on-write bound as
    concept embeddings; expiry is a typed ``failed`` outcome.
    """
    try:
        prov = provider if provider is not None else _resolve_provider()
        if prov is None:
        ...
        embed_timeout_seconds = (
            timeout_seconds if timeout_seconds is not None else resolve_onwrite_embed_timeout_seconds()
        )
        vectors = run_with_daemon_timeout(
            lambda: prov.embed([capped]),
            timeout_seconds=embed_timeout_seconds,
            timeout_message=(
                f"anchor provider.embed timed out after {embed_timeout_seconds}s (compaction_id={compaction_id})"
            ),
        )
        if vectors.shape[0] != 1:
            _log.warning(
        ...
        _record_best_effort_embedding_failure(
            exc,
            task_ref=task_ref,
            detail=f"compaction_id={compaction_id}: {exc}",
            reason="timeout",
        )
        _log.warning(
            "anchor embed-on-write timed out for compaction_id=%s task_ref=%s error_type=TimeoutError: %s",
            compaction_id,
            task_ref,
            exc,
        )
        ...
        # Residual non-fatal arm: classified as "error" (or lock skip) so a
        # dead provider cannot impersonate a healthy NULL-anchor degrade.
        if _is_sqlite_lock_error(exc):
            _log.warning(
                "anchor embed-on-write failed under sqlite lock for compaction_id=%s task_ref=%s error_type=%s: %s",
                compaction_id,
                task_ref,
                type(exc).__name__,
                exc,
...
def _provider_embed_batch(
    ...
    *,
    ...
    forever while inference is stuck ([RES-02]). The bounded worker is a daemon
    thread, so the timeout bounds interpreter exit and not only this return.
    If the provider returns after the caller has timed out, its result is
    discarded, matching the prior future-based path: no later operation can
    safely claim a timed-out result.
    """
    if timeout_seconds is None:
        return provider.embed(texts)
    # A daemon worker lets a timed-out provider remain isolated without making
    # interpreter shutdown wait for it. This bounds every caller's process exit,
    # not just the backfill loop and its writer-heartbeat cleanup.
    return run_with_daemon_timeout(
        lambda: provider.embed(texts),
        timeout_seconds=timeout_seconds,
        timeout_message=f"provider.embed timed out after {timeout_seconds}s (batch size {len(texts)})",
    )
