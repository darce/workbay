...
def _classify_concept_for_backfill(
    ...
    *,
    ...
    two cannot drift. Blank text is ``empty``. When ``model_id`` is set, a
    row is ``skipped`` only on a matching ``(text_hash, model_id)`` pair —
    the production resume gate. When ``model_id`` is ``None`` (preview with
    no resolved provider), skip is hash-only so operators can still count
    remaining work without constructing a provider.
    """
    if text is None or not str(text).strip():
        return "empty", None
...
# Public alias so doctor (and other read-only consumers) share this skip gate
# without reimplementing the (text_hash, model_id) comparison. Additive export
# only — the implementation stays _classify_concept_for_backfill.
