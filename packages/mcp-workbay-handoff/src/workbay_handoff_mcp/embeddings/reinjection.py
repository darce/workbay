...
# Rendered relevant-line shape: header, then "- " + LABEL + ": " + SNIPPET.
# Labels are unbounded in the live DB (finding ids max 155, decision ids max
# 3541), so the floor is exact only after this cap. Truncation uses the
# existing single-character ellipsis; the snippet still carries [kind:id].
RELEVANT_HEADER_LINE = "relevant:"
RENDERED_LABEL_CAP = 48
ELLIPSIS_MARKER = "…"


def render_concept_label(label: str, *, cap: int = RENDERED_LABEL_CAP) -> str:
    """Bound a concept label to ``cap`` characters, truncating with ellipsis."""
    text = str(label or "")
    if cap <= 0:
        return ""
    if len(text) <= cap:
    ...
    return text[: cap - 1] + ELLIPSIS_MARKER


def one_result_floor_chars(snippet_chars: int) -> int:
    """Exact occupancy of the header plus one capped-label result line.

    Occupancy is header line length (including the joining newline) + 2
    (``'- '``) + label cap + 2 (``': '``) + the configured snippet budget.
    """
    header_line_length = len(RELEVANT_HEADER_LINE) + 1
    return header_line_length + 2 + RENDERED_LABEL_CAP + 2 + max(0, int(snippet_chars))
...
class SemanticReinjectionResult:
    ...
    # Set when the packet was built from a linked/related task after the target
    # task had zero ranked candidates (cold-start fallback; PR-0105-03).
...
def resolve_related_task_ref_for_reinjection(conn: sqlite3.Connection, task_ref: str) -> str | None:
    """Deterministic related-task lookup for cold-start reinjection (PR-0105-03).

    Order:
      1. task's ``task_plan_path`` → planning ``review_runs`` for that
         ``subject_path`` (``reviewed_at DESC, id DESC`` — same as
         ``list_review_runs``) → first non-self ``task_ref``;
      2. else any other ``handoff_state`` row with the same ``task_plan_path``
         (``updated_at DESC, task_ref ASC``);
      3. else ``None`` (caller returns ``no_embeddings``).
    """
    row = conn.execute(
        "SELECT task_plan_path FROM handoff_state WHERE task_ref = ?",
        (task_ref,),
    ).fetchone()
    if row is None:
        return None
    raw_plan = row[0] if row[0] is not None else None
    plan_path = str(raw_plan).strip() if raw_plan is not None else ""
    if not plan_path:
        return None

    # (1) planning-review MAINT (or other) row for this plan document.
    for run in conn.execute(
        """
        SELECT task_ref FROM review_runs
        WHERE subject_path = ?
          AND review_mode = 'planning'
          AND task_ref IS NOT NULL
        ORDER BY reviewed_at DESC, id DESC
        """,
        (plan_path,),
    ):
        candidate = str(run[0]).strip() if run[0] is not None else ""
        if candidate and candidate != task_ref:
            return candidate

    # (2) sibling task sharing the same plan path.
    other = conn.execute(
        """
        SELECT task_ref FROM handoff_state
        WHERE task_plan_path = ? AND task_ref != ?
...
def render_snippet(text: str, *, ref_kind: str, ref_id: str, snippet_chars: int) -> str:
    ...
    if len(suffix) > snippet_chars:
        # Keep the display reference within the same cap as the prose. The
        # complete provenance remains in SelectedConcept.id, not this preview.
        if snippet_chars < 2:
            return ELLIPSIS_MARKER[:snippet_chars]
        return render_concept_label(suffix[:-1], cap=snippet_chars - 1) + "]"
...
def select_concepts(
    ...
    *,
    ...
    while remaining and len(chosen) < config.max_concepts:
        ...
        display_label = render_concept_label(source.label)
        line_chars = len(f"- {display_label}: {snippet}")
        if not chosen:
            needed = len(f"{RELEVANT_HEADER_LINE}\n") + line_chars
        else:
            needed = 1 + line_chars
...
def _infer_skip_reason(
    ...
    *,
    ...
    header = len(f"{RELEVANT_HEADER_LINE}\n")
    fits_alone = False
    selectable = False
...
# Skip reasons that are a degrade, not "nothing relevant". Stated on the
# rendered surface so a dead provider cannot impersonate an empty selection.
_DEGRADE_SKIP_REASONS: frozenset[str] = frozenset(
    {
        "disabled",
        "unconfigured",
        "deps_or_model_missing",
...
def render_readable_relevant_lines(
    selected: Sequence[SelectedConcept],
    skip_reason: SkipReason | None = None,
) -> list[str]:
    """Render readable ``relevant:`` lines, or a stated degrade.

    ``skip_reason`` defaults to ``None`` so the four existing call sites stay
    byte-identical. When ``selected`` is empty and ``skip_reason`` names a
    degrade (provider-down / unembedded), return a skip line rather than
    impersonating "nothing relevant" as an absent block.
    """
    if selected:
        lines = [RELEVANT_HEADER_LINE]
        for item in selected:
            lines.append(f"- {render_concept_label(item.label)}: {item.snippet}")
        return lines
    if skip_reason in _DEGRADE_SKIP_REASONS:
        return [f"{RELEVANT_HEADER_LINE} skipped ({skip_reason})"]
    return []
...
def build_semantic_reinjection_packet(
    ...
    *,
    ...
    provider_unavailable_reason: SkipReason | None = None,
    ...
    if provider is None:
        return SemanticReinjectionResult(
            status="skipped",
            skip_reason=provider_unavailable_reason or "provider_unavailable",
            model_id=None,
            chars_budget=budget,
            **result_metadata,
        )
    ...
    kinds = entity_kinds or tuple(_RESOLVER_SPECS)
    ...
    fallback_scope: str | None = None
    # Cold-start: zero ranked candidates for the target task → related-task
    # concepts (planning-review / shared plan path) before no_embeddings.
    if not ranked_candidates:
        related_ref = resolve_related_task_ref_for_reinjection(conn, task_ref)
        if related_ref is not None:
            related_ranked = rank_candidate_vectors_by_anchor(
                conn,
                anchor,
                related_ref,
                candidate_pool=fetch_pool,
            ...
            if related_ranked:
                ranked_candidates = related_ranked
                source_task_ref = related_ref
                fallback_scope = "related_tasks"
        if not ranked_candidates:
            return SemanticReinjectionResult(
                status="skipped",
