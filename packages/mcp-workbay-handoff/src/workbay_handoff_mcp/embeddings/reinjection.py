...
class ReinjectionConfig:
    ...
    def from_env(cls, env: Mapping[str, str] | None = None) -> ReinjectionConfig:
        ...
        candidate_pool = _clamp_int(
            source.get("WORKBAY_REINJECT_SEMANTIC_CANDIDATE_POOL"), default=32, lo=1, hi=256
        )
        ...
        return cls(
            ...
            snippet_chars=_clamp_int(
                source.get("WORKBAY_REINJECT_SEMANTIC_SNIPPET_CHARS"), default=120, lo=40, hi=500
            ),
            ...
            refresh_budget_chars=_clamp_int(
                source.get("WORKBAY_REINJECT_SEMANTIC_REFRESH_BUDGET_CHARS"), default=1500, lo=0, hi=10000
            ),
        )
...
def _infer_skip_reason(
    ...
    *,
    ...
    eligible = [
        c
        for c in candidates
        if c.score >= config.min_score and (c.entity_kind, c.entity_id) in resolved
    ]
    if not eligible:
...
def build_semantic_reinjection_packet(
    ...
    *,
    ...
    candidates = [
        ...
        if (candidate.entity_kind, candidate.entity_id) in resolved_map
