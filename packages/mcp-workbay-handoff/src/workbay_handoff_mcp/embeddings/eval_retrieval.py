...
from ..shared_schema import connect_handoff_db
...
def evaluate_retrieval_query(
    ...
    *,
    ...
    # Hybrid path only: fuse deeper A/B candidate pools, then cut to the same
    # metric_depth as standalone A/B so Recall@10 is depth-honest when the
    # caller passes top_k < 10 ([EVAL-01], plan0141_gate_depth_asymmetry).
    ...
    pool = rrf_pool_depth(metric_depth)
    if provider is not None:
    ...
    hybrid = run_hybrid_arm(semantic_pool, lexical_pool, top_k=metric_depth)
...
def sample_task_refs_stratified(
    ...
    *,
    ...
    given ``seed``. Returns fewer than n when the corpus is small.

    Operator-snapshot preprocessing API — not wired into ``--fixture smoke``
    (smoke passes an explicit task_ref list). Kept as a public helper for the
    operator-supplied-at-runtime corpus path scored via
    :func:`run_snapshot_eval`.
    """
...
def run_snapshot_eval(
    ...
    *,
    ...
    Fails closed on a missing/typo path ([RLSE-05]/[DBG-05]): refuses to let
    the connect create an empty DB that would score zero-recall hold.

    Opens through ``connect_handoff_db`` so the snapshot handle carries the
    package busy_timeout contract ([ARCH-13]). Deliberately *not*
    ``read_only=True``: an operator snapshot is typically a bare copy of a WAL
    database with no ``-wal``/``-shm`` sidecar, and SQLite cannot open that
    read-only at all (``unable to open database file``).
    """
    if cases is None:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"retrieval-eval snapshot not found: {snapshot_path!r} "
            "(refusing to create an empty DB; point WORKBAY_RETRIEVAL_EVAL_SNAPSHOT "
            "or --snapshot at an existing handoff.db)"
        )

    conn = connect_handoff_db(str(path))
    try:
