...
RULE_VERIFICATION_EVIDENCE_MAX_LENGTH = "review_findings.verification_evidence_max_length"
...
RULE_STATUS_DECISION_REQUIRES_RATIONALE = "status_decision_requires_rationale"
...
def rejection_verification_evidence_max_length(
    *,
    actual_length: int,
    max_length: int = MAX_VERIFICATION_EVIDENCE_LENGTH,
    finding_id: str | None = None,
) -> dict[str, Any]:
    """verification_evidence exceeds MAX_VERIFICATION_EVIDENCE_LENGTH (normalized)."""

    fid = finding_id or "<finding_id>"
    example = (
        f'review_findings(review={{"operation":"resolve", "finding_ids":["{fid}"], '
        f'"verification_evidence":"<evidence <= {max_length} chars after trim>"}})'
    )
    return build_structured_rejection(
        rule_id=RULE_VERIFICATION_EVIDENCE_MAX_LENGTH,
        violated=f"verification_evidence length {actual_length} exceeds limit {max_length}",
        expected=f"verification_evidence must be <= {max_length} characters",
        example=example,
        error=f"verification_evidence must be <= {max_length} characters.",
    )
...
def rejection_status_decision_requires_rationale(
    *,
    status: str,
    finding_id: str | None = None,
) -> dict[str, Any]:
    """deferred/wontfix status decisions require non-empty rationale text."""

    fid = finding_id or "<finding_id>"
    example = (
        f'review_findings(review={{"operation":"resolve", "finding_ids":["{fid}"], '
        f'"status":"{status}", "resolution_notes":"<why this is deferred/wontfix>"}})'
    )
    return build_structured_rejection(
        rule_id=RULE_STATUS_DECISION_REQUIRES_RATIONALE,
        violated=(
            f"status={status!r} is a status decision and requires rationale "
            "(resolution_notes, notes, or verification_evidence)"
        ),
        expected=(
            "Provide non-empty resolution_notes (or notes / verification_evidence) when "
            "requesting status deferred or wontfix"
        ),
        example=example,
        error=(
            "status_decision_requires_rationale: deferred/wontfix require non-empty "
            "resolution_notes, notes, or verification_evidence."
        ),
