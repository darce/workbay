#!/usr/bin/env bash
# release_mcp_package.sh — tag a new release of mcp-workbay-handoff on the
...
INSTALL_URL="git+https://github.com/${REPO_SLUG}.git@mcp-workbay-handoff-v${VERSION}#subdirectory=packages/mcp-workbay-handoff"

echo "==> Releasing mcp-workbay-handoff v${VERSION}"
...
git -C "$REPO_ROOT" tag "mcp-workbay-handoff-v${VERSION}"
git -C "$REPO_ROOT" push origin HEAD
git -C "$REPO_ROOT" push origin "mcp-workbay-handoff-v${VERSION}"

echo ""
echo "Released: mcp-workbay-handoff-v${VERSION}"
