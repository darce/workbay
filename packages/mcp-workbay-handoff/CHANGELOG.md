...
### Added
- **Numpy-free vector codec for embeddings.** The new `embeddings.vector_codec` module now holds `text_hash`, `classify_coverage_row`, `serialize_vector` and `deserialize_vector`.
- **One canonical embedding-revision digest.** `model_pin.canonical_revision(...)` is a SHA-256 digest over a domain prefix and a sorted, compact JSON payload: schema version, model and tokenizer SHA-256s, source repo and revision, dimension, recipe version, and the sequence-token and character limits.
- **Typed embedding client contract.** The new `embeddings.client_port` module defines frozen `EmbeddingRequest`, `EmbeddingResult` and `EmbeddingClientError` records and an `EmbeddingClient` protocol at protocol version 1, where every result carries a typed status (`ok`, `unavailable`, `deferred`, `refused`, `deadline`, `revision_mismatch`) and a machine-readable reason token.
- **Typed worker supervision receipts.** The new `embeddings.worker_supervisor` module defines frozen registration and control receipts and the `WorkerRegistration` and `WorkerControl` protocols.

### Changed
- **Importing `workbay_handoff_mcp.embeddings` is now lightweight.** The package now eagerly imports only the stdlib model pin.

## [0.3.6] — 2026-09-21

### Changed
- Requires workbay-protocol 0.3.2.

## [0.3.5] — 2026-09-20

### Added
- **Unmerged-branch backlog detector in the dashboard's workflow-integrity section.** A branch that still carries commits and has never merged is now surfaced on the dashboard, so a lane whose worktree is gone but whose branch holds work stops being invisible.
- **Content-carrier gate at the stdio registration boundary**, with an explicit opt-out.

### Fixed
- **The branch-backlog detector asked `git for-each-ref` for a format field git does not support**, so it failed on every run and reported an empty backlog.
...
### Changed
- `bounded_call` tracks abandoned daemon threads and redirects their stdio; the atexit hook detaches stdio before any flush, so a wedged worker can no longer write into a closed stream at interpreter shutdown.
- Related-prior-work ranking selects the stored embedding dimension and threads it through the query, instead of assuming the default dimension.

## [0.3.3] — 2026-09-16

### Changed
- `review_findings` write contracts now reject unknown fields and require `finding_ids` and `all_open` to be mutually exclusive; adds the `integrate` operation and `status`/`severity` grammars.
- Added the `WORKBAY_TEST_TIER` pytest plugin and package conftest for deterministic fast/e2e test tiering.

## [0.3.2] — 2026-09-11

### Changed
- Bounded-call helper and an errors-record CLI added under `workbay_handoff_mcp`, so a hung dependency degrades typed instead of hanging a handoff write.
- Hold-scan no longer exceeds the global 30-second tool timeout.
- The lazy-export memo no longer shadows the handoff submodule, which had made `mock.patch` a silent no-op against it.

## [0.3.1] — 2026-09-11

### Fixed
- Shipped payload changes since 0.3.0 cover package initialization, bounded-call handling, embedding storage, the lanes API, and related-prior attachment.

## [0.3.0] — 2026-09-10

### Added
- The handoff surface now exposes direct `errors-record` CLI usage, lane `speed`/`tier` routing fields, and typed embedding outcomes (`deferred` and `failed`) so operators can distinguish unavailable retrieval from an empty result.

### Migration
- Schema 40 adds nullable lane routing columns for `speed` and `tier`; older readers may continue to ignore those fields.

## [0.2.27] — 2026-09-03

### Changed
- Re-pinned `workbay-protocol` to 0.2.7 so the handoff server ships against the shared `ok=false` response-envelope contract.

## [0.2.26] — 2026-09-02

### Changed
- Close-check now judges the latest HEAD row per command and names the failed latest-per-command receipt, so a stale earlier pass can no longer mask a current failure.
- Merged-close records `landing_commit_sha` and `branch_tip_sha`, and the branch tip is bound onto the lane row before teardown, so a reaped worktree still leaves provable landing evidence.
- Branch-reclaim records are readable and remain reachable after worktree removal.
- `related_prior` attach reworked: bounded 5s timeout and advisory attach after blocker and runtime agent-error writes, so prior-art lookup can no longer stall or fail a write.
- Bounded worker dormancy replaces unbounded waiting.

### Added
- Schema 37 adds nullable `worker_reports.delivery_id` with a unique receiver claim.

### Migration
- Already-stamped databases add the nullable column and unique index on the next open.
- The minimum compatible reader is deliberately raised to schema 37.

### Fixed
- Skill-roster source audits now validate the decoded YAML document and its `mcp_tools` field before consuming either, so malformed non-authoritative skill metadata emits `roster_audit_failed` without aborting server startup.
- The terminal-roster fallback now refuses an exhausted startup budget and uses SQLite progress interruption to bound ordinary query execution as well as lock contention.

## [0.2.21] — 2026-08-22

### Added
- Schema 34: non-unique ``idx_review_findings_finding_id`` (leading ``finding_id``) so finding_id-only ``UPDATE``s SEARCH a dedicated index instead of skip-scanning the unique ``(task_ref, finding_id)`` composite.
- **Doctor `mcp_protocol` facet.** `doctor` reports `{ok, sdk_version, latest_protocol_version, fastmcp_version, declared_fastmcp, note?}`.

### Migration
- Schema version 34.

### Fixed
- **Declared FastMCP pin is readable in a wheel.** The facet prefers `importlib.metadata.requires("mcp-workbay-handoff")` and only walks `pyproject.toml` when the dist is not installed.
- **`shared_primitives` no longer reimplements pin resolution (round-3 claude-04).** `_resolve_declared_fastmcp_requirement` duplicated the metadata-then-walk ordering that `_declared_fastmcp.resolve_declared_fastmcp_requirement` already owns (REF-09 — one fact, one owner).
- **An unresolvable pin comparison now flips the facet `ok` (round-3 claude-05).** `_version_satisfies` imported `packaging` inside a bare `except Exception -> None`, and `check_mcp_protocol` fed that `None` into an `unresolved` note without ever setting `ok=False` — a genuinely broken comparison read as `ok=True`, the same shape as a clean pass.
...
- **`check_mcp_protocol` no longer has dead degrade arms (round-3 claude-07).** Two `try/except` arms could never fire (`import importlib.metadata` under `requires-python>=3.12`; `getattr(obj, name, default)`, which cannot raise) and are removed.
- **`backfill_concept_embeddings` applied `.workbay/embedding.env` from the wrong worktree (CL0816-SEMRET-R3REV-claude-01).** `RuntimeConfig.from_args` collapses `workspace_root` to the primary git worktree; the CLI now applies the embedding env from `git_workspace_root` (the calling checkout) instead, matching the hooks and the bootstrap doctor.
- **The backfill CLI collapsed every non-`None` artifact probe reason to `artifact_unreadable` (CL0816-SEMRET-R3REV-claude-03).** `_resolve_backfill_provider` now preserves the typed probe reason (`artifact_missing:` / `artifact_sha_mismatch:` / `artifact_unreadable:`) so a missing file and a hash mismatch no longer read identically, and agree with the doctor's action note for the same probe.
- **`backfill_concept_embeddings --dry-run` reported a hash-only "success" even with no provider configured (CL0816-SEMRET-R3REV-claude-07a).** A no-op preview impersonated the same shape as a real preview instead of matching the write path's fail-closed behavior.

### Tests
- **`test_embedding_env.py`'s hermetic-env fixture could leak a direct `os.environ` write past teardown (CL0816-SEMRET-R3REV-claude-06).** `monkeypatch.delenv(key, raising=False)` on an absent key registers no undo action, so `apply_embedding_env`'s direct `os.environ` write survived teardown into the next test / the operator shell.

## [0.2.19] — 2026-08-20

### Added
- `review_findings_archive` cold table (schema v35) mirroring `review_findings` plus `archived_at`.
- Schema 35 is additive.

### Migration
- Already-stamped handoff DBs (operator host is at 33) re-enter bootstrap because `HANDOFF_SCHEMA_VERSION` is now 35 and `review_findings_archive` is in `_HANDOFF_REQUIRED_TABLES`.
- Archival DELETEs rely on `findings_fts_delete` for FTS consistency, then run FTS5 `optimize`.
- MCP `list_review_findings` / `get_review_findings_summary` / `search_handoff` / snapshot export/import remain hot-table-only.

### Fixed
- `archive_terminal_review_findings` no longer assigns `isolation_level` on a connection that already holds a transaction (Python 3.12 would commit it).
- Archive INSERT is abort-on-conflict (not `OR IGNORE`) and DELETE runs only after `changes()` equals the qualify list, so a PRIMARY KEY collision cannot drop a hot row that was not copied.
- `TERMINAL_REVIEW_FINDING_STATUSES` is derived from `FindingStatus` minus OPEN and FIXED in `shared_primitives`, then imported by the reaper.
- Qualification is "task is not live" (`handoff_state.status` outside `LIVE_ACTIVE_STATUSES`), not "a done/archive marker still exists".
- Hot and archive tables share one column list.
- `reap_tasks` / `archive_task_state` still return `ok=True` when the MOVE fails, but attach `data.side_effect_failures` (`effect=archive_terminal_review_findings`, `error_class=system`) plus a warning so the miss is MCP-visible.
- Schema v34 stays unused.

## [0.2.18] — 2026-08-17

### Added
- `find_related_prior_work` accepts `include_scopes`, so prior-work recall can be narrowed to chosen record scopes.

### Fixed
- `search_handoff` ranking is pinned to FTS5 `MATCH` and guarded at execute time, so a non-FTS5 fallback can no longer silently reorder results.
- Oversized compaction residuals are skipped in cosine ranking instead of being reported as `model_mismatch`, and a missing source fails closed on the residual rank bound.
- `include_scopes` reads archived snapshot state, so `include_plan_paths` survives a restore; a `str` or `bytes` scope is treated as one token rather than split per character.
- Embedding backfill CLI: `--kinds` and `--limit` route through the lock-free backfill path, and `--dry-run` shares the write path so a dry run reflects what a real run would do.

## [0.2.17] — 2026-08-10

### Fixed
- **The SQLite write-freeze remediation (internal).** This is the bulk of the release.
- **Whole-call write-lock retry, with honest attribution.** The retry layer no longer reports a waiter as the holder; `write_retry` splits the two and names the real one.
- **Shared heartbeat replaces the per-open thread.** Writer-registry liveness used one thread per open connection plus a 20 Hz poll; it is now a single process-wide pulse.
- **Barrier reachability is established and the opt-in surface is gated.** A new gate pins which modules classify as write-lock-marker modules and what direct-blocking calls they contain, so a module that newly imports the shared connection factory cannot silently sweep pre-existing `subprocess.run` sites into the barrier's blind spot.
- **Operator surfaces are written atomically.** `DASHBOARD.txt`, the render fragments and `CURRENT_TASK.json` go through `mkstemp` + `os.replace`, so a reader never sees a half-written file.
- **Embed-on-write failures reach the caller.** A failed embedding side effect is surfaced on the write envelope with its error class instead of being swallowed; the raise arms are pinned.
- **The test suite no longer writes fixture rows into the operator database.** Hermetic handoff-DB isolation lands per test, which removes a class of cross-run contamination that made suite state depend on run order.
- **Scratch-finding retirement.** The retirement veto is widened to the module's terminal predicate, and `SUPERSEDED` coordinator copies carry a row-local `resolution_notes` discriminator so a merged copy is distinguishable from its source.

### Migration
- No field-shape changes.

## [0.2.16] — 2026-08-02

### Fixed
- **Repo-marker ascent now classifies `.git` the way git does.** The walk accepted any `.git` entry that existed, so litter between a checkout and a deeper probe silently anchored the wrong root.
- **A declared `workspace_root` is authoritative** and is no longer overridden by an enclosing `.git` further up the tree.
- **`set_handoff_state` flushes durably** (finding 10093).
...
### Added
- **Codemap reindex lease and singleton runner (implementation note).** New `codemap_reindex_lease` table plus lease primitives, and a single coalescing reindex runner: concurrent reindex requests join the in-flight run instead of each spawning their own indexer.
- **Retrieval-eval harness (`make retrieval-eval`, implementation note).** BM25 and RRF-hybrid arms scored at record level against 23 mined ground-truth cases spanning 8 `task_ref`s, with an operator-runnable eval path.

### Changed
- `verification_evidence` now advertises its length cap in the tool schema **and** enforces it before the call reaches the server, so an over-length value is rejected where the caller can see it.
- RRF fusion operates on the deeper per-arm candidate pools rather than each arm's already-truncated `top_k`.
- The lexical arm issues a tokenised FTS5 query instead of matching the whole query as one phrase, so the BM25 baseline is a fair comparison rather than a strawman.
- `row_factory` is set during bootstrap for bare connections.

### Fixed
- The package's own type gate was dead, not passing: `[tool.mypy] python_version` said `3.11` while `requires-python` says `>=3.12`, so every run aborted on a dependency stub using a PEP 695 `type` statement before checking a single line of our source.

## [0.2.14] — 2026-07-22

### Changed
- Default CURRENT_TASK and dashboard `turn_metrics` projections exclude `phase='lane_prep'` footprint-telemetry rows (implementation note) so agent-turn evidence is not crowded out of LIMIT windows.

## [0.2.10] — 2026-07-17

### Changed
- Release-cycle maintenance: dependency-floor + monorepo pin sync.

## [0.2.9] — 2026-07-16

### Changed
- Version bump alongside the 0144 `grok-remote` release train (formatter sweep and shared test fixtures only; no behavior change in this package).

## [0.2.8] — 2026-07-14

### Changed
- Write attribution: a live coordinator writing under a different harness now takes precedence over a stale last-writer identity on the same handoff row (fixing the case where the row's prior lane-worker identity silently won).

## [0.2.7] — 2026-07-13

### Changed
- Semantic reinjection reliability (0115): stop the silent embeddings-unavailable no-op — guard `semantic_reinjection_packet` imports and emit typed degrade/remedy hints instead of a hard tool error; dedupe now suppresses rather than duplicates.
- Cross-session continuation packets (0128).
- `agent_errors` pre-rebrand DB retrofit through the warm-start migrator; `backend_transient` error classification so a coordinator re-dispatches mechanically.
- Scope `resolve` pending-uncommitted to finding file paths; brand-token reword fixes.

## [0.2.6] — 2026-07-12

### Changed
- `agent_errors` pre-rebrand DB retrofit: register the `workbay_release` column in the warm-start net (schema v27→v28) so an already-stamped DB retrofits it and error telemetry stops going dark.
...
- Brand: inline-exempt the legacy release column-name references in `shared_schema`.
...
- Fix stale `EXPECTED_HANDOFF_TOOL_COUNT` (25→26); `finalize-plan` now emits `untickable_acceptance_boxes` as a first-class receipt field.
...
- Register `lane_messages.dispatch_id`/`payload_json` columns and bump `HANDOFF_SCHEMA_VERSION` 23 → 24 so the migrator stays reachable on stamped-current DBs (with regression + drift-guard coverage).
...
### Added
- implementation note (internal): `semantic_reinjection_packet` MCP tool wrapping `build_semantic_reinjection_packet` — inputs `task_ref`, `anchor_texts` (slice changed files / rationale), optional budget override; returns bounded `relevant:` lines with `status` / `skip_reason` and clean `provider_unavailable` degradation so review-parallel never blocks on embeddings.
...
### Changed
- internal: harness launch via `mcp_launch.py` shim (workspace `.venv` or bootstrap-provisioned `uv tool` binary); PyPI `uvx …@` retired.
...
### Changed
- **Tool-surface flattening: 30 → 21 (internal)**: the workbay-handoff-mcp surface contracts via the discriminated-operation pattern established by ADR-005.
- `mcp-workbay-handoff validate-decision-id ...` / `mcp-workbay-handoff validate-write ...` → `mcp-workbay-handoff validate --kind decision_id|write ...`
- `mcp-workbay-handoff compact-session ...` / `mcp-workbay-handoff get-compaction ...` / `mcp-workbay-handoff get-latest-compaction ...` → `mcp-workbay-handoff compaction --operation record|get|get_latest ...`
- `mcp-workbay-handoff record-file-touch ...` / `mcp-workbay-handoff get-touched-files ...` → `mcp-workbay-handoff touched-files --operation record|list ...`
- `mcp-workbay-handoff working-tree-integrity-check ...` / `mcp-workbay-handoff post-merge-integrity-check ...` / `mcp-workbay-handoff handoff-close-check ...` → `mcp-workbay-handoff integrity-check --kind working_tree|post_merge|close ...`
- `mcp-workbay-handoff archive-task-state ...` / `mcp-workbay-handoff tasks-gc ...` / `mcp-workbay-handoff get-archived-task ...` → `mcp-workbay-handoff archive --operation archive|gc|get ...`
- `mcp-workbay-handoff task-status ...` → `mcp-workbay-handoff set --status-only --task-ref X --status Y [--expected-revision N] ...`
...
- **Compaction CLI** (`workbay_handoff_mcp.compaction_cli`) — bounded read surface for compacting session state from the command line; `make compact-now` driver target wraps it.
...
- `workbay_handoff_mcp.PlanLocation`, `PlanPathNotRegistered`, `resolve_plan_location`, `list_active_task_locations`, `plan_show_command` — programmatic plan-path resolution.
- `workbay_handoff_mcp.plan_cli` — CLI driver with `{show, edit, list, register}` subcommands invoked by the `make plan-show / plan-edit / plans-list / plan-register` recipes.
- `workbay_handoff_mcp.scripts.backfill_plan_paths` — one-shot enumerator + writer that populates `task_plan_path` on existing in-progress tasks via frontmatter discovery.
