...
| `docs/plugin-distribution.md`                 | Operator guide for `make plugins-build` / `make plugins-check` and the Claude / Codex / Cursor / grok install flow. See also the corresponding implementation note. |
...
`workbay-system 0.3.21` (see `pyproject.toml`); the current monorepo
distribution tag is `v0.1.55`. See `CHANGELOG.md` for the full history.
The skill catalog lives under `workbay_system/payload/skills/` (25 skill
directories at this release).

## Provenance

- `v0.2.0` was extracted from a consumer repository snapshot. It expanded the
  skill catalog, hoists the `.claude/commands/`, `.github/prompts/`, and
