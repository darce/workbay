...
```json
{
  "mcpServers": {
    "workbay-handoff-mcp": {
      "type": "stdio",
      "command": "uvx",
      "args": ["mcp-workbay-handoff@0.13.3", "--workspace-root", ".", "serve-stdio"]
    },
    "workbay-orchestrator-mcp": {
      "type": "stdio",
      "command": "uvx",
      "args": ["mcp-workbay-orchestrator[bridge]@0.8.0", "--workspace-root", ".", "serve-stdio"]
    }
  }
}
```
...
When a shipped plugin surface changes, including MCP runtime pins, skill bodies,
generated prompts, plugin metadata, or MCP server maps:

1. Update the changed canonical inputs. For MCP runtime releases, update the
   `args` entries in
...
4. When MCP runtime pins changed, run `make mcp-pins-sync` to regenerate
   `workbay-bootstrap`'s `DEFAULT_MCP_SERVERS` from `mcp_servers.yaml`
