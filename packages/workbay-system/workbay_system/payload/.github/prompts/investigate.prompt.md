...
Load the `investigate` skill for this workflow.
...
Makefile entry point: `(in-session root-cause skill; no standalone make target)`

Arguments:
- `symptom` (required): Observed failure, error output, or unexpected behavior to diagnose.
- `scope-hint` (optional): Optional file, module, test command, task_ref, or subsystem that bounds the investigation.

Execution context: Use when the symptom is known but the cause is not yet proven: failing tests, runtime errors, unexpected behavior, config drift, or review findings that have reopened.
