...
Load the `auto-fix` skill for this workflow.
...
Execution context: Use when iterating on a specific failing test inside an active feature-branch task. The loop uses bounded handoff reads per iteration, commits each candidate fix before testing so verified_tests provenance matches HEAD, and reserves integrity_check(payload={"kind":"close","enforce":true,"require_fresh_tests":true}) as the single post-loop pre-merge gate. Refuses to run on main/master/None target_branch.
