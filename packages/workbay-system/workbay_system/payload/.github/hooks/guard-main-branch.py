...
def main() -> None:
    ...
    # Must run BEFORE unpack / permitted-candidate carve-out: the synthetic
    # diagnostic path is never a real candidate, so carve-out would ALLOW.
    if isinstance(dirty_result, DirtyProtectedDegraded):
        _log_telemetry(
            tool_name,
            [],
            dirty_result.branch,
            outcome="dirty_protected_probe_degraded",
        )
        print(
            json.dumps(
                {
                    "hookSpecificOutput": {
                        "hookEventName": "PreToolUse",
                        "permissionDecision": "block",
                        "permissionDecisionReason": dirty_result.block_message,
                    }
                }
            )
        )
