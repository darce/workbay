...
def test_hook_blocks_when_dirty_probe_degraded_with_permitted_candidate(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    """ARM C: degraded probe + non-empty permitted candidate_paths → BLOCK.

    Pre-fix the VS Code hook unpacked ``DirtyProtectedDegraded`` via the
    ``__iter__`` shim into a synthetic diagnostic path, then ran the
    permitted-candidate carve-out. The synthetic string is never a real
    candidate path, so the carve-out ALLOWed while git was unhealthy —
    the inverse of fail-closed. Pins the isinstance-branch that blocks
    with the shared degraded wording before unpack/carve-out.
    """
    import io
    ...
    from _harness_protocol import MainSurfacePattern  # noqa: E402

    repo = _write_fixture_repo(tmp_path)
    permitted = repo / "docs" / "tasks" / "plan.md"
    ...
    policy = BranchIsolationPolicy(
        ...
        protected_main_surfaces=(),
        permitted_main_surfaces=(
            MainSurfacePattern(pattern="docs/tasks/**/*.md", reason="Task plans"),
        ),
        first_edit_protected_surfaces=(
            MainSurfacePattern(pattern="docs/tasks/**", reason="planning"),
        ),
    )

    monkeypatch.setattr(_mod, "_current_branch", lambda: "main")
    monkeypatch.setattr(_mod, "_repo_root", lambda: str(repo))
    monkeypatch.setattr(_mod, "load_branch_isolation_policy", lambda _root: policy)
    monkeypatch.setattr(_mod, "_check_branch_naming", lambda _branch: None)
    monkeypatch.setattr(_mod, "_check_file_edit", lambda *_a, **_k: None)
    monkeypatch.setattr(
        _mod,
        "_check_dirty_protected_paths",
        lambda **_k: DirtyProtectedDegraded(branch="main"),
    )

    payload = {
        "toolName": "create_file",
        "toolInput": {"filePath": str(permitted)},
    }
    ...
    assert decision["permissionDecision"] == "block", (
        "degraded probe must BLOCK (ARM C), not allow via permitted carve-out; "
        f"got {decision!r}"
    )
    reason = decision["permissionDecisionReason"]
    assert reason == GIT_PROBE_DEGRADED_BLOCK_MESSAGE, reason
    assert "already dirty" not in reason.lower(), reason
