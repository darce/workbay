...
- [ ] **Coupling-type triage (Nygard)** — classify each cross-module link (Operational / Developmental / Semantic / Functional / Incidental); harmful coupling flagged.
- [ ] **Independent-deployability test** — services claimed independent must ship without being tested alongside collaborators.
- [ ] **Ports & adapters at external seams** — out-of-scope dependencies wrapped behind minimal adapter.

### Non-Functional & Failure-Mode Realism

- [ ] **Consistency model stated** — replicated/cached/multi-reader design declares strong/causal/session/eventual + stale-read tradeoff. Q: what stale reads are acceptable?
- [ ] **Replication-lag anomalies** — async replicas: read-your-writes, monotonic reads, causal order each named with mitigation.
- [ ] **Schema evolution compat** — rolling upgrades coexist old+new code; expand→migrate→contract for no-downtime changes.
- [ ] **SLA / capacity math** — per-feature SLA and front/back capacity ratios; promo load cannot invert safe ratios.
- [ ] **Partition / hot-spot skew** — partition key choice reviewed for range-key and low-cardinality hot spots.
- [ ] **Steady-state reclaimer** — accumulators (tables/logs/sessions/cache) have same-rate purge.
- [ ] **Cache bound + invalidation** — unbounded cache and miss-path latency called out.
- [ ] **Distributed-transaction cost** — atomic writes across 2+ stores justify 2PC/consensus cost.
- [ ] **Integration DB red-flag** — plan does not read another service's DB directly.
...
- [ ] **YAGNI / speculative-generality gate** — reject hooks/abstract layers with one (or zero) current consumers.
- [ ] **Leaky abstraction** — abstractions must not leak transport/storage detail into domain logic.
- [ ] **Trade-off named** — the plan states what its chosen approach sacrifices; a plan with no downsides hasn't found them yet.
- [ ] **Service-split justification** — splits name a measured disintegrator and rule out integrators; no table gains a second writer.
- [ ] **Subdomain classified** — new modules state core/supporting/generic; CRUD-simple logic is not DDD-ified.
- [ ] **Untested-code path planned** — plans touching untested code name the seam/characterization step before the change step.
- [ ] **LLM trust boundaries** — plans granting model agency name scopes, gates, and output mediation.
- [ ] **Collection/latency bounds** — new endpoints bound collections and hold no long synchronous connections.
