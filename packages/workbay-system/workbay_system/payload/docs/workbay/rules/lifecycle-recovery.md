...
- **Escape hatch** (canonical recipe — skills must cross-reference this
  surface rather than re-authoring command names):
  1. Pass `task_ref=<one of the listed candidates>` (or `TASK=` on the
     make target) to bind the operation explicitly when you already know
     which row is correct.
  2. If closeable stale rows are the source of ambiguity: run
     `make task-reap` (dry-run) to see closeable vs active rows;
     `make task-reap REAP_ARGS=--apply` closes only closeable rows; then
     re-run `make context`. `make context` exits `2` (not `1`) on this
     ambiguity.
...
## Codemap index herd (concurrent `auto_watch`)

- **Symptoms**: several agent sessions are open; MCP writes start failing, hooks
  return invalid JSON, and offload lanes refuse admission — all at once. These
  look like three unrelated faults; they are one cascade. Each session that
  enables `auto_watch` spawns its own filesystem watcher. Concurrent sessions
  therefore reindex the same repo in parallel (an index herd), exhaust host RAM,
  and that pressure produces `handoff.db` lock contention, which surfaces as the
  MCP / hook / offload failures above. Without recognising the cascade, an
  operator debugs the wrong layer for an hour.
- **Immediate recovery**: (1) terminate every codemap process; (2) then set
  `auto_watch` false. Order matters — setting the config first does not stop
  watchers that are already running.
- **Standing configuration**: leave `auto_watch` off. Reindex is driven by the
  handoff-coordinated singleton (on commit / main-SHA advance), not by
  per-session watchers. **Cost**: with `auto_watch` off the index is stale
  between commits; that lag is the accepted trade for avoiding the herd.
- **After recovery**: check index freshness via the lifecycle doctor's
  codemap-freshness facet. It reports whether the on-disk index is current
  relative to HEAD, so a stale index is visible rather than silent.
