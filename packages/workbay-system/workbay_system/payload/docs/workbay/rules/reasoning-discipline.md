...
**Agent craft** (session discipline; rationale in [engineering-heuristics.md § Agent Craft](engineering-heuristics.md#agent-craft)): orient before acting [AGT-01]; never cite an unresolved anchor [AGT-02]; make it fail before making it pass [AGT-03]; evidence verbatim in every completion claim [AGT-04]; a guard rejection is a specification, not an obstacle [AGT-08]; externalize findings at discovery — notes survive compaction, working memory doesn't [AGT-07]; timebox after three unproductive attempts and hand off legibly [AGT-12].
