...
**Loop discipline** (rationale in [heuristics canon](heuristics-canon.md)):
...
`code_roots`, `protected_extensions`, `first_edit_protected_surfaces`,
`state_dirty_surfaces`, `root_protected_files`) has uncommitted
modifications, untracked content, or staged changes. The set of protected
paths is the same set the PreToolUse, pre-commit, and pre-push gates
already enforce; the dirty-main policy is the routine-vs-publish severity
matrix layered on top of that detection.
...
`make release-monorepo` runs `make check-release-tag-provenance` (the online,
fatal form) right after it cuts and pushes the consumer-facing monorepo tag,
so the tag's provenance is verified at the moment of the release cut rather
than in every developer's dev-loop `make check-all` run. The target verifies
that the newest release tag is annotated, reachable on the development
origin, and maps to the published distribution SHA. Set `RELEASE_TAG=<tag>`
to check a specific tag; set `PROVENANCE_OFFLINE=1` to skip the distribution
fetch (e.g. for an offline or history-stripped checkout) - the release cut
never passes this knob, so it always runs the online form.
...
`python3 -m workbay_handoff_mcp.compaction_cli`,
