...
- `--tool-profile` or `WORKBAY_HANDOFF_TOOL_PROFILE` — legacy compatibility input. `all`, `core`, and `extended` are accepted, but all launches now expose the same surface (count pinned by `EXPECTED_HANDOFF_TOOL_COUNT`).
...
### `render_handoff` current-task resolution

For `kind="current_task"`, the response envelope separates resolver output
from the rendered workspace summary. `data.task_ref` is the authoritative task
selected for this read. An explicit `task_ref` argument takes precedence;
otherwise normal active-task resolution applies, including a valid
`WORKBAY_HANDOFF_ACTIVE_TASK` pin. A configured pin that is missing or not live
is an error and is never replaced silently with an unpinned selection.

`data.current_task_json` is the rendered snapshot string when
`write_file=False`, and is `null` when the snapshot was written to disk. Its
shape describes workspace occupancy, so it may remain `workspace_ambiguous`
and list multiple live tasks even when `data.task_ref` names the resolver's
read-time selection. Lifecycle `context` may consume that selected task while
also surfacing the ambiguity; lifecycle readers that require a unique
workspace owner may intentionally reject the ambiguous summary instead.

`data.tiebreak_note` and `data.tiebreak_candidates` are nullable resolution
sidecars. Both are `null` when no resolver tiebreak occurred. When the resolver
uses its allowed all-`MAINT-` integration-branch recency tiebreak,
`data.tiebreak_note` explains the choice and pin recovery, and
`data.tiebreak_candidates` names every candidate considered. Consumers must
not infer a tiebreak from `data.current_task_json` alone.

Delivery is **git-only** (internal): consumers install from
tagged refs on [`darce/workbay`](https://github.com/darce/workbay); PyPI
`uvx …@<pin>` launchers are retired.

## Harness MCP launch

Harness root configs (`.mcp.json`, `.vscode/mcp.json`, `.codex/config.toml`)
register this server through the stdlib overlay shim — not `uvx`:
...
`scripts/hooks/mcp_launch.py` resolves the deps-bearing console in this
order:

1. Workspace-root `.venv/bin/<console>` (worktree, then primary checkout).
2. `uv tool` binary installed during `workbay-bootstrap install`
   (one-time git-sourced closure via `gitonly_closure.build_uv_tool_install_argv`
   and `--with` sibling members).
3. **Session serve** (`serve-stdio` default args): fail loud with
   `workbay-bootstrap install` guidance — no per-session PyPI/`uvx`.
4. **Provisioning** (non-default forwarded args, e.g. `init-state`):
   `uv run --no-sync --project <clone>/packages/mcp-workbay-handoff …`
   when the overlay clone carries the package.

Canonical manifest: `config/agent-workflows/mcp_servers.yaml` (generated
into bootstrap `_mcp_pins.py` by `make mcp-pins-sync`).

Consumer install:

```bash
uv tool install --no-sources \
  "git+https://github.com/darce/workbay.git@vX.Y.Z#subdirectory=packages/workbay-bootstrap" \
  --with "git+https://github.com/darce/workbay.git@vX.Y.Z#subdirectory=packages/workbay-protocol" \
  --with "git+https://github.com/darce/workbay.git@vX.Y.Z#subdirectory=packages/mcp-workbay-handoff" \
  --with "git+https://github.com/darce/workbay.git@vX.Y.Z#subdirectory=packages/mcp-workbay-orchestrator" \
  workbay-bootstrap
workbay-bootstrap install --target <repo> --remote-ref vX.Y.Z
```

Install receipt records `gitonly_mcp_tools` when the closure is provisioned.
...
| `search_handoff` | generator | yes | Returns ranked snippets over handoff FTS tables, including verified test evidence. `detail` accepts `full` (default) or `summary`, and `fields` accepts a comma-separated per-result projection. Keyword/ID only — paraphrase / like-this queries use `find_related_prior_work`. |
| `find_related_prior_work` | query | yes | Advisory cosine neighbors of one raw phrase or one `finding_id`. Exactly one of `text` / `finding_id`. `task_ref` excludes the current task. Do not invent FTS synonyms; read `status` / `embeddings_mode` / `semantic_degrade_reason` before treating empty `results` as "nothing similar." |
...
- Preferred escalation order for open-issue lookup is: `get_latest_slice_review_packet` -> `get_review_findings_summary` -> `review_findings(review={"operation":"list","status":"open"})` -> `load_session` / `search_handoff` (tokens/IDs) / `find_related_prior_work` (like-this paraphrase).
...
1. `load_session`
2. `search_handoff(queries=["slice_complete"], record_types=["decision"], limit=1)` for tokens and IDs
3. `find_related_prior_work(text=...)` for like-this / paraphrase queries (do not invent FTS synonyms)
4. `get_verified_tests(task_ref=..., commit_sha=...)`
5. `review_findings(review={"operation":"list","status":"open"})`
...
Terminal-guard state is dormant by design in normal operation. Empty terminal-guard rows are expected, are not an orientation input, and should not be treated as evidence that command validation is active or inactive.
...
## Related Prior Work (`find_related_prior_work`)

`find_related_prior_work` is the paraphrase / like-this discovery verb. It ranks
`concept_embeddings` by cosine and returns **advisory** neighbors (CAL-03): hits
never gate, filter, auto-close, or auto-link. Dual indexes stay unfused —
`search_handoff` remains BM25/FTS5 keyword/ID search; this tool is the
no-synonym path. Do not invent FTS synonyms for `search_handoff`.

### Tool Signature

```python
find_related_prior_work(
    text: str | None = None,              # XOR finding_id
    finding_id: str | None = None,        # XOR text; stored vector, no model load
    task_ref: str | None = None,          # exclude this task (prior work, not siblings)
    include_task_refs: list[str] | None = None,    # membership filter (repeatable)
    include_plan_paths: list[str] | None = None,   # membership filter (repeatable)
    include_statuses: list[str] | None = None,     # membership filter (repeatable)
    limit: int = 5,
    min_score: float | None = None,       # inclusive cosine floor; None = library default
    source: str | None = None,            # orientation-read telemetry
    session: str | None = None,           # orientation-read telemetry
) -> dict:
```

Exactly one of `text` or `finding_id` is required. A both-missing or both-set
call is a **typed** envelope (`status` is not `ok_with_results`,
`note="exactly_one_of_text_or_finding_id"`, `results=[]`, `ok=True`); it is
not a crash. `task_ref` is an **exclude**.
`include_*` filters are additive membership constraints applied before cosine;
they do not invert `task_ref` exclude semantics.

The MCP envelope adds `ok` (transport; typed empties stay `ok=True`),
`embeddings_mode`, and `semantic_degrade_reason`. Callers MUST branch on
`status` — never on `ok` or empty `results` alone.

### Typed Statuses

Always inspect `status` (and MCP `embeddings_mode` / `semantic_degrade_reason`)
before treating `results` as "nothing similar":

- `ok_with_results` — advisory hits above `min_score`
- `no_results_above_threshold` — store had candidates; none cleared the floor
- `provider_unavailable` — free-text mode needs a provider; `finding_id` does not
- `model_mismatch` — store vectors exist but none share the query `model_id`
- `store_empty` — no `concept_embeddings` rows

Include-scope / exclude empties stay `no_results_above_threshold` with
`note="no_candidates_after_exclusions"`. Results are advisory only (CAL-03).
A typed empty state is not an absence proof.

### Escalation Split

- Tokens / IDs / exact keywords → `search_handoff` (BM25/FTS5).
- Paraphrase / like-this / no-synonym queries → `find_related_prior_work`.
- Compaction-anchor refresh remains `semantic_reinjection_packet`; it is not a
  like-this discovery verb.
...
- `review_runs(review={"operation":"record", ...})` requires `review_run_id` (must be globally unique in the ledger), `session`, `subject_path`, and an explicit `task_ref` naming a live or archived task. Unknown task refs are refused with `unknown_task_ref`; blocked success verdicts use `open_high_findings_block_pass_verdict`, and an exhausted enabled round bound uses `review_rounds_exhausted`. `subject_kind` defaults to `task_plan`; valid values are `task_plan`, `epic`, `branch`, `adr`, `roadmap`, `other`. `review_mode` defaults to `planning`; valid values are `branch`, `release_audit`, `planning`. `verdict` is optional; valid values are `pass`, `pass_with_findings`, `fail`, `conditional_pass`. `verdict_decision` is optional and should hold the stable decision string from the decision variant of `record_event` (not the integer id). Finding batch and merge writes canonicalize known live or archived task refs, but preserve an unregistered task ref so orchestrator harvests can create their ledger scope.
...
- **Verification commands (E7):** When `close_check.required_commands` is configured on the handoff runtime (`WORKBAY_HANDOFF_CLOSE_CHECK_REQUIRED_COMMANDS`, JSON array or newline-separated), `integrity_check(kind="close", enforce=True)` runs each command in the resolved task worktree cwd before returning `ready_to_close`. Fail-closed on non-zero exit, timeout (default 600s per command), or unlaunchable command. Captured stdout/stderr are truncated and redacted via `agent_errors._redact_text` (`_DETAIL_LIMIT` 4096). Dry probes (`enforce=False`) and empty config are strict no-ops.
...
After a compaction, the SessionStart reinjection hook (implementation note) re-surfaces handoff state to the model. implementation note adds an **opt-in semantic selection path**: the hook can rank stored handoff concepts by embedding cosine similarity to a composed anchor and emit top-K readable `relevant:` bullets on compact SessionStart. When off or degraded, SessionStart emits the **generic** reinjection block only (no live recency/ID `relevant:` arm — implementation note S4). The feature is **off by default** and never reaches the network. Post-install, toggle the embeddings file gate with `workbay embeddings --status|--enable|--disable` (cwd = workspace root; `workbay-bootstrap embeddings` is a deprecated alias). Operator setup, backfill, and the int8/fp16 choice are documented in `packages/mcp-workbay-handoff/docs/guides/semantic-reinjection.md`.
...
### Embedding artifact (offline, hash-pinned)

The model is `Alibaba-NLP/gte-base-en-v1.5` (int8 ONNX, 768-d, CLS pooling + L2-norm, no query/passage prefix). The ~147 MB artifact is **not** bundled in the wheel/sdist. Digest pin SSOT: `workbay_handoff_mcp.embeddings.model_pin` at HF revision `a829fd0e060bb84554da0dfd354d0de0f7712b7f`:

| Artifact | SHA-256 |
|---|---|
| `onnx/model_int8.onnx` | `e7f6af7a9457d4fdd3af220c68e9a37325aad7c2d306bbc855fe0d019c326509` |
| `tokenizer.json` | `cb374d6bc042c22455946f4e09a89d29882a199fdaf8fb25be00dc8b8857a448` |

**Auto-provision (C1):** `workbay-bootstrap install` / `repair` (default-active) download, verify, cache under `~/.cache/workbay/models/<digest>/`, and write `.workbay/embedding.env` (four artifact vars + `WORKBAY_REINJECT_SEMANTIC=1`). Hooks and `mcp_launch.py` load it set-if-unset. Opt out: `--no-embeddings` or `WORKBAY_HANDOFF_EMBEDDINGS_DISABLED=1`. `workbay-bootstrap doctor` surfaces `embedding_artifact_*` delivery-state.

Manual configuration remains env-only; all four artifact vars must be set for the provider to resolve (else clean degrade):
...
|---|---|---|
| `WORKBAY_REINJECT_SEMANTIC` | off | Master semantic-arm switch. When truthy, SessionStart may append `relevant:` concepts **only** for `source=compact` with a non-empty `compaction_id`, and not when sticky `session_reinjection_exists(task_ref, compaction_id)` already saw a **selected** emission (non-empty `selected` and/or `chars_used` > 0). Resume/startup keep the generic block from `WORKBAY_REINJECT_SOURCES` only. Off ⇒ generic block only (no semantic `relevant:`). |
...
`session_reinjections` rows record each SessionStart reinject emission. Core columns: `reinjection_id`, `session_id`, `harness`, `task_ref`, optional `compaction_id`, `source` (`compact` or `resume`), `emitted_chars`, optional `arm` (legacy nullable; write-retired after implementation note S4 — new rows leave arm NULL; historical treatment/control may exist), and optional `semantic_detail_json` — canonical compact JSON (`sort_keys=true`, `,`/`:` separators) of the typed semantic reinjection telemetry packet when semantic selection runs. Null `semantic_detail_json` means no structured detail was persisted for that row. Sticky dedupe for the semantic arm counts only `status=selected` rows with emission evidence; budget-dropped selections record `status=skipped` / `skip_reason=budget_exhausted` so they do not stick.
...
### Offline recall efficacy gate (implementation note)

Live A/B arm assignment is retired (implementation note S4). The pre-registered offline rule (`embeddings/eval_recall.py::apply_recall_gate`) still compares semantic vs recency on labeled fixtures: adopt semantic only if `recall@K` improves at equal-or-lower reinjected tokens. On the offline labeled fixture semantic recall@3 = 1.0 vs recency 0.0 at equal token cost (decision: adopt); live multi-session token medians remain operator-owned. A cross-encoder reranker is an explicit deferred follow-up (plan non-goal).

### Doctor visibility

`doctor` surfaces informational checks. Embedding facets do not gate overall `ok` (the feature is opt-in). The `mcp_protocol` facet is also informational: `checks.mcp_protocol.ok=false` does **not** flip overall doctor `ok` (`run_doctor` sets overall `ok` from stdio-or-CLI startup only).
...
- `checks.mcp_protocol`: `{ok, sdk_version, latest_protocol_version, fastmcp_version, declared_fastmcp, note?}`. Reports the installed MCP SDK / FastMCP versions and the declared `fastmcp` pin (from installed `mcp-workbay-handoff` metadata, else the checkout `pyproject.toml`). `ok=false` when the declared pin cannot be read (`note` includes `declared_pin_unavailable`) or when the installed FastMCP version is outside the declared range. Missing optional version fields stay `null` and are named in `note`. Orchestrator doctor exposes the same facet at the top-level `mcp_protocol` key.
