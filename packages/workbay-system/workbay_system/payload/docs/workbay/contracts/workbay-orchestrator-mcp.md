...
| `manage_worktree_lane` | action/query  | varies     | Discriminated `upsert`, `update_routing`, `close`, `list`, and `retire` wrapper. `update_routing` changes only the routing fields and mirrors them into an existing lane manifest. `retire` reclaims one terminal lane worktree through the reaper's guarded helper. |
...
Lane-management state is dormant by design in normal operation. Empty lane rows, messages, reports, and briefs are not an orientation input; same-task pickup coordination is convention-only; there is no lane mutual exclusion.
...
Turn metrics are dormant by design in normal operation. Empty `turn_metrics` rows are expected and are not an orientation input until explicit telemetry collection is activated.
...
| `run_offload_pass`      | action        | no         | Runs one bounded synchronous lane pass. Accepts an optional `tier` and returns a terminal dispatch receipt whose execution fields are populated only from observed process evidence. |
| `dispatch_wave`         | action        | no         | Dispatches the admitted ready frontier with bounded width. Lane specs may carry `tier`; the selected tier is forwarded to each pass. |
| `offload_preflight`     | query/action  | yes        | Validates availability, entitlement, policy, routing, and admission without claiming that a worker process ran. |
| `list_available_backends` | query       | yes        | Reads registered backend catalog and, by default, probed availability fields. |
| `lane_dag`              | query         | yes        | Loads and validates one task manifest, then returns deterministic `ascii` and `json` DAG reports with the currently admitted ready-frontier width. It never prompts or mutates state. |

## Tiered Routing and DAG Dispatch

The persisted routing record is the five-field set `backend`, `model`,
`reasoning_effort`, `speed`, and `tier`; the first four fields are the routing
quad and `tier` is its named policy selector. `junior` and `senior` are the
public tier values. A tier-only request is valid and resolves the corresponding
entitled Codex model, default effort, and default speed. When callers also
supply explicit routing fields, every explicit value must agree with the tier
resolution. A disagreement returns `ok=false`, `outcome="routing_refused"`,
and typed `failure_class` and `reason` fields before dispatch.

`manage_worktree_lane(operation="update_routing")` changes only those five
routing columns; it does not reuse lane upsert defaults or modify status,
`test_cmd`, ownership, or other lane metadata. The database transaction and an
existing `config/lane-orchestration/<task_ref>.json` mirror are switched as one
operation with compensation on a later failure. The response is the final lane
row. `None` means the caller did not override that field; it is not an
instruction to erase a persisted value.

`manage_worktree_lane(operation="retire")` reclaims exactly one terminal lane
(`lane_id` + `task_ref` required). It is dry-run by default. A live lane is
refused. The path bundles first, then removes the worktree only through the
reaper's guarded helper, and deletes a merged branch with `git branch -d` only
when requested. A second identical call after success returns `already_retired`.

Workers resolve the authoritative lane-row routing at every cycle boundary,
before breaker admission, so an operator reroute can move work away from an
open backend without restarting the daemon. Manifests may assign `tier` per
lane. `lane_dag(task_ref)` validates that manifest and returns both ASCII and
JSON projections plus the admitted ready-frontier width. `dispatch_wave`
accepts the rendered ready members, preserves each lane's tier through
admission, and returns `wave_id`, `dispatched`, `deferred`, `refused`,
`wave_max_width`, and `stale_waves`.

## Dispatch Receipt Evidence

Agent specs carry separate `routing_requested` and `routing_effective` objects.
The requested object records caller intent; the effective object records the
route actually written for execution, including any supported effort or model
normalization. Terminal `run_offload_pass` results may include a
`dispatch_receipt` with `observed_backend`, `resolved_model`, `served_model`,
`tier`, `originating_tier`, `routing_effective`, `turns`, `rounds`,
`observed_tokens`, `tokens_complete`, `high_finding_count`, optional `escalated_from`, and
`cost_per_accepted_output`. Multi-route passes also carry `routing_history`:
ordered segments containing the exact `routing_effective`, turn count, and
observed token count for that route. `originating_tier` is the first observed
route, even when a later cycle changes the row without writing an escalation.

Execution facts are fail-closed. Preflight and other pre-spawn refusals leave
`resolved_model`, `served_model`, `gate_host`, `observed_backend`, `tier`, and
`routing_effective` null, with zero observed usage. Once a process is observed,
the receipt uses the effective route and observed response model; it does not
substitute requested routing for missing execution evidence. Escalated passes
retain the originating tier in `escalated_from`/`originating_tier` and aggregate
the pass-local turns, rounds, findings, and observed usage across the decision.
Failed and timed-out terminal pass ledgers are also aggregated by a later
explicit redispatch, whether or not an escalation decision exists. A matching
ledger with a missing or malformed `tokens`/`dispatch_receipt` object makes the
history incomplete; an accepted receipt then leaves
`cost_per_accepted_output=null` rather than presenting an authoritative zero.

For off-box execution, `raw_payload.source_provenance` is the fetched,
gate-authored source-to-sandbox receipt. It is present after materialization and
contains `schema_version`, `dispatch_nonce`, `requested_branch`,
`requested_ref`, `agent_spec_head_sha`, `dispatch_ref`,
`remote_source_commit`, `remote_source_tree`, `sandbox_base_commit`,
`sandbox_base_tree`, and `history_stripped=true`. The optional
`model_process_started=true` stamp is written only after agent-spec validation,
source/archive materialization, environment setup, and binary resolution; only
that fetched stamp authorizes `EXEC_SPAWNED`, `routing_effective`, and served
model attribution. A source receipt without it reports
`execution_failure="model_process_not_started"` and is not process evidence.

Remote source identity failures are terminal and non-retryable. The transport
returns exit 12 (`source_ref_missing`), 13 (`source_commit_mismatch`), 14
(`source_tree_mismatch`), 15 (`source_reserved_path_collision`), 16
(`source_sandbox_tree_mismatch`), or 17 (`source_history_not_descendant`). The
adapter exposes the exact value as `raw_payload.source_identity_failure` and as
a blocker. Once a patch or changed-file product exists, an absent, malformed,
oversized, identity-mismatched, or tree-mismatched source receipt is likewise
terminal with `raw_payload.source_provenance_failure`, `retryable=false`, and a
`source_provenance_<failure>` blocker.

Daemon-generated routing-switch audits use decision ids beginning
`worker_routing_switch:` and are explicitly persisted with
`decision_origin="system"`; they are operational history, not agent-authored
judgments.
...
| Tool                | Surface class | Idempotent | Notes                                                                                                                                |
| ------------------- | ------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `get_metrics_summary` | generator  | yes        | Derived metrics snapshot across lanes, retrieval, context pressure, process-health signals, and handoff-memory health. Optional `tier="junior"|"senior"` adds matching persisted dispatch receipts. |
...
When `tier` is omitted, the snapshot is byte-shape compatible with the legacy
ACE snapshot: no dispatch-receipt keys are added. When `tier="junior"` or
`tier="senior"` is supplied, the response additionally contains:

- `dispatch_receipts`: matching terminal `offload-pass-*.json` receipt rows for
  this task. Each row is privacy-bounded to `pass_id`, `lane_id`, `outcome`,
  `wall_seconds`, `tier`, `turns`, `rounds`, `cost_per_accepted_output`, and
  optional `escalated_from`. Prompts, raw transport payloads, result tails, and
  findings are not projected from pass state.
- `dispatch_receipt_filter`: `{ "tier": <tier>, "count": <matching rows> }`.

The filter reads the existing terminal offload-pass receipt ledger. It creates
no table, file, or writer. An unreadable ledger row fails with a typed metrics
error instead of silently undercounting calibration evidence. Values other than
`junior` and `senior` are refused as `invalid_tier`.
...
`turn_metrics` is dormant by design in normal operation. An empty ledger is expected, not a signal about current work, and it is not an orientation input until a task explicitly enables turn telemetry collection.
...
All execution backends MUST implement the `BackendAdapter` protocol defined in `packages/mcp-workbay-orchestrator/src/workbay_orchestrator_mcp/orchestration/backend_adapter.py`. This ensures consistent handling of `execute()` and `resolve_reasoning_effort()` across Codex, Claude, Cursor, and local models. The `BACKENDS` spec table that maps each registry key to its adapter lives separately, in `orchestration/backend_registry.py`.
