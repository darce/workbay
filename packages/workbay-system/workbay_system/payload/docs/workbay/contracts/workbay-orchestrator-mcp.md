...
| `manage_worktree_lane` | action/query  | varies     | Discriminated `upsert`, `update_routing`, `close`, and `list` wrapper. `update_routing` changes only the routing fields and mirrors them into an existing lane manifest. |
