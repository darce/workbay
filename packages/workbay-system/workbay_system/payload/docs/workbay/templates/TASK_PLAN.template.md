...
>
> A plan that cannot be handed to a junior agent and implemented from the text alone is not implementation-ready. `plan-draft` and `planning-review` enforce this standard.
...
> **Machine-checked work-item block (implementation note S1).** When present, this is the
> sole input to `orchestration/plan_decomposer.py`. Prose, inline backticks, and
> the Files tables elsewhere in the plan are documentation-only and are **never**
> scraped. Omit this section for legacy single-lane plans that still use the
> compiler's `--operator-items` / findings path.
>
> **Grammar (fail-closed):** first fenced ` ```json ` block under this heading.
>
> ```
> {
...
>     {
>       "id": "s1",                        // ^s[0-9]+$ , unique
>       "title": "short summary",          // non-empty string → work-item summary
>       "independent": true,               // bool; forced-declaration (see below)
>       "owns": ["path/to/file.py"],       // non-empty repo-relative paths
>       "depends": [],                     // subset of item ids (exact-id only)
>       "blast": "none",                   // "none" | "full"
>       "anchors": ["symbol_name"]         // required non-empty when blast=="full"
>     }
>   ]
> }
> ```
>
> **Forced declaration:** `independent:false` with empty `depends` refuses;
> `independent:true` with non-empty `depends` refuses. Two `independent:true`
> items whose `owns` share an exact root or parent/child prefix also refuse
> (remedy: declare depends, fuse owns, or rely on S2 colour/conflict_edges for
> residual prefix collisions at compile time).
>
> **Emission:** each item becomes a compiler operator work item
> `{id, kind:"operator", file_paths:owns, summary:title, depends_on:depends}`;
> ids are emitted sorted. Compose with the compiler via
> `python -m …plan_decomposer --plan-path <md> --stdout | … --operator-items -`.
...
```json
{
  ...
  "items": [
    {
      "id": "s1",
      "title": "plan decomposer",
      "independent": true,
      "owns": [
        "packages/mcp-workbay-orchestrator/src/workbay_orchestrator_mcp/orchestration/plan_decomposer.py",
        "packages/mcp-workbay-orchestrator/tests/test_plan_decomposer.py"
      ],
      "depends": [],
      "blast": "none"
    },
    {
      "id": "s2",
      "title": "wave-side consume",
      "independent": false,
      "owns": [
        "packages/mcp-workbay-orchestrator/src/workbay_orchestrator_mcp/orchestration/wave_dispatch.py"
      ],
      "depends": ["s1"],
      "blast": "none"
    }
...
> Prefer the machine-checked `## Decomposition` JSON block above when lanes will
> be compiled through `plan_decomposer` → `work_graph_compiler`.
