...
# package at ``scripts/workbay/lifecycle/``. Path resolution is relative
...
#     resolves to ``packages/workbay-system/scripts/workbay/lifecycle``.
#   * Consumer layout (post-bootstrap ``--profile lifecycle``): hoisted
#     to ``Makefile.d/lifecycle.mk`` at consumer root with a sibling
#     ``scripts/workbay/lifecycle/`` directory; the runner resolves to
...
LIFECYCLE_RUNNER    := $(abspath $(LIFECYCLE_MK_DIR)../scripts/workbay/lifecycle)
LIFECYCLE_PYTHON    ?= python3
LIFECYCLE           := $(LIFECYCLE_PYTHON) $(LIFECYCLE_RUNNER)

.PHONY: task-start task-finish task-reap finalize-plan plan-done context slice-start slice-commit review-ready close-check \
