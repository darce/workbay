...
#
# The launcher token defaults to the `python3 -m workbay_handoff_mcp.plan_cli`
# module entry, resolved from the handoff package that bootstrap installs
# alongside the MCP server. Git-only delivery launches that server through the
# `mcp_launch.py` shim in .mcp.json (which execs the git-installed / workspace
# `.venv` console — never a per-session `uvx` PyPI resolve). Override
# `WORKBAY_HANDOFF_PLAN_CLI` if the consumer manages its own venv.
#
...
WORKBAY_HANDOFF_PLAN_CLI ?= python3 -m workbay_handoff_mcp.plan_cli
