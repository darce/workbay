...
# Defer the installed-tool validation until a generator recipe expands this
# variable. The eval replaces the recursive definition with its first result,
# so later references in the same or another recipe reuse the probe outcome.
define _WORKBAY_PROBE_INSTALLED_TOOL_PYTHON
$(eval _WORKBAY_INSTALLED_TOOL_PYTHON := $(shell \
  cand="$(WORKBAY_TOOL_PYTHON)"; \
  if [ -n "$$cand" ] && $$cand -c 'import workbay_system, workbay_protocol' >/dev/null 2>&1; then \
    echo "$$cand"; \
  fi))$(_WORKBAY_INSTALLED_TOOL_PYTHON)
endef
_WORKBAY_INSTALLED_TOOL_PYTHON = $(call _WORKBAY_PROBE_INSTALLED_TOOL_PYTHON)
...
WORKFLOW_GENERATOR = $(if $(_WORKBAY_SYSTEM_PYPROJECT),uv run --project "$(_WORKBAY_SYSTEM_PYPROJECT:%/pyproject.toml=%)" workbay-overlay-tooling generate-agent-workflows,$(if $(_WORKBAY_INSTALLED_TOOL_PYTHON),$(_WORKBAY_INSTALLED_TOOL_PYTHON) -m workbay_system.overlay_tooling.cli generate-agent-workflows,$(WORKFLOWS_PYTHON) $(_WORKFLOW_GENERATOR_SCRIPT)))
