...
_WORKBAY_SYSTEM_PYPROJECT := $(wildcard $(abspath $(WORKFLOWS_MK_DIR)../..)/pyproject.toml)
_WORKFLOW_GENERATOR_SCRIPT := $(WORKFLOWS_ROOT)/scripts/generate_agent_workflows.py
ifneq ($(_WORKBAY_SYSTEM_PYPROJECT),)
WORKFLOW_GENERATOR := uv run --project "$(abspath $(WORKFLOWS_MK_DIR)../..)" workbay-overlay-tooling generate-agent-workflows
else
