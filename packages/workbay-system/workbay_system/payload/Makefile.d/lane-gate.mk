...
def _worktree_state():
    """Return a byte-exact Git status snapshot, or None outside a worktree."""
    try:
        state = subprocess.run(
            ["git", "-C", subject, "status", "--porcelain=v1", "-z", "--untracked-files=all"],
        ...
        return None
    return state.stdout if state.returncode == 0 else None
...
worktree_before = _worktree_state()
...
worktree_after = _worktree_state()
if worktree_before is not None and worktree_after != worktree_before:
    _overlap_advisory()
