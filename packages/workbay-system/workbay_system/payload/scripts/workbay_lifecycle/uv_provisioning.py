...
def _strip_toml_trailing_comment(line: str) -> str:
    """Drop a trailing ``#`` comment, ignoring ``#`` inside simple quotes."""
    in_single = False
    in_double = False
    escape = False
...
def _workspace_enabled_from_parsed(data: object) -> bool:
    """True only when ``tool.uv.workspace`` is a TOML table (mapping).

    Fail-open: wrong value types and unexpected containers return False.
    """
    try:
        ...
        # ``[tool.uv.workspace]`` is a table; scalar/array/false are not.
        return isinstance(workspace, dict)
    except (TypeError, AttributeError, LookupError):
        return False


def _workspace_enabled_text_scan(text: str) -> bool:
    """Approximate the parse arm when ``tomllib`` is unavailable.

    Line scan: strip trailing comments, match a section header whose inner
    text (internal whitespace removed) equals ``tool.uv.workspace``, and
    detect ``workspace = { ... }`` while inside ``[tool.uv]``.

    Residual divergence vs the parse arm: dotted-key table construction
    such as ``workspace.members = [...]`` under ``[tool.uv]`` without an
    explicit ``[tool.uv.workspace]`` header or ``workspace = {`` inline
    table — the parse arm yields a mapping; this scan returns False.
    """
    in_tool_uv = False
    for raw_line in text.splitlines():
        stripped = _strip_toml_trailing_comment(raw_line).strip()
        if not stripped:
            continue
        if stripped.startswith("[") and stripped.endswith("]"):
            compact = "".join(stripped[1:-1].split())
            if compact == "tool.uv.workspace":
                return True
            in_tool_uv = compact == "tool.uv"
            continue
        if not in_tool_uv or "=" not in stripped:
            continue
        key, _, rest = stripped.partition("=")
        if key.strip() == "workspace" and rest.lstrip().startswith("{"):
            return True
    return False
...
def workspace_enabled(worktree_root: Path) -> bool:
    ...
    Fail-open on ambiguity: missing, unreadable, or unparseable ``pyproject.toml``
    returns False. Callers that hard-block on a declared workspace (e.g. the
    dev-install redirect heal) must not raise out of this predicate.
    """
    ...
    if tomllib is not None:
        try:
            ...
            return False
        return _workspace_enabled_from_parsed(data)
    # tomllib unavailable (older runtimes): line-oriented scan (see helper).
    return _workspace_enabled_text_scan(text)
...
def _heal_dev_editables(
    ...
    if not dev_install.is_file():
        # Uv-workspace monorepo roots expect dev editables: missing script is
        # fail-closed. Non-workspace trees (no root ``[tool.uv.workspace]``)
        # keep the silent skip — ``packages/`` alone is not a monorepo signal.
