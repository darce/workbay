...
from . import _common
from . import project_events_replay
...
# Bounded foreground reclaim at task-start (RES-07): keep the wall-clock and
# entry budgets small so a huge orphan spool cannot stall every procurement.
_TASK_START_DRAIN_MAX_ENTRIES = 50
_TASK_START_DRAIN_MAX_SECONDS = 5.0

# Pure-skip reasons that deliberately suppress the projection write path.
# Enumerated from every skipped_reason drain_spool can emit (see
# project_events_replay.drain_spool / _drain):
#   projection_breaker_open  — retry-storm breaker open; forbids project writes
#   projection_replay_locked — another drain holds the lock; no reclaim work
#   handoff_cli_unreachable  — mid-drain abort (may have partial reclaim)
#   dead_letter_sink_full    — mid-drain abort (may have partial reclaim)
#   max_entries_exhausted    — budget stop (typically drained > 0)
#   max_seconds_exceeded     — time stop (typically drained > 0)
#   None                     — completed / empty claim / no skip
# Only the first two are pure pre-reclaim suppressions of projection work.
# Unreadable counters on those paths must still be *reported* on both
# surfaces, but must not re-authorize project_state_sync (REVF-SPOOL2-01).
_PROJECTION_SUPPRESSING_SKIP_REASONS = frozenset(
    {
        "projection_breaker_open",
        "projection_replay_locked",
    }
)


def _draining_snapshot_is_orphan(path: Path) -> bool:
    """True when a ``.draining.<pid>-<ts>`` claim is dead or unparseable.

    Delegates to ``projection_queue.draining_snapshot_is_orphan`` so status,
    doctor, and task-start share one pid-aware definition (OBS-08).
    """
    return projection_queue.draining_snapshot_is_orphan(path)


def _has_orphaned_draining_claim(repo: Path) -> bool:
    """Pid-aware orphan probe (aligned with spool_health.has_orphan_draining)."""
    return projection_queue.has_orphaned_draining_claim(repo)


def _format_drain_error(exc: Exception) -> str:
    """Build a single-token drain_failed= value (type + sanitized message).

    The pending line is space-delimited ``key=value`` fields. Return exactly
    one token: collapse every whitespace run to ``_``, replace ``=`` with
    ``:``, strip C0/C1 controls and zero-width characters (so ESC/NUL cannot
    reposition a terminal cursor), then cap at 120 chars. A hostile message
    cannot inject fields or wrap/recolor the stderr line (OBS-08, TEST-06).
    """
    exc_type = type(exc).__name__
    # Never-raise: a hostile __str__ must not escape the drain handler's
    # except Exception arm and abort task-start (R5C-SANITIZER-RERAISES-...).
    try:
        message = str(exc)
    except Exception:
        message = ""
    if message:
        value = f"{exc_type}: {message}"
    else:
        value = exc_type
    # Single token: no spaces and no '=' (immune to positional and key=value
    # last-wins parsers of the space-delimited pending line).
    value = "_".join(value.split())
    value = value.replace("=", ":")
    # Strip C0 (U+0000–U+001F), DEL, C1 (U+0080–U+009F), and common
    # zero-width code points that survive str.split() but still affect a
    # terminal reading this stderr line (R5C-SANITIZER-PASSES-C0-...).
    value = "".join(
        ch
        for ch in value
        if not (
            ...
            or ord(ch) == 0x7F
            or 0x80 <= ord(ch) <= 0x9F
            or ord(ch) in (0x200B, 0x200C, 0x200D, 0xFEFF)
        )
    )
    return value[:120]


def _parse_strict_count(value: object) -> tuple[int | None, bool]:
    """Type-strict non-negative drain counter parse.

    Accept only a real integer: ``isinstance(v, int) and not isinstance(v, bool)``
    and ``v >= 0``. Booleans are ``int`` subclasses and must be rejected.
    Strings, floats, lists, dicts, ``None``, and negatives are unparseable —
    never coerce via ``int()`` (which accepts ``"0"``, ``True``, ``2.7``, …).

    Returns ``(parsed, unparseable)``. On unparseable, ``parsed`` is ``None``
    (callers map that to drained=null+flag / remaining=null+flag — never a
    fabricated confident 0). Integer ``0`` is legal; there is no distinct int
    ``-0`` in Python (``-0 is 0``).
    """
...
def _parse_strict_bool(value: object) -> tuple[bool | None, bool]:
    """Type-strict bool parse for the floor bit.

    Accept only a real ``bool`` (``True`` / ``False``). Integers, strings,
    lists, dicts, and ``None`` are unparseable — never coerce via ``bool()``
    (which maps ``0``/``""``/``[]``/``None`` to confident ``False`` and any
    non-empty string such as ``"false"`` to confident ``True``).
...
def _publish_drain_value(
    value: object,
    parse_completed: bool,
    *,
    unparseable: bool | None = None,
) -> object | None:
    """Publish a drain counter/bit only when a trustworthy value exists.

    Both the JSON receipt and the human stderr line must call this helper so
    they cannot disagree about confidence:

    * never-read (``parse_completed`` false) → ``None``
    * completed-unreadable (``unparseable`` true) → ``None``
    * completed-readable → ``value`` (may itself be ``None`` for optional
      fields such as ``skipped_reason`` with no skip)
    """
...
def _human_drained_token(
    drained_n: int | None,
    *,
    parse_completed: bool,
    unparseable: bool | None,
) -> str | None:
    """Human ``drained=`` token, or ``None`` to omit the field.

    Derives from the shared publisher so incomplete / unreadable magnitudes
    cannot diverge from the receipt. Completed-unreadable is special-cased
    only because the human grammar emits ``unknown`` where the publisher
    returns ``None`` (omit vs unknown).
    """
    if parse_completed and unparseable:
        # Completed determination — emit even under drain_failed= (L-05).
        return "unknown"
    published = _publish_drain_value(
        drained_n, parse_completed, unparseable=unparseable
    )
...
def _human_remaining_token(
    pending_remaining: int | None,
    *,
    parse_completed: bool,
    unparseable: bool | None,
    capped: bool | None,
    capped_parse_completed: bool,
    capped_unparseable: bool | None = None,
) -> str | None:
    """Human ``pending_remaining=`` token, or ``None`` to omit the field.

    Floor forms: ``N+`` only when the floor bit finished true; exact ``N`` only
    when it finished false. When the magnitude finished readable but the floor
    is unread or completed-unreadable, emit the decimal magnitude (rule b) and
    require the caller to pair it with ``pending_remaining_capped=unread`` or
    ``=unreadable`` so the line does not claim exact-or-floor. Magnitude
    derives from the shared publisher; floor forms layer on top.
    """
    if parse_completed and unparseable:
        return "unknown"
    published = _publish_drain_value(
        pending_remaining, parse_completed, unparseable=unparseable
    )
    if published is None:
        return None
    if (not capped_parse_completed) or capped_unparseable:
        # Third form: remaining known, floor unread/unreadable — magnitude
        # without N/N+ claim (companion capped=unread|unreadable on the line).
        return str(published)
    capped_pub = _publish_drain_value(
        capped, capped_parse_completed, unparseable=capped_unparseable
    )
    if capped_pub:
        return f"{published}+"
    return str(published)
...
def _human_floor_companion(
    *,
    remaining_parse_completed: bool,
    remaining_unparseable: bool | None,
    pending_remaining: int | None,
    capped_parse_completed: bool,
    capped_unparseable: bool | None,
) -> str | None:
    """Human ``pending_remaining_capped=`` companion, or ``None`` to omit.

    Emitted only for the third form (remaining readable, floor unread or
    completed-unreadable) so operators can tell magnitude-known/floor-unknown
    from honest exact ``N``. Tokens mirror the receipt's three-state floor
    pair: ``unread`` when the floor get never finished (capped_parse_completed
    false → receipt null/null); ``unreadable`` when the get completed but the
    value is not a real bool (receipt null/true). Never collapse those two.
    """
    if (
        not remaining_parse_completed
        or remaining_unparseable
        or pending_remaining is None
    ):
        return None
    if not capped_parse_completed:
        return "unread"
    if capped_unparseable:
        return "unreadable"
    return None
...
def _read_workspace_summary_view(repo: Path) -> _common.WorkspaceSummaryView:
    ...
    CLI unavailability without a typed result, malformed render output, and a
    zero-exit envelope without ``ok is True`` degrade to ``shape="none"``. A
    non-zero render exit carrying a typed error fails closed with
    :class:`_common.WorkspaceSummaryRenderError`.
    """
...
def _adopt_overlay_command(*, worktree_path: Path | None = None) -> list[str]:
    ...
    if raw is None:
        if worktree_path is not None:
            local_script = _common._venv_console_script(
                worktree_path, "workbay-bootstrap"
            )
...
def run(argv: list[str]) -> int:
    ...
    except _common.WorkspaceSummaryRenderError as exc:
        return _emit_error(str(exc), task_ref=task_ref, branch=branch)
    ...
    if plan_id:
        branch = resolver.format_branch_name(task_ref, slug=args.slug, plan_id=plan_id)
    ...
    # implementation note [DBG-05]/[OBS-08]: the ``-plan<NNNN>`` segment is the numbered
    # ``docs/plans/`` convention. When strong evidence resolves the task as
    # numbered-plan-backed (a ``docs/plans/`` path came from ``PLAN=`` or the
    # live row's ``task_plan_path``) but that path carries no extractable
    # ``<NNNN>`` id, refuse rather than silently mint a segment-less
    # plan-backed branch — the invariant "never mint ``feature/<task-ref>``
    # for a task we know is numbered-plan-backed" is enforced here where the
    # plan context exists; the remedy names ``PLAN=`` with a correctly-numbered
    # plan doc ([AGT-08] — rejection is a spec). Tasks backed by an unnumbered
    # per-package ``docs/tasks/`` task-plan doc (a distinct, older convention
    # that never carried a ``-plan<NNNN>`` segment) and non-plan tasks (no
    # resolvable plan path) never reach this branch, so their bare
    # ``feature/<task-ref>`` stays valid.
    if (
        plan_path_for_gate is not None
        and "docs/plans/" in plan_path_for_gate
        and plan_id is None
    ):
        remedy = (
            f"make task-start TASK={task_ref} "
            'PLAN=docs/plans/<NNNN>-*.md OBJECTIVE="..."'
        )
        return _emit_error(
            "plan_id_unresolved_for_plan_backed_task",
            task_ref=task_ref,
            branch=branch,
            ...
            safe_next_commands=[
                {
                    "command": remedy,
                    "reason": "plan_id_unresolved_for_plan_backed_task",
                }
            ],
        )
    ...
    if not _branch_matches_grammar(branch):
        ...
        if (
            ...
            and _branch_matches_grammar(suggested_branch)
        ):
        ...
        if baseline.baseline_status != "accepted":
            ...
            if (
                ...
                if draft_ok:
                    ...
                    safe_next_commands = (
                        baseline_receipt.get("safe_next_commands") or []
                    )
    ...
    if not root_venv.ok:
        ...
        if (
            "workspace uv sync" in root_venv.failure_reason
            or "per-package" in root_venv.failure_reason
        ):
    ...
    # worktree carries /wb-review-slice et al. Runs before the optional consumer
    ...
    # Deterministic orphan reclaim: mid-drain kills leave `.draining.*`
    # snapshots that the opportunistic auto-drain may never re-claim.
    # task-start is the foreground reconciliation point; bounded and
    # never-raise for exceptions (RES-07/AGT-10) — drain failure degrades
    # to the actionable line below (OBS-08: surface drain_failed=...).
    # BaseException subclasses (KeyboardInterrupt / SystemExit) still
    # propagate so operator interrupts are not swallowed. Gate on the
    # pid-aware orphan probe so a LIVE concurrent drainer's claim does
    # not trigger a competing foreground drain_spool.
    drain_error: str | None = None
    # Counters/bits start as None — a confident 0/False is unreachable until
    # the conversion that produces it finishes. Outer except Exception must
    # not leave initializer zeros/falses that both surfaces would publish.
    drain_ran: bool | None = None
    drain_skipped_reason: str | None = None
    # True only after drain_result.get("skipped_reason") returns (A-08).
    skipped_reason_parse_completed = False
    # Tri-state twin of drained_unparseable for the skip field (A-08 / R4-07):
    # True when get completed but the value is not a usable reason (empty str
    # or non-string); False when completed with None (honest no-skip) or a
    # non-empty str; null when the get never finished.
    skipped_reason_unparseable: bool | None = None
    drain_attempted = False
    drained_n: int | None = None
    drained_unparseable: bool | None = None
    remaining_unparseable: bool | None = None
    # True only after the type-strict parse for that counter completes.
    # Mid-raise before a parse must not treat default locals as readable
    # counter values on either surface (stale-zero / silent-missing).
    drained_parse_completed = False
    remaining_parse_completed = False
    capped_parse_completed = False
    pending_remaining: int | None = None
    # None until the type-strict floor parse completes. Never initialize False:
    # False is a genuine exact-floor reading and must not be the default.
    pending_remaining_capped: bool | None = None
    pending_remaining_capped_unparseable: bool | None = None
    try:
        if _has_orphaned_draining_claim(repo):
            drain_attempted = True
            drain_result = project_events_replay.drain_spool(
                repo,
                max_entries=_TASK_START_DRAIN_MAX_ENTRIES,
                max_seconds=_TASK_START_DRAIN_MAX_SECONDS,
            )
            # "ran" means reclaim work happened (drained > 0), independent of
            # skipped_reason. Partial reclaims pair drained>0 WITH a stop
            # reason (max_entries_exhausted, max_seconds_exceeded, ...).
            # Zero-work skip paths (breaker open, lock held, empty claim)
            # leave drained=0 so ran stays false. Completed-unreadable
            # drained leaves ran None (tri-state; never fabricates false).
            skipped = drain_result.get("skipped_reason")
            if isinstance(skipped, str) and skipped:
                drain_skipped_reason = skipped
                skipped_reason_unparseable = False
            elif skipped is None:
                # Finished no-skip — honest null, not unreadable.
                drain_skipped_reason = None
                skipped_reason_unparseable = False
            else:
                # Empty string or non-string: completed read, unusable reason.
                drain_skipped_reason = None
                skipped_reason_unparseable = True
            skipped_reason_parse_completed = True
            # Type-strict counter parse (R1/R3): only real non-negative ints.
            # int() coercion of foreign types is forbidden — "0"/True/2.7 must
            # not look healthy. Locals are assigned only after conversion
            # finishes: _parse_strict_count returns int subclasses unchanged,
            # so int() still invokes dunders that can raise. Avoid `or` on
            # the subclass (that invokes __bool__ before int()).
            drained_parsed, drained_is_unparseable = _parse_strict_count(
                drain_result.get("drained")
            )
            if drained_is_unparseable:
                # Completed-unreadable: no trustworthy count (A-02). Do not
                # fabricate 0 — that is byte-identical to genuine zero.
                drained_n = None
                drained_unparseable = True
                drain_ran = None
            else:
                # drained_parsed is a non-neg int (possibly a subclass).
                drained_n = int(drained_parsed)  # type: ignore[arg-type]
                drained_unparseable = False
                drain_ran = drained_n > 0
            drained_parse_completed = True
            # Same type rule for pending_remaining (R2/R3). Unparseable leaves
            # remaining as None (null ≠ empty 0) and sets remaining_unparseable
            # so counters_unparseable + the recompute gate see both counters.
            # Normalise with int() symmetrically with drained so a hostile
            # int subclass cannot disagree between stderr and receipt.
            remaining_parsed, remaining_is_unparseable = _parse_strict_count(
                drain_result.get("pending_remaining")
            )
            if remaining_is_unparseable:
                pending_remaining = None
                remaining_unparseable = True
            else:
                pending_remaining = int(remaining_parsed)  # type: ignore[arg-type]
                remaining_unparseable = False
            remaining_parse_completed = True
            # Floor bit: type-strict bool only (REVA-R6-01). Real True/False
            # only — never bool()-coerce strings/ints/None. Still read when
            # remaining is unparseable so a get-raise can surface as
            # drain_failed=; human N+/exact use the bit only when remaining
            # itself is readable (see _human_remaining_token).
            capped_parsed, capped_is_unparseable = _parse_strict_bool(
                drain_result.get("pending_remaining_capped")
            )
            if capped_is_unparseable:
                pending_remaining_capped = None
                pending_remaining_capped_unparseable = True
            else:
                pending_remaining_capped = capped_parsed
                pending_remaining_capped_unparseable = False
            capped_parse_completed = True
    except Exception as exc:
        drain_error = _format_drain_error(exc)

    # Recompute only when drain work may have changed projection state or when
    # counter honesty is unknown: entries reclaimed (drained_n > 0), a mid-drain
    # raise, or unparseable drain counters (drained and/or pending_remaining).
    # Pure skips with honest zero counters (breaker open, lock held, empty
    # claim) must not re-run project_state_sync — that shells out and can
    # double-tick the retry-storm breaker / re-spool a duplicate state_sync
    # row. Unreadable counters still open recompute *except* when
    # skipped_reason is a deliberate projection-path suppression
    # (_PROJECTION_SUPPRESSING_SKIP_REASONS): an unreadable counter must not
    # override the breaker/lock and authorize the write they exist to stop
    # (REVF-SPOOL2-01). Counters stay reported on both surfaces either way.
    # Pure-skip leaves the pre-drain status untouched (CARD-15 / F2):
    # spool absence is not evidence the handoff row landed, so we never upgrade
    # pending→synced from local FS shape here.
    #
    # A-03 dual (round 5): three-valued monotone aggregate.
    #   True  — either per-counter flag IS True (determined fact; do not
    #           wait for the other counter). TRUE OR UNKNOWN = TRUE.
    #   False — both parses completed and both flags are False.
    #   None  — otherwise (partial finish with no determined True; never a
    #           confident False from an unfinished second counter).
    # Direct three-branch: _publish_drain_value's parse_completed gate cannot
    # express "true wins early" without discarding a determined True.
    #
    # Recompute note (REVD-R4-05): bool(None)==bool(False) keeps the recompute
    # gate closed for the partial-False→None sub-row. The True→None sub-row
    # (drained unreadable, remaining unread) would change bool(True)≠bool(None),
    # but every such mid-raise path also sets drain_error non-null, which opens
    # recompute independently. Monotone True on that row is correct for the
    # aggregate and still leaves recompute count unchanged.
    if (drained_unparseable is True) or (remaining_unparseable is True):
        counters_unparseable = True
    elif drained_parse_completed and remaining_parse_completed:
        counters_unparseable = False
    else:
        counters_unparseable = None
    unparseable_opens_recompute = (
        bool(counters_unparseable)
        and drain_skipped_reason not in _PROJECTION_SUPPRESSING_SKIP_REASONS
    )
    if drain_attempted and (
        (drained_n is not None and drained_n > 0)
        or drain_error is not None
        or unparseable_opens_recompute
    ):
        status = projection.project_state_sync(
            repo,
            task_ref=task_ref,
    ...
    success_events.append("claimed_existing_worktree" if is_claim else "task_started")
    # Success-path receipt always emits spool_drain (OBS-08) so --json
    # consumers can distinguish clean / skip / failed reclaim without reading
    # the human stderr line. ok:false early exits never reach this block (the
    # worktree/branch do not exist yet), so those receipts omit the key.
    # Post-drain orphan probe is outside the drain try/except; guard it so a
    # raise here cannot abort task-start after the worktree exists (no receipt,
    # no telemetry). Degrade to unknown/None and still emit (RES-07).
    try:
        orphan_after: bool | None = _has_orphaned_draining_claim(repo)
    except Exception:
        orphan_after = None
    receipt = {
        "ok": True,
        ...
        "spool_drain": {
            # Publisher-gated keys (completion + unparseable flags, A-02):
            # ran, skipped_reason, skipped_reason_unparseable, drained,
            # pending_remaining, pending_remaining_capped,
            # pending_remaining_capped_unparseable, drained_unparseable,
            # pending_remaining_unparseable.
            # Direct assigns (own completion rule): failed (drain_error),
            # orphan_draining (post-drain probe), attempted (bool bookkeeping),
            # counters_unparseable (pre-reduced three-valued monotone aggregate).
            "ran": _publish_drain_value(
                drain_ran, drained_parse_completed, unparseable=drained_unparseable
            ),
            "failed": drain_error,
            "orphan_draining": orphan_after,
            # null + non-null failed after an attempt means unknown, not no-skip
            # (raise at the first drain_result.get can leave this unread).
            # A-08: skipped_reason_unparseable distinguishes never-read (null)
            # from finished-no-skip (false) and completed-unusable (true);
            # skipped_reason itself stays null for all three (L-09).
            "skipped_reason": _publish_drain_value(
                drain_skipped_reason, skipped_reason_parse_completed
            ),
            "skipped_reason_unparseable": _publish_drain_value(
                skipped_reason_unparseable, skipped_reason_parse_completed
            ),
            # Distinguish never-attempted / attempted-zero / reclaimed.
            "attempted": drain_attempted,
            "drained": _publish_drain_value(
                drained_n, drained_parse_completed, unparseable=drained_unparseable
            ),
            # Entries still parked after drain (including self-pid writebacks).
            # When remaining is readable but the floor bit is null, an exact
            # count means "count read, floor unknown" (contract A-01 pick).
            "pending_remaining": _publish_drain_value(
                pending_remaining,
                remaining_parse_completed,
                unparseable=remaining_unparseable,
            ),
            # Floor bit for pending_remaining (true → human N+; false → exact;
            # null → floor never finished or completed-unreadable). Machine-
            # readable twin of the human floor grammar so a floor of 5 and an
            # exact 5 are not byte-identical on the receipt.
            "pending_remaining_capped": _publish_drain_value(
                pending_remaining_capped,
                capped_parse_completed,
                unparseable=pending_remaining_capped_unparseable,
            ),
            "pending_remaining_capped_unparseable": _publish_drain_value(
                pending_remaining_capped_unparseable, capped_parse_completed
            ),
            # True when drained and/or pending_remaining failed the type-strict
            # non-negative-int rule — must not look like a healthy attempted-
            # zero drain (OBS-08). Monotone: True as soon as either per-counter
            # flag is True; False only when both parses completed readable;
            # null otherwise (A-03 dual).
            "counters_unparseable": counters_unparseable,
            # Per-counter attribution (REVD-SPOOL2-JSON-WHICH-COUNTER-AMBIGUOUS):
            # consumers must tell *which* counter was unreadable from the
            # receipt alone. Aggregate flag above stays "at least one".
            # null when that counter's parse never completed.
            "drained_unparseable": _publish_drain_value(
                drained_unparseable, drained_parse_completed
            ),
            "pending_remaining_unparseable": _publish_drain_value(
                remaining_unparseable, remaining_parse_completed
            ),
        },
    }
    ...
    if not args.emit_json:
        line = (
            f"task-start: task_ref={task_ref} branch={branch} mode={mode} "
            f"head={head[:12]} projection={status}"
        )
        # Depth/orphan/retain telemetry is gated on drain_attempted or
        # projection=pending — never on pending alone. A retain/skip that
        # leaves projection synced/spooled must still surface (R5-RETAIN-...).
        show_drain_telemetry = status == "pending" or drain_attempted
        if show_drain_telemetry:
            # OBS-08: bare projection=pending is silence-as-success; surface
            # depth, orphan distinction, and the drain command so an operator
            # can act without reading the spool by hand.
            # Depth probe is never-raise + capped: an OSError here must not
            # abort task-start after the worktree exists, and a huge spool
            # must not full-scan on the procurement path.
            depth_token = "unknown"
            try:
                depth_limit = project_events_replay.SPOOL_DEPTH_SUMMARY_LIMIT
                depth = project_events_replay.spool_depth(
                    repo, max_count=depth_limit + 1
                )
                if depth > depth_limit:
                    depth_token = f"{depth_limit}+"
                else:
                    depth_token = str(depth)
            except Exception:
                depth_token = "unknown"
            orphan = orphan_after
            retained = (
                pending_remaining is not None
                and pending_remaining > 0
                and drain_attempted
            )
            # Shared renderer with the receipt: helpers derive magnitude from
            # the publisher, then layer floor forms. Remaining known + floor
            # unread/unreadable emits the magnitude with a
            # capped=unread|unreadable companion (third form; rule b) instead
            # of omitting or claiming exact. remaining_display derives solely
            # from the token — never str(pending_remaining) (A-04).
            remaining_token = _human_remaining_token(
                pending_remaining,
                parse_completed=remaining_parse_completed,
                unparseable=remaining_unparseable,
                capped=pending_remaining_capped,
                capped_parse_completed=capped_parse_completed,
                capped_unparseable=pending_remaining_capped_unparseable,
            )
            floor_companion = _human_floor_companion(
                remaining_parse_completed=remaining_parse_completed,
                remaining_unparseable=remaining_unparseable,
                pending_remaining=pending_remaining,
                capped_parse_completed=capped_parse_completed,
                capped_unparseable=pending_remaining_capped_unparseable,
            )
            remaining_display = remaining_token
            floor_known = (
                capped_parse_completed and not pending_remaining_capped_unparseable
            )

            def _retained_prose() -> str:
                # A-04: prose may name a count only when the kv token states
                # it. Rule (b): when the magnitude finished readable, state
                # it even if the floor is unknown — do not withhold a value
                # a completed read determined. retained requires
                # pending_remaining > 0, which forces a readable remaining
                # parse, so third-form always supplies remaining_display
                # (C-04). An absent display is an invariant break, not a
                # withhold path.
                if remaining_display is None:
                    raise AssertionError(
                        "retained with no remaining_display violates "
                        "third-form invariant (C-04)"
                    )
                if floor_known:
                    return f"{remaining_display} entries retained"
                return f"{remaining_display} entries retained (floor unknown)"

            # Unreadable counters win the prose slot (REVE/REVD nuance pair):
            # the contract promises "spool counters unreadable". Orphan stays
            # visible via orphan_draining= always. Retained is NOT fully
            # carried by kv alone when unreadable wins: pending_remaining=
            # publishes the third-form magnitude when remaining finished
            # readable (rule b), but the retain *prose* would otherwise
            # vanish entirely (REVB-R6-03). Compose retain evidence alongside
            # the unreadable nuance rather than letting one slot suppress the
            # other. Probe failure (orphan is None) degrades to unknown, not
            # true/false, in the kv field only.
            if drain_attempted and counters_unparseable:
                nuance = "spool counters unreadable"
                if retained:
                    nuance = f"{nuance}; {_retained_prose()}"
            elif orphan is True:
                nuance = "a prior claim is orphaned"
            elif retained:
                nuance = _retained_prose()
            elif status == "pending":
                nuance = "queued, will reconcile"
            else:
                nuance = None
            if orphan is None:
                orphan_token = "unknown"
            else:
                orphan_token = str(orphan).lower()
            line += f" spool_depth={depth_token} orphan_draining={orphan_token}"
            # Counter tokens on the human line (R4 / REVF-SPOOL2-02).
            # Shared _human_* helpers derive from the publisher so this
            # surface cannot invent a confident token the receipt would null.
            # Completed unreadable determinations emit unknown even when a
            # later raise set drain_error (L-05). Incomplete parses stay
            # silent. Aggregate token is clean-path only (S-04): under
            # drain_failed= the omission carries no aggregate meaning; on
            # the clean path it is two-state (true / false) — null aggregate
            # only arises with incomplete parse, which always sets
            # drain_error and therefore leaves the clean path (REVE-R6-06).
            if drain_attempted:
                drained_token = _human_drained_token(
                    drained_n,
                    parse_completed=drained_parse_completed,
                    unparseable=drained_unparseable,
                )
                if drained_token is not None:
                    line += f" drained={drained_token}"
                if remaining_token is not None:
                    line += f" pending_remaining={remaining_token}"
                if floor_companion is not None:
                    line += f" pending_remaining_capped={floor_companion}"
                if drain_error is None:
                    if counters_unparseable is True:
                        line += " counters_unparseable=true"
                    elif counters_unparseable is False:
                        line += " counters_unparseable=false"
            # skipped_reason distinguishes breaker/lock/unreachable remediations
            # that otherwise collapse to the same retained prose (R5D-SKIPREASON-...).
            # Human surface mirrors the receipt's three rows with distinct
            # tokens: usable reason → skipped_reason=<token>; completed-
            # unusable → skipped_reason=unreadable; never-read after an
            # attempt → skipped_reason=unread; honest no-skip omits the field
            # (C-02 / REVB-R6-01 / REVE-R6-07). Do not collapse the two
            # degraded arms back to a single unknown.
            if drain_skipped_reason:
                line += f" skipped_reason={drain_skipped_reason}"
            elif drain_attempted and skipped_reason_unparseable is True:
                line += " skipped_reason=unreadable"
            elif drain_attempted and not skipped_reason_parse_completed:
                line += " skipped_reason=unread"
            # All structured kv fields before any prose segment (R5C-PENDING-LINE-...).
            # drain_failed= is independent of projection status: a raise that
            # leaves status synced/spooled must still surface on the default
            # human path (OBS-08). Value is a single token (see _format_drain_error).
            if drain_error is not None:
                line += f" drain_failed={drain_error}"
            if nuance is not None:
                line += f"; {nuance}"
            # Breaker-open refuses replay; do not advise the refused command.
            # Gate on the skip tri-state: completed-unusable or never-read
            # after an attempt may encode a breaker-open stop — suppress
            # remediation rather than treating collapsed None as not-breaker.
            # Both degraded arms must stay in this expression (REVB-R6-01);
            # dropping either re-advises refused replay on that row.
            suppress_remediation = (
                drain_skipped_reason == "projection_breaker_open"
                or skipped_reason_unparseable is True
                or (drain_attempted and not skipped_reason_parse_completed)
            )
            if not suppress_remediation:
                line += "; run `make project-events-replay`"
        elif drain_error is not None:
            # Drain failed with no pending/attempted telemetry arm (e.g. sync
            # path that still raised before attempt bookkeeping). Keep the
            # failed token visible.
            line += f" drain_failed={drain_error}"
