    ...
    * ``failed`` — a git step errored; detail is the message.
    ...
    proc = _common.run_subprocess(
        [
            ...
            "commit",
            "-m",
            f"chore(plan): finalize checklist ticks for {task_ref}",
            "--",
            str(plan_path),
        ]
    )
    if proc.returncode != 0:
        return (
            "failed",
            f"git commit: {(proc.stderr or proc.stdout or '').strip()[:200]}",
        )
...
def run(argv: list[str]) -> int:
    ...
    if repo is None:
        _common.emit(
            {"ok": False, "command": "finalize-plan", "error": "not_in_git_repo"}
        )
        return 2
    ...
    if not task_ref:
        try:
            ...
            _common.emit({"ok": False, "command": "finalize-plan", "error": str(exc)})
            return 2
        if view.shape == "single" and view.task_ref:
            task_ref = view.task_ref
    if not task_ref:
        _common.emit(
            {"ok": False, "command": "finalize-plan", "error": "task_ref_required"}
        )
        return 2
    ...
    if not sync.get("ok"):
        ...
        warnings.append(
            f"checklist_sync_failed: {sync.get('warning') or 'sync_not_ok'}"
        )
