...
DEFAULT_TASK_REAP_TIMEOUT = 120.0
    ...
    *,
    ...
    try:
        ...
        proc = run_handoff_subprocess(
            repo, argv, timeout=AGENT_ERRORS_REPLAY_TIMEOUT
        )
...
def maybe_reap_stale_dev_temp(
    ...
    *,
    ...
    """Best-effort ``/tmp/workbay-*`` age-out (internal / [RES-07]).
    ...
    raises, never fails the host lifecycle command. ``repo`` is unused
    (global temp scan) but kept for call-site parity with sibling reclaimers.

    When ``warnings`` is provided, each degradation path appends the same
    cause string written to stderr so receipt consumers can see why the
    sub-step failed without reading stderr [OBS-08][AGT-10].
    """
    del repo  # global /tmp scan; signature mirrors sibling maybe_* reclaimers
...
def maybe_replay_terminal_guard_spool(
    ...
    *,
    ...
    try:
        ...
        proc = run_handoff_subprocess(
            repo, argv, timeout=TERMINAL_GUARD_REPLAY_TIMEOUT
        )
    except Exception as exc:  # noqa: BLE001 — never-raise host contract
        _degrade(f"{type(exc).__name__}: {exc}")
        return None
...
def task_reap_timeout() -> float:
    ...
    classifier walks every live registry row. A ~130-row sweep already
    takes ~6.12s, which races the 6s ``DEFAULT_HANDOFF_TIMEOUT`` inherited
    by :func:`run_handoff_json` when no timeout is supplied — converting a
    successful CLI into ``reap_unavailable`` (timeout, exit 2) on every
    invocation. The default is therefore generous (120s) so production
    still bounds a hung CLI without racing a healthy sweep. Env-overridable
    via ``WORKBAY_TASK_REAP_TIMEOUT`` (seconds) for tests and operators;
    invalid, empty, non-finite, non-positive, or oversized values fall
    back to ``DEFAULT_TASK_REAP_TIMEOUT``.
    """
    return _parse_positive_finite_timeout(
...
def derive_workspace_summary_view(repo: Path) -> WorkspaceSummaryView:
    ...
    - Envelope ``ok`` is not True: ``shape="none"`` (a parseable body
      under ``ok=false`` is still a failed render, not a live view).
    ...
    if not isinstance(envelope, dict):
        return _NONE_VIEW
    # Fail-open to none when the render envelope reports failure even if
    # current_task_json is still parseable [OBS-08][AGT-10].
