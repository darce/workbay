...
def _probe_dirty_main_protected_paths(repo: Path) -> list[str]:
    ...
    - ``find_dirty_protected_paths`` returns ``DirtyProtectedDegraded``
      (git status probe timed out or exited non-zero).

    Degraded probe results intentionally fall through to "no dirty paths"
    rather than blocking the gate or inventing a synthetic dirty path —
    the operator gets the explicit doctor facet for diagnosis. This is
    deliberately softer than the PreToolUse guards, which fail closed on
    ...
    if result is None:
        return []
    # Degraded probe: not proven dirty. Do not surface a synthetic path
    # and do not block close-check; doctor owns operator-facing diagnosis.
