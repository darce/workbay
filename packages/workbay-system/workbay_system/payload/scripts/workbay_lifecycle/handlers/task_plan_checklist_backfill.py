...
def _backfill_one_plan(
    ...
    try:
        with (
            sync_handler.checklist_write_lock(plan_path)
            if apply_changes
            else nullcontext()
        ):
            try:
            ...
            diff_entries = sync_handler._diff_preview(
                original_text, rewritten, resolutions
            )
...
def run(argv: list[str]) -> int:
    ...
    for task_ref in tasks:
        ...
        if not _TASK_REF_RE.match(ref):
            ...
            continue
        discovered, sources = audit_handler.discover_plans_for_task(workspace_root, ref)
        if not discovered:
            rows.append(
                {
                    "task_ref": ref,
