...
def run(argv: list[str]) -> int:
    ...
    result = uv_provisioning.provision_worktree_env(
        worktree,
    ...
    # Structured heal subprocess receipt (returncode/stdout/stderr) when the
    # provision path ran scripts/dev_install.py. Kept alongside the flat
    # stderr-append for backwards-compatible diagnosis.
    if result.dev_install is not None:
        receipt["dev_install"] = result.dev_install
    ...
    # The dev-install src-redirect heal now runs inside provision_worktree_env
    # (_heal_dev_editables), so a heal failure already surfaces here as
    # result.ok=False / failure_reason='dev_install_failed' with the failing
    # dev_install output on result.stderr and the structured ``dev_install``
    # receipt object. Running dev_install a second time here was redundant —
    # the heal has a single home now.
    _common.emit(receipt)
