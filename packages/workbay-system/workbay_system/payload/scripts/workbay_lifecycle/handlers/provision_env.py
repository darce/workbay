...
def run(argv: list[str]) -> int:
    ...
    if result.ok and result.created:
        ...
        dev_install = worktree / "scripts" / "dev_install.py"
        if dev_install.is_file():
            ...
            receipt["dev_install"] = {
                ...
                "stdout": dev_proc.stdout.strip(),
                "stderr": dev_proc.stderr.strip(),
            }
            if dev_proc.returncode != 0:
                receipt["ok"] = False
                receipt["failure_reason"] = "dev_install_failed"
                _common.emit(receipt)
                return 2
    _common.emit(receipt)
