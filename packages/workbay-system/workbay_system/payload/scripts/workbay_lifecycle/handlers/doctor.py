        ...
        if target_path in drifted_paths:
            ...
            if opt_in_flag:
                remediation.append(
                    ...
                    f"`workbay apply-hooks {opt_in_flag}`"
                )
...
def _probe_codemap_freshness(repo: Path) -> DoctorCodemap:
    ...
    if snap.stale:
        return DoctorCodemap(
            ...
            cli_path=snap.cli_path,
            note=snap.note
            or codemap_freshness_probe.reindex_remedy_note(repo),
        )
    if not snap.confirmed:
        return DoctorCodemap(
            ...
            note=snap.note
            or (
                "CLI reported ready but supplied no indexed commit to compare"
            ),
        )
...
def run(argv: list[str]) -> int:
    ...
    if not args.emit_json:
        ...
        if codemap_freshness.status == "unconfigured":
            codemap_line = f"codemap_freshness: unconfigured (skip) — {codemap_freshness.note}"
