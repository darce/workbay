...
import codemap_freshness_probe
...
REMEDIATION_PROBE_DEGRADED = [
    "git status probe degraded — could not determine cleanliness of protected paths",
    "run `git status` by hand, then re-run `make doctor` once git is healthy",
]
...
def _probe_with_retry(
    ...
    *,
    ...
    for attempt_index in range(len(_PROBE_BACKOFF_SLEEPS) + 1):
        ...
        latencies[latency_key] = round((time.monotonic() - started) * 1000.0, 1)
        # Only an ok=true dict counts as reachable; ok=false must stay in the
        # retry loop and surface last_warning as unreachable [OBS-08][AGT-10].
        if (
            payload is not None
    ...
    *,
    ...
    for hook_id, adapter in _load_hook_adapter_decls():
        ...
        if target_path in drifted_paths:
            ...
            remediation.append(
                f"repair the {key} {noun} via "
                f"`workbay repair --hooks recorded` "
                f"(leave this drift with "
                f"`workbay repair --hooks none`)"
            )
            ...
            remediation.append(
                ...
                f"Claude Code ignores) via `workbay repair --hooks recorded` or "
                f"`workbay apply-hooks {adapter.get('opt_in_flag', '')}`"
            )
        else:
            ...
            if opt_in_flag:
                remediation.append(
                    f"install the {key} {noun} via `workbay apply-hooks {opt_in_flag}`"
                )
    ...
    if hooks_path is not None:
        ...
        if drift:
            remediation.append(
                "re-run `workbay install --target <repo>` to hoist the missing git-hook scripts"
            )
...
def _probe_dirty_main(
    ...
    try:
        from _branch_isolation_guard import (  # noqa: PLC0415
    ...
    # Degraded probe: cleanliness unknown. Do not invent a synthetic dirty
    # path or tell the operator to stash a file that does not exist.
    if isinstance(result, DirtyProtectedDegraded):
        return (
            DoctorDirtyMain(
                branch=branch_name,
                protected_paths_dirty=[],
                mode_recommended="warn",
                remediation=REMEDIATION_PROBE_DEGRADED,
                ownership_hint=None,
            ),
            warnings,
        )
...
def _query_live_handoff_rows(repo: Path) -> tuple[list[dict[str, Any]], bool]:
    ...
    This drives the cross-task facets. ``ok=False`` signals an MCP outage so
    the doctor surfaces ``available=False`` instead of an empty-clean state.
    """
...
def _integration_target_branches(repo: Path) -> set[str]:
    ...
    integration = (proc.stdout or "").strip()
    branches = {"main", "master"}
    if integration:
        branches.update({integration, f"origin/{integration}"})
    return branches


def _probe_maint_worktree_collision(
    repo: Path,
) -> tuple[DoctorMaintWorktreeCollision, list[ReceiptWarning]]:
    """Find MAINT rows that resolve to the same worktree identity."""
    warnings: list[ReceiptWarning] = []
    rows, ok = _query_live_handoff_rows(repo)
    if not ok:
        note = "handoff-rows query failed"
        warnings.append(ReceiptWarning(field="maint_worktree_collision", reason=note))
        return (
            DoctorMaintWorktreeCollision(
                available=False,
                status="unavailable",
    ...
    integration_branches = _integration_target_branches(repo)
    primary_worktree = resolver.canonical_workspace_root(repo) or repo
    candidates_by_worktree: dict[str, set[str]] = {}
    for row in rows:
        task_ref = str(row.get("task_ref") or "").strip()
        if not task_ref.startswith("MAINT-"):
            continue
        if str(row.get("target_branch") or "").strip() not in integration_branches:
            continue
        raw_worktree = str(row.get("target_worktree_path") or "").strip()
        worktree = Path(raw_worktree).expanduser() if raw_worktree else primary_worktree
        if raw_worktree and not worktree.is_dir():
            identity = "<unresolvable-worktree>"
        else:
            try:
                identity = os.path.normcase(str(worktree.resolve()))
            except (OSError, RuntimeError, ValueError):
                identity = "<unresolvable-worktree>"
        candidates_by_worktree.setdefault(identity, set()).add(task_ref)

    candidates = sorted(
        task_ref
        for task_refs in candidates_by_worktree.values()
        if len(task_refs) > 1
        for task_ref in task_refs
    )
    if len(candidates) < 2:
        return DoctorMaintWorktreeCollision(
            note="no MAINT worktree collision"
        ), warnings

    note = (
        "multiple live MAINT rows compete for runtime worktree resolution: "
        f"{', '.join(candidates)}"
    )
    warnings.append(ReceiptWarning(field="maint_worktree_collision", reason=note))
    return (
        DoctorMaintWorktreeCollision(
            status="warning",
            severity="warning",
            colliding_task_refs=candidates,
            note=note,
        ),
        warnings,
    )
...
def _probe_codemap_freshness(repo: Path) -> DoctorCodemap:
    """implementation note S3 — offline-safe ``codemap_freshness`` facet.

    Unconfigured (CLI absent) → typed skip (severity ok, never error). CLI
    present but status unreadable → severity **warning** / available=False so a
    dead-but-installed probe is never silent ([OBS-08]; matches
    ``hook_failure_sink``). CLI answered without a comparable indexed sha →
    ``unconfirmed`` / warning (ready-without-evidence is not ``fresh``). Fresh
    → ok / available=True only when an indexed sha matched primary-repo HEAD
    and content drift was checkable (probe ``confirmed``). Stale → warning /
    available=True with a concrete reindex remedy in ``note`` — never error
    ([OBS-04]: staleness degrades search quality, does not break the repo).
    Never raises; never makes a network call.
    """
    snap = codemap_freshness_probe.probe_codemap_freshness(repo)
    if not snap.cli_present:
        return DoctorCodemap(
            available=False,
            stale=False,
            severity="ok",
            status="unconfigured",
            head_sha=snap.head_sha,
            indexed_sha=snap.indexed_sha,
            cli_path=snap.cli_path,
            note=snap.note or "codemap CLI not on PATH (optional; unconfigured)",
        )
    if not snap.available:
        return DoctorCodemap(
            available=False,
            ...
            indexed_sha=snap.indexed_sha,
            cli_path=snap.cli_path,
            note=snap.note or "codemap index_status unreadable",
        )
    if snap.stale:
        return DoctorCodemap(
            available=True,
            ...
            indexed_sha=snap.indexed_sha,
            cli_path=snap.cli_path,
            note=snap.note or codemap_freshness_probe.reindex_remedy_note(repo),
        )
    if not snap.confirmed:
        return DoctorCodemap(
            ...
            cli_path=snap.cli_path,
            note=snap.note
            or ("CLI reported ready but supplied no indexed commit to compare"),
        )
    # Fresh only when probe confirmed (sha match + content check when reached).
    return DoctorCodemap(
        available=True,
        stale=False,
        severity="ok",
...
def _probe_venv(repo: Path) -> tuple[DoctorVenv, list[ReceiptWarning]]:
    ...
    Also probes stale-findings via the handoff CLI. Envelope warnings are
    returned on the warning channel so a failing envelope cannot read as a
    clean zero backlog [OBS-08].
    """
    venv_warnings: list[ReceiptWarning] = []
    venv_dir = repo / ".venv"
    ...
    root_venv_handoff_present = venv_handoff.is_file() and os.access(
        venv_handoff, os.X_OK
    )
    ...
    if resolved_handoff is None:
        ...
        try:
            ...
            in_venv = resolved.is_relative_to(
                (venv_dir / "bin").resolve()
    ...
    stale_payload, stale_warning = _common.run_handoff_json(
        repo,
        argv=["collect-stale-findings"],
        timeout_seconds=_probe_timeout_seconds(),
        field="handoff.stale_findings",
    )
    stale_findings_count: int | None = 0
    ...
    if stale_warning is not None:
        # Surface envelope failure; never report a clean zero backlog while the
        # CLI reported not-ok / unavailable / malformed [OBS-08][AGT-10].
        venv_warnings.append(stale_warning)
        if stale_findings_count == 0:
            stale_findings_count = None
    projection_summary["stale_findings_count"] = stale_findings_count
    if spool_depth > 0:
        depth_label = (
            f"at least {spool_depth}" if spool_depth_capped else str(spool_depth)
        )
    ...
    if stale_findings_count is not None and stale_findings_count > 0:
        remediation.append(
...
def run(argv: list[str]) -> int:
    ...
    maint_worktree_collision, maint_worktree_collision_warnings = (
        _probe_maint_worktree_collision(repo)
    )
    warnings.extend(maint_worktree_collision_warnings)
    venv, venv_warnings = _probe_venv(repo)
    warnings.extend(venv_warnings)
    ...
    if hook_failure_sink.severity == "warning" or hook_failure_sink.status in (
        "warning",
        "unavailable",
    ):
        warnings.append(
    ...
    codemap_freshness = _probe_codemap_freshness(repo)
    if codemap_freshness.severity == "warning":
        warnings.append(
            ReceiptWarning(
                field="codemap_freshness",
                reason=f"{codemap_freshness.severity}: {codemap_freshness.note}",
            )
        )
    ...
    receipt_obj = DoctorReceipt(
        ...
        maint_worktree_collision=maint_worktree_collision,
        venv=venv,
    ...
    if not args.emit_json:
        ...
        if maint_worktree_collision.status == "warning":
            refs = ", ".join(maint_worktree_collision.colliding_task_refs)
            maint_worktree_collision_line = (
                f"maint_worktree_collision: warning task_refs=[{refs}]"
            )
        elif maint_worktree_collision.status == "unavailable":
            maint_worktree_collision_line = (
                "maint_worktree_collision: unavailable "
                f"({maint_worktree_collision.note})"
            )
        else:
            maint_worktree_collision_line = "maint_worktree_collision: clean"
        ...
        if codemap_freshness.status == "unconfigured":
            codemap_line = (
                f"codemap_freshness: unconfigured (skip) — {codemap_freshness.note}"
            )
        elif codemap_freshness.status == "unavailable":
            codemap_line = (
                f"codemap_freshness: warning unavailable ({codemap_freshness.note})"
            )
        elif codemap_freshness.status == "unconfirmed":
            codemap_line = (
                f"codemap_freshness: warning unconfirmed — {codemap_freshness.note}"
            )
        elif codemap_freshness.stale:
            # Parenthetical from probe note: content-drift can stale with
            # matching head/indexed shas, so head= + indexed= alone look broken.
            # Fallback covers an empty note without raising.
            detail = (codemap_freshness.note or "reindex required").strip()
            codemap_line = (
                f"codemap_freshness: warning stale "
                f"head={(codemap_freshness.head_sha or '')[:12]} "
                f"indexed={(codemap_freshness.indexed_sha or '')[:12]} "
                f"({detail})"
            )
        else:
            # Parenthetical derives from the probe note so content-check claims
            # stay accurate (probe may report "content verified (no drift)").
            # Fallback covers an empty probe note without asserting the opposite
            # of what fresh guarantees: reaching status=fresh means the probe
            # confirmed a clean detect_changes content check, so the fallback
            # must not claim "no content check".
            detail = (codemap_freshness.note or "sha match; content verified").strip()
            codemap_line = (
                f"codemap_freshness: ok fresh "
                f"head={(codemap_freshness.head_sha or '')[:12]} "
                f"indexed={(codemap_freshness.indexed_sha or '')[:12]} "
                f"({detail})"
            )
        sys.stderr.write(
            ...
            f"{baseline_line}\n"
            f"{maint_worktree_collision_line}\n"
            f"{venv_line}\n"
            f"{host_memory_line}\n"
