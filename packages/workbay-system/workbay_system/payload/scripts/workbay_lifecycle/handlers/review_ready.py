...
_REASON_OWNER: dict[str, str] = {
    ...
    "findings_unknown": "handoff_evidence",
    ...
    "unattested_acceptance_boxes": "checklist_sync",
    "plan_baseline_missing": "planning_baseline",
    "plan_baseline_unknown": "planning_baseline",
    "plan_baseline_stale_vs_source": "planning_baseline",
    "plan_id_collision_requires_reconcile": "planning_baseline",
    "plan_id_missing_from_branch": "planning_baseline",
...
def _next_command_template(
    *,
    ...
    if planning:
        # implementation note [AGT-08]: a plan-backed branch missing its -plan<NNNN>
        # segment is a torn provenance link. The remedy is a re-tear, not a
        # baseline accept, so surface it before the accept/reconcile hints —
        # re-tearing with PLAN= also re-establishes the baseline.
        if "plan_id_missing_from_branch" in planning:
            return {
                "command": (
                    "# plan-backed branch is missing its -plan<NNNN> segment: "
                    "rename to feature/<task-ref>-plan<NNNN> (retear via "
                    "make task-finish TASK=<task-ref> then make task-start "
                    "TASK=<task-ref> PLAN=docs/plans/<NNNN>-*.md), then re-run "
                    "make review-ready"
                ),
                "reason": "plan_id_missing_from_branch",
            }
...
def _query_open_findings(
    ...
    if isinstance(payload, dict) and payload.get("ok") is False:
        return dict(ZERO_FINDINGS), False
    # [RLSE-05]/[OBS-08]: measured findings always carry a dict
    # data.counts.severity envelope; non-dict / missing data is
    # unrecognized input, not a fabricated clean zero.
    data = payload.get("data") if isinstance(payload, dict) else None
    if not isinstance(data, dict):
        return dict(ZERO_FINDINGS), False
    severity = data.get("counts", {}).get("severity", {})
    counts = {
...
def _commit_tree_sha(repo: Path, rev: str) -> str | None:
    """Return the tree object sha for ``rev``, or None if unresolvable.

    Fail closed: ``_run_git`` returning None, non-zero exit, or empty
    stdout all mean the rev is not a known object in this repo.
    """
    proc = resolver._run_git(repo, "rev-parse", f"{rev}^{{tree}}")
    if proc is None or proc.returncode != 0:
        return None
...
def _evidence_tree_matches_head(repo: Path, evidence_sha: str, head: str) -> bool:
    """True iff evidence and HEAD point at the same tree (byte-identical sources).

    Fail closed: if either tree cannot be resolved, return False so the
    caller treats the evidence as stale. Comparing two Nones as equal
    would let an unknown evidence sha pass the gate.
    """
    evidence_tree = _commit_tree_sha(repo, evidence_sha)
    head_tree = _commit_tree_sha(repo, head)
    if evidence_tree is None or head_tree is None:
        return False
    return evidence_tree == head_tree
...
def _probe_unattested_attestations(
    repo: Path, task_ref: str | None
) -> tuple[list[str], str | None]:
    if not task_ref:
        return [], None
    try:
        return sync_handler.probe_unattested_attestations(repo, task_ref)
    except Exception:
        return [], None
...
def _material_gates_need_task_ref(repo: Path, task_ref: str) -> bool:
    ...
    if not findings_ok:
        return True
    if findings_open["high"] > 0 or findings_open["medium"] > 0:
        return True
    ...
    if identity is not None:
        ...
        if plan_path:
            ...
            if status.baseline_status in ("missing", "unknown", "stale"):
                return True
...
def augment_with_handoff_state(
    ...
    * :func:`_query_latest_passing_test_sha` — appends
      ...
      verified_test's ``commit_sha`` is missing, or when its tree
      differs from HEAD's tree (same commit sha is always fresh;
      tree-identical commits keep evidence valid so a no-op merge
      does not force a suite re-run).
    * :func:`_query_active_task_identity` + :func:`evaluate_plan_baseline` —
    ...
    ``findings_open`` is None when the findings query could not be
    measured. ``handoff_projection`` is ``"synced"`` only when *all*
    queries succeed; if any fails, callers see ``"pending"`` to flag
    ...
    if findings_ok:
        ...
        reasons.append("findings_unknown")
        findings_open = None

    latest_sha, evidence_ok = _query_latest_passing_test_sha(repo, task_ref)
    if evidence_ok and latest_sha != head:
        # Evidence attests to the *bytes tested*, not the commit object.
        # A no-op merge / empty commit creates a new sha over an identical
        # tree and must not invalidate valid suite evidence. Fail closed
        # when either tree is unresolvable (unknown sha, git error) —
        # never treat two failed lookups as "trees match".
        if latest_sha is None or not _evidence_tree_matches_head(
            repo, latest_sha, head
        ):
    ...
    if not identity_ok and mcp_reachable:
        ...
        target_branch = identity.get("target_branch")
        if plan_path:
            # implementation note [OBS-08]: a NUMBERED-plan-backed task (strong evidence —
            # the live row's ``task_plan_path`` is a ``docs/plans/<NNNN>-*.md``
            # doc) MUST carry its ``-plan<NNNN>`` branch segment. When the
            # segment is absent the plan->branch provenance link silently died;
            # break loudly rather than let a segment-less plan-backed branch
            # read as healthy. The ``-plan<NNNN>`` convention is specific to the
            # numbered ``docs/plans/`` home, so tasks backed by an unnumbered
            # per-package ``docs/tasks/`` task-plan doc (a distinct, older
            # convention) are exempt — same scoping as the ``task-start``
            # fail-fast. The segment stays optional at the grammar layer
            # (TASK_REF_RE unchanged); enforcement lives here where task context
            # exists. Reuses ``_plan_id_from_branch`` — no second parser ([REF-12]).
            branch = resolver.current_branch(repo) or ""
            if (
                "docs/plans/" in plan_path
                and branch not in PROTECTED_BASES
                and _plan_id_from_branch(branch) is None
            ):
                reasons.append("plan_id_missing_from_branch")
            status = evaluate_plan_baseline(
                repo,
            ...
            if status.baseline_status == "missing":
                ...
                reasons.append("plan_baseline_unknown")
            elif status.baseline_status == "stale":
                # 7036 follow-through ([RLSE-05]): a stale plan on main (content
                # differs from the reviewed source blob) is NOT accepted. The
                # plan-accept gate already blocks this; review-ready/close-check
                # must too, else a content-divergent plan stays merge-ready
                # without re-accept. Routes to make plan-accept via the
                # planning_baseline owner bucket.
                reasons.append("plan_baseline_stale_vs_source")
...
def run(argv: list[str]) -> int:
    ...
    missing_attest, attest_plan = _probe_unattested_attestations(repo, derived_task_ref)
    if missing_attest:
        warnings.append(
            f"unattested_acceptance_boxes: missing={missing_attest} plan={attest_plan}"
        )
    if pending_count is not None and pending_count > 0:
        warnings.append(
