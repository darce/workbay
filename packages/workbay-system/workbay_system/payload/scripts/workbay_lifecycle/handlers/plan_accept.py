...
def run(argv: list[str]) -> int:
    ...
    # already, or just applied via ``--local``), reconcile a task whose
    # branch/worktree still lack the ``-plan<NNNN>`` suffix in this one
    # invocation. No-op when the suffix is already correct.
    if (
