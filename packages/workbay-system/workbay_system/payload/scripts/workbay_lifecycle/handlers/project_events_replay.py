...
def _count_claim_entries(path: Path) -> int:
    """Count durable entries in a claim/checkpoint path (best-effort exact)."""
    return sum(1 for _ in projection_queue.iter_jsonl_entries(path))


def _pending_remaining_from_spool(
    repo: Path, *, max_count: int
) -> tuple[int, bool]:
    """Return ``(floor_count, capped)`` for a bounded live-spool depth probe.

    When the probe saturates at ``max_count``, the honest floor for ``N+``
    rendering is ``max_count`` itself for a 1-entry probe, or ``max_count - 1``
    when the probe uses the standard ``limit + 1`` convention (same grammar as
    task-start ``spool_depth=N+``).
    """
    raw = spool_depth(repo, max_count=max_count)
    if max_count <= 0:
        return raw, False
    if raw < max_count:
        return raw, False
    # Saturated: report the largest known floor, not the probe's stop value.
    floor = max_count if max_count == 1 else max_count - 1
    return floor, True


def _sibling_claim_entry_count(
    claimed_files: list[Path], claim_index: int
) -> int:
    """Sum entries in claims after ``claim_index`` (unprocessed this pass)."""
    total = 0
    for orphan in claimed_files[claim_index + 1 :]:
        total += _count_claim_entries(orphan)
    return total
...
def _drain_receipt(
    *,
    ...
    compacted: int = 0,
    dead_lettered: int = 0,
    pending_remaining_capped: bool = False,
) -> dict[str, Any]:
    ...
    return {
        ...
        "pending_remaining_capped": pending_remaining_capped,
...
def _drain(
    ...
    *,
    ...
    for claim_index, claim_path in enumerate(claimed_files):
        ...
        while index < len(entries):
            ...
            if status == "pending":
                ...
                # Include sibling claimed snapshots left unprocessed — same
                # accumulation as the budget/time stop path (R5-WRITEBACK-...).
                pending = len(remaining)
                if input_path is None:
                    pending += _sibling_claim_entry_count(
                        claimed_files, claim_index
                    )
            ...
            if attempts > max_attempts:
                ...
                if not parked:
                    ...
                    if input_path is None:
                        ...
                        _write_entries_atomic(destination, remaining)
                        checkpoint_written = str(destination)
                    # Sibling claims left unprocessed must not vanish from the
                    # pending floor (R5-WRITEBACK-ARMS-UNDERCOUNT-...).
                    pending = len(remaining)
                    if input_path is None:
                        pending += _sibling_claim_entry_count(
                            claimed_files, claim_index
    ...
    pending_remaining_capped = False
    if input_path is not None:
        ...
        pending_remaining, pending_remaining_capped = _pending_remaining_from_spool(
            repo, max_count=SPOOL_DEPTH_SUMMARY_LIMIT + 1
        )
        if checkpoint_written is not None:
            pending_remaining += _count_claim_entries(Path(checkpoint_written))
        ...
        for orphan in unprocessed_claims:
            pending_remaining += _count_claim_entries(orphan)
...
def drain_spool(
    ...
    *,
    ...
    if projection_queue.breaker_state(repo).state == "open":
        # max_count=1 is a presence floor: a reported 1 can mean thousands.
        depth, capped = _pending_remaining_from_spool(repo, max_count=1)
        return _drain_receipt(
            drained=0,
            ...
            pending_remaining_capped=capped,
        )
    ...
    with _drain_lock(repo) as acquired:
        if not acquired:
            depth, capped = _pending_remaining_from_spool(
                repo, max_count=SPOOL_DEPTH_SUMMARY_LIMIT + 1
            )
            return _drain_receipt(
                drained=0,
