"""``task-reap`` lifecycle subcommand (internal).

Shells out to ``mcp-workbay-handoff archive --operation reap`` and prints
the closeable / active / ambiguous buckets. Dry-run by default; pass
``--apply`` to close+archive closeable rows only.
"""

from __future__ import annotations

import argparse
import time
from typing import Any

from . import _common


def _flatten_handoff_payload(payload: dict[str, Any]) -> dict[str, Any]:
    if payload.get("schema_version") == 2:
        data = payload.get("data")
        if isinstance(data, dict):
            flat = {**payload, **data}
            scope = payload.get("scope")
            if isinstance(scope, dict) and "task_ref" not in flat and scope.get("task_ref"):
                flat["task_ref"] = scope["task_ref"]
            return flat
    return payload


def _print_plan_cursor_sweep(plan_cursors: dict[str, Any]) -> None:
    """Print internal stale plan-cursor sweep summary."""
    print("plan_cursors:")
    expired = plan_cursors.get("expired") or []
    would_expire = plan_cursors.get("would_expire") or []
    active = plan_cursors.get("active") or []
    ambiguous = plan_cursors.get("ambiguous") or []
    triage = plan_cursors.get("triage") or []
    print(f"  expired ({len(expired)}):")
    for entry in expired:
        if isinstance(entry, dict):
            print(
                f"    - id={entry.get('id')} {entry.get('task_ref')}/"
                f"{entry.get('plan_item_id')}: {entry.get('note') or entry.get('reason', '')}"
            )
    if not plan_cursors.get("applied"):
        print(f"  would_expire ({len(would_expire)}):")
        for entry in would_expire:
            if isinstance(entry, dict):
                print(
                    f"    - id={entry.get('id')} {entry.get('task_ref')}/"
                    f"{entry.get('plan_item_id')}: {entry.get('note') or entry.get('reason', '')}"
                )
    print(f"  active ({len(active)}):")
    for entry in active:
        if isinstance(entry, dict):
            print(
                f"    - id={entry.get('id')} {entry.get('task_ref')}/"
                f"{entry.get('plan_item_id')}: {entry.get('reason', '')}"
            )
    print(f"  ambiguous ({len(ambiguous)}):")
    for entry in ambiguous:
        if isinstance(entry, dict):
            print(
                f"    - id={entry.get('id')} {entry.get('task_ref')}/"
                f"{entry.get('plan_item_id')}: {entry.get('reason', '')}"
            )
    if triage:
        print(f"  triage ({len(triage)}):")
        for line in triage:
            print(f"    - {line}")


def _print_buckets(flat: dict[str, Any]) -> None:
    applied = flat.get("applied")
    if applied is not None:
        print(f"applied={applied}")
    reaped = flat.get("reaped") or []
    if reaped:
        print(f"reaped: {', '.join(reaped)}")
    failed = flat.get("failed") or []
    if failed:
        print(f"failed ({len(failed)}):")
        for entry in failed:
            if not isinstance(entry, dict):
                continue
            print(
                f"  - {entry.get('task_ref', '?')} "
                f"(stage={entry.get('stage', '?')}): {entry.get('error', '')}"
            )
    for bucket in ("closeable", "active", "ambiguous"):
        entries = flat.get(bucket) or []
        print(f"{bucket} ({len(entries)}):")
        for entry in entries:
            if not isinstance(entry, dict):
                continue
            ref = entry.get("task_ref", "?")
            reason = entry.get("reason", "")
            branch = entry.get("target_branch")
            suffix = f" branch={branch}" if branch else ""
            print(f"  - {ref}{suffix}: {reason}")
    plan_cursors = flat.get("plan_cursors")
    if isinstance(plan_cursors, dict):
        _print_plan_cursor_sweep(plan_cursors)


def run(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="lifecycle task-reap", add_help=True)
    parser.add_argument("--apply", action="store_true", default=False)
    parser.add_argument("--task", dest="task", default="")
    parser.add_argument("--json", dest="emit_json", action="store_true", default=False)
    args = parser.parse_args(argv)

    repo = _common.repo_root()
    if repo is None:
        _common.emit({"ok": False, "command": "task-reap", "error": "not_in_git_repo"})
        return 2

    handoff_argv = ["archive", "--operation", "reap"]
    if args.apply:
        handoff_argv.append("--apply")
    # NB: do NOT upper-case here. Unlike task-start/slice-start (which mint
    # canonical UPPER refs), task-reap *targets* pre-existing rows that may be
    # stored verbatim in lower-case (implementation note's motivating duplicates, e.g.
    # `ws-migdoctor-01-plan0043`). The classifier matches task_ref by exact,
    # case-sensitive equality, so upper-casing would make those rows
    # un-targetable by a scoped reap.
    task_ref = (args.task or "").strip()
    if task_ref:
        handoff_argv.extend(["--task-ref", task_ref])

    # internal: the sweep scales with registry size (~0.63 s/row
    # measured 2026-09-05), so every receipt carries the budget used and, on
    # success, the wall time, so operators can size WORKBAY_TASK_REAP_TIMEOUT
    # from evidence rather than guess [AGT-10, OBS-08].
    timeout_seconds = _common.task_reap_timeout()
    started = time.monotonic()
    payload, warning = _common.run_handoff_json(
        repo,
        argv=handoff_argv,
        timeout_seconds=timeout_seconds,
        field="reap",
    )
    elapsed_seconds = round(time.monotonic() - started, 2)
    if payload is None:
        receipt: dict[str, Any] = {
            "ok": False,
            "command": "task-reap",
            "error": "reap_unavailable",
            "timeout_seconds": timeout_seconds,
        }
        if warning is not None:
            receipt["warning"] = warning.__dict__
            if getattr(warning, "reason", None) == "timeout":
                receipt["hint"] = _common.task_reap_timeout_hint(timeout_seconds)
        _common.emit(receipt)
        return 2

    envelope_warning = _common.handoff_envelope_warning(
        payload, warning, field="reap"
    )
    if envelope_warning is not None:
        failed_receipt: dict[str, Any] = {
            "ok": False,
            "command": "task-reap",
            "error": "reap_invalid_envelope",
            "handoff": payload,
            "timeout_seconds": timeout_seconds,
            "elapsed_seconds": elapsed_seconds,
            "warning": envelope_warning.__dict__,
        }
        _common.emit(failed_receipt)
        return 2

    flat = _flatten_handoff_payload(payload)

    plan_cursors = flat.get("plan_cursors")
    receipt = {
        "ok": True,
        "command": "task-reap",
        "applied": flat.get("applied", False),
        "closeable": flat.get("closeable") or [],
        "active": flat.get("active") or [],
        "ambiguous": flat.get("ambiguous") or [],
        "reaped": flat.get("reaped") or [],
        "failed": flat.get("failed") or [],
        # internal: nested stale plan-cursor sweep (empty dict when absent).
        "plan_cursors": plan_cursors if isinstance(plan_cursors, dict) else {},
        # internal: budget sizing evidence for the operator.
        "timeout_seconds": timeout_seconds,
        "elapsed_seconds": elapsed_seconds,
    }
    if args.emit_json:
        _common.emit(receipt)
        return 0

    _print_buckets(flat)
    return 0
