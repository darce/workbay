...
def test_make_standing_gate_wires_review_pipeline_without_wildcards() -> None:
    ...
    assert '"$(_RP_PYTHON)" -m pytest' in makefile
    assert 'JUDGE_ROOT="$(_RP_JUDGE_ROOT)"' in makefile
    assert "$(_RP_JUDGE_ROOT)/scripts/assert_gate_interpreter.sh" in makefile
    assert 'PYTHONPATH="$(_RP_PYTHONPATH)"' in makefile
    assert (
        '"$(_RP_JUDGE_ROOT)/packages/workbay-system/workbay_system/payload/scripts/test_review_pipeline.py"'
        in makefile
    )
    assert re.search(
        r"^override _RP_PYTHON\s*:=\s*\$\(_RP_JUDGE_ROOT\)/\.venv/bin/python$",
        fragment,
        re.MULTILINE,
    )
