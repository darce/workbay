...
def _resolve_protocol_src() -> Path | None:
    """Locate workbay-protocol/src next to the handoff package in this monorepo.

    ``workbay_handoff_mcp.__init__`` imports ``workbay_protocol`` at module load.
    Bare-interpreter hooks that only put mcp-workbay-handoff/src on ``sys.path``
    collapse every probe into allowlisted ``handoff_unavailable`` when protocol
    is not installed as a site package [FIXB-FALLTHROUGH-LANDS-IN-ALLOWLISTED-FAILOPEN-01].
    """
    base = Path(__file__).resolve()
    # parents[2]/[3]: repo-root or shallow co-located layout (scripts/hooks).
    # parents[5]/[6]: payload nest at packages/.../payload/scripts/hooks — without
    # these, Path(__file__).resolve() (symlink-following) leaves protocol off
    # sys.path even after dogfood-link, and the import dies into allowlisted
    # handoff_unavailable [FIXB-FALLTHROUGH-LANDS-IN-ALLOWLISTED-FAILOPEN-01].
    # Skip parent indices that do not exist (Path.parents[n] raises IndexError
    # when n >= len(parents)). Eager tuple construction crashed shallow
    # checkouts at /repo or /srv/app [RELAND-R2-PROTOCOL-SRC-PARENTS-INDEXERROR-ON-SHALLOW-CHECKOUT-01].
    parent_count = len(base.parents)
    candidate_specs: tuple[tuple[int, tuple[str, ...]], ...] = (
        (2, ("packages", "workbay-protocol", "src")),
        (3, ("workbay-protocol", "src")),
        (5, ("workbay-protocol", "src")),
        (6, ("packages", "workbay-protocol", "src")),
    )
    candidates: list[Path] = []
    for index, parts in candidate_specs:
        if index >= parent_count:
            continue
        candidates.append(base.parents[index].joinpath(*parts))
    for candidate in candidates:
        if candidate.is_dir():
            return candidate
    return None
...
def _ensure_handoff_import_paths() -> None:
    """Insert monorepo src trees so bare python3 can import the handoff package."""
    protocol_src = _resolve_protocol_src()
    if protocol_src is not None:
        protocol_str = str(protocol_src)
...
_ensure_handoff_import_paths()

# CURRENT_TASK.json workspace-summary schema emitted by the handoff writer.
_SNAPSHOT_SCHEMA_VERSION = 2
# Bound on how long a content-fresh snapshot may be trusted [RES-08]. This is
# defence in depth, not the invalidation key: the identity fields this module
# reads are covered exactly by the handoff_state fingerprint, and the payload
# fields by the projection mtime, so a snapshot satisfying both terms is
# correct for everything read here no matter its age. The window covers only
# inputs those two terms cannot enumerate. It is deliberately generous: the
# snapshot is rewritten by handoff writes rather than by hook invocations, so
# a short window expires during any lull in write traffic and forces the
# handoff-package fallback this cache exists to avoid -- seconds of wall time
# under load, against a much tighter harness timeout.
_SNAPSHOT_MAX_STALENESS_SECONDS = 900.0
...
def _scan_workspace_ambiguous_tasks(
    ...
    fallback valve can stand down instead of blocking against a guessed
    row [OBS-01][D5].
    """
    try:
    ...
    if len(live_matches) == 1:
        ...
        # WORKBAY_GUARD_AMBIGUITY_FALLBACK disables the valve [OBS-01][D5].
        note: str | None = None
...
def _handoff_content_revision(db_path: Path) -> str | None:
    ...
    update, and a delete. Opened read-only via a file URI with ``mode=ro``
    and a small timeout; stdlib ``sqlite3`` only — hooks must never import
    the handoff package [RES-08][OBS-08].

    Counterpart: workbay_handoff_mcp.current_task_rendering._handoff_content_revision
    — both copies must define the fingerprint identically. Hooks cannot import
    ...
    try:
        try:
            row = con.execute(
                "SELECT COUNT(*), COALESCE(SUM(revision), 0), MAX(updated_at) "
                "FROM handoff_state"
            ).fetchone()
...
def _snapshot_fresh_enough(snapshot_path: Path, payload: dict[str, Any]) -> bool:
    ...
    Consistency model [DATA-01][RES-08]: when the payload carries a non-empty
    string ``authority_content_revision``, freshness requires all three of:
    (1) the observed ``handoff_state`` fingerprint equals the stamp (observed
    None is not-fresh [OBS-08]); (2) the snapshot mtime is at least as new as
    the newest projection JSON under ``authority_projection_dir`` (missing dir
    or no JSON files impose no constraint; unstattable projection contents
    answer not-fresh [OBS-08]); (3) the snapshot is no older than
    ``_SNAPSHOT_MAX_STALENESS_SECONDS`` (bounds inputs the fingerprint cannot
    enumerate, e.g. live compaction_advisory). handoff.db / -wal / -shm mtimes
    never participate in this branch: peers and the read-only fingerprint probe
    itself advance those sidecars without content change [RES-08].

    When the stamp is absent (legacy snapshots written before this field),
    keep the mtime rule exactly as before: snapshot mtime against the newest
    mtime among the db, -wal/-shm sidecars, and projection JSON files named by
    ``authority_db_path`` + ``authority_projection_dir``. No subprocess, no
    package import. When either authority field is missing/empty/non-string,
    the authority is newer than the snapshot, or mtimes cannot be determined,
    return False so the caller falls through to the handoff probe
    ...
    """
    stamped = payload.get("authority_content_revision")
    if isinstance(stamped, str) and stamped:
        db_raw = payload.get("authority_db_path")
        if not isinstance(db_raw, str) or not db_raw:
            return False
        observed = _handoff_content_revision(Path(db_raw))
        if observed is None or observed != stamped:
            return False
        try:
            snap_mtime = snapshot_path.stat().st_mtime
        except OSError:
            return False
        if (time.time() - snap_mtime) > _SNAPSHOT_MAX_STALENESS_SECONDS:
            return False
        proj_raw = payload.get("authority_projection_dir")
        if isinstance(proj_raw, str) and proj_raw:
            projection_dir = Path(proj_raw)
            try:
                if projection_dir.is_dir():
                    proj_mtimes: list[float] = []
                    for entry in projection_dir.iterdir():
                        if not entry.is_file() or entry.suffix != ".json":
                            continue
                        try:
                            proj_mtimes.append(entry.stat().st_mtime)
                        except FileNotFoundError:
                            continue
                        except OSError:
                            return False
                    if proj_mtimes and snap_mtime < max(proj_mtimes):
                        return False
            except OSError:
                return False
        return True

    # Legacy path: no stamp → mtime rule (sidecars included) unchanged.
    try:
        snap_mtime = snapshot_path.stat().st_mtime
    except OSError:
        return False
...
def _load_active_task_via_handoff(workspace_root: Path) -> ActiveTaskContext:
    ...
    try:
        ...
        # sections-only signature only for that overload mismatch. Do not wrap
        # the sections-only call: a genuine TypeError from identity must
        # propagate, not be swallowed and re-issued.
        if task_ref_arg:
            try:
        ...
        if primary is None:
            primary = str(Path(workspace_root).resolve(strict=False))
        # Split transient DB lock/busy from generic probe failure so the drift
        # guard can allowlist only the environmental case. With the protocol
        # sys.path leg, bare python3 now reaches the real probe; a locked DB
        # must not hard-block every Edit/Write [CARD-10]. Unrecognised / other
        # exceptions stay handoff_probe_failed (still blocking) [AGT-10].
        probe_error = "handoff_probe_failed"
        if isinstance(exc, sqlite3.OperationalError):
            detail = str(exc).lower()
            if "locked" in detail or "busy" in detail:
                probe_error = "handoff_probe_locked"
        return ActiveTaskContext(
            task_ref=None,
            target_worktree=None,
            target_branch=None,
    ...
    if isinstance(parsed, dict) and parsed.get("ok") is False:
        ...
        if isinstance(error, str) and (
            ...
            # Return a probe_error token rather than raise: evaluate_payload
            # would convert ValueError into an unconditional block, and
            # ambiguous active task is a recurring recoverable state (make
            # task-reap) that must not hard-block every Edit/Write [CARD-10].
            return ActiveTaskContext(
                task_ref=None,
                target_worktree=None,
                target_branch=None,
                primary_worktree=str(Path(runtime.workspace_root).resolve(strict=False)),
                probe_error="handoff_probe_ambiguous",
                resolution_note=error,
            )
