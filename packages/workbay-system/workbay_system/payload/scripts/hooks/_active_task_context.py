...
def _scan_workspace_ambiguous_tasks(
    ...
    fallback valve stays reachable [OBS-01][D5].
    """
    try:
    ...
    if len(live_matches) == 1:
        ...
        return _context_from_task_entry(
            entry, primary, resolution_note=note
        )
    if terminal_matches:
...
def _load_active_task_via_handoff(workspace_root: Path) -> ActiveTaskContext:
    ...
    try:
        ...
        try:
            from workbay_handoff_mcp.shared_primitives import resolve_active_task_ref_for_hook
