...
def is_metadata_dist_src(path: str) -> bool:
    """True when ``path`` matches the installed-distribution resolution branch.

    :func:`resolve_agent_handoff_src` has three branches: (1) installed dist via
    ``importlib.metadata``, (2) in-tree ``packages/mcp-workbay-handoff/src``,
    (3) overlay clone under ``.workbay/remote``. Only branch (1) is bound to the
    *resolving* interpreter's site-packages. Cross-interpreter subprocess callers
    must suppress PYTHONPATH injection of a metadata-dist path when the
    executing interpreter differs from the resolver (foreign ``.venv`` / uv-tool
    python), while still injecting repo-relative results under any executor.

    Signature stays a pure predicate over a path string so in-process callers
    (``os.path.isdir(src_path)``) and the existing ``str`` pinning assertions
    remain untouched. Returns False on empty path, missing dist, or any probe
    failure.
    """
    if not path:
        return False
    try:
        ...
        if located is None or not os.path.isdir(str(located)):
            return False
        meta_src = os.path.dirname(str(located))
        return os.path.abspath(path) == os.path.abspath(meta_src)
    except Exception:  # noqa: BLE001
        return False
...
def resolve_agent_handoff_src(repo_root: str) -> str:
    ...
    Installed distributions win first for **same-interpreter** use (the
    resolving interpreter will also import from the returned path). Cross-
    interpreter callers must consult :func:`is_metadata_dist_src` before
    prepending the result to a foreign executor's ``PYTHONPATH``. In the
    package-source repo, in-tree ``packages/mcp-workbay-handoff/src`` wins over
    the managed overlay clone; consumer repos keep overlay-preferred order.
    The overlay probe is module-aware: a candidate resolves only when its
    ``src`` actually exposes ``workbay_handoff_mcp``, so an empty/partial clone
    falls through rather than resolving a path the hook import cannot satisfy.
    """
    try:
        from importlib import metadata as importlib_metadata
