...
def test_reinject_notify_claude_emits_json_envelope(workspace: Path) -> None:
    ...
    assert block.startswith("```workbay-reinject")
    assert compaction_id in block
    assert envelope["systemMessage"].startswith("workbay: reinject source=compact")
...
def test_reinject_semantic_embeddings_import_error_degrades_with_telemetry(
    ...
    class _BlockEmbeddings:
        def find_spec(self, fullname, path, target=None):  # noqa: ANN001
            if fullname == "workbay_handoff_mcp.embeddings" or fullname.startswith(
                "workbay_handoff_mcp.embeddings."
            ):
    ...
    purged = [
        ...
        if key == "workbay_handoff_mcp.embeddings"
        or key.startswith("workbay_handoff_mcp.embeddings.")
    ]
    for key in purged:
        del sys.modules[key]
