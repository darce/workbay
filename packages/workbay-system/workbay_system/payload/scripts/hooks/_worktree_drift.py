...
def evaluate_payload(
    ...
    *,
    ...
    primary_worktree = _canonical_target_worktree(context.primary_worktree) or _primary_workspace_root(
        root
    )
    target_worktree = (
        _canonical_target_worktree(context.target_worktree) if context.target_worktree else None
    )
    ...
    # implementation note ambiguity fallback: the resolver guessed the active row from
    # multiple live candidates (resolution_note carries the tiebreak note). We
    # deliberately do NOT evaluate candidate-path drift against a guessed row —
    # a mis-picked row would block legitimate edits belonging to another task —
    # so the guard allows with outcome="fallback". This block sits AFTER the
    # MAINT / ALT_ALLOW / root-worktree-on-non-main guards on purpose: those
    # checks key on the live branch + primary-worktree identity, not on the
    # guessed task row, so the ambiguity rationale does not exempt them.
    # Emitting the fallback earlier would let an ambiguous multi-row DB slip a
    # root-worktree feature-branch edit through (the regression this ordering
    # fixes).
...
def _build_probe_error_block_reason(probe_error: str) -> str:
    ...
    return (
        f"UnresolvedTaskContextError: {probe_error}\n\n"
