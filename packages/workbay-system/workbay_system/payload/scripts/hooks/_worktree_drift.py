...
# Probe errors that mean "no active task / no handoff system" — allow the edit.
# ``handoff_probe_over_budget`` is different: it labels a complete identity
# answer that was merely late, not an undetermined answer. Keep identity and
# evaluate drift as if the same answer had arrived within budget [RES-03].
#
# WIDENING DISCLOSURE (protocol sys.path leg): bare-python3 hooks can now import
# workbay_handoff_mcp and run the authoritative DB probe. Paths that previously
# short-circuited to allowlisted ``handoff_unavailable`` (import death on
# workbay_protocol) are now reachable. Transient environmental states that would
# otherwise hard-block every Edit/Write in the repo are allowlisted here:
#   - handoff_probe_locked: sqlite locked/busy from a concurrent writer
#   - handoff_probe_ambiguous: multi-row / no-active ambiguity (recover via
#     ``make task-reap``; recovery itself needs a working session)
# ``handoff_probe_failed`` and every unrecognised token still BLOCK so the
# fail-toward-the-safe-side intent survives [AGT-10][CARD-10].
_PROBE_ERRORS_ALLOW = frozenset(
    {
        "handoff_unavailable",
        "handoff_active_missing",
        "handoff_probe_over_budget",
        "handoff_probe_locked",
        "handoff_probe_ambiguous",
    }
)
...
def _payload_value(
    payload: dict[str, Any], snake_key: str, camel_key: str, default: Any = ""
) -> Any:
...
def _extract_candidate_paths(
    ...
    if not tool_name:
        ...
        return (
            [str(file_path)] if isinstance(file_path, str) and file_path.strip() else []
        )
...
def evaluate_payload(
    ...
    *,
    ...
    candidate_paths = _extract_candidate_paths(
        tool_name, tool_input, workspace_root=root
    )
    ...
    if isinstance(active_task, ActiveTaskContext):
        ...
        if primary_worktree is None:
            return DriftDecision(
                outcome="block",
                reason="UnresolvedTaskContextError: git_primary_unresolvable",
                primary_worktree=None,
                task_ref="(unresolved task context)",
            )
        if len(active_task) == 2:
            context = ActiveTaskContext(
                task_ref=active_task[0],
                target_worktree=active_task[1],
                target_branch=None,
    ...
    # Resolve primary/target early so documented escape hatches can fire even
    # when the active-task probe could not determine a target worktree. A
    # could-not-determine context has no target_worktree; the hatch checks and
    # the non-allowlisted probe_error gate must run BEFORE the no-target early
    # return or that return would silently permit (or skip hatches) [AGT-10].
    primary_worktree = _canonical_target_worktree(
        context.primary_worktree
    ) or _primary_workspace_root(root)
    target_worktree = (
        _canonical_target_worktree(context.target_worktree)
        if context.target_worktree
        else None
    )
    ...
    # Could-not-determine must not look like "no active task" [OBS-08][AGT-10].
    # Sits AFTER MAINT / env hatches and BEFORE the no-target early return so a
    # missing target cannot short-circuit this fail-closed gate into allow.
    if context.probe_error and context.probe_error not in _PROBE_ERRORS_ALLOW:
        return DriftDecision(
            outcome="block",
            reason=_build_probe_error_block_reason(context.probe_error),
            primary_worktree=primary_worktree,
            task_ref=context.task_ref or "(unresolved task context)",
        )
    ...
    # Ambiguity fallback: the resolver selected among several live candidates
    # (resolution_note carries the tiebreak note). Do not evaluate candidate-
    # path drift against a guessed row — a mis-picked row would block edits
    # that belong to another task — so the guard allows with outcome="fallback".
    # This return sits AFTER the MAINT bypass, the ALT_ALLOW hatch, the
    # non-allowlisted probe_error gate, and the root-worktree-on-non-main
    # guard: those checks key on the live branch and primary-worktree
    # identity, not on the guessed task row, so an ambiguity rationale must
    # not exempt them. Emitting the fallback earlier would let an ambiguous
    # multi-row DB slip a root-worktree feature-branch edit through (the
    # regression this ordering fixes).
    if context.resolution_note:
        return DriftDecision(
            outcome="fallback",
            reason=context.resolution_note,
            primary_worktree=primary_worktree,
            task_ref=context.task_ref,
            target_worktree=target_worktree,
        )
    ...
    for raw_path in candidate_paths:
        ...
        if candidate_worktree == primary_worktree and repo_relative:
            try:
                ...
                handle_missing_contract(exc, policy=HarnessContractMissingPolicy.WARN)
                return None
...
def _escape_hatches_text() -> str:
    """Operator-facing escape hatch list shared by all drift block reasons."""
    return (
        "Escape hatches:\n"
...
def _build_probe_error_block_reason(probe_error: str) -> str:
    """Block reason for non-allowlisted probe_error; names the same hatches."""
    return f"UnresolvedTaskContextError: {probe_error}\n\n{_escape_hatches_text()}"
        ...
        json.dumps(
            {
                "hookSpecificOutput": {
                    ...
                    or _build_block_reason(decision),
                }
            }
        )
    )
