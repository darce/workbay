...
import _worktree_drift as wd  # noqa: E402
...
# ---------------------------------------------------------------------------
# implementation note: ambiguity fallback (internal) + regression ordering
# ---------------------------------------------------------------------------


def _make_primary_on_feature_branch(tmp_path: Path) -> tuple[Path, Path]:
    """Primary + linked feature worktree, then check the PRIMARY worktree out
    onto its own feature branch — the misuse the root-worktree guard catches."""
    primary, feature_root = _make_two_worktrees(tmp_path)
    _git("checkout", "-q", "-b", "feature/root-drift", cwd=primary)
    return primary, feature_root


def test_ambiguity_fallback_allows_with_outcome_fallback(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """When the resolver guessed the active row from multiple candidates
    (``resolution_note`` set), the guard ALLOWS with ``outcome='fallback'`` and
    does NOT evaluate candidate-path drift against the guessed row — even for a
    path (edit into the primary worktree) that would otherwise hard-block."""
    ...
    context = ActiveTaskContext(
        ...
        target_worktree=str(feature_root),
        primary_worktree=str(primary),
        resolution_note="ambiguous active task: chose internal (most recent)",
    )
    payload = {
        "toolName": "Edit",
        "toolInput": {"file_path": str(primary / "docs" / "scopes" / "would-be-drift.md")},
    }

    # workspace_root is the feature worktree (NOT the root worktree), so the
    # root-worktree guard does not apply; the fallback path governs.
    decision = evaluate_payload(payload, workspace_root=feature_root, active_task=context)
    assert decision is not None
    assert decision.outcome == "fallback", (
        "ambiguity fallback must ALLOW (not evaluate drift against a guessed row); "
        f"got outcome={decision.outcome!r} reason={decision.reason!r}"
    )


def test_ambiguity_fallback_still_blocks_root_worktree_on_feature_branch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Regression (fix ordering): an ambiguous multi-row resolution
    (``resolution_note`` set) must NOT exempt the root-worktree-on-non-main
    guard. That guard keys on the live branch + primary-worktree identity, not
    on the guessed task row, so an agent editing in the ROOT worktree while it
    is on a feature branch is still BLOCKED. Emitting the fallback before this
    guard was the regression."""
    monkeypatch.delenv("ALT_ALLOW_WORKTREE_DRIFT", raising=False)
    primary, feature_root = _make_primary_on_feature_branch(tmp_path)
    (primary / "docs" / "scopes").mkdir(parents=True)
    ...
    decision = evaluate_payload(payload, workspace_root=primary, active_task=context)
    assert decision is not None
    assert decision.outcome == "block", (
        "root worktree on a feature branch must BLOCK even under ambiguity "
        f"fallback; got outcome={decision.outcome!r}"
    )
    assert "RootWorktreeNotOnMainError" in (decision.reason or "")


def test_ambiguity_fallback_yields_maintenance_bypass_for_maint_task(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """MAINT bypass still precedes the fallback: a MAINT-* task with a
    ``resolution_note`` resolves to ``maintenance_bypass`` (allow), confirming
    the reorder keeps MAINT ahead of the fallback branch."""
    monkeypatch.delenv("ALT_ALLOW_WORKTREE_DRIFT", raising=False)
    primary, _feature_root = _make_two_worktrees(tmp_path)
    (primary / "docs" / "scopes").mkdir(parents=True)

    context = ActiveTaskContext(
        task_ref="internal",
        target_branch="main",
        target_worktree=str(primary),
        primary_worktree=str(primary),
        resolution_note="ambiguous active task: chose internal (most recent)",
    )
    payload = {
        "toolName": "Edit",
        "toolInput": {"file_path": str(primary / "docs" / "scopes" / "maint.md")},
    }

    decision = evaluate_payload(payload, workspace_root=primary, active_task=context)
    assert decision is not None
    assert decision.outcome == "maintenance_bypass", (
        f"MAINT bypass must precede the fallback branch; got {decision.outcome!r}"
    )


def test_main_fallback_exits_zero_and_logs_jsonl(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """main() on a ``fallback`` decision exits 0 (allow, not block) and writes a
    JSONL trace record with ``outcome=fallback``."""
    monkeypatch.delenv("WORKBAY_HOOK_PROTOCOL_STRICT", raising=False)
    primary = tmp_path / "primary"
    primary.mkdir()

    decision = wd.DriftDecision(
        outcome="fallback",
        reason="ambiguous active task: chose internal (most recent)",
        primary_worktree=str(primary),
        task_ref="internal",
        target_worktree=str(primary),
    )
    monkeypatch.setattr(wd, "evaluate_payload", lambda _payload: decision)

    payload = {
        "hook_event_name": "PreToolUse",
        "toolName": "Edit",
        "toolInput": {"file_path": str(primary / "x.md")},
    }
    monkeypatch.setattr(sys, "stdin", io.StringIO(json.dumps(payload)))

    rc = wd.main()
    assert rc == 0, "a fallback decision must not block (exit 0)"

    trace = primary / ".task-state" / wd.TRACE_LOG_NAME
    assert trace.exists(), "fallback decisions must be traced to the JSONL log"
    records = [json.loads(line) for line in trace.read_text().splitlines() if line.strip()]
    assert records and records[-1]["outcome"] == "fallback"
