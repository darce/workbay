...
def _safe_stream_write(stream, message: str) -> None:
    """Best-effort stream write that never lets BlockingIOError/BrokenPipeError escape.

    Used by the pre-push refusal path so a nonblocking/closed stderr still yields
    the fail-closed exit code without a traceback. Retries transient blocks with
    partial-write accounting and short backoff, flushes the text buffer before any
    raw fileno fallback, then falls back to ``os.write`` when available; last
    resort is swallow. Never raises.

    Exit-code policy for callers: this helper is message delivery only. A write
    failure must not flip an override-accept (allow) decision to refuse, and must
    not flip a refusal decision to allow — callers keep their policy exit code.
    """
    if stream is None:
        return
    remaining = message
    # Retry loop: a single BlockingIOError must not discard the explanation.
    for attempt in range(8):
        ...
        try:
            ...
            if written is None:
                # TextIO often returns None after a full write.
                return
            if written <= 0:
                break
            ...
            # Partial delivery before the block: advance past characters already
            # accepted so the retry does not re-send the prefix (B-01).
            partial = getattr(exc, "characters_written", 0) or 0
            if partial > 0:
                remaining = remaining[partial:]
            if not remaining:
                return
            # Bounded backoff so a genuinely full nonblocking pipe can drain (B-05).
            try:
                time.sleep(min(0.001 * (2 ** attempt), 0.05))
            except Exception:  # noqa: BLE001 — sleep is best-effort only
                pass
            continue
        except BrokenPipeError:
            return
        except Exception:  # noqa: BLE001 — last-resort: message delivery only
            return
    if not remaining:
        return
    # Flush buffered prefix before raw fileno writes so order is preserved (B-02).
    # Without this, TextIOWrapper may still hold a counted-as-written prefix that
    # would flush *after* the os.write bytes and garble/lose the refusal text.
    try:
        flush = getattr(stream, "flush", None)
        if callable(flush):
            flush()
    except Exception:  # noqa: BLE001 — proceed to fileno fallback anyway
        pass
    # Fallback: raw fileno write with partial-write accounting.
    try:
        fd = stream.fileno()
    except Exception:  # noqa: BLE001
        return
    if not isinstance(fd, int):
        # Non-int fileno (test doubles / wrappers) would raise TypeError in
        # os.write and escape the Never-raises contract (B-04).
        return
    try:
        data = remaining.encode("utf-8", errors="replace")
    except Exception:  # noqa: BLE001
        return
    offset = 0
    for attempt in range(8):
        ...
        try:
            ...
            return
        except Exception:  # noqa: BLE001 — never raise from message delivery (B-04)
            return
...
def run_pre_push(
    ...
    *,
    ...
    if _resolve_override_env(env, _PUSH_OVERRIDE_ENV_VAR) != "1":
        _safe_stream_write(stream, _format_pre_push_block(branch))
        return 1
...
def _is_zero_sha(sha: str) -> bool:
    """True when ``sha`` is the all-zeros git delete marker (any length)."""
    return bool(sha) and set(sha) <= {"0"}


def _branch_name_from_ref(ref: str) -> str | None:
    """Extract a branch name from ``refs/heads/<name>``; else None."""
    prefix = "refs/heads/"
...
def collect_pre_push_branch_updates(stdin_text: str) -> list[str]:
    """Parse git pre-push stdin ref lines into non-delete branch names.

    Each line is ``<local_ref> <local_sha> <remote_ref> <remote_sha>``.
    Deletes (local_sha all zeros) are allowed and omitted. Non-branch
    refs (tags, notes, etc.) are ignored. Order is preserved; duplicates
    are kept so each published name is classified.
    """
    branches: list[str] = []
    for raw in stdin_text.splitlines():
        ...
        if _is_zero_sha(local_sha):
            continue
        branch = _branch_name_from_ref(local_ref) or _branch_name_from_ref(remote_ref)
        if branch is not None:
            branches.append(branch)
    return branches


def run_pre_push_refspecs(
    branches: list[str],
    env: dict | None = None,
    *,
    recorder: Callable[[str, str | None], str | None] | None = None,
    ...
    """Classify every branch name being published; first refusal wins.

    Empty ``branches`` means no heads updates in the refspec set (deletes
    only, tags only, or empty stdin) — nothing to refuse on naming grounds.
    """
...
def _read_pre_push_stdin() -> str:
    """Best-effort read of git pre-push ref lines from stdin.

    When stdin is a TTY (manual helper invocation) return empty so callers
    fall back to current-branch checks. For non-TTY pipes (git pre-push /
    tests feeding ``input=``), read the full payload. Never raises.
    """
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Publish gate: classify the branch names on the ref lines being pushed,
    # not merely ``git branch --show-current``. Pushing a nonconforming ref
    # from main / a conforming branch / detached HEAD must still refuse.
    # When no heads updates appear on stdin (TTY / empty / deletes-only),
    # fall back to the current branch so direct helper invocations and
    # existing unit tests keep working; detached HEAD with no refspecs
    # publishes nothing and is allowed.
    ref_branches = collect_pre_push_branch_updates(_read_pre_push_stdin())
    if ref_branches:
        return run_pre_push_refspecs(ref_branches)
