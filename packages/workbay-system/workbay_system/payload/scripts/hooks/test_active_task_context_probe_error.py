...
a short stable token when the probe could not answer and left ``None`` when it
...
def test_import_failure_is_reported_as_a_probe_error(tmp_path, monkeypatch):
    ...
    assert ctx.probe_error == "handoff_unavailable", (
        ...
        f"expected handoff_unavailable, got {ctx.probe_error!r}"
    )
...
def test_raising_probe_is_reported_as_a_probe_error(tmp_path, monkeypatch):
    ...
    assert ctx.task_ref is None
    assert ctx.probe_error == "handoff_probe_failed", (
        "get_handoff_state raised and the exception was swallowed into a "
        f"task_ref=None default -- expected handoff_probe_failed, got {ctx.probe_error!r}"
    )
...
# --- Handoff fallback wall-clock budget (finding 9069) ---------------------------
...
def test_over_budget_relabel_preserves_every_other_field():
    ...
    ``_with_over_budget_probe_error`` has to forward every field of the
    context it relabels. Rebuilding it positionally — the natural thing to do
    when a field is added to ``ActiveTaskContext`` — silently shifts every
    value one slot and still returns a plausible-looking context.

    Asserting field-by-field rather than on one field alone: the
