...
asserts on the JSON shape printed to stdout. The active-task resolvers
are monkeypatched at module level by importing the hook as a module and
swapping `_load_active_task` (SessionStart / full-probe path) or
`_try_load_active_task_from_snapshot` (UserPromptSubmit snapshot-only
path) for fakes.
"""
...
def _fake_context(
    ...
    return ActiveTaskContext(task_ref, target, branch, primary, plan_path)
...
def test_silent_when_no_active_task(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    mod = _load_hook()
    monkeypatch.setattr(
        mod, "_try_load_active_task_from_snapshot", lambda _root: _fake_context(None, None)
    )
...
def test_silent_for_maint_task_prefix(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _root: _fake_context("MAINT-cleanup-20260501", str(target)),
    )
...
def test_silent_on_unresolved_task_context_error(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    monkeypatch.setattr(mod, "_try_load_active_task_from_snapshot", _raise)
    monkeypatch.setattr(mod, "_workspace_root", lambda: tmp_path)
...
def test_emits_directive_in_process(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    monkeypatch.setattr(
        mod,
        "_try_load_active_task_from_snapshot",
        lambda _root: _fake_context("internal", str(target)),
    )
    monkeypatch.setattr(mod, "_workspace_root", lambda: cwd)
    monkeypatch.setattr(mod, "_cwd_worktree", lambda _cwd: str(cwd.resolve(strict=False)))
...
def test_warns_plan_suffix_drift_even_when_cwd_matches(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """implementation note D3: a plan-bound task (``task_plan_path`` = ``docs/plans/<NNNN>-*``)
    whose branch lacks the ``-plan<NNNN>`` suffix triggers a one-line drift warning
    naming the relink command — even when cwd already matches target (so the
    cwd-nudge is silent), proving the warn is independent of the cd early-return.
    The hook only warns; it never mutates."""
    target = tmp_path / "worktree"
    target.mkdir()
    ...
    ctx = _fake_context(
        "internal",
        str(target),
        branch="feature/ws-foo-01",
        plan_path="docs/plans/0068-ws-foo-01.md",
    )
    monkeypatch.setattr(mod, "_load_active_task", lambda _root: ctx)
    monkeypatch.setattr(mod, "_workspace_root", lambda: target)
    monkeypatch.setattr(mod, "_cwd_worktree", lambda _cwd: str(target.resolve(strict=False)))
    ...
    captured = capsys.readouterr()
    assert rc == 0
    assert captured.out, "expected a plan-drift warning even when cwd matches target"
    parsed = json.loads(captured.out)
    addl = parsed["hookSpecificOutput"]["additionalContext"]
    assert "-plan0068" in addl, addl
    assert "make plan-accept TASK=internal" in addl, addl
    # The cwd-nudge must NOT fire (cwd matches target).
    assert "before any further tool calls" not in addl, addl


def test_silent_when_plan_suffix_present(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """implementation note D3: a correctly-suffixed plan-bound branch is silent."""
    target = tmp_path / "worktree"
    target.mkdir()
    ...
    ctx = _fake_context(
        "internal",
        str(target),
        branch="feature/ws-foo-01-plan0068",
        plan_path="docs/plans/0068-ws-foo-01.md",
    )
    monkeypatch.setattr(mod, "_load_active_task", lambda _root: ctx)
    monkeypatch.setattr(mod, "_workspace_root", lambda: target)
...
def test_prompt_event_never_calls_authoritative_probe(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """UserPromptSubmit must not pay the handoff-package fallback: when the
    snapshot cannot answer (returns None), the hook stays silent instead of
    falling back to `_load_active_task` (multi-second import against the
    harness hook timeout, on every prompt)."""
    mod = _load_hook()
    monkeypatch.delenv("WORKBAY_ADVISE_CD_PROMPT_FULL_PROBE", raising=False)

    def _must_not_run(_root: Path) -> Any:
        raise AssertionError("authoritative probe must not run on UserPromptSubmit")

    monkeypatch.setattr(mod, "_load_active_task", _must_not_run)
    monkeypatch.setattr(mod, "_try_load_active_task_from_snapshot", lambda _root: None)
    monkeypatch.setattr(mod, "_workspace_root", lambda: tmp_path)
...
def test_session_start_still_uses_authoritative_probe(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """SessionStart keeps the full probe: the snapshot resolver must not be
    consulted, and the probe's answer drives the directive."""
    target = tmp_path / "worktree-feature"
    target.mkdir()
    cwd = tmp_path / "worktree-primary"
    cwd.mkdir()
    ...
    def _must_not_run(_root: Path) -> Any:
        raise AssertionError("snapshot resolver must not run on SessionStart")

    monkeypatch.setattr(mod, "_try_load_active_task_from_snapshot", _must_not_run)
    monkeypatch.setattr(mod, "_load_active_task", lambda _root: _fake_context("internal", str(target)))
    monkeypatch.setattr(mod, "_workspace_root", lambda: cwd)
    monkeypatch.setattr(mod, "_cwd_worktree", lambda _cwd: str(cwd.resolve(strict=False)))

    payload = {"hook_event_name": "SessionStart", "session_id": "s", "source": "startup", "cwd": str(cwd)}
    monkeypatch.setattr("sys.stdin", _StdinFromString(json.dumps(payload)))
    rc = mod.main()
    captured = capsys.readouterr()
    assert rc == 0
    assert captured.out, "expected directive from the SessionStart full probe"
    parsed = json.loads(captured.out)
    assert parsed["hookSpecificOutput"]["hookEventName"] == "SessionStart"


def test_prompt_full_probe_env_restores_fallback(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """WORKBAY_ADVISE_CD_PROMPT_FULL_PROBE=1 restores the authoritative probe
    on UserPromptSubmit (opt-out valve for the snapshot-only policy)."""
    target = tmp_path / "worktree-feature"
    target.mkdir()
    cwd = tmp_path / "worktree-primary"
    cwd.mkdir()
    ...
    mod = _load_hook()
    monkeypatch.setenv("WORKBAY_ADVISE_CD_PROMPT_FULL_PROBE", "1")

    def _must_not_run(_root: Path) -> Any:
        raise AssertionError("snapshot resolver must not run under the full-probe valve")
    ...
    captured = capsys.readouterr()
    assert rc == 0
    assert captured.out, "expected directive from the env-restored full probe"
