...
class HarnessContractSchemaError(HarnessContractMissingError):
    """Raised when the contract exists but is too stale or malformed to trust."""
...
def handle_missing_contract(
    ...
    *,
    ...
    A present-but-unusable contract (``HarnessContractSchemaError``) is
    untrustworthy and always exits ``2``, even under ``WARN`` or
    ``SILENT``. A merely missing or unreadable file still honours the
    caller's policy so minimal installs without a contract do not
    hard-block.
    """
    out = stream if stream is not None else sys.stderr
    # A present-but-stale or malformed contract is untrustworthy: escalate
    # to a hard exit 2 regardless of policy so it cannot silently bypass
    # main-branch isolation under WARN/SILENT (finding
    # PMR1-B82S1-stale-contract-warn-bypass). A merely missing or
    # unreadable file is raised with schema=False and still honours the
    # caller's policy below.
    if isinstance(error, HarnessContractSchemaError):
...
class BranchIsolationPolicy:
    ...
    # tripwire (runtime state files only). That tripwire reports paths
    # that are actually dirty — not merely present on disk. Tracked files
    # with a plain `H` index tag and no porcelain dirt are excluded;
    # skip-worktree / assume-unchanged / other index flags and any
    # fail-closed git degradation keep the path flagged. Defaulted to ()
    # so existing test fixtures and callers that construct the policy by
    # hand keep working unchanged during the transition.
    state_dirty_surfaces: tuple[MainSurfacePattern, ...] = ()
    first_edit_protected_surfaces: tuple[MainSurfacePattern, ...] = ()


def _contract_error(
    contract_path: Path, detail: str, *, schema: bool = True
) -> HarnessContractMissingError:
    error_cls = HarnessContractSchemaError if schema else HarnessContractMissingError
    return error_cls(
...
def _extract_branch_isolation_lines(contract_path: Path) -> list[str]:
    try:
        ...
        raise _contract_error(contract_path, "File is missing or unreadable.", schema=False) from exc
...
def _load_surface_patterns(
    ...
    *,
    ...
    if key not in mapping and required_non_empty:
        raise _contract_error(contract_path, f"`branch_isolation.{key}` must be present and non-empty.")
    values = mapping.get(key, [])
    if not isinstance(values, list):
        raise _contract_error(contract_path, f"`branch_isolation.{key}` must be a list.")
...
def is_branch_isolation_protected_path(rel_path: str, policy: BranchIsolationPolicy) -> bool:
    ...
    if normalized in policy.root_protected_files:
        return True
    if _find_surface_match(normalized, policy.first_edit_protected_surfaces) is not None:
        return True
