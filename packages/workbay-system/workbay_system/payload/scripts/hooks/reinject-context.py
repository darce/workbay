...
def _reinject(
    *,
    ...
    # The reinject hook is a genuine orientation-read boundary (cold-start
    # reinjection), so mark it explicitly: orientation instrumentation is
    # opt-in and only records inside a boundary. Older handoff builds may not
    # export the boundary context manager; fall back to a no-op so version
    # skew never breaks reinjection (the read just goes uninstrumented).
    try:
        from workbay_handoff_mcp import (  # type: ignore[import-not-found]
            orientation_read_boundary,
        )
    except Exception:  # noqa: BLE001
        from contextlib import nullcontext as orientation_read_boundary  # type: ignore[assignment]
