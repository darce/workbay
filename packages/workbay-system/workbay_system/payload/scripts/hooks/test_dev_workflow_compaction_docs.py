...
_CONTRACT_RELPATH = Path("docs") / "workbay" / "contracts" / "harness-protocol.yaml"


def _payload_root(monorepo: Path) -> Path:
    return (
        monorepo / "packages" / "workbay-system" / "workbay_system" / "payload"
    )


def _find_monorepo_root(start: Path) -> Path:
    """Locate the monorepo root from EITHER twin's own location.

    This module is a byte-identical twin: one copy lives at
    ``scripts/hooks/`` and the other at
    ``packages/workbay-system/workbay_system/payload/scripts/hooks/``. The two
    sit at different depths, so a literal ``parents[N]`` is correct in at most
    one of them. The previous ``parents[6]`` was written for the payload copy,
    where it lands on the monorepo root; from the ROOT copy the same index walks
    four levels ABOVE the repository and resolves to ``/``, so the "root"
    parametrisation read ``/docs/workbay/contracts/harness-protocol.yaml`` --
    a path outside the repository altogether. That failed loudly here, but on a
    host where that path happened to exist it would have silently asserted
    against a foreign file, and the sibling "payload" id was reading the root
    tree at the same time. Anchoring on markers present in both trees keeps one
    shared source correct from either depth and keeps the ids truthful
    [TEST-15].
    """
    for candidate in (start, *start.parents):
        if (candidate / _CONTRACT_RELPATH).is_file() and (
            _payload_root(candidate) / _CONTRACT_RELPATH
        ).is_file():
            return candidate
    raise AssertionError(f"could not locate monorepo root from {start}")


MONOREPO_ROOT = _find_monorepo_root(Path(__file__).resolve().parent)
ROOT_CONTRACT_PATH = MONOREPO_ROOT / _CONTRACT_RELPATH
CONTRACT_PATH = _payload_root(MONOREPO_ROOT) / _CONTRACT_RELPATH
