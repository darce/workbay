...
def _write_contract(repo: Path, *, first_edit_block: str | None) -> None:
    contract = repo / "docs" / "workbay" / "contracts" / "harness-protocol.yaml"
    ...
    lines = [
        ...
        "    - pattern: legacy-only/**",
        "      reason: legacy transition surface",
        ...
        "      reason: active-task snapshot",
    ]
    if first_edit_block is not None:
        lines.extend(
            [
                "  first_edit_protected_surfaces:",
                *first_edit_block.splitlines(),
            ]
        )
    contract.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def test_loader_rejects_missing_first_edit_protected_surfaces(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _write_contract(repo, first_edit_block=None)

    with pytest.raises(HarnessContractMissingError, match="first_edit_protected_surfaces"):
        load_branch_isolation_policy(repo)


def test_loader_rejects_empty_first_edit_protected_surfaces(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _write_contract(repo, first_edit_block="")
...
def test_branch_isolation_protected_path_uses_first_edit_surface_when_lists_diverge() -> None:
    policy = _split_policy()
    assert is_branch_isolation_protected_path("docs/tasks/SOME-TASK-task-plan.md", policy) is True
    assert is_branch_isolation_protected_path("docs/scopes/foo.md", policy) is True
    assert is_branch_isolation_protected_path("legacy-only/old.md", policy) is False
...
def test_find_dirty_state_files_skips_tracked_clean_top_level_state(
    tmp_path: Path,
) -> None:
    """A committed, unmodified state projection must not read as dirty.

    Pass 2 walks the filesystem for state surfaces, so without the
    tracked-and-clean subtraction a clean main with committed
    CURRENT_TASK.json / DASHBOARD.txt always looks dirty even when
    ``git status`` is empty -- a consumer false positive on pre-push.
    """
    repo = _init_repo(tmp_path)
    (repo / "CURRENT_TASK.json").write_text('{"task_ref":"x"}\n', encoding="utf-8")
    (repo / "DASHBOARD.txt").write_text("dashboard\n", encoding="utf-8")
    _git("add", "CURRENT_TASK.json", "DASHBOARD.txt", cwd=repo)
    _git("commit", "-q", "-m", "track state projections", cwd=repo)
    assert _git("status", "--porcelain=v1", cwd=repo) == ""

    dirty = find_dirty_state_files(repo_root=str(repo), policy=_split_policy())
    assert dirty == [], dirty


def test_find_dirty_state_files_reports_skip_worktree_divergence(
    tmp_path: Path,
) -> None:
    """Non-vacuity control for the tracked-and-clean subtraction.

    ``test_find_dirty_state_files_detects_untracked_top_level_state`` does not
    constrain ``_is_tracked_clean_no_suppress``: an untracked file is reported
    by pass 1 (``git status --ignored``) and never reaches pass 2's
    subtraction. Neither does a tracked-but-modified file -- porcelain sees
    that too. Verified by mutation: forcing the helper to return True
    unconditionally leaves both of those tests green.

    The subtraction only matters where porcelain is blind. ``--skip-worktree``
    is that case: git suppresses the divergence, pass 1 reports nothing, and
    only pass 2's walk nominates the path. The ``S`` index tag must then stop
    it being subtracted -- the guarantee find_dirty_protected_paths' own
    docstring states as "Porcelain alone is not a cleanliness proof".
    """
    repo = _init_repo(tmp_path)
    (repo / "CURRENT_TASK.json").write_text('{"task_ref":"x"}\n', encoding="utf-8")
    _git("add", "CURRENT_TASK.json", cwd=repo)
    _git("commit", "-q", "-m", "track state projection", cwd=repo)

    _git("update-index", "--skip-worktree", "CURRENT_TASK.json", cwd=repo)
    (repo / "CURRENT_TASK.json").write_text('{"task_ref":"CHANGED"}\n', encoding="utf-8")

    assert _git("status", "--porcelain=v1", cwd=repo) == "", (
        "skip-worktree did not suppress the divergence; test premise invalid"
    )
