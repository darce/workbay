...
def test_ensure_handoff_import_paths_includes_workbay_protocol() -> None:
    ...
    Payload nested path: parents[2] is payload/, not monorepo root. ``make
    dogfood-link`` replaces ``scripts/hooks`` with a symlink into this payload
    nest; ``Path(__file__).resolve()`` follows that symlink, so the base stays
    nested. The parents[5]/[6] candidates make protocol resolve from this depth
    unconditionally — assertions below are not gated on layout.
    """
