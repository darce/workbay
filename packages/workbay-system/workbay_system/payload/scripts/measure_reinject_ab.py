...
HISTORICAL ONLY (implementation note S4): ``WORKBAY_REINJECT_AB`` assignment was removed
from the reinject hook. New ``session_reinjections.arm`` values are always
NULL; do not set the env for live collection. This script remains for
pre-removal rows only.

Historical context (implementation note era, pre-0093 removal): under WORKBAY_REINJECT_AB
the arm once governed *selection* — treatment=semantic top-K, control=current
selection — and BOTH arms emitted. The m1/m2 reorientation metrics and the
...
historical per-arm token cost only from pre-0093 rows via
``session_reinjections.emitted_chars`` grouped by ``arm``. Post-0093 live rows
have ``arm=NULL`` — group by ``semantic_detail_json`` status / ``chars_used``
instead (see packages/mcp-workbay-handoff/docs/guides/semantic-reinjection.md).
