...
_finish_job() {
    ...
    _stdout_bytes="$(wc -c < stdout 2>/dev/null | tr -d ' ')"
    case "${_stdout_bytes}" in ''|*[!0-9]*) _stdout_bytes=0 ;; esac
    _stdout_sha256="$(sha256sum stdout 2>/dev/null | awk '{print $1}')"
    [ -n "${_stdout_sha256}" ] || _stdout_sha256="missing"
    _job_id="$(basename "$(pwd)")"
    exec 9>>.lock
    flock -w 30 9 || true
    if [ -f done.json ]; then
        return 0
    ...
    printf '{"finished_at":"%s","job_id":"%s","rc":%s,"reason":null,"schema_version":1,"stdout_bytes":%s,"stdout_sha256":"%s"}\n' \
        "${_finished_at}" "${_job_id}" "${_rc}" "${_stdout_bytes}" "${_stdout_sha256}" > done.json.tmp
    if ln done.json.tmp done.json 2>/dev/null; then
        rm -f done.json.tmp
    else
        rm -f done.json.tmp
        return 0
    fi
    if [ -f transients ]; then
        _brief="$(sed -n '1p' transients)"
        _outbox="$(sed -n '2p' transients)"
        _ref="$(sed -n '3p' transients)"
...
_remote_job_ops() {
    ...
    _remote_job_json_field() {
        ...
        _k="${2:-}"
        [ -f "$_f" ] || return 0
        WORKBAY_JSON_FILE="$_f" WORKBAY_JSON_KEY="$_k" timeout 10 python3 - <<'PY' || true
import json
import os
import sys
...
_snapshot_proc_cwds() {
    ...
    _uids=()
    _targets=()
    mapfile -t _uids < <(stat -c %u -- "${_pid_dirs[@]}" 2>/dev/null || true)
    mapfile -t _targets < <(readlink -- "${_cwds[@]}" 2>/dev/null || true)
    _i=0
