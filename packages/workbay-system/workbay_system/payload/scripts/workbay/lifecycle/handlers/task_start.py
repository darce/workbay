...
_DEFAULT_ADOPT_CMD = ("uvx", "workbay-bootstrap", "adopt-worktree")
...
def _adopt_overlay_command(*, worktree_path: Path | None = None) -> list[str]:
    ...
    fall back to ``uvx workbay-bootstrap adopt-worktree`` (the symmetric call to
    the consumer's ``uvx workbay-bootstrap install``). This keeps source
...
_DEFAULT_BOOTSTRAP_SURFACES_CMD = ("uvx", "workbay-bootstrap", "bootstrap-surfaces")
...
def _bootstrap_surfaces_command(*, worktree_path: Path | None = None) -> list[str]:
    ...
    checkouts on the bootstrap code under review), else fall back to
    ``uvx workbay-bootstrap bootstrap-surfaces``.
    """
