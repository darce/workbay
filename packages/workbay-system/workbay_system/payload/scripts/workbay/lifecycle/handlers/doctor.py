...
def _probe_venv(repo: Path) -> DoctorVenv:
    ...
    if int(projection_summary.get("dead_letter_count") or 0) > 0:
        remediation.append(
            "projection dead-letter rows exist; replay them in bounded batches "
            'with `make project-events-replay LIFECYCLE_ARGS="--input '
            '.task-state/projection-dead-letter.jsonl --max-entries 500 --json"`'
        )
