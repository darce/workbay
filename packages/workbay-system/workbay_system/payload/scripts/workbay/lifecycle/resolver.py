...
try:
    ...
    # Mirror branch_naming: accept POSIX ``/`` and Windows ``\`` separators.
    _PLAN_ID_RE = re.compile(r"(?:^|[\\/])(\d+)-[^\\/]+$")

    def extract_plan_id(plan_path: str | None) -> str | None:
        """Stdlib-only mirror of ``branch_naming.extract_plan_id``."""
        if not plan_path:
            return None
...
def _candidates_stdlib(branch: str) -> list[str]:
    """Stdlib mirror of ``workbay_protocol.branch_naming.derive_task_ref_candidates``.

    Only consulted by the inline fallback below when ``workbay_protocol``
