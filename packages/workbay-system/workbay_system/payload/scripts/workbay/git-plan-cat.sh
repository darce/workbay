...
#
# Override `WORKBAY_HANDOFF_PLAN_CLI` to change the default `python3 -m`
# launcher (e.g. when the consumer manages its own venv). Any
...
PLAN_CLI=${WORKBAY_HANDOFF_PLAN_CLI:-python3 -m workbay_handoff_mcp.plan_cli}

# Intentional unquoted expansion: the launcher variable carries multiple shell
# tokens (e.g. `python3 -m workbay_handoff_mcp.plan_cli`) that must word-split
# before exec.
if [ "$#" -eq 0 ]; then
    exec ${PLAN_CLI} show
fi
