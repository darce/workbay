...
_GUARD_LAUNCHER_RELPATH = "scripts/hooks/_launch_python.sh"
...
def wrap_guard_command(
    command: str, *, fail_mode: object = None, policy_aware: object = None
) -> str:
    ...
    ``guard-codemap-first.py`` (or ``policy_aware=True``) also emits
    ``--policy-aware`` so the wrapper classifies before requiring a handler.
    """
    try:
    ...
    if not rest:
        return command
    script = rest[0]
    if any(token.endswith("_launch_python.sh") for token in words[:3]):
        return command
    ...
    tokens = [
        f"{anchor}{_GUARD_LAUNCHER_RELPATH}",
        f"{anchor}{_GUARD_WRAPPER_RELPATH}",
    ]
    if fail_mode == "closed":
        tokens.append("--fail-mode=closed")
    handler_name = _WRAP_ANCHOR_RE.sub("", script).rsplit("/", 1)[-1]
    if policy_aware or handler_name == "guard-codemap-first.py":
        tokens.append("--policy-aware")
    tokens.extend(rest)
