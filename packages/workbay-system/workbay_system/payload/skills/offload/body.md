...
explicit backend (`grok-remote`, `grok-cli`, or `codex-subagent`) at an explicit reasoning
...
- the operator invokes `/offload --agent <grok-remote|grok-cli|codex-subagent> --effort <level> --token-budget <N> "<slice objective>"`
...
- **agent**: `grok-remote`, `grok-cli`, or `codex-subagent`. Normalized through
...
6. `dispatch_lane_work(brief=<brief per the Brief contract below>, dispatch_id=<idempotency key>,
   ...
   **Default to `include_context_packet=true` with `context_targets` = the slice's files**: a
   ...
   absent. Omit only for a trivial single-file slice. Dispatch also warns (never blocks) when
   `test_cmd` looks like a whole-package pytest with no `-k`/`::`/file selector
