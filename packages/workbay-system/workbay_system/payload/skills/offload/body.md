...
explicit backend (`grok-remote`, `grok-cli`, `codex-subagent`, `codex-remote`, `cursor-cli`, or `cursor-remote`) at an explicit reasoning
effort, with a caller-enforced token governor, a manifest-pinned reviewer backend, and a
human/orchestrator review gate before merge. Fail-fast, **no fallback** between backends.

## Trigger

Use this skill when:

- the operator invokes `/wb-offload --agent <grok-remote|grok-cli|codex-subagent|codex-remote|cursor-cli|cursor-remote> --effort <level> --token-budget <N> "<slice objective>"`
...
- **agent**: `grok-remote`, `grok-cli`, `codex-subagent`, `codex-remote`, `cursor-cli`, or
  `cursor-remote`. Normalized through `backend_registry.validate_backend`; anything else
  (including `codex-cli`) fails fast.
  ...
  `codex-remote` and `cursor-remote` are the off-box members (with `grok-remote`) of
  `REMOTE_OFFLOAD_BACKENDS`, derived by `derive_remote_offload_backends` from the
  declared `dispatchable_off_box` capability on each backend-registry row, and are
  explicitly selectable `/wb-offload --agent` values — including under `remote_only`.
...
- **single-cycle bound**: from each profile's `timeout_cap`, read via
  ...
  `codex-remote` is bounded by a 3600-second wall-clock timeout cap.
- **cursor-remote review limitation**: `cursor-remote` is implement-only today — the
  built-in review template contains brace characters, and `cursor-remote` carries the whole
  prompt as a single positional argv element whose remote resolver refuses braces, so the
  review phase fails while implementation commits normally.
...
2. Start from the supported offload profiles (`grok-remote`, `grok-cli`, `codex-subagent`,
   `codex-remote`, `cursor-cli`, `cursor-remote` — under `remote_only`, `grok-remote` is the
   default when no backend is given and every `REMOTE_OFFLOAD_BACKENDS` member
   (`codex-remote`, `cursor-remote`, `grok-remote`) is explicitly selectable); drop any that
...
7. `run_offload_pass(lane_id, backend=<agent>, model, reasoning_effort, token_budget=N,
   ...
   `handoff_ready | review_complete | needs_guidance | no_actionable_work | uncommitted_work | token_budget_exceeded | timeout | error | lane_not_found | self_verify_failed | composer_violation_quarantined | checkpoint | server_stale_restart_required | admission_deferred | admission_refused | remote_required`.
   ...
   (`DEFAULT_GROK_MODEL` / `WORKBAY_GROK_MODEL`, default grok-4.6) returns plain
   ...
   **Remote VM admission is measured.** Off-box (`COST_REMOTE`) lanes are admitted when every
   measured gauge still has headroom after adding one lane at the measured per-lane cost
   (max of live grok-lane scopes, floors of 66 pids and 512 MiB anon) plus the pids floor
   and the memory floor. Gauges: `vm_pids` (`pids.current` vs `pids.max`), `vm_memory`
   (anon+kernel+shmem vs `memory.max`; page cache does not count), `vm_memory_psi`,
   `vm_cpu_psi`, `vm_disk`; locally, disk headroom for the lane worktree and macOS memory
   pressure. A saturated gauge defers with exit 75 and `defer_reason: vm_measured_saturation`
   plus `binding_gauge`; an unreadable gauge defers `vm_admission_unmeasured` (a probe fault:
   check the VM, do not shrink width). Legacy `vm_pid_saturation` is gauge `vm_pids`.
   `WORKBAY_REMOTE_AGENT_MAX_LANES` is an optional operator ceiling (default none); hitting
   it defers `vm_lane_cap`. Off-box admission payloads report `derived_width: null`,
   `width_source: off_box_vm_measured`; `max_width` bounds local heavy slots only. Never
   report a slot or lane count as the reason work is held — report the binding gauge and
   its numbers.
...
9. Present the lane handoff diff to the orchestrator for the **review gate** (`/wb-review-slice`); do not auto-merge.

   Distinct from the in-lane `/wb-review-code` smoke test — `/wb-review-slice` is the
...
- **Branch-complete gate**: `/wb-review-slice` followed by `/wb-auto-fix` is the **default
...
does not gate merge. The orchestrator's `/wb-review-slice` at branch-complete is the
**sole merge gate**.
...
- A single converged `single_pass` cycle is bounded by the profile's single-cycle bound
  ...
  wall-clock cap), not `token_budget`.
...
- Handoff diff presented for orchestrator `/wb-review-slice` review gate (branch-complete
