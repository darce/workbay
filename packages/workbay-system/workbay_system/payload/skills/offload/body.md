...
explicit backend (`grok-remote`, `grok-cli`, `codex-subagent`, or `cursor-cli`) at an explicit reasoning
...
- the operator invokes `/offload --agent <grok-remote|grok-cli|codex-subagent|cursor-cli> --effort <level> --token-budget <N> "<slice objective>"`
...
- **agent**: `grok-remote`, `grok-cli`, `codex-subagent`, or `cursor-cli`. Normalized through
  ...
  `codex-remote` and `cursor-remote` are remote invocation backends reachable via
  `dispatch_lane_work`, and are not `/offload --agent` values.
- **effort**: one of `low|medium|high|xhigh|auto|inherit` (the canonical
  ...
  and are **not** pinned.
- **single-cycle bound**: `grok-cli` and `grok-remote` derive `max_turns`/`timeout` from
  `token_budget`; `codex-subagent` is guarded by the Codex app-server bridge timeout;
  `cursor-cli` is bounded by timeout only (cursor-agent has no max-turns flag).
...
2. Start from the supported offload profiles (`grok-remote`, `grok-cli`, `codex-subagent`, `cursor-cli`
   — under `remote_only` only `grok-remote` is a candidate); drop any that
...
- A single converged `single_pass` cycle is bounded by the profile's single-cycle bound
  ...
  cursor-cli timeout only), not `token_budget`.
