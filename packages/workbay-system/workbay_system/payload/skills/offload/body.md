...
6. `dispatch_lane_work(brief=<brief per the Brief contract below>, dispatch_id=<idempotency key>,
   ...
   **Default to `include_context_packet=true` with `context_targets` = the slice's files**: a
   ...
   (`brief_test_cmd_full_suite`) or the brief text instructs a full re-baseline
   (`brief_requests_full_rebaseline`) — both time out a grok pass; use a scoped
   `build_lane_test_cmd(pkg, selector)` hermetic worktree-venv form instead (implementation note).
...
- **Non-Python / no-root-pyproject consumers**: the secure sandbox is a shallow clone that
  ...
  sandbox (e.g. `npm ci && npm test -- --run path/to/slice`, or
  `composer install && …`). The same pattern applies to a Python monorepo with
