...
- the operator invokes `/offload --agent <grok-remote|grok-cli|codex-subagent|codex-remote|cursor-cli|cursor-remote> --effort <level> --token-budget <N> "<slice objective>"`
...
- **agent**: `grok-remote`, `grok-cli`, `codex-subagent`, `codex-remote`, `cursor-cli`, or
  ...
  `REMOTE_OFFLOAD_BACKENDS`, derived by `_derive_remote_offload_backends` from the
  declared `dispatchable_off_box` capability on each backend-registry row, and are
  explicitly selectable `/offload --agent` values — including under `remote_only`.
