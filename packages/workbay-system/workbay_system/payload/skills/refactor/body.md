...
- **Deceptive booleans → outcome enum** — caller null-checks pile up on bool/`X|null` return. See [heuristics canon](https://github.com/darce/heuristics-canon).
- **Strategy-map over switch** — fat if/else-if grows with each new case. See [heuristics canon](https://github.com/darce/heuristics-canon).
- **Gate class** — repeated async permission checks across handlers. See [heuristics canon](https://github.com/darce/heuristics-canon).
- **Pipe / condition-object** — 4+ booleans combined and reused. See [heuristics canon](https://github.com/darce/heuristics-canon).
- **Backpressure / bounded queue** — producer-consumer with no queue bound. See [heuristics canon](https://github.com/darce/heuristics-canon).
- **Amdahl ceiling** — parallelism proposed without serial-fraction math. See [heuristics canon](https://github.com/darce/heuristics-canon).
- **Algorithmic complexity at scale** — nested iteration on hot path. See [heuristics canon](https://github.com/darce/heuristics-canon).
...
4. Read only enough adjacent code to understand behavior and dependencies. Include callers and tests before prescribing moves. When a codebase-graph MCP is connected, prefer `trace_path(direction='in')` to enumerate callers and `get_architecture` to size change scatter before Move/Extract/Rename (see Codebase-graph capability below).
5. Search for existing evaluations or tracked findings to avoid duplicate work.
6. **Optional quantitative prioritization (Python monorepo):** needs an executable workspace interpreter (`WORKSPACE_PYTHON`, default `.venv/bin/python` after `make sync` / `task-start`; override with `WORKSPACE_PYTHON=/path/to/python` if missing). When no `coverage.py` JSON report exists yet, produce one with `make crap-coverage` (writes `coverage.json` at the repo root). Then run `make crap-report COVERAGE=<path> PATHS=<src>` for a ranked CRAP list (complexity × coverage). By default only methods present in the coverage JSON are ranked; unmeasured high-CC methods are listed separately — pass `INCLUDE_UNMEASURED=1` for the classic cov=0 ranking of every scanned unit. Treat the report as an **advisory** change-risk ranking only — not a quality gate or coverage target ([TEST-11] / proxy-outcome-integrity). CRAP ranks *where to look*; this skill still names *what to do*.

## Codebase-graph capability (optional)

If a codebase-graph MCP is connected (exposes `get_architecture`, `trace_path`, `detect_changes`), use it near Phase 1 step 4:

- `trace_path(direction='in')` to enumerate callers before prescribing Move, Extract, or Rename.
- `get_architecture` to size change scatter and ownership boundaries across the target scope.

Skip when absent — the skill remains valid with direct file reads and tests only.
...
After state-changing MCP writes, refresh the operator view per [../handoff-lifecycle/SKILL.md](../handoff-lifecycle/SKILL.md) step 5 (dashboard policy lives there — do not restate it here).
...
- [../../rules/branch-review-python.md](../../rules/branch-review-python.md) — radon cyclomatic grades (CC signal only)
- implementation note / `make crap-report` — CRAP join of CC × coverage (advisory prioritizer)