...
- On `admission_deferred` / `admission_refused` (internal), `vm_measured_saturation`, or `vm_admission_unmeasured`, do not retry in a tight loop; surface the typed outcome and wait for host recovery or orchestrator guidance.
