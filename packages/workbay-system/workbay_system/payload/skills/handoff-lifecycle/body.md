...
Semantic capability: when consuming `semantic_reinjection_packet` (or other semantic surfaces), read response `embeddings_mode` + `semantic_degrade_reason` and follow [semantic-capability](https://github.com/darce/heuristics-canon) — surface a non-null degrade; never treat lexical-only output as full semantic success under a verified install. Do not invent FTS synonyms for `search_handoff`; call `find_related_prior_work` for like-this / paraphrase queries and read `status` / `embeddings_mode` / `semantic_degrade_reason` before treating empty `results` as "nothing similar."
...
1a. Do not invent FTS synonyms; call `find_related_prior_work` for like-this / paraphrase queries and read `status` / `embeddings_mode` / `semantic_degrade_reason` before treating empty `results` as "nothing similar."
