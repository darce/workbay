...
**Remote-mode reviewer pin (`execution_mode=remote_only`).** When the install is remote-provisioned, the coordinator pins exactly **one** reviewer lane to `grok-remote` via `materialize_offload_lane_manifest(preferred_backend=grok-remote)` before dispatch. Findings land under the existing scoped `<coordinator>-REV-<round>-<letter>` `task_ref` and merge back via the existing `review_findings` merge operation. On probe failure under `remote_only`, the run **fails** with the typed `remote_required` outcome recorded as a `routing_downgrade`-convention decision — **no** silent in-process substitution. Under `local_ok` the deterministic fallback above applies unchanged. See [../../docs/workbay/rules/offload-remote-playbook.md](../../docs/workbay/rules/offload-remote-playbook.md).


## Core Process

0. **Ensure coordinator task scope before any cwd-resolving MCP read.** If `get_handoff_state(sections="identity")` returns `Ambiguous active task`, run `make task-reap` (dry-run) to see closeable vs active rows; `make task-reap REAP_ARGS=--apply` closes only closeable rows, then re-run `make context` (exits `2` on this ambiguity). If no task is active and this is ad-hoc work on main, register a coordinator MAINT task first — `set_handoff_state(task_ref="MAINT-<slug>-<YYYYMMDD>", objective="...", status="in_progress", target_branch="main")` — before step 1.
...
8. Record **exactly one** combined branch-mode review run for the round: `review_runs(review={"operation": "record", "review_mode": "branch", ...})` with `subject_path` pointing at the branch under review. Do not record per-slice or per-reviewer `review_runs` rows on the coordinator — reviewer sub-runs remain under their scoped `task_ref`s as the audit trail only.
...
- The merge operation (`review_findings(review={"operation": "merge", ...})`) has produced N × K rows under the coordinator `task_ref` with `merged_from` provenance on every row (N = reviewer count, K = per-reviewer finding count).
