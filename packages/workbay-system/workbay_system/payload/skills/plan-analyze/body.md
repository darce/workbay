...
Surface likely planning problems early, record them as planning findings under a `plan-analyze-*` session (organizational prefix only), and hand the artifact off to `planning-review` only after the cheap gaps are understood.
...
This skill owns triage only. It records planning-mode findings and a `review_runs(review={"operation": "record", "review_mode": "planning", ...})` entry with a `plan-analyze-*` session prefix so operators can tell triage rows from formal planning-review runs in MCP. It does not replace the formal planning-review run, and no make target prechecks that prefix.
...
5. Record a planning-mode review run whose `session` starts with `plan-analyze-` so operators can distinguish triage rows from formal review runs in MCP (not a make-target precheck).
...
| "I recorded `plan-analyze-*`, so `make plan-review` is gated / safe to skip." | No consumer implements a plan-analyze session precheck; the prefix is organizational only. | Run formal `planning-review` when the artifact needs the planning gate. |
...
| The review run session does not start with `plan-analyze-` | Step 5: fix the session prefix so triage rows stay distinguishable from formal review in MCP (not a make precheck). |
...
- Analysis findings are recorded in MCP with `review_mode="planning"` under a `plan-analyze-*` session (organizational prefix; not a make precheck).
- A planning-mode review run exists for the triage pass and is distinguishable from formal planning-review runs by its `session` prefix in MCP.
