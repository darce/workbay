...
Allocation policy (owned by the `offload` skill, *Allocation and gate policy*): re-offload a fix as a lane slice only when it has a deterministic red test, localized scope, no design decision, and batches ≥2 findings or exceeds the lane fixed overhead — otherwise fix inline. Triage and `resolve` writes stay with the orchestrator; mid-loop handoff reads are identity-only. Remote preference: on a `remote_only` install (`--with-remote`), delegation-eligible mechanical slices default to `grok-remote` at high effort via `/wb-offload`; a `remote_required` refusal (or a failed remote probe) is surfaced as a recorded blocker, never silently downgraded to a local backend ([offload-remote-playbook](../../docs/workbay/rules/offload-remote-playbook.md)).
...
2. Record the canonical slice-complete decision via `close_slice(task_ref=<active>, author_tag=<tag>, work_ref=<task>, slug="autofix", rationale=<structured summary with the four required ## sections>, session=<session>, expected_revision=<rev>, actor={"commit_sha":<HEAD>})`. `close_slice` is the canonical write path because it enforces the four required rationale sections (`## Changes`, `## Verification`, `## Schema / Contract Changes`, `## Open Threads`); plain `record_event(event={"event_kind":"decision","decision":"<tag>_slice_complete_*"})` passes the id regex but skips that structural gate and the post-loop close check then fails the `current_commit_handoff` audit on a malformed rationale.
...
5. Any defects observed mid-loop that deserve review tracking are recorded via `review_findings(review={"operation":"record","review_mode":"branch", ...})`. Close them through the commit-backed `review_findings(review={"operation":"resolve", ...})` path, not manual `update(status="fixed")` — the `guard-review-finding-resolve` hook rejects that combination. Those must be closed or deferred before the post-loop gate; they count as open findings on the active task.
...
- **Codex / Copilot**: no `ScheduleWakeup` primitive. Iterate inline unconditionally. Detect the harness using the same routing table the `/wb-review-slice` skill documents.
...
| "The failing test is on `main` but the fix is tiny — I'll just edit and commit on `main`." | The branch-isolation hooks reject code edits on `main`/`master`. Running `/wb-auto-fix` without a feature branch burns the precondition error every iteration. | Create the feature branch first via `make task-start TASK=<id>`, then re-invoke `/wb-auto-fix`. |
...
- [../investigate/body.md](../investigate/body.md)
- [../review-parallel/body.md](../review-parallel/body.md)
