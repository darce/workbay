...
| No open findings on task | `review_findings(review={"operation": "list", "status": "open"})` | `items: []` or explicit list of open items to address |
| Verdict recorded | `review_runs(review={"operation": "list", "task_ref": ...})` | Entry with matching `review_run_id` and `verdict` |
