...
- MCP-capable hosts: use the compound MCP tools `manage_orchestrator(operation=...)` (valid operations: `start`, `status`, `pause`, `resume`, `stop`, `single_cycle`) and `manage_worker(task_ref=..., lane_id=..., action=...)` (valid actions: `start`, `stop`, `resume`, `status`, `event_history`, `start_all`) — there are no standalone `orchestrator_status`/`worker_start`-style tools; every orchestrator or worker daemon operation routes through one of these two typed surfaces.
...
1. Inspect daemon state before changing anything:
   - `make daemon-status`
   - `manage_orchestrator(operation="status")`
...
2. For pause and resume, use the supported pause sentinel flow:
   - `make daemon-pause`
   - `make daemon-resume`
   - `manage_orchestrator(operation="pause")`
   - `manage_orchestrator(operation="resume")`
...
4. For restart or recovery, stop cleanly first (`manage_orchestrator(operation="stop")`), then start again (`manage_orchestrator(operation="start", task_ref=...)`) through the orchestrator controls. Treat `OrchestratorLock` as the source of truth for stale-lock checks.

### Worker daemon

1. Inspect worker status first:
   - `make worker-daemon-status TASK=<task-ref> LANE=<lane-id>`
   - `manage_worker(task_ref=<task-ref>, lane_id=<lane-id>, action="status")`
   - or use `daemon_status()` in `worker_daemon_ctl.py`
2. Start through the supported control path:
   - `manage_worker(task_ref=<task-ref>, lane_id=<lane-id>, action="start")`
   - `daemon_start()`
   - To start every lane worker declared for a task in one call, use `manage_worker(task_ref=<task-ref>, action="start_all")` — it resolves the task's lane manifest, respects lane dependency ordering, and reports a per-lane result list.
3. Stop cleanly first:
   - `make worker-daemon-stop TASK=<task-ref> LANE=<lane-id>`
   - `manage_worker(task_ref=<task-ref>, lane_id=<lane-id>, action="stop")`
   - `daemon_stop()`
4. Resume only when a worker was intentionally paused:
   - `make worker-daemon-resume TASK=<task-ref> LANE=<lane-id>`
   - `manage_worker(task_ref=<task-ref>, lane_id=<lane-id>, action="resume")`
   - `daemon_resume()`
