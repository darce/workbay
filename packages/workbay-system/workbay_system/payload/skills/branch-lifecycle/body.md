...
7. Run `make handoff-close-check` on the branch HEAD. The branch is not merge-ready until the enforced gate passes. **Read the verdict in its own shell call** — never chain `handoff-close-check` or `review-ready` with merge / `task-finish` in one Bash invocation via `&&`, `;`, or `|`. Chaining skips operator review of a NOT READY verdict (the failure mode this step exists to catch).
...
9. Merge the reviewed branch and return the root worktree to `main`. Do **not** delete the feature branch by hand — `make task-finish` does it as the final step (after the linked worktree is gone), so the branch is still live for the handoff write-side guard at the time the row is set to `done` and archived. Same anti-chaining rule as step 7: merge and `task-finish` are separate operator-visible calls, never glued to a prior gate command.
...
| "I'll chain close-check and merge in one Bash line — if the gate fails the rest won't run." | `;` still continues; even `&&` hides the NOT READY body from a separate agent turn so the operator never reads the blocking reasons. | Run step 7, read the receipt, then run merge/task-finish in a later call. |
