...
| `task-start` created `feature/<task-ref>` with no `-plan<NNNN>` segment though the task implements a numbered plan (and `plan_path` came back `null`) | Step 1: `PLAN=` was omitted. Tear down the branch/worktree and re-run `make task-start TASK=<task-ref> PLAN=docs/plans/<NNNN>-*.md OBJECTIVE="..."`; the segment + `task_plan_path` link only auto-append from `PLAN=` or a pre-existing live row. |
