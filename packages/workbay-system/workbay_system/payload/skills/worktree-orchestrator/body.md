...
- Use [../../../docs/workbay/rules/branch-pipeline.md](../../../docs/workbay/rules/branch-pipeline.md) for the deterministic branch state machine (implement → review/adjudicate → merge → reap) and its failed-turn recovery arms; every branch a lane produces moves through that pipeline.
...
- If host admission refuses fanout, shrink width or wait; do not loop with overrides. Fan-out width for off-box lanes is whatever measured admission admits: on `vm_measured_saturation` wait for that gauge, on `vm_admission_unmeasured` check the VM probe; shrink width applies to local host-memory refusals only.
