...
- Use [../../rules/reasoning-discipline.md](../../rules/reasoning-discipline.md) for the reasoning posture underneath this skill (diagnose-before-execute, position-with-counter-case, escalation tiers).
- Semantic search / recall suggestions: consult [semantic-capability](../../docs/workbay/rules/semantic-capability.md) for `embeddings_mode` + `semantic_degrade_reason` before treating semantic results as authoritative.
...
If a codebase-graph MCP is connected (exposes `get_architecture`, `trace_path`, `detect_changes`), use it to trace failures across symbols and enumerate affected call sites. Skip when absent.
