...
- Use [reasoning-discipline](https://github.com/darce/heuristics-canon) for the reasoning posture underneath this skill (diagnose-before-execute, position-with-counter-case, escalation tiers).
- Semantic search / recall suggestions: consult [semantic-capability](https://github.com/darce/heuristics-canon) for `embeddings_mode` + `semantic_degrade_reason` before treating semantic results as authoritative.
...
0. **Ensure task scope before any cwd-resolving MCP read.** `search_handoff`, `review_findings`, `get_handoff_state` without `task_ref` all resolve from cwd. On cold start:
   ...
   - **`Ambiguous active task` error:** use the canonical escape hatch in [lifecycle-recovery.md § Surface 5](../../rules/lifecycle-recovery.md#surface-5-ambiguous-active-task-raise) (`task_ref=` / `TASK=`, or `make task-reap` then `make context`; exit `2` on ambiguity).

1. Collect symptoms before forming hypotheses:
   - exact error output
   - reproduction steps
   - recency checks with recent git history
   - related MCP findings via `search_handoff(...)` (tokens/IDs) and `review_findings(review={"operation":"list", ...})`. For paraphrase / like-this neighbors do not invent FTS synonyms; call `find_related_prior_work` and branch on `status`.
...
3. Analyze the symptom against common defect patterns:
   ...
   - config drift
   - regression
4. Form one specific, testable hypothesis at a time and inspect the suspected boundary. When a codebase-graph MCP is connected, use the [Codebase-graph capability](#codebase-graph-capability-optional) block — e.g. `trace_path` (direction=both) to walk RAISES/THROWS edges and `query_graph` for call-site fan-out around the suspected symbol.
...
8. Close the investigation finding correctly. Prefer the commit-backed `review_findings(review={"operation":"resolve", "all_open":true, ...})` over manual `update(status="fixed")` — it classifies the finding against the current commit and auto-populates `verified_commit_sha` on a clean tree. For a finding whose `task_ref` is orphaned or already `done` (no live worktree required), use `review_findings(review={"operation":"disposition", "status":"fixed"|"wontfix"|"deferred", ...})` instead.
...
If a codebase-graph MCP is connected (exposes `get_architecture`, `trace_path`, `detect_changes`, `query_graph`), use it to trace failures across symbols and enumerate affected call sites. Prefer `trace_path` (direction=both) for RAISES/THROWS walks and `query_graph` for fan-out. Skip when absent.
...
- [review](../review/body.md)
- [lifecycle-recovery.md](../../rules/lifecycle-recovery.md) — Ambiguous active-task escape hatch (Surface 5)
