...
- Cite stable security rule IDs (`SEC-01`..`SEC-10` and siblings) from the [heuristics canon security lexicon](https://github.com/darce/heuristics-canon); never pin a canon version or date. Those `SEC-*` IDs are **rule citations**, not this skill's finding-id prefix (see Recording Findings).
...
**Finding ID prefix**: `SECAUD-<n>` (e.g., `SECAUD-01`, `SECAUD-02`). Do **not** use `SEC-<n>` — that namespace is reserved for heuristics-canon security rule IDs cited in bracket form by review guides.
...
When recording 3 or more findings, use `review_findings(review={"operation": "batch_record", ...})`.

In comprehensive mode, mark uncertain findings with `TENTATIVE` prefix in description.

## Verdict and Closure

1. Summarize findings: `review_findings(review={"operation": "list", "task_ref": "__repo__", "review_mode": "release_audit"})`.
...
- A `review_runs(review={"operation": "record", ...})` entry exists for this audit.
- A `record_event(event={"event_kind": "decision", ...})` entry exists with the audit verdict.
- Response includes `Handoff updated: yes`.

## See Also

- [../review/body.md](../review/body.md)
- [../../docs/workbay/contracts/](../../docs/workbay/contracts/)
- [../../rules/development-workflow.md](../../rules/development-workflow.md)
- [heuristics canon](https://github.com/darce/heuristics-canon) — security family rule IDs (`SEC-*`)
