...
Remote preference: on a `remote_only` install (`--with-remote`), delegation-eligible mechanical slices default to `grok-remote` at high effort via `/wb-offload`; a `remote_required` refusal (or a failed remote probe) is surfaced as a recorded blocker, never silently downgraded to a local backend ([offload-remote-playbook](../../docs/workbay/rules/offload-remote-playbook.md)).
...
1. Load the active task plan and identify the next slice worth shipping. If prior slices are unclear, use `search_handoff` to find the latest slice summaries first (tokens/IDs only). When mining prior art by paraphrase / like-this, call `find_related_prior_work` — do not invent FTS synonyms — and read `status` / `embeddings_mode` / `semantic_degrade_reason` before treating empty `results` as "nothing similar."
...
6. Implement the minimum to make the path work end to end. Keep each layer thin enough that the test still explains the whole diff. When the slice touches a shared contract, payload shape, status code, envelope, or enum, follow [contract-change-checklist.md](../../../docs/workbay/rules/contract-change-checklist.md) and update the owning contract in the same slice.
7. Re-run the targeted checks and record passing evidence with `record_event(event={"event_kind": "test_result", "passed": true, ...})`.
8. Check the diff boundary and the **Produce-first shape gate** above. If the change now spans multiple independent user paths or grows a second responsibility, split it before commit. Before `make slice-commit`, run `detect_changes` (codebase-graph MCP when connected) to confirm the diff footprint matches the intended slice and there is no accidental blast radius.
9. Close the slice with `make slice-commit TASK=<task-ref> MSG="..."`, then refresh task context if more slices remain.

**Refactoring cadence & scale cues** (rationale in [heuristics canon](https://github.com/darce/heuristics-canon)):
...
**Produce-first shape gate** (apply while writing the slice — complements step 8 diff-boundary check; rationale in [heuristics canon](https://github.com/darce/heuristics-canon)):

- **Cohesion / "and"-test** — a slice objective with "and" usually hides two responsibilities; split before coding. See [heuristics canon](https://github.com/darce/heuristics-canon).
- **Size / nesting budget** — prefer ≤~400 lines and ≤3 nesting levels per unit; Extract when exceeded. See [heuristics canon](https://github.com/darce/heuristics-canon).
- **Extract on second responsibility** — if the diff grows a second concern, stop and Extract before expanding. See [heuristics canon](https://github.com/darce/heuristics-canon).
- **Strategy-map / outcome enum** — prefer enum-keyed handler maps over fat switches and outcome enums over deceptive booleans. See [heuristics canon](https://github.com/darce/heuristics-canon).
...
If a codebase-graph MCP is connected (exposes `get_architecture`, `trace_path`, `detect_changes`, `query_graph`), use it to enumerate call sites and trace error paths while sizing a slice, and to confirm pre-commit diff footprint via `detect_changes`. Prefer `query_graph` over TESTS edges when checking that new code has covering tests. Skip when absent.
...
- [../../../docs/workbay/rules/contract-change-checklist.md](../../../docs/workbay/rules/contract-change-checklist.md)
- [../../../docs/workbay/lifecycle-map.md](../../../docs/workbay/lifecycle-map.md)
- [../../../docs/workbay/rules/development-workflow.md](../../../docs/workbay/rules/development-workflow.md)
