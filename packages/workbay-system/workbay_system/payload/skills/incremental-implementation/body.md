...
Remote preference: on a `remote_only` install (`--with-remote`), delegation-eligible mechanical slices default to `grok-remote` at high effort via `/offload`; a `remote_required` refusal (or a failed remote probe) is surfaced as a recorded blocker, never silently downgraded to a local backend ([offload-remote-playbook](../../docs/workbay/rules/offload-remote-playbook.md)).
