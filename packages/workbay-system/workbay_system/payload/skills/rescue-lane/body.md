...
| Outcome is `vm_measured_saturation` | Wait for `binding_gauge` headroom; never raise a count to get past it. Then re-dispatch. Do not rescue. |
| Outcome is `vm_admission_unmeasured` | Probe fault: inspect the VM slice files; redispatch after. Do not rescue. |
