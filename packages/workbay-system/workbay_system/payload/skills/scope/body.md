...
2. Ask 3-5 targeted questions via `AskUserQuestion` before producing any plan. Cover scope, completion signals, edge cases, non-functional constraints, and explicit not-doing boundaries. When triggers apply, also run the **Non-Functional Intake** question-bank below. When a codebase-graph MCP is connected, optionally call `get_architecture` first to ground scope/coupling questions in the real package layout before asking the operator; skip when the capability is absent.
3. Record the resulting answers as MCP decisions with `record_event(event={"event_kind": "decision", ...})` so later planning sessions can reuse the same intake context.
...
6. Close intake by emitting eval suite specs as DATA. See **Eval Specs**.
7. Hand implementation to remote lanes as two separate dispatches — RED tests first, implementation second. See **Dispatch Decomposition**.
8. When intake is complete and the on-main `MAINT-*` row is no longer needed, close it with `make plan-done TASK=<maint-ref>` (not `make task-finish`).

## Non-Functional Intake

Ask only when the trigger applies (keeps `scope` within budget). Rationale lives in [heuristics canon](https://github.com/darce/heuristics-canon).
...
Close intake by emitting one or more eval suite specs as DATA. Specs live in `packages/workbay-system/config/evals/` and must match the registered contract in `packages/workbay-system/config/evals/schema.json`. The runner already reads this shape; do not invent field names. Copy the live example at `packages/workbay-system/config/evals/handoff-integrity.json`.

Required fields:

- `suite_id` — unique id consumed by `make evals-list` and `runner.py run --suite`
- `command` — invocation string for the suite
- `threshold` — object; `kind` is `status` (`required_status`) or `max_failures` (`max_failures`)
- `evidence_sink` — ledger path, typically `.task-state/evals/results.jsonl`
- also required: `version` (must equal schema `suite_version`, currently `1`), `owner`, `description`

Optional on live suites: `mirror_failures_to_handoff` (bool), `tags` (string array).

Each success criterion from the intake gets a corresponding eval case. A criterion with no eval is not a criterion — it is a hope.

**Current constraint:** a suite dropped into `config/evals/` is listed by `make evals-list` but is not runnable. `resolve_executor()` in `packages/workbay-system/scripts/workbay_evals/registry.py` looks the suite id up in a hardcoded `EXECUTOR_BY_SUITE_ID` dict. Until a generic executor lands, a new suite also needs an executor registered there. Plan for that registration as part of the scoped work. Do not fix it here — a separate lane owns it.

## Dispatch Decomposition

Hand implementation to remote lanes as two separate dispatches, never one.

1. A **RED** dispatch that writes only failing tests, derived from the eval specs and success criteria. It is forbidden from touching implementation source. It is finished when the tests exist, collect cleanly, and fail.
2. An **implementation** dispatch, sent only after the RED tests are committed, whose job is to make those existing tests pass.

When one lane owns both, the tests get written to fit the code and the red gate never existed. Splitting the dispatch means the second lane inherits tests it cannot negotiate with, so no enforcement is needed.
...
| Success criteria have no corresponding eval case | Step 6: emit the eval spec; a criterion with no eval is a hope. |
| Implementation is about to be handed to one remote lane that writes tests and code together | Step 7: split into RED then implementation. |
...
- Eval suite specs were emitted as DATA in `packages/workbay-system/config/evals/`, one case per success criterion.
- Implementation was decomposed into a RED dispatch and a later implementation dispatch.
