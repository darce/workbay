...
RULES: tuple[Rule, ...] = (
    ...
    Rule(
        "repo-name-assumption", re.compile(rf"\b{_LEGACY_REPO_NAME}(?:-[a-z0-9-]+)?\b")
    ),
    ...
    try:
        for target in _iter_targets(repo_root):
            for file_path in _iter_files(target.path):
                findings.extend(
                    _iter_findings(
                        file_path, repo_root=repo_root, category=target.category
                    )
                )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"lint-hoisted-paths: infrastructure error: {exc}", file=sys.stderr)
        return 1

    findings, exit_code = lint_hoisted_paths(repo_root=ctx.target_root)
    if findings:
        heading = (
            "lint-hoisted-paths: FAILED"
            if not findings[0].startswith("infrastructure error:")
            else "lint-hoisted-paths: ERROR"
        )
        stream = sys.stderr
