"""Canonical WorkBay path *name* definitions for the runtime install root and docs mirror.

Defines the directory and file names (for example ``RUNTIME_ROOT_DIRNAME``,
bootstrap/overlay manifest names, and ``DOCS_MIRROR_DIR``) for packages that
import them. This module is not a repository-wide path resolver: many call
sites still hardcode the same strings, so a name change here is not a one-line
flip across the tree (NAME-03).
"""
...
def runtime_root_path(base: Path, *parts: str) -> Path:
    ...
    Currently has no callers in the repository; kept for a later wiring slice.
    """
