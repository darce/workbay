...
class StackSource(BaseModel):
    """Install-ledger source provenance for the package stack.

    ``resolved_from`` is required. ``ref`` / ``sha`` are recorded only when
    known: an unattested git claim (``{"resolved_from": "git"}``) is a legal
    derive-path output and must round-trip.
    """

    model_config = ConfigDict(extra="forbid")

    resolved_from: Literal["git", "local", "unknown"] = Field(
        description=(
            "How the stack packages were resolved: a git checkout, a local "
            "file:// install, or an unresolvable/index source."
        )
    )
    ref: str | None = Field(
        default=None,
        min_length=1,
        description="Requested revision (git) or source URL (local) when known.",
    )
    sha: Sha40 | None = Field(
        default=None,
        description="Resolved 40-char git commit id when known.",
    )
...
class EmbeddingsOperatorPolicy(BaseModel):
    """Persisted operator embeddings policy (implementation note E1).

    Distinct from ``embeddings_mode``, which remains the install receipt.
    ``policy_revision`` is monotonic. ``origin`` records how the policy
    was chosen; omitted history cannot be recovered as explicit intent.
    """
...
class EmbeddingsInferenceReceipt(BaseModel):
    """Additive inference evidence; never a substitute for operator policy."""
...
class BootstrapManifest(BaseModel):
    ...
    stack_source: StackSource | None = Field(
        default=None,
        description=(
            "Source provenance for the package stack (requested ref, resolved "
            "SHA, and how it was obtained) so 'is this really vX.Y.Z?' is "
            "answerable from the ledger. None for git_overlay/worktree "
            "installs and for older package manifests that predate the field."
        ),
    )
    ...
    execution_mode: Literal["local_ok", "remote_only"] = Field(
        default="local_ok",
        description=(
            "Offload execution policy recorded at install time. 'remote_only' "
            "(install --with-remote) declares that subagent implementation "
            "work must dispatch to the remote gate backend; 'local_ok' — the "
            "default, and the value assumed when the field is absent from an "
            "older manifest — keeps local offload allowed."
        ),
    )
    embeddings_mode: Literal["unspecified", "verified", "disabled"] = Field(
        default="unspecified",
        description=(
            "Semantic-embeddings capability recorded at install time. "
            "'verified' (install --with-embeddings) means the [embeddings] "
            "extra and digest pin were hard-verified at install, so semantic "
            "features may rely on them; 'disabled' (install --no-embeddings) "
            "means the operator opted out; 'unspecified' — the default, and "
            "the value assumed when the field is absent from an older "
            "manifest — preserves best-effort provisioning behavior. "
            "Historical 'verified' is a provisioning receipt, not a promise "
            "of current inference."
        ),
    )
    embeddings_operator_policy: EmbeddingsOperatorPolicy | None = Field(
        default=None,
        description=(
            "Operator embeddings/reinjection policy (implementation note E1). None when "
            "an older ledger has not been migrated. Distinct from "
            "embeddings_mode, which remains the install receipt enum."
        ),
    )
    embeddings_inference_receipt: EmbeddingsInferenceReceipt | None = Field(
        default=None,
        description=(
            "Additive current-inference receipt (model identity, digests, "
            "probe). Absent until real verification evidence exists; enable "
            "must not manufacture this field."
        ),
    )
    codemap_mode: Literal["enforced", "available", "off"] | None = Field(
        default=None,
        description=(
            "Codemap-first source-discovery policy recorded at install time. "
            "'enforced' refuses source discovery in favor of codemap; "
            "'available' allows with an advisory; 'off' allows. None when "
            "omitted from an older ledger — omission is not explicit "
            "available or off. Typed reads live in capability_mode."
        ),
    )
...
def _read_manifest_capability(repo_root: Path | str, field: str) -> str | None:
    """Best-effort read of one capability field from the bootstrap manifest.

    Capability readers must be rollback-safe in both directions: a missing
    manifest, unreadable JSON, or an unknown/missing field value never raises —
    consumers fall back to the field's pre-flag default so an old build reading
    a new ledger (and vice versa) degrades to prior behavior.
    """
    root = Path(repo_root)
...
def load_execution_mode(repo_root: Path | str) -> Literal["local_ok", "remote_only"]:
    """Read the install-time offload execution policy for ``repo_root``.

    The single sanctioned reader for ``execution_mode`` — dispatch-side
    consumers (orchestrator offload defaults/refusal) call this instead of
    parsing the ledger ad hoc. Any value other than a literal ``remote_only``
    (including a missing manifest or field) reads as ``local_ok``.

    This adapter keeps its fail-open mapping and is not the 64 KiB typed
    reader; ``load_codemap_mode`` in ``capability_mode`` consumes ModeRead.
    """
    value = _read_manifest_capability(repo_root, "execution_mode")
    return "remote_only" if value == "remote_only" else "local_ok"


def load_embeddings_mode(
    repo_root: Path | str,
) -> Literal["unspecified", "verified", "disabled"]:
    """Read the install-time semantic-embeddings capability for ``repo_root``.

    The single sanctioned reader for ``embeddings_mode`` — semantic features
    gate hard reliance on embeddings on ``verified``. Any value other than the
    known literals (including a missing manifest or field) reads as
    ``unspecified``.
    """
    value = _read_manifest_capability(repo_root, "embeddings_mode")
    return value if value in ("verified", "disabled") else "unspecified"


def load_stack_source(repo_root: Path | str) -> dict[str, str] | None:
    """Read install-ledger stack provenance for ``repo_root``.

    The single sanctioned reader for ``stack_source``. Missing manifest,
    unreadable JSON, absent field, or a value that fails the wire shape
    never raises: consumers treat ``None`` as "provenance unrecorded" so
    an old build reading a new ledger (and vice versa) degrades rather
    than crashing.
    """
    root = Path(repo_root)
    for name in paths.MANIFEST_NAME_PRECEDENCE:
        ...
        try:
            ...
            return None
        if not isinstance(data, dict) or "stack_source" not in data:
            return None
        try:
            parsed = StackSource.model_validate(data["stack_source"])
        except Exception:  # noqa: BLE001 - capability readers never raise
            return None
        dumped = parsed.model_dump(exclude_none=True)
        resolved = dumped.get("resolved_from")
        if not isinstance(resolved, str):
            return None
        out: dict[str, str] = {"resolved_from": resolved}
        ref = dumped.get("ref")
        sha = dumped.get("sha")
