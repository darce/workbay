...
### Fixed
- **The generated tool-serving index omitted the registered `lane_dag` orchestrator tool.** `tool_serving_index.py` had never been synced after `lane_dag` was registered, so any skill resolving tools through the index saw the tool as absent while the server served it.

## [0.3.1] — 2026-09-18

### Changed
- Embeddings policy and migration: the operator embedding policy is persisted and migrated (E1).
- `ModeRead` / `load_codemap_mode` read the codemap mode with fail-loud typed errors (C1).

## [0.3.0] — 2026-09-10

### Added
- Canonical reasoning-effort constants and strict validation now expose the explicit `max` effort to downstream dispatchers without silently rewriting invalid input.

## [0.2.7] — 2026-09-03

### Added
- A shared `ok=false` response-envelope contract.
