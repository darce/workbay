...
def _should_scrub(path: Path) -> bool:
    if path.suffix.lower() in _TEXT_SUFFIXES or path.name in {
        ...
        return True
    # Extensionless executables (git hooks like ``post-merge``/``pre-commit`` — git
    # invokes them by fixed name, so they CANNOT carry a suffix — plus CLI shims)
    # ship internal refs too. Classify by CONTENT, not extension: a leading ``#!``
    # shebang marks a scrubbable text script. The privacy scanner
    # (scripts/check_shipped_privacy.py) reads EVERY shipped file, so the scrub must
    # match that scope; an extension-only gate let a ``Plan NNNN`` token in the
    # extensionless ``post-merge`` hook leak into the built wheel/sdist.
    if not path.suffix:
        try:
...
class ScrubAtBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:
        ...
        if version == "editable":
            # Release only-include entries name remapped imports, which do not
            # exist at the project root in src layouts. Give hatchling the real
            # import roots so its normal .pth builder can keep installs editable.
            # Never force-include a scrubbed snapshot into an editable wheel.
            if not self.build_config.dev_mode_dirs:
                self.build_config.dev_mode_dirs.extend(
                    sorted({str(target.source_path.parent) for target in targets})
                )
            return
