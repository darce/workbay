...
from workbay_orchestrator_mcp.lanes import get_lane_activity, record_turn_metric
from workbay_orchestrator_mcp.orchestration.proc_footprint import (
    current_phys_footprint_mb,
    phys_footprint_source,
)


def _loaded_handoff_mcp() -> Any:
    """Return ``workbay_handoff_mcp`` only if it is already imported.

    Importing the package root executes ``__init__`` → ``api`` → ``compaction``
    → ``concept_embed_hook``. When the optional embeddings extra is present,
    that hook re-exports types from ``embeddings.store`` at module load, which
    lands ``workbay_handoff_mcp.embeddings`` in ``sys.modules``. Isolated
    ``_build_prompt`` renders must not pay that cost (S1 import-elimination).
    ``main()`` imports the package before calling ``_build_prompt``, so
    production artifact retrieval still works.
    """
    return sys.modules.get("workbay_handoff_mcp")
...
_HANDOFF_NOT_LOADED_ERROR = "workbay_handoff_mcp is not loaded"
_ARTIFACT_CONTEXT_UNAVAILABLE = "artifact context: (unavailable: handoff_package_not_loaded)"


def _mcp_get_artifact(*args: Any, **kwargs: Any) -> Any:
    pkg = _loaded_handoff_mcp()
    if pkg is None:
        raise RuntimeError(_HANDOFF_NOT_LOADED_ERROR)
    return pkg.get_artifact(*args, **kwargs)
...
def _mcp_search_artifacts(*args: Any, **kwargs: Any) -> Any:
    ...
    if pkg is None:
        raise RuntimeError(_HANDOFF_NOT_LOADED_ERROR)
    return pkg.search_artifacts(*args, **kwargs)


def _handoff_package_loaded() -> bool:
    """True iff ``workbay_handoff_mcp`` is already in ``sys.modules``.

    The artifact helpers require the package to be *loaded*, not merely
    installed. ``importlib.util.find_spec`` would be true whenever the
    package is on disk, which does not denote that precondition (NAME-04).
    Evaluated at call time so the guard tracks process lifetime.
    """
    return "workbay_handoff_mcp" in sys.modules


# test-patch surface: tests/test_lane_prompt_artifacts.py monkeypatches this
# name to a bool. Production binds a zero-arg callable so loadedness is
# checked at call time rather than frozen at import.
_ARTIFACT_SEARCH_AVAILABLE = _handoff_package_loaded


def _artifact_search_available() -> bool:
    """Resolve the patchable availability flag at call time."""
    flag = _ARTIFACT_SEARCH_AVAILABLE
    if callable(flag):
        return bool(flag())
    return bool(flag)


HEURISTICS_CANON_REL = "docs/workbay/rules/heuristics-canon.md"
...
from lane_test_cmd import classify_remote_test_cmd, classify_test_cmd
...
MAX_TEST_ITEMS = 4
MAX_GLOBAL_ITEMS = 6
MAX_BRIEF_REQUIRED_ACTIONS = 20
MAX_BRIEF_ARTIFACTS = 20
MAX_BRIEF_BODY_CHARS = 32000
ANSI = {
    "reset": "\033[0m",
    "red": "\033[31m",
    "yellow": "\033[33m",
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--backend", help="Effective execution backend selected by the launcher.")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _structured_text(text: str) -> str:
    """Preserve newlines and indent; only normalize line endings.

    ``_line`` is shared by assignment, action, blocker, finding, decision, test,
    global-row, CLI summary, artifact-query, and latest-report formatters. Brief
    bodies need a separate path so those callers stay collapsed.
    """
    return str(text).replace("\r\n", "\n").replace("\r", "\n")


def _bound_announced_text(text: str, *, limit: int, noun: str) -> str:
    """Cut ``text`` at ``limit`` and say so; never silently truncate."""
    if len(text) <= limit:
        return text
    omitted = len(text) - limit
    return f"{text[:limit]}\n... truncated {omitted} character(s) from {noun} (limit {limit})."


def _join_brief_detail_parts(header: str, parts: list[str]) -> str:
    """Join brief fields without rewriting the last body line.

    Compact one-line briefs stay on ``' | '``. When any field already contains a
    newline (a structured ``Body:`` block, fenced markdown, a list), join with
    newlines so Actions/Artifacts cannot glue onto a closing fence or list marker.
    """
    present = [part for part in parts if part]
    if not present:
        return header
    joiner = "\n" if any("\n" in part for part in present) else " | "
    return f"{header}: {joiner.join(present)}"
...
class PromptFormatter:
    ...
    def brief_message(message: dict[str, Any]) -> str:
        ...
        summary = _line(raw_summary) if raw_summary else ""
        body = _bound_announced_text(
            _structured_text(str(message.get("message") or "")),
            limit=MAX_BRIEF_BODY_CHARS,
            noun="brief body",
        )
        ...
        if summary:
            ...
            if body.strip():
                detail_parts.append(f"Body:\n{body}")
        elif body.strip():
            # Fallback arm: empty summary still renders the body, structure intact.
            detail_parts.append(body)
        if required_actions:
            detail_parts.append(
                "Actions: " + "; ".join(_bounded_items(required_actions, limit=MAX_BRIEF_REQUIRED_ACTIONS))
            )
        if artifacts:
            detail_parts.append("Artifacts: " + ", ".join(_bounded_items(artifacts, limit=MAX_BRIEF_ARTIFACTS)))
        return _join_brief_detail_parts(text, detail_parts)
...
def _actionable_state(activity: dict[str, Any]) -> dict[str, Any]:
    # Compare against the protocol string values from workbay_handoff_mcp.enums
    # (LaneMessageDirection, MessageStatus, ActionStatus, BlockerStatus,
    # FindingStatus). Do not import that package here: the package root pulls
    # embeddings.store whenever the embeddings extra is installed, and this
    # helper runs on every isolated _build_prompt render (S1 import-elimination).
    messages = [
        message
...
def _runtime_guidance(
    *,
    ...
    try:
        ...
        # A lane may legitimately have no manifest; the row-owned test_cmd still
        # produces a usable prompt. Crashing prompt construction here would turn a
        # recoverable absence into a dead lane.
        lane_config = {}
    manifest_test_commands = [str(item).strip() for item in lane_config.get("test_commands", []) if str(item).strip()]
    test_commands = list(manifest_test_commands)
    unvalidated_row_commands: list[tuple[str, str]] = []
    if not test_commands and isinstance(lane_row, dict):
        _lane_test_state, lane_test_cmd = classify_test_cmd(lane_row.get("test_cmd"))
        if lane_test_cmd:
            # Supply-state (held) is not admission. Reuse the dispatcher's
            # classifier with repository_root=None so prompt rendering does
            # not touch the filesystem; offload_pass re-admits with the real
            # root at dispatch time.
            admission, reason = classify_remote_test_cmd(lane_test_cmd, repository_root=None)
            if admission == "ok":
                test_commands.append(lane_test_cmd)
            else:
                unvalidated_row_commands.append((lane_test_cmd, reason))
    ...
    # PYENV_VERSION in a manifest command is a requirement of that command,
    # not evidence that every execution backend provisioned the named pyenv.
    # Interpreter selection must therefore always happen in the worker shell.
    pyenv_version = extract_pyenv_version(manifest_test_commands)

    provisions_lane_venv = False
    if backend:
        try:
            provisions_lane_venv = get_backend_spec(backend).capabilities.provisions_lane_venv
        except RuntimeError:
            # Unknown backends must not inherit a provisioning promise. Treating
            # an absent venv as sanctioned avoids falsely halting a local turn.
            provisions_lane_venv = False

    lines: list[str] = [
        "",
        "Lane interpreter:",
        "- From any directory inside the lane, resolve the lane root with `lane_root=\"$(git rev-parse --show-toplevel)\" || { echo 'cannot resolve lane root' >&2; exit 1; }`. Do not derive an interpreter path from an empty lane root.",
    ]
    if provisions_lane_venv:
        lines.extend(
            [
                '- In the same shell invocation where it is used, resolve the lane interpreter with `resolved_python="$lane_root/.venv/bin/python"`.',
                '- Repeat that resolution after every working-directory or shell-session change. The harness provisions this lane-root `.venv`; `if [ ! -x "$resolved_python" ]; then` stop and report `needs_guidance`.',
                "- The managed `.venv` is intentionally a symlink whose target outside the lane worktree is permitted. Do not use a virtualenv from an unrelated checkout.",
                "- Do NOT run bare `python3 -m pytest` as a fallback; backend PATH entries do not guarantee another Python launcher or `uv`.",
                '- Before trusting any test result, and again after the last edit, run `<resolved-python> -c "import <lane_module> as module; print(module.__file__)"` for the package this lane owns.',
            ]
        )
    else:
        lines.extend(
            [
                "- Repeat lane-root resolution after every working-directory or shell-session change. If the lane-root `.venv` is absent, this backend sanctions that state: use the backend-provided interpreter and do not report `needs_guidance` for absence alone.",
                "- Run each declared verification command without substituting bare `python3`; when it invokes Python, use that same backend-provided interpreter for the import-origin check.",
                '- Before trusting any test result, and again after the last edit, run `<backend-provided-interpreter> -c "import <lane_module> as module; print(module.__file__)"` for the package this lane owns.',
            ]
        )
    lines.extend(
        [
            "- If that import resolves outside the lane worktree, stop and report `needs_guidance`; do not treat the test result as passing.",
        ]
    )
    ...
    lines.extend(
        [
            "",
            "Import-origin control (Python package lanes only):",
            "- When lane verification exercises an importable Python project package, identify its module from the lane-owned `pyproject.toml` or setup configuration. Do not probe an unrelated package merely because it is importable in the ambient environment.",
            '- Before trusting any test result, and again after the last edit, run the selected interpreter with `-c "import <lane_module> as module; print(module.__file__)"`.',
            "- If the lane owns no importable Python package (for example, a package-less or `MODE=here` lane), skip this import-origin control; an unavailable unrelated import is not a reason to stall.",
            "- When the lane does own that package, if its import resolves outside the lane worktree, stop and report `needs_guidance`; do not treat the test result as passing.",
        ]
    )
    ...
    if app_root:
        lines.extend(
            [
                "",
                "Working directory:",
                f"- The lane manifest declares `{app_root}/` as the primary application directory. Run its test and build commands from there.",
                f'- From anywhere in the lane, enter it with `lane_root="$(git rev-parse --show-toplevel)" || {{ echo "cannot resolve lane root" >&2; exit 1; }}; cd "$lane_root/{app_root}"` before running lane verification commands.',
            ]
        )
    ...
    # Row-owned test_cmd is verification text only after classify_remote_test_cmd
    # admits it. Held-but-refused row text is labeled unvalidated so a no-op or
    # unsafe command is never presented as the command that verifies the work.
    # A manifest-owned pyenv pin augments the execution-time resolver without
    # replacing its lane-venv arm.
    if unvalidated_row_commands:
        lines.extend(["", "Row test_cmd (unvalidated; NOT a verification command):"])
        lines.extend(_bullet_lines([f"`{command}` — {reason}" for command, reason in unvalidated_row_commands]))
        lines.append("- Do not treat this as a verification command.")
    if pyenv_version:
        app_dir_command = (
            f'lane_root="$(git rev-parse --show-toplevel)" || {{ echo "cannot resolve lane root" >&2; exit 1; }}; cd "$lane_root/{app_root}"'
            if app_root
            else 'lane_root="$(git rev-parse --show-toplevel)" || { echo "cannot resolve lane root" >&2; exit 1; }; cd "$lane_root"'
        )
        lines.extend(
            [
                "",
                "Backend runtime notes:",
                f"- The manifest command requires `PYENV_VERSION={pyenv_version}`; this command text does not prove that the named pyenv is installed or exported by the execution backend.",
                f"- Prefer the executable lane interpreter selected above. Only when the lane `.venv` is unavailable, you may use the `{pyenv_version}` interpreter if `PYENV_VERSION={pyenv_version} pyenv which python` resolves to an executable path; otherwise keep the resolver's `python3` fallback.",
                "- Preserve the manifest's `PYENV_VERSION` assignment and arguments while invoking the selected interpreter. A missing named pyenv alone is not a reason to report `needs_guidance`.",
                f"- Enter the manifest application directory with `{app_dir_command}` first so imports resolve correctly.",
                "- For lane-owned temporary files, use the lane's `.task-state/tmp/` area rather than assuming the launcher exports `TMPDIR`.",
...
def _engineering_guidance_lines(worktree_path: str) -> list[str]:
    guidance_path = Path(worktree_path).expanduser() / HEURISTICS_CANON_REL
    if not guidance_path.is_file():
        return []
    return [f"engineering guidance: {HEURISTICS_CANON_REL}"]


# Warn at most once per process when WORKBAY_OFFLOAD_SEMANTIC_REINJECTION is ON but the
# service is unconfigured (S1 removed the in-process embedding load). A module-level guard
# keeps the operator signal loud without unbounded per-call stderr spam on every
# lane-prompt / slice-brief build (OBS-05: bounded telemetry, not per-call).
_SEMANTIC_UNCONFIGURED_WARNED = False
...
def _relevant_concepts_section(
    *,
    ...
    # D2/D3 (internal S1): the durable fix is REMOVAL, not
    # relocation. lane_prompt is a short-lived per-cycle worker subprocess; loading the
    # gte-base embedding model in-process pins a ~8.7 GB phys_footprint singleton that OOMs
    # a small host and blocks parallel offload. The in-lane model-load path is DELETED.
    # Flag OFF (default): unchanged S0 skip. Flag ON: degrade to a distinct marker so a
    # future single-owner embedding service (D5) is distinguishable from "legitimately no
    # concepts". [REF-05][REF-12][RES-13][API-09]
    # activity/task_ref/lane_id/orchestrator_root/objective are retained for the stable caller
    # interface (_build_prompt passes them by keyword); intentionally unused after the removal —
    # a D5 thin client would consume them again.
    if os.environ.get("WORKBAY_OFFLOAD_SEMANTIC_REINJECTION", "0").strip().lower() not in ("1", "true", "yes"):
        return ["relevant concepts: (skipped: offload_semantic_disabled)"], False
    # Flag ON no longer loads embeddings (S1 removal). Warn once per process so operators who
    # set the flag expecting reinjection do not treat the stub as a silent success — but do not
    # re-emit on every per-cycle lane-prompt build (would spam stderr and train operators to
    # ignore it; OBS-04/OBS-05).
    global _SEMANTIC_UNCONFIGURED_WARNED
    if not _SEMANTIC_UNCONFIGURED_WARNED:
        print(
            "WORKBAY_OFFLOAD_SEMANTIC_REINJECTION=1 but offload semantic service is unconfigured "
            "(S1 removed in-process embedding load; awaiting D5 single-owner service)",
            file=sys.stderr,
        )
        _SEMANTIC_UNCONFIGURED_WARNED = True
    return ["relevant concepts: (unavailable: offload_semantic_service_unconfigured)"], False
...
def _artifact_context_section(
    *,
    ...
    Exhausted budget returns ``[]``. A not-loaded handoff package with remaining
    budget returns the unavailability marker rather than silent empty.
    ...
    if not _artifact_search_available():
        return [_ARTIFACT_CONTEXT_UNAVAILABLE]
    lines: list[str] = []
    used = 0
...
def _build_prompt_sections(
    ...
    *,
    ...
    context_budget = [
        ...
        f"Per-brief required actions are capped at {MAX_BRIEF_REQUIRED_ACTIONS} items; artifacts at {MAX_BRIEF_ARTIFACTS}.",
        f"Per-brief body is capped at {MAX_BRIEF_BODY_CHARS} characters.",
...
def _build_prompt(
    ...
    *,
    ...
    if artifact_context:
        # Match the semantic degrade path: an (unavailable: ...) line is a
        # visible skip, not retrieved content. Wrapping it under Relevant
        # Artifacts and flipping used_artifact_context would report success
        # for a structural miss (PROV-06).
        if artifact_context[0].startswith("artifact context: (unavailable:"):
            lines.extend(["", *artifact_context])
        else:
...
def _emit_lane_prep_footprint_metric(
    *,
    task_ref: str,
    lane_id: str,
    orchestrator_root: Path,
    ...
    """Self-write ONE ``lane_prep`` turn_metric row capturing this short-lived
    ``lane_prompt`` subprocess's phys_footprint (D4/D6). It lands only on the full
    execution-turn render path (never on ``--check``/``--summary``), so a freshness gate
    must key off the worker-daemon liveness heartbeat, not "expect a periodic lane_prep
    row" — idle silence is not breakage ([OBS-08]).

    Observability ONLY: this MUST NOT call ``evaluate_admission`` or refuse any lane —
    ``COST_REMOTE`` stays ungated by directive (D4). Fail-open: any measurement or
    DB-write error is logged to stderr and swallowed so telemetry never blocks the
    render ([OBS-01][RES-13]).
    """
    try:
        cycle = _worker_cycle_marker(orchestrator_root, lane_id)
        phys_mb = current_phys_footprint_mb()
        # cost_class is resolved ONLY for a genuinely registered backend. Reusing
        # cost_class_for_backend's admission-safety fallback (unknown/None -> COST_HEAVY)
        # as an observability LABEL would mislabel the common unpinned local lane as
        # "heavy" and invert the COST_REMOTE dimension this signal exists to surface
        # (REF-16 leaky abstraction / PROV-06). Unregistered/absent backend -> None.
        ...
        if backend:
            ...
            except Exception:  # noqa: BLE001 - cost-class resolution is best-effort telemetry
                cost_class = None
        resolved_backend = backend or "lane_prompt"
        # Flag state only (PROV-06): post-S1 this does NOT mean reinjection ran.
        reinject_flag = os.environ.get("WORKBAY_OFFLOAD_SEMANTIC_REINJECTION", "0").strip().lower() in (
            "1",
            "true",
            "yes",
        )
        attribution = {
            "phys_footprint_mb": phys_mb,
            # A null measurement must not claim a concrete mechanism (PROV-06).
            "phys_footprint_source": phys_footprint_source() if phys_mb is not None else "unavailable",
            "pid": os.getpid(),
            "cost_class": cost_class,
            "lane_id": lane_id,
            "offload_semantic_reinjection_flag": reinject_flag,
        }
        # Latest-wins in the same DB transaction as the insert (RES-07 / CON-11).
        raw = record_turn_metric(
            session=f"lane_prep:{lane_id}:{cycle}",
            phase="lane_prep",
            ...
            task_ref=task_ref,
            attribution=attribution,
            replace_prior_same_phase=True,
        )
        payload = json.loads(raw) if isinstance(raw, str) else raw
        if isinstance(payload, dict) and payload.get("ok"):
            return payload
        return None
    except Exception as exc:  # noqa: BLE001 - telemetry must never break the render
        print(f"lane_prep footprint telemetry skipped: {exc!r}", file=sys.stderr)
        return None
...
def main() -> int:
    ...
    preferred_backend: str | None = args.backend
    ...
    state = _actionable_state(activity)
    # --check / --summary only need actionable state; skip semantic packet +
    # full prompt assembly (capacity-multiplier / chatty path).
    if args.check:
        if state["actionable"]:
            return 0
        if state["awaiting_orchestrator"]:
    ...
    if ctx_metrics:
        ...
        # Per-lane-prep phys_footprint telemetry (S2 / D4-D6): fires once per full
        # execution-turn render, after the --check/--summary early-returns. Gated on a
        # real render (non-empty ctx_metrics) so the WAITING/NO_WORK short-circuits
        # (_build_prompt -> (msg, {})) do NOT dilute the footprint samples. Observability
        # only — never gates a lane. Fail-open, writes to the DB + stderr, never stdout.
        _emit_lane_prep_footprint_metric(
            task_ref=args.task_ref,
            lane_id=args.lane_id,
            orchestrator_root=orchestrator_root,
            backend=preferred_backend,
