...
def _runtime_guidance(
    *,
    ...
    if not test_commands and isinstance(lane_row, dict):
        lane_test_cmd = str(lane_row.get("test_cmd") or "").strip()
        if lane_test_cmd:
    ...
    # Do not inspect the orchestrator host's worktree here.  Remote adapters
    # provision the execution filesystem after prompt construction, so the
    # worker must resolve the interpreter in-turn where commands will run.
    lines = [
        "",
        "Lane interpreter:",
        "- From any directory inside the lane, resolve the lane root with `lane_root=\"$(git rev-parse --show-toplevel)\"` in the same shell invocation where you use it.",
        "- Select the interpreter on the execution filesystem immediately before each Python command: `if [ -x \"$lane_root/.venv/bin/python\" ]; then resolved_python=\"$lane_root/.venv/bin/python\"; else resolved_python=\"$(command -v python3)\" || { echo 'python3 is unavailable' >&2; exit 1; }; fi`.",
        "- Repeat that resolution after every working-directory or shell-session change, then invoke Python as `\"$resolved_python\"`; prompt-build-time host filesystem state is not authoritative.",
    ...
    if app_root:
        lines.extend(
            [
                ...
                f'- From anywhere in the lane, enter it with `cd "$(git rev-parse --show-toplevel)/{app_root}"` before running lane verification commands.',
            ]
        )
    ...
    if pyenv_version:
        app_dir_command = (
            f'cd "$(git rev-parse --show-toplevel)/{app_root}"'
            if app_root
        ...
        lines.extend(
            [
                ...
                "- The launcher exports a writable lane `TMPDIR`; use `$TMPDIR` rather than assuming a cwd-relative temp path.",
