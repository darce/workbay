...
from lane_test_cmd import classify_test_cmd
...
def _runtime_guidance(
    *,
    ...
    try:
        ...
        lane_config = {}
    manifest_test_commands = [
        str(item).strip() for item in lane_config.get("test_commands", []) if str(item).strip()
    ]
    ...
    # Row-owned test_cmd is verification text only.  A manifest-owned pyenv pin
    # augments the execution-time resolver without replacing its lane-venv arm.
    if pyenv_version:
        app_dir_command = (
