...
def _mcp_latest_compaction_id(task_ref: str) -> str:
    """Return newest compaction_id for cache invalidation (fail-open to \"\").

    Uses ``get_latest_compaction`` — not ``get_compaction_status`` (disable-gate
    receipt only; no latest_compaction_id field).
    """
    from workbay_handoff_mcp.compaction import get_latest_compaction

    record = get_latest_compaction(task_ref=task_ref)
    if record is None:
        return ""
    return str(record.summary.compaction_id or "").strip()
...
def _load_semantic_reinjection_cache(
    ...
    *,
    ...
    if str(payload.get("compaction_id") or "").strip() != compaction_id:
        return None
    # Cross-pass cycle reuse: require matching anchors so cycle-0 disk hits from
    # a prior worker pass with different activity cannot re-serve stale concepts.
    if str(payload.get("anchor_fp") or "").strip() != anchor_fp:
        return None
    # Only successful selected packets are sticky within a cycle.
...
def _store_semantic_reinjection_cache(
    ...
    *,
    ...
    # Negative/degraded outcomes are not persisted (transient provider blips
    # must not freeze unavailable for the rest of the cycle).
    if not used:
        return
    try:
...
def _relevant_concepts_section(
    *,
    ...
    anchor_fp = hashlib.sha256("\n".join(anchor_texts).encode("utf-8")).hexdigest()[:16]
    ...
    if cache_cycle >= 0:
        ...
        cached = _load_semantic_reinjection_cache(cache_path, compaction_id=compaction_id, anchor_fp=anchor_fp)
    ...
    if status == "selected" and isinstance(relevant_lines, list) and relevant_lines:
        ...
        # Single source shared with slice_brief_packet (REV-HARM-A-001): both
        # clean the same semantic_reinjection relevant_lines producer, so the
        # logic must change together.
        from workbay_orchestrator_mcp.orchestration.slice_brief_packet import (
            clean_semantic_relevant_lines,
        )

        cleaned = clean_semantic_relevant_lines(relevant_lines)
        if cleaned:
            lines = cleaned
            used = True
        else:
