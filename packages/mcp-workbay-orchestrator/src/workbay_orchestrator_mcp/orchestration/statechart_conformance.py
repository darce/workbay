...
_TRANSITION_SCHEMA_KEYS: frozenset[str] = frozenset({"id", "from", "to", "guard", "guards"})
...
def check_machine_hygiene(chart: Mapping[str, Any]) -> list[str]:
    ...
    for key in ("outer", "child"):
        ...
        if not isinstance(initial, str):
            ...
            errors.append(f"{machine_id}: initial {initial!r} is not in states[]")

        terminals = _as_str_list(machine.get("terminals"))
        for terminal in terminals:
            if terminal not in state_set:
                errors.append(f"{machine_id}: terminal {terminal!r} not in states[]")
        ...
        for transition in transitions:
            ...
            if isinstance(src, str) and src not in state_set:
                errors.append(f"{machine_id} transition {tid!r}: from {src!r} not in states[]")
            for dest in _targets(transition):
                if dest not in state_set:
                    errors.append(f"{machine_id} transition {tid!r}: to {dest!r} not in states[]")
    return errors
    ...
    for key in ("outer", "child"):
        ...
        for state in states:
            if state not in reached:
                errors.append(f"{machine_id}: unreachable state {state!r} from initial {initial!r}")
    return errors
...
def check_terminal_reachability(chart: Mapping[str, Any]) -> list[str]:
    ...
    for key in ("outer", "child"):
        ...
        for state, ok in reach_map.items():
            if not ok:
                errors.append(f"{machine_id}: state {state!r} cannot reach any terminal in {terminals!r}")
    return errors
...
def check_gate_injectivity(chart: Mapping[str, Any]) -> list[str]:
    ...
    for gate in gates:
        ...
        if transition_id not in transition_index:
            errors.append(f"gate {gate_id!r}: orphan reference to unknown transition {transition_id!r}")
            continue
        ...
        if not _iter_guards(transition):
            errors.append(f"gate {gate_id!r}: referenced transition {transition_id!r} has no guard")
            ...
            for transition in transitions:
                ...
                for dest in _targets(transition):
                    if dest in terminals and tid not in gated_transition_ids:
                        errors.append(f"ungated inbound transition {tid!r} to closed-terminal {dest!r}")
    return errors
...
def check_parent_guard_contract(chart: Mapping[str, Any]) -> list[str]:
    ...
    for transition in transitions:
        ...
        for key in transition:
            if key not in _TRANSITION_SCHEMA_KEYS:
                detail = " matches child state name" if key in child_states else ""
                errors.append(
        ...
        for guard in _iter_guards(transition):
            ...
            for ref in _child_state_refs_from_guard(guard):
                if ref not in child_states:
                    errors.append(f"outer transition {tid!r}: child ref {ref!r} is not a child state")
                    continue
    ...
    for terminal in sorted(child_terminals):
        if terminal not in referenced:
            errors.append(f"child terminal {terminal!r} never appears in any outer guard evidence source")
    return errors
...
def check_nested_region(chart: Mapping[str, Any]) -> list[str]:
    ...
    if not isinstance(declared_child, str):
        ...
        errors.append(f"nested_region.child_machine {declared_child!r} does not match child.id {child_id!r}")
    ...
    for state in parent_states:
        if state not in outer_states:
            errors.append(f"nested_region.parent_states member {state!r} is not in outer.states[]")
    ...
    for transition in transitions:
        ...
        if non_terminal:
            errors.append(
                f"outer transition {tid!r}: nested-region exit evidence includes non-terminals {non_terminal}"
            )
    return errors
    ...
    if isinstance(declared, list):
        ...
        for item in declared_set:
            if item not in ALLOWED_EVIDENCE_CLASSES:
                errors.append(f"chart allowed_evidence_classes includes non-grounded class {item!r}")
        ...
        for transition in transitions:
            ...
            for guard in _iter_guards(transition):
                ...
                if not isinstance(evidence_class, str):
                    errors.append(f"{machine_id} transition {tid!r}: guard missing evidence_class")
                    continue
