...
# implementation note: parked SPA npm provision gated on WORKBAY_PROVISION_WEB.
# Identity comes from a monorepo-owned marker on the app root at runtime — never
# from a packaged path literal (private SPA names must not appear in wheel/sdist).
# Other package.json roots without the marker keep provisioning.
_PARKED_WEB_MARKER = ".workbay-parked-web"

# Canonical opt-in spelling. Must match root Makefile WB_PROVISION_WEB_ON:
#   WB_PROVISION_WEB_ON := $(filter 1,$(WORKBAY_PROVISION_WEB))
# Only the literal "1" enables. A falsy-set + .lower() approach diverged from
# Make's case-sensitive filter-out (FALSE off here, on in Make). A single
# truthy token keeps both consumers identical without Make case-folding.
_PROVISION_WEB_TRUTHY = "1"


def _provision_web_enabled() -> bool:
    """True only when WORKBAY_PROVISION_WEB is exactly the literal ``1`` (after strip).

    Unset, empty, ``0``, ``false``, ``FALSE``, ``no``, ``off``, ``true``, and
    every other spelling are off. See docs/workbay/environment-variables.md.
    """
    return os.environ.get("WORKBAY_PROVISION_WEB", "").strip() == _PROVISION_WEB_TRUTHY


def _is_parked_web_app(app_root: Path) -> bool:
    """True when the app root opts into provision-web gating via marker file.

    The monorepo places ``_PARKED_WEB_MARKER`` in parked SPA directories. The
    shipped package never hardcodes private app path names.
    """
    return (app_root / _PARKED_WEB_MARKER).is_file()
...
def _grok_default_agent(model: str) -> str:
    ...
    Fail closed [DOM-06][AGT-10][OBS-01]: import/normalize faults must not be
    swallowed into a silent raw-model fallback that writes an unnormalized
    attribution string and looks successful (cs0166-r09-12). Log to stderr and
    re-raise so callers refuse rather than materializing a bad config.
    ``_bootstrap`` converts the raise into a non-zero int (declared contract);
    direct callers of ``ensure_grok_lane_config`` still see the exception.
    """
    try:
        ...
        print(
            f"ERROR: _grok_default_agent failed for model={model!r}: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        # Mark so _bootstrap can return int without emitting a second ERROR.
        try:
            exc._grok_default_agent_logged = True  # type: ignore[attr-defined]
...
def _bootstrap(
    ...
    if backend == "grok-cli":
        try:
            ...
            # Honour the declared -> int contract so lane_exec's
            # `if bootstrap_result != 0` envelope names the lane instead of a
            # bare TypeError/ModuleNotFoundError traceback [OBS-08].
            # Identity faults already emitted one ERROR in _grok_default_agent
            # (flagged on the exception); do not double-print. Other
            # materialization faults get a single named line here.
            if not getattr(exc, "_grok_default_agent_logged", False):
                print(
                    f"ERROR: grok lane config materialization failed: {type(exc).__name__}: {exc}",
                    file=sys.stderr,
                )
            return 1
    ...
    for full_path in app_roots:
        ...
        if (full_path / "package.json").is_file():
            # implementation note blast-radius: this arm is generic over any package.json in
            # owned_paths. Gate only marker-identified parked SPAs so other node
            # app roots still provision. Both copytree and npm install arms gate.
            if _is_parked_web_app(full_path) and not _provision_web_enabled():
                print(
                    f"  Skipping npm bootstrap for parked SPA {relative_path} (set WORKBAY_PROVISION_WEB=1 to enable)"
                )
                continue
