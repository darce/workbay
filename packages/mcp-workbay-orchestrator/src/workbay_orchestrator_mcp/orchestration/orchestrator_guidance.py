...
from workbay_protocol.envelope import require_success
...
#: Persisted on fallback-blocked lane notes so reclaim can tell unclassifiable
#: guidance apart from a real environment blocker without widening LANE_STATUSES.
BLOCKED_UNCLASSIFIED_NOTE_MARKER = "[guidance:blocked_unclassified]"


class LaneLookupTruncated(RuntimeError):
    """Raised when paginated lane lookup hit the page cap before finding the row.

    Distinct from a genuine absence: exhausting the page budget while the
    backend still reports more pages is not proof the lane is missing.
    """


class GuidanceQueueResult(list):
    """List-compatible open-guidance scan that carries completeness.

    A truncated drain and a complete drain must not collapse onto the same
    bare list. Callers may iterate, index, and compare this object exactly
    like the list previously returned by ``_list_open_worker_guidance``.
    """

    complete: bool
    truncation_detail: str | None
    ...
    def __init__(
        ...
        *,
        ...
        self.truncation_detail = truncation_detail
...
class GuidanceResolution:
    def __init__(
        ...
        *,
        ...
        self.branch_tip_sha = branch_tip_sha
        self.branch_tip_source = branch_tip_source
...
def _list_open_worker_guidance(task_ref: str) -> GuidanceQueueResult:
    """Return open worker-to-orchestrator messages plus a completeness flag.
    ...
    Page until the backend reports exhaustion. Hitting ``_GUIDANCE_QUEUE_MAX_PAGES``
    while the backend still reports more is typed truncation, not a complete
    drain: the result still contains the rows that were collected.
    """
    ...
    for _ in range(_GUIDANCE_QUEUE_MAX_PAGES):
        ...
        if not isinstance(rows, list) or not rows:
            return GuidanceQueueResult(collected, complete=True, truncation_detail=None)
        collected.extend(
        ...
        offset += len(rows)
    truncation_detail = (
        f"guidance_queue_truncated:pages={_GUIDANCE_QUEUE_MAX_PAGES},"
        f"page_size={_GUIDANCE_QUEUE_PAGE_SIZE},collected={len(collected)}"
    )
    return GuidanceQueueResult(collected, complete=False, truncation_detail=truncation_detail)
...
def _list_open_dispatch_messages(task_ref: str, lane_id: str) -> list[dict[str, Any]]:
    ...
    payload = _require_dict_payload(
        lane_communication(
            ...
            status="open",
            limit=200,
            fields="id,direction,dispatch_id",
        ),
...
_REPORT_LOOKUP_PAGE_SIZE = 200
_MAX_REPORT_LOOKUP_PAGES = 1_000
...
def _latest_lane_report(task_ref: str, lane_id: str, *, session: str | None = None) -> dict[str, Any] | None:
    ...
    offset = 0
    saw_report = False
    for _page in range(_MAX_REPORT_LOOKUP_PAGES):
        payload = _require_dict_payload(
            worker_reports(
                operation="list",
                task_ref=task_ref,
                lane_id=lane_id,
                limit=_REPORT_LOOKUP_PAGE_SIZE,
                offset=offset,
            ...
            source=f"worker_reports(list:{lane_id}:offset={offset})",
        )
        if payload.get("ok") is not True:
            raise RuntimeError(f"Failed to list worker reports for {lane_id}.")
        reports = payload.get("reports", [])
        if not isinstance(reports, list):
            raise RuntimeError(f"Worker reports response for {lane_id} has a malformed reports list.")
        saw_report = saw_report or bool(reports)
        if not session:
            return next((report for report in reports if isinstance(report, dict)), None)
        for report in reports:
            if isinstance(report, dict) and report.get("session") == session:
                return report
        if not reports or payload.get("has_more") is False or len(reports) < _REPORT_LOOKUP_PAGE_SIZE:
            break
        offset += len(reports)
    if not saw_report:
        return None
    raise RuntimeError(f"No worker report for lane {lane_id} matched guidance session {session!r}.")
...
def _lane_row(task_ref: str, lane_id: str) -> dict[str, Any]:
    ...
    for _ in range(_LANE_LOOKUP_MAX_PAGES):
        ...
        raise LaneLookupTruncated(
            f"Lane lookup for {lane_id} on task {task_ref} truncated after "
            f"{_LANE_LOOKUP_MAX_PAGES} pages (page_size={_LANE_LOOKUP_PAGE_SIZE}); "
            "the row was not proven absent."
        )
...
def _commit_is_on_target_branch(
    commit_sha: str,
    *,
    orchestrator_root: Path,
    activity: dict[str, Any],
) -> bool:
    """Whether ``commit_sha`` is proven integrated into the target branch."""
    # Intake targets the branch checked out at the orchestrator root. The
    # lane worktree and repository default branch need not be that target.
    # Detached or unreadable HEAD provides no authoritative branch evidence.
    target_branch = _git_text(orchestrator_root, "symbolic-ref", "-q", "HEAD")
    return bool(target_branch) and _is_ancestor(orchestrator_root, commit_sha, target_branch) is True


def _live_branch_tip(orchestrator_root: Path, lane: dict[str, Any]) -> str | None:
    """Observe the named lane ref, never substitute an ancestor from a report."""
    branch = _normalize_text(lane.get("branch"))
    if not branch:
        return None
    repo = _lane_git_repo(orchestrator_root, {"lane": lane})
    return _git_text(repo, "rev-parse", "--verify", "--quiet", f"{branch}^{{commit}}") or None


def _recorded_integration_sha(
    task_ref: str, lane_id: str, report_sha: str, orchestrator_root: Path, activity: dict[str, Any]
) -> str | None:
    """Require a landing receipt as well as ancestry; containment is not a receipt."""
    from orchestrator_lanes import _coerce_full_commit_sha
    from workbay_handoff_mcp import latest_lane_landing

    try:
        payload = _require_dict_payload(
            latest_lane_landing(task_ref=task_ref, lane_id=lane_id), source="latest_lane_landing"
        )
        require_success(payload, source="latest_lane_landing")
    except (OSError, RuntimeError, TypeError, ValueError):
        return None
    data = payload.get("data") if isinstance(payload.get("data"), dict) else payload
    landing = data.get("landing")
    sha = _coerce_full_commit_sha(landing.get("commit_sha")) if isinstance(landing, dict) else None
    if (
        sha
        and _is_ancestor(_lane_git_repo(orchestrator_root, activity), report_sha, sha) is True
        and _commit_is_on_target_branch(sha, orchestrator_root=orchestrator_root, activity=activity)
    ):
        return sha
    return None


def _guidance_effect_key(task_ref: str, lane_id: str, worker_message_id: int) -> str:
    return f"guidance:{task_ref}:{lane_id}:{worker_message_id}"
...
def _redispatch_configuration_block_reason(
    *,
    ...
    if payload is not None:
        # Legacy reports preserve model-pin refusals as text without dispatch
        # provenance. Refuse the same-pin retry locally; do not manufacture
        # typed evidence that could open a shared configuration breaker.
        if payload.get("ok") is not True and dispatch_breaker.is_deterministic_dispatch_refusal(
            str(payload.get("error") or "")
        ):
            return "latest failure is a deterministic model-pin dispatch refusal; operator reset required"
...
def _classify_guidance(
    *,
    ...
    branch_tip_sha = (
        _live_branch_tip(orchestrator_root, activity.get("lane") or {})
        if landed_commit and orchestrator_root is not None
        else None
    )
    branch_tip_source = "branch" if branch_tip_sha else None
    landing_commit_sha = None
    if landed_commit and orchestrator_root is not None:
        landing_commit_sha = _recorded_integration_sha(task_ref, lane_id, landed_commit, orchestrator_root, activity)
    ...
    if next_assignment is not None:
        ...
        # Only a redispatch replay retains its existing dispatch. A changed
        # resolution must reconcile that effect along with other stale work.
        close_dispatch_ids = tuple(
            int(row["id"])
            for row in open_dispatches
            if isinstance(row, dict)
            and row.get("id") is not None
            and row.get("dispatch_id") != f"{_guidance_effect_key(task_ref, lane_id, worker_message_id)}:redispatch"
        )
        return GuidanceResolution(
    ...
    if landed_commit:
        ...
        if unknown_ancestry:
            decision = (
                f"Routed {lane_id} to the review gate with {UNKNOWN_ANCESTRY_TOKEN} warning on commit {landed_commit}."
            )
            rationale = (
    ...
    if has_env_blocker:
        # Real environment/sandbox blocker. Distinct from the unclassifiable
        # fallback below: consumers must branch on kind (BLOCKED vs
        # BLOCKED_UNCLASSIFIED), not on lane_status, which stays "blocked"
        # because LANE_STATUSES is a closed schema-backed set.
        return GuidanceResolution(
    ...
    if combined.strip():
        # Unclassifiable text: same persisted lane_status as a real blocker,
        # but a different kind and a machine-readable notes marker so reclaim
        # can age the fallback without sweeping environment-blocked lanes.
        return GuidanceResolution(
            kind=GuidanceResolutionKind.BLOCKED_UNCLASSIFIED,
            lane_id=lane_id,
            worker_message_id=worker_message_id,
            latest_report_id=latest_report_id,
            ...
            lane_status="blocked",
            lane_notes=(
                f"{BLOCKED_UNCLASSIFIED_NOTE_MARKER} Unclassifiable guidance; marked blocked for operator review."
            ),
...
def _apply_guidance_resolution(
    *,
    ...
    lane_upsert_kwargs: dict[str, Any] = {
        "operation": "upsert",
        "task_ref": task_ref,
    ...
    landing_commit_sha = _normalize_text(resolution.landing_commit_sha)
    if landing_commit_sha:
        # Identity columns are omit-preserve: no evidence on this resolution
        # must not erase landing evidence recorded by an earlier writer.
        lane_upsert_kwargs["landing_commit_sha"] = landing_commit_sha
    # Classification and the durable write can straddle a branch advance.
    # Missing refs omit-preserve previous evidence instead of relabelling a
    # report or an old observation as the current branch tip.
    branch_tip_sha = _live_branch_tip(orchestrator_root, lane) if resolution.branch_tip_sha else None
    if branch_tip_sha:
        lane_upsert_kwargs["branch_tip_sha"] = branch_tip_sha
        lane_upsert_kwargs["branch_tip_source"] = "branch"

    upsert_source = f"manage_worktree_lane(upsert:{task_ref}:{resolution.lane_id})"
    require_success(
        _require_dict_payload(manage_worktree_lane(**lane_upsert_kwargs), source=upsert_source),
        source=upsert_source,
    )

    def acknowledged(payload: Any, source: str) -> None:
        require_success(_require_dict_payload(payload, source=source), source=source)

    effect_key = _guidance_effect_key(task_ref, resolution.lane_id, resolution.worker_message_id)
    for message_id in resolution.close_dispatch_ids:
        acknowledged(
            ...
            f"lane_communication(close dispatch:{message_id})",
        )
    ...
    if resolution.kind == GuidanceResolutionKind.REVIEW:
        for action in _pending_lane_actions(_lane_activity(task_ref, resolution.lane_id)):
            ...
            acknowledged(
                ...
                f"update_next_actions(done:{action_id})",
            )

    if resolution.kind == GuidanceResolutionKind.REDISPATCH and resolution.dispatch_message:
        _dispatch_payload: dict = {"dispatch_id": f"{effect_key}:redispatch"}
        try:
        ...
        acknowledged(
            ...
            f"lane_communication(redispatch:{effect_key})",
        )
    ...
    acknowledged(
        record_decision(
            ...
            task_ref=task_ref,
            # A retry may reclassify after pending actions change. Preserve
            # the prior decision and durably record the new outcome as well.
            event_id=f"{effect_key}:decision:{resolution.kind}",
        ),
        f"record_decision({effect_key})",
    )
    # Every effect is repeatable or keyed durably. Consume retry input last,
    # only after each downstream write explicitly acknowledges success.
    acknowledged(
        lane_communication(
            kind="message",
            operation="update",
        ...
        f"lane_communication(close worker:{resolution.worker_message_id})",
    )
    return resolution
...
def _resolve_guidance_cycle(
    ...
    *,
    ...
    queue = _list_open_worker_guidance(task_ref)
    queue_complete = bool(getattr(queue, "complete", True))
    truncation_detail = getattr(queue, "truncation_detail", None)
    for worker_message in _dedupe_worker_guidance_messages(list(queue)):
        lane_id = _normalize_text(worker_message.get("lane_id"))
        if not lane_id:
            continue
    ...
    if not queue_complete:
        detail = truncation_detail or (
            f"guidance_queue_truncated:pages={_GUIDANCE_QUEUE_MAX_PAGES},"
            f"page_size={_GUIDANCE_QUEUE_PAGE_SIZE},collected={len(queue)}"
        )
        truncation = GuidanceResolution(
            kind=GuidanceResolutionKind.FATAL_ERROR,
            lane_id="",
            worker_message_id=0,
            decision=f"guidance_queue_truncated:{task_ref}",
            rationale=detail,
            error=detail,
        )
        if callable(log):
        ...
        if not dry_run:
            record_decision(
                session=f"{task_ref}-orchestrator-daemon",
                decision=truncation.decision,
                rationale=truncation.rationale,
                task_ref=task_ref,
            )
        results.append(truncation)
    return results
