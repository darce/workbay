...
from workbay_protocol.reasoning_effort import (  # noqa: F401 — compatibility exports
    AUTO_EFFORT_LADDER as EFFORT_LADDER,
)
from workbay_protocol.reasoning_effort import (
    CODEX_REASONING_EFFORTS,
    validate_reasoning_effort,
)
from workbay_protocol.reasoning_effort import (
    STANDARD_WORKER_REASONING_EFFORT_CHOICES as STANDARD_WORKER_REASONING_EFFORT_CHOICES,
)
from workbay_protocol.reasoning_effort import (
    WORKER_REASONING_EFFORT_CHOICES as WORKER_REASONING_EFFORT_CHOICES,
)
...
def resolve_lane_python(
    ...
    *,
    ...
    ``git_timeout`` overrides the five-second timeout for the collapse probe;
    callers that need one timeout policy across several Git probes can pass
    their already-scrubbed ``git_env`` as well. The default keeps the existing
    five-second resolver timeout for other callers.
    ...
    try:
        clean_env = dict(os.environ if git_env is None else git_env)
        if git_env is None:
        ...
        result = subprocess.run(
            ...
            text=True,
            check=False,
            timeout=5 if git_timeout is None else git_timeout,
            env=clean_env,
        )
        ...
        logger.warning(
            "resolve_lane_python: git collapse failed for %s (%s); falling back toward ambient interpreter",
            root,
            exc,
        )
        return sys.executable
    ...
    logger.warning(
        "resolve_lane_python: no in-tree .venv discoverable under %s; using ambient interpreter %s",
        root,
        sys.executable,
    )
    return sys.executable
...
def resolve_auto_reasoning_effort(
    *,
    ...
    """
    normalized = validate_reasoning_effort(requested) or "inherit"
