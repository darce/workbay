...
def _require_ok_dict_payload(payload: Any, *, source: str) -> dict[str, Any]:
    """Require a dict payload whose ``ok`` field is exactly True.

    Mutation sites must not treat a typed dict refusal (``ok: false``) as
    success and then record a decision that claims the write landed. Soft
    read paths that continue on failure keep using ``_require_dict_payload``
    plus an explicit ``ok`` check.
    """
    data = _require_dict_payload(payload, source=source)
    if data.get("ok") is not True:
        error = data.get("error") or data.get("data") or data
        raise RuntimeError(f"{source} failed: {error}")
    return data
