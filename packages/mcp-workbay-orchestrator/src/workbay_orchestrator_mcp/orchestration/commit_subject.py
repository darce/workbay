...
def _has_attribution_byline(normalized_subject: str) -> bool:
    """Tier 2: authorship verb + by/with + bare object is a byline.
    ...
    adjectival modifiers (``assistant``, ``advanced agent``, …). No vendor
    list here — the grammar is the bound.

    Core ``_AUTH_VERBS`` bare-object-refuse any non-determiner object (vendor
    names, unlisted heads). Trailer-only verbs (``assisted``, ``powered``)
    only bare-object-refuse when the object itself is a generic agent head, so
    ``powered by Redis`` survives while ``Powered by an AI assistant`` still
    dies on the determiner+pure-agent path.
    ...
    for match in _SOFT_BYLINE_RE.finditer(normalized_subject):
        obj = match.group(1)
        if obj not in _DETERMINERS:
            # Soft verbs: bare agent head is credit; bare infra/human is not.
            if _word_is_generic_agent_head(obj):
                return True
            continue
        start_idx = bisect.bisect_left(span_starts, match.end())
        # Soft verbs still refuse pure agent NPs, but do not strip closed
        # trailing adjuncts — residual ``Reviewed by … today`` padding is
        # ordinary review prose, not authorship credit.
...
def _has_unbacked_vendor_token(normalized_subject: str, normalized_evidence: str) -> bool:
    ...
    # punctuation and still corroborate it.
    for token in VENDOR_CREDIT_TOKENS:
        tok_n = normalize_for_match(token)
        if _contains_token(normalized_subject, tok_n) and tok_n not in normalized_evidence:
            return True
    return False
...
def sanitize_lane_id_for_commit_message(lane_id: str, *, max_len: int | None = None) -> str:
    ...
    Collapse to a single physical line; screen credit so a vendor-named agent
    identity or byline-bearing id never lands in history, while multi-word
    work-subject phrases that merely name a product under edit still pass;
    when credit is refused, substitute a digest stand-in that still
    discriminates between lanes. Optionally bound length when the value
