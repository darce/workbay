...
_AUTH_VERBS = (
    "built|written|authored|generated|created|implemented|developed|coded|produced|drafted|ported|crafted|made"
)
...
_TRAILER_VERB_BY_RE = re.compile(rf"^{_TRAILER_ROLE_PREFIX}(?:{_TRAILER_VERBS})[\s\-]*by\s*:")
...
_BYLINE_RE = re.compile(rf"\b(?:{_AUTH_VERBS})\s+(?:by|with)\s+{_BYLINE_OBJECT_WRAPPER}*(\w+)")
_SOFT_BYLINE_RE = re.compile(rf"\b(?:{_TRAILER_ONLY_VERBS})\s+(?:by|with)\s+{_BYLINE_OBJECT_WRAPPER}*(\w+)")
# ``generated with`` + object, compiled after the shared wrapper constant.
_GENERATED_WITH_OBJECT_RE = re.compile(rf"generated with\s+{_BYLINE_OBJECT_WRAPPER}*(\w+)")
...
_CREDIT_TO_OBJECT_RE = re.compile(rf"(?<![\w-])credits?\s+to\s+{_BYLINE_OBJECT_WRAPPER}*(\w+)")
...
_CREDIT_FORM_OBJECT_RE = re.compile(rf"(?<![\w-])credits?\s*:\s*{_BYLINE_OBJECT_WRAPPER}*(\w+)")
...
_GENERIC_AGENT_PHRASE_MODIFIERS = frozenset(
    word for phrase in _GENERIC_AGENT_HEAD_PHRASES for word in phrase.split()[:-1]
)
...
def _word_is_generic_agent_head(word: str) -> bool:
    ...
    if _bare_generic_agent_head(joined):
        return True
    if _bare_generic_agent_head(parts[-1]) and all(_is_adjectival(part) for part in parts[:-1]):
        return True
    return False
...
def _is_adjectival(word: str) -> bool:
    ...
    if _word_is_generic_agent_head(word):
        return True
    if word in _GENERIC_AGENT_PHRASE_MODIFIERS or _singularize_agent_token(word) in _GENERIC_AGENT_PHRASE_MODIFIERS:
        return True
...
def _content_tokens_are_pure_generic_agent(content: list[str]) -> bool:
    ...
    while index >= 0:
        ...
        if index >= 1 and _pair_is_generic_agent_phrase(content[index - 1], content[index]):
            right_was_adjectival = True
            index -= 2
            continue
        token = content[index]
...
def _fold_next_token(spans: list[tuple[str, int, int]], i: int) -> tuple[str, int] | None:
...
def _post_determiner_head_is_generic_agent_from_spans(
    ...
    *,
    ...
    content = [seg for word in np_words for seg in _np_word_to_content_tokens(word)]
...
def _post_determiner_head_is_generic_agent(
    ...
    *,
    ...
    if not spans:
        return False
    return _post_determiner_head_is_generic_agent_from_spans(spans, 0, strip_trailing_adjuncts=strip_trailing_adjuncts)
...
_WEAK_PREP_BEFORE_RE = re.compile(r"(?:\b(?:via|from|per|to|using|for|of)\s+|\binstead of\s+|\brather than\s+)$")
_BY_BEFORE_RE = re.compile(r"\bby\s+$")
_WITH_BEFORE_RE = re.compile(r"\bwith\s+$")
_BY_DETERMINER_BEFORE_RE = re.compile(r"\b(?:by|with)\s+(?:the|a|an|our|its|their|this|that|these|those)\s+$")
_THANKS_BEFORE_RE = re.compile(r"(?:courtesy of|thanks to)\s+$")
_AUTH_VERB_IN_TEXT_RE = re.compile(rf"\b(?:{_AUTH_VERBS})\b")
_WITH_CREDIT_VERB_IN_TEXT_RE = re.compile(r"\b(?:" + "|".join(sorted(_WITH_CREDIT_VERBS)) + r")\b")
...
_REGISTERED_BACKEND_IDS_NORM: frozenset[str] = frozenset(b.casefold() for b in _REGISTERED_BACKEND_IDS)
...
_REVIEWED_BYLINE_RE = re.compile(rf"\breviewed\s+(?:by|with)\s+{_BYLINE_OBJECT_WRAPPER}*(\w+)")
...
def normalize_for_match(text: str) -> str:
    ...
    stripped = "".join(
        ch for ch in nfd if ch != _BRAILLE_PATTERN_BLANK and unicodedata.category(ch) not in {"Mn", "Mc", "Me", "Cf"}
    )
...
def _fold_product_id_at(spans: list[tuple[str, int, int]], i: int) -> str | None:
...
def _advance_product_id_span(spans: list[tuple[str, int, int]], i: int) -> int:
...
def _has_attribution_byline(normalized_subject: str) -> bool:
    ...
    for match in _SOFT_BYLINE_RE.finditer(normalized_subject):
        ...
        if obj not in _DETERMINERS:
            ...
            if _word_is_generic_agent_head(obj):
                return True
            if _post_determiner_head_is_generic_agent_from_spans(spans, obj_idx, strip_trailing_adjuncts=False):
                return True
            continue
        start_idx = bisect.bisect_left(span_starts, match.end())
        ...
        if _post_determiner_head_is_generic_agent_from_spans(spans, start_idx, strip_trailing_adjuncts=False):
            return True
...
def _match_covered_by_registered_backend(normalized_subject: str, start: int, end: int) -> re.Match[str] | None:
    ...
    for backend in _REGISTERED_BACKEND_IDS_NORM:
        for form in (backend, backend.replace("-", " ")):
            for bm in re.finditer(rf"(?<!\w){re.escape(form)}(?![a-z])", normalized_subject):
...
def _weak_prep_is_noncredit_routing(prefix: str) -> bool:
    ...
    if not (_AUTH_VERB_IN_TEXT_RE.search(prefix) or _WITH_CREDIT_VERB_IN_TEXT_RE.search(prefix)):
        return True
    ...
    if re.search(r"\b(?:the|a|an|our|its|their|this|that|these|those)\b", between):
        return True
    return False


def _vendor_token_match_is_noncredit_context(normalized_subject: str, match: re.Match[str]) -> bool:
    ...
    * vendor/product name at subject lead with non-claim continuation
    ...
    backend_match = _match_covered_by_registered_backend(normalized_subject, start, end)
    if backend_match is not None:
        ...
        if _WITH_BEFORE_RE.search(b_prefix) and not _WITH_CREDIT_VERB_IN_TEXT_RE.search(b_prefix):
            return True
        lead = b_prefix.strip()
        ...
        if _BY_BEFORE_RE.search(b_prefix):
            if _AUTH_VERB_IN_TEXT_RE.search(b_prefix):
                return False
            if any(tok in _SUBJECT_CLAIM_VERBS for tok in re.findall(r"[a-z0-9]+(?:\.[0-9]+)*", b_prefix)):
                return False
            return True
        return False
    ...
    if _WITH_BEFORE_RE.search(prefix):
        ...
        if with_match is not None:
            ...
            if (
                suffix.startswith("'s")
                or suffix.startswith("\u2019s")
                or (suffix.startswith("'") and "help" in suffix[:12])
            ):
                return False
            return True
