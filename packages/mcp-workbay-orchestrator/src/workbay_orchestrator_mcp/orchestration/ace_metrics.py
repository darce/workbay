...
def _load_wave_metrics_rows(
    state_dir: Path,
    task_ref: str,
    *,
    limit: int = 50,
) -> list[dict]:
    """Load completion-row wave metrics from handoff.db (implementation note S4).

    Uses ``decision LIKE 'dispatch_wave:%'`` — the colon is load-bearing so
    audit open rows (``dispatch_wave_open:%``) are excluded (row-26 mutation:
    ``LIKE 'dispatch_wave%'`` would leak open rows into aggregates).
    """
    handoff_db = state_dir / "handoff.db"
    if not handoff_db.exists():
        return []
    ...
    recent: list[dict] = []
    for decision, rationale, created_at in rows:
        metrics: dict
        if isinstance(rationale, str) and rationale.strip():
            try:
                parsed = json.loads(rationale)
                metrics = parsed if isinstance(parsed, dict) else {"raw_rationale": rationale}
            except json.JSONDecodeError:
                metrics = {"raw_rationale": rationale}
        else:
            metrics = {}
        decision_s = str(decision or "")
        if "wave_id" not in metrics and ":" in decision_s:
            metrics = {**metrics, "wave_id": decision_s.split(":", 1)[1]}
        if created_at is not None and "created_at" not in metrics:
            metrics = {**metrics, "created_at": created_at}
        recent.append(metrics)
    return recent


def _waves_snapshot(task_ref: str, state_dir: Path, *, limit: int = 50) -> dict:
    """Aggregate recent wave completion metrics for the ACE snapshot."""
    recent = _load_wave_metrics_rows(state_dir, task_ref, limit=limit)

    achieved_ratios: list[float] = []
    straggler_ratios: list[float] = []
    makespan_ratios: list[float] = []
    for row in recent:
        if not isinstance(row, dict):
            continue
        admitted = row.get("admitted_width")
        env_cap = row.get("env_cap")
        try:
            admitted_f = float(admitted) if admitted is not None else None
            env_f = float(env_cap) if env_cap is not None else None
        except (TypeError, ValueError):
            admitted_f, env_f = None, None
        if admitted_f is not None and env_f is not None and env_f > 0:
            achieved_ratios.append(admitted_f / env_f)

        sr = row.get("straggler_ratio")
        try:
            if sr is not None:
                sr_f = float(sr)
                if math.isfinite(sr_f):
                    straggler_ratios.append(sr_f)
        except (TypeError, ValueError):
            pass

        mvs = row.get("makespan_vs_sum")
        try:
            if mvs is not None:
                mvs_f = float(mvs)
                if math.isfinite(mvs_f):
                    makespan_ratios.append(mvs_f)
        except (TypeError, ValueError):
            pass
    ...
    return {
        "recent": recent,
        "achieved_vs_cap": _mean(achieved_ratios),
        "mean_straggler_ratio": _mean(straggler_ratios),
        "makespan_vs_sum": _mean(makespan_ratios),
    }
...
def build_snapshot(
    ...
    # implementation note S4 — wave observability (additive key; open rows excluded).
    snapshot["waves"] = _waves_snapshot(task_ref, state_dir)
    return snapshot
...
def _render_waves(waves: dict) -> list[str]:
    """Render the plan-0173 S4 waves section (open rows never appear here)."""
    lines = ["", "## Wave Observability"]
    if not isinstance(waves, dict) or not waves:
        lines.append("_No wave completion metrics recorded._")
        return lines
    recent = waves.get("recent") or []
    if not recent:
        lines.append("_No wave completion metrics recorded._")
        return lines
    achieved = waves.get("achieved_vs_cap")
    mean_straggler = waves.get("mean_straggler_ratio")
    makespan_vs_sum = waves.get("makespan_vs_sum")
    lines += [
        f"- Recent waves: **{len(recent)}**",
        "- Achieved vs cap: " + (f"**{achieved:.2%}**" if isinstance(achieved, (int, float)) else "n/a"),
        "- Mean straggler ratio: " + (f"{mean_straggler:.3f}" if isinstance(mean_straggler, (int, float)) else "n/a"),
        "- Mean makespan/sum: " + (f"{makespan_vs_sum:.3f}" if isinstance(makespan_vs_sum, (int, float)) else "n/a"),
    ]
    lines.append("- Recent:")
    for row in recent[:10]:
        if not isinstance(row, dict):
            continue
        wid = row.get("wave_id") or "?"
        admitted = row.get("admitted_width")
        env_cap = row.get("env_cap")
        straggler = row.get("straggler_ratio")
        lines.append(
            f"  - `{wid}`: admitted={admitted}/{env_cap} straggler={straggler if straggler is not None else 'n/a'}"
        )
    return lines
...
def render_markdown(snapshot: dict) -> str:
    ...
    for section_lines in (
        ...
        _render_waves(snapshot.get("waves", {})),
    ):
