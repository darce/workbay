...
def _current_lane_attestation(
    ...
    """Return the current per-lane record, or an error reason if the store is unreadable."""
    path = Path(root) / STATE_FILE
    if not path.exists():
        return {}, None
    with bounded_file_lock(path):
        ...
        record = lanes.get(_lane_store_key(lane_id, worktree_path))
        return (dict(record), None) if isinstance(record, dict) else ({}, None)
...
def remint_capability_receipt(
    ...
    *,
    ...
    try:
        if source not in _STRENGTH:
            raise ValueError(f"unknown attestation source: {source}")
        existing, store_error = _current_lane_attestation(root, lane_id, worktree_path)
