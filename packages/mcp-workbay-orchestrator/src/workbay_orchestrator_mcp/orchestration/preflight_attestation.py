...
# Write evidence remint and authorization both require this pair. One missing
# or expired gate is a typed refusal, not a partial success.
REQUIRED_WRITE_GATES = ("workspace_write", "writable_roots")
_RECEIPT_EXPIRED = object()
...
def _legacy_lane_store_key(lane_id: Any = None, worktree_path: Any = None) -> str:
    """Return the pre-task-scoped key used by existing documents."""
...
def _lane_store_key(
    ...
    """Return the durable key for one ``(task_ref, lane_id)`` capability.

    A task-scoped caller gets an unambiguous NUL-delimited key, matching the
    existing task/lane state-key convention.  Callers without a task retain
    the old lane/worktree key for the migration window; those records are
    treated as legacy compatibility evidence and are never allowed to replace
    a task-scoped record.
    """
    task = task_ref.strip() if isinstance(task_ref, str) else ""
    lane = lane_id.strip() if isinstance(lane_id, str) else ""
    if task:
        identity = lane or _legacy_lane_store_key(None, worktree_path)
        if identity:
            return f"{task}\0{identity}"
    return _legacy_lane_store_key(lane_id, worktree_path)
...
def _caller_lane_key(identity: Mapping[str, Any]) -> str | None:
    ...
    key = _lane_store_key(lane_id, worktree, task_ref)
    return key or None


def _legacy_record_matches_scope(
    record: Mapping[str, Any],
    *,
    lane_id: Any = None,
    worktree_path: Any = None,
) -> bool:
    """Check whether a taskless legacy record is safe to use as a fallback."""
    record_lane = record.get("lane_id")
    if isinstance(lane_id, str) and lane_id.strip() and record_lane not in (None, lane_id.strip()):
        return False
    expected_worktree = (
    ...
    record_worktree = record.get("worktree_path")
    if expected_worktree and record_worktree:
        try:
            if str(Path(record_worktree).expanduser().resolve()) != expected_worktree:
                return False
...
def _record_for_scope(
    ...
    *,
    ...
    """Find a task-scoped record, then a compatible pre-migration record.

    The boolean marks a taskless legacy fallback.  Such records are readable
    for the migration window, but a new write is always placed under the
    compound key so two tasks can no longer overwrite one another.
    """
    key = _lane_store_key(lane_id, worktree_path, task_ref)
    record = lanes.get(key) if key is not None else None
    if isinstance(record, dict) and record:
        return dict(record), False
    if isinstance(task_ref, str) and task_ref.strip():
        legacy_key = _legacy_lane_store_key(lane_id, worktree_path)
        legacy = lanes.get(legacy_key) if legacy_key else None
        if isinstance(legacy, dict) and legacy and not legacy.get("task_ref"):
            if _legacy_record_matches_scope(legacy, lane_id=lane_id, worktree_path=worktree_path):
                return dict(legacy), True
        # A legacy document may have retained a non-standard key.  Only accept
        # a record that carries the requested task explicitly in that case.
        for candidate in lanes.values():
            if (
                isinstance(candidate, dict)
                and candidate.get("task_ref") == task_ref.strip()
                and candidate.get("lane_id") == lane_id
            ):
                return dict(candidate), False
        return None, False
    return (dict(record), False) if isinstance(record, dict) and record else (None, False)
...
def _record_for_receipt(
    ...
    *,
    ...
    """Find the unique scoped record whose current or predecessor id matches."""
    matched: dict[str, Any] | None = None
    candidates: Iterable[Mapping[str, Any]] = lanes.values()
    if isinstance(task_ref, str) and task_ref.strip():
        candidates = (
            candidate
            for candidate in candidates
            if isinstance(candidate, dict)
            and (candidate.get("task_ref") == task_ref.strip() or not candidate.get("task_ref"))
            and _legacy_record_matches_scope(candidate, lane_id=lane_id, worktree_path=worktree_path)
        )
    elif isinstance(lane_id, str) and lane_id.strip() or worktree_path is not None:
        candidates = (
            candidate
            for candidate in candidates
            if isinstance(candidate, dict)
...
def missing_capability_identity_fields(identity: Mapping[str, Any], *, allow_legacy_task_ref: bool = True) -> list[str]:
    """List required mutating-dispatch identity dimensions absent from *identity*."""
    missing: list[str] = []
    for field in CAPABILITY_REQUIRED_IDENTITY_FIELDS:
        # Receipts minted before task-scoped attestation shipped have no
        # task_ref.  Keep those records readable during migration; every new
        # producer includes the field and therefore gets the stricter check.
        if field == "task_ref" and field not in identity and allow_legacy_task_ref:
            continue
...
def _identity_mismatches(record: Mapping[str, Any], expected: Mapping[str, Any]) -> list[str]:
    ...
    for key, value in expected.items():
        if value is None:
            continue
        # A taskless legacy record cannot prove which historical task owned
        # it.  Scope selection already required the matching lane/worktree;
        # preserve read-old compatibility without weakening new records.
        if key == "task_ref" and not record.get("task_ref"):
            continue
...
def validate_capability_receipt(
    ...
    *,
    ...
    explicit_identity_requirement = require_complete_identity is not None
    with bounded_file_lock(path):
        ...
        record, _legacy = _record_for_scope(
            lanes,
            task_ref=identity.get("task_ref"),
            lane_id=named_lane or None,
            worktree_path=identity.get("worktree_path"),
        )
        # A task-scoped remint can coexist with a legacy lane-only record while
        # old carriers drain. Resolve the named receipt before classifying the
        # legacy scoped record as stale; identity validation must inspect the
        # record that actually carries the receipt.
        if record is None or not _receipt_matches_record(record, capability_receipt_id):
            receipt_record = _record_for_receipt(
                lanes,
        ...
        if record is None and named_lane:
            return {"ok": False, "reason": LANE_ATTESTATION_MISSING_REASON}
        if record is None and not named_lane and len(lanes) == 1:
        ...
        if require_complete_identity:
            missing = missing_capability_identity_fields(
                identity,
                # Explicit identity checks must enforce task_ref for a
                # task-scoped record, while taskless records written before
                # task-scoped attestation remain readable during migration.
        ...
        gate_reasons = [
            _gate_failure_reason(gates.get(name), timestamp=timestamp, ttl=ttl) for name in REQUIRED_WRITE_GATES
        ]
        blocking_reasons = [
...
def _valid_gates(
    ...
    *,
    ...
    if not isinstance(gates, dict):
        return {}
    valid = {}
    for name in REQUIRED_WRITE_GATES:
...
def _has_required_write_gates(gates: Mapping[str, Any]) -> bool:
    """True only when every required write gate is present in the checked set."""
    return all(name in gates for name in REQUIRED_WRITE_GATES)
...
def _clamped_remint_transport_verdict(requested: str, existing: Any) -> str:
    """Never mint a more authorizing transport stamp than the predecessor."""
    existing_deny = _VERDICT_DENY_RANK.get(existing)
    requested_deny = _VERDICT_DENY_RANK.get(requested, _VERDICT_DENY_RANK["unknown"])
    if existing_deny is None:
        return requested if requested_deny >= _VERDICT_DENY_RANK["unknown"] else "unknown"
    if requested_deny < existing_deny:
        return str(existing)
    return requested if requested in _VERDICT_DENY_RANK else str(existing)


def _current_lane_attestation(
    root: Path,
    task_ref: str | None,
    lane_id: str | None,
    worktree_path: Path | str | None,
) -> tuple[dict[str, Any], str | None]:
    """Return the current scoped record, or an error reason if unreadable."""
    path = Path(root) / STATE_FILE
    if not path.exists():
        return {}, None
    with bounded_file_lock(path):
        _version, lanes, error = _parse_attestation_document(_read(path))
        if error is not None or lanes is None:
            return {}, error or UNSUPPORTED_ATTESTATION_SCHEMA_REASON
        record, _legacy = _record_for_scope(
            lanes,
            task_ref=task_ref,
            lane_id=lane_id,
        ...
        return (record, None) if isinstance(record, dict) else ({}, None)
...
def read_attestation(
    ...
    *,
    ...
    with bounded_file_lock(path):
        ...
        timestamp = now()
        ttl = _ttl(env)
        by_gate: dict[str, list[tuple[str, dict[str, Any]]]] = {name: [] for name in REQUIRED_WRITE_GATES}
        # Scope selectors identify the caller's receipt elsewhere, but this
        # read is deliberately a shared-host set reduction.  In particular,
        # a fresh peer negative must remain visible to remote_exec before its
        # own-receipt cache skip; narrowing the candidate set here makes that
        # downstream refusal fail open.
        candidates: Iterable[tuple[str, Mapping[str, Any]]] = lanes.items()
        for lane_key, record in candidates:
            if not isinstance(record, dict):
                continue
...
def record_attestation(
    ...
    *,
    ...
    if verdict != "positive" and not selected:
        selected = {name: False for name in REQUIRED_WRITE_GATES}
    transport_supplied = (
    ...
    with bounded_file_lock(path):
        timestamp = now()
        result = _record_attestation_locked(
            path,
            identity=identity,
            selected=selected,
            source=source,
            ...
            extra_predecessor_receipt_id=extra_predecessor_receipt_id,
            env=env,
            timestamp=timestamp,
            transport_only=transport_only,
        )
        return None if result is _RECEIPT_EXPIRED else result
...
def _record_attestation_locked(
    ...
    *,
    ...
    probe_timeout: bool | None,
    probe_cleanup_outcome: str,
    extra_predecessor_receipt_id: str | None,
    env: Mapping[str, str] | None,
    timestamp: float,
    transport_only: bool,
    allow_unknown_gate_synthesis: bool = True,
    require_complete_write_gates: bool = False,
) -> str | None | object:
    """Persist attestation while the caller already holds ``bounded_file_lock``.

    Remint sets ``require_complete_write_gates`` and disables UNKNOWN-gate
    synthesis so a later timestamp cannot mint a fresh grant after TTL. That
    remint-only flag also clamps the requested transport verdict against the
    locked current record so a newer same-digest refusal cannot be upgraded.
    """
    if not _finite_timestamp(timestamp):
        return None
    _version, lanes, error = _parse_attestation_document(_read(path))
    if error is not None or lanes is None:
        raise ValueError(error or UNSUPPORTED_ATTESTATION_SCHEMA_REASON)
    lane_key = _lane_store_key(lane_id, worktree_path, task_ref)
    existing, _legacy = _record_for_scope(
        lanes,
        task_ref=task_ref,
        lane_id=lane_id,
        worktree_path=worktree_path,
    ...
    if require_complete_write_gates and not _has_required_write_gates(gates):
        return _RECEIPT_EXPIRED
    if transport_only and not gates:
        if not allow_unknown_gate_synthesis:
            return _RECEIPT_EXPIRED
        unknown_gate = {
            "source": source,
        ...
        gates = {name: dict(unknown_gate) for name in REQUIRED_WRITE_GATES}
    ...
    if require_complete_write_gates:
        transport_values["transport_verdict"] = _clamped_remint_transport_verdict(
            transport_values["transport_verdict"],
            existing.get("transport_verdict"),
        )
...
def capability_receipt_record(
    ...
    *,
    ...
    with bounded_file_lock(path):
        ...
        return _strip_envelope(record) if isinstance(record, dict) else None
...
def remint_capability_receipt(
    ...
    *,
    ...
    Freshness check and persist share one lock-held timestamp via
    ``_record_attestation_locked``. TTL and ``_REFRESH_AHEAD_FRACTION`` stay
    inside ``_normalise_gate``; this helper does not invent a second expiry
    policy. Expired or missing required write gates fail closed as
    ``receipt_expired`` instead of synthesizing a fresh UNKNOWN grant, even
    when a caller passes ``transport_verdict='positive'``. A remint never
    stores a transport verdict stronger than the locked current record.
    """
    try:
        if source not in _STRENGTH:
            raise ValueError(f"unknown attestation source: {source}")
        existing, store_error = _current_lane_attestation(root, task_ref, lane_id, worktree_path)
        ...
        identity = _identity(gate_host, codex_version, sandbox_flags)
        gates = _valid_gates(existing, identity, timestamp, _ttl(env), preserve_future=True)
        if not _has_required_write_gates(gates):
            return {"ok": False, "reason": "receipt_expired"}
        path = Path(root) / STATE_FILE
        with bounded_file_lock(path):
            timestamp = now()
            if not _finite_timestamp(timestamp):
                return {"ok": False, "reason": "receipt_expired"}
            receipt_id = _record_attestation_locked(
                path,
                identity=identity,
                selected={},
                source=source,
                ...
                timestamp=timestamp,
                transport_only=True,
                allow_unknown_gate_synthesis=False,
                require_complete_write_gates=True,
            )
    except (OSError, RuntimeError, ValueError) as exc:
        return {"ok": False, "reason": CAPABILITY_RECEIPT_UNAVAILABLE_REASON, "error": str(exc)}
    if receipt_id is _RECEIPT_EXPIRED:
        return {"ok": False, "reason": "receipt_expired"}
