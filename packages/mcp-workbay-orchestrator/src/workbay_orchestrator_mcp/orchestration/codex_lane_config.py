...
from workbay_orchestrator_mcp.orchestration.resolved_role_config import (
    CodexLaneTimeoutError,  # noqa: F401  # re-exported for existing importers
    resolve_lane_timeout,
)
...
# 2026-09-04: 0.145.0 requests for gpt-6-astra fail server-side with
# "requires a newer version of Codex". Remeasured 0.153.4 on the lane VM:
# ChatGPT login, structured output, workspace-write file I/O and .git commits.
# See docs/runbooks/remote-gate-provisioning.md (Codex CLI upgrades).
# 2026-09-22: 0.153.4 requests for gpt-6-luna fail server-side with HTTP 400
# "not supported when using Codex with a ChatGPT account", and the model is
# absent from the catalogue that CLI fetches. CLI 0.155.1, on the same ChatGPT
# account (matching account_id hash), lists gpt-6-luna and completed max-effort
# turns on both service tiers.
...
class CodexModelTier:
    """One curated codex model's routing and invocation envelope.

    ``allowed_service_tiers`` uses codex CLI vocabulary. Operator-facing speed
    values are translated only through :data:`CODEX_SPEED_TO_SERVICE_TIER`.
    """

    slug: str
    tier: Literal["senior", "junior"]
    allowed_efforts: frozenset[str]
    default_effort: str
    allowed_service_tiers: frozenset[str]
    roles: frozenset[Literal["implement", "review", "adjudicate"]]
    entitled: bool


# VM entitlement sweep (2026-09-02): sol and luna answered with a live sol
# control before and after; sparx was refused by the ChatGPT-account gate. Keep
# the unentitled row so a future probe result is a data-only change.
CODEX_MODEL_TIERS: dict[str, CodexModelTier] = {
    row.slug: row
    for row in (
        CodexModelTier(
            slug=TRACKED_CODEX_MODEL,
            tier="senior",
            allowed_efforts=frozenset({"low", "medium", "high", "xhigh", "max"}),
            default_effort="high",
            allowed_service_tiers=frozenset({"default", "fast"}),
            roles=frozenset({"implement", "review", "adjudicate"}),
            entitled=True,
        ),
        CodexModelTier(
            slug="gpt-5.6-luna",
            # Luna degrades quietly below max: the lane still completes and the
            # output still reads plausible. The operator directive is that every
            # Luna lane runs at max, and effort_policy.MAX_EFFORT_MODELS encodes
            # it for omitted efforts -- this row must agree, or the two dispatch
            # paths that read default_effort (lane_escalation, offload_preflight
            # tier resolution) would silently route Luna below max.
            tier="junior",
            allowed_efforts=frozenset({"medium", "high", "xhigh", "max"}),
            default_effort="max",
            # Measured on the gate VM 2026-09-07: `codex exec
            # --skip-git-repo-check -m gpt-5.6-luna -c service_tier=fast`
            # completed the turn with no diagnostic, while the control
            # `service_tier=definitely_not_a_tier` emitted the
            # SERVICE_TIER_WARNING_PATTERNS line. The service tier is
            # orthogonal to reasoning effort, so a fast lane is still a max
            # lane. Prior `{"default"}` row was an unmeasured assumption.
            allowed_service_tiers=frozenset({"default", "fast"}),
            roles=frozenset({"implement"}),
            entitled=True,
        ),
        CodexModelTier(
            slug="gpt-6-luna",
            # Same envelope as gpt-5.6-luna, and the same max-only directive
            # (effort_policy.MAX_EFFORT_MODELS). The catalogue advertises
            # low..max without ultra; low stays out, as for gpt-5.6-luna.
            # Measured 2026-09-22 on CLI 0.155.1, same ChatGPT account as the
            # gate VM: `codex exec --ignore-user-config -m gpt-6-luna -c
            # model_reasoning_effort=max` completed on service_tier=default and
            # service_tier=fast with no diagnostic. The control
            # `service_tier=definitely_not_a_tier` emitted the
            # SERVICE_TIER_WARNING_PATTERNS item. The VM needs CLI 0.155.1 first,
            # because 0.153.4 is refused for this slug (see CODEX_CLI_VERSION).
            tier="junior",
            allowed_efforts=frozenset({"medium", "high", "xhigh", "max"}),
            default_effort="max",
            allowed_service_tiers=frozenset({"default", "fast"}),
        ...
        CodexModelTier(
            slug="gpt-5.3-sparx",
            tier="junior",
            allowed_efforts=frozenset({"medium", "high", "xhigh"}),
            default_effort="xhigh",
            allowed_service_tiers=frozenset({"default"}),
            roles=frozenset({"implement"}),
            entitled=False,
        ),
    )
...
# Authorization indexes are projections of the tier table, never independent
# literals. Membership distinguishes known rows from ``unknown_model``;
# ``entitled`` remains the separate dispatch gate that produces the more useful
# ``tier_not_entitled`` refusal for a known-but-unavailable model.
CODEX_ALLOWED_MODELS: frozenset[str] = frozenset(CODEX_MODEL_TIERS)
...
CODEX_MODEL_ALLOWED_SERVICE_TIERS: dict[str, frozenset[str]] = {
    **{row.slug: row.allowed_service_tiers for row in CODEX_MODEL_TIERS.values()},
    # 2026-09-04, CLI 0.153.4: both explicit tiers completed on the VM without
    # the unsupported-service-tier diagnostic. gpt-6-astra has no tier-table row
    # yet, so keep its measured tiers as an explicit entry -- the projection
    # above would otherwise drop a boundary that was established by evidence.
    "gpt-6-astra": frozenset({"default", "fast"}),
}
...
SERVICE_TIER_WARNING_PATTERNS: tuple[str, ...] = ("Configured service tier",)
...
# Headless per-candidate entitlement probe (internal). The consumer
# appends ``-m <slug>`` and a one-word prompt and isolates stdin from its own
# control script.
CODEX_PROBE_MODEL_ARGV: tuple[str, ...] = (
    "codex",
    "exec",
    "--json",
    "--skip-git-repo-check",
...
# The default is a selection fallback, not an exclusive model restriction: an
# explicit operator env pin must stay admissible even with no curated tier row.
# Union it onto the tier projection above rather than restating the boundary as
# a second literal -- a literal here silently shadowed the projection and
# re-narrowed CODEX_ALLOWED_MODELS to the entitled slugs, which collapses the
# unentitled/unknown distinction the tier table exists to draw (MERGEDEAD-H-01).
CODEX_ALLOWED_MODELS = CODEX_ALLOWED_MODELS | {DEFAULT_CODEX_MODEL}
...
# The short S1 review samples (only one successful regime) cannot estimate
# implementation-turn tails. Until representative tail measurements exist,
# use a policy allowance of 30 minutes, bounded by the 300s floor and SSOT
# 3600s ceiling. This is an operational default, not a measured p95.
# WORKBAY_LANE_TIMEOUT_S (neutral) and WORKBAY_CODEX_LANE_TIMEOUT_S (legacy)
# accept integer seconds in [300, 3600]. One shared resolver reads both;
# this function is the compatibility wrapper. Invalid or conflicting values
# raise a named refusal for Codex defaults without preventing backend
# registry startup. This default does not override wave ceilings or
# token-budget cycle bounds.
WORKBAY_CODEX_LANE_TIMEOUT_ENV = "WORKBAY_CODEX_LANE_TIMEOUT_S"


def resolve_codex_lane_timeout_s() -> int:
    """Resolve the dispatch override, rejecting invalid bounds without clamping."""
    return resolve_lane_timeout().timeout_s


def __getattr__(name: str) -> int:
    """Resolve the compatibility timeout pin only when a consumer requests it."""
    if name == "LANE_TIMEOUT_S":
        return resolve_codex_lane_timeout_s()
...
# Provisioning evidence has a six-hour maximum age. This is a reuse bound, not
# an expiry-only refresh policy: readers should refresh ahead of the bound while
# still admitting a usable positive record. Operators may choose a different
# bounded reuse window via WORKBAY_PREFLIGHT_ATTESTATION_TTL_S.
PREFLIGHT_ATTESTATION_TTL_S = 6 * 60 * 60
PREFLIGHT_VERDICT_RECORDS_ENV = "WORKBAY_PREFLIGHT_VERDICT_RECORDS"
PREFLIGHT_OVERRIDE_FILE = ".task-state/remote-preflight-override.json"
PREFLIGHT_PROBE_MODEL_ENV = "WORKBAY_PREFLIGHT_PROBE_MODEL"
PREFLIGHT_PROBE_MAX_LIVE_ENV = "WORKBAY_PREFLIGHT_MAX_LIVE_PROBES"
PREFLIGHT_PROBE_MAX_LIVE = 2
