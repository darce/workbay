...
# WORKBAY_CODEX_LANE_TIMEOUT_S accepts integer seconds in [300, 3600].
# It is read at dispatch (no restart required); invalid values raise a typed
# refusal for Codex defaults without preventing backend registry startup.
# This default does not override wave ceilings or token-budget cycle bounds.
WORKBAY_CODEX_LANE_TIMEOUT_ENV = "WORKBAY_CODEX_LANE_TIMEOUT_S"


class CodexLaneTimeoutError(ValueError):
    """The operator supplied an invalid Codex lane wall-clock bound."""


def resolve_codex_lane_timeout_s() -> int:
    """Resolve the dispatch override, rejecting invalid bounds without clamping."""
    raw = os.environ.get(WORKBAY_CODEX_LANE_TIMEOUT_ENV)
    if raw is None:
        return max(LANE_TIMEOUT_MIN_S, min(30 * 60, LANE_TIMEOUT_MAX_S))
    message = (
        f"{WORKBAY_CODEX_LANE_TIMEOUT_ENV} must be an integer between "
        f"{LANE_TIMEOUT_MIN_S} and {LANE_TIMEOUT_MAX_S} seconds"
    )
    try:
        timeout = int(raw)
