...
def floor_for(lane_kind: str, env: Mapping[str, str] | None = None) -> int:
    ...
    kind_default = MIN_REMOTE_REVIEW_TOKEN_BUDGET if kind == "review" else MIN_REMOTE_IMPLEMENT_TOKEN_BUDGET
    ...
    if raw:
        try:
            ...
            raise ValueError(f"{MIN_REMOTE_TOKEN_BUDGET_ENV} must be a positive integer, got {raw!r}") from exc
        if override > 0:
...
def recommend_token_budget(
    ...
    *,
    ...
    ``max(floor, min(400_000, round_up(int(p90 * 1.2))))``.
    ...
    *env* is forwarded to :func:`floor_for`; omitted means ``os.environ``.
    ...
    rounded = _round_up_to(scaled)
    recommended = max(floor, min(RECOMMENDED_BUDGET_CAP, rounded))
    return BudgetAdvice(
        floor=floor,
        sample_lanes=sample_lanes,
        p50=p50,
...
def _format_reason(
    *,
    ...
    clock = ", ".join(clock_parts) if clock_parts else f"timeout={clock_status or 'not_derived'}"
    return (
        f"token_budget={token_budget} {relation} {bound_label} {bound_value} with sample_lanes={sample_lanes} ({clock})"
    )
...
def check_token_budget(
    ...
    *,
    ...
    except Exception as exc:  # noqa: BLE001 — still emit a verdict if bounds cannot be derived
        _logger.warning(
            "could not derive token-budget bounds for token_budget=%s: %s",
            token_budget,
            exc,
            exc_info=True,
        )
    ...
    clock_status = "unavailable" if bounds_failed else None
