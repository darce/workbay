...
from workbay_handoff_mcp import (
    get_handoff_state as _handoff_get_handoff_state,
)
from workbay_handoff_mcp import (
    latest_lane_landing as _handoff_latest_lane_landing,
)
from workbay_handoff_mcp.lanes_api import get_lane as _handoff_get_lane

from workbay_orchestrator_mcp.orchestration.git_lock import classify_git_failure
...
# The read-only database seams below are intentionally active only while the
# scanner evaluates a row. Direct predicate callers retain the established
# handoff API behavior and its test seams; a scan cannot accidentally fall
# back to that writer-backed path [DBG-06][OBS-08].
_SCAN_READ_PASS_ACTIVE: ContextVar[bool] = ContextVar("lane_reclaim_scan_read_pass", default=False)


@contextmanager
def _scan_read_pass():
    token = _SCAN_READ_PASS_ACTIVE.set(True)
    try:
        yield
    finally:
        _SCAN_READ_PASS_ACTIVE.reset(token)
...
_SCAN_BUDGET_ENV = "WORKBAY_RECLAIM_SCAN_BUDGET_S"
_SCAN_GIT_TIMEOUT_ENV = "WORKBAY_RECLAIM_SCAN_GIT_TIMEOUT"
_SCAN_BUDGET_DEFAULT_SEC = 300.0
_SCAN_GIT_TIMEOUT_DEFAULT_SEC = 10.0
_SCAN_WRITE_CHUNK_SIZE = 50


def _scan_budget_seconds() -> float:
    """Return the non-negative scan wall budget, defaulting on bad input."""
    raw = os.environ.get(_SCAN_BUDGET_ENV)
    if raw is None:
...
def _scan_git_timeout_seconds() -> float:
    """Return the positive per-probe git timeout [AGT-10]."""
    raw = os.environ.get(_SCAN_GIT_TIMEOUT_ENV)
    if raw is None:
        return _SCAN_GIT_TIMEOUT_DEFAULT_SEC
    try:
        value = float(raw)
    except (TypeError, ValueError):
        return _SCAN_GIT_TIMEOUT_DEFAULT_SEC
...
def _update_ref_reports_lock_contention(stderr: str | None) -> bool:
    ...
    """
    return classify_git_failure(stderr or "") is not None
...
def _worktree_status(worktree: Path | str) -> str | None:
    ...
    result = subprocess.run(
        ...
        text=True,
        check=False,
        timeout=_scan_git_timeout_seconds(),
    )
...
def _git_run(
    ...
    *args: str,
    ...
    effective_timeout = _scan_git_timeout_seconds() if timeout is None else timeout
    try:
...
def _list_lanes_by_worktree_path(*, worktree_path: str) -> dict[str, Any]:
    """Cross-task path owners through the scan's read-only connection."""
    if not _SCAN_READ_PASS_ACTIVE.get():
        from workbay_handoff_mcp.lanes_recording import (  # noqa: PLC0415
            list_lanes_by_worktree_path as handoff_list_lanes_by_worktree_path,
        )

        return handoff_list_lanes_by_worktree_path(worktree_path=worktree_path)
    normalized = str(worktree_path or "").strip()
    if not normalized:
        return {
            "ok": False,
    ...
    try:
        ...
        return _scan_read_error("list_lanes_by_worktree_path", exc)


def _list_lanes_by_branch(*, branch: str) -> dict[str, Any]:
    """Cross-task owners of *branch* for C4 through a read-only connection."""
    if not _SCAN_READ_PASS_ACTIVE.get():
        from workbay_handoff_mcp.lanes_recording import list_lanes_by_branch  # noqa: PLC0415

        return list_lanes_by_branch(branch=branch)
    normalized = str(branch or "").strip()
    if not normalized:
        return {"ok": False, "tool": "list_lanes_by_branch", "data": {"error": "branch is required."}}
    short = _short_branch_name(normalized)
    try:
        with _scan_read_connection() as conn:
            rows = [dict(row) for row in conn.execute("SELECT * FROM worktree_lanes ORDER BY id DESC").fetchall()]
    except Exception as exc:  # noqa: BLE001 — unknown listing is not an empty universe
        return _scan_read_error("list_lanes_by_branch", exc)
    matched: list[Any] = []
    for row in rows:
        if not isinstance(row, dict):
            return {"ok": False, "tool": "list_lanes_by_branch", "data": {"error": "malformed_lane_row"}}
...
def _parse_lane_timestamp(value: object) -> datetime | None:
...
def _lane_notes_prose(lane: dict[str, Any]) -> str:
    """Return notes envelope prose; never substring-search the raw column."""
    from workbay_orchestrator_mcp.lanes import NOTES_PROSE_KEY  # noqa: PLC0415
    ...
    if envelope is None:
        return ""
    prose = envelope.get(NOTES_PROSE_KEY)
    return prose if isinstance(prose, str) else ""


def blocked_fallback_is_ageable(
    ...
    *,
    ...
    """True when a fallback-blocked lane may clear reclaim condition 1.

    All of these must hold: status is ``blocked``; the notes envelope prose
    carries ``BLOCKED_UNCLASSIFIED_NOTE_MARKER``; ``COALESCE(updated_at,
    created_at)`` is at least ``grace_hours`` older than ``now``; and the
    worktree session is not live. Environment-blocked lanes share status
    ``blocked`` but not the marker, so they stay refused as ``not_terminal``.
    """
    from workbay_orchestrator_mcp.orchestration.orchestrator_guidance import (  # noqa: PLC0415
        BLOCKED_UNCLASSIFIED_NOTE_MARKER,
    )
    ...
    if lane.get("status") != "blocked":
        return False
    if BLOCKED_UNCLASSIFIED_NOTE_MARKER not in _lane_notes_prose(lane):
        return False
    stamp = _parse_lane_timestamp(lane.get("updated_at")) or _parse_lane_timestamp(lane.get("created_at"))
    if stamp is None:
        return False
    if _as_aware_datetime(now) - stamp < timedelta(hours=float(grace_hours)):
        return False
    worktree = lane.get("worktree_path")
    if not isinstance(worktree, str) or not worktree.strip():
        return False
    if _session_live(orchestrator_root=orchestrator_root, worktree=worktree):
        return False
    return True
...
def lane_worktree_reclaimable(
    *,
    ...
    # -- 1: terminal status --------------------------------------------------
    # Ageable unclassifiable-blocked lanes clear this gate without widening
    # _TERMINAL_STATUSES. Carry that admission in observed, never in reason.
    fallback_reclaim = blocked_fallback_is_ageable(
        lane if isinstance(lane, dict) else {},
        orchestrator_root=root,
        now=now,
    )
    if fallback_reclaim:
        observed["blocked_fallback_reclaim"] = True
    elif status not in _TERMINAL_STATUSES:
        return _refuse("not_terminal", observed)

    # -- 2/3: landing (waived for ageable fallback; those lanes never landed)
    if not fallback_reclaim:
...
def lane_branch_reclaimable(
    *,
    ...
    scan = _branch_unreachable_commits(root, branch, integration_ref=integration_ref, branch_sha=branch_sha)
    if not scan.ok:
        observed["orphan_scan_failed_probe"] = scan.failed_probe
        observed["orphan_scan_stderr"] = scan.stderr
        return _refuse(f"unknown:{scan.failed_probe}", observed)
...
def _lane_row_is_branch_reclaim_scan_target(lane: dict[str, Any], *, branch_refs: dict[str, str] | None) -> str | None:
...
# The scan is a read-mostly maintenance operation. ``lanes_api.list_lanes``
# routes through the handoff writer/bootstrap connection, which is safe for a
# normal API call but lets a large registry walk participate in RESERVED-lock
# contention. Keep the module-level seam named ``list_lanes`` so existing
# callers/tests can still replace it, while production scans use the explicit
# read-only factory [DBG-06][OBS-08].
@contextmanager
def _scan_read_connection():
    """Yield one mode=ro handoff connection for scan-only registry reads."""
    ...
    conn = connect_handoff_db(get_runtime_config().db_path, read_only=True)
    try:
        yield conn
    finally:
        conn.close()
...
def _scan_read_error(tool: str, exc: Exception) -> dict[str, Any]:
    return {"ok": False, "tool": tool, "data": {"error": str(exc)}}


def get_lane(*, lane_id: str, task_ref: str | None = None) -> dict[str, Any]:
    """Read one lane through the scan's read-only connection."""
    if not _SCAN_READ_PASS_ACTIVE.get():
        return _handoff_get_lane(lane_id=lane_id, task_ref=task_ref)
    if not isinstance(lane_id, str) or not lane_id.strip():
        return {"ok": False, "tool": "get_lane", "data": {"error": "lane_id is required."}}
    if not isinstance(task_ref, str) or not task_ref.strip():
        return {"ok": False, "tool": "get_lane", "data": {"error": "task_ref is required."}}
    try:
        with _scan_read_connection() as conn:
            row = conn.execute(
                "SELECT * FROM worktree_lanes WHERE task_ref = ? AND lane_id = ?",
                (task_ref.strip(), lane_id.strip()),
            ).fetchone()
            if row is None:
                return {
                    "ok": False,
                    "tool": "get_lane",
                    "data": {"error": f"Lane '{lane_id.strip()}' not found for task '{task_ref.strip()}'."},
                }
            return {
                "ok": True,
                "tool": "get_lane",
                "data": {"lane": dict(row), "task_ref": task_ref.strip()},
            }
    except Exception as exc:  # noqa: BLE001 — typed read refusal
        return _scan_read_error("get_lane", exc)


def latest_lane_landing(*, lane_id: str, task_ref: str) -> dict[str, Any]:
    """Read the newest lane landing without entering the writer factory."""
    if not _SCAN_READ_PASS_ACTIVE.get():
        return _handoff_latest_lane_landing(lane_id=lane_id, task_ref=task_ref)
    if not isinstance(lane_id, str) or not lane_id.strip():
        return {"ok": False, "tool": "latest_lane_landing", "data": {"error": "lane_id is required."}}
    if not isinstance(task_ref, str) or not task_ref.strip():
        return {"ok": False, "tool": "latest_lane_landing", "data": {"error": "task_ref is required."}}
    normalized_task = task_ref.strip()
    normalized_lane = lane_id.strip()
    decision_id = f"lane_landed_{normalized_task}_{normalized_lane}"
    try:
        with _scan_read_connection() as conn:
            row = conn.execute(
                """
                ...
                FROM decisions
                WHERE task_ref = ? AND decision = ? AND commit_sha IS NOT NULL AND TRIM(commit_sha) <> ''
                ORDER BY created_at DESC, id DESC
                LIMIT 1
                """,
                (normalized_task, decision_id),
            ).fetchone()
            landing = None if row is None else dict(row)
            return {
                "ok": True,
        ...
        return _scan_read_error("latest_lane_landing", exc)


def get_handoff_state(task_ref: str | None = None, **_kwargs: Any) -> dict[str, Any]:
    """Read only the active handoff identity needed by reclaim predicates."""
    if not _SCAN_READ_PASS_ACTIVE.get():
        return _handoff_get_handoff_state(task_ref=task_ref, **_kwargs)
    if not isinstance(task_ref, str) or not task_ref.strip():
        return {"ok": False, "tool": "get_handoff_state", "data": {"error": "task_ref is required."}}
    normalized_task = task_ref.strip()
    try:
        with _scan_read_connection() as conn:
            row = conn.execute("SELECT * FROM handoff_state WHERE task_ref = ?", (normalized_task,)).fetchone()
            active = None if row is None else dict(row)
            return {
                "ok": True,
        ...
        return _scan_read_error("get_handoff_state", exc)
...
def list_lanes(
    *,
    ...
    """Read a lane page without opening the handoff writer connection.

    This is intentionally a local read seam rather than a change to the
    handoff package: the write-set for this lane owns the reclaim scanner, and
    the existing module-level ``list_lanes`` patch point remains compatible.
    The connection is opened ``mode=ro`` and is closed before the next page.
    """
    ...
    if not all_tasks and task_ref is None:
        return {
            "ok": False,
            "tool": "list_lanes",
            "data": {"error": "task_ref is required for a reclaim scan."},
        }
    try:
        with _scan_read_connection() as conn:
            page_limit = max(1, int(limit))
            page_offset = max(0, int(offset))
            params: list[object] = [] if all_tasks else [task_ref]
            ...
            if type(after_id) is int:
                query_params: list[object] = [*params, after_id, page_limit + 1]
                rows = conn.execute(
                ...
                rows = conn.execute(
                    f"SELECT * FROM worktree_lanes WHERE {where_sql} ORDER BY id DESC LIMIT ? OFFSET ?",
            ...
            page = [dict(row) for row in rows[:page_limit]]
            next_after_id = page[-1].get("id") if has_more and page else None
            return {
                "ok": True,
                "tool": "list_lanes",
                "data": {
                    "task_ref": None if all_tasks else task_ref,
                    "status": status,
    ...
    except Exception as exc:  # noqa: BLE001 — a failed read is a typed refusal
        return {"ok": False, "tool": "list_lanes", "data": {"error": str(exc)}}
...
@dataclass
class _PendingScanWrite:
    """One post-read ledger write, kept out of the probe/evaluation pass."""

    task_ref: str
    lane_id: str
    verdict: Any
    branch: bool
    candidate: ReclaimCandidate | None = None
...
def _load_scan_cursor(root: Path, *, task_ref: str | None, all_tasks: bool) -> tuple[int | None, str | None]:
...
def _save_scan_cursor(root: Path, *, task_ref: str | None, all_tasks: bool, after_id: int) -> None:
    path = _scan_cursor_path(root, task_ref=task_ref, all_tasks=all_tasks)
...
def _list_all_lanes(
    ...
    *duplicate or skip existing rows under an OFFSET cursor*
    ...
    rows_to_skip = max(0, int(skip_rows))
    for _ in range(_LIST_LANES_MAX_PAGES):
        ...
        if not has_more:
            ...
            if start_after_id is None and row_cap is None and type(total) is int and total != len(all_lanes):
                return None, env
            return _LaneSweep(all_lanes[rows_to_skip:], page_count=page_count), first_env
...
def _normalize_scan_git_timeout(verdict: ReclaimVerdict | None) -> ReclaimVerdict | None:
    """Name timeout outcomes consistently at the scan boundary [AGT-10]."""
    if verdict is None:
        return None
    observed = verdict.observed if isinstance(verdict.observed, dict) else {}
    timed_out = verdict.reason == "containment_probe_timeout" or any(value == "timeout" for value in observed.values())
    if timed_out:
        verdict.reason = "unknown_git_timeout"
        observed["git_timeout"] = True
...
def scan_terminal_lanes(
    *,
    ...
    verdict via ``record_reclaim_candidate``. Evaluation is a read-only pass;
    ledger writes are queued and flushed afterwards in bounded chunks so a
    slow git/OS probe never holds the shared RESERVED lock [DBG-06][OBS-08].
    Removes nothing. Keyword-only so ``task_ref`` cannot be swapped with a path
    by positional call order.
    ...
    def emit_progress(rows_done: int, rows_total: int | None = None) -> None:
        if rows_done <= 0 or rows_done % 50 != 0:
            return
        try:
            if callable(progress):
                progress(rows_done, rows_total)
        except Exception:  # noqa: BLE001 — progress is diagnostic only
            pass
    ...
    lanes, env = _list_all_lanes(
        ...
        max_rows=(
            _SCAN_PER_CALL_CAP + requested_offset if requested_limit is None else requested_limit + requested_offset
        ),
        skip_rows=requested_offset,
    )
    if lanes is None:
    ...
    if budget_expired():
        _safe_log(
            log,
            "WARNING",
            "lane_reclaim_scan_budget_exceeded",
    ...
    examined_count = 0
    decided_count = 0
    pending_writes: list[_PendingScanWrite] = []
    budget_hit = False
    ...
    for row_index, raw_lane in enumerate(lanes):
        ...
        if not isinstance(raw_lane, dict):
            ...
            examined_count += 1
            emit_progress(examined_count, len(lanes))
        ...
        try:
            ...
            if (
                ...
                branch_target = lane_id
            if not git_backed and lane_branch_reclaimable is _LANE_BRANCH_RECLAIMABLE_IMPL:
            ...
            try:
                ...
                branch_verdict = _normalize_scan_git_timeout(branch_verdict)
            except AssertionError:
                raise
                ...
                branch_verdict = ReclaimVerdict(
                    ...
                    reason=(
                        "unknown_git_timeout"
                        if isinstance(branch_exc, subprocess.TimeoutExpired)
                        else "branch_evaluation_failed"
                    ),
                    observed={
                        "task_ref": row_task_ref,
                        "lane_id": lane_id,
            ...
            if branch_target is not None and branch_verdict is not None:
            ...
            verdict = _normalize_scan_git_timeout(verdict)
        except AssertionError:
            raise
            ...
            # Invalidate any durable reclaimable:true after the read pass. A
            # timeout is a typed unknown rather than a generic evaluation
            # failure [AGT-10]; it still queues a non-positive durable verdict.
            ...
            inv_verdict = ReclaimVerdict(
                reclaimable=False,
                reason=("unknown_git_timeout" if isinstance(exc, subprocess.TimeoutExpired) else "evaluation_failed"),
                observed=inv_observed,
            )
            timeout_candidate: ReclaimCandidate | None = None
            if isinstance(exc, subprocess.TimeoutExpired):
                timeout_candidate = ReclaimCandidate(
                    task_ref=row_task_ref,
                    lane_id=lane_id,
                    worktree_path=worktree_path,
                    verdict=inv_verdict,
                ...
                candidates.append(timeout_candidate)
            pending_writes.append(
                _PendingScanWrite(
                    task_ref=row_task_ref,
                    lane_id=lane_id,
        ...
        # The branch write was queued before the candidate existed; attach the
        # result object now so the post-read flush can report its outcome.
        for pending in reversed(pending_writes):
            if (
                pending.branch
                and pending.candidate is None
                and pending.task_ref == row_task_ref
                and pending.lane_id == lane_id
            ):
    ...
    # All ledger writes happen after the read/evaluation pass. Each recorder
    # owns one short transaction; chunking keeps this maintenance path from
    # creating a single long RESERVED window [DBG-06][OBS-08].
    for chunk_start in range(0, len(pending_writes), _SCAN_WRITE_CHUNK_SIZE):
        chunk = pending_writes[chunk_start : chunk_start + _SCAN_WRITE_CHUNK_SIZE]
        for pending in chunk:
            ...
            except Exception as write_exc:  # noqa: BLE001 — one write must not abort a batch
                _safe_log(
                    log,
                    "ERROR",
                    ("lane_branch_reclaim_scan_record_failed" if pending.branch else "lane_reclaim_scan_record_failed"),
                    lane=pending.lane_id,
                    task_ref=pending.task_ref,
                    error=str(write_exc),
            ...
            if pending.branch:
                if candidate is not None:
                    candidate.branch_recorded = recorded
                if not recorded:
                    branch_unrecorded_lanes.append(pending.lane_id)
            else:
                if candidate is not None:
                    candidate.recorded = recorded
                if not recorded:
                    unrecorded_lanes.append(pending.lane_id)
    ...
    elif branch_inventory_unavailable and lane_branch_reclaimable is _LANE_BRANCH_RECLAIMABLE_IMPL:
        scan_complete = False
