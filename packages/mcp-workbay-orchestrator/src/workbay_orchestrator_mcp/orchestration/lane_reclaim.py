...
def _survivor_blob_indexes(
    ...
    *index* is None only when the for-each-ref probe itself fails [CON-05].
    ...
    candidates: list[str] = []
    seen: set[str] = set()
    for raw in (integration_ref, *[line.strip() for line in (heads.stdout or "").splitlines() if line.strip()]):
        ref = raw.strip()
        if not ref:
            continue
...
def _branch_unreachable_commits(root: Path, branch: str) -> OrphanScan:
    ...
    survivors = [
        ref.strip() for ref in (survivors_proc.stdout or "").splitlines() if ref.strip() and ref.strip() != lane_ref
    ]
...
def scan_terminal_lanes(
    *,
    ...
    for row_index, raw_lane in enumerate(lanes):
        ...
        try:
            target = _lane_row_is_reclaim_scan_target(raw_lane, orchestrator_root=root)
            if target is None:
                continue
        ...
        recorded = False
        try:
            recorded = bool(record_reclaim_candidate(task_ref=task_ref, lane_id=lane_id, verdict=verdict, log=log))
