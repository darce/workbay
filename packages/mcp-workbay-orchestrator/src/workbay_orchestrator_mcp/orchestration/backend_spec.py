...
from workbay_orchestrator_mcp.orchestration.grok_lane_config import GROK_MAX_TURNS_CAP
...
CURSOR_ENV_FILE = "~/.config/cursor-agent/env"
...
# Backends with an invocation recipe in this module (S2–S7).
_INVOCATION_BACKENDS = frozenset({"grok-remote", "codex-remote", "cursor-remote"})
...
def _validate_env_file(env_file: str | None) -> None:
    ...
    if "\0" in env_file or not _ENV_FILE_RE.fullmatch(env_file):
        raise ValueError(
            f"env_file not whole-token safe (need ^~?/[A-Za-z0-9._+/-]+$): {env_file!r}"
        )
...
def _refuse_placeholder_in_operator(value: str, *, field_name: str) -> None:
    if _PLACEHOLDER_TOKEN.search(value):
        raise PlaceholderInOperatorValueError(
            f"operator value {field_name}={value!r} contains a placeholder token"
        )
...
def _refuse_nul(value: str, *, field_name: str) -> None:
    if "\0" in value:
        raise NulInSpecValueError(
            f"operator value {field_name}={value!r} contains a NUL character "
            "(collides with .argv sidecar delimiter)"
        )
...
def _refuse_placeholder_element(value: str, *, field_name: str) -> None:
    ...
    if value.strip() in _KNOWN_PLACEHOLDERS:
        raise PlaceholderInOperatorValueError(
            f"operator value {field_name}={value!r} is exactly a recipe placeholder"
        )
...
def build_agent_spec(
    ...
    *,
    ...
    if effort not in REMOTE_EFFORTS:
        raise ValueError(
            f"effort {effort!r} not in {sorted(REMOTE_EFFORTS)}; "
            "refusing to ship a substituted effort"
        )
    ...
    if max_turns is not None and not caps.supports_token_budget_cycle_bounds:
        raise UnenforceableBoundError(
            f"{backend_id} cannot enforce max_turns "
            "(supports_token_budget_cycle_bounds is False)"
        )
    ...
    if backend_id == "grok-remote":
        if max_turns is None:
            raise UnenforceableBoundError(
                "grok-remote requires max_turns (supports_token_budget_cycle_bounds)"
            )
        return AgentSpec(
            spec_version=2,
    ...
    if backend_id == "codex-remote":
        return AgentSpec(
            ...
            requires_live_sandbox_preflight=WORKSPACE_WRITE_REQUIRES_LIVE_PREFLIGHT,
        )

    if backend_id == "cursor-remote":
        slug, encoded_effort, _downgrade = resolve_cursor_model(model, effort)
...
def write_agent_spec(spec: AgentSpec, path: Path | str) -> tuple[Path, Path]:
    ...
    for i, el in enumerate(spec.argv):
        if "\0" in el:
            raise NulInSpecValueError(
                f"argv[{i}] contains a NUL character "
                "(collides with .argv sidecar delimiter)"
            )
...
def _cli_main(argv: list[str] | None = None) -> int:
    ...
    build.add_argument(
        ...
        default=None,
        help=(
            "Required for turn-bounded backends; grok-remote defaults to "
            f"GROK_MAX_TURNS_CAP ({GROK_MAX_TURNS_CAP})."
        ),
    )
    build.add_argument(
        "--prompt-file",
