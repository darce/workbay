...
from workbay_protocol.reasoning_effort import CONCRETE_EFFORTS as REMOTE_EFFORTS  # noqa: F401 — compatibility
...
# Effort defaults live in orchestration/effort_policy.py; re-exported here
# because pre-move call sites still import the name from this module, and two
# definitions of a default is exactly how the luna max rule gets lost. Absolute,
# not relative: this module is also executed as a script by the retired-env-pin
# CLI test, where a relative import has no parent package.
from workbay_orchestrator_mcp.orchestration.effort_policy import default_effort_for_model  # noqa: F401 — compatibility
...
class OperatorValuesArityError(ValueError):
    """AgentSpec does not carry the fixed operator-values compatibility tuple."""
...
class AgentSpec:
    ...
    # Additive v2 routing receipts.  Keep these as the shared typed value in
    # process; ``asdict`` emits the stable JSON object expected by the VM-side
    # artifact readers.
    routing_requested: RoutingQuad = field(default_factory=RoutingQuad)
    routing_effective: RoutingQuad = field(default_factory=RoutingQuad)

    def __post_init__(self) -> None:
        _validate_env_file(self.env_file)
        if not isinstance(self.routing_requested, RoutingQuad):
            raise TypeError("routing_requested must be a RoutingQuad")
        if not isinstance(self.routing_effective, RoutingQuad):
            raise TypeError("routing_effective must be a RoutingQuad")

    def to_json_dict(self) -> dict[str, Any]:
        d = asdict(self)
        d.pop("effort_downgrade_reason", None)
        # RoutingQuad carries an in-process OBS-08 provenance receipt, but the
        # remote AgentSpec wire shape remains the five routing coordinates.
        d["routing_requested"] = self.routing_requested.as_dict()
        d["routing_effective"] = self.routing_effective.as_dict()
        return d


_OPERATOR_VALUE_FIELDS = ("model_or_slug", "effort_or_empty", "codex_service_tier_or_empty")


def _require_operator_values_contract(spec: AgentSpec) -> None:
    expected = len(_OPERATOR_VALUE_FIELDS)
    values = spec.operator_values
    if not isinstance(values, list) or len(values) != expected or not all(isinstance(value, str) for value in values):
        raise OperatorValuesArityError(
            f"{spec.backend_id} AgentSpec requires exactly {expected} operator_values "
            f"{_OPERATOR_VALUE_FIELDS} as a list of exactly {expected} strings; "
...
def _openrouter_remote_argv(*, model: str, effort: str | None) -> list[str]:
    ...
    """
    return _codex_cli_argv(model=model, effort=effort) + [
        "-c",
...
def _codex_cli_argv(
    *,
    ...
    """Build provider-neutral ``codex exec`` argv.

    Provider/model entitlement checks belong to the dispatch surface that owns
    the model catalogue.  OpenRouter uses this recipe too, but its slugs are
    intentionally absent from ``CODEX_MODEL_TIERS``.
    """
    argv = ["exec", "--json", "--ignore-user-config", "-C", ".", "-m", model]
...
def _codex_remote_argv(
    *,
    ...
    # Two SSOTs answer "may this model dispatch", and a curated tier row is only
    # one of them. CODEX_MODEL_ALLOWED_SERVICE_TIERS records models MEASURED on
    # the lane VM; gpt-6-astra has a measured entry and deliberately no tier row
    # (test_codex_model_tiers asserts it stays out of CODEX_ALLOWED_MODELS, so
    # adding a row is not an available fix). Requiring a row here refused a
    # model the operator explicitly pinned and the VM was measured against.
    # A slug with NEITHER a row NOR a measured entry is still unknown_model, so
    # a typo cannot reach the VM.
    model_tier = CODEX_MODEL_TIERS.get(model)
    measured_service_tiers = CODEX_MODEL_ALLOWED_SERVICE_TIERS.get(model)
    if model_tier is None and measured_service_tiers is None:
        raise ValueError(
            f"Refusing codex-remote dispatch with model {model!r}: unknown_model (codex_lane_config.CODEX_MODEL_TIERS)"
        )
    if model_tier is not None and not model_tier.entitled:
        raise ValueError(
            f"Refusing codex-remote dispatch with model {model!r}: tier_not_entitled; "
            f"tier {model_tier.tier!r} is not entitled"
        )
    # No curated row means no curated effort vocabulary; fall back to the
    # remote-wide vocabulary rather than accepting anything.
    allowed_efforts = model_tier.allowed_efforts if model_tier is not None else REMOTE_EFFORTS
    tier_clause = f"tier {model_tier.tier!r}" if model_tier is not None else "measured (no curated tier row)"
    if effort is not None and effort not in allowed_efforts:
        raise ValueError(
            f"Refusing codex-remote dispatch with effort {effort!r}: {model!r} "
            f"{tier_clause} advertises {sorted(allowed_efforts)} "
            "(codex_lane_config.CODEX_MODEL_TIERS)"
        )

    if service_tier is not None:
        advertised_tiers = measured_service_tiers or frozenset()
        if service_tier not in advertised_tiers:
            raise ValueError(
                f"Refusing codex-remote dispatch for {model!r} {tier_clause} "
                f"with service tier {service_tier!r}: advertises {sorted(advertised_tiers)} "
                "(codex_lane_config.CODEX_MODEL_TIERS)"
            )
    return _codex_cli_argv(model=model, effort=effort, service_tier=service_tier)
...
def _seal_agent_spec(spec: AgentSpec) -> AgentSpec:
    """Construction chokepoint: enforce wire shape and a probeable recipe set."""
    _require_operator_values_contract(spec)
...
def resolve_remote_lane_timeout_s(backend_id: str, requested: int | None = None) -> int:
    ...
    ceiling = remote_lane_timeout_ceiling_s(backend_id)
    if int(requested) > ceiling:
        # REF-03: an explicit request must never silently become a shorter run.
        raise ValueError(
            f"timeout_request_clamped: requested={requested} effective={ceiling} backend={backend_id}"
        )
...
def build_agent_spec(
    ...
    *,
    ...
    validate_reasoning_effort(effort, allow_modes=False)
    ...
    requested_routing = RoutingQuad(
        backend=backend_id,
        model=model,
        effort=effort,
        speed=speed,
        tier=CODEX_MODEL_TIERS[model].tier if model in CODEX_MODEL_TIERS else None,
    )
    ...
    if backend_id == "grok-remote":
        ...
        return _seal_agent_spec(
            AgentSpec(
                ...
                routing_requested=requested_routing,
                routing_effective=requested_routing,
            )
        )
    ...
    if backend_id == "codex-remote":
        ...
        return _seal_agent_spec(
            AgentSpec(
                ...
                # Compatibility carrier retained for one release.  Its shape is
                # fixed even when no service tier was requested; typed routing
                # above is the new source for field names.
                operator_values=[model, operator_effort, service_tier or ""],
                stdin="{brief_file}",
                stdout="{stream_file}",
                stderr="{run_log}",
                close_fds=[9],
                ...
                routing_requested=requested_routing,
                routing_effective=RoutingQuad(
                    backend="codex-remote",
                    model=model,
                    effort=effort,
                    speed=speed,
                    # A measured-but-uncurated model has no curated tier. None
                    # is RoutingQuad's explicit "no source supplied a value",
                    # which is the honest receipt -- unlike a KeyError or a
        ...
        return _seal_agent_spec(
            AgentSpec(
                ...
                routing_effective=RoutingQuad(
                    ...
                    model=slug,
                    effort=encoded_effort or effort,
                    speed=None,
                    tier=None,
                ),
                requires_live_sandbox_preflight=False,
