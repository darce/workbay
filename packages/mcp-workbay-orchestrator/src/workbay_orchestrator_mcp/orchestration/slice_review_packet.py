...
def _matching_plan_cursor(
    ...
    *,
    ...
    # LIMIT 1 single-row selection (the latest cursor at/before the decision):
    # order-invariant, contributes scalar fields only — no rendered growing
    # list, so no ascending reversal is required for prefix-stability here.
...
def _matching_worker_report(
    ...
    *,
    ...
    # Selection keeps the newest 20 (DESC+LIMIT above). Iterate chronological
    # ascending (reverse the DESC fetch) and let the newest row win score ties
    # (>=), which reproduces the prior newest-first selection exactly while
    # making the scan order deterministic and tail-growing (prefix-stable).
...
def _matching_test_commands(
    ...
    *,
    ...
    # Selection keeps the newest 20 (DESC+LIMIT above), but this list is
    # rendered as-is into the packet. Reverse to chronological ascending
    # (verified_at ASC, id ASC) so a newly verified command appends at the
    # tail and prior rendered entries keep their positions (prefix-stable).
...
def _lane_grants(workspace_root: Path, task_ref: str, lane_id: str | None) -> dict[str, Any] | None:
    """Return the declared ``grants`` block for a lane, or ``None``.

    Reviewers see the lane's declared write surface (adoption A) next to the
    touched files. Missing manifest / lane / grants block degrades to ``None``.
    """
    if not lane_id:
        return None
    try:
        ...
        lane = get_lane_config(task_ref, lane_id, orchestrator_root=str(workspace_root))
    except (FileNotFoundError, RuntimeError, OSError):
        return None
    if not isinstance(lane, dict):
        return None
    grants = lane.get("grants")
...
def _build_packet_for_decision(
    ...
    *,
    ...
    return {
        ...
        "grants": _lane_grants(workspace_root, task_ref, decision_row.get("lane_id")),
        "outcome": matched_report.get("outcome") if matched_report else None,
...
def _slice_complete_decision_rows(
    ...
    *,
    ...
    # unrelated decisions and consumes the LIMIT window before the post-filter.
    escaped_prefix = SLICE_DECISION_PREFIX.replace("_", r"\_")
    where_sql = "task_ref = ? AND (decision LIKE ? ESCAPE '\\' OR decision LIKE ? ESCAPE '\\')"
    params: list[Any] = [task_ref, f"{escaped_prefix}%", f"%\\_{escaped_prefix}%"]
    if lane_id is not None:
        where_sql += " AND lane_id = ?"
        params.append(lane_id)
    rows = conn.execute(
    ...
    # DESC is deliberate and MUST NOT be reversed here: callers treat the
    # first element as the *latest* slice-complete decision
    # (``get_latest_slice_review_packet_data`` selection + slice_label lookup).
    # This is a selection helper (which single decision to render a packet
    # for), not a rendered growing list; flipping to ascending would silently
    # select the oldest decision — the regression the plan calls out (§6).
