...
def _run_model_curation(
    *,
    ...
    # Coupling note: see lane_exec — cycle-bounds gate implies grok CTOR kwargs (unused here).
    adapter_kwargs: dict[str, Any] = {}
    if backend_supports_token_budget_cycle_bounds(backend):
        # Curation path has no cycle bounds today; still enter the gate so a
        # future bounds thread lands in one place. Local Composer config is
        # grok-cli-only (remote turns do not consume a local config).
        if backend == "grok-cli":
            from workbay_orchestrator_mcp.orchestration.bootstrap_lane import (  # noqa: PLC0415
                ensure_grok_lane_config,
            )

            ensure_grok_lane_config(Path.cwd(), model)
