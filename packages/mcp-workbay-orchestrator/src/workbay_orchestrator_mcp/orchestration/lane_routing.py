...
def _tier_quad(tier: str, tier_table: Mapping[str, Any]) -> RoutingQuad | None:
    ...
    for slug, row in tier_table.items():
        ...
        if row_tier == tier and bool(entitled):
    ...
    speed = "standard" if "default" in service_tiers else None
...
def resolve_routing_quad(
    *,
    ...
    source_by_field = {
        field: (
            profile_value.source_by_field[field]
            if profile_value.source_by_field[field] != "unset"
            else "profile"
        )
        for field in _FIELDS
    ...
    tier_value = None
    if selected_tier:
        tier_value = _tier_quad(selected_tier, tiers)
