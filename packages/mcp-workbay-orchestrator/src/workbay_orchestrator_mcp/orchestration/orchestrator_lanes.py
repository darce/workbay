...
# ``scripts/workbay/lifecycle`` under the orchestrator repo root.
...
def _lifecycle_dir(orchestrator_root: Path) -> Optional[Path]:
    ...
    candidate = Path(override) if override else orchestrator_root / "scripts" / "workbay" / "lifecycle"
...
def _provision_root_venv(orchestrator_root: Path, worktree: Path) -> dict[str, Any]:
    ...
    if lifecycle_dir is None:
        sys.stderr.write(
            ...
            f"(set {WORKBAY_LIFECYCLE_DIR_ENV} or add scripts/workbay/lifecycle "
