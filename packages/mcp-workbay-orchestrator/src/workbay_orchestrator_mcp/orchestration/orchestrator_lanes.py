...
from git_lock import classify_git_failure
...
# Named outcomes of the optional no-ff topology step. The helper returns bool
# (and raises on ownership refusal); the caller names all three cases before
# applying the fail-closed intake policy from implementation note L6.
NO_FF_TOPOLOGY_LANDED = "landed"
NO_FF_TOPOLOGY_DECLINED = "declined"
NO_FF_TOPOLOGY_REFUSED = "refused"
NO_FF_TOPOLOGY_SUPERSEDED = "superseded_by_integration"
...
class NoFfLandingOwnershipError(Exception):
    ...
    def __init__(self, lane_id: str, offending_paths: list[str] | tuple[str, ...]):
        ...
        super().__init__(
            f"no-ff landing refused for lane {lane_id}: prospective merge tree imports paths intake excluded: {paths}"
        )
...
def _lane_branch_on_first_parent_lineage(
    ...
    count_out = _git_stdout(orchestrator_root, "rev-list", "--count", f"{commit}..{tip}")
...
def _lookup_recorded_landing_sha(*, task_ref: str, lane_id: str) -> str | None:
    ...
    newest ``latest_lane_landing`` record. Missing optional rows or storage
    access are absence — this helper never writes, and the caller decides what
    a missing SHA means. A missing handoff runtime is a configuration fault
    and propagates instead of becoming absence.
    """
    ...
    try:
        ...
        raise
    except (ImportError, OSError, sqlite3.Error):  # noqa: BLE001 — POLICY: optional row/storage absence may continue to ledger lookup; config faults propagate above
        pass
    ...
    try:
        ...
        raise
    except (ImportError, OSError, sqlite3.Error):  # noqa: BLE001 — POLICY: an absent/unreadable optional ledger is no evidence, never a configured-runtime fault
        return None
    return None
...
REASON_ABSENT_BRANCH_EVIDENCE_NEVER_CAPTURED = "empty_lane_absent_branch_evidence_never_captured"
...
def _absent_lane_branch_never_produced_answer(
    ...
    *,
    ...
    receipt_sha = _lookup_integration_receipt_landed_revision(orchestrator_root, task_ref=task_ref, lane_id=lane_id)
    evidence: list[str] = []
...
def _no_ff_prospective_tree(repo: Path, ours: str, theirs: str) -> str | None:
    ...
    return sha if _FULL_COMMIT_SHA_RE.fullmatch(sha) else None


def _pure_ancestry_binder_receipt(
    repo: Path,
    integration_ref: str,
    lane_branch: str,
) -> dict[str, Any] | None:
    """Return a typed outcome when a no-ff merge can change ancestry only.

    ``merge-tree --write-tree`` is deliberately an observation-only probe. A
    missing ref, unsupported Git, non-zero merge-tree exit, malformed output,
    or any other probe failure returns ``None`` so the caller retains the
    ordinary content-gated path. Tree identity, rather than commit counts,
    is the only positive binder evidence.
    """
    if not integration_ref or not lane_branch:
        return None
    integration_tip = _rev_parse_sha(repo, integration_ref)
    lane_tip = _rev_parse_sha(repo, lane_branch)
    integration_tree = _git_stdout(repo, "rev-parse", f"{integration_ref}^{{tree}}")
    if (
        not _is_full_commit_sha(integration_tip)
        or not _is_full_commit_sha(lane_tip)
        or not integration_tree
        or not _FULL_COMMIT_SHA_RE.fullmatch(integration_tree)
    ):
        return None
    prospective_tree = _no_ff_prospective_tree(repo, integration_ref, lane_branch)
    if prospective_tree != integration_tree:
        return None
    return {
        "outcome": NO_FF_TOPOLOGY_SUPERSEDED,
        "reason": "prospective_tree_equals_integration_tree",
...
def _emit_landing_outcome(
    ...
    *,
    ...
    """Emit one structured landing receipt without making logging a gate."""
    payload: dict[str, Any] = {
        "task_ref": task_ref,
        "lane_id": lane_id,
        **receipt,
        "gate_skipped": receipt.get("gate_skipped") is True,
    }
    _safe_log(
        log,
        "INFO",
        "lane_landing_outcome",
        receipt=payload,
        landing_outcome=payload,
        **payload,
    )
    logger.info("lane_landing_outcome %s", json.dumps(payload, sort_keys=True))
...
def _run_no_ff_git(repo: Path, *args: str) -> subprocess.CompletedProcess[str]:
    """Run one actuator git command under the landing/recovery time bound."""
...
@contextmanager
def _landing_mutation_lock(repo: Path):
    """Coordinate landing actuators across worktrees without unbounded waiting."""
    try:
        git_dir = repo / ".git"
        if git_dir.is_file():
            ...
            git_dir = repo / pointer.removeprefix("gitdir: ").strip()
        common_dir = git_dir
        ...
        if not common_dir.is_dir():
            common = _git_stdout(repo, "rev-parse", "--git-common-dir")
            candidate = repo / common if isinstance(common, str) and common else None
            common_dir = candidate if candidate is not None and candidate.is_dir() else repo / ".git"
        if not common_dir.is_dir():
            raise ValueError("common git directory unavailable")
        handle = (common_dir / "workbay-landing.lock").open("a")
    except (OSError, ValueError):
        logger.warning("landing_lock_unavailable repo=%s", repo)
        yield None
        return
    with handle:
        try:
...
def _restore_no_ff_landing(
    ...
    *,
    ...
    """Bound and verify recovery after an incomplete or rejected merge."""
    try:
        if not _rev_parse_sha(repo, "HEAD") or not _git_stdout(repo, "rev-parse", "HEAD^{tree}"):
            raise RuntimeError(f"unresolved recovery fingerprint for lane {lane_id}")
        merge_head = _run_no_ff_git(repo, "rev-parse", "-q", "--verify", "MERGE_HEAD")
        if merge_head.returncode == 0:
            if _rev_parse_sha(repo, "HEAD") != tip_before:
                raise RuntimeError(f"unproven merge recovery state for lane {lane_id}")
            abort = _run_no_ff_git(repo, "merge", "--abort")
            if abort.returncode != 0:
                raise RuntimeError(
                    f"git merge --abort failed for lane {lane_id}: {abort.stderr.strip() or 'no stderr'}"
                )

        restored_tip = _rev_parse_sha(repo, "HEAD")
        restored_tree = _git_stdout(repo, "rev-parse", "HEAD^{tree}")
        if not restored_tip or not restored_tree:
            raise RuntimeError(f"unresolved recovery fingerprint for lane {lane_id}")
        if restored_tip != tip_before or restored_tree != tree_before:
            status = _run_no_ff_git(repo, "status", "--porcelain", "--untracked-files=no")
            if (
                not merge_receipt
                or restored_tip != merge_receipt
                or status.returncode != 0
                or status.stdout.strip()
                or _rev_parse_sha(repo, "HEAD^1") != tip_before
            ):
                raise RuntimeError(f"unproven merge recovery state for lane {lane_id}")
            # The status probe is evidence, not exclusion against later writers.
            # Checkout refuses tracked edits and untracked obstructions at the
            # tree update. Explicitly protect ignored files too; reset --keep
            # and read-tree both allow those to be overwritten.
            branch = _run_no_ff_git(repo, "symbolic-ref", "-q", "HEAD")
            if branch.returncode not in (0, 1):
                raise RuntimeError(f"unresolved recovery branch for lane {lane_id}")
            reset = _run_no_ff_git(repo, "checkout", "--no-overwrite-ignore", "--detach", tip_before)
            if reset.returncode != 0:
                raise RuntimeError(
                    f"git recovery tree update failed for lane {lane_id}: {reset.stderr.strip() or 'no stderr'}"
                )
            if branch.returncode == 0:
                branch_ref = branch.stdout.strip()
                publish = _run_no_ff_git(repo, "update-ref", branch_ref, tip_before, merge_receipt)
                if publish.returncode != 0:
                    raise RuntimeError(
                        f"git recovery publication failed for lane {lane_id}: {publish.stderr.strip() or 'no stderr'}"
                    )
                attach = _run_no_ff_git(repo, "symbolic-ref", "HEAD", branch_ref)
                if attach.returncode != 0:
                    raise RuntimeError(f"git recovery branch attachment failed for lane {lane_id}")
            restored_tip = _rev_parse_sha(repo, "HEAD")
            restored_tree = _git_stdout(repo, "rev-parse", "HEAD^{tree}")
    except (OSError, subprocess.TimeoutExpired) as exc:
        raise RuntimeError(f"bounded git recovery failed for lane {lane_id}: {exc}") from exc

    if restored_tip != tip_before or restored_tree != tree_before:
        raise RuntimeError(f"git merge recovery did not restore integration tip/tree for lane {lane_id}")
    return restored_tip, restored_tree


def _no_ff_containment_failure_reason(contained: bool | str | None) -> str:
    """Map the containment probe's typed non-success arms to telemetry."""
...
def _merge_lane_no_ff(
    ...
    lane_id: str,
    *,
    path_restricted_intake: bool = False,
    tested_sha: str | None = None,
    _lock_held: bool = False,
    log: Any | None = None,
) -> bool:
    """Land a whole branch, restoring the original tip/tree on failure.

    Whole-branch success requires verified parents and commit containment,
    allowing main-side supersession (implementation note L6.3). Tree identity is not a
    landing fact: a legitimate merge can differ from both parent trees.

    After path-restricted intake, callers must set ``path_restricted_intake``:
    this additionally refuses any tree delta that imports excluded content.
    ``False`` means topology declined and the caller must fail closed, even
    when the product remains landed through that prior intake. Merge and
    recovery commands are bounded; recovery verifies the original tip/tree.
    """
    if not _lock_held:
        with _landing_mutation_lock(orchestrator_root) as acquired:
            if not acquired:
                _safe_log(
                    log,
                    "WARNING",
                    ...
                    lane=lane_id,
                    reason="landing_lock_unavailable" if acquired is None else "mutation_lock_held",
                )
                return False
            return _merge_lane_no_ff(
                orchestrator_root,
                task_ref,
                lane_id,
                path_restricted_intake=path_restricted_intake,
                tested_sha=tested_sha,
                log=log,
                _lock_held=True,
            )
    lane_branch = tested_sha or _resolve_lane_branch(orchestrator_root, task_ref, lane_id)
    if not lane_branch:
        _safe_log(
            log,
            "WARNING",
    ...
    lane_tip = _rev_parse_sha(orchestrator_root, lane_branch)
    if not _is_full_commit_sha(lane_tip):
        return False
    lane_branch = lane_tip
    merge_marker = f"Workbay-Landing-Run: {uuid4().hex}"
    merge_receipt = None
    tip_before = _rev_parse_sha(orchestrator_root, "HEAD")
    tree_before = _git_stdout(orchestrator_root, "rev-parse", "HEAD^{tree}")
    if not _is_full_commit_sha(tip_before) or not tree_before:
        _safe_log(
            log,
            "WARNING",
            "lane_no_ff_merge_fallback",
    ...
    # Fail closed before mutation: recovery must never discard prior edits.
    try:
        status = _run_no_ff_git(orchestrator_root, "status", "--porcelain", "--untracked-files=no")
    except (OSError, subprocess.TimeoutExpired):
        status = None
    if status is None or status.returncode != 0 or status.stdout.strip():
        _safe_log(log, "WARNING", "lane_no_ff_merge_fallback", lane=lane_id, reason="integration_dirty_or_unresolved")
        return False

    prospective = _no_ff_prospective_tree(orchestrator_root, "HEAD", lane_branch) if path_restricted_intake else None
    ...
    try:
        merge = _run_no_ff_git(
            orchestrator_root, "merge", "--no-ff", "--no-edit", "-m", f"Land {lane_id}\n\n{merge_marker}", lane_branch
        )
    except (OSError, subprocess.TimeoutExpired) as exc:
        merge = None
        merge_error = str(exc)
    ...
    if merge is not None and merge.returncode == 0:
        new_head = _rev_parse_sha(orchestrator_root, "HEAD")
        proof = _git_stdout(orchestrator_root, "show", "-s", "--format=%P%n%B", new_head or "HEAD") or ""
        if (
            new_head
            and new_head != tip_before
            and proof.splitlines()[:1] == [f"{tip_before} {lane_tip}"]
            and merge_marker in proof.splitlines()
        ):
            merge_receipt = new_head
        else:
            _safe_log(log, "WARNING", "lane_no_ff_merge_fallback", lane=lane_id, reason="merge_refused")
            return False
        tree_after = _git_stdout(orchestrator_root, "rev-parse", "HEAD^{tree}") or ""
        drifted = _no_ff_tree_delta(orchestrator_root, tree_before, tree_after) if path_restricted_intake else []
        if drifted:
            _restore_no_ff_landing(
                orchestrator_root,
                lane_id=lane_id,
        ...
        lane_tip = _rev_parse_sha(orchestrator_root, lane_branch)
        landing_sha = _rev_parse_sha(orchestrator_root, "HEAD")
        first_parent = _rev_parse_sha(orchestrator_root, "HEAD^1")
        second_parent = _rev_parse_sha(orchestrator_root, "HEAD^2")
        contained = (
            _lane_branch_contained_in(
                ...
                target_tip_before_landing=tip_before,
            )
            if _is_full_commit_sha(landing_sha)
            else None
        )
        refusal_reason = ""
        if not _is_full_commit_sha(landing_sha):
            refusal_reason = "landing_sha_unresolved"
        elif landing_sha != merge_receipt or first_parent != tip_before or second_parent != lane_tip:
            refusal_reason = "landing_topology_unverified"
        elif contained is not True:
            refusal_reason = _no_ff_containment_failure_reason(contained)

        if refusal_reason:
            restored_tip, restored_tree = _restore_no_ff_landing(
                orchestrator_root,
                lane_id=lane_id,
                tip_before=tip_before,
                tree_before=tree_before,
        ...
        _safe_log(
            log,
            "INFO",
            "lane_no_ff_merge_landed",
            lane=lane_id,
            branch=lane_branch,
            verified_sha=landing_sha,
            merge_receipt=merge_receipt,
        )
        return True

    if classify_git_failure(merge_error) == "git_index_lock_held":
        tip_after = _git_stdout(orchestrator_root, "rev-parse", "HEAD")
        tree_after = _git_stdout(orchestrator_root, "rev-parse", "HEAD^{tree}")
        reason = "merge_blocked_git_index_lock_held"
        fields = {
            "lane": lane_id,
            "branch": lane_branch,
            "reason": reason,
    ...
    # A refused merge owns no new commit. Never reset an unrelated writer's HEAD.
    if _rev_parse_sha(orchestrator_root, "HEAD") != tip_before:
        _safe_log(log, "WARNING", "lane_no_ff_merge_fallback", lane=lane_id, reason="merge_refused")
        return False
    ...
    tip_after, tree_after = _restore_no_ff_landing(
        orchestrator_root,
        lane_id=lane_id,
        tip_before=tip_before,
        tree_before=tree_before,
    )
    reason = "merge_conflict" if conflicting_paths else "merge_refused"
    if not conflicting_paths and classify_git_failure(merge_error) == "git_index_lock_held":
        reason = "merge_blocked_git_index_lock_held"
    fields = {
        "lane": lane_id,
        "branch": lane_branch,
...
def _intake_lane(
    ...
    *,
        ...
        with _landing_mutation_lock(orchestrator_root) as acquired:
            if not acquired:
                _safe_log(
                    log,
                    "WARNING",
                    "lane_content_gate_refused",
                    reason="landing_lock_unavailable" if acquired is None else "mutation_lock_held",
                )
                return False
            return _intake_lane(orchestrator_root, task_ref, lane_id, log=log, _lock_held=True)
    landing_envelope: dict[str, Any] | None = None
    binder_receipt: dict[str, Any] | None = None
    no_ff_enabled = False
    config: dict[str, Any] = {}
    if not dry_run:
        from workbay_orchestrator_mcp.orchestration.lane_ready_set import inspect_landing_ready_set
        ...
        try:
            loaded_config = get_lane_config(task_ref, lane_id, orchestrator_root=str(orchestrator_root)) or {}
            if isinstance(loaded_config, dict):
                config = loaded_config
        except Exception:  # noqa: BLE001 -- absent/corrupt evidence refuses before mutation
            config = {}

        # Probe the prospective no-ff tree before the content gate. A tree
        # match proves the lane contributes no new integration bytes, so the
        # expensive gate is unnecessary. Any unavailable/ambiguous probe keeps
        # the established gated path (fail closed per the canon).
        no_ff_enabled = no_ff_landing_enabled(orchestrator_root, task_ref)
        if no_ff_enabled:
            lane_branch = _resolve_lane_branch(
                orchestrator_root,
                task_ref,
                lane_id,
                branch_hint=config.get("branch") if isinstance(config.get("branch"), str) else None,
            )
            if lane_branch:
                binder_receipt = _pure_ancestry_binder_receipt(
                    orchestrator_root,
                    "HEAD",
                    lane_branch,
                )
                if binder_receipt is not None:
                    _emit_landing_outcome(
                        log,
                        task_ref=task_ref,
                        lane_id=lane_id,
                        receipt=binder_receipt,
                    )

        if binder_receipt is None:
            landing_envelope = inspect_landing_ready_set(
                orchestrator_root,
                [{"lane_id": lane_id, "branch": config.get("branch"), "content_gate": config.get("content_gate")}],
            )
            if landing_envelope["refused"]:
                _safe_log(log, "WARNING", "lane_content_gate_refused", landing_envelope=landing_envelope)
                logger.warning("lane_content_gate_refused %s", json.dumps(landing_envelope, sort_keys=True))
                return False
    cmd = [
        "make",
        "lane-intake",
    ...
    if dry_run:
        ...
        if binder_receipt is not None:
            validated_base = binder_receipt.get("integration_tip")
            tested_sha = binder_receipt.get("lane_tip")
        else:
            assert landing_envelope is not None
            validated_base = landing_envelope.get("base_sha")
            tested_sha = landing_envelope["ready"][0]["candidate_sha"]
        if not _is_full_commit_sha(validated_base) or _rev_parse_sha(orchestrator_root, "HEAD") != validated_base:
            _safe_log(log, "WARNING", "lane_content_gate_refused", reason="base_moved", validated_base=validated_base)
            return False
        if not _is_full_commit_sha(tested_sha):
            _safe_log(log, "WARNING", "lane_content_gate_refused", reason="lane_tip_unresolved")
            return False
        cmd.append(f"LANE_TESTED_SHA={tested_sha}")
        cmd.append(f"LANE_BASE_SHA={validated_base}")
    result = subprocess.run(
        cmd,
        cwd=orchestrator_root,
        capture_output=True,
    ...
    if no_ff_enabled:
        ...
        try:
            produced = _merge_lane_no_ff(
                orchestrator_root,
                task_ref,
                lane_id,
                path_restricted_intake=True,
                tested_sha=tested_sha,
                _lock_held=True,
                log=log,
            )
            ...
            if produced:
                topology_outcome = NO_FF_TOPOLOGY_SUPERSEDED if binder_receipt is not None else NO_FF_TOPOLOGY_LANDED
            elif binder_receipt is not None:
                # A strictly-behind lane has no merge commit for Git to make,
                # but the proven tree identity still makes it a successful
                # superseded binder. Re-check the unchanged integration tree
                # and ancestry before accepting that no-op outcome.
                current_tip = _rev_parse_sha(orchestrator_root, "HEAD")
                current_tree = _git_stdout(orchestrator_root, "rev-parse", "HEAD^{tree}")
                already_integrated = (
                    _is_full_commit_sha(current_tip)
                    and current_tree == binder_receipt.get("integration_tree")
                    and _git_is_ancestor(orchestrator_root, tested_sha, current_tip) is True
                )
                topology_outcome = NO_FF_TOPOLOGY_SUPERSEDED if already_integrated else NO_FF_TOPOLOGY_DECLINED
            else:
                topology_outcome = NO_FF_TOPOLOGY_DECLINED
        if topology_outcome == NO_FF_TOPOLOGY_REFUSED:
            return False
        if topology_outcome == NO_FF_TOPOLOGY_DECLINED:
            return False
    return True
...
def record_reclaim_candidate(
    *,
    ...
    decision_id_builder = branch_reclaim_candidate_decision_id if _branch_verdict else reclaim_candidate_decision_id
...
# Bundling is the only reclaim work that scales with *repository history* rather
# than with the branch, so it needs its own bounds rather than the reaper's
# 20s probe budget: `git bundle create` for a branch sharing main's history
# measures ~14s on this repo today and grows with every landed commit. Bounded
# explicitly so a stalled pack or a slow state directory degrades typed instead
# of hanging the reaper's never-raise sweep [RES-02][RES-07][RES-13].
_BUNDLE_PROBE_TIMEOUT_S = 20.0
_BUNDLE_VERIFY_TIMEOUT_S = 120.0
_BUNDLE_CREATE_TIMEOUT_S = 300.0


def _fsync_path(path: Path, *, directory: bool = False) -> None:
    """Best-effort durability barrier; never raises into the caller's contract."""
    try:
    ...
    **bundle → repoint ledger → delete ref**, never any other order.  It
    ...
    try:
        resolved = subprocess.run(
            ...
            text=True,
            check=False,
            timeout=_BUNDLE_PROBE_TIMEOUT_S,
        )
    except (OSError, subprocess.TimeoutExpired) as exc:
    ...
    def verify(path: Path) -> bool:
        try:
            ...
            checked = subprocess.run(
                ...
                text=True,
                check=False,
                timeout=_BUNDLE_VERIFY_TIMEOUT_S,
            )
        except (OSError, subprocess.TimeoutExpired):
            # An unverifiable bundle is an unusable bundle: refuse, never assume.
            return False
    ...
    try:
        ...
        created = subprocess.run(
            ...
            text=True,
            check=False,
            timeout=_BUNDLE_CREATE_TIMEOUT_S,
        )
        ...
        # ``os.replace`` orders the rename, not the bytes. Without both fsyncs a
        # crash can leave a published name pointing at unflushed content -- a
        # valid-looking rollback handle for garbage, with the ref already gone
        # [DATA-16][RES-18].
        _fsync_path(temporary_path)
        os.replace(temporary_path, bundle_path)
        _fsync_path(bundle_dir, directory=True)
        temporary_path = None
    except (OSError, subprocess.TimeoutExpired) as exc:
        return finish(
            False,
...
def lane_dependency_satisfied(
    ...
    *,
    ...
    try:
        ...
        # A missing handoff runtime is an operator configuration fault, not a
        # missing landing record; preserve its named reason at the dependency
        # boundary so it cannot be laundered into routine absence.
        if callable(log):
            try:
                log(
                    "ERROR",
                    ...
                    upstream_id=upstream_id,
                    task_ref=task_ref,
                    error=REASON_RUNTIME_NOT_CONFIGURED,
                    error_type=type(exc).__name__,
                )
            except Exception:  # noqa: BLE001 — never raise out of intake
                pass
        return _finish(False, REASON_RUNTIME_NOT_CONFIGURED)
    except (ImportError, OSError, RuntimeError, TypeError, ValueError, sqlite3.Error) as exc:  # noqa: BLE001 — POLICY: known reader/storage faults fail closed; only ok=True with landing=None is absence, and the runtime sentinel is named above
        # Defensive log: a broken log callable must not convert a handled
