    ...
    *,
    ...
    if isinstance(obj, tuple):
        ...
        return {
            "__t": "tuple",
            "v": [
                _canonical_for_digest(x, _depth=_depth + 1, _stack=_stack)
                for x in obj
            ],
        }
            ...
            items.append(
                [
                    ...
                    _canonical_for_digest(
                        obj[k], _depth=_depth + 1, _stack=_stack
                    ),
                ]
            )
...
def record_reclaim_candidate(
    *,
    ...
    decision_id = reclaim_candidate_decision_id(task_ref=task_ref, lane_id=lane_id)
    ...
    session = f"lane-reclaim-{lane_id}"
...
def __getattr__(name: str) -> Any:
    ...
    """
    if name == "reclaim_candidate_decision_id":
        from workbay_handoff_mcp.lanes_recording import (  # noqa: PLC0415
