...
* **semantic** — in-process embedding model-load path DELETED (D2/D3; same code
  class as :mod:`lane_prompt`). Degrades to
  ``offload_semantic_service_unconfigured`` until a future single-owner service
  (D5) can be a thin client.
...
def clean_semantic_relevant_lines(raw_lines: Any) -> list[str]:
    ...
    ``- `` bullet so callers can re-wrap the concept bodies.
    """
...
def _build_semantic_channel(
    ...
    *,
    ...
    # D2/D3 (internal S1): in-process embedding model-load path
    # DELETED (same code class as lane_prompt). build_slice_brief_packet has no production
    # caller today; deleted for consistency. Future single-owner service (D5) = thin client.
    # [REF-05][REF-12]
    return _semantic_channel_from_payload(None, error="offload_semantic_service_unconfigured")
...
def build_slice_brief_packet(
    ...
    *,
    ...
    anchor_texts:
        Retained for interface stability; UNUSED after S1 (internal)
        deleted the in-process semantic-reinjection model-load path. The semantic channel now
        always degrades to ``(unavailable: offload_semantic_service_unconfigured)``; a future
        D5 single-owner embedding service would reintroduce a thin client that consumes these.
    semantic_content_budget_chars:
        Retained for interface stability; UNUSED after S1 (see ``anchor_texts``).
    """
