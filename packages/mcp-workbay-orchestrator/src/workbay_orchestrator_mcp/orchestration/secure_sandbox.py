...
_SCAN_SKIP_DIRS = {
    ".git", ".venv", "node_modules", "__pycache__", ".task-state", ".workbay",
    "dist", ".mypy_cache", ".pytest_cache", ".cache",
}
...
class SecureSandboxError(RuntimeError):
    ...
    provisioning (uv sync) failed. Secret findings are advisory, not errors."""
...
def sandbox_provision_enabled() -> bool:
    """Whether to ``uv sync`` the sandbox so grok's self-verify runs against
    sandbox src. ON by default; opt out with ``WORKBAY_GROK_SANDBOX_PROVISION``
    falsey (e.g. tests whose fake grok never self-verifies)."""
...
class ShallowSandbox:
    ...
    def verify_isolated(self) -> None:
        ...
        """
        shallow = _git(
            ["rev-parse", "--is-shallow-repository"], cwd=self.path, check=False
        ).stdout.strip()
        if shallow != "true":
            raise SecureSandboxError(
        ...
        if self.base_sha:
            ancestors = _git(
                ["rev-list", "--count", self.base_sha], cwd=self.path, check=False
            ).stdout.strip()
            if ancestors != "1":
                raise SecureSandboxError(
        ...
        if remotes:
            raise SecureSandboxError(
                f"secure sandbox has a remote ({remotes!r}); refusing — a fetch "
                "could re-pull full history."
            )

    def provision_env(self, *, timeout: int = 900) -> None:
        """Provision the sandbox test env so grok's self-verify runs against the
        SANDBOX src, not a stale/absent one.

        A fresh ``--depth=1`` clone has no ``.venv``; grok's lane ``TEST_CMD``
        (e.g. ``../../.venv/bin/python -m pytest``) would die on
        ``ModuleNotFoundError``. Run the canonical workspace-root ``uv sync``
        (implementation note D3b) so ``<sandbox>/.venv`` holds editables pointing at the
        clone. FAIL-CLOSED: a provision failure raises rather than letting grok
        self-verify against a broken env (a false-red that would waste the pass).
        """
        ...
        if proc.returncode != 0:
            raise SecureSandboxError(
                "secure sandbox env provisioning (uv sync) failed: "
    ...
    def port_commits_back(self) -> list[str]:
        ...
        patch_text = _git(
            ["format-patch", "--stdout", f"{self.base_sha}..HEAD"], cwd=self.path
        ).stdout
        ...
        if proc.returncode != 0:
            ...
            git_dir = _git(
                ["rev-parse", "--absolute-git-dir"], cwd=self.worktree_path, check=False
            ).stdout.strip()
