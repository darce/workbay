...
def build_review_context_payload(
    ...
    *,
    ...
    tip_sha = _resolve_review_commit(repository, tip, "tip")
    ...
    names_raw = _git_bytes(["diff", "--name-only", "--no-renames", "-z", revision_range], cwd=repository)
    changed_paths = tuple(
        item.decode("utf-8", errors="surrogateescape") for item in names_raw.split(b"\0") if item
    )
    diff_raw = _git_bytes(
        ["diff", "--binary", "--no-color", "--no-ext-diff", "--no-renames", revision_range],
        cwd=repository,
    )
    ...
    verified_names_raw = _git_bytes(
        ["diff", "--name-only", "--no-renames", "-z", revision_range], cwd=repository
    )
    ...
    commits = _git_bytes(
        ["log", "--abbrev=12", "--format=%h %s", revision_range], cwd=repository
    ...
    try:
        ...
        if staged_changed == 1:
            _git_bytes(
                [
                    ...
                    "commit",
                    "-m",
                    "chore(review): procure governed context payload",
                    "--",
        ...
        _git(["restore", "--staged", "--worktree", "--source=HEAD", "--", REVIEW_CONTEXT_DIRNAME], cwd=repository, check=False)
...
class ShallowSandbox:
    ...
    def provision_env(self, *, timeout: int = 900, test_cmd: str | None = None) -> str:
        ...
        # TODO(internal): the grok adapter (outside this lane's write-set)
        # must pass its lane test command and reject both typed failure prefixes.
