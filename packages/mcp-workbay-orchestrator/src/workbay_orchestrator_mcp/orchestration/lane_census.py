...
def _remote_disk_probe_script() -> str | None:
    sandbox_root = _resolve_sandbox_root(None)
    if sandbox_root is None:
        return None
    quoted_root = _sh_single_quote(sandbox_root)
    # Same $HOME expansion as remote_sandbox_reap: relative roots live under the
    # remote account home. Keep df's stdout a single integer for callers.
    return (
        f"ROOT={quoted_root}; "
        'case "$ROOT" in /*) ;; *) ROOT="$HOME/$ROOT" ;; esac; '
        'df --output=avail -B1 "$ROOT" | tail -n 1'
    )


def _remote_free_bytes(*, root: Path | str, timeout: float) -> tuple[int | None, str | None]:
    script = _remote_disk_probe_script()
    if script is None:
        return None, "sandbox_root is empty or unsafe"
    try:
