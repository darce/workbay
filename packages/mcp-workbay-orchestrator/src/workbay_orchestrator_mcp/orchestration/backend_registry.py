...
BACKENDS: dict[str, BackendSpec] = {
    ...
    "grok-cli": BackendSpec(
        ...
        description="Shell out to the grok CLI (xAI Composer-2.5 junior worker).",
        capabilities=BackendCapabilities(
            supports_structured_output=True,
            supports_sandbox=True,
            supports_sync_turn=False,
            ...
            supports_reasoning_effort=True,
            # grok-cli's envelope in offload session_mode does not surface usable
            # token usage (total_tokens=0), so cumulative_tokens never advances.
            # Declare no telemetry: the offload token-budget governor must fall
            # back to turn/time bounds rather than hard-erroring a working turn
            # (internal / TB-001).
