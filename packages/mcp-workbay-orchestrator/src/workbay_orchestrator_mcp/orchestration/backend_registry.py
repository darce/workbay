...
def _probe_codex_remote(*, workspace_root: Path | str | None = None) -> dict[str, Any]:
    ...
    if result.returncode == 10 or "CODEX_MISSING" in combined:
        payload = {
            ...
            "is_available": False,
            "state": AVAIL_UNAVAILABLE,
            "detail": (f"Remote gate '{host}' reachable but pinned codex binary missing at {CODEX_REMOTE_BIN}."),
            "auth_state": REMOTE_AUTH_NA,
        }
        _probe_codex_remote_cache[host] = (now + _PROBE_CODEX_REMOTE_TTL_S, payload)
        return payload
    if result.returncode != 0:
        payload = {
            ...
            "is_available": False,
            "state": AVAIL_UNAVAILABLE,
            "detail": (f"Remote gate '{host}' reachable but codex-remote probe failed (exit {result.returncode})."),
            "auth_state": REMOTE_AUTH_NA,
        }
        _probe_codex_remote_cache[host] = (now + _PROBE_CODEX_REMOTE_TTL_S, payload)
        return payload
    ...
    if authenticated:
        detail = (
            ...
            f"reports version {remote_version} (matches pin); authenticated."
        )
    else:
        detail = (
            f"Remote gate '{host}' reachable; VM codex at {CODEX_REMOTE_BIN} "
            f"reports version {remote_version} (matches pin) but auth is "
