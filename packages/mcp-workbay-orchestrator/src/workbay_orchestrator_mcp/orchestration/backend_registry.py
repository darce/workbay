...
from workbay_orchestrator_mcp.orchestration.oxalpha_lane_config import (
    ...
    OXALPHA_ENV_FILE,
    OXALPHA_KEY_INFO_URL,
    OXALPHA_MIN_REMAINING_USD,
    TRACKED_0XALPHA_MODEL,
    WORKBAY_0XALPHA_API_KEY_ENV,
    WORKBAY_0XALPHA_MODEL_ENV,
)
...
class AuthPort:
    ...
    def __post_init__(self) -> None:
        ...
        if self.kind == "env_file":
            ...
            if self.key_info_url is not None or self.min_remaining_usd is not None:
                raise ValueError("device_login AuthPort must not set key_info_url/min_remaining_usd")
...
BACKENDS: dict[str, BackendSpec] = {
    ...
    # implementation note S4: 0xAlpha = codex CLI on the OCI VM against OpenRouter's
    # OpenAI-compatible transport. stealth/ox-alpha is NOT an OpenAI model; the
    # codex binary/sandbox pins are reused, the provider + credential port are
    # its own (oxalpha_lane_config). No list_models_argv: OpenRouter's
    ...
    "0xalpha-remote": BackendSpec(
        kind="cli",
        adapter_path="workbay_orchestrator_mcp.orchestration.adapters.remote_exec.RemoteExecAdapter",
        cost_class=COST_REMOTE,
        model_family="0xalpha",
        ...
        review_eligible=True,
        description="0xAlpha: codex CLI on the OCI VM against OpenRouter (OpenAI-compatible)",
        shaping_adapter_path=("workbay_orchestrator_mcp.orchestration.adapters.codex_cli.CodexCliAdapter"),
        availability_probe_path=("workbay_orchestrator_mcp.orchestration.backend_registry._probe_0xalpha_remote"),
        allowed_model=DEFAULT_0XALPHA_MODEL,
        allowed_models=OXALPHA_ALLOWED_MODELS,
        allowed_model_env=WORKBAY_0XALPHA_MODEL_ENV,
        tracked_model=TRACKED_0XALPHA_MODEL,
        post_turn_fetch_headroom_s=15,
        ...
        # (bearer on stdin to OXALPHA_KEY_INFO_URL, run on the VM); the first
        # turn's auth_match is the real check.
        auth=AuthPort(
            kind="env_file",
            env_file=OXALPHA_ENV_FILE,
            env_var=WORKBAY_0XALPHA_API_KEY_ENV,
            binary=CODEX_REMOTE_BIN,
            status_argv=(),
            key_info_url=OXALPHA_KEY_INFO_URL,
            min_remaining_usd=OXALPHA_MIN_REMAINING_USD,
        ),
...
_0XALPHA_REMOTE_REMEDY = (
    "0xalpha-remote requires WORKBAY_REMOTE_GATE_HOST and a provisioned OCI VM with "
    f"the pinned codex binary plus a readable {BACKENDS['0xalpha-remote'].auth.env_file} carrying a "
    f"non-empty {BACKENDS['0xalpha-remote'].auth.env_var} whose OpenRouter key-info reports remaining "
...
#        (distinct from 13; emitted by key_info_url ports — 0xalpha-remote since
#        implementation note S4 — when data.limit is null or limit_remaining < threshold)
...
# One stdout data line ``limit=<n> usage=<n> remaining=<n>`` on success; parsed
# back by probe_remote_auth into ``key_info``.
_KEY_INFO_LINE_RE = re.compile(r"^limit=(?P<limit>\S+) usage=(?P<usage>\S+) remaining=(?P<remaining>\S+)$")


def _render_key_info_stage(port: AuthPort, *, prefix: str, env_var: str) -> str:
...
def render_env_file_auth_probe(port: AuthPort) -> str:
...
def render_device_login_auth_probe(port: AuthPort) -> str:
...
def render_auth_probe(port: AuthPort) -> str:
...
# The cursor literal is derived from its declared port (implementation note S2); the
# registry test pins byte-equality with the pre-0208 hand-written script.
_REMOTE_CURSOR_AUTH_PROBE = render_env_file_auth_probe(auth_port("cursor-remote"))


# ---------------------------------------------------------------------------
# implementation note S5 — `remote_agent.sh doctor` auth lines, derived from the registry
# ---------------------------------------------------------------------------
...
def doctor_auth_ports() -> list[tuple[str, AuthPort]]:
    ...
    """
    return [(name, spec.auth) for name, spec in BACKENDS.items() if spec.auth is not None]


def _doctor_auth_artifact(port: AuthPort) -> str:
...
def render_doctor_auth_line(name: str, port: AuthPort) -> str:
...
def _render_doctor_key_info_block(name: str, port: AuthPort) -> str:
...
def _parse_key_info_line(lines: list[str]) -> dict[str, float] | None:
    ...
    for line in lines:
        match = _KEY_INFO_LINE_RE.fullmatch(line)
        if match is None:
            continue
        try:
...
_PROBE_0XALPHA_REMOTE_TTL_S = 30.0
_probe_0xalpha_remote_cache: dict[str, tuple[float, dict[str, Any]]] = {}


def _probe_0xalpha_remote(*, workspace_root: Path | str | None = None) -> dict[str, Any]:
    """Availability probe for 0xalpha-remote (implementation note S4). Never raises.
    ...
    """
    base = BACKENDS["0xalpha-remote"].capabilities
    host = _resolve_remote_gate_host_for_probe(workspace_root=workspace_root)
    if not host:
        return {
            "capabilities": _availability_caps(base, is_available=False),
    ...
    now = time.monotonic()
    cached = _probe_0xalpha_remote_cache.get(host)
    if cached is not None:
        expires_at, payload = cached
        if now < expires_at:
            return payload
        del _probe_0xalpha_remote_cache[host]

    auth = probe_remote_auth("0xalpha-remote", host=host, timeout=30)
    auth_state = auth["auth_state"]
    rc = auth["exit_code"]
    key_info = auth.get("key_info")
    # Belt and braces with probe_remote_auth: never available without key_info.
    authenticated = auth_state == REMOTE_AUTH_AUTHENTICATED and key_info is not None
    if rc == _REMOTE_AUTH_TRANSPORT_EXIT:
        detail = f"Remote gate '{host}' 0xalpha-remote probe did not run ({auth['marker']}); fail-closed."
    elif authenticated:
        detail = (
            f"Remote gate '{host}' reachable; VM codex at {CODEX_REMOTE_BIN} runs and "
            f"{WORKBAY_0XALPHA_API_KEY_ENV} is authenticated against {OXALPHA_KEY_INFO_URL} "
            f"(limit={key_info['limit'] if key_info else '?'} "
            f"remaining={key_info['remaining'] if key_info else '?'})."
        )
    elif auth_state == REMOTE_AUTH_BUDGET_EXHAUSTED:
        detail = (
            f"Remote gate '{host}' reachable; {WORKBAY_0XALPHA_API_KEY_ENV} is valid but OpenRouter "
            f"reports no usable budget (remaining below {OXALPHA_MIN_REMAINING_USD} USD, or an uncapped "
            f"key refused by policy); exit {rc}, marker {auth['marker']!r}."
        )
    else:
        detail = (
            f"Remote gate '{host}' reachable but 0xalpha-remote auth is {auth_state} "
            f"(exit {rc}, marker {auth['marker']!r}). {_0XALPHA_REMOTE_REMEDY}"
        )
    payload = {
        "capabilities": _availability_caps(base, is_available=authenticated),
        "is_available": authenticated,
    ...
    if key_info is not None:
        payload["key_info"] = key_info
    _probe_0xalpha_remote_cache[host] = (now + _PROBE_0XALPHA_REMOTE_TTL_S, payload)
    return payload
