...
from workbay_orchestrator_mcp.orchestration.codex_lane_config import (
    ...
    CODEX_LIST_MODELS_ARGV,
    CODEX_MODEL_TIERS,
    CODEX_PROBE_MODEL_ARGV,
    CODEX_REMOTE_BIN,
    DEFAULT_CODEX_MODEL,
    TRACKED_CODEX_MODEL,
    WORKBAY_CODEX_MODEL_ENV,
)
from workbay_orchestrator_mcp.orchestration.cursor_lane_config import (
    CURSOR_LIST_MODELS_ARGV,
    CURSOR_REMOTE_ALLOWED_MODELS,
    CURSOR_REMOTE_BIN,
    DEFAULT_CURSOR_MODEL,
    TRACKED_CURSOR_MODEL,
    WORKBAY_CURSOR_MODEL_ENV,
)
from workbay_orchestrator_mcp.orchestration.env_file_path import _validate_env_file
from workbay_orchestrator_mcp.orchestration.grok_lane_config import (
    DEFAULT_GROK_MODEL,
    GROK_LIST_MODELS_ARGV,
    GROK_REMOTE_BIN,
    TRACKED_GROK_MODEL,
    WORKBAY_GROK_MODEL_ENV,
)
...
from workbay_orchestrator_mcp.orchestration.openrouter_lane_config import (
    ...
    OPENROUTER_MIN_REMAINING_USD,
    TRACKED_OPENROUTER_MODEL,
    WORKBAY_OPENROUTER_API_KEY_ENV,
    WORKBAY_OPENROUTER_MODEL_ENV,
)
...
class BackendCapabilities:
    ...
    # Whether the execution environment provisions ``<lane-root>/.venv`` before
    # the worker reads its prompt. This is an environment guarantee, not a
    # routing permission; keep it separate from ``dispatchable_off_box``.
    provisions_lane_venv: bool = False


# Whole-token grammars for AuthPort fields that are substituted into the remote
# bash probes (implementation note S2). Every token is emitted unquoted or inside a
# double-quoted "$HOME/..." expansion, so none may carry shell metacharacters.
_AUTH_ENV_VAR_RE = re.compile(r"^[A-Z_][A-Z0-9_]*$")
_AUTH_STATUS_ARGV_TOKEN_RE = re.compile(r"^[A-Za-z0-9._=+:@/-]+$")
# VM-relative (``.local/bin/x``), ``~/``-relative or absolute; same charset as
# env_file. A trailing ``/`` is refused (S2-L-03): it names a directory, not a
# binary, and would leave the device_login marker prefix empty.
_AUTH_BINARY_RE = re.compile(r"^(?:~/|/)?[A-Za-z0-9._+][A-Za-z0-9._+/-]*(?<!/)$")
# Key-info URL for an env_file port's authenticated step (implementation note S4). It is
# emitted inside double quotes in the probe, so: https only (a bearer never
# travels in clear), and no ``$``, backtick, quote, backslash or whitespace.
_AUTH_KEY_INFO_URL_RE = re.compile(r"^https://[A-Za-z0-9][A-Za-z0-9._/:?=&%+-]*$")


@dataclass(frozen=True)
class AuthPort:
    """Declared credential port for an off-box backend (implementation note S1).

    ``env_file`` kind: the VM sources ``env_file`` to export ``env_var``.
    ``device_login`` kind: a login ``artifact`` (file/dir under the VM $HOME)
    must exist. ``binary`` is VM-relative (e.g. ``.local/bin/codex``);
    ``status_argv`` is a headless auth check — empty means the port can only
    report ``unverified`` unless the port declares a ``key_info_url``.

    ``key_info_url`` (env_file only, implementation note S4): the authenticated step is
    a bearer-on-stdin ``curl`` to this URL run on the VM; the JSON reply's
    ``data.limit`` / ``data.limit_remaining`` / ``data.usage`` decide 0 / 15
    (``min_remaining_usd`` is the policy floor, passed into the rendered
    script from the port so there is one source).

    ``credits_url`` (env_file only, requires ``key_info_url``): a second
    bearer-on-stdin ``curl`` after a successful key-info parse, to the
    account credit pool. ``data.total_credits - data.total_usage`` is the
    spendable balance the 402 names; ``is_free_tier: true`` on the already-
    fetched key-info body is also budget-exhausted. Same https / no-shell-
    metacharacter grammar as ``key_info_url`` (reuse ``_AUTH_KEY_INFO_URL_RE``).
    """

    kind: Literal["env_file", "device_login"]
    binary: str
    env_file: str | None = None
    env_var: str | None = None
    artifact: str | None = None  # device_login only
    status_argv: tuple[str, ...] = ()
    key_info_url: str | None = None  # env_file only
    min_remaining_usd: float | None = None  # requires key_info_url
    credits_url: str | None = None  # env_file only; requires key_info_url

    def __post_init__(self) -> None:
        if self.kind not in ("env_file", "device_login"):
            raise ValueError(f"AuthPort.kind must be 'env_file' or 'device_login', got {self.kind!r}")
        if not isinstance(self.binary, str) or not self.binary:
            raise ValueError("AuthPort.binary must be a non-empty VM-relative path")
        if "\0" in self.binary or not _AUTH_BINARY_RE.fullmatch(self.binary):
            raise ValueError(f"AuthPort.binary not whole-token safe: {self.binary!r}")
        if not isinstance(self.status_argv, tuple):
            raise ValueError("AuthPort.status_argv must be a tuple of str tokens")
        for token in self.status_argv:
            if not isinstance(token, str) or not _AUTH_STATUS_ARGV_TOKEN_RE.fullmatch(token):
                raise ValueError(f"AuthPort.status_argv token not whole-token safe: {token!r}")
        if self.kind == "env_file":
            if not self.env_file or not self.env_var:
                raise ValueError("env_file AuthPort requires both env_file and env_var")
            if not _AUTH_ENV_VAR_RE.fullmatch(self.env_var):
                raise ValueError(f"env_file AuthPort env_var must match ^[A-Z_][A-Z0-9_]*$: {self.env_var!r}")
            if self.artifact is not None:
                raise ValueError("env_file AuthPort must not set artifact (device_login only)")
            # Same regex as backend_spec.AgentSpec.env_file (DATA-14): one validator.
            _validate_env_file(self.env_file)
            if self.key_info_url is not None:
                if not isinstance(self.key_info_url, str) or not _AUTH_KEY_INFO_URL_RE.fullmatch(self.key_info_url):
                    raise ValueError(
                        f"AuthPort.key_info_url must be an https URL with no shell metacharacters: {self.key_info_url!r}"
                    )
                if self.status_argv:
                    raise ValueError(
                        "AuthPort.key_info_url and status_argv are alternative authenticated steps; declare one"
                    )
                if (
                    isinstance(self.min_remaining_usd, bool)
                    or not isinstance(self.min_remaining_usd, (int, float))
                    or not math.isfinite(self.min_remaining_usd)
                    or self.min_remaining_usd < 0
                ):
                    raise ValueError(
                        f"AuthPort.key_info_url requires a finite non-negative min_remaining_usd, got {self.min_remaining_usd!r}"
                    )
            elif self.min_remaining_usd is not None:
                raise ValueError("AuthPort.min_remaining_usd requires key_info_url")
            if self.credits_url is not None:
                if self.key_info_url is None:
                    raise ValueError("AuthPort.credits_url requires key_info_url")
                if not isinstance(self.credits_url, str) or not _AUTH_KEY_INFO_URL_RE.fullmatch(self.credits_url):
                    raise ValueError(
                        f"AuthPort.credits_url must be an https URL with no shell metacharacters: {self.credits_url!r}"
                    )
                if self.status_argv:
                    raise ValueError(
                        "AuthPort.credits_url and status_argv are alternative authenticated steps; declare one"
                    )
        else:
            if self.key_info_url is not None or self.min_remaining_usd is not None or self.credits_url is not None:
                raise ValueError("device_login AuthPort must not set key_info_url/min_remaining_usd/credits_url")
            if self.env_file is not None or self.env_var is not None:
                raise ValueError("device_login AuthPort must not set env_file/env_var")
            if not self.artifact:
                raise ValueError("device_login AuthPort requires artifact")
            # Same path grammar as env_file: whole-token safe, no shell metacharacters.
            _validate_env_file(self.artifact)
        # S2-L-03: the probe's stdout markers are ``<PREFIX>_AUTH_OK`` etc.; an
        # empty prefix would render bare ``_AUTH_OK`` markers. Refuse at
        # declaration time rather than at render time.
        if not self.marker_prefix():
            raise ValueError(
                f"AuthPort marker prefix would be empty (env_var={self.env_var!r}, binary={self.binary!r})"
            )

    def marker_prefix(self) -> str:
        """Uppercase token for the probe's stdout markers (``<PREFIX>_AUTH_OK`` ...).

        env_file ports derive it from the key variable with a trailing ``_API_KEY``
        stripped (``CURSOR_API_KEY`` -> ``CURSOR``, preserving the pre-0208 cursor
        markers byte-for-byte); device_login ports use the binary basename.
        """
        if self.kind == "env_file" and self.env_var:
            return self.env_var.removesuffix("_API_KEY")
        basename = self.binary.rsplit("/", 1)[-1]
        return re.sub(r"[^A-Z0-9]", "_", basename.upper())

    def require_env_file(self) -> str:
        """Return ``env_file`` for an env_file-kind port; raise otherwise (narrows str | None)."""
        if self.kind != "env_file" or self.env_file is None:
            raise ValueError(f"AuthPort kind={self.kind!r} has no env_file")
...
class BackendSpec:
    ...
    # CURATED authorization set of caller-selectable model slugs for remote
    # transports (implementation note finding P0199-S4-adapter-backend-id-name-branch).
    # Hand-authored under SECD-05 deny-by-default — never derived from a vendor
    # catalogue, a probe, or ``--list-models``. None means the single configured
    # ``allowed_model`` pin is the only allowed model.
    ...
    # Catalogue listing argv + compile-time tracked pin (implementation note M2/M3).
    # Discovery reads these fields instead of a backend-name triple (REF-24).
    # A dispatchable_off_box row that omits them fails closed (pin_home_undeclared).
    list_models_argv: tuple[str, ...] | None = None
    # Headless per-candidate model entitlement command prefix. Consumers append
    # the model slug and probe prompt; None is the typed unsupported contract.
    probe_model_argv: tuple[str, ...] | None = None
    # Native metadata producer (ACP / newline JSON-RPC). Distinct from
    # prompt-based probe_model_argv: this port observes advertised models and
    # an exact session/set_model selection. None means the backend has no
    # native metadata handshake.
    native_metadata_producer_path: str | None = None
    tracked_model: str | None = None
    # Optional per-model tier registry for routing-capable backends. It is the
    # same mapping object exported by the backend config, not a copied table.
    model_tiers: Mapping[str, Any] | None = None
    # Declared credential port (implementation note S1). Every dispatchable_off_box row
    # must declare one — _require_auth_port fails closed at import otherwise.
...
BACKENDS: dict[str, BackendSpec] = {
    "codex-cli": BackendSpec(
        ...
        list_models_argv=CODEX_LIST_MODELS_ARGV,
        probe_model_argv=CODEX_PROBE_MODEL_ARGV,
        tracked_model=TRACKED_CODEX_MODEL,
        allowed_model_env=WORKBAY_CODEX_MODEL_ENV,
    ...
    "grok-cli": BackendSpec(
        ...
        list_models_argv=GROK_LIST_MODELS_ARGV,
        tracked_model=TRACKED_GROK_MODEL,
        allowed_model_env=WORKBAY_GROK_MODEL_ENV,
        native_metadata_producer_path=(
            "workbay_orchestrator_mcp.orchestration.adapters.grok_cli.produce_native_metadata"
        ),
    ),
    "grok-remote": BackendSpec(
        ...
        shaping_adapter_path=("workbay_orchestrator_mcp.orchestration.adapters.grok_cli.GrokCliAdapter"),
        availability_probe_path=("workbay_orchestrator_mcp.orchestration.backend_registry._probe_grok_remote"),
        ...
        auth=AuthPort(
            kind="device_login",
            artifact="~/.grok/auth.json",
            binary=GROK_REMOTE_BIN,
        ),
        capabilities=BackendCapabilities(
            supports_structured_output=True,
            supports_reasoning_effort=True,
    ...
    "codex-remote": BackendSpec(
        ...
        description=("Ship a codex exec turn to the remote OCI VM via remote_agent.sh --agent-spec (implementation note)."),
        shaping_adapter_path=("workbay_orchestrator_mcp.orchestration.adapters.codex_cli.CodexCliAdapter"),
        availability_probe_path=("workbay_orchestrator_mcp.orchestration.backend_registry._probe_codex_remote"),
        allowed_model=DEFAULT_CODEX_MODEL,
        allowed_models=CODEX_ALLOWED_MODELS,
        allowed_model_env=WORKBAY_CODEX_MODEL_ENV,
        list_models_argv=CODEX_LIST_MODELS_ARGV,
        probe_model_argv=CODEX_PROBE_MODEL_ARGV,
        tracked_model=TRACKED_CODEX_MODEL,
        ...
        post_turn_fetch_headroom_s=15,
        # S2-H-01 / S2-L-01: ``codex login status`` prints "Logged in using
        # ChatGPT" and exits 0 when logged in (VM-verified, codex-cli 0.145.0);
        # the login artifact is ``~/.codex/auth.json`` (implementation note measured
        # layout) — ``~/.codex`` alone is created by any codex invocation.
        auth=AuthPort(
            kind="device_login",
            artifact="~/.codex/auth.json",
            binary=CODEX_REMOTE_BIN,
    ...
    # implementation note S4: OpenRouter = codex CLI on the OCI VM against OpenRouter's
    # OpenAI-compatible transport. z-ai/glm-5.3-flash is NOT an OpenAI model; the
    # codex binary/sandbox pins are reused, the provider + credential port are
    # its own (openrouter_lane_config). No list_models_argv: OpenRouter's
    # catalogue must never feed the allow-list (SECD-05). tracked_model is the
    # pin home for discovery (env > tracked; never a catalogue probe).
    "openrouter-remote": BackendSpec(
        kind="cli",
        adapter_path="workbay_orchestrator_mcp.orchestration.adapters.remote_exec.RemoteExecAdapter",
        cost_class=COST_REMOTE,
        model_family="openrouter",
        # After codex-remote (6) and cursor-remote (7) among off-box peers.
        review_rank=8,
        # --output-schema via codex -> supports_structured_output=True, so the
        # transport can return a parseable findings block.
        review_eligible=True,
        description="OpenRouter: codex CLI on the OCI VM against OpenRouter (OpenAI-compatible)",
        shaping_adapter_path=("workbay_orchestrator_mcp.orchestration.adapters.codex_cli.CodexCliAdapter"),
        availability_probe_path=("workbay_orchestrator_mcp.orchestration.backend_registry._probe_openrouter_remote"),
        allowed_model=DEFAULT_OPENROUTER_MODEL,
        allowed_models=OPENROUTER_ALLOWED_MODELS,
        allowed_model_env=WORKBAY_OPENROUTER_MODEL_ENV,
        tracked_model=TRACKED_OPENROUTER_MODEL,
        post_turn_fetch_headroom_s=15,
        # Empty status_argv is honest: codex has no headless "am I authed to
        # provider X" command. The authenticated step is the key-info stage
        # (bearer on stdin to OPENROUTER_KEY_INFO_URL, run on the VM); the first
        # turn's auth_match is the real check.
        auth=AuthPort(
            kind="env_file",
            env_file=OPENROUTER_ENV_FILE,
            env_var=WORKBAY_OPENROUTER_API_KEY_ENV,
            binary=CODEX_REMOTE_BIN,
            status_argv=(),
            key_info_url=OPENROUTER_KEY_INFO_URL,
            min_remaining_usd=OPENROUTER_MIN_REMAINING_USD,
            credits_url=OPENROUTER_CREDITS_URL,
        ),
        # Capabilities as codex-remote.
        capabilities=BackendCapabilities(
            is_available=False,
            supports_structured_output=True,
            supports_reasoning_effort=True,
            supports_token_telemetry=False,
            supports_sandbox=True,
            supports_token_budget_cycle_bounds=False,  # -> requires_timeout
            supports_adapter_timeout_bounds=True,
            runs_self_verify_off_box=True,
            dispatchable_off_box=True,
            provisions_lane_venv=True,
    ...
    "cursor-remote": BackendSpec(
        ...
        description=("Ship a cursor-agent turn to the remote OCI VM via remote_agent.sh --agent-spec (implementation note S7)."),
        shaping_adapter_path=("workbay_orchestrator_mcp.orchestration.adapters.cursor_cli.CursorCliAdapter"),
        availability_probe_path=("workbay_orchestrator_mcp.orchestration.backend_registry._probe_cursor_remote"),
        allowed_model=DEFAULT_CURSOR_MODEL,
        # Curated authorization set (implementation note finding 18210): the registry row
        # owns both the pin and the allow-list. Hand-curated (SECD-05) - never
        # catalogue- or probe-derived. list_models_argv/tracked_model below feed
        # the EFFORT vocabulary only and must never widen this set.
        allowed_models=CURSOR_REMOTE_ALLOWED_MODELS,
        allowed_model_env=WORKBAY_CURSOR_MODEL_ENV,
        list_models_argv=CURSOR_LIST_MODELS_ARGV,
        tracked_model=TRACKED_CURSOR_MODEL,
        post_turn_fetch_headroom_s=15,
        # Single literal for the cursor credential path; backend_spec.CURSOR_ENV_FILE
        # re-exports this value (implementation note S1).
        auth=AuthPort(
            kind="env_file",
            env_file="~/.config/cursor-agent/env",
            env_var="CURSOR_API_KEY",
            status_argv=("status",),
            binary=CURSOR_REMOTE_BIN,
        ),
        capabilities=BackendCapabilities(
...
def _require_auth_port(name: str, spec: BackendSpec) -> None:
    """Refuse an off-box row with no declared credential port (implementation note S1)."""
    if spec.capabilities.dispatchable_off_box and spec.auth is None:
        raise ValueError(
            f"backend {name!r} is dispatchable_off_box but declares no AuthPort; "
            f"set BackendSpec.auth (env_file or device_login)"
        )


def auth_port(name: str) -> AuthPort:
    """Return the declared AuthPort for ``name``; raise if the row has none."""
    spec = get_backend_spec(name)
    if spec.auth is None:
        raise ValueError(f"backend {name!r} declares no AuthPort")
    return spec.auth
...
def register_backend(name: str, spec: BackendSpec) -> None:
    ...
    _require_auth_port(name, spec)
    BACKENDS[name] = spec


# Static registry: same guards as register_backend so a future table edit cannot
# silently land a default-ranked local reviewer (WIDTH-47) or an off-box row
# with no credential port (implementation note S1).
for _name, _spec in BACKENDS.items():
    _require_explicit_review_rank(_name, _spec)
    _require_auth_port(_name, _spec)


# Retired backend ids are fail-closed discovery metadata, never resolvable
# (CLAUDE.md retired-id pattern). Historical worktree_lanes rows keep the old
# string; looking the id up must name the successor and must never alias.
RETIRED_BACKENDS: dict[str, str] = {
    "0xalpha-remote": "openrouter-remote",
}


class RetiredBackendError(RuntimeError):
    """Raised when a retired backend id is resolved. Fail-closed; never aliases."""


def raise_if_retired_backend(name: str) -> None:
    successor = RETIRED_BACKENDS.get(name.strip())
    if successor is not None:
        raise RetiredBackendError(f"Backend {name!r} is retired; use {successor!r}.")


def validate_backend(name: str) -> str:
    normalized = name.strip()
    raise_if_retired_backend(normalized)
...
def _probe_subprocess_run(*args: Any, **kwargs: Any) -> Any:
    """Run an availability-probe subprocess without inheriting MCP JSON-RPC stdin.

    The orchestrator MCP server's fd0 is the JSON-RPC stdio pipe — a non-tty,
    never-EOF fd. Probe children (``codex --help``, ``ssh``, ``git rev-parse``)
    that inherit it consume the next request and hang the server
    (internal). Force ``stdin=DEVNULL`` at this chokepoint so
    a new sibling cannot omit the kwarg. SSH probes also pass ``-n``.
    """
    kwargs.pop("stdin", None)
    return subprocess.run(*args, stdin=subprocess.DEVNULL, **kwargs)  # noqa: S603


def _ssh_probe_argv(host: str, remote_cmd: str, *, connect_timeout: int) -> list[str]:
    """SSH argv for availability probes: ``-n`` plus caller-supplied ``stdin=DEVNULL``."""
    return [
        "ssh",
        "-n",
        "-o",
...
def _resolve_remote_probe_repo_root(workspace_root: Path | str | None = None) -> Path:
    ...
    if workspace_root is not None:
        try:
            ...
            start = Path.cwd()
    try:
        completed = _probe_subprocess_run(
...
def probe_capabilities(name: str, *, workspace_root: Path | str | None = None) -> BackendCapabilities:
    ...
    if name == "codex-cli":
        ...
        try:
            ...
            help_res = _probe_subprocess_run(
                [bin_path, "exec", "--help"],
                capture_output=True,
                text=True,
                check=False,
...
# S4-L-06: env file / env var come from the row's AuthPort (single literal).
_OPENROUTER_REMOTE_REMEDY = (
    "openrouter-remote requires WORKBAY_REMOTE_GATE_HOST and a provisioned OCI VM with "
    f"the pinned codex binary plus a readable {BACKENDS['openrouter-remote'].auth.env_file} carrying a "
    f"non-empty {BACKENDS['openrouter-remote'].auth.env_var} whose OpenRouter key-info reports remaining "
    "budget (authenticated readiness, not mere file presence). "
    "See docs/runbooks/remote-gate-provisioning.md and scripts/provision_remote_auth.sh "
    "--backend openrouter-remote."
)
# Remote credential-probe exit-code contract (S6-M01; generalised in implementation note S2).
# Shared by every AuthPort renderer below, whatever the port kind:
#   0  — binary present + credential present + headless status ok
#   10 — binary missing / not executable
#   11 — credential artifact (env file / login artifact) missing or unreadable
#   12 — env present but no valid non-empty assignment of the declared env var
#   13 — credential present but headless status reports unauthenticated
#   14 — installed but auth could not be verified (status ambiguous / no status cmd)
#   15 — budget exhausted: key valid, but the provider reports no remaining spend
#        (distinct from 13; emitted by key_info_url ports — openrouter-remote since
#        implementation note S4 — when data.limit is null, limit_remaining < threshold,
#        is_free_tier is true, or the credits-pool available balance is below
#        threshold)
# Never treat mere env-file presence as available.
# Machine-readable auth states for remote probe payloads (values are a wire
# contract — the doctor and tests compare the strings).
REMOTE_AUTH_AUTHENTICATED = "authenticated"
REMOTE_AUTH_MISSING = "missing"
REMOTE_AUTH_INVALID = "invalid"
REMOTE_AUTH_FAILED = "unauthenticated"
REMOTE_AUTH_UNVERIFIED = "unverified"
REMOTE_AUTH_NA = "not_applicable"
REMOTE_AUTH_BUDGET_EXHAUSTED = "budget_exhausted"
_REMOTE_AUTH_EXIT_CODES: dict[int, str] = {
    0: REMOTE_AUTH_AUTHENTICATED,
    10: REMOTE_AUTH_MISSING,
    11: REMOTE_AUTH_MISSING,
    12: REMOTE_AUTH_INVALID,
    13: REMOTE_AUTH_FAILED,
    14: REMOTE_AUTH_UNVERIFIED,
    15: REMOTE_AUTH_BUDGET_EXHAUSTED,
}
# Machine-readable install states for cursor-remote probe payloads.
CURSOR_REMOTE_INSTALL_INSTALLED = "installed"
CURSOR_REMOTE_INSTALL_MISSING = "not_installed"
# Deprecated aliases (implementation note S2): same objects as REMOTE_AUTH_*; prefer the generic names.
CURSOR_REMOTE_AUTH_AUTHENTICATED = REMOTE_AUTH_AUTHENTICATED
CURSOR_REMOTE_AUTH_MISSING = REMOTE_AUTH_MISSING
CURSOR_REMOTE_AUTH_INVALID = REMOTE_AUTH_INVALID
CURSOR_REMOTE_AUTH_FAILED = REMOTE_AUTH_FAILED
CURSOR_REMOTE_AUTH_UNVERIFIED = REMOTE_AUTH_UNVERIFIED
CURSOR_REMOTE_AUTH_NA = REMOTE_AUTH_NA


from workbay_orchestrator_mcp.orchestration.auth_probe_render import (
    _CREDITS_LINE_RE,
    _KEY_INFO_LINE_RE,
    render_auth_probe,
    render_env_file_auth_probe,
)

_lazy_render_exports = {
    "CREDITS_PARSER_PY": "auth_probe_render",
    "FREE_TIER_CHECK_PY": "auth_probe_render",
    "KEY_INFO_PARSER_PY": "auth_probe_render",
    "_auth_marker_prefix": "auth_probe_render",
    "_render_binary_health_check": "auth_probe_render",
    "_render_key_info_stage": "auth_probe_render",
    "_render_status_command": "auth_probe_render",
    "_vm_home_path": "auth_probe_render",
    "render_device_login_auth_probe": "auth_probe_render",
    "OPENROUTER_MODELS_LIST_URL": "doctor_render",
    "_doctor_auth_artifact": "doctor_render",
    "_render_doctor_key_info_block": "doctor_render",
    "doctor_auth_ports": "doctor_render",
    "render_doctor_auth_line": "doctor_render",
    "render_doctor_auth_script": "doctor_render",
}


def __getattr__(name: str):
    module_name = _lazy_render_exports.get(name)
    if module_name is None:
        raise AttributeError(name)
    module = importlib.import_module(f"workbay_orchestrator_mcp.orchestration.{module_name}")
    return getattr(module, name)


_render_doctor_key_info_block: Callable[..., str]
doctor_auth_ports: Callable[[], list[tuple[str, Any]]]
render_doctor_auth_line: Callable[[str, Any], str]
render_doctor_auth_script: Callable[[], str]

_REMOTE_CURSOR_AUTH_PROBE = render_env_file_auth_probe(auth_port("cursor-remote"))
...
_probe_codex_remote_cache: dict[str, tuple[float, dict[str, Any], str, Literal["disk", "import"], str]] = {}


def _cache_codex_remote_payload(
    host: str,
    now: float,
    payload: dict[str, Any],
    *,
    pin: str,
    pin_source: Literal["disk", "import"],
    imported_pin: str,
) -> None:
    """Cache a codex probe only for the pin snapshot that produced it."""
    _probe_codex_remote_cache[host] = (
        now + _PROBE_CODEX_REMOTE_TTL_S,
        payload,
        pin,
        pin_source,
        imported_pin,
...
def _resolve_remote_gate_host_for_probe(*, workspace_root: Path | str | None = None) -> str:
...
def _remote_host_is_malformed(host: str) -> bool:
    """Fail-closed host guard shared by the remote probes (SEC / RES-13 / CWE-88).

    A leading '-' is parsed by ssh as an option (e.g. -oProxyCommand=…) and
    whitespace can smuggle extra tokens; refuse before any subprocess.
    """
    return host.startswith("-") or any(ch.isspace() for ch in host)


_REMOTE_AUTH_TRANSPORT_EXIT = -1


def probe_remote_auth(
    backend_id: str,
    *,
    host: str,
    timeout: float,
    ...
    """Run ``backend_id``'s declared AuthPort probe on ``host`` over ssh (implementation note S2).

    Returns ``{"auth_state": REMOTE_AUTH_*, "exit_code": int, "marker": str}``.
    ``exit_code`` is the remote script's exit status mapped through the shared
    contract (0/10/11/12/13/14/15). Any other code (ssh's own 255, a killed
    shell, a garbage returncode) is transport, not a credential verdict, and
    maps to ``REMOTE_AUTH_UNVERIFIED`` (S2-M-03) — never to FAILED, so an
    operator is not sent to re-authenticate when the VM was unreachable. A
    refused host or a row without an AuthPort yields ``REMOTE_AUTH_NA``; an
    ssh/timeout exception yields ``REMOTE_AUTH_UNVERIFIED``; all with
    ``exit_code == -1`` and a marker naming the cause (``host_refused``,
    ``no_auth_port``, ``ssh_error:<ExceptionClass>``, ``bad_returncode``).
    ``runner`` defaults to :func:`_probe_subprocess_run` (stdin isolated) and is
    injectable so tests can fake ssh. Never raises.
    """
    try:
        port = auth_port(backend_id)
    except (KeyError, ValueError, RuntimeError):
        # validate_backend raises RuntimeError for an unknown row; auth_port
        # raises ValueError for a row that declares no port.
        return {"auth_state": REMOTE_AUTH_NA, "exit_code": _REMOTE_AUTH_TRANSPORT_EXIT, "marker": "no_auth_port"}
    script = render_auth_probe(port)
    host = (host or "").strip()
    if not host or _remote_host_is_malformed(host):
        return {"auth_state": REMOTE_AUTH_NA, "exit_code": _REMOTE_AUTH_TRANSPORT_EXIT, "marker": "host_refused"}
    run = runner if runner is not None else _probe_subprocess_run
    try:
        result = run(
            _ssh_probe_argv(host, script, connect_timeout=5),
            capture_output=True,
            text=True,
            check=False,
            timeout=timeout,
        ...
        # Transport never shares a state with a credential failure.
        return {
            "auth_state": REMOTE_AUTH_UNVERIFIED,
            "exit_code": _REMOTE_AUTH_TRANSPORT_EXIT,
            "marker": f"ssh_error:{type(exc).__name__}",
        }
    try:
        ...
        return {
            "auth_state": REMOTE_AUTH_UNVERIFIED,
            "exit_code": _REMOTE_AUTH_TRANSPORT_EXIT,
            "marker": "bad_returncode",
        }
    stdout = result.stdout if isinstance(result.stdout, str) else ""
    lines = [line.strip() for line in stdout.splitlines() if line.strip()]
    # The marker is the ``<PREFIX>_…`` line; a key_info port prints its data
    # line first (implementation note S4), so prefer the prefixed line over "first line".
    prefix = port.marker_prefix() + "_"
    marker = next((line for line in lines if line.startswith(prefix)), lines[0] if lines else "")
    payload: dict[str, Any] = {
        "auth_state": _REMOTE_AUTH_EXIT_CODES.get(rc, REMOTE_AUTH_UNVERIFIED),
        "exit_code": rc,
        "marker": marker,
    }
    key_info = _parse_key_info_line(lines)
    if key_info is not None:
        payload["key_info"] = key_info
    elif rc == 0 and port.key_info_url:
        # S4-M-03: a key_info port's AUTH_OK is only credible with a finite,
        # well-formed data line. rc 0 + garbled/missing line -> unverified.
        payload["auth_state"] = REMOTE_AUTH_UNVERIFIED
        payload["key_info_error"] = "missing_or_garbled_data_line"
    credits = _parse_credits_line(lines)
    if credits is not None:
        payload["credits"] = credits
    elif rc == 0 and port.credits_url and payload["auth_state"] == REMOTE_AUTH_AUTHENTICATED:
        # A credits_url port's AUTH_OK is only credible with both readings.
        # Reporting remaining=24.874 without the wallet is the 402-dead lie.
        payload["auth_state"] = REMOTE_AUTH_UNVERIFIED
        payload["credits_error"] = "missing_or_garbled_data_line"
    return payload


def _parse_probe_data_line(lines: list[str], pattern: re.Pattern[str]) -> dict[str, float] | None:
    """First full-match of ``pattern`` -> finite floats, or None when absent/garbled."""
    for line in lines:
        match = pattern.fullmatch(line)
        if match is None:
            continue
        try:
            parsed = {key: float(value) for key, value in match.groupdict().items()}
        except ValueError:
            return None
...
def _parse_key_info_line(lines: list[str]) -> dict[str, float] | None:
    """``limit=<n> usage=<n> remaining=<n>`` -> floats, or None when absent/garbled."""
    return _parse_probe_data_line(lines, _KEY_INFO_LINE_RE)


def _parse_credits_line(lines: list[str]) -> dict[str, float] | None:
    """``credits=<n> credit_usage=<n> credits_available=<n>`` -> floats, or None."""
    return _parse_probe_data_line(lines, _CREDITS_LINE_RE)
...
def _probe_cursor_remote(*, workspace_root: Path | str | None = None) -> dict[str, Any]:
    ...
    auth = probe_remote_auth("cursor-remote", host=host, timeout=20)
    rc = auth["exit_code"]
    auth_state = auth["auth_state"]
    if rc == _REMOTE_AUTH_TRANSPORT_EXIT:
        # ssh/timeout error (host was validated above): unreachable, auth n/a.
        payload = {
            "capabilities": _availability_caps(base, is_available=False),
            "is_available": False,
            "state": AVAIL_UNAVAILABLE,
            "detail": f"Remote gate '{host}' unreachable for cursor-remote: {auth['marker']}.",
            "install_state": CURSOR_REMOTE_INSTALL_MISSING,
            "auth_state": REMOTE_AUTH_NA,
        }
    ...
    # S2-M-03: one classification — branch on the auth_state probe_remote_auth
    # already derived, never on a second rc switch (so an unlisted rc such as
    # ssh's 255 reads as unverified here exactly as it does there). The only
    # rc consulted below is 10-vs-11 inside MISSING, which the shared contract
    # deliberately collapses into one state.
    if auth_state == REMOTE_AUTH_AUTHENTICATED:
        payload = {
            ...
            "auth_state": REMOTE_AUTH_AUTHENTICATED,
        }
    elif auth_state == REMOTE_AUTH_MISSING and rc == 10:
        payload = {
            "capabilities": _availability_caps(base, is_available=False),
            "is_available": False,
            "state": AVAIL_UNAVAILABLE,
            "detail": (f"Remote gate '{host}' reachable but cursor-agent binary missing at ~/.local/bin/cursor-agent."),
            "install_state": CURSOR_REMOTE_INSTALL_MISSING,
            "auth_state": REMOTE_AUTH_NA,
        }
    elif auth_state == REMOTE_AUTH_MISSING:
        payload = {
            "capabilities": _availability_caps(base, is_available=False),
            "is_available": False,
            "state": AVAIL_UNAVAILABLE,
            ...
            "install_state": CURSOR_REMOTE_INSTALL_INSTALLED,
            "auth_state": REMOTE_AUTH_MISSING,
        }
    elif auth_state == REMOTE_AUTH_INVALID:
        payload = {
            "capabilities": _availability_caps(base, is_available=False),
            "is_available": False,
            "state": AVAIL_UNAVAILABLE,
            ...
            "install_state": CURSOR_REMOTE_INSTALL_INSTALLED,
            "auth_state": REMOTE_AUTH_INVALID,
        }
    elif auth_state == REMOTE_AUTH_FAILED:
        payload = {
            "capabilities": _availability_caps(base, is_available=False),
            "is_available": False,
            "state": AVAIL_UNAVAILABLE,
            ...
            "install_state": CURSOR_REMOTE_INSTALL_INSTALLED,
            "auth_state": REMOTE_AUTH_FAILED,
        }
    elif auth_state == REMOTE_AUTH_BUDGET_EXHAUSTED:
        payload = {
            "capabilities": _availability_caps(base, is_available=False),
            "is_available": False,
            "state": AVAIL_UNAVAILABLE,
            "detail": (
                f"Remote gate '{host}' has cursor-agent authenticated but the "
                "provider reports no remaining spend (budget exhausted)."
            ),
            "install_state": CURSOR_REMOTE_INSTALL_INSTALLED,
            "auth_state": REMOTE_AUTH_BUDGET_EXHAUSTED,
        }
    else:
        # REMOTE_AUTH_UNVERIFIED (rc 14 or any unlisted rc): installed but auth
        # not verified — refuse to advertise availability (never convert file
        # presence into available).
        payload = {
            "capabilities": _availability_caps(base, is_available=False),
            "is_available": False,
            "state": AVAIL_UNAVAILABLE,
...
def _codex_cli_version_pin() -> tuple[str, Literal["disk", "import"], str]:
    """Read the current codex CLI pin and record where the value came from.

    The MCP server can outlive an edit to ``codex_lane_config.py``. Read the
    source on every probe so a long-lived process does not misdiagnose a newer
    on-disk pin as a remote binary upgrade. ``pin_source=import`` is the
    fail-closed fallback when the source cannot be read or does not contain a
    literal ``CODEX_CLI_VERSION`` assignment; that source marker is retained in
    the probe detail as the durable record of the fallback.
    """
    from workbay_orchestrator_mcp.orchestration import codex_lane_config  # noqa: PLC0415

    imported_pin = codex_lane_config.CODEX_CLI_VERSION
    pin_loaded_at = datetime.now(tz=timezone.utc).isoformat()
    try:
        source_path = Path(codex_lane_config.__file__)
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Assign):
                ...
                continue
            if not any(isinstance(target, ast.Name) and target.id == "CODEX_CLI_VERSION" for target in targets):
                continue
            parsed_pin = ast.literal_eval(value)
            if isinstance(parsed_pin, str) and parsed_pin:
                return parsed_pin, "disk", pin_loaded_at
            break
    except (AttributeError, OSError, SyntaxError, TypeError, ValueError, UnicodeError):
        # ``pin_source=import`` above is the durable fallback record. A probe
        # must remain available to report its imported pin even when a source
        # read is interrupted by an edit or filesystem failure.
        pass
    return imported_pin, "import", pin_loaded_at


def _codex_pin_detail(
    *,
    pin: str,
    imported_pin: str,
    pin_source: Literal["disk", "import"],
    ...
    """Render pin provenance for the human-facing availability detail."""
    metadata = f"pin={pin}; pin_source={pin_source}; pin_loaded_at={pin_loaded_at}"
    if pin_source == "disk" and pin != imported_pin:
        return f"stale server pin: imported pin is {imported_pin}, disk pin is {pin}; {metadata}"
    return metadata
...
def _probe_codex_remote(*, workspace_root: Path | str | None = None) -> dict[str, Any]:
    ...
    imported_pin = codex_lane_config.CODEX_CLI_VERSION
    pin, pin_source, pin_loaded_at = _codex_cli_version_pin()
    pin_detail = _codex_pin_detail(
        pin=pin,
        imported_pin=imported_pin,
        pin_source=pin_source,
        pin_loaded_at=pin_loaded_at,
    )
    ...
    if not host:
        return {
            ...
            "is_available": False,
            "state": AVAIL_NOT_INSTALLED,
            "detail": f"{_CODEX_REMOTE_REMEDY}; {pin_detail}",
            "auth_state": REMOTE_AUTH_NA,
        }
    if _remote_host_is_malformed(host):
        return {
    ...
    if cached is not None:
        expires_at, payload, cached_pin, cached_source, cached_imported_pin = cached
        if now < expires_at and (cached_pin, cached_source, cached_imported_pin) == (
            pin,
            pin_source,
            imported_pin,
        ):
    ...
    try:
        result = _probe_subprocess_run(
            _ssh_probe_argv(host, remote_cmd, connect_timeout=5),
            capture_output=True,
            text=True,
            check=False,
            timeout=20,
        ...
        payload = {
            ...
            "is_available": False,
            "state": AVAIL_UNAVAILABLE,
            "detail": f"Remote gate '{host}' unreachable for codex-remote: {exc}; {pin_detail}",
            "auth_state": REMOTE_AUTH_NA,
        }
        _cache_codex_remote_payload(
            host,
            now,
            payload,
            pin=pin,
    ...
    if result.returncode == 10 or "CODEX_MISSING" in combined:
        payload = {
            ...
            "state": AVAIL_UNAVAILABLE,
            "detail": (
                f"Remote gate '{host}' reachable but pinned codex binary missing at {CODEX_REMOTE_BIN}; "
                f"{pin_detail}"
            ),
            "auth_state": REMOTE_AUTH_NA,
        }
    ...
    if result.returncode != 0:
        payload = {
            ...
            "state": AVAIL_UNAVAILABLE,
            "detail": (
                f"Remote gate '{host}' reachable but codex-remote probe failed (exit {result.returncode}); "
                f"{pin_detail}"
            ),
            "auth_state": REMOTE_AUTH_NA,
        }
    ...
    if remote_version is None:
        payload = {
            ...
            "detail": (
                ...
                f"({combined.strip()!r}); fail-closed until remeasured; {pin_detail}"
            ),
            "auth_state": REMOTE_AUTH_NA,
        }
        _cache_codex_remote_payload(
        ...
        return payload
    if not _codex_version_in_supported_range(remote_version, pin):
        if pin_source == "disk" and pin != imported_pin:
            detail = (
                f"codex-remote stale server pin: imported pin is {imported_pin}, disk pin is {pin}; "
                f"remote reports {remote_version}; fail-closed until sandbox/auth fixtures are remeasured."
            )
        else:
            detail = (
                f"codex-remote version drift: remote reports {remote_version}, "
                f"supported pin is {pin}; fail-closed until sandbox/auth fixtures are remeasured."
            )
        payload = {
            "capabilities": _availability_caps(base, is_available=False),
            "is_available": False,
            "state": AVAIL_UNAVAILABLE,
            "detail": f"{detail}; {pin_detail}",
            "auth_state": REMOTE_AUTH_NA,
        }
        _cache_codex_remote_payload(
            host,
    ...
    # implementation note S2: the version gate alone no longer advertises availability —
    # the declared AuthPort must also report authenticated (fail-closed).
    auth = probe_remote_auth("codex-remote", host=host, timeout=20)
    authenticated = auth["auth_state"] == REMOTE_AUTH_AUTHENTICATED
    if authenticated:
        detail = (
            f"Remote gate '{host}' reachable; VM codex at {CODEX_REMOTE_BIN} "
            f"reports version {remote_version} (matches {'disk ' if pin_source == 'disk' else ''}pin); "
            f"{pin_detail}; authenticated."
        )
    else:
        detail = (
            f"Remote gate '{host}' reachable; VM codex at {CODEX_REMOTE_BIN} "
            f"reports version {remote_version} (matches {'disk ' if pin_source == 'disk' else ''}pin) but auth is "
            f"{auth['auth_state']} (exit {auth['exit_code']}, marker {auth['marker']!r}); "
            f"selection refuses availability; {pin_detail}"
        )
    payload = {
        "capabilities": _availability_caps(base, is_available=authenticated),
        "is_available": authenticated,
        "state": AVAIL_AVAILABLE if authenticated else AVAIL_UNAVAILABLE,
        "detail": detail,
        "auth_state": auth["auth_state"],
...
_PROBE_OPENROUTER_REMOTE_TTL_S = 30.0
_probe_openrouter_remote_cache: dict[str, tuple[float, dict[str, Any]]] = {}


def _probe_openrouter_remote(*, workspace_root: Path | str | None = None) -> dict[str, Any]:
    """Availability probe for openrouter-remote (implementation note S4). Never raises.

    One ssh round-trip: the declared env_file AuthPort's rendered probe checks
    the pinned codex binary (``--version``), the env file, and then runs the
    key-info stage on the VM (bearer on stdin; the key never leaves the VM).
    ``is_available`` flips only on ``REMOTE_AUTH_AUTHENTICATED``; the payload
    carries ``spend_bound`` = ``data.limit`` as read at probe time (FM-08: the
    bound a lane declares, recorded on dispatch) and the parsed ``key_info``.
    TTL-cached per host like the sibling remote probes.
    """
    base = BACKENDS["openrouter-remote"].capabilities
    host = _resolve_remote_gate_host_for_probe(workspace_root=workspace_root)
    if not host:
        return {
            "capabilities": _availability_caps(base, is_available=False),
            "is_available": False,
            "state": AVAIL_NOT_INSTALLED,
            "detail": _OPENROUTER_REMOTE_REMEDY,
            "auth_state": REMOTE_AUTH_NA,
        }
    if _remote_host_is_malformed(host):
        return {
    ...
    now = time.monotonic()
    cached = _probe_openrouter_remote_cache.get(host)
    if cached is not None:
        expires_at, payload = cached
        if now < expires_at:
            return payload
        del _probe_openrouter_remote_cache[host]

    auth = probe_remote_auth("openrouter-remote", host=host, timeout=30)
    auth_state = auth["auth_state"]
    rc = auth["exit_code"]
    key_info = auth.get("key_info")
    credits = auth.get("credits")
    # Belt and braces with probe_remote_auth: never available without key_info.
    # A credits_url port is also never available without the wallet reading —
    # remaining=24.874 on a 402-dead account is the lie this check exists to stop.
    authenticated = auth_state == REMOTE_AUTH_AUTHENTICATED and key_info is not None and credits is not None
    credits_bit = (
        f" credits={credits['credits']} credit_usage={credits['credit_usage']} "
        f"credits_available={credits['credits_available']}"
        if isinstance(credits, dict)
        else ""
    )
    if rc == _REMOTE_AUTH_TRANSPORT_EXIT:
        detail = f"Remote gate '{host}' openrouter-remote probe did not run ({auth['marker']}); fail-closed."
    elif authenticated:
        detail = (
            f"Remote gate '{host}' reachable; VM codex at {CODEX_REMOTE_BIN} runs and "
            f"{WORKBAY_OPENROUTER_API_KEY_ENV} is authenticated against {OPENROUTER_KEY_INFO_URL} "
            f"(limit={key_info['limit'] if key_info else '?'} "
            f"remaining={key_info['remaining'] if key_info else '?'}{credits_bit})."
        )
    elif auth_state == REMOTE_AUTH_BUDGET_EXHAUSTED:
        remaining_bit = (
            f"remaining={key_info['remaining']}"
            if isinstance(key_info, dict) and "remaining" in key_info
            else "remaining=?"
        )
        detail = (
            f"Remote gate '{host}' reachable; {WORKBAY_OPENROUTER_API_KEY_ENV} is valid but OpenRouter "
            f"reports no usable budget ({remaining_bit}{credits_bit} below {OPENROUTER_MIN_REMAINING_USD} USD, "
            f"is_free_tier, or an uncapped key refused by policy); exit {rc}, marker {auth['marker']!r}."
        )
    else:
        detail = (
            f"Remote gate '{host}' reachable but openrouter-remote auth is {auth_state} "
            f"(exit {rc}, marker {auth['marker']!r}). {_OPENROUTER_REMOTE_REMEDY}"
        )
    payload = {
        "capabilities": _availability_caps(base, is_available=authenticated),
        "is_available": authenticated,
        ...
        "exit_code": rc,
        "spend_bound": key_info["limit"] if (authenticated and key_info) else None,
    }
    if key_info is not None:
        payload["key_info"] = key_info
    if credits is not None:
        payload["credits"] = credits
    _probe_openrouter_remote_cache[host] = (now + _PROBE_OPENROUTER_REMOTE_TTL_S, payload)
    return payload
