...
Unregistered paths are the immediate subdirectories of the parent of the
collector root as the caller spelled it — not the parent of a symlink's
target — that are not in the registered set, counted regardless of
naming shape. The volume probe is a single stat on that parent
directory; it assumes all sibling worktrees share that volume (a linked
worktree on another volume would not have its own volume measured).
...
def _measure_unregistered_stock(
    ...
    if sibling_paths is None:
        ...
        return None
    # Every sibling of the primary checkout that is not registered is
    # unregistered stock. No name-prefix gate: lane paths outside the
    # `{primary_name}-*` shape are still physically present and must be
    # visible to the gauge. Both sides are normalised so a relative
    # collector root cannot double-count registered paths.
    try:
        registered_set = {
...
def _list_sibling_paths(parent):
    """Immediate subdirectories of the directory the worktrees live in."""
    return sorted(
...
def collect_worktree_stock(
    ...
    *,
    ...
    # the link's target and miss orphans placed beside the link. The
    # gauge normalises both sides of the comparison, so the spelled
    # spelling still matches the registered set.
    ...
    try:
        sibling_paths = _list_sibling_paths(worktree_parent)
    except OSError:
        sibling_paths = None
