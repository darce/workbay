...
caller spelled it — not the parent of a symlink's target — that belong
to this repository and are not in the registered set. Membership is the
git common directory the candidate actually resolves to on disk, not a
directory-name prefix. The collector applies that discriminator; the
core only tallies the paths it is given. An unrelated repository, a
non-git directory, or an unreadable candidate is not stock, regardless
of naming shape. The volume
...
class WorktreeStock:
    ...
    # Registered lanes which have not consumed local or remote work. They
    # remain in outstanding_unlanded/physical stock status, but do not consume
    # their class-specific admission ceiling (CON-14).
    excluded_never_started_landable: int = 0
    excluded_never_started_record_only: int = 0
...
_PRESTART_PASS_OUTCOMES = frozenset(
    {
        "admission_deferred",
        "admission_refused",
        "disk_headroom_refused",
        "dispatch_refused",
        "remote_required",
        "worktree_claim_held",
...
def _rows_grouped_by_lane(rows):
    grouped = {}
    for row in rows or ():
...
def _pass_state_outcome(pass_state):
    if str(pass_state.get("status") or "").strip().lower() == "running":
        return None
    result = pass_state.get("result")
    if not isinstance(result, dict):
        return None
    return str(result.get("outcome") or "").strip().lower() or None


def _lane_never_started(row, pass_states_by_lane, pass_states_measured):
    """True when durable pass state proves a planned lane consumed no work.

    Worker reports are intentionally not used for this boundary: persistence
    maps raw pass outcomes such as ``admission_deferred`` to ``failed`` or
    ``stopped``. Offload pass-state files retain both the pre-report ``running``
    marker (so a crashed worker counts as started) and raw terminal outcomes.
    """
    # Registration creates planned rows before dispatch. Other states are
    # evidence that the lane crossed the start boundary, including legacy rows.
    if row.get("status") != "planned":
        return False
    if not pass_states_measured:
        return False
    pass_states = pass_states_by_lane.get(_row_key(row), ())
    if not pass_states:
        return True
    outcomes = tuple(_pass_state_outcome(pass_state) for pass_state in pass_states)
    return all(outcome in _PRESTART_PASS_OUTCOMES for outcome in outcomes)
...
def _measure_listing_stock(listing_text, primary_state, normalise, probe_errors):
    ...
    if primary_state == PRIMARY_UNREADABLE:
        ...
        return [], None, None, None, [], False
    try:
        records, registered, primary, linked, registered_paths = _dedupe_listed_records(records, normalise)
...
def _split_outstanding_classes(
    ...
    unknown_kind = any(row.get("lane_kind", "implement") not in ("implement", "review") for row in outstanding_rows)
    if unknown_kind:
        return None, None, (), (), None, None
    pass_states_by_lane = _rows_grouped_by_lane(pass_states)
    never_started = [
        row for row in outstanding_rows if _lane_never_started(row, pass_states_by_lane, pass_states is not None)
    ]
    never_started_keys = {_row_key(row) for row in never_started}
    landable = sum(
        row.get("lane_kind", "implement") == "implement" and _row_key(row) not in never_started_keys
        for row in outstanding_rows
    )
    record_only = sum(
        row.get("lane_kind") == "review" and _row_key(row) not in never_started_keys for row in outstanding_rows
    )
    unmerged_landable = tuple(
        path
        for path, row in zip(outstanding_paths, outstanding_rows)
        if row.get("lane_kind", "implement") == "implement" and _row_key(row) not in never_started_keys
    )
    ...
    return (
        ...
        unmerged_landable,
        sum(row.get("lane_kind", "implement") == "implement" for row in never_started),
        sum(row.get("lane_kind") == "review" for row in never_started),
    )
...
def _measure_outstanding_stock(
    ...
    try:
        ...
        outstanding_rows, outstanding_paths = _outstanding_from_registered(registered_paths, rows_by_path, normalise)
        ...
        (
            ...
            reclaimable,
            unmerged,
            excluded_never_started_landable,
            excluded_never_started_record_only,
        ) = _split_outstanding_classes(
            outstanding_rows,
            outstanding_paths,
            records,
            lane_rows,
...
def _measure_unregistered_stock(listing_ok, sibling_paths, registered_paths, normalise, probe_errors):
    if not listing_ok:
        return None
    ...
    try:
        registered_set = {_call_normaliser(normalise, path) for path in registered_paths}
        return sum(1 for path in sibling_paths if _call_normaliser(normalise, path) not in registered_set)
    except _NormaliseFailed:
        _note_normalise_error(probe_errors)
        return None
...
def measure_worktree_stock(
    ...
    *,
    ...
    probe_errors = []
    records, registered, primary, linked, registered_paths, listing_ok = _measure_listing_stock(
        listing_text, primary_state, normalise, probe_errors
    )
    (
        outstanding,
        landable,
    ...
    unregistered = _measure_unregistered_stock(listing_ok, sibling_paths, registered_paths, normalise, probe_errors)
    free, total = _measure_volume_stock(worktree_parent, registered_paths, normalise, disk_usage, probe_errors)
    ...
    return WorktreeStock(
        ...
        excluded_never_started_landable=excluded_never_started_landable or 0,
        excluded_never_started_record_only=excluded_never_started_record_only or 0,
    )
...
_GIT_METADATA_LIMIT_BYTES = 4096


class _SiblingMetaUnknown(OSError):
    """Typed unknown occupancy outcome; its message is the probe reason."""


def _read_sibling_metadata(path, kind):
    """Bound every metadata read and reject unsafe file types before open."""
    try:
        if not stat.S_ISREG(os.stat(path).st_mode):
            raise _SiblingMetaUnknown(f"{kind}_nonregular")
        with open(path, "rb") as handle:
            raw = handle.read(_GIT_METADATA_LIMIT_BYTES + 1)
    except OSError as exc:
        if isinstance(exc, _SiblingMetaUnknown):
            raise
        raise _SiblingMetaUnknown(f"{kind}_unreadable") from exc
    if len(raw) > _GIT_METADATA_LIMIT_BYTES:
        raise _SiblingMetaUnknown(f"{kind}_oversized")
    return raw


def _gitdir_pointer(git_file):
    raw = _read_sibling_metadata(git_file, "gitdir")
    if not raw.startswith(b"gitdir: "):
        raise _SiblingMetaUnknown("gitdir_malformed")
    raw = raw[8:].rstrip(b"\r\n")
    if b"\x00" in raw:
        raise _SiblingMetaUnknown("gitdir_malformed")
    pointer = os.fsdecode(raw)
    if not pointer:
        raise _SiblingMetaUnknown("gitdir_empty")
    return pointer


def _resolve_gitdir_pointer(git_file, pointer):
    # Preserve symlink/.. components until realpath resolves them in order.
    if os.path.isabs(pointer):
        return pointer
    return os.path.join(os.path.dirname(git_file), pointer)


def _checkout_gitdir(path):
    """None means no .git entry; any present but unreadable entry is unknown."""
    ...
    try:
        ...
        raise _SiblingMetaUnknown("gitdir_unreadable") from exc
    if stat.S_ISLNK(mode):
        # A present broken link is unknown, not an absent checkout.
        try:
            mode = os.stat(git_path).st_mode
        except OSError as exc:
            raise _SiblingMetaUnknown("gitdir_unreadable") from exc
    if stat.S_ISREG(mode):
        pointer = _gitdir_pointer(git_path)
        # A missing private gitdir may be a known pruned leftover. Resolve
        # existing symlinks strictly first so loops cannot masquerade as one.
        target = _resolve_gitdir_pointer(git_path, pointer)
        try:
            return os.path.realpath(target, strict=True)
        except FileNotFoundError:
            return os.path.realpath(target)
        except OSError as exc:
            raise _SiblingMetaUnknown("gitdir_resolution") from exc
    if stat.S_ISDIR(mode):
        return os.path.realpath(git_path, strict=True)
    raise _SiblingMetaUnknown("gitdir_nonregular")


def _validate_common_repository(gitdir, reason):
    """Use the same fail-closed structure check for every common identity."""
    try:
        for entry in ("objects", "refs"):
            if not stat.S_ISDIR(os.stat(os.path.join(gitdir, entry)).st_mode):
                raise _SiblingMetaUnknown(reason)
        _gitdir_has_valid_head(gitdir)
    except OSError as exc:
        raise _SiblingMetaUnknown(reason) from exc


def _git_common_dir_from_gitdir(gitdir):
    """Resolve common identity, preserving filesystem bytes and embedded lines."""
    commondir_path = os.path.join(gitdir, "commondir")
    try:
        os.lstat(commondir_path)
    except FileNotFoundError:
        # A private worktree gitdir without commondir is damaged metadata,
        # unless it independently has the structure of a standalone repository.
        _validate_common_repository(gitdir, "commondir_missing")
        return os.path.realpath(gitdir, strict=True)
    except OSError as exc:
        raise _SiblingMetaUnknown("commondir_unreadable") from exc
    raw = _read_sibling_metadata(commondir_path, "commondir")
    raw = raw.rstrip(b"\r\n")
    if b"\x00" in raw:
        raise _SiblingMetaUnknown("commondir_malformed")
    common = os.fsdecode(raw)
    if not common:
        raise _SiblingMetaUnknown("commondir_empty")
    try:
        resolved = os.path.realpath(os.path.join(gitdir, common), strict=True)
        if not os.path.isdir(resolved):
            raise _SiblingMetaUnknown("commondir_nonregular")
    except OSError as exc:
        raise _SiblingMetaUnknown("commondir_resolution") from exc
    _validate_common_repository(resolved, "commondir_invalid")
    return resolved


def _gitdir_has_valid_head(gitdir):
    """A rejected HEAD is unknown occupancy, never evidence of no checkout."""
    head_path = os.path.join(gitdir, "HEAD")
    try:
        symbolic = stat.S_ISLNK(os.lstat(head_path).st_mode)
        if symbolic:
            target = os.fsencode(os.readlink(head_path))
            if len(target) > _GIT_METADATA_LIMIT_BYTES:
                raise _SiblingMetaUnknown("head_oversized")
        else:
            head = _read_sibling_metadata(head_path, "head").split(b"\x00", 1)[0].rstrip()
            if not head.startswith(b"ref:"):
                if re.fullmatch(rb"(?:[0-9a-fA-F]{40}|[0-9a-fA-F]{64})", head):
                    return True
                raise _SiblingMetaUnknown("head_malformed")
            target = head[4:].strip()
    except OSError as exc:
        if isinstance(exc, _SiblingMetaUnknown):
            raise
        raise _SiblingMetaUnknown("head_unreadable") from exc
    if not target.startswith(b"refs/") or len(target) <= 5 or any(byte <= 32 or byte == 127 for byte in target):
        raise _SiblingMetaUnknown("head_malformed")
    return True
...
def _repo_worktrees_dir(root):
    ...
    A primary checkout has a ``.git`` directory; a linked worktree's
    private gitdir names the common directory through ``commondir``,
    even when the private gitdir has been relocated. Missing ``.git``
    falls back to the lexical primary layout so a
    ...
    try:
        ...
        return None
    if stat.S_ISLNK(mode):
        # Keep target failures out of the missing-.git layout fallback.
        try:
            mode = os.stat(git_path).st_mode
        except OSError:
            return None
    if stat.S_ISREG(mode):
        ...
        common = _git_common_dir_from_gitdir(gitdir)
        return os.path.join(common, "worktrees") if common is not None else None
    if stat.S_ISDIR(mode):
        common = _git_common_dir_from_gitdir(git_path)
        return os.path.join(common, "worktrees") if common is not None else None
    return None


def _is_linked_worktree_of(path, worktrees_dir, common_dir):
    """Return understood affinity or a typed unknown metadata outcome."""
    try:
        return _classify_linked_worktree(path, worktrees_dir, common_dir)
    except _SiblingMetaUnknown as exc:
        return exc
    except (OSError, ValueError, UnicodeError) as exc:
        return _SiblingMetaUnknown(f"metadata_resolution:{type(exc).__name__}")


def _classify_linked_worktree(path, worktrees_dir, common_dir):
    """True when ``path`` belongs to the repository at ``worktrees_dir``.

    Affinity is the git common directory the candidate actually resolves
    to, not a directory-name prefix. A leftover whose gitdir still sits
    in this repository's worktrees admin is also stock, even when the
    private gitdir (and its ``commondir`` file) has been pruned.
    Classification failures propagate to the typed unknown outcome.
    """
    gitdir = _checkout_gitdir(path)
    if gitdir is None:
        return False
    admin = os.path.realpath(worktrees_dir)
    try:
        nested = os.path.commonpath([gitdir, admin])
    except ValueError:
        nested = None
    try:
        os.stat(gitdir)
    except FileNotFoundError:
        # Only pruned private directories qualify for the layout fallback.
        if nested == admin and gitdir != admin:
            return True
        raise _SiblingMetaUnknown("gitdir_missing")
    except OSError as exc:
        raise _SiblingMetaUnknown("gitdir_unreadable") from exc
    candidate_common = _git_common_dir_from_gitdir(gitdir)
    _gitdir_has_valid_head(gitdir)
    return candidate_common == common_dir


def _list_sibling_paths(parent, repo_root, probe_errors):
    """Checkouts of ``repo_root`` among the immediate children of ``parent``.

    The collector root is never its own sibling. Only understood unrelated
    repositories and non-git directories are zero. Every rejected metadata
    outcome makes the whole occupancy unknown.
    """
    ...
    root_gitdir = _checkout_gitdir(repo_root)
    root_key = os.path.realpath(repo_root)
    # The lexical fallback only supports a missing root's pruned leftovers.
    # Never infer repository identity from the resolved admin directory.
    common_dir = (
        _git_common_dir_from_gitdir(root_gitdir)
        if root_gitdir is not None
        else os.path.realpath(os.path.dirname(worktrees_dir))
    )
    if common_dir is None:
        raise OSError("cannot resolve repository common directory")
    # Skip only a proven primary: its gitdir is the common directory.
    # An unregistered linked collector still occupies leftover stock.
    exclude_root = root_gitdir is not None and root_gitdir == common_dir
    found = []
    for name in os.listdir(parent):
        ...
        if not is_dir:
            continue
        try:
            if exclude_root and os.path.realpath(path) == root_key:
                continue
        except OSError:
            pass
        outcome = _is_linked_worktree_of(path, worktrees_dir, common_dir)
        if isinstance(outcome, _SiblingMetaUnknown):
            probe_errors.append(f"sibling_{outcome}:{path}")
            raise outcome
        if outcome:
            found.append(path)
    return sorted(found)
...
def collect_worktree_stock(
    ...
    *,
    ...
    verdict: only directories whose resolved git common directory is
    this repository's (or whose gitdir still sits in this repository's
    worktrees admin, including pruned leftovers) are handed to the core
    as stock. Identity is read from on-disk git metadata; the scan does
    not spawn git per candidate.
    """
    ...
    sibling_errors = []
    try:
        sibling_paths = _list_sibling_paths(worktree_parent, spelled_root, sibling_errors)
    except OSError as exc:
        if not sibling_errors:
            sibling_errors.append(f"sibling_metadata_unknown:{exc}")
        sibling_paths = None

    stock = measure_worktree_stock(
        listing_text,
        lane_rows,
    ...
    return replace(stock, probe_errors=stock.probe_errors + tuple(sibling_errors))
