...
def admission_defer_summary(backend: str | None, defer_reason: str | None) -> str:
    """Build one operator-facing summary for a typed admission deferral.

    The backend and reason are both data supplied by the dispatch path. Keeping
    this formatting here prevents a consumer from silently attributing a
    non-grok backend to a generic VM-memory diagnosis.
    """
    backend_name = "unknown-backend"
    if isinstance(backend, str) and backend.strip():
        backend_name = " ".join(backend.split())
    reason_name = "unknown"
    if isinstance(defer_reason, str) and defer_reason.strip():
        reason_name = " ".join(defer_reason.split())
    return f"{backend_name} turn deferred by VM admission ({reason_name}; retryable)"
