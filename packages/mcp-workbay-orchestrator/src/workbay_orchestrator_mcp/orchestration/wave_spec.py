...
from workbay_orchestrator_mcp.orchestration.offload_timeout_ssot import (
    ...
    LANE_TIMEOUT_MARGIN_SECONDS,
)

# Margin subtracted from a backend's declared wall-clock cap so the join still
# has slack after the worker's own deadline. Owned by offload_timeout_ssot.
TIMEOUT_MARGIN_SECONDS = LANE_TIMEOUT_MARGIN_SECONDS
DEFAULT_TIMEOUT_SECONDS = DEFAULT_LANE_TIMEOUT_SECONDS
...
def _derive_quad(row: Mapping[str, Any], entry: Mapping[str, Any], lane_kind: str):
    ...
    from workbay_orchestrator_mcp.orchestration.lane_routing import resolve_routing_quad  # noqa: PLC0415

    profile_backend = _text(row.get("backend")) or _text(entry.get("preferred_backend"))
    profile_model = _text(row.get("model")) or _text(entry.get("preferred_model"))
    profile = {
        "backend": profile_backend,
        "model": profile_model,
        "reasoning_effort": default_effort_for_model(profile_backend, profile_model),
    }
    # Historical DB rows may contain empty strings for omitted columns. The
    # write surfaces normalize explicit clears to NULL; treat legacy blanks as
    # absence rather than manufacturing a disagreement during rollout.
    normalized_row = {key: value for key, value in row.items() if _text(value)}
    return resolve_routing_quad(
        row=normalized_row,
...
def _spec_for_lane(
    *,
    ...
    # Materialization can persist an explicit unavailable-write-set note.  Wave
    # dispatch does not pass through the single-lane ensure helper, so consume the
    # same fail-closed scope predicate here rather than treating [] as unscoped.
    from workbay_orchestrator_mcp.orchestration.offload_preflight import (  # noqa: PLC0415
        lane_scope_dispatch_error,
    )

    scope_error = lane_scope_dispatch_error(lane_id, dict(entry))
    if scope_error is not None:
        return WaveSpecRefusal(lane_id, "unknown_scope", scope_error)
    ...
    lane_kind = derive_stock_lane_kind(lane_row=row)
    if lane_kind not in ("implement", "review"):
        return WaveSpecRefusal(
            lane_id,
            "routing_disagreement",
            f"lane {lane_id!r} has invalid lane_kind {lane_kind!r}",
        )

    from workbay_orchestrator_mcp.orchestration.lane_routing import RoutingDisagreement  # noqa: PLC0415

    try:
        quad = _derive_quad(row, entry, lane_kind)
    except RoutingDisagreement as exc:
        kind = (
            "backend_mismatch"
            if exc.disagreements and {item[0] for item in exc.disagreements} == {"backend"}
            else "routing_disagreement"
        )
        return WaveSpecRefusal(lane_id, kind, f"{exc.reason}: {exc}")
    backend = quad.backend or ""
    if not backend:
        return WaveSpecRefusal(
            lane_id,
            "backend_mismatch",
