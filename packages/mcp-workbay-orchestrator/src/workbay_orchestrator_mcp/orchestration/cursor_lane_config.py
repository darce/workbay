...
from workbay_orchestrator_mcp.orchestration import offload_timeout_ssot

# Wall-clock ceiling for one cursor cycle. The SSOT module owns the sole env
# resolution site; production consumers share its import-time snapshot.
CURSOR_TIMEOUT_CAP_DEFAULT = offload_timeout_ssot.CURSOR_TIMEOUT_CAP_DEFAULT
CURSOR_TIMEOUT_CAP = offload_timeout_ssot.CURSOR_TIMEOUT_CAP
...
# Declared default for a cursor-remote turn when no caller timeout is supplied.
# Distinct from CURSOR_TIMEOUT_CAP (the env-driven ceiling). If the cap
# changed tomorrow, this default should not follow (REF-10).
CURSOR_REMOTE_LANE_TIMEOUT_S = 900
