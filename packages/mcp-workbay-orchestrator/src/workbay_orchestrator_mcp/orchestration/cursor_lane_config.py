...
#
# Hand-authored from coordinator-verified published slugs (2026-08-17 VM
# catalogue). This table is the effort-to-slug vocabulary ONLY — it is not
# ``cursor-agent --list-models``, not the dispatch allow-list, and must never
# be derived from a remote probe (implementation note S4/S5; [SECD-05]).
CURSOR_EFFORT_SLUGS: dict[str, dict[str, str]] = {
    ...
    # Vendor publishes low|high|max only — no medium. Map orchestrator xhigh → max.
    "kimi-k3": {
        "low": "kimi-k3-low",
        "high": "kimi-k3-high",
        "xhigh": "kimi-k3-max",
...
# Curated dispatch allow-list for cursor-remote (implementation note S4 Option A).
# Hand-authored constant — NEVER derive from ``cursor-agent --list-models``,
# a remote probe, a network call, or any VM-fetched catalogue. The VM publishes
# 200+ slugs; coupling the allow-list to that catalogue would legalize every
# frontier model and collapse deny-by-default polarity ([SECD-05]).
# ``BackendSpec.allowed_model`` / ``DEFAULT_CURSOR_MODEL`` remains the default
# when the caller omits a model; membership here gates caller-supplied slugs.
