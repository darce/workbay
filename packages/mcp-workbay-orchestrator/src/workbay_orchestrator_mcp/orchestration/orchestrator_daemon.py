...
def _reconcile_unmerged_unknown_streaks(
    ...
    *,
    ...
    rf_any = read_failure_any_this_cycle if read_failure_any_this_cycle is not None else read_failed_this_cycle
    ...
    *,
    ...
    if callable(log):
        log(
            ...
            preserved=(count is None or count <= UNMERGED_UNKNOWN_READ_FAILURE_TOLERANCE),
    ...
    try:
        for lane_id in lane_ids:
            ...
            try:
                ...
                if callable(log):
                    reason = "reader_type_error" if isinstance(exc, TypeError) else f"reader_{type(exc).__name__}"
                    try:
                        log(
                            "ERROR",
            ...
            if reports and isinstance(reports[0], dict) and reports[0].get("merge_ready"):
                ...
                try:
                    ...
                    if callable(log):
                        reason = (
                            "predicate_type_error" if isinstance(exc, TypeError) else f"predicate_{type(exc).__name__}"
                        )
                        try:
                            log(
                                "ERROR",
...
def _run_cross_lane_verify(
    ...
    *,
    ...
    The bool stays True only for ``LANE_GATE_STATUS=passed``. Make's process
    exit is not a discriminator (every non-zero recipe becomes exit 2). A
    hang becomes False via TimeoutExpired, not an unbounded wait.
    """
    if dry_run:
        return True
...
def daemon_status(state_dir: Path, log_dir: Path) -> dict[str, Any]:
    ...
    heartbeat_path = log_dir / "reap_heartbeat.json"
    if heartbeat_path.exists():
        try:
            heartbeat = json.loads(heartbeat_path.read_text(encoding="utf-8"))
            if isinstance(heartbeat, dict):
                heartbeat_ts = _parse_utc_timestamp(heartbeat.get("ts"))
                if heartbeat_ts is not None:
                    heartbeat_age = max(0.0, time.time() - heartbeat_ts)
                heartbeat_degraded = bool(heartbeat.get("degraded"))
                if heartbeat.get("ok") is False:
                    heartbeat_degraded = True
                try:
                    probe_failures = heartbeat.get("probe_failures")
                    if not isinstance(probe_failures, bool) and int(probe_failures or 0) > 0:
                        heartbeat_degraded = True
    ...
    age_stale = heartbeat_age is None or heartbeat_age > 2 * DEFAULT_REAP_INTERVAL_SECONDS
    return {
        "mode": "singleton",
        ...
        "reaper_heartbeat_age_seconds": heartbeat_age,
        "reaper_stale": age_stale or heartbeat_degraded,
        "reaper_degraded": heartbeat_degraded,
    }
...
def _dormancy_deadline_holds_for_operator(status: dict[str, Any]) -> bool:
    """True when a stopped worker is waiting for operator triage after a dormancy deadline.

    The worker status file writes ``state="dormancy_deadline_exceeded"`` with
    ``attention_required=True`` and a summary that the lane requires triage.
    ``attention_required`` is otherwise the auto-close signal, so this state is
    an explicit exemption: hold for the operator, do not salvage-and-close.
    """
    if status.get("worker_state") == "dormancy_deadline_exceeded":
        return True
    status_record = status.get("status_record")
    return isinstance(status_record, dict) and status_record.get("state") == "dormancy_deadline_exceeded"
...
def _check_lane_health(status: dict[str, Any]) -> tuple[str, str | None]:
    ...
    ``state="dormancy_deadline_exceeded"`` and ``state="orphaned"`` are
    hold-for-operator exemptions: ``attention_required`` does not map them
    to ``close_lane``. An orphan is a stability event that must be triaged,
    not converted into a successful termination.
    """
    ...
    if _dormancy_deadline_holds_for_operator(status):
        return "unhealthy", "hold"

    if worker_state == "orphaned" or status_record.get("state") == "orphaned":
        return "unhealthy", "hold"
...
def _ensure_lane_workers(
    ...
    *,
    ...
    remote_dispatch_refused: bool = False,
) -> list[dict[str, Any]]:
    ...
    for lane_id in ordered:
        ...
        if worker_start_mode == "mcp" and not dry_run:
            ...
            if lane_backend == "codex-remote":
                disk_floor = (
                    {"ok": False, "refused_reason": "remote_disk_floor_refused"}
                    if remote_dispatch_refused
                    else _remote_worker_disk_admission(orchestrator_root, log=log)
                )
                if disk_floor.get("ok") is not True:
                    status_payload.update(
                        started=False,
                        skipped=True,
                        reason=disk_floor.get("refused_reason") or "remote_disk_floor_refused",
                        remote_dispatch_refused=True,
                    )
                    rows.append(status_payload)
                    continue
...
def _remote_worker_disk_admission(root: Path, *, log: Any = None) -> dict[str, Any]:
    """Fail closed on remote disk ambiguity without blocking local workers."""
    from workbay_orchestrator_mcp.orchestration.lane_census import (  # noqa: PLC0415
        REMOTE_DISK_FLOOR_MB,
        remote_disk_floor_check,
    )

    try:
        floor_mb = max(0, int(os.environ.get("WORKBAY_REMOTE_DISK_FLOOR_MB", str(REMOTE_DISK_FLOOR_MB))))
    except (TypeError, ValueError):
        floor_mb = REMOTE_DISK_FLOOR_MB
    try:
        disk_floor = remote_disk_floor_check(root=root, floor_mb=floor_mb, log=log)
    except Exception as exc:  # noqa: BLE001 - an ambiguous floor refuses only remote starts
        disk_floor = {
            "ok": False,
            "free_bytes": None,
            "floor_bytes": floor_mb * 1024 * 1024,
            "reaped": 0,
            "refused_reason": "remote_disk_floor_exception",
            "error": str(exc)[:REAP_ERROR_CAP],
        }
    if disk_floor.get("ok") is not True and log is not None:
        log("WARN", "remote_disk_floor_refused", **disk_floor)
    return disk_floor


def _dispatch_phase(ctx: OrchestratorContext) -> None:
    """Step 1: Dispatch open issues to lanes and emit ACE advisory if needed."""
    ctx.remote_dispatch_refused = False
    if ctx.backend == "codex-remote" and not ctx.dry_run:
        disk_floor = _remote_worker_disk_admission(ctx.orchestrator_root, log=ctx.log)
        if disk_floor.get("ok") is not True:
            ctx.remote_dispatch_refused = True
            return
    try:
...
def _remote_sandbox_reap_maintenance(ctx: OrchestratorContext) -> None:
    """Run and report the shared cadence arm without endangering the cycle."""
    from workbay_orchestrator_mcp.orchestration.lane_census import (  # noqa: PLC0415
    ...
    try:
        ctx.log("INFO", "remote_sandbox_reap_maintenance", **result)
    except Exception:  # noqa: BLE001 - observability cannot take down maintenance
        pass
...
DEFAULT_REAP_DROUGHT_SECONDS = 3600.0
DEFAULT_REAP_INTERVAL_SECONDS = 60.0
# Cross-session preserved-lane reaper is git-heavy; run every N reap ticks
# (wall-clock ~ DEFAULT_REAP_INTERVAL_SECONDS * N when poll matches the reap interval).
DEFAULT_CROSS_SESSION_REAP_EVERY_N = 10
DEFAULT_CROSS_SESSION_REAP_APPLY = False


def _parse_utc_timestamp(value: object) -> float | None:
    """Return an epoch timestamp for handoff/heartbeat timestamp values."""
...
def _reaper_stock_telemetry(ctx: OrchestratorContext) -> dict[str, object]:
    """Measure merge-ready stock and recent landing flow from handoff rows."""
    ...
    now = time.time()
    ready_lane_ids = list(dict.fromkeys(str(item) for item in ctx.ordered_ready))
    ...
    for lane_id in ready_lane_ids:
        payload = _require_dict_payload(
            ...
            source=f"worker_reports(reaper stock:{lane_id})",
        )
        if payload.get("ok") is not True:
            raise RuntimeError(f"worker report stock read failed for {lane_id}")
        reports = payload.get("reports") or []
        if not isinstance(reports, list) or not reports or not isinstance(reports[0], dict):
            continue
        created_at = _parse_utc_timestamp(reports[0].get("created_at"))
        if created_at is not None and (oldest_created_at is None or created_at < oldest_created_at):
            oldest_created_at = created_at

    landing_envelope = _require_dict_payload(
        get_handoff_state(
            task_ref=ctx.task_ref,
            sections="decisions_recent",
            top_n_decisions=10_000,
            verbose=True,
            detail="summary",
            decision_fields=["decision", "created_at"],
            decision_id_prefix=f"lane_landed_{ctx.task_ref}_",
        ),
        source="get_handoff_state(reaper landings)",
    )
    if landing_envelope.get("ok") is not True:
        raise RuntimeError("landing stock read failed")
    landing_data = landing_envelope.get("data")
    landing_data = landing_data if isinstance(landing_data, dict) else {}
    decisions = landing_data.get("decisions_recent") or []
    landings_in_last_hour = 0
    if isinstance(decisions, list):
        for decision in decisions:
            if not isinstance(decision, dict):
                continue
            created_at = _parse_utc_timestamp(decision.get("created_at"))
            if created_at is not None and 0 <= now - created_at <= 3600:
                landings_in_last_hour += 1

    return {
        "merge_ready_branch_count": len(ready_lane_ids),
        "age_seconds_of_oldest_merge_ready_branch": (
            max(0.0, now - oldest_created_at) if oldest_created_at is not None else None
        ),
        "landings_in_last_hour": landings_in_last_hour,
    }


def _reap_drought_seconds() -> float:
    try:
        return max(0.0, float(os.environ.get("WORKBAY_REAP_DROUGHT_SECONDS", DEFAULT_REAP_DROUGHT_SECONDS)))
    except (TypeError, ValueError):
        return DEFAULT_REAP_DROUGHT_SECONDS


def _existing_reap_heartbeat_degraded(heartbeat_path: Path) -> bool:
    """Read the shared heartbeat's degraded marker; missing/invalid is not degraded."""
    try:
        raw = json.loads(heartbeat_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError, UnicodeError):
        return False
    return isinstance(raw, dict) and bool(raw.get("degraded"))


def _write_reap_heartbeat(
    ctx: OrchestratorContext,
    *,
    pass_id: str,
    closed: int,
    reclaimed: int,
    merge_ready_branch_count: int,
    degraded: bool | None = None,
) -> None:
    """Atomically publish a receipt for one completed reaper pass.

    Shared producer contract (REF-26 / CARD-07 / OBS-08):
    - Always refreshes freshness (``ts``).
    - ``degraded=True`` records a degraded pass.
    - ``degraded=False`` is a successful producer and clears a prior marker.
    - ``degraded=None`` is a freshness-only refresh and preserves an existing
      degraded marker. The blocked/orphan arm uses this so an off-cadence
      cycle cannot erase a failed cross-session pass.
    """
    log_dir = Path(ctx.orchestrator_root) / "logs" / "daemon"
    log_dir.mkdir(parents=True, exist_ok=True)
    heartbeat_path = log_dir / "reap_heartbeat.json"
    if degraded is None:
        write_degraded = _existing_reap_heartbeat_degraded(heartbeat_path)
    else:
        write_degraded = bool(degraded)
    receipt = {
        "ts": time.time(),
        "pid": os.getpid(),
        "pass_id": pass_id,
        "closed": closed,
        "reclaimed": reclaimed,
        "merge_ready_branch_count": merge_ready_branch_count,
    }
    if write_degraded:
...
def _cross_session_reap_every_n() -> int:
    raw = os.environ.get("WORKBAY_CROSS_SESSION_REAP_EVERY_N")
    if raw is None or not str(raw).strip():
        n = DEFAULT_CROSS_SESSION_REAP_EVERY_N
    else:
        try:
            n = int(str(raw).strip())
        except (TypeError, ValueError):
...
def _cross_session_reap_apply(*, dry_run: bool) -> bool:
    """Daemon reap policy for cross-session reclaim (TEST-15: default False)."""
    if dry_run:
        return False
    raw = os.environ.get("WORKBAY_CROSS_SESSION_REAP_APPLY")
    if raw is not None and str(raw).strip() != "":
        return str(raw).strip().lower() in {"1", "true", "yes", "on"}
    return bool(DEFAULT_CROSS_SESSION_REAP_APPLY)


def _cross_session_lane_reap_due(ctx: OrchestratorContext) -> bool:
    n = _cross_session_reap_every_n()
    tick = int(getattr(ctx, "cross_session_reap_tick", 0) or 0) + 1
    ctx.cross_session_reap_tick = tick
    return tick % n == 0


def _in_progress_task_refs(ctx: OrchestratorContext) -> list[str]:
    task = str(getattr(ctx, "task_ref", "") or "").strip()
    return [task] if task else []


def _cross_session_unintegrated_gauges(result: dict) -> dict[str, object]:
    """Operator scheduling gauges: unintegrated backlog count and oldest age."""
    ub = result.get("unintegrated_backlog")
    backlog = result.get("backlog") if isinstance(result.get("backlog"), list) else []
    oldest_lane: object = None
    if isinstance(ub, dict):
        count_raw = ub.get("count", len(backlog))
        oldest_raw = ub.get("oldest_age_seconds")
        oldest_lane = ub.get("oldest_lane_id")
    else:
        count_raw = len(backlog)
        oldest_raw = None
        for item in backlog:
            ...
            if isinstance(age, bool) or not isinstance(age, (int, float)):
                continue
            if oldest_raw is None or age > oldest_raw:
                oldest_raw = age
    ...
    try:
        ...
        count = 0
    if isinstance(oldest_raw, bool) or not isinstance(oldest_raw, (int, float)):
        oldest: int | None = None
    else:
        oldest = max(0, int(oldest_raw))
    return {
        "unintegrated_backlog": count,
        "oldest_unintegrated_age_seconds": oldest,
        "oldest_unintegrated_lane_id": (
            _reap_cap_error_text(oldest_lane) if oldest_lane not in (None, "") else None
        ),
    }


def _cross_session_capped_list(value: object) -> object:
    items = value if isinstance(value, list) else []
    return _reap_cap_value(items)


def _cross_session_nonneg_int(value: object) -> int:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        return 0
    return max(0, int(value))


def _cross_session_report_health(result: dict) -> dict[str, object]:
    """Carry ok / probe_failures / scan fields the shared telemetry helper drops."""
    raw_ok = result.get("ok")
    probe_failures = _cross_session_nonneg_int(result.get("probe_failures"))
    rows_scanned = _cross_session_nonneg_int(result.get("rows_scanned"))
    error = result.get("error")
    if isinstance(raw_ok, bool):
        ok = raw_ok
    else:
        ok = probe_failures == 0 and not error
    degraded = (ok is False) or probe_failures > 0 or bool(error)
    return {
        "ok": ok,
        ...
        "degraded": degraded,
        "error": _reap_cap_error_text(error) if error else None,
    }


def _cross_session_lane_reap_maintenance(ctx: OrchestratorContext) -> None:
    """Reap preserved, clean, unowned lanes of in-progress tasks (RES-07).

    Runs every ``DEFAULT_CROSS_SESSION_REAP_EVERY_N`` ticks (default 10) because
    the probe is git-heavy. ``apply`` is a dedicated policy flag defaulting to
    False (TEST-15); a dry-run daemon never reclaims. Failures are logged and
    swallowed so this arm cannot take down a cycle (OBS-08). List payloads are
    sliced to ``REAP_LIST_CAP`` (OBS-06). Gauges ``unintegrated_backlog`` and
    ``oldest_unintegrated_age_seconds`` are the scheduling metrics the operator
    asked to track. A report with ``ok=False``, ``probe_failures > 0``, or an
    error is a capped WARN with ``degraded=True`` before the heartbeat write
    (CARD-07 / OBS-08); ``daemon_status`` must not treat that pass as a healthy
    fresh heartbeat. Integration refs are resolved at this boundary via the
    reaper's shared resolver and passed explicitly (REF-26 / RES-07).
    """
    try:
        due = _cross_session_lane_reap_due(ctx)
    except Exception:  # noqa: BLE001 — cadence bookkeeping cannot break the cycle
        return
    if not due:
        return
    ...
    try:
        ...
        try:
            ctx.log(
                "WARN",
                "cross_session_lane_reap_failed",
                degraded=True,
    ...
    for task_ref in _in_progress_task_refs(ctx):
        try:
            _assert_no_reaper_write_lock(reason="cross_session_lane_reap")
            try:
                integration_refs = _resolve_integration_refs(
                    task_ref,
                    integration_refs=None,
                    repo=Path(ctx.orchestrator_root),
                    orchestrator_root=ctx.orchestrator_root,
                )
            except Exception as resolve_exc:  # noqa: BLE001 — never fall back to silent main
                pass_degraded = True
                try:
                    ctx.log(
                        "WARN",
                        "cross_session_lane_reap_degraded",
                        degraded=True,
                        error=_reap_cap_error_text(resolve_exc),
                        task_ref=task_ref,
                    )
                except Exception:  # noqa: BLE001 — outer log failure must not escape
                    pass
                continue
            result = reap_preserved_lanes_cross_session(
                task_ref=task_ref,
                root=Path(ctx.orchestrator_root),
                apply=apply,
                orchestrator_root=ctx.orchestrator_root,
        ...
        if not isinstance(result, dict):
            ...
            try:
                ctx.log(
                    "WARN",
                    "cross_session_lane_reap_bad_shape",
                    degraded=True,
                ...
                pass
            continue
        reclaimed_raw = result.get("reclaimed") or []
        reclaimed_n = len(reclaimed_raw) if isinstance(reclaimed_raw, list) else 0
        reclaimed_total += reclaimed_n
        closed_total += reclaimed_n
        try:
            gauges = _cross_session_unintegrated_gauges(result)
            fields = _reap_result_telemetry_fields(result)
            health = _cross_session_report_health(result)
            degraded = bool(health["degraded"])
            if degraded:
                pass_degraded = True
            # Overwrite count keys with OBS-06 capped lists in one mapping so
            # ctx.log does not see duplicate would_reclaim kwargs.
            log_fields: dict[str, object] = {
                **fields,
                **gauges,
                **health,
                "would_reclaim": _cross_session_capped_list(result.get("would_reclaim")),
                "backlog": _cross_session_capped_list(result.get("backlog")),
                "kept": _cross_session_capped_list(result.get("kept")),
                "would_reclaim_count": fields.get("would_reclaim"),
                "task_ref": task_ref,
            }
            if degraded:
                ctx.log("WARN", "cross_session_lane_reap_degraded", **log_fields)
            ctx.log("INFO" if not degraded else "WARN", "cross_session_lane_reap", **log_fields)
    ...
    try:
        _write_reap_heartbeat(
            ctx,
            pass_id=pass_id,
            closed=closed_total,
            reclaimed=reclaimed_total,
            merge_ready_branch_count=len(list(getattr(ctx, "ordered_ready", []) or [])),
            degraded=pass_degraded,
        )
    except Exception as exc:  # noqa: BLE001 — receipt failure is loud but never fatal
        try:
            ctx.log(
                "WARNING",
                "reaper_heartbeat_write_failed",
                error=_reap_cap_error_text(exc),
                pass_id=pass_id,
            )
        except Exception:  # noqa: BLE001 — observability cannot break maintenance
            pass
...
def _reap_result_telemetry_fields(result: dict) -> dict[str, object]:
    ...
    rename keys here. Count keys: closed/would_close/reported/ambiguous/alive/
    triage/failed/reclaimed/would_reclaim.
    ...
    reclaimed = result.get("reclaimed") or []
    would_reclaim = result.get("would_reclaim") or []
    ...
    closed_lane_ids: list[str] = [_reap_cap_error_text(lid) for lid in closed_lane_ids_raw[:REAP_LIST_CAP]]
    ...
    return {
        ...
        "would_reclaim": len(would_reclaim) if isinstance(would_reclaim, list) else 0,
        "truncated": bool(result.get("truncated")),
        ...
        "error": error,
        "merge_ready_branch_count": max(0, int(result.get("merge_ready_branch_count") or 0)),
        "age_seconds_of_oldest_merge_ready_branch": result.get("age_seconds_of_oldest_merge_ready_branch"),
        "landings_in_last_hour": max(0, int(result.get("landings_in_last_hour") or 0)),
    }
...
def _reap_cap_value(
    ...
    *,
                ...
                if capped_key in out:
                    ...
                    while True:
                        ...
                        if candidate not in out:
                            ...
                            break
                        n += 1
                out[capped_key] = _reap_cap_value(val, limit=limit, depth=depth + 1, _seen=_seen)
            if omitted:
    ...
    if isinstance(value, list):
        ...
        try:
            capped_list = [
                _reap_cap_value(item, limit=limit, depth=depth + 1, _seen=_seen) for item in value[:REAP_LIST_CAP]
            ]
            omitted = max(0, len(value) - REAP_LIST_CAP)
            if omitted:
...
def _reap_blocked_lanes_maintenance(ctx: OrchestratorContext) -> None:
    ...
    Worktree reclamation is operator-opt-in. This maintenance arm does **not**
    pass ``reclaim_worktrees=True``; ``reap_blocked_lanes`` defaults that flag
    to False so the 60s daemon tick cannot delete worktrees. Operators reclaim
    via CLI/make ``lane-reap --apply --reclaim-worktrees``, which passes
    ``reclaim_worktrees=True`` explicitly.
    """
    pass_id = str(uuid.uuid4())
    closed_total = 0
    reclaimed_total = 0
    stock_fields: dict[str, object] = {
        "merge_ready_branch_count": len(set(str(item) for item in ctx.ordered_ready)),
        "age_seconds_of_oldest_merge_ready_branch": None,
        "landings_in_last_hour": 0,
    }
    stock_read_ok = False
    try:
        stock_fields = _reaper_stock_telemetry(ctx)
        stock_read_ok = True
    except Exception as exc:  # noqa: BLE001 — telemetry cannot break reaping
        try:
            ctx.log("WARNING", "reaper_stock_telemetry_failed", error=_reap_cap_error_text(exc))
        except Exception:  # noqa: BLE001 — observability cannot break reaping
            pass
    ...
    try:
        ...
        if isinstance(result, dict):
            closed_raw = result.get("closed") or []
            reclaimed_raw = result.get("reclaimed") or []
            closed_total += len(closed_raw) if isinstance(closed_raw, list) else 0
            reclaimed_total += len(reclaimed_raw) if isinstance(reclaimed_raw, list) else 0
            ...
            try:
                fields = _reap_result_telemetry_fields({**result, **stock_fields})
    ...
    try:
        ...
        if isinstance(orphan_result, dict):
            orphan_closed_raw = orphan_result.get("closed") or []
            orphan_reclaimed_raw = orphan_result.get("reclaimed") or []
            closed_total += len(orphan_closed_raw) if isinstance(orphan_closed_raw, list) else 0
            reclaimed_total += len(orphan_reclaimed_raw) if isinstance(orphan_reclaimed_raw, list) else 0
            try:
                o_fields = _reap_result_telemetry_fields({**orphan_result, **stock_fields})
    ...
    # Arm 3 — bounded remote-exec staging reaper (Release It! ch-5.4 Steady State).
    # The reaper must be part of the system, not merely available to it. Writes
    # are gated with apply=not ctx.dry_run (same as arms 1 and 2): a dry-run
    # cycle logs would-remove/would-cull counts and mutates nothing. Failures
    # are logged and swallowed; this arm must never break the cycle.
    try:
        from workbay_orchestrator_mcp.orchestration import (  # noqa: PLC0415
            remote_exec_staging_reaper,
        )

        staging_result = remote_exec_staging_reaper.reap_remote_exec_staging(
            staging_root=Path(ctx.state_dir).resolve(),
            git_repo=Path(ctx.orchestrator_root).resolve(),
            apply=not ctx.dry_run,
        )
        try:
            ctx.log(
                "INFO",
                "remote_exec_staging_reap",
                removed=len(tuple(getattr(staging_result, "removed", ()) or ())),
                skipped=len(tuple(getattr(staging_result, "skipped", ()) or ())),
                truncated_debug=len(tuple(getattr(staging_result, "truncated_debug", ()) or ())),
                dry_run=ctx.dry_run,
            )
        except Exception:  # noqa: BLE001 — telemetry must not fail the arm
            pass
    except Exception as exc:  # noqa: BLE001 — maintenance must never break the cycle
        try:
            ctx.log(
                "WARN",
                "remote_exec_staging_reap_failed",
                error=_reap_cap_error_text(exc),
            )
        except Exception:  # noqa: BLE001 — outer log failure must not escape maintenance
            pass

    oldest_age = stock_fields.get("age_seconds_of_oldest_merge_ready_branch")
    if (
        stock_read_ok
        and isinstance(oldest_age, (int, float))
        and not isinstance(oldest_age, bool)
        and float(oldest_age) > _reap_drought_seconds()
        and stock_fields.get("landings_in_last_hour") == 0
    ):
        try:
            ctx.log("WARNING", "reaper_landing_drought", **stock_fields)
        except Exception:  # noqa: BLE001 — symptom logging cannot break reaping
            pass
    try:
        _write_reap_heartbeat(
            ctx,
            ...
            closed=closed_total,
            reclaimed=reclaimed_total,
            merge_ready_branch_count=int(stock_fields.get("merge_ready_branch_count") or 0),
            # degraded omitted: freshness-only refresh must not clear a prior
            # cross-session degraded marker (REAPWIRE06RV-001).
        )
    except Exception as exc:  # noqa: BLE001 — receipt failure is loud but never fatal
        try:
            ctx.log("WARNING", "reaper_heartbeat_write_failed", error=_reap_cap_error_text(exc), pass_id=pass_id)
        except Exception:  # noqa: BLE001 — observability cannot break maintenance
            pass


def _containment_failure_reason(contained: bool | str | None) -> str:
    if contained is None:
        return "absence_of_evidence"
    if contained == "timeout":
        return "containment_probe_timeout"
    return "commits_outside_candidate"


_PRE_LANDING_TIP_NOTE_PREFIX = "[workbay:pre_landing_tip="
_LANDING_RECOVERY_REFUSAL_PREFIX = "landing recovery refused: "


def _pre_landing_tip_from_notes(notes: Any) -> str | None:
    if not isinstance(notes, str):
        return None
    for part in reversed(notes.splitlines()):
        stripped = part.strip()
        if stripped.startswith(_PRE_LANDING_TIP_NOTE_PREFIX) and stripped.endswith("]"):
            sha = stripped[len(_PRE_LANDING_TIP_NOTE_PREFIX) : -1].strip()
            if _is_full_commit_sha(sha):
                return sha
    return None


def _persist_pre_landing_tip(
    ctx: OrchestratorContext,
    lane_id: str,
    target_tip: str,
) -> bool:
    """Store the historical target tip on the lane before its landing recipe."""
    if not _is_full_commit_sha(target_tip):
        return False
    from workbay_orchestrator_mcp.lanes import manage_worktree_lane  # noqa: PLC0415

    try:
        raw = manage_worktree_lane(
            operation="list",
            task_ref=ctx.task_ref,
            status="all",
            limit=500,
        ...
        if isinstance(raw, str):
            raw = json.loads(raw)
        rows = raw.get("lanes") if isinstance(raw, dict) and raw.get("ok") is True else None
        row = next(
            (item for item in rows or [] if isinstance(item, dict) and str(item.get("lane_id")) == lane_id),
            None,
        )
        if not isinstance(row, dict):
            return False
        ...
        kept_lines = [
            ...
            if not line.strip().startswith(_PRE_LANDING_TIP_NOTE_PREFIX)
        ]
        kept_lines.append(f"{_PRE_LANDING_TIP_NOTE_PREFIX}{target_tip}]")
        updated = manage_worktree_lane(
            operation="upsert",
            lane_id=lane_id,
        ...
        return isinstance(updated, dict) and updated.get("ok") is True
    except Exception as exc:  # noqa: BLE001 — persistence failure is typed below
        ctx.log("ERROR", "pre_landing_tip_persist_failed", lane=lane_id, error=str(exc))
        return False
...
def _recover_stranded_landing_lanes(ctx: OrchestratorContext) -> list[str]:
    ...
    Recovery runs after landing and does not retain the target tip from before
    that landing.  A lane branch equal to the candidate tip is therefore
    ambiguous (it may merely have inherited the task tip), so both sub-arms
    deliberately fail closed with ``landing_recovery_lane_not_contained`` and
    ``reason=absence_of_evidence``.  The lane and its branch remain untouched
    for operator inspection; recovery must never manufacture authorship proof.
    ...
    pre_landing_tip_by_lane: dict[str, str] = {}
    row_by_lane: dict[str, dict[str, Any]] = {}
    for row in listed.get("lanes") or []:
        if isinstance(row, dict) and row.get("lane_id") is not None:
            ...
            pre_landing_tip = _pre_landing_tip_from_notes(row.get("notes"))
            if pre_landing_tip is not None:
                pre_landing_tip_by_lane[lid] = pre_landing_tip
    ...
    def _record_refusal(lane_id: str, reason: str) -> bool:
        """Persist one operator-facing refusal note, preserving lane metadata."""
        row = row_by_lane.get(lane_id)
        if not isinstance(row, dict):
            return False
        existing_notes = str(row.get("notes") or "").strip()
        note = f"{_LANDING_RECOVERY_REFUSAL_PREFIX}{reason}; operator must confirm authorship"
        if any(line.strip().startswith(_LANDING_RECOVERY_REFUSAL_PREFIX) for line in existing_notes.splitlines()):
            return True
        ...
        if isinstance(updated, str):
            updated = json.loads(updated)
        if not isinstance(updated, dict) or updated.get("ok") is not True:
            return False
        row["notes"] = updated_notes
        return True

    def _refresh_after_recovery(lane_id: str) -> None:
        deps = downstream_lanes(ctx.task_ref, lane_id, orchestrator_root=str(ctx.orchestrator_root))
        if deps:
    ...
    for lane_id in ctx.m_order:
        try:
            ...
            if status == merged_token or status == "merged":
                continue
            existing_notes = str(row_by_lane.get(lane_id, {}).get("notes") or "")
            if any(
                line.strip().startswith(_LANDING_RECOVERY_REFUSAL_PREFIX)
                for line in existing_notes.splitlines()
            ):
                continue
            ...
            if isinstance(landing, dict):
                ...
                if sha and _is_full_commit_sha(sha):
                    landing_is_ancestor = _git_is_ancestor(ctx.orchestrator_root, sha, tip)
                    if landing_is_ancestor == "timeout":
                        ctx.log(
                            "INFO",
                        ...
                        continue
                    if landing_is_ancestor is not True:
                        ctx.log(
                            "INFO",
                    ...
                    contained = _lane_branch_contained_in(
                        ...
                        task_ref=ctx.task_ref,
                        lane_id=lane_id,
                        allow_candidate_tip=True,
                        target_tip_before_landing=pre_landing_tip_by_lane.get(lane_id),
                    )
                    if contained is not True:
                        refusal_reason = _containment_failure_reason(contained)
                        ctx.log(
                            "WARNING",
                            "landing_recovery_lane_not_contained",
                            lane=lane_id,
                        ...
                        _record_refusal(lane_id, refusal_reason)
                        continue
...
def _worker_management_phase(ctx: OrchestratorContext) -> None:
    ...
    ctx.autostart_results = _ensure_lane_workers(
        ...
        state_dir=ctx.state_dir,
        remote_dispatch_refused=getattr(ctx, "remote_dispatch_refused", False),
    )
    disk_deferred = getattr(ctx, "remote_dispatch_refused", False) or any(
        isinstance(row, dict) and row.get("remote_dispatch_refused") is True for row in ctx.autostart_results
    )
    ...
    # Disk deferral is retried next cycle; it is not a stalled plan.
    if noop_completed or recovered_landings or disk_deferred:
    ...
    _remote_sandbox_reap_maintenance(ctx)
    _cross_session_lane_reap_maintenance(ctx)
...
def _lane_intake_phase(ctx: OrchestratorContext) -> None:
    ...
    for lane_id in ctx.ordered_ready:
        ...
        target_tip_before_landing, _ = _task_branch_landing(ctx.orchestrator_root)
        if target_tip_before_landing and not _persist_pre_landing_tip(
            ctx,
            lane_id,
            target_tip_before_landing,
        ):
            ctx.log(
                "ERROR",
                "pre_landing_tip_persist_failed",
                lane=lane_id,
                target_tip_before_landing=target_tip_before_landing,
            )
        ...
        decision_text = (
            f"Orchestrator daemon validated intake preconditions for lane {lane_id}."
            if intake_ok
        ...
        if not ctx.dry_run:
            ...
            if landed_sha is None:
                ...
                if contained == "empty":
                    record_decision(
                        ...
                        task_ref=ctx.task_ref,
                        decision=(
                            f"Lane {lane_id} terminalized EMPTY: its branch has "
                            "no lane-authored commits; no lane_landed decision was recorded."
                        ),
                        rationale="Fail-closed terminal disposition for a zero-commit lane.",
                    )
                    close_raw = manage_worktree_lane(
                        operation="close",
                        lane_id=lane_id,
                        status=LaneStatus.CLOSED_STALE,
                        notes="Auto-closed EMPTY by orchestrator daemon; no lane-authored commits.",
                        task_ref=ctx.task_ref,
                    )
                    if isinstance(close_raw, str):
                        close_raw = json.loads(close_raw)
                    ...
                    if close_payload.get("ok") is True:
                        ...
                        ctx.log("INFO", "lane_empty_terminalized", lane=lane_id)
                    else:
                        ctx.log("ERROR", "lane_empty_close_failed", lane=lane_id, payload=close_payload)
                    write_merged = False
                    continue
                if contained is not True:
                    # H1/H2: recipe exit-0 without merge, empty-cut, or
                    # unresolvable branch — record NOTHING. Absence is safe;
                    # a false landing is not.
                    ctx.log(
                        "ERROR",
                        ...
                        branch=lane_branch,
                        contained=contained,
                        reason=_containment_failure_reason(contained),
                    )
                    write_merged = False
            ...
            if write_merged:
                ...
                if close_payload.get("ok") is True:
                    ...
                    try:
                        from workbay_orchestrator_mcp.orchestration.lane_census import (  # noqa: PLC0415
                            request_remote_sandbox_reap,
                        )

                        request_remote_sandbox_reap(ctx.state_dir)
                    except Exception as exc:  # noqa: BLE001 - merge success cannot be rolled back for a hint
                        ctx.log("WARN", "remote_sandbox_reap_request_failed", lane=lane_id, error=str(exc)[:512])
                else:
                    ctx.log(
                        "ERROR",
        ...
        if not ctx.dry_run:
            record_test_result(
                ...
                passed=verify_ok,
                result=(
                    "Cross-lane verification passed."
                    if verify_ok
                    else "Cross-lane verification did not pass (failed, skipped, system_error, or timed_out)."
                ),
            )
...
def _build_orchestrator_context(
    ...
    m_order = manifest_merge_order(task_ref, orchestrator_root=str(orchestrator_root))
...
def _run_orchestrator_cycle(ctx: OrchestratorContext) -> int:
    ...
    # [AGT-21] Exit status is the machine-readable outcome contract. Guidance
    # failures are contained per lane, so this sentinel must outrank a
    # coincident success condition.
    if single_pass and ctx.dispatch_failure_count >= 3:
        log("ERROR", "terminal_error", reason="guidance_failure")
        return 1
