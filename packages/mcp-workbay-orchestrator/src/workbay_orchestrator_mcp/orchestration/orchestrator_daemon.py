...
def daemon_status(state_dir: Path, log_dir: Path) -> dict[str, Any]:
    ...
    if heartbeat_path.exists():
        try:
            ...
            heartbeat_ts = _parse_utc_timestamp(heartbeat.get("ts")) if isinstance(heartbeat, dict) else None
    ...
    return {
        ...
        "reaper_stale": heartbeat_age is None or heartbeat_age > 2 * DEFAULT_REAP_INTERVAL_SECONDS,
    }
