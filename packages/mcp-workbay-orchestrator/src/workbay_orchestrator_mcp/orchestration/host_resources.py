...
_PACKAGE_NAME = "mcp-workbay-orchestrator"
...
# footprint is ~0 (ssh + git). The host does not bound off-box width; the VM admits
# from measured saturation (COST-03). The local host-memory guard must NOT gate it —
# gating a remote lane on local RAM is a false positive that blocks useful off-box
# work whenever the local box is merely busy (internal). Never
# gated (absent from ``_GATED_COST_CLASSES``) — like ``light``, but for the opposite
# reason: no LOCAL footprint at all, rather than "too small to matter". STOCK (local
# worktree count/volume) still applies: an off-box lane still creates a local
# worktree. Do NOT lump grok-cli here: its tests run locally.
COST_REMOTE = "remote"
...
class HostResources:
    ...
    # Free space on the workspace/repository volume. ``None`` means the
    # statvfs probe was unavailable; zero is a measured full volume.
    repo_volume_free_gib: float | None = None
...
class HostMemoryPolicy:
    ...
    max_width: int = 4
    # Cap on concurrent LOCAL heavy slots. Never a bound on off-box (COST_REMOTE)
    # width; the VM's measured admission sizes that.
    ...
    # to fully off-box workers — the host does not bound their width.
...
_cache_at: float = 0.0
_cache_workspace_root: Path | None = None


def _probe_repo_volume_free_gib(workspace_root: Path) -> float | None:
    """Measure the volume lanes actually populate (OBS-12)."""
    try:
        stats = os.statvfs(workspace_root)
    except Exception:  # noqa: BLE001 -- probe_host must never raise
        return None
    return (stats.f_bavail * stats.f_frsize) / _GIB


def probe_host(*, force: bool = False, workspace_root: Path | None = None) -> HostResources:
    """Cached (~5 s) host snapshot. Never raises."""
    global _cache, _cache_at, _cache_workspace_root
    resolved_root = Path.cwd() if workspace_root is None else Path(workspace_root)
    mono = time.monotonic()
    if (
        not force
        and _cache is not None
        and _cache_workspace_root == resolved_root
        and (mono - _cache_at) < _PROBE_CACHE_TTL_S
    ):
        return _cache
    now = time.time()
    try:
    ...
    snapshot = replace(
        snapshot,
        repo_volume_free_gib=_probe_repo_volume_free_gib(resolved_root),
    )
    _cache, _cache_at, _cache_workspace_root = snapshot, mono, resolved_root
    return snapshot
...
_LOCAL_STAGING_DISK_FLOOR_ENV = "WORKBAY_LOCAL_STAGING_DISK_FLOOR_GIB"


def _apply_policy_env_overrides(policy: HostMemoryPolicy) -> HostMemoryPolicy:
    raw = os.environ.get(_LOCAL_STAGING_DISK_FLOOR_ENV)
    if raw is None:
        return policy
    try:
        value = float(raw)
    except ValueError:
        warning = f"{_LOCAL_STAGING_DISK_FLOOR_ENV}={raw!r} is malformed; configured floor retained"
        return replace(policy, warnings=(*policy.warnings, warning))
    if not math.isfinite(value) or value < 0:
        warning = f"{_LOCAL_STAGING_DISK_FLOOR_ENV}={raw!r} is non-finite/negative; configured floor retained"
        return replace(policy, warnings=(*policy.warnings, warning))
    return replace(policy, local_staging_disk_floor_gib=value)
...
def load_host_memory_policy(workspace_root: Path) -> HostMemoryPolicy:
    ...
    try:
        ...
        return _apply_policy_env_overrides(defaults)
    ...
    if not raw:
        ...
        if top_level_misplaced:
            return _apply_policy_env_overrides(
                replace(
                    defaults,
    ...
    for key, value in raw.items():
        ...
        if allowed and typed not in allowed:
            ...
            continue
        if key in _STOCK_CEILING_KEYS and isinstance(typed, int) and not isinstance(typed, bool) and typed == 0:
            warnings.append(f"host_memory: {key}=0 is a size-0 bulkhead (admits nothing); not unlimited")
        policy = replace(policy, **{key: typed})
    if warnings:
        policy = replace(policy, warnings=tuple(warnings))
    return _apply_policy_env_overrides(policy)


def log_host_memory_policy_warnings(workspace_root: Path) -> HostMemoryPolicy:
    """Load the policy and make every ignored setting visible at startup."""
    policy = load_host_memory_policy(workspace_root)
    for warning in policy.warnings:
        _LOGGER.warning("host_memory config warning at server startup: %s", warning)
    return policy


def _loaded_package_version() -> str:
    """Best-effort installed version for a self-describing policy echo."""
    try:
...
def host_memory_policy_echo(workspace_root: Path) -> dict:
    ...
    return {
        ...
        "package_version": _loaded_package_version(),
    }
...
class AdmissionDecision:
    ...
    def width_label(self) -> str:
        """Operator-visible width token driven by ``width_source`` (REF-23)."""
        if self.width_source == "off_box_vm_measured" or self.derived_width is None:
            return "off-box(vm-measured)"
        return str(self.derived_width)

    def demand_label(self) -> str:
        """Name the dimension that fired: slots_full, disk, or memory.
        ...
        if self.reason_code == "memory":
            return "memory"
        if self.reason_code == "refused_disk_floor":
            return "disk"
...
def format_admission_gate_error(decision: AdmissionDecision) -> str:
    """Operator-visible spawn error that names the constrained resource."""
...
def derive_width(resources: HostResources, policy: HostMemoryPolicy, cost_class: str = COST_HEAVY) -> int:
    """Elastic slot width for local ``cost_class`` (D3, single-source formula).
    ...
    Sizes LOCAL classes only. ``COST_REMOTE`` is fully off-box: the host does not
    bound its width (the VM admits from measured saturation), so this raises
    rather than returning ``policy.max_width``. Call :func:`reported_width` at
    decision-building sites.
    """
    if cost_class == COST_REMOTE:
        raise ValueError("off-box width is not sized on the host")
...
def reported_width(
    ...
    """Width carried on an admission decision, plus the source of that figure.

    Off-box (``COST_REMOTE``) width is measured on the VM, never invented here.
    Every other class keeps the RAM-derived local width.
    """
    if cost_class == COST_REMOTE:
        return None, "off_box_vm_measured"
    return derive_width(resources, policy, cost_class), "local_ram"
...
def _classify_admission(
    ...
    # This policy field is explicitly local-staging headroom. Preserve the
    # established exemption for cost classes which do not execute locally.
    if cost_class not in _GATED_COST_CLASSES:
        return "allow", f"{cost_class} cost class is never gated by local memory", None
    if width is None:
        raise ValueError("gated cost class missing local RAM-derived width")
    repo_free = resources.repo_volume_free_gib
    if repo_free is not None and repo_free < policy.local_staging_disk_floor_gib:
        return (
            "refuse",
            f"repo-volume free disk {repo_free:.2f}GiB below floor {policy.local_staging_disk_floor_gib}GiB",
            "refused_disk_floor",
        )
    ...
    if resources.available_ram < available_ram_floor:
        return (
            "refuse",
            f"available RAM {resources.available_ram / _GIB:.1f}GiB below floor {policy.available_ram_floor_gib}GiB",
            "memory",
        )
...
def _stock_capacity_context(stock: WorktreeStock, ceiling: int) -> str:
    """Actionable counts shared by every stock-related slots_full message."""
    registered = "unknown" if stock.registered_worktrees is None else str(stock.registered_worktrees)
    unregistered = "unknown" if stock.unregistered_paths is None else str(stock.unregistered_paths)
    return f"registered={registered}, unregistered={unregistered} (stale), effective ceiling={ceiling}"


def _stock_admission_counts(stock: WorktreeStock, lane_kind: str, ceiling: int) -> str:
    outstanding = _split_outstanding(stock, lane_kind)
    if lane_kind == "review":
        return (
            f"outstanding_record_only={outstanding} "
            f"excluding_never_started={stock.excluded_never_started_record_only} ceiling={ceiling}"
        )
    return (
        f"outstanding_landable={outstanding} "
        f"excluding_never_started={stock.excluded_never_started_landable} ceiling={ceiling}"
    )
...
def _stock_exhaustion_escalation(
    ...
    *,
    ...
    if lane_kind == "review":
        ...
        return (
            "refuse",
            f"{stock_label} stock {_stock_admission_counts(stock, lane_kind, ceiling)} at/above ceiling; "
            f"{_stock_capacity_context(stock, ceiling)}; "
...
def _classify_stock(
    ...
    *,
    ...
    if ceiling <= 0:
        return (
            "refuse",
            f"{stock_label} stock ceiling {ceiling} is non-positive; "
            f"{_stock_capacity_context(stock, ceiling)}; size-0 bulkhead admits nothing",
            "slots_full",
        )
    ...
    if outstanding is None:
        return (
            "refuse",
            f"{stock_label} stock unmeasurable (outstanding unknown); "
            f"{_stock_capacity_context(stock, ceiling)}; stock dimension fail-closed",
            "gauge_unmeasurable",
        )
...
def evaluate_admission(
    ...
    *,
    ...
    """
    width, width_source = reported_width(resources, policy, cost_class)
    ...
    if cost_class not in _GATED_COST_CLASSES:
        ...
        reason = f"{cost_class} cost class is never gated by local memory"
        if stock is not None:
            ceiling, _stock_label = _split_ceiling(policy, lane_kind)
            reason = f"{reason}; {_stock_admission_counts(stock, lane_kind, ceiling)}"
        return AdmissionDecision(
            "allow",
            reason,
            cost_class,
    ...
    elif reason_code == "slots_full" and stock is not None:
        ceiling, _stock_label = _split_ceiling(policy, lane_kind)
        reason = f"{reason}; {_stock_capacity_context(stock, ceiling)}"
    elif decision == "allow" and stock is not None:
        ceiling, _stock_label = _split_ceiling(policy, lane_kind)
        reason = f"{reason}; {_stock_admission_counts(stock, lane_kind, ceiling)}"
    return _apply_enforcement(
        decision,
    ...
    if claimed is None:
        ...
        if outcome == "defer" and max(0, stock_defer_count) + 1 >= max(1, policy.stock_defer_limit):
            ...
            reason = (
                ...
                f"({_stock_admission_counts(stock, lane_kind, ceiling)}); "
                f"{_stock_capacity_context(stock, ceiling)}"
            )
        return (
            _apply_enforcement(
...
def _try_review_reports_for_stock(workspace_root: Path) -> object:
    ...
    try:
        with _get_db_connection() as conn:
            rows = conn.execute("SELECT * FROM worker_reports ORDER BY created_at DESC, id DESC").fetchall()
        return [dict(row) for row in rows]
    except RuntimeNotConfiguredError:
        raise
...
def _try_pass_states_for_stock(workspace_root: Path) -> object:
    """Read durable raw offload-pass states used to identify actual starts.

    Worker-report outcomes are lossy by design: raw admission/defer outcomes
    are mapped to the report enum before persistence. Pass-state documents
    retain the raw outcome and publish ``running`` before worker execution, so
    a crash before report submission remains positive start evidence.
    """
    from workbay_handoff_mcp import api as handoff_api  # noqa: PLC0415
    from workbay_handoff_mcp.config import RuntimeConfig  # noqa: PLC0415

    runtime = RuntimeConfig.for_repo(Path(workspace_root))
    handoff_api.configure_runtime(runtime)
    try:
        paths = sorted(runtime.state_dir.glob("offload-pass-*.json"))
        states = []
        for path in paths:
        ...
        # Doubt means no planned row may be excluded from admission stock.
        return None
...
def _collect_live_stock(workspace_root: Path) -> WorktreeStock | None:
    ...
    try:
        ...
        pass_states = _try_pass_states_for_stock(workspace_root)
...
def resolve_live_admission(
    ...
    *,
    ...
    """
    # Keep the historical no-argument probe call for injected probe functions,
    # then bind its disk reading to this resolver's explicit repository root.
    resources = replace(
        probe_host(),
        repo_volume_free_gib=_probe_repo_volume_free_gib(Path(workspace_root)),
    )
    policy = load_host_memory_policy(workspace_root)
    held = 0
    ...
    if cost_class in _GATED_COST_CLASSES:
        try:
            if lock_root is not None:
                if width is None:
                    raise TypeError("local heavy-slot count requires a RAM-derived width")
    ...
    stock_defer_count = _read_stock_defer_count(lock_root, stock_class) if lock_root is not None else 0
...
def record_admission_telemetry(
    ...
    *,
    ...
    try:
        ...
        avail_detail = f"{snap.available_ram / _GIB:.1f}GiB" if snap.available_ram is not None else "unavailable"
        rationale = (
            ...
            f"pressure={snap.pressure} width={decision.width_label()} held={decision.held_slots}"
