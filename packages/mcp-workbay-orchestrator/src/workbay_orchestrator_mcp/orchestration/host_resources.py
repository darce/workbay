...
COST_SUITE = "suite"
COST_LIGHT = "light"
_GATED_COST_CLASSES = (COST_HEAVY, COST_SUITE)
...
def derive_width(resources: HostResources, policy: HostMemoryPolicy) -> int:
    """Elastic heavy-slot width (D3, single-source formula).

    ``width = clamp(floor((available_ram - os_reserve) / rss_per_heavy), 0, max_width)``

    Floor is **0**, not 1: width 0 means heavy admission refuses. A floor of 1
    would admit a heavy spawn into the OS reserve, exactly the headroom the
    ...
    if not math.isfinite(policy.rss_per_heavy_gib) or not math.isfinite(policy.os_reserve_gib):
        return 0
    rss = policy.rss_per_heavy_gib * _GIB
    if rss <= 0:
        return 0
...
def _classify_admission(
    ...
    if width == 0:
        return (
            "refuse",
            "derived width 0 (available RAM minus OS reserve < rss_per_heavy)",
        )
