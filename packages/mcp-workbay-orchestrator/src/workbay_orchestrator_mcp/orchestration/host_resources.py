...
def evaluate_admission(
    ...
    *,
    ...
    # Per-backend local cap: additional constraint, never a replacement for the
    # width/pressure gate. Only evaluated when class admission would allow, so a
    # starved host still refuses at held-count 0. COST_REMOTE never reaches here
    # (absent from _GATED_COST_CLASSES). Outcome matches slots_full_outcome —
    # backend saturation is retryable when a peer finishes (same as slot full).
    if decision == "allow" and backend is not None and policy.per_backend_local_cap > 0:
        held_for_backend = (held_slots_by_backend or {}).get(backend, 0)
        if held_for_backend >= policy.per_backend_local_cap:
            outcome = policy.slots_full_outcome if policy.slots_full_outcome in ("defer", "refuse") else "defer"
            decision = outcome
            reason = (
                f"backend {backend} at local concurrency cap "
                f"({held_for_backend} held, cap {policy.per_backend_local_cap})"
            )
...
def resolve_live_admission(
    ...
    *,
    ...
    if cost_class in _GATED_COST_CLASSES:
        width = derive_width(resources, policy, cost_class)
        try:
            held = count_held_heavy_slots(locks_root(workspace_root), width, exclude_slots=exclude_slots)
