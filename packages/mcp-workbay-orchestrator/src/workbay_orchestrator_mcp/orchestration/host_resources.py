...
COST_SUITE = "suite"
COST_LIGHT = "light"
# A remote-API CLI driver (e.g. grok-cli): tiny LOCAL RSS (~0.5 GiB) because the
# inference runs off-box. GATED (internal D1 / PF-1) so the
# swap-floor + pressure dimensions still refuse it under thrash — but its width is
# sized on ``rss_per_remote_api_gib``, not the heavy RSS, so it admits on a small
# (e.g. 8 GiB) host under normal memory. Deliberately NOT ``COST_LIGHT``, which
# bypasses the thrash gate entirely.
...
_TOP_LEVEL_HOST_MEMORY_WARNING = (
    "host_memory: block found at top level; must be nested under 'orchestrator:'"
)
...
def load_host_memory_policy(workspace_root: Path) -> HostMemoryPolicy:
    ...
    if top_level_misplaced:
        ...
        warnings.append(
            f"{_TOP_LEVEL_HOST_MEMORY_WARNING} — ignored; the nested "
            "orchestrator.host_memory block is authoritative"
        )
...
def derive_width(
    resources: HostResources, policy: HostMemoryPolicy, cost_class: str = COST_HEAVY
) -> int:
    """Elastic slot width for ``cost_class`` (D3, single-source formula).

    ``width = clamp(floor((available_ram - os_reserve) / rss_per_<class>), 0, max_width)``
    ...
    protect. ``cost_class`` selects the per-class RSS (heavy vs remote-API), so a
    remote-API lane is not force-sized against the heavy footprint (D1/PF-1).
    """
    ...
    if not math.isfinite(rss_per_gib) or not math.isfinite(policy.os_reserve_gib):
        return 0
    rss = rss_per_gib * _GIB
    if rss <= 0:
        return 0
    usable = resources.available_ram - int(policy.os_reserve_gib * _GIB)
    if usable <= 0:
        return 0
    raw = int(usable // rss)
