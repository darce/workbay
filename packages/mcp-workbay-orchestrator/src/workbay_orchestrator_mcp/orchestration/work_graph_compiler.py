...
The compiler is deterministic for its work items and tracked-overlay contract
input. Twin provisioning, admit-time blockers, and ``save_manifest`` live in
the thin ``__main__`` / CLI layer.
"""
...
from workbay_orchestrator_mcp.orchestration.harness_protocol import (
    load_tracked_overlay_source_paths,
)
...
from workbay_orchestrator_mcp.orchestration.lane_ready_set import compute_ready_set, compute_slack
...
_LOAD_TRACKED_OVERLAY_PATHS = object()
...
def _validate_component_preferred_backends(
    by_id: dict[str, dict[str, Any]],
    components: dict[str, list[str]],
) -> None:
    """Refuse fused components whose members disagree on routing authority."""
    for members in sorted(components.values(), key=min):
        declarations: dict[str, list[str]] = defaultdict(list)
        for member in sorted(members):
            preferred_backend = by_id[member].get("preferred_backend")
            if isinstance(preferred_backend, str) and preferred_backend.strip():
                declarations[preferred_backend.strip()].append(member)
        if len(declarations) <= 1:
            continue
        disagreement = "; ".join(
            f"{backend!r} declared by {declarations[backend]!r}" for backend in sorted(declarations)
        )
        component_id = min(members)
        raise WorkGraphCompilerError(
            f"component {component_id!r} members {sorted(members)!r} has conflicting "
            f"preferred_backend values: {disagreement}"
        )
...
def _lane_test_commands(by_id: dict[str, dict[str, Any]], members: list[str]) -> list[str]:
    """Normalize the two accepted operator spellings into manifest commands."""
    ...
    for member in members:
        ...
        if test_cmd is not None:
            if not isinstance(test_cmd, str) or not test_cmd.strip():
                raise WorkGraphCompilerError(f"work item {member!r} test_cmd must be a non-empty string")
            commands.add(test_cmd.strip())
        test_commands = item.get("test_commands")
        if test_commands is not None:
            if not isinstance(test_commands, list):
                raise WorkGraphCompilerError(f"work item {member!r} test_commands must be a list")
            for command in test_commands:
                if not isinstance(command, str) or not command.strip():
                    raise WorkGraphCompilerError(
                        f"work item {member!r} test_commands entries must be non-empty strings"
                    )
                commands.add(command.strip())
    return sorted(commands)


def _path_sets_overlap(left: str, right: str) -> bool:
    left = left.rstrip("/")
    right = right.rstrip("/")
...
def classify_difficulty(
    *,
    owned_paths: Collection[str],
    tracked_overlay_source_paths: Collection[str] | None,
    test_commands: Collection[str],
    produces: Collection[str],
    verify: bool = False,
) -> str:
    """Classify one lane from the four plan-locked inputs.

    ``bounded_edit`` requires one or two owned paths, a non-empty deterministic
    test command, no produced state contract, and no overlap with a tracked
    overlay source.  An unreadable overlay contract (``None``) fails closed to
    ``contract_bearing``. Verify twins are the explicit third class.
    """
    if verify:
        return "verify"
    normalized_owned = sorted({str(path).strip().rstrip("/") for path in owned_paths if str(path).strip()})
    if tracked_overlay_source_paths is None:
        return "contract_bearing"
    overlay = sorted({str(path).strip().rstrip("/") for path in tracked_overlay_source_paths if str(path).strip()})
    touches_overlay = any(_path_sets_overlap(owned, source) for owned in normalized_owned for source in overlay)
    has_test = any(isinstance(command, str) and command.strip() for command in test_commands)
    has_produced_state = any(isinstance(key, str) and key.strip() for key in produces)
    if 1 <= len(normalized_owned) <= 2 and not touches_overlay and has_test and not has_produced_state:
        return "bounded_edit"
    return "contract_bearing"
...
def _select_shared_no_reviewer_backend(
    uncovered_lane_ids: Collection[str],
    lanes: dict[str, dict[str, Any]],
) -> str:
    """Choose one reviewer independent of every lane covered by the shared gate.

    A shared review result can resolve all per-lane ``no_reviewer`` holds, so
    its backend must be a declared review route whose model family differs
    from every implementation family in that corpus.  Intersecting each
    implementer's normal complementary candidates preserves the same registry
    eligibility contract as per-lane twins.  No common candidate is a typed
    compilation refusal, never a same-family or registry-order fallback.
    """
    common_candidates: set[str] | None = None
    implementation_families: set[str] = set()
    for lane_id in sorted(uncovered_lane_ids):
        impl_backend = str(lanes[lane_id].get("preferred_backend") or "")
        impl_spec = BACKENDS.get(impl_backend)
        if impl_spec is None or not impl_spec.model_family:
            raise WorkGraphCompilerError(
                f"shared no_reviewer gate cannot prove an independent reviewer "
                f"for implement lane {lane_id!r} with backend {impl_backend or '<undeclared>'!r}"
            )
        implementation_families.add(impl_spec.model_family)
        candidates = set(select_review_backends(impl_backend))
        common_candidates = candidates if common_candidates is None else common_candidates & candidates

    eligible: list[str] = []
    for backend in common_candidates or set():
        spec = BACKENDS.get(backend)
        if spec is None or not spec.review_eligible or not spec.model_family:
            continue
        if spec.model_family in implementation_families:
            continue
        eligible.append(backend)

    if not eligible:
        families = ", ".join(sorted(implementation_families)) or "<unknown>"
        raise WorkGraphCompilerError(
            f"shared no_reviewer gate has no independent reviewer route for implementation model families: {families}"
        )
    return min(eligible, key=lambda name: _rank_key(BACKENDS[name], name))
...
def _emit_verify_twins(
    *,
    ...
    if uncovered_for_hold:
        ...
        # One report resolves every hold below.  Its route therefore has to be
        # review-eligible and model-family-independent of the whole corpus,
        # not merely the first remote backend in registry order (CARD-01/04).
        shared_backend = _select_shared_no_reviewer_backend(uncovered_for_hold, lanes)
        ...
        # The shared gate reviews the aggregate output of every uncovered
        # producer.  It cannot enter the ready frontier until that complete
        # corpus exists (GRPH-32/33); blocker resolution is a completion gate,
        # not a substitute for these scheduling edges.
        depends_on[shared_hold_id] = sorted(uncovered_for_hold)
        provisioning.append(
            {
...
def compile_work_manifest(
    ...
    *,
    ...
    max_conflict_degree: int | None = None,
    host_memory_policy: HostMemoryPolicy | None = None,
    tracked_overlay_source_paths: Collection[str] | None | object = _LOAD_TRACKED_OVERLAY_PATHS,
    workspace_root: Path | str | None = None,
) -> CompileResult:
    """Compile work items into a validate_manifest-clean lane manifest.

    No DB or environment reads. The tracked-overlay contract is the sole
    filesystem input and is injectable for byte-identical tests. Resolve
    ``WORKBAY_COMPILER_MAX_LANE_FILES`` and
    ...
    ``calibration_accepted`` is an explicit, pure fail-closed gate for junior
    routing. The default keeps every work lane senior; accepting calibration
    enables the deterministic bounded-edit/noncritical/slack rule.
    ...
    *allow_conflict_waves* / *max_conflict_degree* are append-only (implementation note S2).
    ...
    if tracked_overlay_source_paths is _LOAD_TRACKED_OVERLAY_PATHS:
        root = Path(workspace_root).expanduser().resolve() if workspace_root is not None else Path.cwd().resolve()
        tracked_overlay_source_paths = load_tracked_overlay_source_paths(root)

    uf = _build_union(by_id)
    components = uf.components()
    _validate_component_preferred_backends(by_id, components)
    ...
    lanes: dict[str, dict[str, Any]] = {}
    # Severity from work items only (WIDTH-101); no environment-derived rank.
    ...
    for lane_id in work_lane_ids:
        ...
        lane: dict[str, Any] = {
            ...
            "owned_paths": owned,
            "test_commands": _lane_test_commands(by_id, members),
            "title": lane_id,
    ...
    # Tiering reads only the work-lane precedence graph. Verify twins are
    # appended afterwards and therefore cannot manufacture slack for their
    # producer or become a critical-path authority.
    work_manifest_view = {
        "lanes": {lane_id: lanes[lane_id] for lane_id in work_lane_ids},
        "depends_on": {key: list(value) for key, value in depends_on.items()},
    }
    try:
        lane_slack = compute_slack(work_manifest_view)
    except ValueError as exc:
        raise WorkGraphCompilerError(f"compiled manifest failed validation: {exc}") from exc
    critical_lanes = {
        str(row["lane_id"])
        for row in compute_ready_set(work_manifest_view, satisfied=work_lane_ids)["ready"]
        ...
        produced = sorted(
            {
                str(key).strip()
                for member in members
                for key in (by_id[member].get("produces") or [])
                if isinstance(key, str) and key.strip()
            }
        )
        difficulty = classify_difficulty(
            owned_paths=lanes[lane_id]["owned_paths"],
            tracked_overlay_source_paths=(
                tracked_overlay_source_paths
                if tracked_overlay_source_paths is None or isinstance(tracked_overlay_source_paths, Collection)
                else None
            ),
            test_commands=lanes[lane_id]["test_commands"],
            produces=produced,
        )
        slack = int(lane_slack.get(lane_id, 0))
        tier = (
            "junior"
            if calibration_accepted
            and lane_id not in critical_lanes
            and difficulty == "bounded_edit"
            and slack >= 1
            else "senior"
        )
        lanes[lane_id]["difficulty"] = difficulty
        lanes[lane_id]["slack"] = slack
        lanes[lane_id]["critical"] = lane_id in critical_lanes
        if lanes[lane_id].get("preferred_backend") in (None, "codex-remote"):
            lanes[lane_id]["preferred_tier"] = tier
            lanes[lane_id].setdefault("preferred_backend", "codex-remote")
    ...
    if emit_verify_twins:
        for lane_id in work_lane_ids:
            if lanes[lane_id].get("preferred_tier") != "junior":
                continue
            impl_backend = str(lanes[lane_id].get("preferred_backend") or "")
            if not impl_backend or not select_review_backends(impl_backend):
                raise WorkGraphCompilerError(
                    f"junior lane {lane_id!r} requires a mandatory distinct-family reviewer; "
                    f"none is available for preferred_backend {impl_backend or '<undeclared>'!r}"
                )
        ...
        provisioned_ids = {str(row.get("lane_id") or "") for row in provisioning}
        for lane_id in work_lane_ids:
            if lanes[lane_id].get("preferred_tier") != "junior":
                continue
            if not any(twin_id.startswith(f"{lane_id}{VERIFY_NAMESPACE}") for twin_id in provisioned_ids):
                raise WorkGraphCompilerError(
                    f"junior lane {lane_id!r} requires a mandatory distinct-family reviewer; "
                    "reviewer provisioning produced none"
                )
    elif any(lanes[lane_id].get("preferred_tier") == "junior" for lane_id in work_lane_ids):
        raise WorkGraphCompilerError("junior lanes require mandatory distinct-family reviewer twins")
