...
def _tracked_openrouter_model():
    from workbay_orchestrator_mcp.orchestration.openrouter_lane_config import TRACKED_OPENROUTER_MODEL

    return TRACKED_OPENROUTER_MODEL
...
def _render_doctor_key_info_block(name: str, port) -> str:
    """Budget + list-price lines for a key_info port (openrouter, implementation note S5).

    The registry-rendered auth probe runs on the VM (bearer on stdin, key never
    on argv) and its ``limit=… usage=… remaining=…`` data line is echoed
    alongside the credits-pool reading when the port declares ``credits_url``;
    the slug's current list price comes from the public models index, fetched
    once per doctor run with ``--max-time 10`` and reported as ``unavailable``
    on any failure — never a hang, never a fabricated "$0".
    """
    ...
    slug = _tracked_openrouter_model()
...
_wb_credits=$(printf '%s\\n' "$_wb_out" | grep '^credits=' | head -n 1)
_wb_marker=$(printf '%s\\n' "$_wb_out" | grep '^{port.marker_prefix()}_' | head -n 1)
echo "budget  : {name} ${{_wb_line:-unavailable}}${{_wb_credits:+ $_wb_credits}} (probe exit $_wb_rc${{_wb_marker:+, $_wb_marker}}; threshold {port.min_remaining_usd} USD)"
else
echo "budget  : {name} unavailable (mktemp failed)"
fi
if [ -z "${{_wb_models_json:-}}" ] && command -v curl >/dev/null 2>&1; then
_wb_models_json=$(curl -sS --max-time 10 --connect-timeout 5 "{OPENROUTER_MODELS_LIST_URL}" 2>/dev/null) || _wb_models_json=""
fi
_wb_price=""
...
echo "price   : {slug} ${{_wb_price:-unavailable}} (list, {OPENROUTER_MODELS_LIST_URL})"
"""
