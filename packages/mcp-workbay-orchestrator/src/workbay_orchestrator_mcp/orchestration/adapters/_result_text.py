...
def handoff_action_needs_clamp(payload: dict[str, Any]) -> bool:
    ...
    * the payload was recovered via :data:`RECOVERY_TIER_UNSHAPED` (parsed but
      not shape-selected), or
    * ``handoff_action`` is present but not a known enum member (null, ``""``,
      off-enum, non-string).

    Absent ``handoff_action`` on a shaped payload (e.g. findings-only review)
    does **not** clamp here — :meth:`BackendResult.from_dict` already defaults
    absent to ``needs_guidance`` without discarding the report.
    """
    if payload.get(SHAPED_PAYLOAD_RECOVERY_KEY) == RECOVERY_TIER_UNSHAPED:
        return True
    if "handoff_action" not in payload:
        return False
    action = payload.get("handoff_action")
    return not (isinstance(action, str) and action in KNOWN_HANDOFF_ACTIONS)
