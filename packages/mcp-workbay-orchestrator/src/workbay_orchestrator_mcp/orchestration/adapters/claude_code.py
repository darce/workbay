...
# Cap ``claude execute`` actually passes as ``--max-turns``. Keep this on the
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        with tempfile.TemporaryDirectory(prefix="claude-code-") as tmpdir:
            ...
            identity = payload is response
            return stamp_attempt_evidence(
                result,
                # Host stdout only. The extracted worker document is authorable
                # and must not mint tool-call evidence. When stdout *is* that
                # document, count nothing from it.
                tool_call_count=(
                    0 if identity else count_observed_tool_events(output)
                ),
                ...
                receiver_num_turns=None if identity else host_session_num_turns(response),
            )
