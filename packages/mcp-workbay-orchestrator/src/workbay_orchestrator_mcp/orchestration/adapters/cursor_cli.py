...
def timeout_derived_max_turns(timeout: Any) -> int | None:
    """Turn cap implied by the wall-clock bound cursor actually enforces.

    ``cursor-agent`` has no ``--max-turns`` flag; the process-group kill is the
    bound. Convert that timeout through the shared seconds-per-turn constant so
    handoff classification sees the same cap the execute path used.
    """
    from ..grok_lane_config import SECONDS_PER_TURN  # noqa: PLC0415

    if isinstance(timeout, bool) or not isinstance(timeout, int) or timeout < 1:
        return None
    return max(1, timeout // SECONDS_PER_TURN)
...
class CursorCliAdapter(BackendAdapter):
    ...
    def _resolve_model(self, model: str | None, reasoning_effort: str | None) -> tuple[str, str | None, str | None]:
        """Resolve ``--model``, the effort that slug ACTUALLY encodes, and any downgrade.
        ...
        Returns ``(slug, effective_effort, downgrade_reason)``. A request that
        cannot be honored is logged at WARNING ([AGT-10] degrade loudly) and the
        reason is stamped on ``BackendResult`` (implementation note S5 / [RLSE-05]).
        """
        ...
        return slug, effective_effort, downgrade
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        model_slug, effective_effort, effort_downgrade = self._resolve_model(model, reasoning_effort)
        ...
        if effort_downgrade:
            raw_payload["downgrade_reason"] = effort_downgrade
        result = BackendResult(
            ...
            reasoning_effort=effective_effort,
            downgrade_reason=effort_downgrade,
        ...
        identity = extracted is payload
        return stamp_attempt_evidence(
            result,
            # Host stdout only, and never the worker result document. When
            # --output-format json emits the result object itself, that object
            # is authorable and must not mint tool-call evidence.
            tool_call_count=(
                0 if identity else count_observed_tool_events(completed.stdout)
            ),
            max_turns=(
                requested_max_turns
                if requested_max_turns is not None
                else timeout_derived_max_turns(self.timeout)
            ),
            # Host wrapper session counter only. A stdout-identical worker
            # document must not buy a receiver turn copy (F3).
            receiver_num_turns=None if identity else host_session_num_turns(payload),
        )
