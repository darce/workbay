        ...
        **kwargs: Any,
        ...
        with ExitStack() as _stack:
            ...
            if secure_sandbox_enabled():
                _sandbox = _stack.enter_context(
                    ShallowSandbox(Path(worktree_path), _worktree_branch(worktree_path))
                )
                # Provision the sandbox env (uv sync) so grok's self-verify runs
                # against sandbox src. Fail-closed like the clone itself.
                if sandbox_provision_enabled():
                    _sandbox.provision_env(timeout=self.timeout)
