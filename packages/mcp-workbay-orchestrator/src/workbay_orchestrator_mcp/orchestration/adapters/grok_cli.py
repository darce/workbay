...
whole process GROUP, not just the direct child. A model outside the allowed
set (the configured ``WORKBAY_GROK_MODEL`` pin or the Composer family; never
grok-build) is refused pre-exec (fail fast; implementation note amendment); an
...
#: Composer-only guarantee: grok-build must never author task work. Any resolved
...
#: A grok-build-authored debug-log item — the Composer-only contamination marker
...
def _verify_model_pin(debug_file: Path, expected_model: str) -> tuple[str, list[str]] | None:
    """Post-turn model-pin check on the always-on debug log (REV-S4-02).

    Returns ``(blocker, evidence_lines)`` if the pin cannot be confirmed —
    grok-build contamination, any non-pinned ``model_id`` marker, scan-input
    absence (missing/empty file), OR a non-empty log with no recognizable
    AssistantItem/model_id markers (format drift). Fail-closed polarity is
    preserved: this function never fails OPEN. Returns ``None`` when clean.

    Parameterized over any pin (composer family or ``grok-4.5``); callers that
    only attest Composer pins still pass the composer slug as
    ``expected_model``.
    """
    ...
    if count > 0:
        return (
            f"grok-build authored {count} AssistantItem(s) — model pin '{pin}' violated",
...
class GrokCliAdapter(BackendAdapter):
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        # relaxed 2026-07-09 per the implementation note amendment: the hard Composer-only
        # requirement is retired). Allowed: the configured pin DEFAULT_GROK_MODEL
        # (env WORKBAY_GROK_MODEL > shipped default) OR any model normalizing to
        # a "Grok Composer ..." label (pre-amendment behavior, kept for
        # compatibility). grok-build (any spelling) and unknown slugs are still
        # refused — allow-list polarity preserved, cannot fail OPEN (s3-a-009).
        effective_model = model or DEFAULT_GROK_MODEL
        # Only a Composer pin runs post-turn model-pin attestation (the debug-log
        # scan asserts only the expected pin authored work). A non-Composer pin
        # (e.g. grok-4.5, the offload default / DEFAULT_GROK_MODEL per implementation note)
        # is permitted by the allow-list below but MUST NOT be run through
        # _verify_model_pin at the execute gate (would fail closed to
        # needs_guidance on a green, committed diff without attestation wiring).
        # Gate off the configured pin slug form (``grok-composer-*``) — not a
        # ``Grok Composer`` human-label literal — so re-pinning DEFAULT_GROK_MODEL
        # is the single switch (internal).
        from workbay_orchestrator_mcp.orchestration.grok_lane_config import (  # noqa: PLC0415
            model_requires_composer_attestation,
        )

        is_composer_pin = model_requires_composer_attestation(effective_model)
        allowed = effective_model == DEFAULT_GROK_MODEL or is_composer_pin
        if _GROK_BUILD_RE.search(effective_model) or not allowed:
            raise RuntimeError(
                f"Refusing to dispatch grok with model '{effective_model}': allowed are "
                f"the configured pin '{DEFAULT_GROK_MODEL}' (WORKBAY_GROK_MODEL) or "
                "Composer models (decision #2799, implementation note amendment)."
            )
        ...
        if _GROK_BUILD_RE.search(joined_extra) or _MODEL_OVERRIDE_RE.search(joined_extra):
            raise RuntimeError(
                ...
                f"(Composer-only guard bypass, decision #2799): {joined_extra!r}"
            )
        ...
        # default — s6-a-003).
        composer_identity = normalize_model_identity(normalize_model_label(effective_model), None) or effective_model
        ...
        with tempfile.TemporaryDirectory(prefix="grok-cli-") as tmpdir:
            ...
            full_prompt = (
                ...
                f"'{composer_identity}' (your Composer model identity), not the orchestrator.\n"
            )
            ...
            try:
                completed = _run_bounded(
                    ...
                    # Composer-only config regardless of whether it keys off the
            ...
            # Model-pin post-turn verification (D3 / REV-S4-02): scan the always-on
            # debug log. Fail closed on grok-build contamination, non-pinned
            # model markers, OR scan-input absence; no auto-retry (decision
            # #2799) — the worktree diff is left for the operator to inspect.
            # Gated to Composer pins only (implementation note): a non-Composer pin
            # (DEFAULT_GROK_MODEL / grok-4.5 default) has no Composer authorship
            # to attest, so the guard is skipped and a parseable green result
            # flows through. The engine then surfaces
            # ``handoff_ready_unattested`` + ``reason: attestation_missing`` so the
            # skip is explicit in the pass payload (never silent).
            pin_violation = _verify_model_pin(debug_file, effective_model) if is_composer_pin else None
            if pin_violation is not None:
                blocker, evidence = pin_violation
                return BackendResult(
                    handoff_action="needs_guidance",
                    summary="grok Composer-only guarantee not confirmed",
