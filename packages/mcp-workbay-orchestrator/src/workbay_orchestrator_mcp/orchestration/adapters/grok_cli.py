...
from ..lane_test_cmd import TEST_CMD_NOT_SUPPLIED, resolve_dispatch_test_cmd
...
def _last_flag_value(argv: list[str], flag: str) -> str | None:
    """Return the last value for ``flag`` in ``argv``, or ``None`` if omitted."""
    value: str | None = None
    pending = False
    for part in argv:
        if pending:
...
_TRACE_LINE_RE = re.compile(r"^\S+\s+\S+\s+(?P<module>[A-Za-z0-9_]+(?:::[A-Za-z0-9_]+)+):\s+(?P<message>.*)$")
_BUILD_SUFFIX = "-build"
_OPTION_NONE = "None"
...
def _scan_conversation_served_models(
    ...
    *base* (``-build`` stripped) to that pin so a vendor-resolved
    ...
    """
    requested_base = _served_base_model(requested_model).strip().lower() if requested_model else ""
    ...
    if saw_retired_or_off_pin and not saw_bootstrap:
        return (
            "authored served-model is retired or does not match the requested pin — contamination quarantine",
            evidence[:20],
        )
    return (
...
def _receiver_num_turns_from_grok_debug(debug_file: Path) -> int | None:
    """Host-observed turn count from the grok debug log, if any.

    Prefers the gateway ``numTurns`` scrape, then JSON lines carrying
    ``numTurns`` / ``num_turns`` / ``turnCount``. Returns None when the log
    has no such signal; never invents a count from the worker result.
    """
    from .remote_exec import (  # noqa: PLC0415
        _receiver_num_turns_from_debug_log,
        host_session_num_turns,
    )

    harvested = _receiver_num_turns_from_debug_log(debug_file)
    ...
    for line in text.splitlines():
        ...
        if not isinstance(obj, dict):
            continue
        session = host_session_num_turns(obj)
        if session is not None:
            last = session
            continue
        meta = obj.get("_meta")
        if isinstance(meta, dict):
            session = host_session_num_turns(meta)
            if session is not None:
                last = session
    return last
...
def find_grok(explicit_path: str | None = None) -> str:
    ...
    PATH lookup uses ``shutil.which`` so a locator miss cannot inherit the MCP
    server's JSON-RPC stdin (internal).
    """
    if explicit_path:
...
GROK_NATIVE_METADATA_ARGV: tuple[str, ...] = ("agent", "--no-leader", "stdio")
NATIVE_METADATA_WALL_TIMEOUT_S = 8.0
NATIVE_METADATA_READLINE_TIMEOUT_S = 2.0
NATIVE_METADATA_MAX_STDOUT_BYTES = 64 * 1024
NATIVE_METADATA_MAX_STDERR_BYTES = 16 * 1024
_JSONRPC_INVALID_PARAMS = -32602
_NATIVE_METADATA_KINDS = frozenset(
    {
        "timeout",
        "malformed",
        "unavailable",
...
class NativeMetadataError(RuntimeError):
    """Typed native-metadata handshake refusal. Never a model fallback."""
    ...
    def __init__(
        ...
        *,
        ...
        if kind not in _NATIVE_METADATA_KINDS:
            kind = "malformed"
...
class NativeMetadataResult:
    """Observed ACP handshake. Names and registry pins are not capability."""
...
def native_metadata_command(*, grok_bin: str, gate_host: str | None = None) -> list[str]:
    """Argv for ``grok agent --no-leader stdio``, wrapped in SSH when remote."""
    argv = [grok_bin, *GROK_NATIVE_METADATA_ARGV]
    if not gate_host:
        return argv
    ...
    return build_remote_list_models_argv(argv, host=gate_host)


def produce_native_metadata(
    *,
    backend_id: str,
    model: str,
    ...
    readline_timeout_s: float | None = None,
) -> NativeMetadataResult:
    """Observe native Grok ACP metadata. Never sends ``session/prompt``."""
    requested = str(model or "").strip()
    if not requested:
        raise NativeMetadataError(
            "unsupported_model",
            "native metadata requires an explicit model",
            requested_model=requested or None,
        )
    off_box = backend_id == "grok-remote"
    if off_box:
        if not gate_host:
            raise NativeMetadataError("unavailable", "native metadata requires the resolved remote gate host")
        binary = grok_bin or "grok"
        argv = native_metadata_command(grok_bin=binary, gate_host=gate_host)
        bound_host = gate_host
    else:
        if gate_host:
            raise NativeMetadataError("mismatch", "grok-cli native metadata must not bind a remote gate host")
        try:
            binary = grok_bin or find_grok()
        except RuntimeError as exc:
            raise NativeMetadataError("unavailable", str(exc)) from exc
        argv = native_metadata_command(grok_bin=binary, gate_host=None)
        bound_host = None
    return produce_native_metadata_from_argv(
        argv,
        ...
        readline_timeout_s=readline_timeout_s,
    )


def produce_native_metadata_from_argv(
    argv: Sequence[str],
    *,
    backend_id: str,
    model: str,
    ...
    """Exchange initialize → session/new → session/set_model on *argv*."""
    ...
    requested_effort = str(effort).strip() if effort else None
    transport = native_transport_for_backend(backend_id)
    wall_s = NATIVE_METADATA_WALL_TIMEOUT_S if wall_timeout_s is None else float(wall_timeout_s)
    line_s = NATIVE_METADATA_READLINE_TIMEOUT_S if readline_timeout_s is None else float(readline_timeout_s)
    if not (wall_s > 0 and line_s > 0):
        raise NativeMetadataError("timeout", "native metadata timeout bound must be positive")
    child_env = dict(env) if env is not None else os.environ.copy()
    for malloc_key in _MALLOC_DEBUG_ENV_KEYS:
        child_env.pop(malloc_key, None)
    methods: list[str] = []
    ...
    try:
        ...
        raise NativeMetadataError("unavailable", f"native metadata spawn failed: {exc}") from exc
    ...
    def _drain_stderr() -> None:
        ...
        try:
            while True:
                ...
                if len(stderr_buf) > NATIVE_METADATA_MAX_STDERR_BYTES:
                    stderr_overflow.set()
                    return
        except (OSError, ValueError):
            return

    stderr_thread = threading.Thread(target=_drain_stderr, name="grok-acp-stderr", daemon=True)
    stderr_thread.start()
    ...
    try:
        init = _acp_rpc(
            ...
            1,
            deadline=deadline,
            line_timeout_s=line_s,
            stdout_buf=stdout_buf,
            methods=methods,
            requested_model=requested,
        )
        advertised_models, efforts_by_model, agent_version = _parse_initialize(init)
        if requested not in advertised_models:
            # Still select: default currentModelId is not requested identity.
            pass
        supports, advertised_efforts = efforts_by_model.get(requested, (False, ()))
        if requested_effort is not None:
            if requested not in efforts_by_model or not supports or requested_effort not in advertised_efforts:
                raise NativeMetadataError(
                    "unsupported_effort",
                    f"unsupported_effort: {requested_effort!r} is not advertised for {requested!r}",
                    method="initialize",
                    requested_model=requested,
                )
        created = _acp_rpc(
        ...
        session_id = _parse_session_id(created)
        selected = _acp_rpc(
            proc,
            "session/set_model",
        ...
        observed = _parse_set_model_ok(selected, requested)
        if requested not in advertised_models:
            raise NativeMetadataError(
                "unsupported_model",
                f"unsupported_model: {requested!r} was not advertised by native metadata",
                method="session/set_model",
                requested_model=requested,
            )
        if "session/prompt" in methods:
            raise NativeMetadataError("malformed", "native metadata must not send session/prompt")
        captured_at = time.monotonic()
        return NativeMetadataResult(
            backend_id=backend_id,
            transport=transport,
            selected_model=observed,
            advertised_models=advertised_models,
            advertised_efforts=advertised_efforts,
            capabilities=((observed, transport),),
        ...
        raise NativeMetadataError("timeout", f"native metadata timed out: {exc}") from exc
    finally:
        if stderr_overflow.is_set():
            pass
        _cleanup_native_metadata_process(proc)
        stderr_thread.join(timeout=1.0)
        if stderr_overflow.is_set():
            raise NativeMetadataError("malformed", "native metadata stderr exceeded bound")
...
def _acp_rpc(
    ...
    *,
    ...
    if method == "session/prompt":
        raise NativeMetadataError(
            "malformed",
            "native metadata must not send session/prompt",
            method=method,
    ...
    if proc.stdin is None or proc.stdout is None:
        raise NativeMetadataError(
            "unavailable",
            "native metadata stdio is unavailable",
            method=method,
            requested_model=requested_model,
        )
    if proc.poll() is not None:
        raise NativeMetadataError(
            "unavailable",
            f"native metadata process exited {proc.returncode}",
            method=method,
            requested_model=requested_model,
        )
    ...
    if remaining <= 0:
        raise NativeMetadataError(
            "timeout",
            "native metadata wall time exceeded",
            method=method,
            requested_model=requested_model,
        )
    payload = json.dumps({"jsonrpc": "2.0", "id": req_id, "method": method, "params": params}, separators=(",", ":"))
    try:
        ...
        raise NativeMetadataError(
            "unavailable",
            f"native metadata stdin write failed: {exc}",
            method=method,
            requested_model=requested_model,
        ) from exc
    methods.append(method)
    reply = _read_jsonrpc_reply(
        proc,
        req_id=req_id,
        deadline=min(deadline, time.monotonic() + line_timeout_s),
        stdout_buf=stdout_buf,
    )
    if "error" in reply:
        raise _classify_acp_rpc_error(
            method,
            params,
            reply.get("error"),
            requested_model=requested_model,
    ...
    if not isinstance(result, dict):
        raise NativeMetadataError(
            "malformed",
            f"native metadata {method} result is not an object",
            method=method,
            requested_model=requested_model,
        )
    return result
...
def _classify_acp_rpc_error(
    ...
    *,
    ...
    selected = params.get("modelId")
    selected_model = selected.strip() if isinstance(selected, str) and selected.strip() else None
    if code == _JSONRPC_INVALID_PARAMS and method == "session/set_model":
        return NativeMetadataError(
            "unsupported_model",
            f"unsupported_model: session/set_model rejected {selected_model!r} ({code})",
            method=method,
    ...
    if code == _JSONRPC_INVALID_PARAMS:
        return NativeMetadataError(
            "malformed",
            f"malformed: {method} rejected protocol parameters ({code})",
            method=method,
            requested_model=requested_model,
        )
    return NativeMetadataError(
        "malformed",
        f"native metadata JSON-RPC error for {method}: {error!r}",
        method=method,
        requested_model=requested_model,
    )


def _read_jsonrpc_reply(
    proc: subprocess.Popen[bytes],
    *,
    req_id: int,
    deadline: float,
    ...
    stdout = proc.stdout
    if stdout is None:
        raise NativeMetadataError("unavailable", "native metadata stdout is unavailable")
    fd = stdout.fileno()
    while True:
        remaining = deadline - time.monotonic()
        if remaining <= 0:
            raise NativeMetadataError("timeout", "native metadata readline timeout")
        ...
        try:
            ...
            raise NativeMetadataError("unavailable", f"native metadata stdout read failed: {exc}") from exc
        if not chunk:
            raise NativeMetadataError("unavailable", "native metadata stdout closed")
        stdout_buf.extend(chunk)
        if len(stdout_buf) > NATIVE_METADATA_MAX_STDOUT_BYTES:
            raise NativeMetadataError("malformed", "native metadata stdout exceeded bound")
        while b"\n" in stdout_buf:
            raw_line, _, rest = bytes(stdout_buf).partition(b"\n")
            stdout_buf[:] = rest
            line = raw_line.strip()
            if not line:
                continue
            try:
                ...
                raise NativeMetadataError("malformed", f"native metadata reply is not JSON: {exc}") from exc
            if not isinstance(message, dict):
                raise NativeMetadataError("malformed", "native metadata reply is not an object")
...
def _parse_initialize(
    result: dict[str, Any],
) -> tuple[tuple[str, ...], dict[str, tuple[bool, tuple[str, ...]]], str | None]:
    meta = result.get("_meta") if isinstance(result.get("_meta"), dict) else {}
    agent_version_raw = meta.get("agentVersion")
    agent_version = str(agent_version_raw) if agent_version_raw not in (None, "") else None
    ...
    if not isinstance(available, list):
        raise NativeMetadataError("malformed", "native metadata initialize omitted availableModels")
    models: list[str] = []
    efforts_by_model: dict[str, tuple[bool, tuple[str, ...]]] = {}
    for item in available:
        ...
        supports = item_meta.get("supportsReasoningEffort") is True
        efforts_by_model[slug] = (supports, _parse_reasoning_efforts(item_meta.get("reasoningEfforts")))
    if not models:
        raise NativeMetadataError("unavailable", "native metadata advertised no models")
    return tuple(dict.fromkeys(models)), efforts_by_model, agent_version


def _parse_reasoning_efforts(raw: Any) -> tuple[str, ...]:
    tokens: list[str] = []
    ...
    def _take(entry: Any) -> None:
        if isinstance(entry, str) and entry.strip():
            ...
            token = entry.get("id") or entry.get("value")
    ...
    if isinstance(raw, list):
        ...
        for key, entry in raw.items():
            before = len(tokens)
            _take(entry)
            if len(tokens) == before and isinstance(key, str) and key.strip():
                tokens.append(key.strip())
    return tuple(dict.fromkeys(tokens))


def _parse_session_id(result: dict[str, Any]) -> str:
    session_id = result.get("sessionId")
    if not isinstance(session_id, str) or not session_id.strip():
        raise NativeMetadataError("malformed", "native metadata session/new omitted sessionId")
    return session_id.strip()


def _parse_set_model_ok(result: dict[str, Any], requested: str) -> str:
    meta = result.get("_meta") if isinstance(result.get("_meta"), dict) else {}
    model_field = meta.get("model")
    observed: str | None = None
    if isinstance(model_field, dict) and "Ok" in model_field:
        raw = model_field.get("Ok")
        if isinstance(raw, str) and raw.strip():
            observed = raw.strip()
    elif isinstance(model_field, str) and model_field.strip():
        observed = model_field.strip()
    if observed is None:
        raise NativeMetadataError(
            "malformed",
            "native metadata set_model omitted _meta.model.Ok",
            method="session/set_model",
            requested_model=requested,
        )
    if observed != requested:
        raise NativeMetadataError(
            "mismatch",
            f"mismatch: requested {requested!r} native selection Ok={observed!r}",
            method="session/set_model",
            requested_model=requested,
        )
    return observed
...
def _cleanup_native_metadata_process(proc: subprocess.Popen[bytes]) -> None:
    try:
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        if resolved_role is None:
            resolved_role = kwargs.pop("resolved_role", None)
        ...
        # A catalogue-admitted resolved role may drive one non-default Grok model;
        # a supported name alone is not capability proof.
        effective_model = model or DEFAULT_GROK_MODEL
        retired_warning = retired_model_warning(effective_model)
        if _GROK_BUILD_RE.search(effective_model) or retired_warning is not None:
            message = (
        ...
        if resolved_role is not None:
            from workbay_orchestrator_mcp.orchestration.resolved_role_config import (  # noqa: PLC0415
                admit_grok_cli_execution,
            )

            admit_grok_cli_execution(resolved_role, model=effective_model, effort=reasoning_effort)
            effective_model = resolved_role.model or effective_model
        elif effective_model != DEFAULT_GROK_MODEL:
            raise RuntimeError(
                f"Refusing to dispatch grok with model '{effective_model}': allowed is "
                f"the configured pin '{DEFAULT_GROK_MODEL}' (WORKBAY_GROK_MODEL); "
                "grok-build family is refused (decision #2799)."
        ...
        # onto the result verbatim (s3-a-006). A resolved-role execution must
        # apply the admitted effort or refuse; it cannot stamp a dropped request.
        admitted_effort = getattr(resolved_role, "effort", None) if resolved_role is not None else None
        if reasoning_effort in _VALID_REASONING_EFFORTS:
            applied_effort = reasoning_effort
        elif reasoning_effort is None and admitted_effort in _VALID_REASONING_EFFORTS:
            applied_effort = admitted_effort
        else:
            applied_effort = None
        ...
        with ExitStack() as _stack:
            ...
            # sanctioned skips (`provision_skipped: no_python_project` and
            # `provision_skipped: no_runner_for_test_cmd`) — which still launch grok
            # and surface on BackendResult.sandbox_provision — from typed
            # `provision_failed_*` results. Those raise SecureSandboxError inside
            # this ExitStack so the sandbox is cleaned up and grok never spawns.
            ...
            if secure_sandbox_enabled():
                ...
                # Provision the sandbox env so the worker's self-verify runs against
                # sandbox src. The lane row is the test-command owner: resolve through
                # lane_test_cmd so a TEST_CMD_NOT_SUPPLIED sentinel never reaches
                # _runner_arm as a non-string. A cleared command (`""`) becomes None.
                if sandbox_provision_enabled():
                    ...
                    if test_cmd_warning:
                        _LOGGER.warning(
                            "lane %s test_cmd: row value len=%s digest=%s overrides explicit carrier len=%s digest=%s",
                            lane_id,
                    ...
                    sandbox_provision = _sandbox.provision_env(
                        timeout=self.timeout,
                        test_cmd=resolved_test_cmd or None,
                    )
                    if sandbox_provision.startswith("provision_failed_"):
                        raise SecureSandboxError(sandbox_provision)
            ...
            if resolved_role is not None:
                from workbay_orchestrator_mcp.orchestration.resolved_role_config import (  # noqa: PLC0415
                    require_applied_effort_matches_resolved,
                )

                require_applied_effort_matches_resolved(
                    applied=_last_flag_value(cmd, "--reasoning-effort"),
            ...
            def _stamp_grok_result(grok_result: BackendResult) -> BackendResult:
                return stamp_attempt_evidence(
                    grok_result,
                    # Host stdout only. Envelope/structuredOutput worker JSON is
                    # authorable and must not mint tool-call evidence.
                    tool_call_count=count_observed_tool_events(stdout),
                    max_turns=self.max_turns,
                    # Host-observed turn count from the debug log this adapter
                    # wrote. Stamp nothing when the log carries no numTurns /
                    # turnCount signal; never copy from the worker result.
                    receiver_num_turns=_receiver_num_turns_from_grok_debug(debug_file),
                )

            contamination = _detect_grok_build_contamination(debug_file, requested_model=effective_model)
            if contamination is not None:
                blocker, evidence = contamination
                return _stamp_grok_result(
                    BackendResult(
            ...
            return _stamp_grok_result(result)
