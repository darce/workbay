"""Execution adapter for the ``grok`` CLI (xAI Composer-2.5 junior worker).
...
whole process GROUP, not just the direct child. A non-Composer model pin is
refused pre-exec (fail fast); an unparseable turn fails closed to
``needs_guidance`` rather than returning a silent-empty result.
"""
...
_ENVELOPE_MARKERS = ("structuredOutput", "structuredOutputError", "usage", "model", "text", "output_text")
...
#: all; a non-empty log with none of these markers cannot attest Composer-only
...
def _verify_composer_only(debug_file: Path) -> tuple[str, list[str]] | None:
    """Post-turn Composer-only check on the always-on debug log.

    Returns ``(blocker, evidence_lines)`` if the guarantee cannot be confirmed —
    grok-build contamination, scan-input absence (missing/empty file), OR a
    non-empty log with no recognizable AssistantItem/model_id markers (format
    drift, fail closed rather than silently pass). Returns None when clean.
    """
    if not debug_file.is_file():
        return ("grok debug log absent — Composer-only guarantee unverifiable (failing closed)", [])
    text = debug_file.read_text(errors="replace")
    if not text.strip():
        return ("grok debug log empty — Composer-only guarantee unverifiable (failing closed)", [])
    if not _DEBUG_MARKER_RE.search(text):
        return (
            "grok debug log has no recognizable AssistantItem/model_id markers — "
            "Composer-only guarantee unverifiable (failing closed)",
            [],
        )
    count = count_grok_build_items(text)
    if count > 0:
        return (
            f"grok-build authored {count} AssistantItem(s) — Composer-only guarantee violated",
...
class GrokCliAdapter(BackendAdapter):
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        # Composer-only pre-exec hard-fail (fail fast, no retry — decision #2799).
        # Allow-list polarity via the S1 label recognizer: only models that
        # normalize to a "Grok Composer ..." label may run. grok-build (and any
        # build spelling) normalizes to "Grok Build"; unknown slugs fall through
        # to verbatim — both are refused, so the guard does not fail OPEN on an
        # alternate build spelling (s3-a-009).
        effective_model = model or DEFAULT_GROK_MODEL
        composer_label = normalize_model_label(effective_model) or ""
        if _GROK_BUILD_RE.search(effective_model) or not composer_label.startswith("Grok Composer"):
            raise RuntimeError(
                f"Refusing to dispatch grok with model '{effective_model}': only Composer "
                "models satisfy the Composer-only guarantee (decision #2799)."
            )
        ...
        with tempfile.TemporaryDirectory(prefix="grok-cli-") as tmpdir:
            ...
            # Composer-only post-turn verification (D3): scan the always-on debug
            # log. Fail closed on grok-build contamination OR scan-input absence;
            # no auto-retry (decision #2799) — the worktree diff is left for the
            # operator to inspect.
            composer_violation = _verify_composer_only(debug_file)
            if composer_violation is not None:
                blocker, evidence = composer_violation
            ...
            if payload is None:
                ...
                return BackendResult(
                    ...
                    response_model=response_model,
                    reasoning_effort=applied_effort,
                    raw_payload={"stdout_tail": _tail_text(stdout)},
                )

            result = BackendResult.from_dict(payload)
            if token_usage or response_model is not None or applied_effort is not None:
...
def _parse_envelope(stdout: str) -> dict[str, Any] | None:
    ...
    A clean whole-stdout parse wins. Otherwise scan for a balanced object and
    prefer one that carries envelope markers or a result key, so a stray CLI
    banner line around the JSON (``npm notice: update available\\n{...}``) does
    not defeat envelope recovery and lose usage/model (s3-a-003).
    """
    try:
    ...
    for block in _iter_balanced_objects(stdout):
        ...
        if best is None:
            best = d
        if _RESULT_KEY in d or any(marker in d for marker in _ENVELOPE_MARKERS):
            return d
    return best
