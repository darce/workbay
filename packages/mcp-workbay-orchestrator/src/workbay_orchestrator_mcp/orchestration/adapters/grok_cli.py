...
def count_grok_build_items(debug_text: str, requested_model: str | None = None) -> int:
    ...
    corresponding build spelling is not counted (not contamination).
    """
    count = 0
    for match in _MODEL_ID_VALUE_RE.finditer(debug_text):
        mid = match.group(1)
        if not _GROK_BUILD_RE.search(mid):
            continue
...
def _grok_build_evidence(
    debug_text: str, requested_model: str | None = None, limit: int = 20
) -> list[str]:
    ...
    hits = [
        ...
        if count_grok_build_items(line, requested_model) > 0
    ]
    return hits[:limit]
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        with ExitStack() as _stack:
            ...
            # contamination.
            contamination = _detect_grok_build_contamination(
