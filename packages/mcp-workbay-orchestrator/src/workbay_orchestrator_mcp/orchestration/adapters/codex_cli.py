...
# Cap ``codex exec`` actually passes as ``--max-turns``. Keep this on the
# adapter so WorkerConfig.grok_max_turns is not the Codex handoff source.
DEFAULT_CODEX_MAX_TURNS = 60


def _codex_cli_max_turns(extra_args: list[str], default: int) -> tuple[list[str], int]:
    """Return extra ``--max-turns`` flags and the cap those flags enforce."""
    for index, token in enumerate(extra_args):
        if token == "--max-turns" and index + 1 < len(extra_args):
            try:
                parsed = int(extra_args[index + 1])
            except (TypeError, ValueError):
                break
            if parsed >= 1:
                return [], parsed
            break
    return ["--max-turns", str(default)], default


def find_codex(explicit_path: str | None = None) -> str:
    """Find the codex CLI executable.

    PATH lookup uses ``shutil.which`` rather than ``subprocess.run(["which",
    ...])`` so a locator miss cannot inherit the MCP server's JSON-RPC stdin
    (internal).
    """
    if explicit_path:
...
class CodexCliAdapter(BackendAdapter):
    ...
    def __init__(
        ...
        *,
        max_turns: int = DEFAULT_CODEX_MAX_TURNS,
    ):
        # Resolve the binary LAZILY (in execute), not here: an eager resolve
        # raises when codex is absent, and RemoteExecAdapter constructs the
        # shaping class for codex-remote on the orchestrator host where only the
        # OCI VM needs codex (s4-h-05). Same reasoning as cursor_cli/grok_cli/
        # claude_code — daemon constructs adapters outside EXEC_FAILED try/except.
        self.codex_bin = codex_bin
        self.codex_args = codex_args or []
        self.max_turns = (
            max_turns
            if isinstance(max_turns, int) and not isinstance(max_turns, bool) and max_turns >= 1
            else DEFAULT_CODEX_MAX_TURNS
        )
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        codex_bin = find_codex(self.codex_bin)
        ...
        with tempfile.TemporaryDirectory(prefix="codex-cli-") as tmpdir:
            ...
            # Lane write-jail prefix (implementation note / adoption C). Empty unless the
            # orchestrator gated a Seatbelt profile in; prepended so codex runs as
            # `sandbox-exec -f <profile> codex exec ...`.
            jail_prefix = list(kwargs.get("jail_argv_prefix") or [])
            extra_args = list(extra_args)
            from .remote_exec import (  # noqa: PLC0415
                count_observed_tool_events,
                count_observed_turn_events,
                positive_max_turns,
                stamp_attempt_evidence,
            )

            requested_max_turns = positive_max_turns(kwargs.get("max_turns"))
            max_turns_flag, enforced_max_turns = _codex_cli_max_turns(
                extra_args,
                requested_max_turns if requested_max_turns is not None else self.max_turns,
            )
            cmd = [
                *jail_prefix,
                codex_bin,
            ...
            return stamp_attempt_evidence(
                result,
                # Host stdout only. The worker -o JSON is authorable and must
                # not mint tool-call evidence.
                tool_call_count=count_observed_tool_events(completed.stdout),
                max_turns=enforced_max_turns,
                receiver_num_turns=count_observed_turn_events(completed.stdout),
            )
...
def _normalize_flat_usage(usage: dict[str, Any]) -> dict[str, Any]:
    ...
    # implementation note D5/R2-L04: vendor total_tokens if present, else
    # input + output + reasoning. cached_input_tokens is a subset of input —
    # never added again.
