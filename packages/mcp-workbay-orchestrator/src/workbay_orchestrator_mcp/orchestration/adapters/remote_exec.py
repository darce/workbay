...
_RC_GROK_FAILED = 3
_RC_NO_CHANGES = 4
_RC_ADMISSION_DEFERRED = 75  # VM admission (mem floor or lane cap); retryable (S3b/S5)
        ...
        **kwargs: Any,
        ...
        with tempfile.TemporaryDirectory(prefix="remote-exec-") as tmpdir:
            ...
            if rc == _RC_ADMISSION_DEFERRED:
                # VM admission fired (remote_agent.sh exit 75): memory floor or
                # concurrent lane cap. Retryable backpressure defer ([RES-14]),
                ...
                # recover via a fresh re-dispatch when the VM has headroom.
                ...
                if "residual timeout exhausted" in stderr_l:
                    # Third exit-75 cause (0144 R4): slow transport ate the turn
                    # budget before grok started. NOT a memory diagnosis — a
                    # false vm_memory_pressure here sends the operator hunting
                    # VM RAM when the fix is budget/transport (r0716789b S1A-01).
                    return _remote_unavailable_result(
                        ...
                        raw_payload={
                            "admission_deferred": True,
                            "defer_reason": "residual_timeout_exhausted",
                        },
                    )
                return _remote_unavailable_result(
