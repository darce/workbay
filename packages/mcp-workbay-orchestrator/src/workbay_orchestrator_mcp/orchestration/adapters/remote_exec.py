...
def patch_touches_git_control_paths(patch_text: str) -> bool:
    ...
    +<new>[,<newcount>] @@`` header consumes exactly
    ``max(oldcount, newcount)`` body lines (leading ``+``, ``-``, space, or
    ``\\``; omitted counts default to 1). On the line *after* that budget is
    exhausted, ``in_hunk`` is cleared so a following traditional
    ``---`` / ``+++`` / ``diff --git`` / rename / copy header is re-classified.
    A line that starts with ``@@`` but does not parse as a hunk header is
    fail-closed (treated as control-touching). A hunk body that merely
    *renders* like ``--- .gitattributes`` or ``+++ b/.gitattributes`` is
    not a file header.
    """
    if not patch_text:
        return False
    ...
    for line in patch_text.split("\n"):
        if in_hunk:
            if hunk_remaining > 0 and _is_hunk_body_line(line):
                hunk_remaining -= 1
                if hunk_remaining == 0:
                    in_hunk = False
                continue
        ...
        if line.startswith("@@"):
            ...
            if parsed is None:
                return True
            hunk_remaining = max(parsed[0], parsed[1])
            in_hunk = hunk_remaining > 0
            continue
...
def _apply_and_commit(worktree_path: Path, patch_file: Path, *, lane_id: str, summary: str) -> None:
    ...
    # Read above apply so a failure cannot leave the index staged. An unreadable
    # patch is quarantine (fail-closed): never treat "could not read" as "no
    # control path → proceed to apply".
    try:
        grounding: tuple[str, ...] = (read_patch_text(patch_file),)
    except OSError as exc:
        raise RuntimeError("quarantined: patch touches git control paths") from exc
...
def _token_usage_from_debug_log(debug_file: Path) -> dict[str, Any] | None:
    ...
    * ``_meta.usage``  -- cumulative session totals, identified by ``modelCalls``
    ...
    if cumulative is not None:
        ...
        rec["degraded"] = (
            "no cumulative _meta.usage in log; undercounts multi-call turns"
        )
...
#: Operator override for the writable_roots live probe. Distinct from
...
def writable_roots_preflight_argv() -> list[str]:
    """Exact ``codex exec`` sandbox flags the live writable_roots probe uses."""
    return [
        "-s",
        LANE_SANDBOX,
        "-c",
...
def _default_writable_roots_sandbox_exec(
    ...
    *,
    ...
    """Run ``codex exec`` with *argv* so a git commit can land in *repo*.

    Fail-closed: missing flags, missing binary, timeout, or spawn error → rc=1
    and no commit. Tests inject an honoring/ignoring ``sandbox_exec`` instead
    of requiring a live Codex seatbelt.
    """
    joined = " ".join(argv)
    if (
        "-s" not in argv
        ...
        return subprocess.CompletedProcess(
            argv, 1, "", "writable_roots probe argv missing sandbox flags"
        )
    codex = shutil.which("codex")
    if not codex:
        return subprocess.CompletedProcess(
            argv, 1, "", "codex binary not found for writable_roots probe"
        )
    cmd = [
        codex,
        "exec",
...
def probe_writable_roots_git_commit(
    ...
    *,
    ...
    Fail-closed: any exception, nonzero exec, or missing HEAD is False.
    """
...
def resolve_effective_model(row: BackendSpec, backend_id: str, model: str | None) -> str:
    ...
    if pin is None:
        ...
        if not effective_model:
            raise RuntimeError(
                f"Refusing {backend_id} dispatch: allowed_model unset on BackendSpec (pin required)"
            )
    ...
    if _GROK_BUILD_RE.search(effective_model):
        raise RuntimeError(
            f"Refusing {backend_id} dispatch with a build model "
            f"'{effective_model}' (grok-build family refused)."
        )
    if retired_warning is not None:
        raise RuntimeError(
            f"Refusing {backend_id} dispatch with retired model "
            f"'{effective_model}'. {retired_warning}"
        )
    ...
    if effective_model not in allowed_models:
        ...
        if allowed_models == frozenset({pin}):
            ...
            allowed_clause = (
                f"allowed is the curated allow-list including configured "
                f"pin '{pin}'{env_hint}."
            )
        raise RuntimeError(
            f"Refusing {backend_id} dispatch with model "
            f"'{effective_model}': {allowed_clause}"
        )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            def _envelope(result: BackendResult) -> BackendResult:
                ...
                try:
                    stamped = _stamp_backend_result_token_usage(
                        stamped, debug_file=debug_file
                    )
            ...
            if agent_spec.requires_live_sandbox_preflight:
                ...
                workspace_ok = bool(kwargs.get("sandbox_preflight_ok")) or (
                    run_env.get(SANDBOX_PREFLIGHT_ENV) == "1"
                )
                    ...
                    if roots_override:
                        ...
                        try:
                            if probe_fn is not None:
                                ...
                                roots_ok = bool(
                                    probe_writable_roots_git_commit(worktree_path)
                                )
                ...
                if WRITABLE_ROOTS_REQUIRES_LIVE_PREFLIGHT and not roots_ok:
                    blockers.append(
                        ...
                        "-s workspace-write -c sandbox_workspace_write.writable_roots=['.git'] "
                        "did not land. Re-run the probe or pass "
                        "writable_roots_preflight_ok=True / "
                        f"{WRITABLE_ROOTS_PREFLIGHT_ENV}=1 after a fresh "
                        "writable_roots commit probe [FM-08]"
                    )
                ...
                if blockers:
                    return _envelope(
                        BackendResult(
                            handoff_action="needs_guidance",
                            summary=(
                                f"{self.backend_id} refused: live sandbox preflight required"
                            ),
                            details="",
                            merge_ready=False,
                            blockers=blockers,
            ...
            # untyped apply/commit crash. Genuine apply/commit failures re-raise.
            ...
            try:
                ...
                return _envelope(
                    BackendResult(
                        handoff_action="needs_guidance",
                        summary=(
                            f"{self.backend_id} refused: remote patch touches git control paths"
                        ),
