...
def _probe_receipt_result(
    ...
    *,
    ...
    elapsed = record.get("elapsed_s", fallback_elapsed_s)
    remote_count = record.get("live_probe_count", live_probe_count)
    if type(remote_count) is not int or remote_count < 0:
        remote_count = live_probe_count
    try:
...
def _request_vm_probe(
    ...
    *,
    ...
    probe_env.setdefault(PREFLIGHT_PROBE_MODEL_ENV, TRACKED_CODEX_MODEL)
    try:
    ...
    try:
        ...
        cmd = [str(script), kind, "--timeout", str(max(1, int(timeout)))]
    ...
    *,
    ...
    with _PREFLIGHT_FLIGHT_LOCK:
        ...
        if flight is None:
            ...
            owner = False
    if not owner:
        if not flight.event.wait(timeout=max(1, int(timeout))):
            return _unknown_preflight_pair("timeout")
    ...
    durable_deadline = time.monotonic() + max(1, int(timeout)) + 5
    try:
        ...
        if existing_result is not None and existing.get("status") == "complete" and existing_cooldown > time.time():
            ...
            try:
                ...
                if durable_fd is not None:
                    try:
                        ...
                        pass
                    durable_fd = None
                logger.warning("preflight durable flight degraded to process-local coordination", exc_info=True)
            if durable_fd is not None:
                try:
            ...
            if outcome[0].reason == "transport_failure" and outcome[1].reason == "transport_failure":
                # The transport helper owns a fixed five-second SSH slack;
                # include it in this one total deadline, not two turn budgets.
                roots = request_vm_writable_roots_probe(
                    worktree,
                    script=script,
                    env={**env, "WORKBAY_PROBE_NONCE": durable_nonce},
                ...
                if roots.verdict == "positive":
                ...
                elif time.monotonic() >= durable_deadline:
                    ...
                    remaining = max(1, int(durable_deadline - time.monotonic()))
        ...
        if flight_record_path is not None and durable_fd is not None:
            try:
