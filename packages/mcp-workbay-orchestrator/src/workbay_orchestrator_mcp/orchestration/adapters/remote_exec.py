        ...
        **kwargs: Any,
        ...
        if _local_dirty(worktree_path):
            return BackendResult(
                ...
                details=_refusal_details(
                    "grok-remote refused before dispatch: the lane worktree has uncommitted "
        ...
        try:
            ...
            if rc == _RC_RATE_LIMITED:
                ...
                if patch_touches_git_control_paths(patch_text):
                    ...
                    return _envelope(
                        BackendResult(
                            handoff_action="needs_guidance",
                            summary="grok-remote rate-limited (salvage patch quarantined)",
                            details=detail,
                            merge_ready=False,
                            blockers=[
                ...
                if patch_has_product and skip_rc4_salvage:
                    ...
                    try:
                        ...
                        return _envelope(
                            BackendResult(
                                handoff_action="needs_guidance",
                                summary="grok-remote produced recoverable work but host salvage apply failed",
                    ...
                    try:
                        ...
                        return _envelope(
                            BackendResult(
                                handoff_action="needs_guidance",
                                summary=(
                                    "grok-remote produced recoverable work but host salvage listing failed"
                                ),
                                details=apply_detail,
                                merge_ready=False,
                                blockers=_exit4_refusal_blockers(
            ...
            if rc == _RC_BOUND_EXPIRED:
                ...
                if salvage_patch_cause == "patch_empty":
                    ...
                    summary = (
                        "grok-remote unattended re-dispatch exhausted after repeated "
                        "wall-clock expiry with no salvageable patch"
                        if exhausted
                        else "grok-remote wall-clock bound expired on a reachable VM (no salvageable patch)"
                    )
                    blockers = [
                        (
