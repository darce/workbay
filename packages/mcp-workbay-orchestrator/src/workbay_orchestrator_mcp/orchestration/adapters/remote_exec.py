...
def _rate_limit_detail_from_stream(stream_file: Path) -> str | None:
    ...
    for index in range(len(events) - 1, -1, -1):
        ...
        if event_type not in {"error", "turn.failed"} and not wrapped_error:
            continue
        if any(
            isinstance(later.get("type"), str) and later["type"].startswith("turn.")
            for _, later in events[index + 1 :]
        ):
            continue
...
def _transport_failure_result(
    *,
    ...
    msg = (
        f"{backend_id} remote provider transport failure (exit {exit_code}): "
        f"{stable_code}: {log_detail}"
    )
    logger.error(msg)
...
def _rate_limited_result(
    *, detail: str, reset: str | None, model: str, effort: str | None, reset_header: str | None = None
) -> BackendResult:
    payload: dict[str, Any] = {
    ...
    return BackendResult(
        ...
        summary="remote provider refused the turn because its quota is exhausted",
        details=detail,
        merge_ready=False,
...
def _staged_changed_files(worktree_path: Path) -> list[str]:
    ...
    if res.returncode != 0:
        ...
        raise RuntimeError(
            f"git diff --cached --name-only failed: {stderr.strip()[-500:] or 'no stderr'}"
        )
    return [
        path.decode("utf-8", errors="surrogateescape")
        for path in res.stdout.split(b"\0")
        if path
    ]
    ...
    *,
    ...
    try:
        ...
        if spool_tmpdir_fallback:
            detail += (
                "; the volatile TMPDIR spool will not exist after return "
                "(result_path is not the salvage copy)"
            )
...
def _preserve_salvage_patch(worktree_path: Path, patch_file: Path, *, lane_id: str) -> Path:
    ...
    if git_path.returncode != 0 or not destination_dir_text:
        raise RuntimeError(
            "could not resolve durable Git salvage directory: "
            f"{git_path.stderr.strip()[-500:] or 'no stderr'}"
        )
...
def _apply_and_commit(
    ...
    *,
    ...
    message = (
        build_remote_turn_commit_message(lane_id=lane_id, summary=summary, grounding=grounding)
        if commit
        else ""
    )
...
def _eager_terminate_probe_group(pgid: int) -> None:
    """Immediate SIGTERM of *pgid*; no grace wait.

    Called only from the ``TimeoutExpired`` arm of
    ``_default_writable_roots_sandbox_exec``. Does not replace ``finally``.

    Refusing the caller's own process group is fail-closed insurance against
    a caller passing the wrong integer. It is not the reason the production
    path is safe — the spawn ordering is: the parent does not return from
    ``Popen.__init__`` until the child has already called ``setsid()``.
    """
    if pgid <= 0:
        return
    if pgid == os.getpgrp():
        logger.warning(
            "refusing probe kill of caller's own process group pgid=%s",
            pgid,
        )
        return
    try:
...
def _reap_probe_process_group(pgid: int) -> None:
    """SIGTERM the process group, wait 2 s, then SIGKILL.

    ``ProcessLookupError`` is success (already gone). Do not SIGKILL first.
...
def _default_writable_roots_sandbox_exec(
    ...
    *,
    ...
    try:
        ...
        pgid = None
    if on_spawn is not None and pgid is not None:
        on_spawn(pgid)
    try:
        stdout, stderr = proc.communicate(timeout=max(1, timeout - 2))
    except subprocess.TimeoutExpired as exc:
        if pgid is not None:
            _eager_terminate_probe_group(pgid)
        return subprocess.CompletedProcess(cmd, 1, "", str(exc))
    except OSError as exc:
        return subprocess.CompletedProcess(cmd, 1, "", str(exc))
    ...
    *,
    ...
    try:
        ...
        for pgid in recorded:
            _reap_probe_process_group(pgid)
...
class RemoteExecAdapter(BackendAdapter):
    ...
    def __init__(self, *args: Any, backend_id: str = "grok-remote", **kwargs: Any) -> None:
        ...
        if transport_timeout is not None and row.capabilities.supports_adapter_timeout_bounds:
            ...
            self._transport_timeout = int(
                backend_spec_mod.resolve_remote_lane_timeout_s(backend_id)
            )
        else:
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            spec_effort = default_effort_for_model(self.backend_id, effective_model) if applied_effort is None else applied_effort
            ...
            try:
                ...
                return _envelope(
                    BackendResult(
                        handoff_action="needs_guidance",
                        summary=(
                            f"{self.backend_id} refused: host recipe "
                            "required-placeholder set is empty"
                        ),
                        details=_refusal_details(
                ...
                if blockers:
                    return _envelope(
                        BackendResult(
                            ...
                            details=_refusal_details(
                                f"{self.backend_id} refused before dispatch: the live sandbox "
            ...
            if rc == _RC_RATE_LIMITED:
                ...
                salvage_contamination = (
                    _detect_grok_build_contamination(debug_file, requested_model=effective_model)
                    if self.backend_id == "grok-remote"
                    else None
                )
                ...
                if apply.returncode != 0:
                    ...
                    return _envelope(
                        BackendResult(
                            handoff_action="rate_limited",
                            summary=(
                                "remote provider refused the turn because its quota is exhausted "
            ...
            if rc == _RC_NO_CHANGES:
                ...
                patch_blocker = (
                    f"salvage patch invalid: {patch_apply_diagnosis}"
                    if patch_apply_diagnosis
                    else ""
                )
                rc4_payload = (
                ...
                if patch_has_product and skip_rc4_salvage:
                    ...
                    logger.warning(
                        "refusing exit-4 salvage for findings-only review turn; patch left unapplied"
                    )
                    ...
                    try:
                        ...
                        apply_detail = (
                            f"{patch_salvage_cause}: {listing_exc}; {preservation_detail}"
                        )
                        logger.warning("rc-4 salvage staged listing failed: %s", apply_detail)
                        if not _index_observably_staged(worktree_path):
                            logger.warning(
                                "rc-4 salvage listing failed and staged index was not observable"
                            )
                    ...
                    if patch_apply_diagnosis and not patch_salvage_cause:
                        rc4_refusal_head = (
                            f"salvage patch invalid: {patch_apply_diagnosis}; {preservation_detail}"
                        )
                    elif patch_apply_diagnosis:
                        rc4_refusal_head = (
                            f"{patch_salvage_cause}: {patch_apply_diagnosis}; {preservation_detail}"
                        )
                    else:
            ...
            if rc == _RC_GROK_FAILED:
                ...
                if patch_text.strip():
                    ...
                    if sv_outcome is not None:
                        salvage_payload["self_verify_outcome"] = sv_outcome
                    stream_detail = (
                        _rate_limit_detail_from_stream(stream_file) if not result_file.is_file() else None
                    )
            ...
            if rc == _RC_BOUND_EXPIRED:
                ...
                if salvage_patch_cause == "patch_empty":
                    ...
                    summary = (
                        f"{self.backend_id} wall-clock bound expired on a reachable VM (host failed to read salvage patch)"
                    )
                    ...
                    summary = f"{self.backend_id} wall-clock bound expired on a reachable VM (salvage patch absent on host)"
                    blockers = [
                        "remote wall-clock bound expired (remote_agent.sh exit 8) on a "
            ...
            if contamination is not None:
                ...
                contam_payload: dict[str, Any] = _typed_payload({
                    "composer_violation_evidence": evidence,
