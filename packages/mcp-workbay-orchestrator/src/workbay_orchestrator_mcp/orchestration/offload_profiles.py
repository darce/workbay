...
def derive_adapter_timeout_bounds(
    ...
    *,
    ...
    unchanged. Budget-only derivation cannot exceed
    ``GROK_MAX_TURNS_CAP * SECONDS_PER_TURN = 1800``; a codex lane reaches
    ``CODEX_TIMEOUT_CAP = 3600`` only through ``requested_timeout``. Raising
    the ceiling past that saturation point buys wall-clock only when a
    request is supplied.

    ``timeout_saturated`` is True when the turns term, not the cap, bounded
    the budget-only clock. A ``requested_timeout`` above the cap is refused
    with ``ValueError`` (never silently clamped). Without a request the
    ...
    turns_term = max(MIN_TIMEOUT_SECONDS, notional_turns * SECONDS_PER_TURN)
    timeout = min(cap, turns_term)
    timeout_saturated = turns_term < cap
    if requested_timeout is not None:
        ...
        if requested_timeout > cap:
            raise ValueError(f"requested_timeout {requested_timeout} exceeds adapter ceiling {cap}")
        return {
            "timeout": requested_timeout,
            "timeout_ceiling": cap,
            "timeout_saturated": timeout_saturated,
            "timeout_source": "requested",
        }
    return {
        "timeout": timeout,
        "timeout_ceiling": cap,
...
def derive_single_cycle_bounds(
    ...
    *,
    ...
    its own grok ceilings. ``requested_timeout`` is forwarded only to the
    adapter-timeout branch.
    """
    if bound_kind == BOUND_GROK_DERIVED:
        return derive_grok_single_cycle_bounds(token_budget)
    if bound_kind == BOUND_ADAPTER_TIMEOUT:
        return derive_adapter_timeout_bounds(
            token_budget,
...
class OffloadAgentProfile:
    ...
    set; Codex includes max, OpenRouter uses its per-model advertised union,
    and other profiles retain STANDARD_WORKER_REASONING_EFFORT_CHOICES.
