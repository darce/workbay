...
OFFLOAD_AGENT_PROFILES: dict[str, OffloadAgentProfile] = {
    ...
    # catalogue probe).
    # Efforts: the advertised subset only (S4-M-05). "auto" is excluded because
    # its scorer may pick medium/xhigh, which build_agent_spec would refuse at
    # dispatch; "inherit" resolves to the adapter default ("high"), inside the set.
    "0xalpha-remote": OffloadAgentProfile(
        agent="0xalpha-remote",
        allowed_efforts=tuple(
            e for e in WORKER_REASONING_EFFORT_CHOICES if e == "inherit" or e in OXALPHA_ALLOWED_EFFORTS
        ),
        single_cycle_bound=BOUND_ADAPTER_TIMEOUT,
        pinned_model=DEFAULT_0XALPHA_MODEL,
