...
# Lane worktree HEAD vs primary root HEAD: a second measurement, not a stale
# verdict. Reindex of the primary cannot clear it; structural excerpts come
# from the index and may predate lane commits.
CODEMAP_LANE_DIVERGENCE_NOTE = (
    "codemap_lane_divergence: lane worktree HEAD differs from the indexed primary root; "
    "structural excerpts may predate lane commits (not cleared by reindex)"
)
...
def run_codemap_cli(
    ...
    *,
    ...
    # codebase-memory-mcp 0.10.8 emits allocator diagnostics followed by its
    # error envelope on stderr when a project is not indexed.  Preserve that
    # typed envelope so the freshness gate can recognize project absence and
    # try the primary checkout.  Do not use the raw stderr prefix as the
    # diagnostic: the log lines can consume the useful-message cap.
    stderr_data = _extract_json_object(stderr) if data is None else None
    if data is None and isinstance(stderr_data, dict) and "error" in stderr_data:
        data = stderr_data
        message = _error_message(data)
        if tool == _TOOL_INDEX_STATUS:
            return {
                "ok": False,
                "data": data,
                "error": f"index_status_failed:{message}",
                "cli_path": resolved,
            }
        return {
            "ok": False,
            "data": data,
            "error": f"{CODEMAP_SECTION_OMITTED_PREFIX}{tool}:exit={completed.returncode}:{message}",
            "cli_path": resolved,
        }
    ...
    if tool == _TOOL_INDEX_STATUS and completed.returncode != 0:
        hint = _clean_cli_stderr_hint(stderr) or f"exit={completed.returncode}"
        return {
            "ok": False,
            "data": data,
    ...
    if data is None:
        # Some CLIs print log lines to stdout; fall back to a cleaned stderr
        # tail for error text.  Log-only output is not a useful error message.
        hint = _clean_cli_stderr_hint(stderr or stdout) or f"exit={completed.returncode}"
        return {
            "ok": False,
            "data": None,
...
def _clean_cli_stderr_hint(text: str) -> str:
    """Return the useful non-log tail from CLI diagnostics.

    The real codemap CLI prefixes allocator/status chatter with ``level=``.
    Those lines are operational noise and must not crowd the 200-character
    error-message cap used by normalized CLI results.
    """
    lines = []
...
def _extract_json_object(text: str) -> dict[str, Any] | None:
    """Parse the first JSON object from CLI output (skip log lines)."""
...
def _result_is_project_not_indexed(result: Mapping[str, Any]) -> bool:
    ...
    # Transport diagnostics and malformed stdout cannot establish absence.
    ...
    *,
    ...
    Returns::

        {
          ...
          "lane_divergent": bool,           # lane HEAD vs primary HEAD (not stale)
        }
    ...
    # Lane-vs-primary is a separate measurement from index-vs-primary stale.
    # Missing or short shas → False (no claim; do not invent incomparable).
    lane_divergent = bool(
        head_sha
        and primary_head_sha
        and len(str(head_sha).strip()) >= _MIN_SHA_COMPARE_LEN
        and len(str(primary_head_sha).strip()) >= _MIN_SHA_COMPARE_LEN
        and not _sha_equal(str(head_sha), str(primary_head_sha))
    )
    ...
    if not detect_determined:
        ...
        cause = (
            "freshness_budget_exhausted"
            if error == "freshness_budget_exhausted"
            else "detect_changes_timeout"
    ...
    if lane_divergent:
        note = f"{note}; {CODEMAP_LANE_DIVERGENCE_NOTE}" if note else CODEMAP_LANE_DIVERGENCE_NOTE
...
def _semantic_rows(data: Mapping[str, Any]) -> list[dict[str, Any]] | None:
    ...
    if isinstance(semantic, list):
        ...
        if not isinstance(rows, list) or not isinstance(cols, list) or not all(isinstance(col, str) for col in cols):
            return None
        raw_rows = []
        for row in rows:
...
def _dedupe_related_prior_rows(result: Mapping[str, Any]) -> dict[str, Any]:
    """Collapse repeated prior-art snippets while preserving score order."""
    ...
    for row in rows:
        ...
        key = (entity_kind, snippet)
        first = first_by_key.get(key)
        if first is None:
            first = dict(row)
            first_by_key[key] = first
            deduped.append(first)
            continue
        entity_id = str(row.get("entity_id") or "").strip()
        if entity_id:
            duplicate_ids.setdefault(key, []).append(entity_id)

    for key, ids in duplicate_ids.items():
        first = first_by_key[key]
        existing_also = first.get("also")
        also_ids = [str(item).strip() for item in existing_also] if isinstance(existing_also, list) else []
        also_ids.extend(ids)
        also_ids = [item for item in also_ids if item]
        if not also_ids:
            continue
        first["also"] = also_ids
        snippet = str(first.get("snippet") or "").strip()
        suffix = f"also: {', '.join(also_ids)}"
        first["snippet"] = f"{snippet} {suffix}".strip()

    normalized = dict(result)
    normalized["results"] = deduped[:RELATED_PRIOR_LIMIT]
    return normalized
...
def _lookup_related_prior(*, query_text: str, task_ref: str) -> dict[str, Any]:
    ...
    if not isinstance(result, dict):
        return {"status": "error", "results": [], "note": "related_prior:error"}
    return _dedupe_related_prior_rows(result)
...
def _path_identity(path: Any) -> str:
    """Normalize only path spelling needed to compare codemap hit paths."""
...
def _sibling_hits(
    ...
    *,
    ...
    """Select unique same-symbol definitions outside the target file."""
    target_key = _path_identity(target_path)
    unique: list[dict[str, Any]] = []
    seen: set[tuple[str, str]] = set()
    for raw_hit in _pick_search_hits(data, limit=20):
        hit = dict(raw_hit)
        hit_path = str(hit.get("path") or "").strip()
        if not hit_path:
            continue
        hit_symbol = str(hit.get("symbol") or symbol).strip()
        if hit_symbol != symbol:
            continue
        if target_key and _path_identity(hit_path) == target_key:
            continue
        key = (_path_identity(hit_path), hit_symbol)
        if key in seen:
            continue
        seen.add(key)
        hit["symbol"] = hit_symbol
        hit["anchor"] = f"{hit_path}:{hit_symbol}"
        unique.append(hit)
    return unique[:SIBLING_LIMIT], max(0, len(unique) - SIBLING_LIMIT)
...
def build_lane_context_packet(
    *,
    ...
    def mark_packet_empty(*, codemap_available: bool) -> None:
        """Expose a typed cause when structural and related channels are empty."""
        if sections["anchors"]:
            return
        related = sections.get("related_prior")
        related_rows = related.get("results") if isinstance(related, Mapping) else None
        if isinstance(related_rows, list) and related_rows:
            return

        freshness_note = str(sections["meta"].get("freshness_note") or "").lower()
        if "project not found" in freshness_note and "not indexed" in freshness_note:
            cause = "codemap_project_not_indexed"
        elif not normalized_targets:
            cause = "targets_none"
        elif budget_exhausted:
            cause = "budget_exhausted"
        elif not codemap_available:
            cause = "codemap_unavailable"
        else:
            # Structural failures are advisory and do not have a more specific
            # empty-packet cause than an unavailable codemap response.
            cause = "codemap_unavailable"
        sections["meta"]["packet_empty_cause"] = cause
        note = f"packet_empty:{cause}"
    ...
    if not resolved_cli:
        ...
        mark_packet_empty(codemap_available=False)
        if related_query:
    ...
    sections["meta"]["lane_divergent"] = bool(freshness.get("lane_divergent"))
    ...
    if not normalized_targets:
        ...
        mark_packet_empty(codemap_available=bool(freshness.get("available")))
        packet = _render_packet(sections, notes=notes)
        packet, truncated = _cap_packet(packet, max_bytes=max_bytes)
        return {
            "available": True,
    ...
    # --- sibling definitions via exact-name search_graph (advisory) ---
    sibling_targets: list[tuple[str, str | None]] = []
    seen_sibling_targets: set[tuple[str, str | None]] = set()
    for target in normalized_targets:
        symbol = str(target.get("symbol") or "").strip()
        if not symbol:
            for hit in sections["anchors"]:
                if hit.get("path") == target.get("path") and hit.get("symbol"):
                    symbol = str(hit["symbol"]).strip()
                    break
        if not symbol:
            continue
        target_path = str(target.get("path") or "").strip() or None
        request_key = (symbol, _path_identity(target_path) or None)
        if request_key in seen_sibling_targets:
            continue
        seen_sibling_targets.add(request_key)
        sibling_targets.append((symbol, target_path))

    for sibling_index, (symbol, target_path) in enumerate(sibling_targets):
        sibling = _budgeted_call(
            _TOOL_SEARCH_GRAPH,
            {
                "project": proj,
                "name_pattern": symbol,
        ...
        if sibling is None:
            remaining_symbols = len(sibling_targets) - sibling_index
            notes.append(f"{CODEMAP_SECTION_OMITTED_PREFIX}siblings:budget_exhausted:{remaining_symbols}")
            break
        group: dict[str, Any] = {
            "symbol": symbol,
            "results": [],
        ...
        if not sibling["ok"] or not isinstance(sibling.get("data"), Mapping):
            # Sibling definitions are advisory: retain the empty group and
            # record the typed omission without failing packet construction.
            notes.append(sibling.get("error") or f"{CODEMAP_SECTION_OMITTED_PREFIX}siblings:{symbol}")
        else:
            results, more = _sibling_hits(sibling["data"], symbol=symbol, target_path=target_path)
            group["results"] = results
            group["more"] = more
        sections["siblings"].append(group)
...
def _render_packet(sections: Mapping[str, Any], *, notes: Sequence[str]) -> str:
    ...
    lines.extend(["", "### Sibling definitions (same symbol, other files)"])
    siblings = sections.get("siblings") or []
    if isinstance(siblings, list):
        for group in siblings:
            if not isinstance(group, Mapping):
                continue
            symbol = str(group.get("symbol") or "?").strip() or "?"
            lines.append(f"- {symbol}")
            results = group.get("results") or []
            rendered_group = 0
            if isinstance(results, list):
                for hit in results[:SIBLING_LIMIT]:
                    if not isinstance(hit, Mapping):
                        continue
                    path = str(hit.get("path") or "").strip()
                    hit_symbol = str(hit.get("symbol") or symbol).strip() or symbol
                    if not path:
                        continue
                    lines.append(f"  - {path}:{hit_symbol}")
                    rendered_group += 1
            if rendered_group == 0:
                lines.append("  - (none)")
            more = group.get("more")
    ...
    if isinstance(related, Mapping):
        ...
        if isinstance(hits, list):
            for hit in hits[:RELATED_PRIOR_LIMIT]:
                ...
                also = hit.get("also")
                also_ids = [str(item).strip() for item in also] if isinstance(also, list) else []
                also_ids = [item for item in also_ids if item]
                also_suffix = f"also: {', '.join(also_ids)}" if also_ids else ""
                if also_suffix and also_suffix not in snippet:
                    snippet = f"{snippet} {also_suffix}".strip()
                if len(snippet) > RELATED_PRIOR_SNIPPET_MAX_CHARS:
                    if also_suffix:
                        suffix = f" {also_suffix}"
                        if len(suffix) >= RELATED_PRIOR_SNIPPET_MAX_CHARS:
                            snippet = suffix.strip()[-RELATED_PRIOR_SNIPPET_MAX_CHARS:]
                        else:
                            room = RELATED_PRIOR_SNIPPET_MAX_CHARS - len(suffix)
                            base = snippet.split(also_suffix, 1)[0].rstrip()
                            snippet = f"{base[:room].rstrip()}{suffix}"
                    else:
    ...
    if stale is True:
        ...
        if not indexed_sha_readable:
            ...
            if meta.get("lane_divergent"):
                lane_sha = meta.get("worktree_head_sha")
                lines.append(
                    "Index freshness: fresh for primary root "
                    f"{indexed_sha or 'unknown'}; lane worktree at "
                    f"{lane_sha or 'unknown'} differs — structural excerpts "
                    "may predate lane commits."
                )
            else:
        ...
        cause in freshness_note
        for cause in ("freshness_budget_exhausted", "detect_changes_timeout", "detect_changes_failed")
    ):
