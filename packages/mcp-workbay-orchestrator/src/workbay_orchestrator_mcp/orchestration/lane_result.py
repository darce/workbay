...
Turn count is read only from ``receiver_num_turns``.  The daemon passes the
receiver-owned execute cap explicitly to the final handoff boundary; payload
copies of ``max_turns`` are fallback metadata only when no explicit cap exists.
"""
...
# Format-class blanks: Unicode whitespace (\s, including NBSP/EM SPACE), all
# ASCII C0 controls, plus Cf/format controls that are not \s (ZWSP/ZWNJ/ZWJ/
# BOM, SHY, CGJ, ALM, MVS, LRM/RLM, bidi embeddings/overrides, WJ and the
# rest of U+2060..U+206F except the unassigned U+2065 hole). jsonschema
# `pattern` is Python re here. Strip this class, then apply the substance
# minimum (20 for prose/blockers, 3 for tests_run command items). `\S` alone
# is not enough: U+200B is not \s, and non-whitespace C0 controls are not text.
_C0_CONTROLS = "".join(chr(codepoint) for codepoint in range(0x20))
_FORMAT_CONTROLS = (
    "\u00ad"  # SOFT HYPHEN
...
_FORMAT_BLANKS = "\\s" + _C0_CONTROLS + _FORMAT_CONTROLS
...
class DegenerateHandoffError(RuntimeError):
    ...
    outcome = "degenerate_no_attempt"

    def __init__(self, reason: DegenerateHandoffReason, requirement: str) -> None:
        self.reason = reason
        self.requirement = requirement
        self.outcome = "degenerate_no_attempt"
        self.degenerate_reason = reason.value
        super().__init__(f"degenerate lane handoff ({reason.value}): {requirement}")


class ContentInvalidHandoffError(RuntimeError):
    """A saved result is deterministically unacceptable to the handoff sink."""

    def __init__(self, reason: str) -> None:
        self.reason = reason
        super().__init__(f"content-invalid lane handoff: {reason}")
...
def adapter_refusal_metadata(raw_payload: Any) -> dict[str, Any] | None:
    """Read constructor-owned refusal provenance at the durable boundary."""
    if not isinstance(raw_payload, dict) or raw_payload.get("adapter_refusal") is not True:
        return None
    cause = raw_payload.get("refusal_cause")
    retryable = raw_payload.get("retryable")
    if not isinstance(cause, str) or not cause.strip() or not isinstance(retryable, bool):
        return None
    return {"kind": "refused", "refusal_cause": cause, "retryable": retryable}
...
def _is_typed_transport_result(result: dict[str, Any]) -> bool:
    ...
    if isinstance(original_action, str) and original_action in {"rate_limited", "transport_failure"}:
        return True
    ...
    return isinstance(raw_payload, dict) and (
        ...
        or raw_payload.get("preflight_required") is True
        # OBS-08: an adapter rescue uncertainty is a typed pause, not worker evasion.
        or raw_payload.get("salvage_state") == "salvage_unknown"
    )
...
def _has_structured_output_error_with_placeholder(result: dict[str, Any]) -> bool:
    ...
    raw_payload = _raw_payload(result)
    structured = (
        raw_payload["structuredOutput"] if "structuredOutput" in raw_payload else result.get("structuredOutput")
    )
...
def _has_nonempty_changed_files(result: dict[str, Any]) -> bool:
    """A non-empty changed_files harvest is lifecycle evidence; absence is not."""
    for candidate in (result.get("changed_files"), _raw_payload(result).get("changed_files")):
        if isinstance(candidate, list) and any(isinstance(item, str) and item.strip() for item in candidate):
            return True
    return False


def _attempt_evidence_absence_reason(result: dict[str, Any]) -> str:
    """Name the lifecycle facts that were absent (CARD-07)."""
    parts: list[str] = []
    num_turns = _payload_num_turns(result)
    if num_turns is None or num_turns <= 1:
        parts.append("num_turns<=1")
    tool_call_count = _payload_tool_call_count(result)
    if tool_call_count is None or tool_call_count == 0:
        parts.append("tool_call_count==0")
    if not _has_nonempty_changed_files(result):
        parts.append("no changed files")
    return ", ".join(parts)
...
def _has_attempt_evidence(result: dict[str, Any]) -> bool:
    ...
    Attempt evidence is lifecycle evidence only: ``num_turns > 1``, a counted
    positive ``tool_call_count``, or a non-empty ``changed_files`` harvest.
    A schema-shaped envelope is never itself evidence of work. A first-turn
    counted-zero ``merge_ready`` with no changed files stays
    ``degenerate_no_attempt``.
    """
    num_turns = _payload_num_turns(result)
    ...
    if tool_call_count is not None and tool_call_count > 0:
        return True
    return _has_nonempty_changed_files(result)
...
def _resolve_max_turns(result: dict[str, Any], explicit: int | None) -> int:
    """Prefer the receiver's explicit cap, then stamped fallback metadata."""
...
def classify_degeneracy(payload: dict[str, Any], *, max_turns: int) -> DegeneracyClassification:
    ...
    merge_ready. ``degenerate_no_attempt`` is for first-turn documents with no
    adapter-owned lifecycle evidence, including a schema-shaped ``merge_ready``
    placeholder. Observed token spend and an ``unobservable`` tool-call source
    are not attempt evidence (F1 / OBS-08).
    """
    ...
    if (
        ...
        and num_turns >= cap
        and (_payload_stop_reason(payload) == "Cancelled" or _has_structured_output_error_with_placeholder(payload))
    ):
...
def _degenerate_handoff_reason(
    ...
    *,
    ...
    first-turn toolless ``merge_ready`` with no changed files is refused like
    a first-turn toolless ``needs_guidance`` (F1 / OBS-08).
    """
...
def _envelope_proves_unattempted(result: dict[str, Any]) -> bool:
    """True only when receiver facts prove first-turn, counted-zero, no files.

    Missing harvest is not proof (CARD-07 / OBS-08), so unmarked documents
    keep the hard substance rejection.
    """
    num_turns = _payload_num_turns(result)
    tool_call_count = _payload_tool_call_count(result)
    return num_turns is not None and num_turns <= 1 and tool_call_count == 0 and not _has_attempt_evidence(result)


def _raise_if_envelope_proves_degenerate(
    result: dict[str, Any],
    *,
    max_turns: int | None = None,
) -> None:
    """Raise typed degeneracy when the receiver envelope proves no attempt.

    Shared by ``_load_result`` and the shallow lane-exec prevalidator so empty
    worker prose cannot outrank a proving envelope (ENG01R3RV-001 / REF-26).
    """
    if not _envelope_proves_unattempted(result):
        return
    reason = _degenerate_handoff_reason(result, max_turns=max_turns)
    if reason is None:
        return
    raise DegenerateHandoffError(
        reason,
        "needs_guidance requires positive receiver-recorded lifecycle evidence "
        f"({_attempt_evidence_absence_reason(result)})",
    )
...
def apply_per_action_floor(result: Mapping[str, Any]) -> dict[str, Any]:
...
_apply_advisory_floors = apply_per_action_floor
...
def _load_result(path: Path, *, max_turns: int | None = None) -> dict[str, Any]:
    ...
    if (
        payload.get("handoff_action") in {"rate_limited", "transport_failure"}
        and adapter_refusal_metadata(payload.get("raw_payload")) is not None
    ):
        payload["original_handoff_action"] = payload["handoff_action"]
        payload["handoff_action"] = "needs_guidance"
    refusal = (
        adapter_refusal_metadata(payload.get("raw_payload"))
        if payload.get("handoff_action") == "needs_guidance"
        else None
    )
    if refusal is not None:
        # Adapter machine tokens remain verbatim in the source envelope; make
        # each receiver-facing blocker substantive without relaxing the floor.
        payload["blockers"] = [f"Remote execution refused: {blocker}" for blocker in payload.get("blockers", [])]
    # Adapter-owned transport markers describe a run that ended before a
    # worker authored the prose fields.  Consult them before applying worker
    # substance requirements so terse adapter diagnostics retain their typed
    # failure class and bounded retry budget.  A valid refusal envelope still
    # takes the receiver path below so its typed provenance is preserved.
    if _is_typed_transport_result(payload) and refusal is None:
        return payload
    # ENG01R2RV-001 / ENG01R3RV-001: when the receiver envelope *proves*
    # first-turn, counted zero tool calls, and no changed files, raise typed
    # degeneracy before the worker-prose substance floor. Missing harvest is
    # not proof, so unmarked malformed documents still take the hard substance
    # rejection below. Shared with the lane-exec prevalidator (REF-26); do
    # not special-case the observed stub string (DBG-10).
    if refusal is None:
        _raise_if_envelope_proves_degenerate(payload, max_turns=max_turns)
    try:
...
def validate_handoff_content(path: Path, *, max_turns: int | None = None) -> dict[str, Any]:
    """Validate immutable result bytes before deciding whether a retry can help.

    This is the daemon-facing, typed boundary for deterministic receiver
    refusals.  It deliberately delegates lifecycle classification to
    ``_load_result`` so a genuine first-turn no-attempt result keeps the same
    verdict; callers merely gain a stable way to distinguish that four-hundred-
    class content failure from a transient handoff transport failure.
    """
    try:
        result = _load_result(path, max_turns=max_turns)
    except (json.JSONDecodeError, UnicodeDecodeError, RuntimeError) as exc:
        raise ContentInvalidHandoffError(str(exc)) from exc
    ...
    if action not in {"merge_ready", "needs_guidance"}:
        raise ContentInvalidHandoffError(f"unsupported handoff_action: {action or '<missing>'}")
    if not summary:
        raise ContentInvalidHandoffError("missing summary in result payload")
    return result
