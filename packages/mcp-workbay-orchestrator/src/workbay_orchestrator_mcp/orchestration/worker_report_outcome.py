...
# Daemon-emitted terminal tokens are not PASS_OUTCOMES. Keep their declared
# compatibility mapping separate so pass vocabulary totality remains exact.
DAEMON_OUTCOME_TO_WORKER_REPORT: dict[str, str] = {
    "exec_failed": "failed",
    # Routing disagreement refuses dispatch before execution, like admission.
    "routing_refused": "stopped",
}
...
PASS_OUTCOME_TO_WORKER_REPORT: dict[str, str] = {
    ...
    # implementation note R3: a completed review lane is a clean terminal success.
    "review_complete": "finished",
    # Routing escalation landed coherently and awaits an explicit re-dispatch.
    "escalated": "stopped",
    ...
    # A provider quota stop is a resource bound running out, not a worker
    # asking for guidance — same bucket as timeout/token_budget_exceeded,
    # not "stopped" (canon AGT-21).
    "rate_limited": "exhausted",
    "transport_failure": "failed",
    # Explicit failures.
    "error": "failed",
    "self_verify_failed": "failed",
    # Zero tests executed (pytest usage error / no tests collected). Not red, but
    # no clean terminal either — same conservative bucket as self_verify_failed.
    "self_verify_inconclusive": "failed",
    "composer_violation_quarantined": "failed",
    # Green self-verified committed work whose handoff carried no genuine question.
    # Distinct from handoff_ready (verified merge-ready close); same verify-at-gate
    # bucket as needs_guidance so unreviewed work never auto-closes a slice.
    "completed_unreviewed": "stopped",
    ...
    # A hard admission refusal stops before a turn starts. Keep it out of the
    # durable failed bucket so the lane row cannot resemble an execute error.
    "admission_refused": "stopped",
    # Pre-pass dispatch gate: unusable test_cmd / starved budget. Terminal
    # failure — not the retryable admission_refused meaning.
    "dispatch_refused": "failed",
    # implementation note S2: remote_only ledger refused an explicit local backend.
    "remote_required": "failed",
    # Missing lane worktree that cannot be re-materialized.
    "worktree_unrecoverable": "failed",
    # Peer holds the rematerialize/reaping flock — retry, do not fail the lane.
    "worktree_claim_held": "stopped",
    # Work landed but handoff/reporting ceremony failed — operator must recover
    # the committed product; never auto-close as finished [OBS-08].
    "ceremony_failed": "stopped",
}
