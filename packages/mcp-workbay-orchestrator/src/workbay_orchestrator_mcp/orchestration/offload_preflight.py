...
def check_disk_headroom(
    ...
    *,
    ...
    """Refuse local dispatch when either staging volume lacks free disk space."""
    ...
    if configured_floor is None:
        ...
        floor_bytes = int(effective_policy.local_staging_disk_floor_gib * 1024**3)
    else:
        try:
    ...
    for volume in (Path(path), Path(tmpdir)):
        try:
...
def materialize_offload_lane_manifest(
    *,
    ...
    if normalized_preferred_effort in WORKER_REASONING_EFFORT_CHOICES and normalized_preferred_effort not in {
        "auto",
        "inherit",
    }:
    ...
    if preferred_speed is not None and not clear_preferred_speed:
        ...
        if normalized_preferred_speed not in profile.allowed_speeds:
            raise OffloadPreflightError(f"{preferred_backend} does not advertise speed {normalized_preferred_speed!r}")
...
def _lane_kind_from_lane_row(*, task_ref: str | None, lane_id: str | None) -> str | None:
    ...
    except Exception:  # noqa: BLE001 — kind lookup is advisory; default implement
        return None
    if not isinstance(row, dict):
        return None
...
def _token_budget_advice_payload(
    *,
    ...
    except Exception:  # noqa: BLE001 — metrics are advisory; never fail preflight
        advice = BudgetAdvice(
            floor=floor,
            sample_lanes=0,
            p50=None,
    ...
    *,
    ...
    try:
        ...
        if reasoning_effort is not None:
            reasoning_effort = require_canonical_operator_value(
                reasoning_effort, field_name="effort", lowercase=True
            )
    ...
    check_disk_headroom(resolved_worktree, cost_class=cost_class_for_backend(profile.agent))
    ...
    if backend_spec is not None and bool(getattr(backend_spec.capabilities, "dispatchable_off_box", False)):
        ...
        try:
            ...
            transport = remote_exec_mod.resolve_transport(resolved_worktree)
    ...
    if normalized_speed is not None and normalized_speed not in profile.allowed_speeds:
        ...
        raise OffloadPreflightError(
            f"agent {profile.agent!r} does not advertise speed {normalized_speed!r}; "
            f"advertised speeds: {advertised}"
        )
