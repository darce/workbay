...
def _generate_finding_id(lane_id: str | None, index: int, finding: dict[str, Any]) -> str:
    ...
    content_key = hashlib.sha256(json.dumps(content, ensure_ascii=False, separators=(",", ":")).encode()).hexdigest()[:24]
    severity_char = {"high": "H", "medium": "M", "low": "L"}.get(
        str(finding.get("severity", "")).strip().lower(), "X")
...
def _record_findings(
    ...
    *,
    ...
    while findings and (runtime.db_path.exists() or reconcile_committed):
        try:
            response = _load_mcp_payload(list_review_findings(
                task_ref=task_ref, status="all", limit=500, offset=offset,
            ))
        ...
        if not rows:
            ...
            break
        offset += len(rows)
    snapshot_state: Literal["none", "ok", "unknown"] = (
        "unknown" if legacy_cause else "ok" if legacy_rows else "none"
    )
    if legacy_cause:
        ...
        logger.warning(
            ...
            task_ref, session, lane_id, legacy_cause,
        )
    legacy_ids: set[str] = set()

    def persist_result(
        recorded_ids: list[str], *, unrecorded_ids: list[str] | None = None,
        unrecorded_findings: list[dict[str, Any]] | None = None,
        causes: dict[str, str] | None = None,
    ) -> FindingsPersistResult:
        return _persist_result(
            recorded_ids, unrecorded_ids=unrecorded_ids,
            unrecorded_findings=unrecorded_findings,
            causes={**(causes or {}), **({"legacy_reconciliation": legacy_cause} if legacy_cause else {})},
            legacy_reconciliation=("unknown" if legacy_cause else
                                   "reconciled" if legacy_ids else "none"),
        )
    ...
    ordered = sorted(findings, key=lambda f: (
        _generate_finding_id(lane_id, 0, f), json.dumps(f, sort_keys=True, ensure_ascii=False)))
    for finding in ordered:
        ...
        candidates = [row for row in legacy_rows if
            re.fullmatch(rf"{re.escape(prefix)}-{severity}-[0-9]+", str(row.get("finding_id") or ""))
            ...
            == " ".join(f"[{str(finding.get('category') or '').strip()}] {finding.get('description') or ''}".lower().split())]
        if len(candidates) == 1:
            ...
            if lane_less or not lane_id or stored_lane != lane_id:
                ...
                warnings.warn(
                    ...
                    + ("aliasing sole lane-less content match" if lane_less
                       else "keeping separate hashed finding"),
                    LegacyFindingLaneWarning, stacklevel=2,
                )
            ...
            warnings.warn(
                f"Ambiguous legacy finding {base_id}: "
                + ", ".join(sorted(row["finding_id"] for row in candidates)),
                LegacyFindingAmbiguityWarning, stacklevel=2,
            )
    ...
    def record(items: list[BatchFindingItem]) -> dict[str, Any]:
        acknowledged = [{"finding_id": item["finding_id"]} for item in items
                        if item["finding_id"] in legacy_ids]
        ...
        if reconcile_committed and snapshot_state == "ok":
            ...
            for item in items:
                rows = [row for row in legacy_rows
                        if row.get("finding_id") == item["finding_id"]]
                if any(row.get("finding_id") == item["finding_id"] and ((operation_token or row.get("session") == session) and all(
                    row.get(key) == item[key] for key in ("severity", "file_path", "description")
                )) for row in rows):
    ...
    unrecorded_ids = [
        finding_id for finding_id in recorded_ids
    ...
    *,
    ...
    if record_findings and review_status != REVIEW_SKIPPED_UNPARSEABLE and review_result_is_harvestable(output):
        ...
        try:
            ...
            output["legacy_cause"] = exc.persist.get(
                "legacy_cause", shortfall["causes"].get("legacy_reconciliation")
            )
