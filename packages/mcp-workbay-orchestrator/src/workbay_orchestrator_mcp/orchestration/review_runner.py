...
def _validate_review_result(result: Any, *, tolerant: bool = False) -> dict[str, Any]:
    ...
    When ``tolerant`` is True (grok-cli smoke review):
    - Envelope misses (non-object, missing/non-list findings, missing/empty summary)
      degrade to typed ``skipped_unparseable`` ([OBS-08]) — never hard-fail a green pass.
    - When the findings key is present as a list, salvage well-formed items and drop
      invalid ones with a note in ``raw_tail`` ([OBS-04] never information-destroying).
...
def run_review(
    *,
    ...
    tolerant = backend_name == "grok-cli"
