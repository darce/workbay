...
REVIEW_OUTPUT_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        ...
        # Absent means "ok" for backward-compatible clean reviews.
        "result_parse": {"type": "string", "enum": ["ok", "degraded"]},
    },
}
...
def review_result_is_harvestable(result: dict[str, Any] | None) -> bool:
    ...
    Absent ``result_parse`` means ``ok`` for backward-compatible clean reviews.
    """
    if not isinstance(result, dict):
        return False
    parse = result.get("result_parse", "ok")
    if parse != "ok":
        return False
...
def run_review(
    *,
    ...
    if (
        record_findings
        and review_status != REVIEW_SKIPPED_UNPARSEABLE
        and review_result_is_harvestable(output)
    ):
        if not task_ref:
