...
def classify_test_cmd(value: Any) -> tuple[str, str | None]:
    ...
    if isinstance(value, str):
        text = value.strip()
        if text == "true":
            raise ValueError("test_cmd must run verification; shell no-op 'true' is not admissible")
...
def _required_remote_test_cmd(text: str | None) -> str:
    ...
    try:
        ...
        raise _RemoteTestCmdRefuse(str(exc)) from exc
...
_PROVENANCE_ENV_NAMES: frozenset[str] = frozenset(
    {
        "PATH",
        "PYTHONHOME",
        "PYTHONEXECUTABLE",
...
def _is_provenance_env_name(name: str) -> bool:
    """Return whether a prefix assignment can rebind the approved runner."""
    return name in _PROVENANCE_ENV_NAMES or name.startswith(("LD_", "DYLD_"))
...
def _prefix_env_refusal(command: _NormalizedRemoteTestCommand) -> str | None:
    ...
    for name, value in command.prefix_env_assignments:
        if _is_provenance_env_name(name):
...
def classify_remote_test_cmd(
    ...
    *,
    ...
    Provenance-changing names always refuse because they rebind the approved
    implementation: the named set (``PATH``, ``PYTHONHOME``, interpreter and
    Node provenance keys) plus any name starting with ``LD_`` or ``DYLD_``.
    Every ``PYTHONPATH`` entry must be non-empty, repository-relative, and
    free of parent-directory escapes. Other values
