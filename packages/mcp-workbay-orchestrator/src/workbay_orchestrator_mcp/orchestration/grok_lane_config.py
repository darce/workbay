...
* Post-turn ``_detect_grok_build_contamination`` in ``adapters/grok_cli.py``
  (debug-log scan for grok-build AssistantItems only) remains the hard
  backstop. Pre-exec model allow-list on the adapter is a second backstop.
  Full Composer pin attestation was retired in implementation note S2.
...
from workbay_orchestrator_mcp.orchestration.offload_timeout_ssot import (
    GROK_TIMEOUT_CAP,  # noqa: F401  # re-exported for existing importers (worker_daemon, tests)
)
...
# Tracked pin (implementation note M3). This is the governing default in repo config —
# not `.claude/settings.local.json`. WORKBAY_GROK_MODEL remains the highest-
# precedence override. Discovery (M2) publishes onto BACKENDS[].allowed_model
# / the offload profile only; probe failure degrades here. DEFAULT_* stays
# the flock/CLI snapshot so a remote catalogue cannot leak onto local CLI.
#
# 2026-08-13: bumped grok-4.5 -> grok-4.6. grok-4.5 is retired as the vendor
# default. Verified against the live catalog on the lane VM rather than assumed
# — `grok models` reports "Default model: grok-4.6" with grok-4.6 marked
# (default) and grok-4.5 demoted to a plain legacy entry.
TRACKED_GROK_MODEL = "grok-4.6"
WORKBAY_GROK_MODEL_ENV = "WORKBAY_GROK_MODEL"
# VM catalogue probe (implementation note M2). Fixture-driven in tests; never invent a slug.
GROK_LIST_MODELS_ARGV: tuple[str, ...] = ("grok", "models")

# VM-relative grok binary (implementation note S1 AuthPort.binary). Home-relative segment
# like CODEX_REMOTE_BIN / CURSOR_REMOTE_BIN; matches the ``.grok/bin`` path
# prepend build_agent_spec emits for the grok-remote AgentSpec.
GROK_REMOTE_BIN = ".grok/bin/grok"

# SHIPPED_GROK_MODEL is the pre-0195 name for the same shipped/vendor-default
# pin; kept as an alias of TRACKED_GROK_MODEL (not a second literal) so the
# retired-model plumbing below and its callers/tests keep one source of truth
# instead of two constants that could drift apart [REF-09]. Split from
# DEFAULT_GROK_MODEL so the shipped pin stays observable even when an operator
# override is active: asserting on the env-resolved value alone would silently
# measure the operator's environment instead of this repo.
SHIPPED_GROK_MODEL = TRACKED_GROK_MODEL

#: Slugs the vendor has retired. Kept as data, not a comment, so a stale pin
#: fails loudly here instead of as an opaque vendor error mid-lane ([ARCH-13]:
#: a property enforced only by everyone-will-be-careful is eventually violated).
#: 2026-08-13 discovery: bumping the code default was NOT sufficient — a stale
#: WORKBAY_GROK_MODEL=grok-4.5 in operator settings silently outranked it, so
#: every dispatch still used the retired slug. Env is the thing to check.
RETIRED_GROK_MODELS: frozenset[str] = frozenset({"grok-4.5"})


def resolve_tracked_or_env_grok_model() -> str:
    """Env override if set, else the tracked pin. Does not read settings.local.json."""
    raw = (os.environ.get(WORKBAY_GROK_MODEL_ENV) or "").strip()
    return raw or TRACKED_GROK_MODEL


# Import-time snapshot: env > tracked. Discovery does not rebind this attr.
DEFAULT_GROK_MODEL = resolve_tracked_or_env_grok_model()


def retired_model_warning(model: str | None) -> str | None:
    """Return an operator-facing message when *model* is a retired slug.

    Returns ``None`` for every live slug, so callers can treat a truthy result
    as the whole finding. Kept pure and side-effect free — the caller decides
    whether to log, refuse, or surface it in a doctor facet.
    """
    slug = (model or "").strip()
    if slug in RETIRED_GROK_MODELS:
        return (
            f"grok model {slug!r} is retired by the vendor; the shipped pin is "
            f"{SHIPPED_GROK_MODEL!r}. If this came from WORKBAY_GROK_MODEL, the "
            "environment outranks the code default — update the operator "
            "setting, not just this module."
        )
    return None


if (_retired := retired_model_warning(DEFAULT_GROK_MODEL)) is not None:
    logger.warning("%s", _retired)

# Default flock est_tokens when the manifest omits the column (implementation note S6).
DEFAULT_EST_TOKENS = 80_000

# Engine-authored git identity for offload/remote commits. Single-sourced here so
# remote_exec and offload_pass share byte-identical author flags without either
# importing the other (would risk a cycle). Value must stay stable — tests assert
# author == "workbay-offload-engine <offload-engine@workbay.local>".
ENGINE_GIT_IDENTITY = [
    "-c",
    "user.name=workbay-offload-engine",
    "-c",
    "user.email=offload-engine@workbay.local",
]

# Single-cycle bound ceilings for grok offload lanes (Release It! §5.1).
# The wall-clock cap lives in offload_timeout_ssot; this module owns the turn
# sizing and re-exports the cap for existing importers.
# The two caps are coupled through SECONDS_PER_TURN: a derived pass gets
# min(GROK_TIMEOUT_CAP, max_turns * SECONDS_PER_TURN) seconds.
GROK_MAX_TURNS_CAP = 60
SECONDS_PER_TURN = 30

# implementation note S2 [REF-19]: model_requires_composer_attestation / Composer pin slug
# matcher removed — operator decision retires Composer-model enforcement. The
# pin is DEFAULT_GROK_MODEL (env WORKBAY_GROK_MODEL); grok-build quarantine stays.
