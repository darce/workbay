...
def _orchestration_import(modname: str) -> Any:
    """Load a sibling orchestration module without shadowing the package copy.

    This file is both ``workbay_orchestrator_mcp.orchestration.lane_exec`` and a
    bare script. Script mode (and ``spec_from_file_location`` loads) have no
    ``__package__`` and need ``SCRIPT_DIR`` on ``sys.path``. Package imports
    must not insert that path or use bare names: tests already put this
    directory on ``sys.path``, and ``import lane_test_cmd`` then creates a
    second module object whose sentinels/functions are not the ones
    ``lanes.upsert_worktree_lane`` patches.
    """
    if __package__:
...
_env = _orchestration_import("_env")
WORKER_REASONING_EFFORT_CHOICES = _env.WORKER_REASONING_EFFORT_CHOICES
apply_backend_runtime_hints = _env.apply_backend_runtime_hints
pythonpath_env = _env.pythonpath_env
resolve_lane_python = _env.resolve_lane_python

_backend_registry = _orchestration_import("backend_registry")
backend_supports_adapter_timeout_bounds = _backend_registry.backend_supports_adapter_timeout_bounds
backend_supports_token_budget_cycle_bounds = _backend_registry.backend_supports_token_budget_cycle_bounds
get_adapter = _backend_registry.get_adapter
get_backend_choices = _backend_registry.get_backend_choices
validate_backend = _backend_registry.validate_backend

_bootstrap_mod = _orchestration_import("bootstrap_lane")
bootstrap_lane = _bootstrap_mod._bootstrap
format_missing_lane_manifest_error = _bootstrap_mod.format_missing_lane_manifest_error

_lane_jail = _orchestration_import("lane_jail")
build_jail_prefix = _lane_jail.build_jail_prefix

_lane_manifest = _orchestration_import("lane_manifest")
get_lane_config = _lane_manifest.get_lane_config

_lane_routing = _orchestration_import("lane_routing")
RoutingQuad = _lane_routing.RoutingQuad
normalize_speed = _lane_routing.normalize_speed
resolve_routing_quad = _lane_routing.resolve_routing_quad

_lane_test_cmd = _orchestration_import("lane_test_cmd")
lane_row_test_cmd_read_failure = _lane_test_cmd.lane_row_test_cmd_read_failure

BACKEND_CHOICES = get_backend_choices()


class LaneExecError(RuntimeError):
    """Execution failure annotated with whether the lane agent was jailed."""

    def __init__(self, message: str, *, jailed: bool, cause: BaseException | None = None) -> None:
        super().__init__(message)
        self.jailed = jailed
        source = cause if cause is not None else self.__cause__
        outcome = getattr(source, "outcome", None) if source is not None else None
        degenerate_reason = getattr(source, "degenerate_reason", None) if source is not None else None
        self.outcome = outcome if isinstance(outcome, str) and outcome.strip() else None
        self.degenerate_reason = (
            degenerate_reason if isinstance(degenerate_reason, str) and degenerate_reason.strip() else None
        )


def _execute_path_max_turns(adapter: Any, backend_name: str) -> int | None:
    """Cap the constructed adapter will enforce at execute time (F2).

    Derived from the live adapter so Codex CLI defaults, Cursor's
    timeout-derived bound, and Grok/Claude ``max_turns`` share one source
    with ``adapter.execute(max_turns=...)``.
    """
    from workbay_orchestrator_mcp.orchestration.adapters.remote_exec import (  # noqa: PLC0415
        positive_max_turns,
    )

    if backend_supports_adapter_timeout_bounds(backend_name):
        from workbay_orchestrator_mcp.orchestration.adapters.cursor_cli import (  # noqa: PLC0415
            timeout_derived_max_turns,
        )

        return timeout_derived_max_turns(getattr(adapter, "timeout", None))
    return positive_max_turns(getattr(adapter, "max_turns", None))
...
def _validate_lane_result_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    Before rejecting empty worker prose, defer to receiver-envelope
    degeneracy so a proving one-turn counted-zero stub is typed
    ``DegenerateHandoffError`` rather than a generic schema error
    (ENG01R3RV-001). Attempted results and documents without a proving
    envelope still take the hard empty-prose rejection (CARD-07 / OBS-08).
    """
    ...
    from workbay_orchestrator_mcp.orchestration.lane_result import (  # noqa: PLC0415
        _raise_if_envelope_proves_degenerate,
    )

    _raise_if_envelope_proves_degenerate(payload)
...
# Result-path helpers (durable lane-state spool with TMPDIR fallback)
# ---------------------------------------------------------------------------


def _lane_state_dir(orchestrator_root: Path) -> Path:
    """Return the orchestrator lane-state directory (sibling of status JSON files)."""
    return Path(orchestrator_root) / ".task-state"


def _spool_file_prefix(lane_id: str) -> str:
    """Filesystem-safe prefix for lane-exec result files (lane ids may contain '/')."""
    safe_lane = "".join(ch if ch.isalnum() or ch in "-_." else "-" for ch in lane_id) or "lane"
    return f"lane-exec-{safe_lane}-"


def _temp_output_path(*, lane_id: str) -> Path:
    """Volatile TMPDIR spool (fallback when durable write is unavailable)."""
    fd, name = tempfile.mkstemp(suffix=".json", prefix=_spool_file_prefix(lane_id))
    os.close(fd)
    return Path(name)


def _durable_output_path(*, orchestrator_root: Path, lane_id: str) -> Path:
    """Create an empty result file under the durable lane-state directory."""
    state_dir = _lane_state_dir(orchestrator_root)
    state_dir.mkdir(parents=True, exist_ok=True)
    fd, name = tempfile.mkstemp(
        suffix=".json",
        prefix=_spool_file_prefix(lane_id),
        dir=str(state_dir),
    )
    os.close(fd)
    return Path(name)
...
def _resolve_result_output_path(
    *,
    ...
    """Resolve where to write the lane exec result.

    Prefer an explicit ``output_path``, else a durable file under the lane state
    directory (``.task-state``). If the durable write cannot be prepared, fall
    back to process TMPDIR and flag the fallback for the envelope.
    """
    if output_path is not None:
        return Path(output_path), False
    try:
        return _durable_output_path(orchestrator_root=orchestrator_root, lane_id=lane_id), False
    except Exception:  # noqa: BLE001 - any durable failure falls back to TMPDIR
        return _temp_output_path(lane_id=lane_id), True


def _stamp_result_path_fields(
    payload: dict[str, Any],
    *,
    result_path: Path,
    tmpdir_fallback: bool,
) -> dict[str, Any]:
    """Surface a stable, machine-readable result location on the envelope."""
    stamped = dict(payload)
    stamped["result_path"] = str(result_path)
    stamped["result_path_tmpdir_fallback"] = bool(tmpdir_fallback)
    return stamped
...
def _write_result_envelope(
    ...
    *,
    ...
    """Write the structured result JSON with path fields stamped on the envelope."""
    stamped = _stamp_result_path_fields(
        payload,
        result_path=out,
        tmpdir_fallback=tmpdir_fallback,
    )
...
def _lane_row_test_cmd(orchestrator_root: Path, task_ref: str, lane_id: str) -> tuple[Any, dict[str, Any] | None]:
    """Read ``worktree_lanes.test_cmd`` or return a fail-closed typed failure.

    Same list-lanes read as ``worker_daemon._fetch_mcp_lane_params``. Kept here
    so the lane-exec route can pass the row value as ``lane_test_cmd`` even
    when the downstream ``test_cmd`` carrier is empty.

    Every read failure returns an empty row value so adapter arbitration omits
    a leftover carrier, plus a typed note for the result envelope.
    """
    try:
        from workbay_handoff_mcp import api  # noqa: PLC0415
        from workbay_handoff_mcp.config import RuntimeConfig  # noqa: PLC0415
        from workbay_handoff_mcp.runtime import get_runtime_config, reset_runtime_config  # noqa: PLC0415
        ...
        try:
            ...
            if not isinstance(data, dict) or not data.get("ok"):
                ...
                return None, lane_row_test_cmd_read_failure(
                    task_ref=task_ref, lane_id=lane_id, cause="list_not_ok", error=error
                )
            for lane in data.get("lanes", []):
                if lane.get("lane_id") == lane_id:
                    return lane.get("test_cmd"), None
            return None, lane_row_test_cmd_read_failure(
                task_ref=task_ref, lane_id=lane_id, cause="missing_lane", error="lane row not found"
            )
        finally:
            if prior_runtime is not None:
                api.configure_runtime(prior_runtime)
            else:
        ...
        logger.warning("lane row test_cmd unavailable for %s/%s: %s", task_ref, lane_id, exc)
        return None, lane_row_test_cmd_read_failure(task_ref=task_ref, lane_id=lane_id, error=exc)
...
def run_lane_exec(
    *,
    ...
    # Worker cycles pass the atomic row snapshot. Direct legacy callers may
    # still supply individual keyword overrides; the resolver owns even that
    # compatibility merge so selection semantics do not drift between fields.
    caller = None if routing_row is not None else RoutingQuad(backend, model, reasoning_effort, speed, tier)
    resolved_routing = resolve_routing_quad(
        caller=caller,
        row=routing_row,
        manifest_entry=lane_cfg,
        tier_table=CODEX_MODEL_TIERS,
        profile=RoutingQuad(backend=backend),
        allow_partial_caller=routing_row is None,
    )
    backend_name = resolved_routing.backend or backend
    backend_name = validate_backend(backend_name)
    # harm2: pin-honor path must re-check execution_mode on the executing
    # backend (same semantics as review_runner) so a stale/absent pin under
    # remote_only cannot execute a local turn.
    ...
    model_name = resolved_routing.model
    reasoning_effort = resolved_routing.effort
    speed = normalize_speed(backend_name, resolved_routing.speed)
    ...
    if not dry_run:
        # implementation note S3 [OBS-08]: fail named before bootstrap when the lane
        # manifest entry is missing (stdout from bootstrap is often swallowed).
        try:
            if get_lane_config(task_ref, lane_id, orchestrator_root=str(orchestrator_root)) is None:
                raise RuntimeError(format_missing_lane_manifest_error(task_ref, lane_id))
        except FileNotFoundError:
            raise RuntimeError(format_missing_lane_manifest_error(task_ref, lane_id)) from None
        ...
        if bootstrap_result != 0:
            # Re-check missing-manifest so a race/stale read still surfaces the
            # named operator fix instead of bare exit-code-1.
            try:
                still_missing = get_lane_config(task_ref, lane_id, orchestrator_root=str(orchestrator_root)) is None
            except FileNotFoundError:
                still_missing = True
            if still_missing:
                raise RuntimeError(format_missing_lane_manifest_error(task_ref, lane_id))
        ...
        if not preflight.get("ok", True):
            out, tmpdir_fallback = _resolve_result_output_path(
                orchestrator_root=orchestrator_root,
                lane_id=lane_id,
            ...
            _write_result_envelope(
                out,
                _preflight_failure_payload(lane_id=lane_id, preflight=preflight),
                tmpdir_fallback=tmpdir_fallback,
            )
            return out
    ...
    if dry_run:
        ...
        _write_result_envelope(out, result, tmpdir_fallback=tmpdir_fallback)
        return out
    ...
    # Grok-family backends (capability-gated) also accept derived cycle bounds so
    # RemoteExecAdapter / GrokCliAdapter do not silently fall back to defaults
    # (0144 S5a / [RES-02]).
    ...
    if backend_name == "codex-cli":
        ...
        # applied, so the adapter silently used its ctor default.
        if adapter_timeout is not None:
            adapter_kwargs = {"timeout": adapter_timeout}
    adapter = get_adapter(backend_name, **adapter_kwargs)
    execute_max_turns = _execute_path_max_turns(adapter, backend_name)

    # Lane write-jail (implementation note / adoption C). Call-site inventory finding
    # PLAN0084-AN-003: NONE of the subprocess.run sites in lane_exec.py or
    # worker_daemon.py spawn the coding agent -- they run lane_prompt.py /
    # lane_result.py / git / make (all AUXILIARY). The real agent process is the
    # CLI subprocess launched inside adapter.execute() (adapters/*_cli.py). So the
    # jail is gated HERE (where the manifest grants + worktree + primary repo root
    # are known) and threaded into the adapter as an argv prefix it prepends ahead
    # of its binary. build_jail_prefix fail-opens to [] (unjailed) off-darwin,
    # without sandbox-exec, when the opt-in env WORKBAY_LANE_JAIL=1 is not set
    # (the jail is opt-in), or when grants are absent.
    jail_prefix = build_jail_prefix(
        grants=lane_cfg.get("grants"),
        worktree_root=worktree_path,
        primary_repo_root=orchestrator_root,
        env=env,
    )
    jail_capable = bool(getattr(adapter, "supports_jail", False))
    if jail_prefix and not jail_capable:
        print(
            f"lane jail: backend {backend_name!r} does not support jail_argv_prefix; running unjailed.",
            file=sys.stderr,
        )
        jail_prefix = []

    jailed = bool(jail_prefix and jail_capable)
    row_read_note: dict[str, Any] | None = None
    try:
        lane_test_cmd, row_read_note = _lane_row_test_cmd(orchestrator_root, task_ref, lane_id)
        result = adapter.execute(
            ...
            jail_argv_prefix=jail_prefix,
            # Off-box backends (grok-remote) run this self-verify on the VM and
            # return it on off_box_self_verify; on-box adapters ignore the kwarg.
            test_cmd=test_cmd,
            # Producer for the adapter recovery keyword (one read site in
            # remote_exec.resolve_dispatch_test_cmd). Do not fold the row
            # into test_cmd here — that would hide a falsy carrier.
            lane_test_cmd=lane_test_cmd,
            lane_id=lane_id,
            orchestrator_root=orchestrator_root,
            # implementation note S1.3: per-cycle decision_ts for pre_spawn. Adapters that
            # do not know the keyword accept it via **kwargs; None is fail-open.
            decision_ts=decision_ts,
            # Remote capability evidence is explicit at the worker → adapter
            # boundary. The key is present even when absent so a managed remote
            # worker cannot silently use ambient WORKBAY_CAPABILITY_RECEIPT_ID.
            capability_receipt_id=capability_receipt_id,
            capability_config_digest=capability_config_digest,
            pass_id=pass_id,
            dispatch_id=dispatch_id,
            # F2: the cap this execute path enforces (Codex CLI flag, Cursor
            # timeout-derived bound, Grok/Claude max_turns). Same value the
            # adapter stamps and the handoff argv later advertises.
            max_turns=execute_max_turns,
        )
    except Exception as exc:  # noqa: BLE001 - annotate for daemon telemetry
        raise LaneExecError(str(exc), jailed=jailed, cause=exc) from exc
    ...
    # Patch result JSON with context_utilization and scope violations in one pass
    result_data = dict(result_payload)
    result_data["jailed"] = jailed
    if ctx_metrics:
    ...
    if row_read_note:
        ...
        warnings.append(dict(row_read_note))
    ...
    _write_result_envelope(out, result_data, tmpdir_fallback=tmpdir_fallback)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--speed", choices=("standard", "fast"), help="Codex service speed.")
    parser.add_argument("--tier", choices=("senior", "junior"), help="Model tier selector.")
