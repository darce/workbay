...
_REMAT_PRECLAIM_BARRIER_ENV = "WORKBAY_TEST_REMAT_PRECLAIM_BARRIER"
...
PATH_SHARE_SCOPE_SHARED_REVIEW_SAME_BRANCH = "shared_review_same_branch"
...
def _worker_lock_path_for_lane(lane_id: str) -> Path | None:
    """Return the worker flock path, or None when no path can be formed.

    An empty ``lane_id`` returns ``None`` before consulting the runtime; the
    caller must re-check ``_runtime_is_configured()`` rather than treating
    that ``None`` as genuine absence of a shared runtime. After a non-empty
    key, ``None`` means unconfigured or unimportable runtime, so there is
    no claim to coordinate against. A configured runtime whose path cannot
    be resolved must not be folded into that meaning: ``AttributeError``, a
    missing ``state_dir``, and similar bugs propagate so the caller can
    refuse the same way the reaper refuses ``worker_lock_path_unresolved``.
    """
...
def _wait_rematerialize_test_barrier(env_var: str) -> None:
    """Park at a named test barrier so a race window can be driven on purpose.

    Inert unless ``env_var`` names a directory. Tests touch ``go`` inside it to
    release the waiter. Two windows need barriers on the recovery path — before
    the claim flock and after it — so each gets its own env var.
    """
...
def _rematerialize_claim_barrier() -> None:
    _wait_rematerialize_test_barrier(_REMAT_CLAIM_BARRIER_ENV)


def _rematerialize_preclaim_barrier() -> None:
    _wait_rematerialize_test_barrier(_REMAT_PRECLAIM_BARRIER_ENV)
...
class EnsureLaneWorktreeResult:
    ...
    provision_skipped: list[dict[str, str]] = field(default_factory=list)
...
def _ref_resolves(primary: Path, ref: str) -> bool:
    """True when *ref* directly names a commit in *primary*."""
    proc = _git(primary, "rev-parse", "--verify", f"{ref}^{{commit}}")
    return proc.returncode == 0 and bool((proc.stdout or "").strip())


def _provision_lane_surfaces(primary: Path, path: Path) -> tuple[list[str], list[dict[str, str]]]:
    """Ensure untracked protocol surfaces are symlinked into a lane worktree.

    Provisioning is receipt-driven and best-effort: a successfully created git
    worktree stays usable even when a primary surface is absent or a target is
    unexpectedly occupied. Every skip carries a stable typed reason so callers
    do not need to parse log text. Exclude registration failure is fatal to
    materialization; roll back only links created by this call. [ARCH-13] [RES-07]
    """
    provisioned: list[str] = []
    skipped: list[dict[str, str]] = []
    ignored: list[str] = []

    for surface in ("Makefile.d", ".venv"):
        if surface == ".venv" and os.environ.get("WORKBAY_LANE_VENV_MODE", "").strip().lower() == "none":
            skipped.append({"surface": surface, "reason": "disabled_by_env"})
            continue

        source = primary / surface
        destination = path / surface
        if not source.exists():
            skipped.append({"surface": surface, "reason": "primary_missing"})
            continue

        if destination.exists() or destination.is_symlink():
            if destination.is_symlink() and _paths_equivalent(destination, source):
                ignored.append(surface)
                continue
            else:
                skipped.append({"surface": surface, "reason": "target_occupied"})
            continue

        try:
            destination.symlink_to(source, target_is_directory=True)
        except OSError as exc:
            skipped.append({"surface": surface, "reason": "symlink_failed"})
            _logger.warning(
                "lane surface provisioning failed surface=%s path=%s source=%s error=%s",
                surface,
                destination,
                source,
                exc,
            ...
            continue
        provisioned.append(surface)
        ignored.append(surface)

    if ignored:
        try:
            proc = _git(path, "rev-parse", "--git-path", "info/exclude")
            if proc.returncode != 0 or not proc.stdout.strip():
                raise OSError(proc.stderr.strip() or "cannot resolve info/exclude")
            # Git resolves this to the common git dir for linked worktrees;
            # their private git-dir/info/exclude is not consulted by Git.
            exclude = path / proc.stdout.strip()
            exclude.parent.mkdir(parents=True, exist_ok=True)
            with exclude.open("a+", encoding="utf-8") as handle:
                # Multiple lanes can provision against the same exclude file.
                fcntl.flock(handle.fileno(), fcntl.LOCK_EX)
                handle.seek(0)
                existing = handle.read()
                lines = set(existing.splitlines())
                missing = [f"/{surface}" for surface in ignored if f"/{surface}" not in lines]
                if missing:
            ...
            for surface in provisioned[:]:
                destination = path / surface
                try:
                    if destination.is_symlink() and _paths_equivalent(destination, primary / surface):
                        destination.unlink()
                    provisioned.remove(surface)
                except OSError as cleanup_exc:
                    skipped.append({"surface": surface, "reason": "rollback_failed"})
                    _logger.warning("lane surface rollback failed path=%s error=%s", destination, cleanup_exc)
            skipped.extend({"surface": surface, "reason": "exclude_failed"} for surface in ignored)
            _logger.warning("lane surface exclude registration failed path=%s error=%s", path, exc)
...
class _SharedPathGuardOutcome:
    ...
    path_share_scope: str | None = None
    shared_path_lookup_error: str | None = None
    shared_with_lane_id: str | None = None
...
def _shared_path_ownership_guard(
    *,
    ...
    """Refuse live co-owners except known same-branch review sharing.
    ...
    The compound caller row supplies kind and branch. Sharing requires two
    known implement/review kinds, at least one review, and equal nonempty
    branches after stripping. Missing or ambiguous metadata never grants an
    exception. Every co-owner must pass before a sharing receipt is returned.
    ...
    def _row_token(row: dict, key: str) -> str:
        value = row.get(key)
    ...
    caller_rows = [
        ...
        and caller_id
        and caller_task
        and _row_token(row, "lane_id") == caller_id
        and _row_token(row, "task_ref") == caller_task
    ]
    caller = caller_rows[0] if len(caller_rows) == 1 else {}
    caller_kind = _row_token(caller, "lane_kind")
    caller_branch = _row_token(caller, "branch")
    shared_with_lane_id: str | None = None
    for owner in lanes:
        ...
        if (
            ...
            continue
        owner_kind = _row_token(owner, "lane_kind")
        if (
            owner_id
            and owner_task
            and caller_kind in {"implement", "review"}
            and owner_kind in {"implement", "review"}
            and "review" in (caller_kind, owner_kind)
            and caller_branch
            and caller_branch == _row_token(owner, "branch")
        ):
            # Keep scanning: a legitimate review cannot hide another writer.
            if shared_with_lane_id is None:
                shared_with_lane_id = owner_id
            continue
    ...
    return _SharedPathGuardOutcome(
        path_share_scope=(
            PATH_SHARE_SCOPE_SHARED_REVIEW_SAME_BRANCH if shared_with_lane_id is not None else PATH_SHARE_SCOPE_RESOLVED
        ),
        shared_with_lane_id=shared_with_lane_id,
    )
...
def ensure_lane_worktree(
    *,
    ...
    TODO(internal): The task-start/API and manage-worktree surfaces must
    expose their requested integration ref and pass it here as ``base_ref``;
    those production call sites are owned by sibling lanes.
    ...
    base_ref:
        Commit-ish from which to create ``branch`` when that branch does not
        exist. Ignored when ``branch`` already resolves, preserving existing-
        branch re-materialization semantics.
    lane_id:
    ...
    base_ref_name = (base_ref or "").strip()
    ...
    def _stamp(
        ...
        return EnsureLaneWorktreeResult(
            ...
            provision_skipped=result.provision_skipped,
        )
    ...
    def _present_success(*, rematerialized: bool = False) -> EnsureLaneWorktreeResult:
        provisioned, provision_skipped = _provision_lane_surfaces(primary, path)
        exclude_failed = any(item["reason"] == "exclude_failed" for item in provision_skipped)
        return _stamp(
            EnsureLaneWorktreeResult(
                ok=not exclude_failed,
                worktree_path=path,
                rematerialized=rematerialized,
                outcome=OUTCOME_WORKTREE_UNRECOVERABLE if exclude_failed else None,
                failure_kind=FAILURE_SURFACE_EXCLUDE if exclude_failed else None,
                error=f"Lane surface exclude registration failed: {path}" if exclude_failed else None,
                provisioned=provisioned,
                provision_skipped=provision_skipped,
            )
        )
    ...
    # Missing-path recovery starts here. Take the rematerialize flock BEFORE
    # branch resolution and the shared-path sweep. A reaper CAS-close landing
    # in that pre-claim window is invisible to both checks, and ``git worktree
    # add`` then succeeds into a lane the reaper has already closed — the
    # checks would have been reading state that goes stale the instant they
    # return. Under the flock they cannot. [CON-04] [RES-02]
    _rematerialize_preclaim_barrier()
    ...
    try:
        # Branch resolution and the shared-path sweep run *under* the flock so
        # their verdicts still hold at ``git worktree add`` time. ``finally``
        # below releases the claim on every one of these early returns.
        branch_resolves = _branch_resolves(primary, branch_name)
        create_branch = not branch_resolves and bool(base_ref_name)
        if not branch_resolves:
            if not create_branch:
                error = (
                    f"Lane worktree does not exist for lane '{lane_label}': {path} "
                    f"(branch ref does not resolve: {branch_name})"
                )
                return EnsureLaneWorktreeResult(
                    ok=False,
                    worktree_path=path,
            ...
            if not _ref_resolves(primary, base_ref_name):
                error = (
                    f"Lane worktree does not exist for lane '{lane_label}': {path} "
                    f"(base ref does not resolve: {base_ref_name})"
                )
                return EnsureLaneWorktreeResult(
                    ok=False,
                    worktree_path=path,
                    ...
                    outcome=OUTCOME_WORKTREE_UNRECOVERABLE,
                    error=error,
                    failure_kind=FAILURE_BASE_REF_UNRESOLVABLE,
                )
        ...
        path.parent.mkdir(parents=True, exist_ok=True)
        if create_branch:
            add = _git(primary, "worktree", "add", "-b", branch_name, str(path), base_ref_name)
        else:
        ...
        return _present_success(rematerialized=True)
    finally:
