...
# PASS_OUTCOME_TO_WORKER_REPORT (maps to failed).
    ...
    *,
    ...
    def _page(active_conn: sqlite3.Connection) -> dict[str, Any]:
        count_row = active_conn.execute(
            "SELECT COUNT(*) AS count FROM worktree_lanes"
        ).fetchone()
        try:
        ...
        if after_id is None:
            ...
            fetched_rows = active_conn.execute(
                "SELECT * FROM worktree_lanes WHERE id < ? "
        ...
        for _ in range(_LIST_LANES_MAX_PAGES):
            ...
            if not has_more:
                ...
                if total != len(all_lanes):
                    return None, (
                        f"total_matching drift: collected={len(all_lanes)} "
                        f"total_matching={total}"
                    )
    ...
    *,
    ...
    for owner in lanes:
        ...
        if not isinstance(owner, dict):
            return _SharedPathGuardOutcome(
                path_share_scope=PATH_SHARE_SCOPE_LOOKUP_UNAVAILABLE,
                shared_path_lookup_error=(
                    f"malformed lane row: {type(owner).__name__}"
                ),
            )
        ...
        if owner_path is None or owner_path == "":
            ...
            return _SharedPathGuardOutcome(
                path_share_scope=PATH_SHARE_SCOPE_LOOKUP_UNAVAILABLE,
                shared_path_lookup_error=(
                    "empty or missing worktree_path on owner row"
                ),
            )
        ...
        owner_id = (
            ...
            else str(owner_id_raw or "").strip()
        )
        owner_task_raw = owner.get("task_ref")
        owner_task = (
            owner_task_raw.strip()
            if isinstance(owner_task_raw, str)
            else str(owner_task_raw or "").strip()
        )
    ...
    *,
    ...
    def _stamp(
        ...
        if path_share_scope is None and shared_path_lookup_error is None:
            return result
        if (
            result.path_share_scope is not None
            or result.shared_path_lookup_error is not None
        ):
            return result
    ...
    if path.exists():
        ...
        if _is_git_worktree(path):
            ...
            if not branch_name:
                ...
                return _stamp(
                    EnsureLaneWorktreeResult(
                        ok=True, worktree_path=path, rematerialized=False
                    )
                )
        ...
        if not _is_empty_directory(path):
            error = (
                f"Lane worktree path for lane '{lane_label}' is occupied by a "
                f"non-worktree path: {path}"
            )
            return EnsureLaneWorktreeResult(
                ok=False,
                worktree_path=path,
    ...
    if not primary.is_dir():
        error = (
            f"Lane worktree does not exist for lane '{lane_label}': {path} "
            f"(primary repo not found: {primary})"
        )
        return EnsureLaneWorktreeResult(
            ok=False,
            worktree_path=path,
