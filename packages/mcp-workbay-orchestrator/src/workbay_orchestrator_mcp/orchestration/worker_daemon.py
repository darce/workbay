...
def _consume_off_box_self_verify(ctx: WorkerRunContext, test_cmd: str) -> dict[str, Any]:
    ...
    if captured is None:
        return {
            ...
            "output_tail": _tail_text(
                "off-box self-verify result unavailable: this backend runs tests off-box "
                "but reported no off_box_self_verify capture, so the gate cannot be confirmed "
    ...
    return {
        ...
        "passed": passed,
        "command": command,
        "output_tail": _tail_text(str(captured.get("output_tail") or "")),
        "exit_code": exit_code,
        "injected_env": [],
        "off_box_delegated": True,
    }
...
def _acquire_worker_heavy_slot(config: WorkerConfig) -> tuple[bool, str]:
    ...
    if cost_class_for_backend(getattr(config, "backend", None)) == COST_REMOTE:
    ...
    cost_class = cost_class_for_backend(getattr(config, "backend", None))
    decision = evaluate_admission(resources, cost_class, policy, held_slots=0)
