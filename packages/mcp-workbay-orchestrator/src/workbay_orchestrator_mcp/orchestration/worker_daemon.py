...
def _self_verify_phase(ctx: WorkerRunContext) -> dict[str, Any]:
    ...
    Returns a result dict with keys: skipped, passed, command, output_tail, exit_code.
    Patches the lane result file's tests_run when a result path exists.
    """
    test_cmd = str(ctx.config.test_cmd or "").strip()
    if not test_cmd:
        return {"skipped": True, "passed": True, "command": None, "output_tail": "", "exit_code": 0}
...
def _composer_violation_info(ctx: WorkerRunContext) -> dict[str, Any] | None:
    """Return composer-only violation metadata from the execution result, if present."""
    ...
    elif any("debug log absent" in blocker.lower() for blocker in blockers):
        branch = "scan_input_absence"
    elif any("debug log empty" in blocker.lower() for blocker in blockers):
        branch = "scan_input_absence"
    elif any("no recognizable assistantitem" in blocker.lower() for blocker in blockers):
        branch = "format_drift"
    elif any("grok-build" in blocker.lower() for blocker in blockers):
        branch = "contamination"
    if branch is None:
        return None
    return {
