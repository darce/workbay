...
def _record_terminal_outcome(
    *,
    ...
    ``worker_reports.outcome`` enum value; ``record_worker_report`` maps to a
    non-NULL report outcome (implementation note S1).
    """
    try:
        from workbay_handoff_mcp import api  # noqa: PLC0415
        from workbay_handoff_mcp.config import RuntimeConfig  # noqa: PLC0415
        ...
        response = worker_reports(
            ...
            session=session,
            summary=summary,
            outcome=map_pass_outcome_to_worker_report(outcome),
            merge_ready=False,
        )
...
def _resolve_grok_cycle_bounds(config: WorkerConfig) -> WorkerConfig:
    ...
    if backend_supports_adapter_timeout_bounds(config.backend):
        ...
        return replace(
            config,
            adapter_timeout=derive_adapter_timeout_bounds(config.token_budget, timeout_cap=timeout_cap)[
                "timeout"
            ],
        )
...
class WorkerConfig:
    ...
    # Wall-clock-only family (cursor-cli): the per-cycle AGENT bound. Separate
...
def _execute_phase(ctx: WorkerRunContext) -> bool:
    ...
    if ctx.backend == "codex-cli":
        ...
        # PRIMARY cursor dispatch path (offload_pass -> WorkerConfig -> worker
        ...
        cycle_timeout = config.adapter_timeout if config.adapter_timeout is not None else config.grok_timeout
    ...
    try:
        ...
        ctx.final_result_path = run_lane_exec(
            ...
            # Coalesce the wall-clock-only family's bound into the cycle-timeout
            # parameter; adapter_timeout is None for grok, so this is a no-op there.
            grok_timeout=(config.grok_timeout if config.grok_timeout is not None else config.adapter_timeout),
...
def _verify_phase(ctx: WorkerRunContext, review_output: "dict[str, Any]") -> bool:
    ...
    Called from the converged path and from the shared review-loop terminal
    handler. Exhaustion-streak reset is owned by the converged success path
    only — a bound-stop must not clobber ``_update_exhaustion_streak``.
    """
    from workbay_handoff_mcp.enums import WorkerEventName  # noqa: PLC0415
...
def _handoff_phase(ctx: WorkerRunContext, check_ok: bool) -> None:
    ...
    ctx.handoff_exit = _run_final_handoff(
        ...
        dry_run=config.dry_run,
        run_id=ctx.run_id,
        outcome="finished" if check_ok else "self_verify_failed",
    )
    ctx.handoff_attempted = True
    if ctx.handoff_exit == 0:
        _cleanup_result_file(final_result_path)
...
def _handle_review_loop_terminal(
    ...
    *,
    ...
    if check_ok:
        ...
        failed_summary = "The exhausted-loop worker handoff failed; the saved lane result must be retried without re-running execution."
        ...
        failed_summary = (
            "The blocked worker handoff failed; the saved lane result must be retried without re-running execution."
        )
