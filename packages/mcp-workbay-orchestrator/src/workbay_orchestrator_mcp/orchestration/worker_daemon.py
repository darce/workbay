...
def _execute_phase(ctx: WorkerRunContext) -> bool:
    ...
    try:
        ...
        ctx.final_result_path = run_lane_exec(
            ...
            # defaults (timeout=900, max_turns=30).
    ...
    # implementation note S3: salvage channel for remote exit-3 with committed work.
    # Marker PAIR required (resumable_work is True AND truthy resumable_reason).
    # Sets execute_stop_reason and returns False so the offload checkpoint arm
    # can run — NOT the _review_phase admission_deferred path (opposite shape).
    raw = result.get("raw_payload") if isinstance(result.get("raw_payload"), dict) else {}
    if raw.get("resumable_work") is True and raw.get("resumable_reason"):
        ctx.execute_stop_reason = "agent_exit_with_work"
        ctx.log(
            "INFO",
            WorkerEventName.EXEC_COMPLETE,
            result_path=str(ctx.final_result_path),
            cycle=ctx.cycle,
            execute_stop_reason="agent_exit_with_work",
            resumable_reason=str(raw.get("resumable_reason")),
        )
        return False
