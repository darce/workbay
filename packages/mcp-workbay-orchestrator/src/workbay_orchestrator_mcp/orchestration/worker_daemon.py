...
from backend_registry import backend_supports_token_budget_cycle_bounds, get_backend_choices
...
def _resolve_grok_cycle_bounds(config: WorkerConfig) -> WorkerConfig:
    ...
    if (
        not backend_supports_token_budget_cycle_bounds(config.backend)
        or config.token_budget is None
        or config.token_budget <= 0
    ):
        return config
