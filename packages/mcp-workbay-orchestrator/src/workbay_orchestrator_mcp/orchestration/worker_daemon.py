...
def _usage_component(
    primary: Any, fallback: Any, name: str
) -> Any:
    ...
    *,
    ...
    entry: dict[str, Any] = {
        ...
        "token_usage_totals": {
            ...
            "reasoning_output_tokens": _usage_component(
                total_usage, last_usage, "reasoning_output_tokens"
            ),
...
def _resolve_grok_cycle_bounds(config: WorkerConfig) -> WorkerConfig:
    ...
    grok_max_turns = (
        config.grok_max_turns if config.grok_max_turns is not None else bounds["max_turns"]
    )
...
def _execute_phase(ctx: WorkerRunContext) -> bool:
    ...
    if raw.get("resumable_work") is True and raw.get("resumable_reason"):
        ...
        if stop_reason not in _ALLOWED_EXECUTE_STOP_REASONS:
            ...
            logger.warning(
                "rejecting unknown resumable_reason %r (not in allowlist %s); "
                "substituting host sentinel %r",
                stop_reason,
                allowlist_sorted,
                UNKNOWN_REMOTE_STOP,
            )
