...
_REVIEW_REQUIRED_IDENTITY_FIELDS = (
    "pass_id", "task_ref", "lane_id", "base_sha", "tip_sha",
...
def _review_identity_complete(identity: dict[str, Any]) -> bool:
    return (
        all(isinstance(identity.get(key), str) and bool(identity[key].strip())
            for key in _REVIEW_REQUIRED_IDENTITY_FIELDS)
        and all(key in identity and (
            identity[key] is None or (isinstance(identity[key], str) and bool(identity[key].strip()))
        ) for key in _REVIEW_DECLARABLE_IDENTITY_FIELDS)
    )


def _review_identity_matches(saved: dict[str, Any], current: dict[str, Any]) -> bool:
    return (_review_identity_complete(saved) and _review_identity_complete(current)
...
def _review_stream_text(identity: dict[str, Any]) -> str | None:
    ...
    def texts(value: Any, depth: int = 0) -> list[str]:
        ...
        if isinstance(value, dict):
            if value.get("role") in {"user", "system", "tool"} or value.get("type") in {"user", "tool_use", "tool_result"}:
                return []
            return [text for key in ("text", "details", "content", "message", "delta", "result", "item", "output")
                    if key in value for text in texts(value[key], depth + 1)]
        return []
...
def _harvest_review_findings(
    ...
    *,
    ...
    receipt = dict(
        identity=identity, result=result, report=report, kwargs=kwargs, status="record_pending",
    ...
    harvest = _harvest_review_findings_once(
        result, report, reconcile_committed=receipt_path is not None,
        operation_token=receipt.get("operation_token") if receipt_path is not None else None, **kwargs,
    )
    ...
    receipt["status"] = "recorded" if (
        _harvest_block_parsed(harvest) or harvest.get("status") == "salvaged_unstructured"
    ) else "record_pending"
    if receipt_path is not None:
        _atomic_write(receipt_path, json.dumps(receipt))
    return harvest
        ...
        for carrier_name, text in carriers:
            ...
            try:
                ...
                return {"status": "skipped", "reason": "record_failed",
                        **{key: evidence[key] for key in ("legacy_reconciliation", "legacy_cause")
                           if key in evidence},
    ...
    for row in listed.get("lanes") or []:
        ...
        for ref, tip in usable_tips:
            ...
            if never_produced is not False:
                # True: empty cut / never diverged. None: check failed — refuse
                # rather than fabricate a landing_sha for work that may not exist.
                skip_payload = {
                    "lane_id": lane_id,
                    "task_ref": row_task,
                    "branch": lane_branch,
    ...
    *,
    ...
    def _payload(
        ...
        *,
        ...
        result["self_verify_gate"] = _self_verify_gate_status(self_verify if isinstance(self_verify, dict) else None)
    ...
    def _execute_pass() -> dict[str, Any]:
        nonlocal run_ctx, start_head_ref, budget_exhausted, persisted_escalation_history
        ...
        if refuse_error is not None:
            return _payload("dispatch_refused", error=refuse_error, failed_stage=None, ok=False)
        ...
        for cycle in range(max_review_cycles):
            ...
            rescue_identity.update(base_sha=start_head_ref,
                                   tip_sha=_git_stdout(Path(worktree_path), "rev-parse", "HEAD"))
            rescue_identity["review_carrier_path"] = str(run_ctx.final_result_path) if run_ctx.final_result_path else None
            write_pass_state(resolved_state_dir, resolved_pass_id, {
                **rescue_identity, "status": "running", "task_ref": task_ref,
                "lane_id": lane_id, "session": session,
                "orchestrator_root": str(orchestrator_root),
            })
            try:
        ...
        landed_before_salvage = bool(checkpoints) or (
            bool(start_head) and _git_stdout(Path(worktree_path), "rev-parse", "HEAD") not in ("", start_head)
        )
        ...
        # OL-PF2: remaining green committed work with a no-question blocked
        # handoff is completed_unreviewed. Post-salvage land status is valid for
        # this fallback because salvage may land an inspectable checkpoint.
