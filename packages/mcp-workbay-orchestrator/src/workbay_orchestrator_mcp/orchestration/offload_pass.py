...
def close_lanes_merged_into_integration(
    *,
    ...
    tip = _git_stdout(root, "rev-parse", "HEAD")
    if not tip:
        return {
            "ok": False,
            "error": "integration_tip_unresolvable: cannot rev-parse HEAD at orchestrator_root",
            "closed": [],
            "skipped": [],
            "already_terminal": [],
            "failed": [],
    ...
    for row in listed.get("lanes") or []:
        ...
        if receipt_matches:
            # Receipt written at landing is sufficient close evidence.
            pass
        else:
            ...
            if never_produced == ABSENT_LANE_BRANCH_WORK_LANDED:
                ...
                if contained is not True:
                    # Fail-closed: not an ancestor → out of scope, never force.
                    skipped.append(
                        {
                            "lane_id": lane_id,
                            "task_ref": row_task,
                            "branch": lane_branch,
                            "reason": (
                                "containment_probe_timeout"
                                if contained == "timeout"
                                else "unmerged_commits_not_in_integration_tip"
                            ),
                            "contained": contained,
                        }
                    )
    ...
    if list_truncated:
        ...
        result["error"] = str(listed.get("list_truncation_reason") or "list_page_cap_reached")
    return result
