...
# implementation note S3: checkpoint arm stop-reason gate (max_turns local + agent_exit remote).
_STOP_REASONS_CHECKPOINT = frozenset({"max_turns", "agent_exit_with_work"})
# End-of-pass salvage for failed self-verify after remote exit-3 only (F1: excludes max_turns).
_SALVAGE_STOP_REASONS = frozenset({"agent_exit_with_work"})
...
def build_checkpoint_commit_message(*, lane_id: str, checkpoint_number: int) -> str:
    ...
    model, or trailer. ``lane_id`` is sanitized so it cannot alter message line
    structure, truncated so the whole subject stays within 72 chars, and run
    through the shared subject screen (SG-20) so an operator-chosen id cannot
    smuggle a credit claim into permanent history. Ordinary lane ids survive.
    """
    # Shared sanitizer + screen (REF-26 / SG-20): one neutralization path.
    from workbay_orchestrator_mcp.orchestration.commit_subject import (  # noqa: PLC0415
        sanitize_lane_id_for_commit_message,
        screen_commit_subject_body,
    )
    ...
    screened_lane = screen_commit_subject_body(safe_lane, max_body_len=max_lane_len)
    return f"{prefix}{screened_lane}{suffix}"
...
def _lane_kind(orchestrator_root: Path, task_ref: str, lane_id: str) -> tuple[str, bool]:
    """Return (kind, confirmed). confirmed only when MCP params carry an explicit kind.

    Fail-open kind is always ``"implement"`` when the lookup is unresolved (implementation note R3 /
    implementation note S4). ``confirmed`` is True IFF params is a non-empty-path dict whose
    ``lane_kind`` is explicitly ``"implement"`` or ``"review"`` — never "no exception"
    (``_fetch_mcp_lane_params`` returns ``{}`` on nearly every failure without raising).
    """
    try:
...
def run_offload_pass_engine(
    *,
    ...
    def _execute_pass() -> dict[str, Any]:
        ...
        # implementation note S4: confirmed only when MCP params carry an explicit kind (F6).
        ...
        for cycle in range(max_review_cycles):
            ...
            if not dry_run and cycle > 0:
                ...
                if park_reason is not None:
                    ...
                    checkpointed = _checkpoint_if_dirty(Path(worktree_path), lane_id, checkpoints)
            ...
            if not wd._execute_phase(run_ctx):
                ...
                if stop_reason in _STOP_REASONS_CHECKPOINT and _worktree_dirty(Path(worktree_path)):
                    if config.test_cmd and not dry_run:
                        ...
                        if not self_verify_result.get("passed"):
                            ...
                            break
                    if _checkpoint_if_dirty(Path(worktree_path), lane_id, checkpoints):
                        outcome = "checkpoint"
            ...
            if not _checkpoint_if_dirty(Path(worktree_path), lane_id, checkpoints):
        ...
        # implementation note S3a: also salvage when self-verify failed after remote exit-3
        # (agent_exit_with_work). Deliberately excludes max_turns so the F1
        # invariant (red max-turns stays dirty/uncommitted) holds.
        if outcome in ("timeout", "token_budget_exceeded", "error") or (
            ...
            if not classification_guard_failed:
                try:
                    salvaged = _checkpoint_if_dirty(Path(worktree_path), lane_id, checkpoints)
