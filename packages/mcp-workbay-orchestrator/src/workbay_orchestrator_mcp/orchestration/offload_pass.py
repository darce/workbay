...
# provider from stalling pass publish; the embeddings store uses the same
# ThreadPoolExecutor + wait=False pattern (`_provider_embed_batch`).
...
def _call_related_prior_with_timeout(fn: Any, *, timeout_seconds: float) -> Any:
    """Run *fn* with a wall-clock bound; abandon hung work (wait=False).

    Mirrors ``workbay_handoff_mcp.embeddings.store._provider_embed_batch`` so a
    stuck embedding provider raises ``TimeoutError`` instead of blocking forever.
    """
    from concurrent.futures import ThreadPoolExecutor  # noqa: PLC0415
    from concurrent.futures import TimeoutError as FuturesTimeout  # noqa: PLC0415
    ...
    try:
        ...
        try:
            ...
            raise TimeoutError(f"failure_prior_art lookup timed out after {timeout_seconds}s") from exc
    finally:
...
def _harvest_verdict(harvest: dict[str, Any] | None) -> str:
    ...
    - ``harvest_failed``: every other case — including ``salvaged_unstructured``,
      missing marker, unparseable JSON, all-malformed array (``dropped > 0``),
      persist failure, or non-dict harvest. Never eligible for ``review_complete``;
...
def outcome_dispatch_receipt(
    ...
    *,
    ...
    """Walk-back receipt for a finished pass. Never re-derives after TTL expiry."""
    if not backend_id:
        return None
    try:
        from workbay_orchestrator_mcp.orchestration.offload_profiles import (  # noqa: PLC0415
        ...
        return build_offload_dispatch_receipt(
            backend_id,
    ...
    *,
    ...
    def _payload(
        ...
        *,
        ...
        served_model = getattr(run_ctx, "response_model", None) or result_model
    ...
    def _execute_pass() -> dict[str, Any]:
        nonlocal run_ctx, start_head_ref, budget_exhausted
        ...
        for cycle in range(max_review_cycles):
            ...
            if time.monotonic() >= deadline:
                ...
                break
            if _live_token_telemetry_supported() and run_ctx.cumulative_tokens >= token_budget:
                if _compute_commit_landed(since=cycle_start_head):
                    # The turn already produced a durable commit. Stop before
                    # spending on review, but keep the landed product reapable;
                    # budget exhaustion is advisory once work has succeeded.
