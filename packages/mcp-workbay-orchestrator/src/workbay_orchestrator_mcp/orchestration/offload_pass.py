...
def _host_admission_should_park(orchestrator_root: Path) -> str | None:
    ...
    try:
        ...
        if policy.enforcement != "enforce":
            return None
        decision = evaluate_admission(probe_host(), "heavy", policy, held_slots=0)
    ...
    *,
    ...
    def _execute_pass() -> dict[str, Any]:
        ...
        grok_timeout = (
            ...
            if turn_timeout_seconds and backend_supports_token_budget_cycle_bounds(backend)
            else None
        )
        ...
        for cycle in range(max_review_cycles):
            ...
            if not dry_run and cycle > 0:
                park_reason = _host_admission_should_park(Path(orchestrator_root))
