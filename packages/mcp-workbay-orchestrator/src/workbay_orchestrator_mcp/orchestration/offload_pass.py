...
def _call_related_prior_with_timeout(fn: Any, *, timeout_seconds: float) -> Any:
    ...
    try:
        ...
        try:
            ...
            raise TimeoutError(
                f"failure_prior_art lookup timed out after {timeout_seconds}s"
            ) from exc
    finally:
...
_POST_REVIEW_GATE_NON_PASS_STATUSES = frozenset(
    {"skipped", "failed", "system_error", "timed_out", "unknown"}
)
    ...
    try:
        ...
        for carrier_name, text in carriers:
            ...
            try:
                ...
                logger.warning(
                    "review findings persist failed with zero rows recorded "
...
def _remote_implement_input_gate(
    *,
    ...
    decision, reason = classify_remote_test_cmd(test_cmd)
    if decision == "refuse":
        return f"{TEST_CMD_UNUSABLE_PREFIX}{reason}", None
    ...
    *,
    ...
    if speed is not None:
        ...
        if profile is None or speed not in profile.allowed_speeds:
            ...
            raise ValueError(
                f"speed {speed!r} is not advertised by {backend!r}; advertised speeds: {advertised}"
            )
    ...
    def _execute_pass() -> dict[str, Any]:
        ...
        if budget_tripped:
            ...
            if (
                ...
                and not (
                    getattr(run_ctx, "handoff_attempted", False) is True
                    and getattr(run_ctx, "handoff_exit", None) != 0
                )
            ):
                outcome = "completed_unreviewed"
                failed_stage = None
        ...
        if review_product_harvest and harvest_verdict == HARVEST_VERDICT_HARVEST_FAILED:
            ...
            findings_suppressed_reason = (
                ...
                if isinstance(findings_harvest, dict)
                and findings_harvest.get("status") == "salvaged_unstructured"
                else "harvest_failed"
            )
