...
def run_offload_pass_engine(
    *,
    ...
    def _resolved_model(run_ctx: Any = None) -> str | None:
        return getattr(run_ctx, "model", None) or model

    def _resolved_backend(run_ctx: Any = None) -> str:
        return str(getattr(run_ctx, "backend", None) or backend)

    def _is_unattested_grok_pin(run_ctx: Any = None) -> bool:
        """Non-composer grok pin: green work cannot carry Composer attestation."""
        if _resolved_backend(run_ctx) != "grok-cli":
            return False
        return not model_requires_composer_attestation(_resolved_model(run_ctx))
    ...
    def _execute_pass() -> dict[str, Any]:
        ...
        for cycle in range(max_review_cycles):
            ...
            try:
                ...
                if run_ctx.handoff_exit == 0:
                    ...
                    self_verify_ok = self_verify_result is None or bool(self_verify_result.get("passed"))
                    if violation is not None and checkpoints:
                        composer_violation_result = violation
                        # implementation note: green + committed + *missing/unverifiable*
                        # attestation under a NON-composer pin is NOT a genuine
                        # blocker — surface handoff_ready_unattested. Real
                        # grok-build contamination still quarantines regardless
                        # of pin (never silent-skip, never merge on poison).
                        violation_branch = str(violation.get("branch") or "")
                        is_missing_attestation = violation_branch in (
                            "scan_input_absence",
                            "format_drift",
                            "",
                        )
                        if _is_unattested_grok_pin(run_ctx) and self_verify_ok and is_missing_attestation:
                            outcome = "handoff_ready_unattested"
                            # Success outcome (merge-ready): no stage FAILED. The
                            # reason discriminator carries the unattested signal; a
                            # gate branching on `if failed_stage:` must not misread
                            # this as a failure (REV-S1-A-002).
                            failed_stage = None
                            outcome_reason = "attestation_missing"
                            error_reason = (
                                "self-verify green and commit landed under a non-composer pin; "
                                "composer attestation missing/failed (explicit discriminator)"
                            )
                        else:
        ...
        if outcome == "handoff_ready":
            ...
            elif _is_unattested_grok_pin(run_ctx):
                # Reconcile HEAD gate with implementation note: non-composer pin green is not
                # silent skip — emit an explicit unattested discriminator so the
                # orchestrator can close_slice without treating it as a blocker.
                self_verify_ok = self_verify_result is None or bool(self_verify_result.get("passed"))
                if self_verify_ok:
                    outcome = "handoff_ready_unattested"
                    # Success outcome: no failed stage; reason carries the signal
                    # (REV-S1-A-002).
                    failed_stage = None
                    outcome_reason = "attestation_missing"
