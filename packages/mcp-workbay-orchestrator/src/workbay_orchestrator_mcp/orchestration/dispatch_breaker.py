...
_PRESPAWN_PREFLIGHT_MARKERS = frozenset(
    {
...
def _is_worktree_local_prespawn_refusal(result: dict[str, object], raw: dict[str, Any]) -> bool:
    """True when the host refused before any VM contact.

    ``_capability_gate_result`` stamps ``capability_receipt_reason`` for
    capability/attestation unknown, incomplete identity (empty gate_host),
    missing transport, and stale worktree configuration. Host recipe parse
    refusals stamp ``preflight``. Those are per-worktree facts, not evidence
    that the external model or transport is failing.
    """
    for source in (result, raw):
        reason = source.get("capability_receipt_reason")
        if isinstance(reason, str) and reason.strip():
            return True
        if source.get("preflight") in _PRESPAWN_PREFLIGHT_MARKERS:
            return True
        missing = source.get("missing_fields")
        if isinstance(missing, list) and any(str(item) == "gate_host" for item in missing):
            return True
    return False


def _is_degenerate_no_attempt(result: dict[str, object], raw: dict[str, Any]) -> bool:
    """True when the envelope carries a typed first-turn no-attempt marker.

    A model that returned a well-formed stub reached the VM; this is a
    per-attempt behaviour, not configuration or transport health. Substring
    inference on the error message is not a classification signal.
    """
    for source in (result, raw):
        if source.get("outcome") == "degenerate_no_attempt":
            return True
        reason = source.get("degenerate_reason")
        if isinstance(reason, str) and reason.strip():
            return True
    return False


def _is_operator_stop(result: dict[str, object], raw: dict[str, Any]) -> bool:
    """True when the envelope carries a typed operator-stop marker.

    A mid-probe SIGTERM can still surface a completed-looking configuration
    stamp from the adapter. Typed ``operator_stop is True`` is the host-side
    fact; substring inference and truthy strings are not a classification
    signal.
    """
    for source in (result, raw):
        if source.get("operator_stop") is True:
            return True
    return False
...
def classify_failure(result: dict[str, object]) -> str | None:
    ...
    Worktree-local pre-spawn refusals stay neutral even when the adapter
    stamps ``failure_class=configuration``: they never reached the VM.
    A typed first-turn ``degenerate_no_attempt`` is a per-lane model
    hiccup, never a configuration latch, even when the adapter stamps
    ``failure_class=configuration``.
    A typed ``operator_stop`` is an interrupted probe, never a
    configuration latch, even when the adapter stamps
    ``failure_class=configuration``.
    """
    ...
    if defer_reason.startswith("residual_timeout_"):
        return "neutral"
    if _is_worktree_local_prespawn_refusal(result, raw):
        return "neutral"
    if _is_degenerate_no_attempt(result, raw):
        return "neutral"
    if _is_operator_stop(result, raw):
        return "neutral"
    ...
    if defer_reason in {
        "vm_lane_cap",
        "vm_memory_pressure",
        "vm_measured_saturation",
        "vm_admission_unmeasured",
