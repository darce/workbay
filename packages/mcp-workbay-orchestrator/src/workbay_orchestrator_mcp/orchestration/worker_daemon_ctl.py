    ...
    *,
    ...
    if row is not None:
        ...
        if row.capabilities.supports_token_budget_cycle_bounds:
            ...
            cycle_timeout = int(
                cycle_timeout if cycle_timeout is not None else resolve_remote_lane_timeout_s(backend)
            )
        ...
        if effective_model:
            try:
                spec = build_agent_spec(
                    ...
                    max_turns=(single_cycle_bounds.get("max_turns") if row.capabilities.supports_token_budget_cycle_bounds else None),
                    agent_turn_timeout_s=(
                        cycle_timeout if row.capabilities.supports_adapter_timeout_bounds else None
                    ),
                )
...
def mint_worktree_capability_receipt(
    *,
    ...
    but does not replace the current attestation record.
    """
    ...
    if not isinstance(transport_digest, str) or not transport_digest.strip():
        return WorktreeCapabilityReceiptRefusal(
            ...
            error=(
                "remote worker start could not mint a capability receipt: "
                "worktree transport is missing"
            ),
        )
...
def daemon_start(
    *,
    ...
    if backend in {"remote", "codex-remote"} or backend.endswith("-remote") or capability_receipt_id is not None:
        ...
        if (
            capability_config_digest is not None
            and capability_config_digest != expected_identity.get("config_digest")
        ):
            return {
                "ok": False,
    ...
    if force and escalated:
        ...
        if lease is not None:
            lease_release = release_remote_lane_lease(
                ...
                sandbox_root=(
                    lease.get("sandbox_root") if isinstance(lease.get("sandbox_root"), str) else None
                ),
            )
    base_payload.update(
        {
