"""``workbay-hostgov`` — standalone host-memory admission probe (internal).

A single admission implementation shared by the in-process orchestrator gate
and out-of-process callers (notably ``scripts/remote_gate.sh``'s pre-``make``
hook on the OCI gate host). ``probe`` reports the host snapshot + admission
verdict and maps the verdict onto an exit code so a shell caller can gate on
it without parsing:

    0   allow    (spawn may proceed)
    75  defer    (retryable — pressure/slots; caller should back off and retry)
    76  refuse   (a resource floor is breached; do not spawn)

remote_gate.sh treats any non-zero as "back off" (its own ``exit 75``), so the
distinct 75/76 split is informational for humans and richer callers.
"""

from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

from .host_resources import (
    COST_HEAVY,
    COST_LIGHT,
    COST_REMOTE,
    COST_REMOTE_API,
    COST_SUITE,
    AdmissionDecision,
    evaluate_admission,
    load_host_memory_policy,
    probe_host,
    resolve_live_admission,
)
from .worktree_stock import WORKTREE_STOCK_CEILING_CONTRACT

_EXIT_BY_DECISION = {"allow": 0, "defer": 75, "refuse": 76}


def _admission_reason(reason: str) -> str:
    """Surface the per-class ceiling contract at this CLI decision site."""
    if WORKTREE_STOCK_CEILING_CONTRACT in reason:
        return reason
    return f"{reason}; {WORKTREE_STOCK_CEILING_CONTRACT}"


def _evaluate(
    *,
    cost_class: str,
    workspace_root: Path,
    held_slots: int | None,
    lane_kind: str = "implement",
) -> AdmissionDecision:
    # held_slots=None => count live slots (the real gate path); an explicit int
    # is an override for testing/manual what-if probing.
    if held_slots is None:
        return resolve_live_admission(workspace_root, cost_class, lane_kind=lane_kind)
    resources = probe_host(force=True)
    policy = load_host_memory_policy(workspace_root)
    return evaluate_admission(resources, cost_class, policy, held_slots, lane_kind=lane_kind)


def _render_human(decision: AdmissionDecision) -> str:
    snap = decision.snapshot
    avail_detail = (
        f"{snap.available_ram / 1024**3:.1f}GiB" if snap.available_ram is not None else "unavailable"
    )
    return (
        f"admission: {decision.decision.upper()} ({_admission_reason(decision.reason)})\n"
        f"  cost_class={decision.cost_class} derived_width={decision.width_label()} "
        f"width_source={decision.width_source} "
        f"held_slots={decision.held_slots} enforced={decision.enforced}\n"
        f"  platform={snap.platform} available_ram={avail_detail} "
        f"pressure={snap.pressure}" + (f" probe_error={snap.probe_error}" if snap.probe_error else "")
    )


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="workbay-hostgov",
        description="Host-memory admission probe (exit 0 allow / 75 defer / 76 refuse).",
    )
    sub = parser.add_subparsers(dest="command", required=True)
    probe = sub.add_parser("probe", help="probe host + report admission verdict")
    probe.add_argument("--json", action="store_true", help="emit the decision as JSON")
    probe.add_argument(
        "--cost-class",
        default=COST_HEAVY,
        # PMH-F5: include every gate-visible cost class (COST_REMOTE_API AND the
        # fully off-box COST_REMOTE) so the standalone probe reaches parity with
        # the in-process gate; omitting one made `--cost-class <tier>` a nonzero
        # argparse error that shell wrappers misread as an admission defer, and
        # left the tier un-inspectable from the CLI.
        choices=[COST_HEAVY, COST_SUITE, COST_LIGHT, COST_REMOTE_API, COST_REMOTE],
        help="spawn cost class to evaluate (default: heavy)",
    )
    probe.add_argument(
        "--workspace-root",
        default=".",
        help="repo root to load the host_memory policy from (default: cwd; "
        "absent/invalid contract => enforce defaults)",
    )
    probe.add_argument(
        "--held-slots",
        type=int,
        default=None,
        help="override the live held-slot count (default: count the slot registry)",
    )
    probe.add_argument(
        "--lane-kind",
        default="implement",
        choices=["implement", "review"],
        help="stock bulkhead to evaluate (default: implement)",
    )
    args = parser.parse_args(argv)

    if args.command == "probe":
        held = None if args.held_slots is None else max(0, args.held_slots)
        workspace_root = Path(args.workspace_root).expanduser()
        decision = _evaluate(
            cost_class=args.cost_class,
            workspace_root=workspace_root,
            held_slots=held,
            lane_kind=args.lane_kind,
        )
        # internal D2b: `probe` is the natural "confirm my
        # contract took effect" surface, so surface loader warnings (e.g. a
        # misplaced top-level `host_memory:` block) here too — not only in the
        # offload_preflight MCP response.
        try:
            warnings = list(load_host_memory_policy(workspace_root).warnings)
            if args.json:
                out = decision.to_dict()
                out["reason"] = _admission_reason(decision.reason)
                if warnings:
                    out["policy_warnings"] = warnings
                print(json.dumps(out, sort_keys=True))
            else:
                print(_render_human(decision))
                for warning in warnings:
                    print(f"  warning: {warning}", file=sys.stderr)
            return _EXIT_BY_DECISION.get(decision.decision, 76)
        finally:
            # Rendering/output failures must not strand a stock reservation.
            decision.release_stock_claim()

    parser.print_help(sys.stderr)  # pragma: no cover - argparse requires subcommand
    return 2


if __name__ == "__main__":  # pragma: no cover
    raise SystemExit(main())
