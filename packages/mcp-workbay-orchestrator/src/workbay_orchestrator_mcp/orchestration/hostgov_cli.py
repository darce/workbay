...
def main(argv: list[str] | None = None) -> int:
    ...
    probe.add_argument(
        "--cost-class",
        default=COST_HEAVY,
        # PMH-F5: include COST_REMOTE_API so the standalone probe reaches parity
        # with the in-process gate; omitting it made `--cost-class remote_api`
        # a nonzero argparse error that shell wrappers misread as an admission
        # defer, and left the new tier un-inspectable from the CLI.
        choices=[COST_HEAVY, COST_SUITE, COST_LIGHT, COST_REMOTE_API],
