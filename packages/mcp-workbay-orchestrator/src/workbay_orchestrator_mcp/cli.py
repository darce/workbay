...
Subcommands:
  ...
  lane-reap                Reap conclusive-dead non-terminal lane rows (dry-run by default).
                           ...
                           or empty never-started lanes). Keep is sticky until w1-45
                           routes keep:empty_lane through close_lane_stale.
...
def _lane_dispose_backfill_rows(
    *,
    ...
    for row in rows:
        ...
        has_receipt = _lane_dispose_receipt_has_evidence(state_dir, task, lane_id, recorded_only=True)
        candidate = dict(row)
...
def _handle_lane_reap_cross_session(args: argparse.Namespace) -> None:
    ...
    empty_raw = payload.get("empty_lanes")
    try:
        empty_count = int(empty_raw or 0)
    except (TypeError, ValueError):
        empty_count = 0
    empty_ids = payload.get("empty_lane_ids") if isinstance(payload.get("empty_lane_ids"), list) else []
    if empty_count > 0 or empty_ids:
        raise SystemExit(_CROSS_SESSION_BACKLOG_EXIT)
