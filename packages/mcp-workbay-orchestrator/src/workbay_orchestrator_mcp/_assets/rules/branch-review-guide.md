...
**Parallel review fan-out (branch/worktree isolation):** when fanning out multiple reviewers over an **identical commit**, do not leave a branch+worktree per reviewer. Prefer sharing one branch/worktree for all identical-commit reviewers (scoped write isolation is via per-reviewer `task_ref`, not a private git tip). If a harness creates a per-reviewer branch/worktree, after findings are harvested **retire the reviewer branch**: once that tip is an ancestor of the harvest target, run `git worktree remove <path>` then `git branch -d <branch>`. MCP finding-row retirement does not reclaim git resources. See the `review-parallel` skill for the coordinator merge protocol.
...
### Reviewer Branch Naming (Remote Dispatch)

Name reviewer branches with the conforming task-ref grammar: `feature/rev-<slug>-r<N>` (lowercase, hyphen-separated, starts with a letter, contains a digit — e.g. `feature/rev-spool-drainflow-r1`).

Do not use `review/*` names for branches that will be dispatched to remote lanes. The engine's VM push runs the pre-push naming hook (`scripts/hooks/check_branch_naming.py`) without `WORKBAY_ALLOW_NONCONFORMING_BRANCH_PUSH`, so a `review/*` branch is refused at push time and the lane dies with a "review phase ended without clean handoff" error that does not name the real cause. The grammar is only enforced at dispatch, so a nonconforming reviewer branch can work locally for the entire review and still fail the moment it is offloaded.
...
**Runtime red-flags:** this guide does not vendor heuristics. Pull the lexicon from the canon repo (see [heuristics canon](heuristics-canon.md)), filter on the `·r` review-phase tag, and cite rules by ID in findings. In a network-less sandbox, inline the rules you need into the dispatch brief.
...
| **Resilience/Stability** | Timeouts, circuit-breaker behavior, pool exhaustion, unbounded result sets, retry/backoff, steady-state reclaimers | Missing timeout, retry storm, resource leak, cache without bound |
| **Concurrency/Async** | Event-loop blocking, shared state across `await`, cancellation hygiene, orphan tasks, lost-update/write-skew | Blocking async path, race across await, swallowed `CancelledError` |
| **Data/Consistency** | Consistency model, replication lag, schema compat, fencing tokens, partition skew | Unstated consistency, lost update, integration DB access |
| **Performance/Tail-Latency** | p99 targets, fan-out amplification, coordinated omission, hot-path complexity | Average-only claims, scatter/gather tail risk, O(n²) on hot path |
