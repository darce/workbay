...
- [ ] **Coupling-type triage (Nygard)** — classify each cross-module link (Operational / Developmental / Semantic / Functional / Incidental); harmful coupling flagged. See [engineering-heuristics.md § Refactoring & Design](engineering-heuristics.md#refactoring-design).
- [ ] **Independent-deployability test** — services claimed independent must ship without being tested alongside collaborators. See [engineering-heuristics.md](engineering-heuristics.md#refactoring-design).
- [ ] **Ports & adapters at external seams** — out-of-scope dependencies wrapped behind minimal adapter. See [engineering-heuristics.md](engineering-heuristics.md#refactoring-design).

### Non-Functional & Failure-Mode Realism

- [ ] **Consistency model stated** — replicated/cached/multi-reader design declares strong/causal/session/eventual + stale-read tradeoff. Q: what stale reads are acceptable? See [engineering-heuristics.md § Data & Consistency](engineering-heuristics.md#data-consistency).
- [ ] **Replication-lag anomalies** — async replicas: read-your-writes, monotonic reads, causal order each named with mitigation. See [engineering-heuristics.md](engineering-heuristics.md#data-consistency).
- [ ] **Schema evolution compat** — rolling upgrades coexist old+new code; expand→migrate→contract for no-downtime changes. See [engineering-heuristics.md](engineering-heuristics.md#data-consistency).
- [ ] **SLA / capacity math** — per-feature SLA and front/back capacity ratios; promo load cannot invert safe ratios. See [engineering-heuristics.md § Resilience](engineering-heuristics.md#resilience-failure-modes) and [§ Performance](engineering-heuristics.md#performance-tail-latency).
- [ ] **Partition / hot-spot skew** — partition key choice reviewed for range-key and low-cardinality hot spots. See [engineering-heuristics.md](engineering-heuristics.md#data-consistency).
- [ ] **Steady-state reclaimer** — accumulators (tables/logs/sessions/cache) have same-rate purge. See [engineering-heuristics.md](engineering-heuristics.md#resilience-failure-modes).
- [ ] **Cache bound + invalidation** — unbounded cache and miss-path latency called out. See [engineering-heuristics.md](engineering-heuristics.md#resilience-failure-modes).
- [ ] **Distributed-transaction cost** — atomic writes across 2+ stores justify 2PC/consensus cost. See [engineering-heuristics.md](engineering-heuristics.md#data-consistency).
- [ ] **Integration DB red-flag** — plan does not read another service's DB directly. See [engineering-heuristics.md](engineering-heuristics.md#resilience-failure-modes).
...
- [ ] **Orientation-readout gate.** Plans that change orientation, context loading, task selection, or planning/review intake must cite a current `orientation_reads_report` assessment. Proceeding without that citation requires a reviewer-approved waiver recorded as an MCP decision on the task ref, and the plan must cite that decision id; a self-attested "not required yet" note does not satisfy this gate.
...
- [ ] **YAGNI / speculative-generality gate** [REF-12] — reject hooks/abstract layers with one (or zero) current consumers. See [engineering-heuristics.md § Refactoring & Design](engineering-heuristics.md#refactoring-design).
- [ ] **Leaky abstraction** [REF-16] — abstractions must not leak transport/storage detail into domain logic. See [engineering-heuristics.md](engineering-heuristics.md#refactoring-design).
- [ ] **Trade-off named** [ARCH-06] — the plan states what its chosen approach sacrifices; a plan with no downsides hasn't found them yet. See [§ Architecture & Trade-offs](engineering-heuristics.md#architecture-trade-offs).
- [ ] **Service-split justification** [ARCH-01]/[ARCH-02] — splits name a measured disintegrator and rule out integrators; no table gains a second writer.
- [ ] **Subdomain classified** [DOM-01]/[DOM-02] — new modules state core/supporting/generic; CRUD-simple logic is not DDD-ified. See [§ Domain Modeling](engineering-heuristics.md#domain-modeling).
- [ ] **Untested-code path planned** [TEST-01]/[TEST-02] — plans touching untested code name the seam/characterization step before the change step. See [§ Testing Strategy](engineering-heuristics.md#testing-strategy).
- [ ] **LLM trust boundaries** [SEC-01]/[SEC-04]/[SEC-05] — plans granting model agency name scopes, gates, and output mediation. See [§ Security](security-heuristics.md#1-aiml-agent-security).
- [ ] **Collection/latency bounds** [API-01]/[API-07] — new endpoints bound collections and hold no long synchronous connections. See [§ API Design](engineering-heuristics.md#api-design).

Cite rules by ID in planning findings. Plan-phase rules carry the `·p` tag in the lexicon's `T·P` column — filter on the tag; do not copy rows into this guide.
