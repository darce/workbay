...
CURRENT_TASK_SIDE_EFFECT_WRITER_UNAVAILABLE_TYPE = (
    "current_task_side_effect_writer_unavailable"
)
...
def resolve_handoff_orchestrator_cli(
    *,
    ...
    """
    running = Path(expected_module_location).resolve() if expected_module_location is not None else running_orchestrator_module_location()
    ...
    try:
        ...
        if slash > 0:
            ...
            if remotes_list.returncode == 0:
                configured = {
                    line.strip()
                    for line in (remotes_list.stdout or "").splitlines()
                    if line.strip()
                }
        ...
        with _get_db_connection() as conn:
            ...
            for row in rows:
                ...
                if entry is None:
                    ...
                    age = format_lane_age_label(
                        row["updated_at"], row["created_at"], now=clock
                    )
                    blocker = _last_blocker_text_for_lane(
                        conn,
                        task_ref=task_ref,
