...
def get_turn_metrics_summary(
    ...
    with _get_db_connection() as conn:
        ...
        for row in rows:
            ...
            cost_by_backend_model[backend_model_key] = (
                cost_by_backend_model.get(backend_model_key, 0.0) + row_cost
            )
                ...
                "summary": {
                    ...
                    "derived_cost": {
                        ...
                        "by_backend_model_approximate_usd": {
                            key: round(value, 6)
                            for key, value in sorted(cost_approximate_by_backend_model.items())
                        },
                    },
                },
            }
...
def record_worker_report(
    ...
    if raw_outcome is not None and raw_outcome not in WORKER_REPORT_OUTCOMES and raw_outcome not in PASS_OUTCOME_TO_WORKER_REPORT:
        return _json_response(
            {
                "ok": False,
                "error": (
...
def reap_task_archived_orphan_lanes(
    *,
    ...
    ABSENT from live ``handoff_state`` — that never reached ``blocked`` is invisible
    to :func:`reap_blocked_lanes`; this reaps it regardless of lane status. A
    LIVE-task lane (``task_ref`` still in ``handoff_state``) is NEVER touched (the
