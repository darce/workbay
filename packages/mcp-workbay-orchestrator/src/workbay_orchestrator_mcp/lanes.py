...
def upsert_worktree_lane(
    ...
    supplied = {"backend": backend, "lane_kind": lane_kind, "test_cmd": test_cmd}
    defaults = {"backend": None, "lane_kind": "implement", "test_cmd": None}
    effective = {
        key: value if value is not None else (previous or defaults).get(key) for key, value in supplied.items()
    }
    changed = previous is None or any(effective[key] != previous.get(key) for key in supplied)
    if (
        changed
