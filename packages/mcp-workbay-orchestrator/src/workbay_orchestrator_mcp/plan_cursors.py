...
Seam note: this module owns ``list_plan_cursors`` at call time (the
``plan_cursor`` wrapper below calls it as an unqualified module global), so
any monkeypatch of it must target
``workbay_orchestrator_mcp.plan_cursors.list_plan_cursors`` — patching
``workbay_orchestrator_mcp.lanes.list_plan_cursors`` does not intercept calls
made from within this module.
"""
...
def _facade_get_db_connection() -> "AbstractContextManager[sqlite3.Connection]":
    """Resolve through ``lanes`` at call time for facade-compatible patching."""
    from . import lanes  # noqa: PLC0415

    return lanes._get_db_connection()
...
def upsert_plan_cursor(
    ...
    with _facade_get_db_connection() as conn:
