...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "dispatch_wave": (
        ...
        f"lane_specs items: lane_id, backend, token_budget (int>0), timeout_seconds (seconds, must be > 0 — "
...
TOOL_DESCRIPTIONS["run_offload_pass"] = TOOL_DESCRIPTIONS["run_offload_pass"].replace(
    "Never auto-retries; recovery is a new idempotent dispatch.",
    "Optional speed=standard|fast is codex-remote only. "
    "Never auto-retries; recovery is a new idempotent dispatch.",
)
...
_ORCHESTRATOR_OPS_TOOL_NAMES = frozenset({"dispatch_wave", "get_metrics_summary", "await_offload_passes"})
...
def list_tool_domains(*, include_ops: bool = False) -> dict:
    ...
    if include_ops:
        domain_map["ops"] = ["dispatch_wave", "get_metrics_summary", "await_offload_passes"]
...
def _lane_kind_from_row(lane_row: Mapping[str, Any] | None) -> str:
    ...
    return derive_stock_lane_kind(
        lane_row=lane_row if isinstance(lane_row, Mapping) else None
    )
...
def _evaluate_host_admission(
    ...
    *,
    ...
    return resolve_live_admission(
        workspace_root, cost_class, exclude_slots=exclude_slots, lane_kind=kind
    )
...
def _derive_worker_start_cycle_bounds(
    *,
    ...
    """Return grok turns, grok clock, and wall-clock-only adapter clock."""
    ...
    if token_budget is None or token_budget <= 0:
        return grok_max_turns, grok_timeout, adapter_timeout
    if backend_registry.backend_supports_token_budget_cycle_bounds(backend_name):
        ...
        adapter_timeout = offload_preflight.derive_adapter_timeout_bounds(
            token_budget,
...
def worker_start(
    ...
    try:
        ...
        grok_max_turns, grok_timeout, adapter_timeout = _derive_worker_start_cycle_bounds(
        ...
        payload = worker_daemon_ctl.daemon_start(
            ...
            model=model,
            speed=lane.get("preferred_speed") if backend_name == "codex-remote" else None,
            poll_interval=poll_interval,
    ...
    *,
    ...
    try:
        ...
        result = offload_pass.run_offload_pass_engine(
            ...
            state_dir=paths["state_dir"],
            grok_max_turns=grok_max_turns,
            test_cmd=classify_test_cmd(lane_row.get("test_cmd"))[1],
        )
...
def dispatch_wave(
    ...
    *,
    ...
    Each lane dict may set ``speed`` to ``standard`` or ``fast`` only when its
    backend is ``codex-remote``; admission refuses speed for other backends.
...
def offload_preflight(
    *,
    ...
    admission = _evaluate_host_admission(
        ...
        lane_kind=_lane_kind_from_row(preflight_row),
