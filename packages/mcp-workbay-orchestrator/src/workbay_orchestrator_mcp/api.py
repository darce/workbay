...
LOCK_RETRY_EXCLUDED_TOOLS: dict[str, str] = {
    ...
    "turn_metrics": (
        "record always INSERTs a new metrics row; identical re-invocation appends"
    ),
    "lane_communication": (
        "record appends lane_messages / briefs; identical re-invocation is not a no-op"
    ),
    "worker_reports": (
        "record always INSERTs a new report row; identical re-invocation appends"
    ),
    "plan_cursor": (
        "upsert increments dispatch_count when state=dispatched; not whole-call idempotent"
    ),
    "switch_task": (
        "re-invoke on an active row bumps revision; identical replay is not a no-op"
    ),
    "get_latest_slice_review_packet": "read path; whole-call write-lock retry is unnecessary",
    "reconcile_review_findings": (
        "apply=True mutates finding state; dry-run is mixed with a write path"
    ),
    "get_review_findings_summary": "read path; whole-call write-lock retry is unnecessary",
    "manage_orchestrator": (
        "daemon start/stop/pause/single_cycle has process side effects outside SQLite"
    ),
    "manage_worker": "worker process lifecycle is not safe to whole-call re-run",
    "run_structured_turn": (
        "executes a turn with external side effects; re-run is not a no-op"
    ),
    "dispatch_lane_work": (
        "dispatches work / may spawn a worker; re-run is a double dispatch"
    ),
    ...
    "dispatch_wave": (
        "multi-lane wave dispatch; re-run can double-dispatch lanes"
    ),
...
def worker_start(
    ...
    if not _wt_ensure.ok:
        body: dict[str, Any] = {
            "ok": False,
            "error": _wt_ensure.error
            or f"Lane worktree does not exist for lane '{lane_id}': {worktree_path}",
...
def dispatch_lane_work(
    ...
    with core._get_db_connection() as conn:
        ...
        ctx = _lanes._resolve_write_actor(
            conn, None, allow_missing_worktree_fallback=True
        )
        ...
        try:
            ...
            if want_packet:
                skip_w = _cursor_remote_context_packet_skip_warning(
                ...
                if skip_w is not None:
                    ...
                    worktree_for_packet = str(
                        lane_row.get("worktree_path") or paths["workspace_root"]
                    )
                    ...
                    normalized_brief = lcp.append_packet_to_brief(
                        normalized_brief, built.get("packet")
                    )
    ...
    if _pending_ct_render:
        ...
        try:
            ...
            _writer_result = _write_current_task_md_for_task(
                object(), _ct_render_task_ref
            )
        except Exception as exc:
            _logger.warning(
        ...
        if current_task_md_written is False:
            side_effect_error_type = (
                side_effect_error_type or _side_effect_fallback
            )
...
def assert_multipass_outcome_partition_total() -> None:
    ...
    covered = (
        ...
        | (
            _multipass_terminal_failure_outcomes()
            - _MULTIPASS_JOIN_ONLY_FAILURE_OUTCOMES
        )
    )
    ...
    overlap_success_nonterm = sorted(
        _MULTIPASS_TERMINAL_SUCCESS_OUTCOMES & _MULTIPASS_NONTERMINAL_OUTCOMES
    )
    if overlap_success_nonterm:
        raise AssertionError(
            f"multipass partitions overlap: {overlap_success_nonterm!r}"
        )
    ...
    if "worktree_unrecoverable" not in failures:
        raise AssertionError(
            "worktree_unrecoverable must be a multipass terminal failure outcome"
        )
