...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "offload_preflight": "Fail-Fast grok offload pre-checks before dispatch: probe grok-cli availability, model pin, clean worktree, token_budget set; returns derived single-cycle max_turns/timeout.",
    "materialize_offload_lane_manifest": "Write/patch the lane manifest preferred_backend/preferred_model so review_runner pins review to grok-cli for offload lanes.",
}
...
def offload_preflight(
    *,
    ...
    """Fail-Fast grok offload pre-checks before any dispatch spend."""
    paths = _worker_paths()
    offload_preflight = _import_orchestration_module("offload_preflight")
    backend_registry = _import_orchestration_module("backend_registry")
    try:
...
def materialize_offload_lane_manifest(
    *,
    ...
    """Patch lane manifest reviewer-backend pin for grok offload lanes."""
    paths = _worker_paths()
    offload_preflight = _import_orchestration_module("offload_preflight")
    try:
        manifest_path = offload_preflight.materialize_offload_lane_manifest(
            orchestrator_root=paths["workspace_root"],
            task_ref=task_ref,
            lane_id=lane_id,
            worktree_path=worktree_path,
    ...
    return core._json_response(
        {
            ...
            "preferred_backend": offload_preflight.GROK_OFFLOAD_BACKEND,
            "preferred_model": offload_preflight.GROK_OFFLOAD_MODEL,
        }
    )
