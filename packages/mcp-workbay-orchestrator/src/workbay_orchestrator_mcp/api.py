...
def _evaluate_host_admission(workspace_root: Path, *, cost_class: str = "heavy") -> Any:
...
def _admission_gate_error(
    ...
    *,
    ...
    if override:
        ...
        if workspace_root is not None and task_ref:
            ...
            if clear_crash_breaker(workspace_root, task_ref):
                _logger.info(
                    "admission_override cleared the crash-breaker marker for %s", task_ref
                )
        return None
    ...
    if workspace_root is not None:
        ...
        record_admission_telemetry(
            workspace_root, admission, surface=surface, task_ref=task_ref, lane_id=lane_id
        )
...
def worker_start(
    ...
    admission = _evaluate_host_admission(paths["workspace_root"], cost_class="heavy")
    ...
    with core._get_db_connection() as conn:
        ...
        if normalized_brief is not None:
            ...
            if existing_open is not None:
                message_row = _lanes._decode_lane_message_row_dict(
                    _lanes._row_to_dict(existing_open)
                )
        ...
        resolved_backend = backend or (updated_lane or {}).get("backend")
...
def run_offload_pass(
    ...
    Returns a typed outcome enum (`handoff_ready | needs_guidance |
    ...
    checkpoint | server_stale_restart_required`) with token telemetry, checkpoint refs, and always-present
    discriminators: ``commit_landed: bool``, ``failed_stage`` in
    ...
    if not dry_run:
        ...
        if gate is not None:
            gate["lane_id"] = lane_id
            gate["outcome"] = (
                "admission_refused" if admission.decision == "refuse" else "admission_deferred"
            )
            return core._json_response(gate)
...
def offload_preflight(
    *,
    ...
    # unless the operator passes admission_override.
    admission = _evaluate_host_admission(paths["workspace_root"], cost_class="heavy")
