...
from workbay_protocol import HARNESS_CONTRACT_RELPATH, INSTRUCTIONS_RELPATH
...
_OVERLAY_MANIFEST_NAMES = (".workbay-overlay.json",)
...
def _resolve_contract_paths(workspace_root: Path) -> tuple[Path, Path | None]:
    ...
    manifest_path = next(
        (workspace_root / n for n in _OVERLAY_MANIFEST_NAMES if (workspace_root / n).is_file()),
        None,
    )
...
def _build_mcp_from_registry(entries: list[ToolEntry]) -> FastMCP:
    mcp = FastMCP(
        ...
        instructions=(
            "You are connected to the WorkBay Orchestrator MCP server. "
