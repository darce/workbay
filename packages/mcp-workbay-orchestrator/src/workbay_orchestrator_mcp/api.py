...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "run_offload_pass": "Run one synchronous offload pass over an actionable lane: bounded execute→review→fix with a commit gate, mandatory positive token_budget + timeout_seconds, and a typed outcome enum (handoff_ready|needs_guidance|no_actionable_work|uncommitted_work|token_budget_exceeded|timeout|error|lane_not_found; await_offload_pass additionally reports still_running). needs_guidance means the worker submitted a blocked/unverified handoff — never merge-ready. Never auto-retries; recovery is a new idempotent dispatch.",
    "await_offload_pass": "Bounded continuation for an offload pass that outlived one client call window: long-poll (pass_id, wait_seconds) for the persisted pass outcome or still_running with a progress snapshot. One call per wait window, not a poll loop.",
...
def run_offload_pass(
    ...
    error | lane_not_found`) with token telemetry and checkpoint refs.
    `needs_guidance` means the worker submitted a blocked or verification-failed
    handoff — the submission landed but the work is NOT merge-ready. Un-budgeted
    or un-timeboxed calls are refused before any spend. The engine never
...
def await_offload_pass(
    ...
    the pass outcome, or ``still_running`` with a progress snapshot. This is a
    coarse bounded wait, not a poll loop — one call per wait window.
    """
