...
# pure read; retire is fail-closed and a second identical call after success
# returns already_retired without touching disk. Every operation is safe to
# re-run as a unit.
# ---------------------------------------------------------------------------
...
LOCK_RETRY_ALL_OPERATIONS_SAFE: dict[str, frozenset[str]] = {
    "manage_worktree_lane": frozenset({"close", "list", "update_routing", "upsert", "retire"}),
}
...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "manage_worktree_lane": "Compound tool: create, close, list, update routing, or retire worktree lanes in one call. Use operation='upsert'|'close'|'list'|'update_routing'|'retire'. retire is dry-run by default (apply=false).",
