...
### Added
- **`gpt-6-luna` on `codex-remote`.** It gets a tier row and joins `MAX_EFFORT_MODELS`.
- **Resolved role record in `offload_preflight`.** Preflight now builds one frozen, versioned role record from the request, the role manifest and the persisted lane row, and returns it as a new `resolved_role` block.
- **Catalogue-backed model admission.** Preflight admits the resolved model and effort against a catalogue snapshot no older than 300 seconds; a stale snapshot is refreshed and revalidated, and a snapshot with a future or invalid timestamp, a mismatched digest or a failed refresh is refused as `catalogue_unavailable`.
- **Native Grok model metadata.** For `grok-cli` and `grok-remote`, preflight asks the Grok agent's stdio protocol which models and efforts it offers and merges them into the catalogue snapshot.
- **`manage_worktree_lane(operation="retire")`.** Retires a terminal lane, dry-run by default (`would_retire`).

### Changed
- **The remote Codex CLI pin is now 0.155.1.** CLI 0.153.4 cannot use `gpt-6-luna`: the server refuses it with HTTP 400 ("not supported when using Codex with a ChatGPT account") and leaves it out of that client's model catalogue.
- **`review_complete` now requires a declared verdict.** A review lane completes only when its findings block parsed and the reviewer's last `REVIEW_VERDICT:` line says `pass`, `pass_with_findings` or `fail`.
- **One lane-timeout resolver.** Lane timeouts now resolve through a shared resolver that reads the neutral `WORKBAY_LANE_TIMEOUT_S` alongside the legacy `WORKBAY_CODEX_LANE_TIMEOUT_S`: whole seconds from 300 to 3600, default 1800.
- **Role identity is checked, not relabelled.** A conflicting explicit identity, or an unreadable lane row or manifest, is refused with `conflicting_role_identity`.
- **The Grok CLI adapter accepts an admitted role.** `GrokCliAdapter.execute` takes an optional `resolved_role`.
- **Remote admission reports measured VM capacity.** VM admission defers are now classified by the gauge that bound them: `vm_measured_saturation`, `vm_admission_unmeasured`, or the legacy `vm_pid_saturation` (gauge `vm_pids`).

### Fixed
- **Never-started lanes are no longer reclaimed as merged.** A lane is now kept as `keep:empty_lane`, instead of being reclaimed on ancestry alone, when its branch has no commits beyond its recorded base or, without a recorded base, when its tip is on the target branch's first-parent history or was committed before the lane was created.
- **Reap and drain previews no longer write.** `reap_merged_registry_worktrees(apply=False)`, the `reap_blocked_lanes` wrapper and the `drain_branch_reclaim_queue` preview no longer enqueue branch-reclaim rows or persist the drain cursor.
- **The cross-session reap probes instead of assuming.** Its dry run now runs the apply-time guard probes read-only and reports what apply would refuse (`would_refuse`, per-item `guard_outcome`).
- **Lane provisioning symlinks no longer block reclaim.** An ignored symlink that resolves outside the worktree, such as a linked `.venv`, now counts as regenerable.

## [0.3.9] — 2026-09-21

### Changed
- Requires workbay-protocol 0.3.2, whose tool-serving index now lists this package's `lane_dag` tool.

## [0.3.8] — 2026-09-20

### Added
- **Lane-terminal branch disposition (implementation note).** New `orchestration/lane_terminal_dispose.py` introduces a `BranchDisposition` taxonomy and an enqueue-only disposer: a lane that reaches a terminal state is classified and queued for the branch reclaimer rather than deleted inline.
- **`lane-dispose` operator CLI** with `scan`, `backfill`, and `abandon` subcommands over the disposition queue, so an unclassified or stuck terminal lane can be inspected and resolved without hand-editing state.
- **Dispatch gate and review-complete-close test coverage** for the offload pass, plus a bash 3.2 portability gate over the shipped worktree-lane shell paths.

### Changed
- **Preflight attestation threads `task_ref`** through receipt minting end to end, and a merged-negative attestation is visible again instead of being dropped on the floor.
- **A semantic manifest failure no longer quarantines sibling lanes** (`RV-MANIFEST-H-01`) — the error is scoped to the lane that produced it.
- **Review lanes no longer commit their `.review` scratch as an implementation checkpoint** (`RV-REVIEW-CHECKPOINT-H-01`).
- **Self-verify is gated on `BRIEF_ACK` and skipped entirely for review lanes**, and `remote_exec` treats an unknown self-verify capability tolerantly instead of refusing the dispatch.
- **An absent-branch lane with no recoverable evidence closes as stale** instead of wedging: the caller's integration tip is recorded as landing proof, and a parked review lane keeps its tree as review evidence.
- **The branch reclaimer only reclaims branches that already merged** — the guard is now documented and covered by tests, so "reclaimed" can never mean "deleted unmerged work".

## [0.3.7] — 2026-09-19

### Changed
- Lane lifecycle contracts are frozen and given JSON codecs; `lane_manifest` gains `repair_manifest` and `missing_ok` retirement, owned-path backfill accepts doubles, and an invalid present `integration_branch` returns a typed refusal instead of being coerced.
- Post-merge reaping uses the tolerant retirement mode in the production path, closes lanes merged into the registered integration branch, and forwards `task_ref` through census and post-merge.
- Capability receipts hash the concrete `auto`/`inherit` effort token, so preflight, execute and `worker_daemon_ctl` agree on a per-cycle digest; `remint_capability_receipt` fails closed as `receipt_expired` on expired or partial write gates under a single lock, and never stores a transport verdict stronger than the locked current record.
- Dispatch refusals are typed rather than silent: a no-op `test_cmd`, a dirty worktree (now naming the dirty count), and `run_offload_pass` all return `dispatch_refused`, and `remote_exec` reports wall-clock expiry.

## [0.3.6] — 2026-09-18

### Changed
- Lane-check types a missing Ruff or mypy module as `system_error` with cause `unresolvable_style_module` instead of a behavioral `failed` or advisory style debt; style-chain fixtures no longer depend on host tool installs.

### Fixed
- Worker-daemon checkpoint writes preserve prior handoff failure/refusal states, and same-file baseline overlap refuses the checkpoint instead of mixing pre-dispatch edits into the salvage commit.
- grok-cli now passes the resolved lane `test_cmd` into sandbox `provision_env` and raises `SecureSandboxError` on `provision_failed_*` so grok never launches against an unprovisioned environment.
- Package-imported `lane_exec` binds sibling modules through the package (not a second `sys.path` copy) and restores the handoff runtime after reading the lane-row `test_cmd`.

## [0.3.5] — 2026-09-18

### Changed
- Capability receipts: per-lane attestation records so concurrent mints no longer stale each other; omitted codex-remote speed hashes canonically; stale receipts remint once at dispatch; same-pass remint keeps mint, bind and live-probe in one chain.
- Merge-gate tests are selected from the integration delta and wired into lane-check; hidden shell separators fail safe; the row's direct runner argv is kept.
- Lane-row `test_cmd` must pass `classify_remote_test_cmd` before dispatch; remote environment denylist replaced by provenance classes; `remote-gate.env` is literal KEY=VALUE data.
- Reaping: cross-session reap CAS-closes against the row's recorded base; post-merge reap retires merged-lane owned_paths; sibling scan skips the live primary.
- Daemon: replay-lock descriptor leak closed and handoff subprocesses bounded; mid-probe SIGTERM is a typed operator stop; first-turn degenerate model exits are non-latching per lane.
- Corrupt remote debug logs fail closed for every backend; review lanes pin the original subject tip so branch-as-tip retries no longer diff the previous payload into itself.
- Typed refusals: exit-3 invalid salvage patches, worktree-local pre-spawn attestation refusals, schema refusals keep string blockers whole; tolerant review harvest turns HIGH/MEDIUM blockers into findings.

## [0.3.4] — 2026-09-16

### Changed
- Capability receipts are enforced before every remote attempt; preflight receipts persist observed probe evidence, and admission fail-closes on model-scoped configuration latches.
- Dispatch breaker: half-open trials land correctly, admission-defer streaks no longer leak, and daemon configuration digest drift is rejected.
- Cross-session lane reaping (`lane-reap --cross-session`, daemon N-tick cadence) reclaims preserved, clean, unowned lanes with one under-claim guard and degraded reporting for failed passes.
- Review lanes: `review_subject` is derived and pinned at provisioning, review output preservation fail-closes, and finding-id collisions are re-keyed instead of dropped.
- grok-remote one-turn placeholder and empty-details stub exits now classify as typed failures instead of success.
- Terminal events without launch receipts are born complete, so redispatch no longer deadlocks on operator recovery.
- Salvage and rescue: retained-attempt rescue before redispatch, nonce-bound root-confined rescue evidence, harvest reserve, and host timeout refusal.
- Codex lanes default to a bounded 1800-second wall clock; lane-gate Git probes are bounded and identity-checked; force stop is fenced to verified descendant identities.
- Context packets fall back cleanly when the codemap index is missing, and prior-art injection is wired into lane briefs.
- Added the `WORKBAY_TEST_TIER` pytest plugin for deterministic fast/e2e test tiering.

## [0.3.3] — 2026-09-13

### Changed
- Remote preflight uses a READY/GRANT transport: stdin carries only the one-shot grant, the caller's deadline is carried through host bootstrap, a term-resistant model is killed at the deadline, and remote clock skew no longer refuses a valid probe.
- Probe cleanup binds each probe to its producer before it acts, and closes a race between cleanup and a concurrent rematerialize.
- Remote sandbox reaping and worker-daemon control are now their own modules (`remote_sandbox_reap`, `worker_daemon_ctl`).
- Lane routing fields are written as one tuple and roll back together if the SQL write fails.

### Known issues
- Finding G1C-01 (high) is still open.

## [0.3.2] — 2026-09-11

### Changed
- Structured-turn subprocess exit is now bounded, so a worker that overruns its body timeout cannot hold the wave open at interpreter exit.
- Probe reaping verifies process identity before signalling and no longer re-looks-up a pid in a way that defeated the escalation to SIGKILL.
- Lane-attribution reaping and preflight-drill coverage added.
- Census git failures preserve DAG provenance instead of dropping it.

## [0.3.1] — 2026-09-11

### Fixed
- Shipped payload changes since 0.3.0 cover the API, lane management, remote execution adapter, budget-floor, dispatch-breaker, offload-pass, offload-preflight, and wave-dispatch modules.

## [0.3.0] — 2026-09-10

### Added
- Lane orchestration now exposes bounded rescue, reclaim, and reaping operations with validated routing metadata and typed refusal outcomes, so callers can distinguish admitted work from deferred or unsafe cleanup.

### Changed
- Reaping now scans beyond permanently ambiguous rows under a progress-based batch budget, admits guarded cleanup of eligible terminal-lane worktrees, and classifies generated ignored content from bounded provisioning evidence.
- Terminal lane closeability is distinct from worktree reclaim eligibility; merged, closed, and closed-stale rows can shed landed worktrees without a second lifecycle transition, while archived rows remain excluded.

### Fixed
- Blocked-lane scans tolerate a missing decisions ledger and report typed cursor-persistence degradation while preserving other database errors.
- Blocked-lane sweeps resume budget-deferred rows before fresh priorities so repeated previews and CAS refusals cannot starve later eligible rows.
- Generated-content reclaim coverage supplies the required bootstrap ledger configs array while retaining ignored-secret safety assertions.
- Merged-registry sweeps persist and count typed checked-out-branch refusals without creating positive branch-reclaim candidates.
- Readable `.git` symlinks now resolve for collector roots and neighboring checkouts, preserving repository affinity and admission.
- Gitdir and commondir paths containing NUL now report unknown occupancy and refuse admission, preventing CR/LF-before-NUL paths from resolving to an unrelated repository (RVSIBA11-M-01).
- Surviving private gitdirs with missing common-directory metadata now make occupancy unknown unless standalone repository structure is valid.
- Every rejected sibling metadata parse now reports unknown occupancy and refuses admission at any ceiling.
- Worktree stock fails admission closed on oversized sibling common-directory metadata, resolves directory-form private gitdirs through their common directory, and accepts filesystem-byte paths and Git-compatible detached HEAD trailing whitespace (RVSIBA6-M-01 through RVSIBA6-M-04).
- Worktree stock accepts chained symbolic HEADs and validates bounded common-directory metadata before HEADs, without launching Git for sibling HEAD validation (RVSIBA5-M-01, RVSIBA5-M-02).
- Worktree stock counts genuine unregistered siblings with Git-created symbolic HEADs or readable common-directory symlinks, preserving admission accounting for relocated private gitdirs (RVSIBA4-M-01, RVSIBA4-M-02).
- Worktree stock resolves the collector's actual Git common directory, so relocated private gitdirs and symlinked worktree administration directories preserve sibling counts.

### Fixed
- Worktree stock sibling affinity now uses the checkout's resolved git common directory instead of a directory-name prefix, so an unrelated neighbour that shares a name prefix is not counted and a genuine worktree under a different name is.
- Worktree stock requires readable, valid HEAD metadata for existing private git directories and preserves Git's common-directory path semantics, so malformed sibling metadata cannot inflate stock or crash the scan.

### Fixed
- Updated the remote Codex CLI pin to 0.153.4.

## [0.2.25] — 2026-09-03

### Changed
- Raised the `mcp-workbay-handoff` floor to 0.2.27.

## [0.2.24] — 2026-09-03

### Added
- Declared-capability remote test-command classifier: structural command containment, a per-repository runner-declaration contract, and the two-gate grammar.
- Capability-matrix coverage, with graceful degradation when a backend does not declare a capability.

### Changed
- Remote test-command input gates admit HTTP(S) URL values on runner-selecting wrapper options, refuse `file:` URLs, and keep `python -m` module approval to the built-in runner set.
- Worktree stock probes bootstrap a cold runtime before counting, so a cold worktree is no longer misread as unstocked.

### Fixed
- Remote refusals name the backend that actually refused.
- Two remote test-runner trust-boundary bypasses are refused: attached-connector command forms (`&&`, leading-attached), and `PATH=` / `env PATH=` rebinding plus the `uv --python` selector.

### Known limitation
- The loader-override family (`PYTHONPATH=`, `LD_PRELOAD`, `DYLD_INSERT_LIBRARIES`) is still admitted by the remote test-command guard, which is a PATH-specific denylist rather than a trusted-loader boundary.

## [0.2.23] — 2026-09-02

### Changed
- Live admission can no longer allow without the stock gate (FIXADM-H-01); it now refuses with `namespace_unusable` instead of silently admitting.
- Merged-registry reap enqueues an authorized branch-reclaim candidate, and the branch-reclaim scan admits the post-teardown absent-and-free state, so a branch whose worktree is already gone stays reclaimable.
- Absent-branch with no landing evidence is now a tagged terminal state rather than an indefinite ambiguous row.
- Landing writes a durable integration receipt; no-ff merges whose tree is unchanged are refused (NOFFLANDING-B-01), and declined no-ff topology is named separately from an intake failure.
- Uncontained post-merge rows split into named content-landed classes instead of one undifferentiated bucket.
- Lane listings always state a named collection with a count.
- Findings persist publishes only ids covered by a claimed-ok receipt.

### Added
- Worker handoff recovery adds `manage_worker action="retry_handoff"`, the `api.worker_retry_handoff` entry point, and `lane-report --delivery-id` for stable exactly-once report claims across retries.

### Changed
- Malformed (non-list) and whitespace-only worker test evidence is now rejected with a worker end-state violation, so those reports no longer earn a slice closure.
- The `mcp-workbay-handoff` dependency floor is now `>=0.2.23,<0.3.0` because the orchestrator report INSERT requires schema 37's `delivery_id` column.

### Fixed
- Failed offload-pass prior-art lookup is wall-clock bounded and runs after the bare failure is persisted, so a hung embedding provider cannot leave the pass unpublished.
- Review structured output now satisfies strict schema rules, treats nullable clean parse status as harvestable, and validates every object node before dispatch.
- Exit-4 host salvage now applies only to execute/build product: a findings-only review turn no longer stages sandbox hunks onto the operator worktree.
