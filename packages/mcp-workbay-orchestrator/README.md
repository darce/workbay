...
# --no-sources is required (the pyproject carries a workspace source); the two
# workbay deps (workbay-protocol + mcp-workbay-handoff) are git-sourced via
# --with, never PyPI. Add `--with "$R#subdirectory=packages/workbay-codex-bridge"`
# too if you install the `[bridge]` extra (the codex-subagent backend).
uv tool install --no-sources \
  --with "git+https://github.com/darce/workbay.git@$REF#subdirectory=packages/workbay-protocol" \
  --with "git+https://github.com/darce/workbay.git@$REF#subdirectory=packages/mcp-workbay-handoff" \
  --from "git+https://github.com/darce/workbay.git@$REF#subdirectory=packages/mcp-workbay-orchestrator" \
...
`workbay-codex-bridge` remains optional unless you want the local bridge backend.
