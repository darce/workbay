...
The orchestration layer supports every backend declared in
`workbay_orchestrator_mcp.orchestration.backend_registry.BACKENDS`:
...
`codex-subagent` is the default `--backend` for package-mode worker CLI
commands such as `worker-start` / `worker-all`; the git-tool install above
includes `workbay-codex-bridge` so that default is reachable after the MCP
server reconnects. Slash `/offload` and `run_offload_pass` default to
`grok-cli` (or `grok-remote` under a remote-only install), not `codex-subagent`.
