...
`workbay install` delegates to `workbay-bootstrap` and defaults
`--source package` (the published `workbay-system` payload). Pass
`--source git_overlay` to clone `darce/workbay` at the pinned ref.
`doctor` / `status` / `repair` default to cwd. Hook adapters use
`--hooks none|repo|all` (`none` is the install default; `recorded` is repair-only).
The installer materializes overlay surfaces, registers both MCP servers
