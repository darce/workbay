...
# Watched trees hold ~411 files; snapshotting after each of ~13,000 tests is
# ~5.4 million stat calls plus rglob scans. Cache file lists by directory mtime.
_DirSignature: TypeAlias = tuple[tuple[str, int], ...]
_WatchedFileCache: TypeAlias = dict[str, tuple[_DirSignature, list[str]]]
_WATCHED_FILE_CACHE: _WatchedFileCache = {}


def _wb_dir_signature(root: Path) -> _DirSignature:
    """Return ``(relative dir, st_mtime_ns)`` for ``root`` and every subdirectory."""
    ...
    while stack:
        ...
        try:
            ...
            continue
        collected.append((os.path.relpath(current, root_str), st.st_mtime_ns))
        try:
...
def _wb_enumerate_watched_files(root: Path) -> list[str]:
    """Return unsorted absolute file paths under ``root`` via ``os.scandir``."""
...
def _wb_cached_watched_files(root: Path) -> list[str]:
    """Return cached file paths for ``root``, re-enumerating only on dir-mtime change."""

    key = os.fspath(root)
    signature = _wb_dir_signature(root)
    cached = _WATCHED_FILE_CACHE.get(key)
    if cached is not None and cached[0] == signature:
        return cached[1]
    files = _wb_enumerate_watched_files(root)
    _WATCHED_FILE_CACHE[key] = (signature, files)
    return files
...
def _wb_snapshot_live_state(repo: Path) -> _Snapshot:
    ...
    def add_file(path: Path) -> None:
        if not path.is_file():
            return
        st = path.stat()
        snapshot[str(path.relative_to(repo))] = (st.st_size, st.st_mtime_ns)
    ...
    for rel in _WATCHED_STATE_DIRS:
        ...
        if not root.is_dir():
            continue
        for path_str in _wb_cached_watched_files(root):
            try:
                st = os.stat(path_str)
            except FileNotFoundError:
                continue
            if not stat.S_ISREG(st.st_mode):
                continue
            snapshot[os.path.relpath(path_str, repo_str)] = (st.st_size, st.st_mtime_ns)
