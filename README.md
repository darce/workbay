...
Install the overlay into any git repository with one command:
...
registers the two MCP servers (`mcp-workbay-handoff`,
`mcp-workbay-orchestrator`), provisions `.task-state/handoff.db`, and
...
Before a release is published — or to track an unreleased `git` ref —
install the bootstrap CLI straight from the monorepo subdirectory
instead of the published front door:

```bash
uvx --from "git+https://github.com/darce/workbay@v0.1.24#subdirectory=packages/workbay-bootstrap" \
    workbay-bootstrap install --source git_overlay --target /path/to/your/repo
```

This is the clone-backed git-overlay flow. See
[`docs/CONSUMER.md`](docs/CONSUMER.md) for upgrades and
...
`mcp-workbay-canvas` is private/internal today. Cross-cutting changes
...
| `mcp-workbay-canvas` | Private MCP server: work-graph canvas, lane/task renderers, reconciliation, annotations, diagram export |
...
| `workbay-stack` | Meta-package pinning a known-good set |
...
Install **`workbay`** — the front door. `uvx workbay install` (or a
persistent `uv tool install workbay`) pulls the pinned `workbay-stack`
runtime and runs the bootstrap installer for you, so you never name the
component packages directly.

| You want to… | Install |
| --- | --- |
| Install or upgrade the overlay in a repo | `workbay` |
| Pin the runtime stack explicitly (CI, reproducible envs) | `workbay-stack` |
| Track an unreleased `git` ref / hack on the installer | `workbay-bootstrap` |

The component distributions (`workbay-protocol`, `mcp-workbay-handoff`,
`mcp-workbay-orchestrator`, `workbay-system`, `workbay-codex-bridge`)
are pulled in by the stack; you do not install them one at a time.
