...
| Cheap work can go to a cheap backend | `/wb-offload` hands one self-contained slice to a bounded lane under a hard token cap, with a fail-fast preflight and no silent substitution of a different backend when the chosen one is unavailable. The lane never merges its own work. |
...
| Command | Use when |
| --- | --- |
| `/wb-scope` | "We should probably add X" and nothing is bounded yet |
| `/wb-draft` | A plan needs drafting or triaging before review; runs the eight analysis passes |
| `/wb-review-plan` | The plan needs formal sign-off with recorded verdicts |

### You are writing code

| Command | Use when |
| --- | --- |
| `/wb-land` | Opening, advancing, or closing the task branch and worktree |
| `/wb-tdd` | Starting a slice test-first, red before green |
| `/wb-implement` | Working through an approved plan slice by slice |
| `/wb-investigate` | A defect needs root-causing before anyone writes a fix |
| `/wb-auto-fix` | A failing test has a known, bounded scope |
| `/wb-refactor` | Auditing code health or sequencing tech-debt work |
| `/wb-offload` | A mechanical slice can go to a cheaper backend under a cost cap |

### You are checking work

| Command | Use when |
| --- | --- |
| `/wb-review-code` | A branch claims to be done |
| `/wb-review-slice` | One reviewer pass is not enough for the diff |

### You are managing the session or the install

| Command | Use when |
| --- | --- |
| `/wb-handoff` | Resuming, switching tasks, or ending a session |
| `/wb-harness` | Turning semantic embeddings on or off, or checking their state |
| `/wb-ux-map` | Authoring or critiquing a text-first UX inventory |

A typical task, end to end:

```text
/wb-scope          "we should probably add X"
/wb-draft          draft and triage the plan through the analysis passes
/wb-review-plan    formal review, findings recorded
make task-start    feature branch + linked worktree
/wb-tdd            first failing test for implementation note
/wb-implement      drive slices to green
/wb-review-code    pre-merge review, findings persisted
...
```text
/wb-scope  we should probably add rate limiting somewhere
```
...
### What `/wb-offload` and lane dispatch do
...
Backends are chosen explicitly, never inferred. `/wb-offload` accepts
...
invocation backends. `codex-remote` and `cursor-remote` are not `/wb-offload`
...
(`/wb-review-slice`). Each lane owns a worktree (or remote workspace), a
...
- The lane commits its own verified work and submits a handoff. It never
  merges. Review the diff (`/wb-review-code` or `/wb-review-slice`) first.
...
| Toggle | Product default on | Consent at install; `/wb-harness` or `workbay embeddings` on/off/status |
...
**What reads the index.** `find_related_prior_work` (an MCP tool, and a
`mcp-workbay-handoff find-related-prior-work` CLI verb) ranks earlier
decisions, findings, and tests against the work in front of you, so a resumed
session or a lane can cite prior art it never saw in a transcript.

**Coverage is measured, not assumed.** `workbay provision-embeddings` fetches
and verifies the pinned model into the shared cache before an install would
otherwise pull it on first use. `workbay doctor` then carries an
`embedding_backfill_gap` facet that counts embeddable rows against embedded
ones per kind and reports the shortfall, so a partial index answers as a
warning instead of as a complete one; the backfill itself is opt-in and never
runs on sync (`make backfill-handoff-embeddings`, or
`python -m workbay_handoff_mcp.scripts.backfill_concept_embeddings` where that
target is not installed). Coverage the probe cannot measure at all is a
distinct `embedding_backfill_unmeasurable` finding rather than a silent pass.
...
2. A reviewing agent, typically a different model family, runs
   `/wb-draft`, `/wb-review-plan`, or `/wb-review-code`. Each is a
...
`doctor` also reports an `mcp_protocol` facet per managed MCP server: the
installed SDK version, the latest protocol version it speaks, the resolved
`fastmcp` version, and the `fastmcp` specifier the package itself declares.
A disagreement between the two servers' declared pins is named there rather
than left to appear later as an import error; the facet is informational and
does not by itself fail `doctor`. Git-only installs carry that declared pin on
the `uv tool install` argv, because `--no-sources` does not inherit it — and an
install that cannot resolve the pin from any source aborts with a typed error
instead of quietly installing an unpinned server.
