...
```bash
REF=v0.1.54   # needs a mirror tag that ships scripts/install-workbay-cli.sh
...
`workbay doctor --target .` detects drift after upgrades; `status`, `update`,
and `repair` round out the lifecycle. See
...
| You want to… | Command |
| --- | --- |
| Install the CLI from a tag | `./scripts/install-workbay-cli.sh v0.1.54` (or the curl one-liner above) |
| Install or upgrade the overlay in a repo | `workbay install --target . --remote-ref v0.1.54` |
| Install via bootstrap CLI | `workbay-bootstrap install --target . --remote-ref v0.1.54` |
