...
# Install the workbay CLI with its whole runtime closure git-sourced (no PyPI).
# --no-sources is required: each member pyproject carries a workspace source
# that uv rejects on a #subdirectory= install without a workspace root.
...
lifecycle. Pass `--with-codebase-graph` to also register the optional
`codebase-graph-mcp` server (needs the `codebase-memory-mcp` binary on
`PATH`); it is excluded by default so consumers without the binary never
inherit a dead entry. Restart your agent so it picks up the new
surfaces, then:
...
Twelve portable commands cover the lifecycle. Each resolves to the same
...
| `/offload` | Hands a self-contained slice to a bounded grok Composer-2.5 lane, token-governed, behind a review gate | A cheap junior backend can do a slice under a cost cap |
...
WorkBay is often compared to three neighboring product shapes. It is not
a substitute for any of them; each owns a different layer.

### Memory and persistence tools

A memory tool such as mem0, engram, Contynu, or beads saves what was
said and lets an agent search it later. WorkBay saves the workflow
itself and enforces it:

| | Memory tools | WorkBay |
| --- | --- | --- |
| What persists | Conversation summaries or free-text facts | Tasks, decisions, findings, tests, blockers |
| Retrieval | Semantic or full-text recall | Ranked session packet (`load_session`) plus full-text search |
| Authority | Advisory notes the agent may ignore | Git hooks the agent cannot route around |
| Provenance | Text, no git attribution | Rows stamped with branch and commit SHA, checkable against history |
| Review | Not modeled | Findings, verdict-bearing review runs, and merge gates |
| Multi-agent | Shared notes | Decoupled author and reviewer, plus orchestrated worktree lanes |

The two compose: run a recall memory tool for fuzzy cross-session
recall, and let WorkBay own the enforced workflow state.
[`docs/COMPARISON.md`](docs/COMPARISON.md) maps this against mem0,
Contynu, engram, beads, and the other persistence tools in depth.

### Multiplayer company harnesses (for example YC QM)

[QM](https://github.com/yc-software/qm) is Y Combinator's open-source
multiplayer agent harness: cloud-first, Slack + web as primary
surfaces, per-person and per-room scopes, durable sandboxes, crons and
webhooks, company-brain connectors, and org admin / security postures.
It is built for whole-company work (engineering, legal, accounting,
events), not only for code.

WorkBay is a local, git-native **process control plane** for coding
agents. It attaches to Claude, Codex, Cursor, grok, or Copilot via MCP
instead of owning the agent loop, and it enforces merge readiness at
...
| Multiplayer | Humans + agents in Slack and web rooms | Multiple agents on one repo; humans via CLI, dashboard, canvas |
| Authority | Identity, scope grants, sandbox policy | Git hooks the agent cannot route around |
| Background work | Crons, webhooks, watches | Task-driven lanes, offload, review runs |

They compose cleanly for engineering orgs: use a multiplayer harness (or
plain Slack) as the human front door, and WorkBay as the repo process
backend that blocks merge until findings, decisions, and tests are
clean. WorkBay is stronger at SHA-anchored review dispositions and
cross-harness attach without a control-plane deploy; QM is stronger at
always-on automation, non-engineering domains, and productized multiplayer
UX.

The full product comparison and adaptation notes are kept internally.

### Orchestration engines

LangGraph, Temporal, XState, n8n, and Airflow own *what runs next* —
graphs, durable workflows, statecharts, or batch DAGs. WorkBay does not:
the coding agent (or operator) decides the next step, and WorkBay records
obligation and refuses a dirty merge. Pairing notes and graph-theory
directions are kept internally.
...
| You want to… | Command |
| --- | --- |
| Install or upgrade the overlay in a repo | `workbay-bootstrap install --remote-ref v0.1.35` |
