...
The monorepo distributes a single CLI — `workbay-bootstrap` — that
...
package/version**: install or upgrade `workbay-stack`, then run the
package-aware `workbay-bootstrap` install/update.
...
`workbay-system` distribution — no git clone of the monorepo is
needed.

For repeated `status` / `doctor` runs after install, use a persistent
tool install instead so `workbay-bootstrap` lands on `$PATH` (the
`--with workbay-stack` constraint keeps every member at the released
pin set):

```bash
uv tool install workbay-bootstrap --with workbay-stack
workbay-bootstrap status  --target /path/to/your/repo
workbay-bootstrap doctor  --target /path/to/your/repo
uv tool upgrade workbay-bootstrap   # later, to pull a newer stack
```
...
| Server                   | Command line                                                |
| ------------------------ | ----------------------------------------------------------- |
| `workbay-handoff-mcp`      | `uvx mcp-workbay-handoff@0.13.3 --workspace-root . serve-stdio`      |
| `workbay-orchestrator-mcp` | `uvx mcp-workbay-orchestrator[bridge]@0.8.0 --workspace-root . serve-stdio` |
