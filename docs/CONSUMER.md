...
workbay install --target /path/to/your/repo --remote-ref v0.2.1
```
...
The SSOT toggle is the CLI verb (not hand-edited harness settings):

```bash
workbay-bootstrap embeddings --status --target <path>
workbay-bootstrap embeddings --enable --target <path>
workbay-bootstrap embeddings --disable --target <path>
```
...
`/workbay embeddings <on|off|status>`, which delegates to `workbay-bootstrap embeddings`.
...
```bash
workbay-bootstrap update --target . --remote-ref v0.2.1
```
...
```bash
uvx --from "git+https://github.com/darce/workbay@v0.2.1#subdirectory=packages/workbay-bootstrap" \
    workbay-bootstrap install --target /path/to/your/repo --remote-ref v0.2.1
```
