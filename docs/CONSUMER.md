...
```bash
REF=v0.1.54   # tag must include scripts/install-workbay-cli.sh (shipped from this change forward)
...
```bash
./scripts/install-workbay-cli.sh v0.1.54
workbay install --target /path/to/your/repo --remote-ref v0.1.54
```
...
Bootstrap-only CLI (narrower tool; same overlay semantics):
...
What `remote_only` changes at dispatch time: `/offload` resolves an omitted
...
`remote_only` and `grok-cli` under `local_ok`. `/offload` resolves `--agent`
against these profiles:

| `--agent` | Where it runs | Model pin |
| --- | --- | --- |
| `grok-remote` | Provisioned gate host | `grok-4.5` |
| `grok-cli` | Local `grok` binary | `grok-4.5` |
...
records `execution_mode: remote_only`; `/offload` may select only
...
- The lane commits its own self-verified work and submits a handoff. It never
  auto-merges. Review the diff (`/branch-review` or `/review-parallel`) first.
...
```text
/offload --agent grok-remote --effort high --token-budget 120000 "implement the CSV export helper"
```
...
```bash
workbay-bootstrap update --target . --remote-ref v0.1.54
```
