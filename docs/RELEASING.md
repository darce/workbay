...
Consumers then install from the synced tag (git-only closure — `--no-sources`
plus the `--with` deps, since each member pyproject carries a workspace source
uv rejects on a `#subdirectory=` install; see docs/CONSUMER.md for the rationale):
