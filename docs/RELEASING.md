...
make release-public                  # PRIMARY path (dry-run): export + install-check, plan the rest
make release-public FLAGS=--execute  # export -> install-check -> push darce/workbay -> force-sync tags
make smoke                           # git-install the latest consumer tag into /tmp
```

**`make release-public` is the only supported release path** (git mirror
only). Pipeline: `export` -> `install-check` -> `push` -> `tag-sync` ->
`status` (see `scripts/release_public.py`). The legacy `release-pending` /
`release-all` / `release-package` flow is **gone**: the Make targets and
the underlying `release.sh` subcommands (the retired PyPI publish path)
were removed. Use `release-public`.
...
## The release flow: export -> install-check -> push -> tag-sync -> status

Dist-1 made the public git mirror the **only** consumer channel. A
release is five steps, orchestrated by `make release-public`
...
2. **Install-check** — non-mutating pre-push gate
   (`scripts/check_consumer_install.py`): replay the documented consumer
   install recipe against the export tree in a scratch uv venv (every
   exported member as a local-path requirement, **without**
   `--no-sources`). A red gate aborts before any remote mutation.
3. **Push** that commit force to the public `main` on
...
4. **Tag-sync** the per-package `<pkg>-vX.Y.Z` family plus the
   consumer-facing `vX.Y.Z` monorepo tag onto the exported commit, and
   force-push them.
5. **Status** — read-only; confirm the tags and print the suggested next
   consumer bump.

```bash
make release-public                  # dry-run: real export + install-check, no remote mutation
make release-public FLAGS=--execute  # maintainer-run: export + install-check + push + tag-sync
```
...
two mutating steps are `push` and `tag-sync`; `export`, `install-check`,
and `status` never touch the remote. Dry-run is not plan-only, though: it
performs a real export to a temp tree and runs the install-check gate
against it (so it needs `uv` on PATH and index access for dependency
resolution), and it exits non-zero when the gate fails. Pass
`FLAGS=--json` for a machine-readable plan/report; the install-check step
carries a `verdict` (and a resolver-output excerpt on failure).

Consumers then install from the synced tag (git-only closure — the
`--with` deps supply the sibling members that live on no index. Exported
member pyprojects no longer carry `{ workspace = true }` uv sources — the
export strips them and a fail-closed gate verifies none remain — so
`--no-sources` is defense-in-depth rather than load-bearing; see
docs/CONSUMER.md for the rationale):

```bash
REF=vX.Y.Z  # the tag just synced above
...
it, and runs the full git-only closure install
`uvx --no-sources --with "$R#subdirectory=packages/workbay-protocol" --with "$R#subdirectory=packages/workbay-system" --from "$R#subdirectory=packages/workbay-bootstrap" workbay-bootstrap install --target <dir> --remote-ref <tag>`
(where `R="git+https://github.com/darce/workbay.git@<tag>"`).
The install must:
...
Dry-run prints the ordered pipeline
(`export`/`install-check`/`push`/`tag-sync`/`status`), flags which steps
mutate the remote, and lists the git-mirror packages and the suggested
monorepo tag — without mutating any remote state. It is not inert,
though: it runs a real export to a temp tree and the consumer-install
gate against it (needs `uv` and index access), so the reported
install-check verdict is earned, and a red gate fails the dry-run. It is
also the rehearsal path before a high-stakes coordinated release: it
exercises the plan and gate without pushing or tagging.
