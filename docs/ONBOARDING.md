...
- /wb-implement — Drive an approved task plan slice-by-slice. The team's primary build loop.
- /wb-land — Start/advance/finish a task branch (`make task-start`, `make review-ready`, `make task-finish`).
- /wb-review-code — Pre-merge review of a feature branch (`make review-run`).
- /wb-draft — Triage a planning doc for ambiguity/gaps before formal review (`make plan-analyze DOC=...`).
- /wb-review-plan — Formal review of task plans, epics, ADRs (`make plan-review DOC=...`).
- /wb-review-slice — Run multiple independent branch reviewers and merge their findings.
- /wb-tdd — Open a slice from a failing test before implementation.
- /wb-auto-fix — Bounded loop for a known failing test.
- /wb-investigate — Root-cause a defect before editing.
- /wb-scope — Clarify requirements before drafting a feature or epic.
- /wb-handoff — Enter, resume, or end a task session (`make context`).
