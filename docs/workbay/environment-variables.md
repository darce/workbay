...
| `WORKBAY_HANDOFF_EMBEDDINGS_DISABLED` | bool | unset | When truthy, skips embedding provider resolve / auto-provision (kill-switch). Also written as a **file gate** into `.workbay/embedding.env` by `workbay embeddings --disable` (SSOT; `workbay-bootstrap embeddings` is a deprecated alias). **Two overlay rules:** hooks (`scripts/hooks/_envfile.load_embedding_env`) are presence-wins set-if-unset (an empty process value is already present, so the file is not applied); CLI (`apply_embedding_env`) and doctor (`merge_embedding_env`) use nonempty-process-overlay (empty process values lose to the file pin). See `workbay_handoff_mcp.embedding_env`. Status JSON exposes `process_env_disabled` + `effective_enabled`. | `workbay-bootstrap/embedding_provision.py`, `mcp-workbay-handoff/embeddings/provider.py`, `mcp-workbay-handoff/embedding_env.py`, hooks `_envfile.py` |
| `WORKBAY_HANDOFF_EMBEDDING_MODEL` | path | unset | Local ONNX model path for offline embeddings (hash-pinned). Overlay rules match `WORKBAY_HANDOFF_EMBEDDINGS_DISABLED`: hooks = presence-wins set-if-unset; CLI/doctor = nonempty-process-overlay via `workbay_handoff_mcp.embedding_env`. | `mcp-workbay-handoff/embeddings/provider.py`, `mcp-workbay-handoff/embedding_env.py` |
| `WORKBAY_HANDOFF_EMBEDDING_TOKENIZER` | path | unset | Local `tokenizer.json` path. Same two overlay rules as `WORKBAY_HANDOFF_EMBEDDING_MODEL` (`workbay_handoff_mcp.embedding_env`). | `mcp-workbay-handoff/embeddings/provider.py`, `mcp-workbay-handoff/embedding_env.py` |
| `WORKBAY_HANDOFF_EMBEDDING_MODEL_SHA256` | sha256 | pin | Expected model digest; fail-closed on mismatch. Same two overlay rules as `WORKBAY_HANDOFF_EMBEDDING_MODEL` (`workbay_handoff_mcp.embedding_env`). | `mcp-workbay-handoff/embeddings/model_pin.py`, `mcp-workbay-handoff/embedding_env.py` |
| `WORKBAY_HANDOFF_EMBEDDING_TOKENIZER_SHA256` | sha256 | pin | Expected tokenizer digest; fail-closed on mismatch. Same two overlay rules as `WORKBAY_HANDOFF_EMBEDDING_MODEL` (`workbay_handoff_mcp.embedding_env`). | `mcp-workbay-handoff/embeddings/model_pin.py`, `mcp-workbay-handoff/embedding_env.py` |
...
| `WORKBAY_TERMINAL_DISPOSE_ENABLED` | bool | `1` (on) | Enables terminal-lane branch-disposition classification and its actuator arm. Unset is on; set to `0`, `false`, `no`, or `off` for the operator kill switch. `held_unique_work` and `undecidable` are hard refusals and are never armable by this flag or the apply allow-list. | `mcp-workbay-orchestrator/src/workbay_orchestrator_mcp/orchestration/branch_reclaim_delete.py` |
| `WORKBAY_TERMINAL_DISPOSE_APPLY` | comma-list | `review_persisted,review_unproduced,never_started` | Allow-list for terminal dispositions that may delete on apply. An empty value makes terminal disposal report-only; unknown values and the hard-refusal dispositions `held_unique_work` and `undecidable` can never arm deletion. | `mcp-workbay-orchestrator/src/workbay_orchestrator_mcp/orchestration/branch_reclaim_delete.py` |
| `WORKBAY_TOOL_ROSTER_POLICY` | `all` \| `skill` | `all` | Session-start MCP serving policy. `all` preserves the full pinned server surfaces; `skill` derives each server's whitelist from the resolved task roster, status defaults, activated domains, catalog, and `ALWAYS_SERVE`. Unknown values coerce to `all`. This is not an alias of `WORKBAY_HANDOFF_TOOL_PROFILE`. | `mcp-workbay-handoff/src/workbay_handoff_mcp/config.py`, `mcp-workbay-handoff/src/workbay_handoff_mcp/tool_roster.py` |
...
| `WORKBAY_MIN_REMOTE_TOKEN_BUDGET` | int (raiser-only) | `120000` implement / `80000` review | Token-budget floor override for remote offload lanes. Applied as `max(kind_default, override)` so a value below the kind default cannot drop the floor. Non-integer values raise rather than silently keeping 120000/80000. Non-positive integers are ignored. | `mcp-workbay-orchestrator/src/workbay_orchestrator_mcp/orchestration/budget_floor.py` |
