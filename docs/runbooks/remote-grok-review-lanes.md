...
VM admission cap is `WORKBAY_REMOTE_AGENT_MAX_LANES` — the script default is 3,
but it is env-only and this deployment sets it higher (currently 19, measured
against the 19-lane stress envelope). Read the live value rather than assuming
the default; at/above the cap a lane defers with exit 75.

Keep `WORKBAY_REMOTE_AGENT_MAX_LANE_VENVS` strictly above `MAX_LANES`: the
cross-lane LRU venv reap protects only the reaping lane's own venv, so other
live lanes survive only by ranking inside the retention cap by mtime.
