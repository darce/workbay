...
The remote binary and the host adapter must agree on
`codex_lane_config.CODEX_CLI_VERSION`, currently **0.155.1**. Update both
together: the availability probe deliberately refuses version drift.

On 2026-09-22 the same failure shape recurred for `gpt-6-luna`. CLI 0.153.4
on the VM was refused with HTTP 400: “The 'gpt-6-luna' model is not supported
when using Codex with a ChatGPT account.” The model was also missing from the
catalogue that client fetched (`Model metadata for gpt-6-luna not found`).
CLI 0.155.1, logged into the same ChatGPT account (the `account_id` hashes
match), lists the model and completed `-c model_reasoning_effort=max` turns
on `service_tier=default` and on `service_tier=fast`. The
`service_tier=definitely_not_a_tier` control emitted the not-advertised
diagnostic. The server gates the catalogue by client version, so the account
was never the problem. Upgrade the VM binary before landing the pin bump:
between the two steps the availability probe reports version drift, which is
the intended fail-closed state.

On 2026-09-04, CLI 0.145.0 authenticated with the existing VM ChatGPT login
but a `gpt-6-astra` request failed with HTTP 400: “requires a newer version
of Codex.” CLI 0.153.4 succeeded with that same login. The verification also
used the adapter's `--ignore-user-config`, `--output-schema`, `-o`,
`-s workspace-write`, and `.git` writable-root flags: it read a fixture,
wrote an output file, committed in a disposable repository, and returned
the required JSON. This is compatibility evidence, not an Astra throughput
or token-cost measurement; the older timing fixtures remain historical.

Install the measured version as the gate user:

```bash
npm install --global --prefix "$HOME/.local" @openai/codex@0.155.1
"$HOME/.local/bin/codex" --version
"$HOME/.local/bin/codex" login status
```

For a persistent project preference, add `"WORKBAY_CODEX_MODEL": "gpt-6-astra"`
to `.mcp.json` under `mcpServers.workbay-orchestrator-mcp.env`, preserving the
existing keys. Use that file as the source for the native client configs:

```bash
workbay mcp-sync --target . --mcp-servers .mcp.json --apply
workbay mcp-sync --target . --mcp-servers .mcp.json --check
codex mcp get workbay-orchestrator-mcp --json
```

Codex reads `[mcp_servers.workbay-orchestrator-mcp.env]` in
`.codex/config.toml`; it does not use the root `.mcp.json` for this server.
The sync command projects the saved value into Codex, VS Code and Cursor.
Restart the MCP server to load the new environment. An export in a terminal
does not change an already-running server or persist a GUI launch preference.
For standalone offload CLI invocations, the existing environment override
still applies; those commands do not load client MCP configuration themselves.

Durability boundary: a bootstrap config rewrite using `--mcp-servers default`
can replace the managed Codex table and remove project overrides. Supply
`.mcp.json` as the input when refreshing configs, or re-run the sync/check
above afterward. Do not maintain independent model values in each client.
This applies canon `AGT-20` (preference lifetime) and `REF-26` (one intent
across representations), consistent with *Release It!* chapter 14's separation
of deployment configuration from replaceable installation files. `CARD-02`
applies to correcting the source that regeneration reads.

The package default stays `gpt-5.6-sol`; merely passing a different
`--model` does not override the configured model admission policy. Account
access is still required; a version upgrade does not grant model access.
Both `standard` and `fast` speed requests are admitted for Astra after live
CLI 0.153.4 probes completed without unsupported-service-tier diagnostics.
The model identifier is documented in the
[official Astra model reference](https://developers.openai.com/api/docs/models/gpt-6-astra).

Luna admits both speeds on the same evidence standard. A 2026-09-07 gate-VM
probe (`codex exec --skip-git-repo-check -m gpt-5.6-luna -c
service_tier=fast`) completed the turn with no diagnostic, and the control
probe with `service_tier=definitely_not_a_tier` emitted the
`Configured service tier ... is not advertised as supported` line that the
classifier watches for. The `{"default"}` row that preceded it was an
unmeasured assumption, not a measurement. Note the orthogonality: a service
tier is a scheduling class, not a reasoning level, so a Luna lane on the
`fast` tier still runs at `max` effort — the standing Luna directive is
unaffected by the speed choice.

`gpt-6-luna` has its own tier row, with the same envelope as `gpt-5.6-luna`:
junior tier, `max` default, efforts `medium`..`max`, both service tiers, and
the `implement` role. It is also in `effort_policy.MAX_EFFORT_MODELS`. The
catalogue advertises `low`..`max` and no `ultra`. `low` stays out of the row,
as it does for `gpt-5.6-luna`. The service tiers follow the evidence standard
above: 2026-09-22 probes on CLI 0.155.1, with the same bogus-tier control.
Selecting this model does not change the review default. `ROLE_REVIEW` still
...
| `openrouter-remote` | `env_file` + key-info | `WORKBAY_OPENROUTER_API_KEY` | `~/.config/openrouter/env` (mode `0600`); probe also calls OpenRouter `/api/v1/key` |
...
```bash
WORKBAY_OPENROUTER_API_KEY="$(security find-generic-password -s openrouter -a workbay -w)" \
  ./scripts/provision_remote_auth.sh --backend openrouter-remote --host gate@<host>
```
...
## Remote agent execution (OpenRouter on the VM)

Operator-gated provisioning for `openrouter-remote` (implementation note shipped the
credential-port row as `0xalpha-remote`; implementation note retired that id): the
**same codex binary** the `codex-remote` backend uses (installed per implementation note S4, nothing new to install) pointed at OpenRouter's
`z-ai/glm-5.3-flash` listing through a `model_providers.openrouter.*`
override set and an OpenRouter API key. The key lives only on the VM in
the declared `env_file` port; the laptop only SSHes.
Config module:
`packages/mcp-workbay-orchestrator/src/workbay_orchestrator_mcp/orchestration/openrouter_lane_config.py`
(slug, env var, env-file path, key-info URL, `OPENROUTER_MIN_REMAINING_USD`,
advertised effort set).

### Data-handling note (read before routing anything here)

The published pin is Z.ai GLM 5.3 Flash (`z-ai/glm-5.3-flash`), successor of
the 2026-08-23 stealth listing. Every lane brief, every file the agent reads
from the checked-out tree, and every tool output still leaves this host under
OpenRouter's terms. Treat it as publication: do **not** route lanes that
touch private canon, credentials,
...
- codex installed for `gate` on the VM (implementation note S4); `scripts/remote_agent.sh
  doctor` already shows it. `openrouter-remote` reuses that binary — the auth
...
- An OpenRouter API key created for this VM only, **with a spend cap set in
  ...
  ceiling, `OPENROUTER_MIN_REMAINING_USD` (default `1.0`) is the soft floor below
...
```bash
security add-generic-password -s openrouter -a workbay -w     # prompts for the value
security find-generic-password -s openrouter -a workbay -w    # reads it back
```
...
```bash
WORKBAY_OPENROUTER_API_KEY="$(security find-generic-password -s openrouter -a workbay -w)" \
  ./scripts/provision_remote_auth.sh --backend openrouter-remote --host gate@<your-host>
# or from a file you then shred:
./scripts/provision_remote_auth.sh --backend openrouter-remote --host gate@<your-host> --key-file ./openrouter.key
```

**Probe only** (no key touched; exit 0 iff authenticated with usable budget):

```bash
./scripts/provision_remote_auth.sh --backend openrouter-remote --host gate@<your-host> --env-already-written
# stdout: limit=<n> usage=<n> remaining=<n>
#         WORKBAY_OPENROUTER_AUTH_OK
```
...
file on the VM (`rm ~/.config/openrouter/env` as `gate`); the next probe then
reports `WORKBAY_OPENROUTER_AUTH_MISSING` (exit 11) and the backend is not
dispatchable.
...
| exit | marker | meaning |
| --- | --- | --- |
| 0 | `WORKBAY_OPENROUTER_AUTH_OK` | key valid, `remaining >= OPENROUTER_MIN_REMAINING_USD`; the `limit=… usage=… remaining=…` line precedes the marker |
| 10 | `WORKBAY_OPENROUTER_INSTALL_MISSING` / `_INSTALL_BROKEN` | codex absent, not executable, or `--version` fails (checked **before** the env file is read) |
| 11 | `WORKBAY_OPENROUTER_AUTH_MISSING` | env file absent or unreadable |
| 12 | `WORKBAY_OPENROUTER_AUTH_INVALID` | env file has no `WORKBAY_OPENROUTER_API_KEY=` assignment, the value is empty, or it contains a double quote / backslash |
| 13 | `WORKBAY_OPENROUTER_AUTH_FAILED` | OpenRouter answered 401 — key revoked or wrong |
| 14 | `WORKBAY_OPENROUTER_AUTH_UNVERIFIED` | curl/python3 missing on the VM, non-2xx other than 401, non-JSON or non-finite body — never treated as green |
| 15 | `WORKBAY_OPENROUTER_BUDGET_EXHAUSTED` | key valid but `limit` null (uncapped, refused), `limit_remaining` null, or below the threshold; the reading line is still printed when present |
...
gate is inert (no probe, no change). For `openrouter-remote` it reads the
TTL-cached probe's `key_info` (30 s cache — not a new ssh round-trip per
dispatch); when `remaining` is below `OPENROUTER_MIN_REMAINING_USD` (or the probe
reports exit 15) it records an open **blocker** on the lane's task —
`openrouter budget below threshold: remaining=… limit=… usage=… (threshold 1.0
USD; backend openrouter-remote); dispatch refused …` — and returns
...
the stable `openrouter budget below threshold` prefix: a retry loop refreshes the
...
(grok, codex, openrouter, cursor — rendered from the registry, never the value or
...
`auth : registry unavailable … last error: …` line on stderr) and, for openrouter, a `budget` line with the
...
- JSONL item `{"type":"item.completed","item":{"type":"error","message":"Model
  ...
  metadata…"}}` — (quoted slug is the retired stealth pin, kept verbatim as a
  dated 2026-08-23 observation) codex has no metadata for a non-OpenAI slug;
...
`WORKBAY_OPENROUTER_MODEL` selects one of the five hand-curated slugs
`z-ai/glm-5.3-flash`, `deepseek/deepseek-v4-flash`, `qwen/qwen3.8-flash`,
`qwen/qwen3.8-max`, or `qwen/qwen3.8-27b`. Efforts are per-model
(`OPENROUTER_MODEL_ALLOWED_EFFORTS`) and are the slug's advertised
`reasoning.supported_efforts` ∩ `REMOTE_EFFORTS`; `max` and `minimal` never
ship because the remote effort set excludes them. Unadvertised pairs are
refused because OpenRouter accepts them silently. `qwen/qwen3.8-flash` exposes
no `reasoning_effort`, so its recipe omits the effort override.

### Operator checklist (openrouter-remote)

- [ ] OpenRouter key created for the VM only, **credit limit set** in the dashboard
- [ ] Keychain item `-s openrouter -a workbay` holds the key; no file copy left behind
- [ ] `provision_remote_auth.sh --backend openrouter-remote --host … --env-already-written` prints `WORKBAY_OPENROUTER_AUTH_OK` with a `limit=` line
- [ ] `scripts/remote_agent.sh doctor` shows four `auth` lines, `budget  : openrouter-remote limit=…`, and a `price` line
