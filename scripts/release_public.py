...
def render_text(report: dict[str, object]) -> str:
    ...
    for index, step in enumerate(report["steps"], start=1):
        ...
        lines.append(line)
        if (
            step.get("step") == "install-check"
            and step.get("verdict") == "fail"
            and step.get("resolver_output")
        ):
...
def _run_install_check(export_dir: Path, report: dict[str, object]) -> int:
    ...
    try:
        ...
        message = (
            "consumer-install gate timed out after "
            f"{_INSTALL_CHECK_TIMEOUT_S}s\n{out}{err}"
        )
    ...
    set_install_check_verdict(
        report, passed=passed, resolver_output="" if passed else combined
    )
    if not passed:
        sys.stderr.write(
            "[release-public] ABORT: consumer-install gate failed; refusing to "
            "publish an unresolvable export.\n"
        )
        if combined:
...
def _run_export(export_dir: Path) -> None:
    ...
    TimeoutExpired propagate to the caller.
    """
...
def _run_export_and_install_check(report: dict[str, object]) -> int:
    ...
    try:
        ...
        set_install_check_verdict(
            ...
            passed=False,
            resolver_output=(
                f"export timed out after {_GIT_TIMEOUT_S}s; install-check not run"
            ),
        )
        sys.stderr.write(
            f"[release-public] ABORT: export timed out after {_GIT_TIMEOUT_S}s; "
            "install-check not run.\n"
        )
        return 1
    except subprocess.CalledProcessError as exc:
        set_install_check_verdict(
        ...
        sys.stderr.write(
            f"[release-public] ABORT: export failed (rc={exc.returncode}); "
            "install-check not run.\n"
        )
