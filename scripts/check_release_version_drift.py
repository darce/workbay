...
def main(argv: list[str] | None = None) -> int:
    ...
    if not drifts:
        print(
            "ok: no git-mirror package has shipped-payload drift since its version was set"
        )
        return 0

    print(
        "release version drift — these git-mirror packages are not safe to release as-is:"
    )
    for d in drifts:
    ...
    print(
        "Bump each package's version (+ CHANGELOG) so the shipped artifact matches HEAD."
    )
    return 1
