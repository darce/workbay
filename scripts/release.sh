#!/usr/bin/env bash
#
# Release driver for the WorkBay repository.
#
# Usage:
#   scripts/release.sh package <name>            # build + tag + trigger Trusted Publishing for one package
#   scripts/release.sh validate-gate <name>      # build + dispatch dry_run=true workflow (gate-only, no PyPI upload)
...
#
# Publish path: `package` builds locally (twine check only), pushes the package tag, then
# triggers `gh workflow run release-publish.yml -R darce/workbay` for OIDC upload.
# Requires `release-public --execute` to have synced sources to darce/workbay first.
# Fallback: manual `uvx --with keyring twine upload` (see docs/RELEASING.md).
#
...
cmd_preflight() {
    ...
    log "preflight: workbay-stack pin gate"
    if ! python scripts/stack_pins.py check; then
        warn "workbay-stack pins drift from member versions — run 'make stack-pins-sync' (+ stack version bump/CHANGELOG) before releasing."
        if [[ $started_run -eq 1 ]]; then
            write_release_summary_artifacts "preflight" "" "failed"
        fi
        return 1
...
dispatch_published_bytes_workflow() {
    local pkg="$1" dry_run_flag="$2" source_date_epoch="$3"
    local wheel_sha="" sdist_sha=""
    if [[ $DRY_RUN -eq 1 ]]; then
        printf '\033[1;90m[dry-run]\033[0m %s\n' \
            "python scripts/published_bytes_gate.py --dist-dir packages/$pkg/dist --emit-json"
        printf '\033[1;90m[dry-run]\033[0m %s\n' \
            "gh workflow run release-publish.yml -R darce/workbay -f package=$pkg -f dry_run=$dry_run_flag -f expected_sha256_wheel=<wheel> -f expected_sha256_sdist=<sdist> -f source_date_epoch=$source_date_epoch"
        return 0
    fi
    local gate_json parsed
    gate_json="$(python scripts/published_bytes_gate.py --dist-dir "packages/$pkg/dist" --emit-json)" \
        || die "published-bytes gate digest computation failed for $pkg"
    parsed="$(python -c 'import json,sys; d=json.loads(sys.stdin.read()); print(d["wheel_sha256"]); print(d["sdist_sha256"])' <<<"$gate_json")" \
        || die "failed to parse published-bytes gate JSON for $pkg"
    { read -r wheel_sha && read -r sdist_sha; } <<<"$parsed"
    [[ -n "$wheel_sha" && -n "$sdist_sha" ]] || die "published-bytes gate JSON missing wheel/sdist digests for $pkg"
    run "gh workflow run release-publish.yml -R darce/workbay -f package=$pkg -f dry_run=$dry_run_flag -f expected_sha256_wheel=$wheel_sha -f expected_sha256_sdist=$sdist_sha -f source_date_epoch=$source_date_epoch"
}
...
build_gate_validated_dist() {
    ...
    if [[ $DRY_RUN -eq 1 ]]; then
        printf '\033[1;90m[dry-run]\033[0m %s\n' "uvx twine check packages/$pkg/dist/*"
    else
...
cmd_validate_gate() {
    local pkg="${1:-}"
    [[ -n "$pkg" ]] || die "usage: release.sh validate-gate <name>"
    [[ -d "packages/$pkg" ]] || die "no such package: packages/$pkg"

    local source_date_epoch
    source_date_epoch="$(git log -1 --format=%ct HEAD)"

    log "validate-gate: $pkg (dispatch dry_run=true — runner build+gate, no PyPI upload)"
    if ! command -v gh &>/dev/null; then
        die "gh CLI not found — validate-gate dispatches release-publish.yml on darce/workbay"
    fi

    build_gate_validated_dist "$pkg" "$source_date_epoch"
    dispatch_published_bytes_workflow "$pkg" "true" "$source_date_epoch"
    log "validate-gate: dispatched dry_run=true workflow for $pkg"
}
...
cmd_package() {
    ...
    # The single-package publish path must also gate on version drift — otherwise
    # `release-package` ships payload changes with no bump (the 0019 bug class).
    if ! python scripts/check_release_version_drift.py; then
        die "release version-drift gate failed — a publishable package's shipped payload changed since its version was set; bump it (+ CHANGELOG) before releasing."
    fi

    # Same fail-closed posture for the meta-package anchor: a member must never
    # publish while the workbay-stack pins still say the previous version.
    if ! python scripts/stack_pins.py check; then
        die "workbay-stack pin gate failed — run 'make stack-pins-sync' (+ stack version bump/CHANGELOG) so the meta-package anchors this release."
    fi
    ...
    # Publish via Trusted Publishing on darce/workbay (primary path).
    # Requires `release-public --execute` to have synced the source + tag to
    # darce/workbay before this step. The workflow uses GitHub OIDC — no local
    # PyPI token needed.
    if ! command -v gh &>/dev/null; then
        die "gh CLI not found — install it (https://cli.github.com) or use the twine fallback: uvx --with keyring twine upload packages/$pkg/dist/${prefix}-${version}*"
    fi
    dispatch_published_bytes_workflow "$pkg" "false" "$source_date_epoch"
...
case "$SUBCOMMAND" in
    ...
    validate-gate) shift; cmd_validate_gate "${1:-}" ;;
