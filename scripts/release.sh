...
cmd_monorepo() {
    ...
    # Tag + push are one atomic step: a local-only release tag is invisible to
    # every other clone (the observed v0.1.58 failure mode), so a failed push
    # must fail the cut loudly — but never delete the tag already created.
    if [[ $DRY_RUN -eq 1 ]]; then
