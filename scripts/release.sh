...
ensure_no_existing_tag() {
    ...
    if git rev-parse -q --verify "refs/tags/$tag" >/dev/null; then
        die "tag $tag already exists locally — if a prior run cut the tag but the push failed, do NOT delete it: verify with 'git ls-remote --tags origin refs/tags/$tag' and, when the tag is absent from origin, publish it with 'git push origin $tag' instead of re-running release.sh. Delete with 'git tag -d $tag' only when the tag itself is genuinely wrong."
    fi
...
cmd_preflight() {
    ...
    log "preflight: uv.lock staleness gate"
    if ! make check-uvlock-current; then
        warn "root uv.lock is stale relative to workspace pyprojects — run 'uv lock' (or 'make release-uv-lock') and commit it before releasing."
        if [[ $started_run -eq 1 ]]; then
            write_release_summary_artifacts "preflight" "" "failed"
        fi
        return 1
...
cmd_monorepo() {
    ...
    log "monorepo: cutting $tag (annotated, with provenance message)"
    # Provenance (REQUEST 2026-08-23): the tag message records the monorepo
    # SHA being tagged and — when resolvable — the published distro tag SHA in
    # the machine-greppable form 'distro: darce/workbay@<sha>'. The distro URL
    # is factored from scripts/release_public.py (PUBLIC_GIT_REMOTE) — the one
    # place that already knows it — never hardcoded a second time here.
    local head_sha distro_url distro_sha distro_line tag_msg
    head_sha="$(git rev-parse HEAD)"
    distro_url="$(sed -n 's/^PUBLIC_GIT_REMOTE = "\(.*\)"$/\1/p' scripts/release_public.py)"
    distro_line="distro: pending"
    if [[ -z "$distro_url" ]]; then
        warn "monorepo: PUBLIC_GIT_REMOTE not parseable from release_public.py — recording 'distro: pending' (never fails the cut; fix the extraction or release_public.py so future cuts can record the distro SHA)."
    elif [[ $DRY_RUN -eq 0 ]]; then
        distro_sha="$(git ls-remote --tags "$distro_url" "refs/tags/$tag" 2>/dev/null \
            | awk -v ref="refs/tags/$tag" '$2 == ref {print $1; exit}')" || distro_sha=""
        if [[ -n "$distro_sha" ]]; then
            distro_line="distro: darce/workbay@${distro_sha}"
        else
            warn "monorepo: distro tag $tag not resolvable from $distro_url yet — recording 'distro: pending' (never fails the cut)."
        fi
    fi
    tag_msg="$(printf 'workbay monorepo release %s\nmonorepo: %s\n%s\n' "$tag" "$head_sha" "$distro_line")"
    # Tag, local provenance gate, push: one atomic step. A local-only release
    # tag is invisible to every other clone (the observed v0.1.58 failure
    # mode), so a failed push must fail the cut loudly — and once the push has
    # SUCCEEDED the tag must never be deleted. The only window in which
    # deleting the tag is correct is between the two, when the gate below
    # rejects a tag this run just created and has not yet published.
    if [[ $DRY_RUN -eq 1 ]]; then
        printf '\033[1;90m[dry-run]\033[0m git tag -a %s -m <provenance message>\n' "$tag"
        printf '\033[1;90m[dry-run]\033[0m git push origin %s\n' "$tag"
    else
        log "git tag -a $tag"
        git tag -a "$tag" -m "$tag_msg"
        # Gate the cut BEFORE publishing (canon RLSE-02, RLSE-11). The two
        # local facts — the object is annotated, and the 'distro:' line is
        # well-formed — are knowable the instant the tag exists, so they are
        # asserted here in the checker's --offline (local-only) mode rather
        # than after the push. This matters more than usual: once the tag
        # reaches origin, ensure_no_existing_tag HARD-REFUSES re-publication,
        # so a violation detected post-push cannot be corrected in place and
        # costs a version. The origin-presence and distro-SHA checks genuinely
        # cannot run yet; `make release-monorepo` runs the same checker online
        # after this function returns to confirm those.
        log "monorepo: verifying local tag provenance before publishing"
        if ! python scripts/check_release_tag_provenance.py "$tag" --offline; then
            # The tag was created seconds ago by THIS run and has not been
            # pushed, so removing it is a precise undo of our own action — and
            # it is what leaves the repo re-runnable, since ensure_no_existing_tag
            # refuses to start again while a local tag of this name exists.
            warn "monorepo: local provenance check FAILED for $tag — deleting the unpublished local tag to restore the pre-cut state."
            git tag -d "$tag" || warn "monorepo: could not delete local tag $tag — remove it by hand with 'git tag -d $tag' before re-running."
            die "monorepo: refusing to publish $tag — the tag failed its own provenance check (see the violations above). Nothing was pushed and no version was consumed; fix the cause and re-run release.sh for the same version."
        fi
        log "git push origin $tag"
        if ! git push origin "$tag"; then
            die "monorepo: tag $tag was created locally but the push to origin FAILED — the tag is NOT published and other clones cannot see it. Keep the local tag (do NOT delete it), fix connectivity/permissions, then run exactly: git push origin $tag — do NOT re-run release.sh for this version; the tag is already cut and only the push is missing."
        fi
    fi
