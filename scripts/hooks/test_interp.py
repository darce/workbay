...
def test_resolve_deps_python_returns_current_when_deps_present_and_no_venv(
    interp, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_resolve_deps_python_prefers_venv_even_when_deps_present(
    interp, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """implementation note D3: ambient pyenv with stale-but-present stack must not win."""
    _deps_present(monkeypatch)
    _ban_execv(monkeypatch)
    venv_py = _make_venv(tmp_path)
    monkeypatch.setattr(interp, "_git_repo_root", lambda: str(tmp_path))
...
def test_ensure_noop_when_deps_present_and_no_venv(
    interp, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    _deps_present(monkeypatch)
    monkeypatch.setattr(interp, "_git_repo_root", lambda: str(tmp_path))
...
def test_ensure_reexecs_to_venv_when_deps_present_but_venv_differs(
    interp, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """implementation note D3: prefer project .venv even when ambient imports the stack."""
    _deps_present(monkeypatch)
