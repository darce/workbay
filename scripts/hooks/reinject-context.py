...
_FENCE_CLOSE = "```"
_RECOVER_HINT = 'recover: compaction(get_latest) | get_handoff_state(read_profile="hot_summary")'
_HARNESS_CHOICES = ("claude-code", "codex", "grok", "cursor", "manual")
# Cap the one-result reservation at this fraction of the block budget so a
# large operator snippet_chars (clamp 500) cannot consume the generic block.
_SEMANTIC_RESERVATION_FRACTION_DENOMINATOR = 3
_RESERVATION_CAPPED_SKIP = "reservation_capped"
...
class _SemanticBudgetAllocation:
    """Granted semantic-arm budget plus reservation outcome.

    ``kept_base_lines`` is set only when the one-result floor was reserved by
    shrinking the fixed-content preview; the caller adopts that list only
    when semantic lines are available to use the granted space.
    """

    __slots__ = ("granted", "skip_reason", "kept_base_lines")

    def __init__(
        self,
        granted: int,
        skip_reason: str | None = None,
        kept_base_lines: list[str] | None = None,
    ) -> None:
        self.granted = granted
        self.skip_reason = skip_reason
        self.kept_base_lines = kept_base_lines


def _one_result_floor_chars(snippet_chars: int) -> int:
    """Header + one capped-label result; falls back if embeddings are unimportable."""
    try:
        from workbay_handoff_mcp.embeddings.reinjection import (  # type: ignore[import-not-found]
            one_result_floor_chars,
        )

        return one_result_floor_chars(snippet_chars)
    except ImportError:
        header_line_length = len("relevant:") + 1
        return header_line_length + 2 + 48 + 2 + max(0, int(snippet_chars))


def _kept_content_lines(lines: list[str], *, budget_chars: int) -> list[str] | None:
    """Content lines ``_render_block`` would keep (fences stripped), or None."""
    block = _render_block(lines, budget_chars=budget_chars)
    if block is None:
        return None
    return [line for line in block.splitlines() if line not in (_FENCE_OPEN, _FENCE_CLOSE)]


def _rendered_block_chars(content: list[str]) -> int:
    """Occupancy including every joining newline and the newline from print."""
...
def _compute_semantic_content_budget(
    *,
    ...
    base_lines: list[str],
    recover_hint: str,
    notify_allowance_chars: int,  # noqa: ARG001 — signature shared with notify preview callers
    provisional_notify: str,
    snippet_chars: int = _DEFAULT_SNIPPET_CHARS,
) -> _SemanticBudgetAllocation:
    """Return character budget reserved for readable semantic snippet lines.

    Leftover-after-fixed can collapse below one renderable result. When that
    happens and the one-result floor fits in a bounded fraction of the block
    budget, reserve the floor by shrinking the fixed-content preview. When
    the cap prevents honouring the floor, report ``reservation_capped``
    instead of silently clamping. [FM-02] [OBS-08] [RLSE-05]
    """
    envelope_overhead = 0
    ...
    floor = _one_result_floor_chars(snippet_chars)
    reservation_cap = block_budget // _SEMANTIC_RESERVATION_FRACTION_DENOMINATOR
    preview_lines = [*base_lines, recover_hint]
    fixed = _kept_content_lines(preview_lines, budget_chars=block_budget)
    if fixed is None:
        return _SemanticBudgetAllocation(granted=0)
    # The empty semantic placeholder charges its joining newline separately
    # from the service's standalone header-and-bullet character budget.
    leftover = max(0, block_budget - _rendered_block_chars([*fixed, ""]))
    if leftover >= floor:
        return _SemanticBudgetAllocation(granted=leftover)
    if floor + 1 > reservation_cap:
        return _SemanticBudgetAllocation(
            granted=leftover,
            skip_reason=_RESERVATION_CAPPED_SKIP,
        )
    reserved_budget = max(1, block_budget - (floor + 1))
    kept = _kept_content_lines(preview_lines, budget_chars=reserved_budget)
    if kept is None:
        return _SemanticBudgetAllocation(
            granted=leftover,
            skip_reason=_RESERVATION_CAPPED_SKIP,
        )
    if kept and kept[-1] == recover_hint:
        kept = kept[:-1]
    reserved_preview = _kept_content_lines([*kept, recover_hint], budget_chars=reserved_budget)
    if reserved_preview is None:
        return _SemanticBudgetAllocation(
            granted=leftover,
            skip_reason=_RESERVATION_CAPPED_SKIP,
        )
    granted = max(0, block_budget - _rendered_block_chars([*reserved_preview, ""]))
    return _SemanticBudgetAllocation(granted=granted, kept_base_lines=kept)
...
def _render_block(lines: list[str], *, budget_chars: int) -> str | None:
    ...
    if not lines or _rendered_block_chars(lines[:1]) > budget_chars:
        return None
    kept: list[str] = [lines[0]]
    for line in lines[1:]:
        if _rendered_block_chars([*kept, line]) <= budget_chars:
            kept.append(line)
...
def _reinject(
    *,
    ...
    try:
        configure_runtime(RuntimeConfig.for_repo(Path(repo_root), state_dir=state_dir_override))
    ...
    try:
        with _get_db_connection() as conn:
            ...
            disabled, disabled_source = resolve_compaction_disabled(env=os.environ, conn=conn, task_ref=task_ref)
    ...
    if use_semantic and source != "compact":
        _emit(f"reinject semantic skipped: requires source=compact (got source={source})")
        use_semantic = False
    elif use_semantic and source == "compact":
        compaction_id = (
            str(latest_summary.compaction_id) if latest_summary is not None and latest_summary.compaction_id else ""
        )
        if not compaction_id:
            ...
            try:
                ...
                _emit(f"reinject semantic suppressed: sticky-check helper unavailable ({exc}); skip=dedupe_unavailable")
            ...
            if already_reinjected:
                _emit(f"reinject semantic skipped: already reinjected for compaction_id={compaction_id}")
                use_semantic = False
    ...
    if use_semantic:
        ...
        allocation = _compute_semantic_content_budget(
            ...
            snippet_chars=(reinjection_config.snippet_chars if reinjection_config else _DEFAULT_SNIPPET_CHARS),
        )
        semantic_budget = allocation.granted
        if allocation.skip_reason:
            _emit(f"reinject semantic skipped: {allocation.skip_reason}")
            try:
                from workbay_handoff_mcp.embeddings.reinjection import (  # type: ignore[import-not-found]
                    SemanticReinjectionResult,
                )

                semantic_result = SemanticReinjectionResult(
                    status="skipped",
                    skip_reason=allocation.skip_reason,  # type: ignore[arg-type]
                    model_id=None,
                    chars_budget=max(0, semantic_budget),
                )
            except ImportError:
                semantic_result = _PlainSemanticSignal(
                    status="skipped",
                    skip_reason=allocation.skip_reason,
                    chars_budget=max(0, semantic_budget),
                )
        else:
            try:
                ...
                try:
                    from workbay_handoff_mcp.embeddings.reinjection import (
                        SemanticReinjectionResult,  # type: ignore[import-not-found]
                    )

                    sem_lines, semantic_result = (
                        [],
                        SemanticReinjectionResult(
                            status="degraded",
                            skip_reason="error",
            ...
            if sem_lines:
                if allocation.kept_base_lines is not None:
                    lines[:] = allocation.kept_base_lines
    ...
    provisional_notify = format_reinject_notify_message(
        ...
        selected_kinds=([item.kind for item in semantic_result.selected] if semantic_result is not None else None),
    ...
    if block is None:
        _emit(f"reinject skipped: budget {budget_chars} cannot fit the mandatory task_ref line")
        return 0
    ...
    notify_selected_count = len(semantic_result.selected) if semantic_result is not None else None
    notify_selected_kinds = [item.kind for item in semantic_result.selected] if semantic_result is not None else None
    if semantic_result is not None:
        semantic_detail_for_record = semantic_result.to_dict()
        if semantic_result.status == "selected" and not _selected_semantic_delivered(block, semantic_result):
            ...
            _emit("reinject semantic skipped: selected concepts dropped by block budget")
    ...
    if harness == "claude-code" and settings.compaction_notify:
        ...
        _emit(f"reinject emitted: task_ref={task_ref} chars={emitted_chars} shape=json_envelope")
    else:
        print(block)
...
def main() -> int:
    ...
    if source not in enabled:
        _emit(f"reinject skipped: source {source or '<unset>'!r} not enabled (enabled: {','.join(enabled)})")
        return 0
