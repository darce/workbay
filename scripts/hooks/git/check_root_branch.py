...
def main() -> int:
    ...
    try:
        # Use the already-resolved toplevel; do not re-probe raw cwd and discard
        # the answer _repo_toplevel already paid for.
        primary_raw = primary_workspace_root(Path(toplevel))
        if primary_raw is None:
            # Could not determine primary — do not fabricate cwd as primary [OBS-08].
            # Fail-open (return 0) is intentional; the message still names why [OBS-01].
            print(
                "check_root_branch: could not determine primary worktree "
                "from on-disk git metadata",
                file=sys.stderr,
            )
            return 0
