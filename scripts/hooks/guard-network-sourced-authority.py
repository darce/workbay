...
No permitted network→rules crossing
-----------------------------------
There is **no** exception path. Network-fetched content may never land in
``docs/workbay/rules/`` or skill bodies, by any caller and under any
provenance claim. Heuristics canon is consumed raw from its own public
repo by judgment; it is never projected into the local rules tree.
...
def refuse(path: str, detail: str) -> int:
    msg = (
        ...
        f"rules or skill bodies. No network→rules crossing is admitted."
    )
...
def evaluate(
    ...
    *,
    ...
    # Network-sourced write into authority surface — always refuse.
    if is_rules_path(file_path):
        return refuse(
            file_path,
            "network-sourced content must not be written into rules",
        )
    return refuse(
        file_path,
