...
_TASK_PLAN_DIR_NAMES = frozenset({"tasks", "epics"})


def _is_task_plan_path(path: str) -> bool:
    """True for repo-root or nested docs/tasks|epics markdown plans.

    Staged paths from ``git diff --cached --name-only`` are repo-relative and
    have no leading slash, so a bare substring match against markers that
    include a leading slash misses plans at the repository root. Match on path
    segments instead so both ``docs/tasks/...`` and
    ``packages/*/docs/tasks/...`` are recognised without treating an unrelated
    path that merely contains those names as a substring as a plan.
    """
    if not path.endswith(".md"):
        return False
    parts = Path(path).parts
    for index in range(len(parts) - 1):
        if parts[index] == "docs" and parts[index + 1] in _TASK_PLAN_DIR_NAMES:
            return True
    return False
...
def _current_branch(repo: Path) -> str:
    """Return the current branch name, or empty string when git cannot answer.

    An unborn HEAD (repository first commit) makes ``rev-parse --abbrev-ref``
    exit non-zero. Returning empty keeps the guard from crashing the pre-commit
    hook; empty is not a protected branch, so scan_staged short-circuits.
    """
    proc = subprocess.run(
