...
# 2026-09-10: the original named tests measured 5.49s wall on this unloaded
# host after the import fix; 20s leaves 10s before the unchanged 30s harness
# bound for contention and the failure sink.
_HOOK_TOTAL_BUDGET_SECONDS = 20.0
# 2026-09-10: the lightweight errors-record child measured 0.39s wall
# (0.35s user, 0.02s sys) in a temp git repo, and the DB-backed package
# subprocess proof passed in 0.76s test time. Keep a 10s cap, but always clamp
# it to the parent budget's remaining time before spawning.
_ERRORS_RECORD_TIMEOUT_CAP_SECONDS = 10.0
_ACTIVE_BUDGET: _HookBudget | None = None


class _CompactionBudgetExceeded(BaseException):
    """Control-flow interrupt that must cross existing fail-open exception arms."""

    def __init__(self, phase: str, total_seconds: float) -> None:
        self.phase = phase
        self.total_seconds = total_seconds
        super().__init__(f"phase={phase}; total budget={total_seconds:.2f}s")


class _HookBudget:
    """One monotonic deadline shared by all Stop-hook phases and child waits."""
    ...
    def checkpoint(self, phase: str) -> None:
        ...
        if self.remaining() <= 0.0:
            raise _CompactionBudgetExceeded(phase, self.total_seconds)

    def child_timeout(self, cap: float, *, phase: str) -> float:
        """Grant a child only the smaller of its cap and remaining parent time."""
        self.checkpoint(phase)
        timeout = min(float(cap), self.remaining())
        if timeout <= 0.0:
            raise _CompactionBudgetExceeded(phase, self.total_seconds)
        return timeout

    def _alarm(self, _signum: int, _frame: object) -> None:
        raise _CompactionBudgetExceeded(self.phase, self.total_seconds)
    ...
    def arm(self) -> None:
        ...
        signal.setitimer(signal.ITIMER_REAL, max(self.remaining(), 0.001))
        self._armed = True
    ...
    def close(self) -> None:
        ...
        if self._previous_timer is not None and self._previous_timer[0] > 0.0:
...
def _git_repo_root(*, budget: _HookBudget | None = None) -> str:
    active_budget = budget or _ACTIVE_BUDGET
    try:
        proc = subprocess.run(
            ["git", "rev-parse", "--show-toplevel"],
            capture_output=True,
            text=True,
            timeout=(active_budget.child_timeout(5.0, phase="repo-root discovery") if active_budget else 5.0),
        )
...
def _resolve_errors_record_argv() -> list[str]:
    # The package module routes this command through the lightweight direct
    # writer; the full console entry point imports the MCP server API first.
...
def _record_compaction_failed(failure_line: str) -> None:
    ...
    # Argv construction outside the write try (mirror capture-agent-errors
    # REV-B-06): resolution failures are recorded, not silently dropped.
    active_budget = _ACTIVE_BUDGET
    try:
        argv = [
        ...
        try:
            ...
            record_hook_failure(
                ...
                kind="exception",
                detail=f"argv: {type(exc).__name__}: {exc}",
    ...
    try:
        ...
        # _hook_failure_sink can never prevent the primary errors-record write.
        from resolve_handoff_src import is_metadata_dist_src

        env = os.environ.copy()
        repo_root = _git_repo_root() or os.getcwd()
        src_path = _resolve_agent_handoff_src(repo_root)
        # Module branch is [sys.executable, "-m", ...] — after
        # ensure_deps_interpreter() the executor IS sys.executable, so
        # injection is always safe. Only the console-script branch needs
        # metadata-dist suppression (do not copy capture-agent-errors'
        # resolve_deps_python equality test).
        is_module_branch = len(argv) >= 2 and argv[1] == "-m"
        if src_path and (is_module_branch or not is_metadata_dist_src(src_path)):
            env["PYTHONPATH"] = src_path + (os.pathsep + env.get("PYTHONPATH", ""))
        try:
            timeout = (
                active_budget.child_timeout(_ERRORS_RECORD_TIMEOUT_CAP_SECONDS, phase="errors-record")
                if active_budget
                else _ERRORS_RECORD_TIMEOUT_CAP_SECONDS
            )
        except _CompactionBudgetExceeded:
            return
        proc = subprocess.run(argv, capture_output=True, timeout=timeout, env=env, cwd=repo_root)
        # internal: surface non-zero writes via hook-failures.log.
        if proc.returncode != 0:
        ...
        # ImportError of is_metadata_dist_src lands as kind=exception (REV-A-03).
        try:
            from _hook_failure_sink import record_hook_failure
...
def _compact(
    *,
    ...
    """
    if _ACTIVE_BUDGET:
        _ACTIVE_BUDGET.checkpoint("compaction imports")
    try:
        ...
        from workbay_handoff_mcp.config import RuntimeConfig  # type: ignore[import-not-found]
        from workbay_handoff_mcp.runtime import configure_runtime  # type: ignore[import-not-found]
    ...
    if _ACTIVE_BUDGET:
        _ACTIVE_BUDGET.checkpoint("runtime configuration")
    try:
    ...
    if _ACTIVE_BUDGET:
        _ACTIVE_BUDGET.checkpoint("transcript read")
    try:
    ...
    if _ACTIVE_BUDGET:
        _ACTIVE_BUDGET.checkpoint("active task resolution")
    try:
    ...
    if _ACTIVE_BUDGET:
        _ACTIVE_BUDGET.checkpoint("latest compaction lookup")
    try:
    ...
    if _ACTIVE_BUDGET:
        _ACTIVE_BUDGET.checkpoint("turn range derivation")
    try:
    ...
    if latest_summary is not None and latest_summary.session_id == session_id:
        ...
        if current_range.end_turn <= latest_summary.turn_range.end_turn:
            _emit(f"compaction skipped: no new turns since {latest_summary.compaction_id}")
            return 0
    ...
    if new_token_count < settings.min_new_tokens:
        _emit(f"compaction skipped: only {new_token_count} new tokens; threshold {settings.min_new_tokens}")
        return 0

    if _ACTIVE_BUDGET:
        _ACTIVE_BUDGET.checkpoint("compaction gate")
    try:
...
def _run_main(budget: _HookBudget) -> int:
    ...
    budget.checkpoint("interpreter self-heal")
    ensure_deps_interpreter()
    budget.checkpoint("repo-root discovery")
    repo_root = _git_repo_root(budget=budget)
    if not repo_root:
        _emit("compaction failed: unable to resolve repo root")
        return 0
    from _envfile import load_embedding_env

    budget.checkpoint("embedding environment")
    load_embedding_env(repo_root)
    budget.checkpoint("source resolution")
    ...
    budget.checkpoint("compaction settings import")
    try:
        from workbay_handoff_mcp.compaction import CompactionSettings  # type: ignore[import-not-found]
    except ImportError as exc:
        _emit(f"compaction failed: workbay_handoff_mcp import: {exc}")
        return 0

    budget.checkpoint("compaction settings validation")
    try:
    ...
    budget.checkpoint("resolver imports")
    try:
        from workbay_handoff_mcp.compaction import (  # type: ignore[import-not-found]
    ...
    state_dir_override = _env_alias("WORKBAY_HANDOFF_STATE_DIR") or None
    budget.checkpoint("runtime configuration")
    try:
        configure_runtime(RuntimeConfig.for_repo(Path(repo_root), state_dir=state_dir_override))
    except Exception as exc:  # noqa: BLE001
        _emit(f"compaction failed: runtime configuration: {exc}")
    ...
    budget.checkpoint("compaction disable resolver")
    try:
        with _get_db_connection() as conn:
            try:
                resolution = resolve_active_task_ref_for_hook(conn, strict=False)
    ...
    budget.checkpoint("stdin payload")
    try:
    ...
    budget.checkpoint("Stop payload validation")
    try:
    ...
    budget.checkpoint("compaction dispatch")
    return _compact(
        transcript_path=transcript_path,
        session_id=session_id,
        repo_root=repo_root,
...
def main() -> int:
    """Run the Stop hook under one bounded, fail-open wall-clock budget."""
    global _ACTIVE_BUDGET
    budget = _HookBudget(_HOOK_TOTAL_BUDGET_SECONDS)
    previous_budget = _ACTIVE_BUDGET
    _ACTIVE_BUDGET = budget
    budget.arm()
    try:
        return _run_main(budget)
    except _CompactionBudgetExceeded as exc:
        # Stop-hook budget exhaustion is an operational failure, not a reason
        # to block the session that is ending. Disarm before emitting the
        # typed signal; no child is started after the parent deadline expires.
        budget.close()
        _emit(f"compaction failed: hook budget exhausted ({exc})")
        return 0
    except Exception as exc:  # noqa: BLE001 -- every operational path is fail-open
        budget.close()
        _emit(f"compaction failed: hook internal error: {type(exc).__name__}: {exc}")
        return 0
    finally:
        budget.close()
