...
def test_workspace_anchor_prefers_claude(monkeypatch: pytest.MonkeyPatch) -> None:
    """CLAUDE_PROJECT_DIR keeps precedence (Claude is anchor #0)."""
    mod = _load_module()
    monkeypatch.setenv("CLAUDE_PROJECT_DIR", "/tmp/claude-ws")
    monkeypatch.setenv("GROK_WORKSPACE_ROOT", "/tmp/grok-ws")

    assert mod.workspace_env_anchor() == "/tmp/claude-ws"


def test_workspace_anchor_falls_back_to_grok(monkeypatch: pytest.MonkeyPatch) -> None:
    """The regression this fix targets: a non-Claude (Grok) harness resolves a
    root via GROK_WORKSPACE_ROOT instead of dropping to the Claude-only var."""
    mod = _load_module()
    monkeypatch.delenv("CLAUDE_PROJECT_DIR", raising=False)
    monkeypatch.setenv("GROK_WORKSPACE_ROOT", "/tmp/grok-ws")

    assert mod.workspace_env_anchor() == "/tmp/grok-ws"


def test_workspace_anchor_returns_default_when_unset(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """No anchor set → caller-supplied default (empty string preserves the
    ``_git_repo_root`` 'unresolved' contract)."""
    mod = _load_module()
    monkeypatch.delenv("CLAUDE_PROJECT_DIR", raising=False)
    monkeypatch.delenv("GROK_WORKSPACE_ROOT", raising=False)

    assert mod.workspace_env_anchor() == ""
    assert mod.workspace_env_anchor("/fallback") == "/fallback"


def test_workspace_anchor_empty_claude_falls_through(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """An empty-string CLAUDE_PROJECT_DIR is treated as unset and falls through
    to GROK (truthiness delta vs a bare ``os.environ.get`` — intentional, mirrors
    ``_run_guard._workspace_root``)."""
    mod = _load_module()
    ...
    # Empty CLAUDE and no GROK → default (still honours the unresolved contract).
    monkeypatch.delenv("GROK_WORKSPACE_ROOT", raising=False)
    assert mod.workspace_env_anchor() == ""


def test_workspace_anchor_treats_whitespace_only_as_unset(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A whitespace-only anchor is skipped (never returned as a garbage root),
    falling through to the next anchor and then the default."""
    mod = _load_module()
    monkeypatch.setenv("CLAUDE_PROJECT_DIR", "   ")
    monkeypatch.setenv("GROK_WORKSPACE_ROOT", "/tmp/grok-ws")
    assert mod.workspace_env_anchor() == "/tmp/grok-ws"

    monkeypatch.setenv("GROK_WORKSPACE_ROOT", "\t\n")
    assert mod.workspace_env_anchor("/fallback") == "/fallback"
