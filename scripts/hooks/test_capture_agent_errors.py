...
def test_classify_workbay_uv_cli_failure() -> None:
    mod = _load_hook_module()
    event = mod.classify(
        command="uv run workbay-bootstrap doctor",
        output="doctor found install drift",
        exit_code=1,
    )
    assert event is not None
    assert event["error_class"] == "cli_failure"
...
def test_classify_workbay_make_check_install_and_dogfood_failures() -> None:
    mod = _load_hook_module()
    for command in (
        ...
        event = mod.classify(command=command, output="failed", exit_code=2)
        assert event is not None
        assert event["error_class"] == "cli_failure"
