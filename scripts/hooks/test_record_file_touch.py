...
def _touch_subprocess_calls(
    seen: list[tuple[list[str], dict]],
) -> list[tuple[list[str], dict]]:
    """Select the record_file_touch stack subprocess among incidental probes.

    Patching ``subprocess.run`` also intercepts ``_interp`` git discovery
    (``rev-parse --git-common-dir`` / primary-checkout venv probe) because the
    stdlib ``subprocess`` module is shared. internal's injection path
    always calls ``resolve_deps_python``, so call index 0 is no longer the
    touch write — pin assertions must filter by argv shape.
    """
    return [
        (cmd, env)
        for cmd, env in seen
        if len(cmd) >= 3
        and cmd[1] == "-c"
        and "record_file_touch" in cmd[2]
    ]
...
def test_monorepo_layout_fallback_uses_renamed_workbay_handoff_mcp_path(
    ...
    internal disposition: assert against the touch subprocess, not
    the first ``subprocess.run`` (which may be an ``_interp`` git probe).
    """
    ...
    assert mod.main() == 0
    touch_calls = _touch_subprocess_calls(seen)
    assert touch_calls, f"expected record_file_touch subprocess to run; seen={seen!r}"
    pythonpath = touch_calls[0][1].get("PYTHONPATH", "")
...
def test_main_accepts_camel_case_post_tool_use_payload(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    internal disposition: incidental ``_interp`` git probes also hit
    the patched ``subprocess.run``; select the touch argv, not call index 0.
    """
    mod = _load_hook_module()
    tracked = tmp_path / "README.md"
    tracked.write_text("hello")
    ...
    assert mod.main() == 0
    touch_calls = _touch_subprocess_calls(seen)
    assert touch_calls, f"expected record_file_touch subprocess for camelCase payload; seen={seen!r}"
    touch_cmd = touch_calls[0][0]
    # Executor is resolve_deps_python(repo_root) — may be project .venv, not
    # necessarily sys.executable when a primary-checkout venv exists.
    assert touch_cmd[0].endswith("python") or "python" in Path(touch_cmd[0]).name
    assert "record_file_touch" in touch_cmd[2]
    assert "file_path='README.md'" in touch_cmd[2]
    assert "change_kind='edit'" in touch_cmd[2]
