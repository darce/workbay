...
def test_handoff_probe_locked_allows_edit(tmp_path: Path) -> None:
    """Transient sqlite locked/busy must not hard-block every Edit/Write.

    Protocol sys.path made the real DB probe reachable under bare python3; a
    concurrent writer locking the DB is environmental, not identity failure.
    """
    ...
    assert decision is None, (
        "handoff_probe_locked must be allowlisted so a busy DB does not "
        f"hard-block edits; got {decision.outcome if decision else None!r} "
        f"reason={getattr(decision, 'reason', None)!r}"
    )


def test_handoff_probe_ambiguous_allows_edit(tmp_path: Path) -> None:
    """Ambiguous active task is recoverable (make task-reap) and must allow.

    Previously raised ValueError → unconditional block; now a returned token
    on the allowlist so recovery tooling can still edit.
    """
    primary, _feature_root = _make_two_worktrees(tmp_path)
    context = ActiveTaskContext(
        task_ref=None,
        ...
        resolution_note="Ambiguous active task: multiple live rows",
    )
    ...
    assert decision is None, (
        "handoff_probe_ambiguous must be allowlisted; got "
        f"{decision.outcome if decision else None!r} "
        f"reason={getattr(decision, 'reason', None)!r}"
    )


def test_unrecognised_probe_error_still_blocks(tmp_path: Path) -> None:
    """Control: unknown tokens stay fail-closed [AGT-10].

    Proves the locked/ambiguous allowlist is not a blanket allow of every
    probe_error string. Edit targets the resolved worktree so path-drift cannot
    fire; the non-allowlisted token is the sole block-worthy property
    [RELAND-REV-C-02].
    """
    primary, feature_root = _make_two_worktrees(tmp_path)
    unknown = "handoff_probe_totally_unknown_token_xyz"
    context = ActiveTaskContext(
        task_ref="internal",
        target_branch="feature/ambiguity-drift",
        target_worktree=str(feature_root),
    ...
    # Path inside the task target — ordinary drift arm must not fire.
    (feature_root / "docs" / "scopes").mkdir(parents=True, exist_ok=True)
    payload = {
        "toolName": "Edit",
        "toolInput": {
    ...
    decision = evaluate_payload(
        payload, workspace_root=primary, active_task=context
    )

    assert decision is not None and decision.outcome == "block", (
        "unrecognised probe_error must still block (no blanket allow); got "
        f"{decision.outcome if decision else None!r}"
    )
    assert unknown in (decision.reason or ""), (
        "block reason must name the non-allowlisted token so the probe_error "
        f"gate (not path-drift) is what fired; got {decision.reason!r}"
    )
