...
def test_compaction_failed_child_timeout_draws_from_remaining_budget(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """The failure child receives the remaining Stop-hook budget, not a fresh 10s."""
    mod = _load_module(HOOK_SCRIPT, "compact_session_hook_remaining_budget_test")
    captured: dict[str, object] = {}

    class _Budget:
        def child_timeout(self, cap: float, *, phase: str) -> float:
            assert cap == 10.0
            assert phase == "errors-record"
            return 0.25

    class _Proc:
        returncode = 0

    monkeypatch.setattr(mod, "_ACTIVE_BUDGET", _Budget(), raising=False)
    monkeypatch.setattr(mod, "_resolve_errors_record_argv", lambda: ["python", "errors-record"])
    monkeypatch.setattr(mod, "_resolve_harness", lambda: "claude-code")
    ...
    mod._record_compaction_failed("compaction failed: budget exhausted")
...
def test_invalid_settings_records_compaction_failed_row(workspace: Path) -> None:
    ...
    env["PATH"] = os.pathsep.join(dict.fromkeys([os.path.dirname(git_path), "/usr/bin", "/bin"]))
    proc = subprocess.run(
    ...
    failure_log = workspace / ".task-state" / "hook-failures.log"
    if failure_log.exists():
        assert "TimeoutExpired" not in failure_log.read_text(encoding="utf-8")
