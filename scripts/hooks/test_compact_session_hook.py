...
def test_jsonl_default_gate_writes_row_above_default(workspace: Path) -> None:
    ...
    assert "compaction_id=" in result.stderr, (
        "default gate must allow compaction on large JSONL slice; "
        f"stderr={result.stderr!r}"
    )
...
def _jsonl_transcript_at_least_tokens(min_tokens: int) -> str:
    ...
    chunk = "a "
    probe_n = 256
    probe_tokens = _jsonl_gate_token_count(
        _jsonl_turn_transcript(chunk * probe_n, "ack")
    )
    if probe_tokens <= 0:
    ...
    if tokens < min_tokens:
        raise AssertionError(
            f"could not reach {min_tokens} gate tokens (got {tokens} at n={n})"
        )
    return transcript
