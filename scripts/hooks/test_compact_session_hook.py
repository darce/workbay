...
# implementation note installs a default token gate (``DEFAULT_MIN_NEW_TOKENS``, resolved
# from the contract when a workspace contract is present); tests that expect a row
# write must lower the threshold explicitly. ``=0`` disables the gate entirely.
...
def _load_hook_module(name: str = "compact_session_hook_test") -> ModuleType:
    spec = spec_from_file_location(name, str(HOOK_SCRIPT))
...
def test_compact_session_import_path_does_not_load_server_stack() -> None:
    """The Stop-hook import surface must not materialize FastMCP server code."""
    env = os.environ.copy()
    env["PYTHONPATH"] = os.pathsep.join([str(HANDOFF_SRC), str(PROTOCOL_SRC)])
    proc = subprocess.run(
        [
            sys.executable,
            "-c",
            (
                "import sys; import workbay_handoff_mcp.compaction; "
                "print('fastmcp_server_loaded=' + str(int('fastmcp.server.server' in sys.modules)))"
            ),
        ],
        capture_output=True,
        text=True,
    ...
    assert proc.stdout.strip() == "fastmcp_server_loaded=0", proc.stdout
...
def test_compact_session_hook_writes_row(workspace: Path) -> None:
    ...
    assert "compaction_id=" in result.stderr, f"expected compaction_id= on stderr; got stderr={result.stderr!r}"
    ...
    stderr_id = next(
        line.split("=", 1)[1].strip() for line in result.stderr.splitlines() if line.startswith("compaction_id=")
    )
...
def test_compact_session_hook_success_emits_receipt_fields(workspace: Path) -> None:
    ...
    receipt_index = next(
        (index for index, line in enumerate(lines) if line.startswith("compaction_id=")),
        None,
    )
    assert receipt_index is not None, lines
    assert all(
        line.startswith(
            (
                "No git repository found under probed directory",
                "ANALYZE review_findings skipped on connect",
            )
        )
        for line in lines[:receipt_index]
    )
    lines = lines[receipt_index:]
...
def test_compact_session_notify_codex_keeps_stderr_receipt_only(
    workspace: Path,
) -> None:
...
def test_compact_session_hook_failure_is_non_fatal(workspace: Path, tmp_path: Path) -> None:
    ...
    assert "compaction_id=" not in result.stderr, (
        f"failure path must not emit a compaction_id= envelope; stderr={result.stderr!r}"
    )


def test_compact_session_hook_budget_exhaustion_is_non_fatal(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """A slow phase consumes one total budget and degrades through the normal failure signal."""
    mod = _load_hook_module("compact_session_hook_budget_test")
    monkeypatch.setattr(mod, "_HOOK_TOTAL_BUDGET_SECONDS", 0.05)
    monkeypatch.setattr(mod, "_record_compaction_failed", lambda _line: None)

    def slow_phase(_budget: object) -> int:
        time.sleep(0.2)
        return 0

    monkeypatch.setattr(mod, "_run_main", slow_phase)

    assert mod.main() == 0
    captured = capsys.readouterr()
    assert "compaction failed: hook budget exhausted" in captured.err
    assert "0.05s" in captured.err


def test_compact_session_hook_round_trips_into_cold_start_render(
    workspace: Path,
) -> None:
    ...
    assert block is not None, "render_cold_start_compaction must return a block after the Stop hook persists a row"
    assert compaction_id in block, (
        f"cold-start block must dereference {compaction_id} written by the hook; block={block!r}"
    )
...
def test_compact_session_hook_uses_env_harness(workspace: Path) -> None:
    ...
    assert latest.summary.harness == "codex", (
        f"expected harness=codex from WORKBAY_HANDOFF_HARNESS env; got harness={latest.summary.harness!r}"
    )
...
def test_compact_session_hook_does_not_skip_on_new_session(workspace: Path) -> None:
    ...
    assert second.returncode == 0, f"hook exited {second.returncode}; stderr={second.stderr!r}"
    assert "compaction_id=" in second.stderr, (
        f"new session must not be skipped just because turn numbering restarted; stderr={second.stderr!r}"
    )
...
def test_disabled_env_var_skips(workspace: Path) -> None:
    ...
    assert result.returncode == 0, f"hook exited {result.returncode}; stderr={result.stderr!r}"
    assert "compaction skipped: disabled" in result.stderr, f"expected disabled-skip line; got stderr={result.stderr!r}"
    assert "compaction_id=" not in result.stderr
...
def test_min_new_tokens_threshold_skips(workspace: Path) -> None:
    ...
    assert result.returncode == 0, f"hook exited {result.returncode}; stderr={result.stderr!r}"
    assert "compaction skipped: only" in result.stderr, f"expected min-tokens skip line; got stderr={result.stderr!r}"
...
def test_workbay_compaction_disabled_skips(workspace: Path) -> None:
    ...
    assert "deprecated" not in result.stderr.lower(), (
        f"no deprecation warning expected when only the canonical name is set; stderr={result.stderr!r}"
    )
...
def test_compaction_settings_db_disable_skips_hook(workspace: Path) -> None:
    ...
    configure_runtime(RuntimeConfig.for_repo(workspace, state_dir=workspace / ".task-state"))
    with _get_db_connection() as conn:
...
def test_invalid_int_env_var_emits_compaction_failed(workspace: Path) -> None:
    ...
    assert "compaction failed: invalid compaction settings:" in result.stderr, (
        f"expected loud failure on invalid int env var; got stderr={result.stderr!r}"
    )
    assert "compaction_id=" not in result.stderr, (
        f"invalid env value must not silently fall back to default and persist a row; stderr={result.stderr!r}"
    )
    ...
    assert row_count["n"] == 0, f"invalid-settings path must not insert any row; saw {row_count['n']} rows"


def test_thresholds_default_match_plan_0037_gate(workspace: Path) -> None:
    """With no WORKBAY_HANDOFF_COMPACTION_* env vars set (and no workspace
    contract), the hook applies the plan-0037 install fallback default
    (``DEFAULT_MIN_NEW_TOKENS``): skip until new_tokens reaches the gate.
    """
    from workbay_handoff_mcp.compaction import DEFAULT_MIN_NEW_TOKENS
    ...
    assert result.returncode == 0, f"hook exited {result.returncode}; stderr={result.stderr!r}"
    assert "compaction skipped: only" in result.stderr
    assert f"threshold {DEFAULT_MIN_NEW_TOKENS}" in result.stderr
    assert "compaction_id=" not in result.stderr
...
def test_jsonl_default_gate_writes_row_above_default(workspace: Path) -> None:
    """implementation note: real-shape JSONL above the fallback default gate writes a row.

    Mutation check (by hand): deleting the default gate or breaking JSONL ordinal
    slicing must turn this case red alongside
    ``test_jsonl_default_gate_skips_below_default``.
    """
    from workbay_handoff_mcp.compaction import DEFAULT_MIN_NEW_TOKENS

    transcript = _write_transcript(
        workspace,
        _jsonl_transcript_at_least_tokens(DEFAULT_MIN_NEW_TOKENS + 1),
    )
    payload = {
        "hook_event_name": "Stop",
    ...
    assert result.returncode == 0, f"hook exited {result.returncode}; stderr={result.stderr!r}"
    assert "compaction_id=" in result.stderr, (
        f"default gate must allow compaction on large JSONL slice; stderr={result.stderr!r}"
    )
...
def test_jsonl_default_gate_skips_below_default(workspace: Path) -> None:
...
def test_compact_session_hook_skips_when_no_new_turns(workspace: Path) -> None:
    ...
    assert second.returncode == 0, f"second invocation exited {second.returncode}; stderr={second.stderr!r}"
    assert "compaction skipped:" in second.stderr, (
        f"expected 'compaction skipped:' on second invocation; stderr={second.stderr!r}"
    )
    ...
    assert row_count["n"] == 1, f"skip path must not insert a second row; saw {row_count['n']} rows"
...
@pytest.fixture()
def workspace_no_active_task(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Iterator[Path]:
...
def _assert_unresolved_failure(result: subprocess.CompletedProcess) -> None:
    ...
    assert "compaction failed: active task unresolved" in result.stderr, (
        f"expected 'compaction failed: active task unresolved' on stderr; got stderr={result.stderr!r}"
    )
    assert "compaction_id=" not in result.stderr, (
        f"unresolved-task path must not emit a compaction_id= envelope; stderr={result.stderr!r}"
    )
...
def test_compact_session_hook_none_shape_exits_clean(
    ...
    _assert_unresolved_failure(_run_hook(payload, workspace=workspace_no_active_task))
...
def test_min_new_tokens_zero_disables_gate(workspace: Path) -> None:
    ...
    *never fire* the token gate — every turn is skipped and no row is written.
    ...
    assert result.returncode == 0, f"hook exited {result.returncode}; stderr={result.stderr!r}"
    assert "compaction skipped: min_new_tokens gate disabled (override is 0)" in result.stderr, (
...
def test_compact_session_skip_keeps_empty_stdout_under_claude(workspace: Path) -> None:
    ...
    gate skips the tiny transcript, so stdout stays empty and no envelope
    ...
    assert result.stdout == "", (
        f"skip path must keep stdout empty under claude-code+notify-on; got stdout={result.stdout!r}"
    )
...
def _jsonl_transcript_at_least_tokens(min_tokens: int) -> str:
    ...
    Gate token count is essentially linear in repeated chunk count. Measure the
    ratio once on a small probe, build the target corpus analytically, verify
    once, and correct with at most a couple of small increments if the first
    estimate undershoots. Avoids ~35 full-corpus re-encodes from exponential
    probe + binary search.

    Uses a dense whitespace-delimited chunk so the gated count can still clear
    ``DEFAULT_MIN_NEW_TOKENS`` under the default ``max_extract_chars`` cap
    (a lower-density "token " corpus saturates below the gate and would make
    any search non-terminating).
    """
    # Dense: ~1 token / 2 chars after slim+cap, enough headroom above 75k.
    chunk = "a "
    probe_n = 256
    probe_tokens = _jsonl_gate_token_count(_jsonl_turn_transcript(chunk * probe_n, "ack"))
    if probe_tokens <= 0:
        raise AssertionError("probe produced zero gate tokens")
    if probe_tokens >= min_tokens:
        return _jsonl_turn_transcript(chunk * probe_n, "ack")

    # ceil(min_tokens * probe_n / probe_tokens) with a small pad: the single-
    # probe ratio slightly overstates per-chunk yield because fixed JSONL
    # framing is amortized into the rate.
    n = -(-min_tokens * probe_n // probe_tokens)
    n += max(1, n // 50)
    transcript = _jsonl_turn_transcript(chunk * n, "ack")
    tokens = _jsonl_gate_token_count(transcript)
    bumps = 0
    while tokens < min_tokens and bumps < 2:
        bumps += 1
        add = max(1, -(-(min_tokens - tokens) * n // tokens))
        n += add
        transcript = _jsonl_turn_transcript(chunk * n, "ack")
        tokens = _jsonl_gate_token_count(transcript)
    if tokens < min_tokens:
        raise AssertionError(f"could not reach {min_tokens} gate tokens (got {tokens} at n={n})")
    return transcript
...
def test_jsonl_second_stop_compacts_disjoint_turn_range(workspace: Path) -> None:
    ...
    assert second.returncode == 0, f"hook exited {second.returncode}; stderr={second.stderr!r}"
    assert "compaction_id=" in second.stderr, (
        f"second Stop over growing JSONL must compact new turns; stderr={second.stderr!r}"
    )
    ...
    with _get_db_connection() as conn:
        rows = conn.execute("SELECT structured_summary_json FROM session_compactions ORDER BY created_at").fetchall()
    assert len(rows) == 2
...
def test_jsonl_turn_derivation_hook_matches_library(workspace: Path) -> None:
    ...
    assert _derive_turn_range(full_transcript).model_dump() == {
        "start_turn": 1,
        "end_turn": 4,
    }
