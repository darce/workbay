...
def _enforced_manifest_payload() -> dict[str, object]:
    return {
        "schema_version": 1,
        "remote_url": "https://example.invalid/repo.git",
        "remote_ref": "main",
        "remote_sha": "0" * 40,
        "codemap_mode": "enforced",
    }


def test_launch_python_isolated_exec_honors_env_anchor_enforced_manifest(
    tmp_path: Path,
) -> None:
    """NW-0225-02R5RV-004 production path (TEST-15): the real launcher execs
    python -I, so sibling imports are not on sys.path. cwd is unrelated; the
    enforced ledger lives at CLAUDE_PROJECT_DIR and must still refuse."""
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    (workspace / ".workbay-bootstrap.json").write_text(
        json.dumps(_enforced_manifest_payload()),
        encoding="utf-8",
    )
    # Symlink the whole venv, not just the binary: python -I still needs
    # pyvenv.cfg so site-packages (workbay_protocol) stay importable.
    # Use sys.prefix (do not Path(sys.executable).resolve() — that follows
    # the venv python symlink into the base CPython prefix).
    lane_venv = Path(sys.prefix)
    if not (lane_venv / "bin" / "python").is_file():
        pytest.skip("lane venv interpreter unavailable")
    (workspace / ".venv").symlink_to(lane_venv)
    ...
    env.pop("WORKBAY_CODEMAP_MODE_READ", None)
    ...
    proc = subprocess.run(
        ...
        cwd=other_cwd,
        env=env,
        input=json.dumps({"tool_name": "Bash", "tool_input": {"command": "rg foo src/a.py"}}),
        text=True,
        capture_output=True,
        timeout=15,
        check=False,
    ...
    assert "refuse_source_query" in proc.stderr
...
def test_launch_python_bounds_term_immune_candidate_and_reaps_descendant(
    ...
    try:
        try:
            ...
            pytest.fail("launcher hung waiting on TERM-immune candidate (TERM-only probe then blocking wait)")
        assert proc.returncode == 0, stderr
        assert stdout == "ok"
        parent_pid = None
        child_pid = None
        if pidfile.is_file():
            lines = [line.strip() for line in pidfile.read_text(encoding="utf-8").splitlines() if line.strip()]
            if lines:
...
def test_launch_python_ignores_pythonpath_sitecustomize_before_guard(
    ...
    assert not site_marker.exists(), "sitecustomize ran during probe or before _run_guard.py"
...
_TIMER_ARM_DELAYED = '        command -p sleep "$PROBE_SECS" &\n' + _ARM_SPIN + "        _timer=$!\n"
