...
build)
    ...
    exec </dev/null
    BRANCH="" BRIEF="" SCHEMA="" OUT="" RESULT_OUT="" DEBUG_OUT="" STREAM_OUT="" TIMEOUT="0" SELFVERIFY_CMD="" SELFVERIFY_OUT="" PHASES_OUT=""
    AGENT_SPEC=""
...
blob = _stream_blob(auth)
rate_blob = _stream_blob(rate_limit)
service_tier_warning_blob = _stream_blob(service_tier_warning)
...
PY
    ...
    if GIT_NO_LAZY_FETCH=1 git -c protocol.file.allow=never -c protocol.ext.allow=never -c protocol.ssh.allow=never -c uploadpack.allowFilter=false -c core.fsmonitor=false -c core.hooksPath=/dev/null diff --quiet --no-textconv --no-ext-diff "\$BASE"..HEAD; then
        ...
        # Exit-4 salvage (rev-flockschema-r2): BASE..HEAD is quiet, so
        # committed format-patch cannot recover anything. Intent-to-add
        # untracked paths first (git add -N) so git diff HEAD captures new
        # files and unions worktree+index. Emit onto stdout (host --out)
        # BEFORE sandbox teardown. Keep exit 4 and _SALVAGE_DONE.
        _e4_tmp=\$(mktemp "\${TMPDIR:-/tmp}/ra-e4-uncommitted.XXXXXX") || true
        if [ -n "\${_e4_tmp:-}" ]; then
            GIT_NO_LAZY_FETCH=1 git -c protocol.file.allow=never -c protocol.ext.allow=never -c protocol.ssh.allow=never -c uploadpack.allowFilter=false -c core.fsmonitor=false -c core.hooksPath=/dev/null add -N . >/dev/null 2>&1 || true
            GIT_NO_LAZY_FETCH=1 git -c protocol.file.allow=never -c protocol.ext.allow=never -c protocol.ssh.allow=never -c uploadpack.allowFilter=false -c core.fsmonitor=false -c core.hooksPath=/dev/null diff --binary --no-textconv --no-ext-diff HEAD > "\$_e4_tmp" 2>/dev/null || true
            if [ ! -s "\$_e4_tmp" ]; then
                GIT_NO_LAZY_FETCH=1 git -c protocol.file.allow=never -c protocol.ext.allow=never -c protocol.ssh.allow=never -c uploadpack.allowFilter=false -c core.fsmonitor=false -c core.hooksPath=/dev/null diff --binary --no-textconv --no-ext-diff --cached > "\$_e4_tmp" 2>/dev/null || true
            fi
            if [ -s "\$_e4_tmp" ]; then
                cat "\$_e4_tmp"
            fi
