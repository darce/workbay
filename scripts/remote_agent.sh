...
# TTL/idle values are snapshotted before sourcing and then resolved from the
# file, so remote-gate.env may set DISPATCH_TTL_SEC, SANDBOX_TTL_SEC, or
# SANDBOX_IDLE_SEC without overriding an explicit process environment value.
...
# Process snapshots win. TTL/idle fall back to their post-source values so the
...
writable-roots-probe|workspace-write-probe)
    ...
    if ! timeout -k 5 "$((_probe_timeout + 10))" "${SSH[@]}" bash -s -- "$_probe_kind" "$_probe_timeout" "$_probe_model" "$_probe_cap" "$AGENT_ROOT" "$_probe_nonce" <<'REMOTE_PREFLIGHT_PROBE_EOF'
set -u
kind="${1:-}"
bound="${2:-}"
model="${3:-}"
...
cleanup() {
    ...
    flock -u "$slot_fd" 2>/dev/null || true
...
if [ "$kind" = "writable-roots-probe" ]; then
    if ! timeout -k 2 10 git -C "$probe_root" init -b probe >/dev/null 2>&1; then
...
if [ -n "$sandbox_extra" ]; then
    if timeout -k 5 "$bound" "$codex" exec --ignore-user-config -C . -m "$model" -s workspace-write -c "$sandbox_extra" "$prompt" >"$codex_out" 2>"$codex_err"; then _rc=0; else _rc=$?; fi
else
    if timeout -k 5 "$bound" "$codex" exec --ignore-user-config -C . -m "$model" -s workspace-write "$prompt" >"$codex_out" 2>"$codex_err"; then _rc=0; else _rc=$?; fi
fi
...
if [ "$kind" = "writable-roots-probe" ]; then
    _head="$(timeout -k 2 10 git -C "$probe_root" rev-parse --verify HEAD 2>/dev/null || true)"
...
exit 0
REMOTE_PREFLIGHT_PROBE_EOF
    then
        _probe_remote_script=ok
    fi
    if [ -z "$_probe_remote_script" ]; then
        printf '{"schema":"workbay.preflight.v1","probe":"%s","ok":false,"verdict":"unknown","reason":"timeout","elapsed_s":%s,"live_probe_count":0,"head":"","model":"%s","nonce":"%s","sandbox_flags":"workspace-write"}\n' "$_probe_kind" "$_probe_timeout" "$_probe_model" "$_probe_nonce"
    fi
    exit 0
    ;;
doctor)
...
_lease_live() {
    _key="$1"
    _lease="$ROOT/.lane-live-$_key"
    [ -f "$_lease" ] || return 1
    _expiry=
    _issued=
    while IFS= read -r _line || [ -n "$_line" ]; do
        case "$_line" in
            expiry=*) _expiry="${_line#expiry=}" ;;
            issued=*) _issued="${_line#issued=}" ;;
        esac
    done <"$_lease" || return 0
    case "$_expiry:$_issued" in
        *[!0-9:]*) return 0 ;;
        :*|*:) return 0 ;;
    esac
    [ "$(date +%s)" -ge "$_expiry" ] && return 1
    return 0
}
...
for _sd in "$ROOT"/*/; do
    ...
    if _lease_live "$_key" || _pid_in_sandbox "$_sd"; then
    ...
    if [ "$dry_run" -eq 0 ]; then
        if ! printf '{"key":"%s","outcome":"started","bytes_before":%s,"at":%s}\n' \
            "$_key" "$_before" "$(date +%s)" >>"$ROOT/.reap-outcomes.jsonl"; then
        ...
        if [ ! -e "$_sd" ]; then
            ...
            printf '{"key":"%s","outcome":"reaped","bytes_freed":%s,"at":%s}\n' \
                "$_key" "$_before" "$(date +%s)" >>"$ROOT/.reap-outcomes.jsonl" || \
                echo "remote_agent: reap completion journal failed for $_key" >&2
        else
            printf '{"key":"%s","outcome":"incomplete","at":%s}\n' \
                "$_key" "$(date +%s)" >>"$ROOT/.reap-outcomes.jsonl" || \
...
for _scratch_path in "$tmp_root"/*/; do
    ...
    if _scratch_pid_live "$_scratch_pid" || _lease_live "$_scratch_key" || _pid_in_sandbox "$_scratch"; then
        _scratch_skip live
        continue
    fi
...
REMOTE_EOF
    ...
    run_patch() {
        ...
        _run_patch_rc=0
        # shellcheck disable=SC2029
        "${SSH[@]}" env "WORKBAY_REMOTE_AGENT_UV_CACHE_CAP_MB=$UV_CACHE_CAP_MB" bash -s <<REMOTE_EOF || _run_patch_rc=$?
set -euo pipefail
...
_lane_lru_reap_candidate() {
    ...
    (
        ...
        if [ -f "\$_lf" ]; then
            ...
            case "\$_expiry" in ''|*[!0-9]*) exit 0 ;; esac
            case "\$_issued" in ''|*[!0-9]*) exit 0 ;; esac
            _now=\$(date +%s)
            [ "\$_now" -ge "\$_expiry" ] || exit 0
        fi
...
if not names:
    ...
    if scrubbed:
        ...
        sys.exit(1)
    print("remote_agent: venv_origin_guard not_applicable consumer layout has no workbay_* packages", file=sys.stderr)
    sys.exit(0)
for name in names:
    try:
        module = importlib.import_module(name)
        origin = getattr(module, "__file__", None)
        if origin and Path(origin).resolve().is_relative_to(expected):
            continue
...
' "\$SBX/packages" </dev/null 9>&- || {
        echo 'remote_agent: venv_origin_shadow interpreter probe_failed_or_timed_out' >&2
        return 1
    }
}
...
_PHASES_JSON_PARTS="\${_PHASES_JSON_PARTS%?},\"origin_ok\":1}"
...
' "\$_sv_log" "\$_sv_cmd" </dev/null 9>&- ); then _sv_rc=0; else _sv_rc=\$?; fi
        ...
        SV_DEADLINE="\$_sv_deadline" SV_STDERR="\$_sv_log.stderr" SV_RC="\$_sv_rc" SV_CMD="\$_sv_cmd" SV_LOG="\$_sv_log" SV_GUARD_NONCE="\$_sv_guard_nonce" python3 - > "\$OUT_DIR/.grok-selfverify.json" 2>/dev/null <<'PYEOF' || true
