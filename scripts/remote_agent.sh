...
for _sd in "$ROOT"/*/; do
    ...
    if _lane_occupant_live "$_key" || _pid_in_sandbox "$_sd"; then
    ...
    if [ "$dry_run" -eq 0 ]; then
        if ! printf '{"key":"%s","outcome":"started","reason":"sandbox_idle","age":%s,"actor":"maintenance","bytes_before":%s,"at":%s}\n' \
            "$_key" "$_age" "$_before" "$(date +%s)" >>"$ROOT/.reap-outcomes.jsonl"; then
        ...
        if [ ! -e "$_sd" ]; then
            ...
            printf '{"key":"%s","outcome":"reaped","reason":"sandbox_idle","age":%s,"actor":"maintenance","bytes_freed":%s,"at":%s}\n' \
                "$_key" "$_age" "$_before" "$(date +%s)" >>"$ROOT/.reap-outcomes.jsonl" || \
                echo "remote_agent: reap completion journal failed for $_key" >&2
        else
            printf '{"key":"%s","outcome":"incomplete","reason":"sandbox_idle","age":%s,"actor":"maintenance","at":%s}\n' \
