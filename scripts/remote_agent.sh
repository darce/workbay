...
#       [--agent-spec <file.json>]   # implementation note: mutually exclusive with --model/--effort/--max-turns
...
_salvage_committed_work() {
    ...
    if ! git format-patch "\$BASE"..HEAD --stdout > "\$_salv_tmp"; then
    ...
    if [ ! -s "\$_salv_tmp" ]; then
        ...
        echo "remote_agent: BASE=\$BASE HEAD=\$(git rev-parse HEAD 2>/dev/null) branch=\$(git rev-parse --abbrev-ref HEAD 2>/dev/null)" >&2
        rm -f "\$_salv_tmp"
        return 1
    fi
    cat "\$_salv_tmp"
...
_emit_nonzero_exit_diagnostics() {
    ...
    if [ -n "\${BASE:-}" ] && git rev-parse --verify HEAD >/dev/null 2>&1; then
        if ! git diff --quiet "\$BASE"..HEAD 2>/dev/null; then
            echo "remote_agent: sandbox HEAD diverges from BASE (HEAD=\$(git rev-parse --short HEAD) BASE=\$(git rev-parse --short "\$BASE")) — salvage should have emitted a patch" >&2
        fi
    fi
}
...
PY
    ...
    if ! git format-patch "\$BASE"..HEAD --stdout > "\$_ok_patch_tmp"; then
    ...
    if [ ! -s "\$_ok_patch_tmp" ]; then
        ...
        echo "remote_agent: BASE=\$BASE HEAD=\$(git rev-parse HEAD) branch=\$(git rev-parse --abbrev-ref HEAD)" >&2
        rm -f "\$_ok_patch_tmp"
        exit 3
    fi
    _SALVAGE_DONE=1
