...
# A version pin in any of six spellings that appear in the install snippets and
# MUST agree: the git spec ``darce/workbay(.git)?@v<semver>``, the bootstrap flag
# ``--remote-ref v<semver>``, the pyproject / ``uv add`` TOML form
# ``tag = "v<semver>"``, the shell-var assignment ``REF=v<semver>``,
# the README prose ``distribution tag is `v<semver>` ``, or a positional
# ``install-workbay-cli.sh v<semver>`` / ``$0 v<semver>`` usage example.
...
# mcp-workbay-orchestrator) invisible to the gate (PPSSOT-TAG / HARM).
# Omitting prose/positional forms left mixed v0.1.54/v0.1.55 green (BR-D4-01).
# Branch refs like ``@main`` / ``--remote-ref main`` are not version pins.
PIN_RE = re.compile(
    r"(?:darce/workbay(?:\.git)?@|--remote-ref |tag\s*=\s*[\"']|REF="
    r"|distribution tag is `|install-workbay-cli\.sh |\$0 )(v\d+\.\d+\.\d+)"
)
...
def scan(repo_root: Path = REPO_ROOT) -> dict[str, list[str]]:
    ...
    proc = subprocess.run(
        [
            ...
            "grep",
            "-nIE",
            r"(darce/workbay(\.git)?@|--remote-ref |tag[[:space:]]*=[[:space:]]*[\"']|REF="
            r"|distribution tag is `|install-workbay-cli\.sh |\$0 )v[0-9]+\.[0-9]+\.[0-9]+",
            "--",
            *SCAN_GLOBS,
        ],
        cwd=repo_root,
...
def docs_pin_status(tag: str, released: list[str]) -> str:
    """Classify a unanimous docs pin against released monorepo tags.

    ``ok`` only when ``tag`` is in ``released``. A unanimous phantom
    (``v0.1.99``) is ``phantom``, not ``ok`` (BR-D5-01 / REF-26).
    Empty ``released`` is ``unverified`` (shallow / tagless checkout).
    """
    if not released:
        return "unverified"
    return "ok" if tag in released else "phantom"
