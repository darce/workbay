...
@dataclass(frozen=True)
class DoctorMaintWorktreeCollision:
    """Ambiguous on-integration MAINT handoff-row facet."""

    available: bool = True
    status: str = "clean"  # clean | warning | unavailable
    severity: str = "ok"  # ok | warning
    colliding_task_refs: list[str] = field(default_factory=list)
    note: str = ""
...
@dataclass(frozen=True)
class DoctorGrokRemote:
    """implementation note S4 — ``grok_remote`` readiness facet (offline-safe).

    Unconfigured is a typed skip (``available=False``, severity ok), never an
    error — the feature is opt-in. Configured reports host presence only
    (never the concrete host string — doctor output is shareable) and points
    operators at ``scripts/remote_agent.sh doctor`` for the live SSH probe.

    ``available`` mirrors :class:`DoctorHostMemory`: ``False`` when this offline
    facet did not (or could not) run a live readiness probe. A merely-configured
    host therefore stays ``configured=True`` / ``available=False`` until the
    operator runs the live check named in ``live_check``.
    """
    ...
    status: str = "unconfigured"  # unconfigured | configured
    live_check: str = ""
    note: str = ""
...
@dataclass(frozen=True)
class DoctorCodemap:
    """implementation note S3 — codemap index-freshness facet (offline-safe).

    Severity contract (matches ``_probe_codemap_freshness``; silence is not
    success — [OBS-08]):

    * ``unconfigured`` — CLI absent: typed skip, severity **ok** only.
    * ``unavailable`` — CLI present but status unreadable / error-shaped /
      timed out / non-zero exit: severity **warning** (dead-but-installed
      probe must not look healthy).
    * ``unconfirmed`` — CLI answered without affirmative ready+sha evidence:
      severity **warning**.
    * ``stale`` — explicit stale flag, sha mismatch, OR content drift found by
      ``detect_changes`` even when ``head_sha == indexed_sha`` (the H2 path
      landed at 0ac6b337): severity **warning** with a concrete reindex remedy
      in ``note``; never ``error`` (staleness degrades search quality, does not
      break the repo — [OBS-04]).
    * ``fresh`` — ready status AND a HEAD-matching indexed sha AND a clean
      ``detect_changes`` content check (probe ``confirmed``): severity **ok**.
      A sha match alone is never ``fresh``; the probe consults ``detect_changes``
      on the would-be-fresh path, so ``fresh`` guarantees content was verified,
      not sha-level only.

    Computed by a stdlib-only mini-probe (this package must NOT import
    ``workbay_orchestrator_mcp``); it mirrors orchestrator
    ``check_codemap_index_freshness`` / ``_indexed_sha_from_status`` semantics.
    ...
    stale: bool = False
    severity: str = "ok"  # ok | warning
    # unconfigured | unavailable | unconfirmed | fresh | stale
...
@dataclass(frozen=True)
class DoctorHookFailureSink:
    """implementation note S3 — hook provenance-write failure facet.

    Reads the Slice-2 append-only sink at the git-common-dir-anchored path
    (``<git-common-dir>/../.task-state/hook-failures.log``) from outside the
    broken hook path so ``make doctor`` surfaces silent provenance failures
    ([OBS-08]). Semantics (settled):

    - ANY entry → ``status=warning`` / ``severity=warning``, listing the
      newest few entries with timestamps. No wall-clock gate, no activity
      gate, no last-clean stamp.
    - Missing log / missing parent (``FileNotFoundError``) → CLEAN.
    - Any other OSError / decode / parse error → ``unavailable`` WITH a
      note and ``severity=warning`` (a dead monitor must not look healthy).

    Stdlib-only; must not import the hook package or ``workbay_handoff_mcp``
    (the doctor must not inherit the silence it monitors).
    """
...
class DoctorReceipt:
    ...
    maint_worktree_collision: DoctorMaintWorktreeCollision = field(
        default_factory=lambda: DoctorMaintWorktreeCollision()
    )
    ...
    grok_remote: DoctorGrokRemote = field(
        default_factory=lambda: DoctorGrokRemote()
    )
    codemap_freshness: DoctorCodemap = field(
        default_factory=lambda: DoctorCodemap()
    )
    hook_failure_sink: DoctorHookFailureSink = field(
        default_factory=lambda: DoctorHookFailureSink()
    )
