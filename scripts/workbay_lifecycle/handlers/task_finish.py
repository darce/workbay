...
5a. Record the branch identity (``branch_tip_sha`` / ``branch_tip_observed_at``
   / ``branch_tip_source``) on the task's ``worktree_lanes`` rows. This runs
   BEFORE steps 6 and 7 and runs unconditionally, because the failure it
   closes is ordered: lane branch tip -> branch ref -> row-side decision.
   Deleting the ref without first copying its identity onto the row leaves the
   row permanently undecidable — a branch *name* stops resolving the instant
   the ref is collected, and no later heuristic can reconstruct which commit
   the lane produced. An unresolvable tip records nothing rather than
   overwriting an earlier observation with NULL.
...
Codemap reindex requests are queued by default and a detached runner is
started. Set ``WORKBAY_CODEMAP_REINDEX`` to ``0``, ``off``, ``false``, or
``no`` to keep the queued request for a later manual run without spawning the
runner during task-finish.
"""
...
_CANONICAL_CONNECT_UNRESOLVED = object()
_canonical_connect_handoff_db: Any = _CANONICAL_CONNECT_UNRESOLVED


HANDOFF_SQLITE_BUSY_TIMEOUT_MS = 30_000


def _busy_timeout_ms() -> int:
    """Return the configured SQLite wait without permitting a sub-contract bound."""
    configured = os.environ.get("HANDOFF_SQLITE_BUSY_TIMEOUT_MS", "")
    try:
        return max(HANDOFF_SQLITE_BUSY_TIMEOUT_MS, int(configured))
    except ValueError:
        return HANDOFF_SQLITE_BUSY_TIMEOUT_MS


def _is_lock_contention(exc: BaseException) -> bool:
    """Recognize the lock failures SQLite may return without using its handler."""
    message = str(exc).lower()
    return "locked" in message or "busy" in message


def _apply_wal_with_deadline(conn: sqlite3.Connection, *, deadline: float) -> None:
    """Apply WAL despite journal-mode conversion bypassing ``busy_timeout``."""
    ...
    while True:
        try:
            ...
            last_error = sqlite3.OperationalError(
                "PRAGMA journal_mode=WAL did not take effect"
            )
...
def _connect(path: str | Path, *, read_only: bool = False) -> sqlite3.Connection:
    """Open handoff SQLite with one bounded contention contract.

    Readers use the canonical read-only path so merely observing a legacy
    DELETE-journal database never attempts a mode conversion. Writers retain
    the canonical WAL contract, but retry its exclusive journal transition up
    to the same deadline because SQLite can bypass ``busy_timeout`` for that
    statement.
    """
    global _canonical_connect_handoff_db
    if _canonical_connect_handoff_db is _CANONICAL_CONNECT_UNRESOLVED:
        try:
            from workbay_handoff_mcp.shared_schema import (  # noqa: PLC0415
                connect_handoff_db,
            )
        except ImportError:
            # The lifecycle payload also runs before/without the handoff package.
            _canonical_connect_handoff_db = None
        else:
            _canonical_connect_handoff_db = connect_handoff_db
    timeout_ms = _busy_timeout_ms()
    deadline = time.monotonic() + timeout_ms / 1000.0
    if _canonical_connect_handoff_db is not None:
        while True:
            try:
                if read_only:
                    conn = _canonical_connect_handoff_db(path, read_only=True)
                else:
                    conn = _canonical_connect_handoff_db(path)
            except sqlite3.OperationalError as exc:
                retryable = not read_only and _is_lock_contention(exc)
                final_error: BaseException = exc
            except RuntimeError as exc:
                # Canonical WAL readback reports a silent conversion no-op as
                # RuntimeError rather than sqlite3.OperationalError.
                retryable = not read_only and "journal_mode" in str(exc).lower()
                final_error = sqlite3.OperationalError(
                    "canonical handoff SQLite connection failed"
                )
    ...
    connect_kwargs: dict[str, Any] = {"timeout": timeout_ms / 1000.0}
    if read_only:
        connect_target = f"{Path(path).resolve().as_uri()}?mode=ro"
        connect_kwargs["uri"] = True
    conn = sqlite3.connect(connect_target, **connect_kwargs)
    try:
        conn.execute(f"PRAGMA busy_timeout={timeout_ms}")
        if not read_only:
            _apply_wal_with_deadline(conn, deadline=deadline)
    except Exception:
        conn.close()
        raise
    return conn
...
def _log_sqlite_error(exc: sqlite3.Error, *, site: str) -> None:
    """Make a best-effort SQLite degradation visible without leaking details."""
    print(
        f"task_finish: sqlite_error class={type(exc).__name__} site={site}",
        file=sys.stderr,
    )


def _is_legacy_lane_schema_error(exc: sqlite3.Error) -> bool:
    """True only when the rich lane UPDATE references a legacy-missing column."""
    if not isinstance(exc, sqlite3.OperationalError):
        return False
    message = str(exc).lower()
    return any(
        f"no such column: {column}" in message for column in ("notes", "updated_at")
    )
...
def _read_handoff_identity(repo: Path, task_ref: str) -> dict[str, Any]:
    ...
    try:
        ...
        _log_sqlite_error(exc, site="_read_handoff_identity")
        return {"source": "absent"}
...
def _read_archived_identity(conn: sqlite3.Connection, task_ref: str) -> dict[str, Any]:
    ...
    try:
        ...
        _log_sqlite_error(exc, site="_read_archived_identity")
        return {}
    if arow is None:
        return {}
    ...
    if not snapshot_json:
        ...
        if isinstance(snapshot, dict) and isinstance(snapshot.get("active"), dict):
            ...
            if "target_worktree_path" not in active and "target_branch" not in active:
                snapshot_corrupt = True
...
_TERMINAL_LANE_STATUSES = frozenset({"closed", "archived", "merged", "closed_stale"})
...
def _open_lanes_for_task(repo: Path, task_ref: str) -> list[str]:
    ...
    try:
        ...
        _log_sqlite_error(exc, site="_open_lanes_for_task")
        return []
...
def _close_lane_via_sql(
    ...
    *,
    ...
    try:
        ...
        _log_sqlite_error(exc, site="_close_lane_via_sql")
        if not _is_legacy_lane_schema_error(exc):
            return False
...
def _reap_open_lanes_for_task(
    ...
    *,
    ...
    close_status = "merged" if _branch_is_merged(root, target_branch) else "closed"
...
def _auto_reap_stale_rows(repo: Path, task_ref: str) -> tuple[list[str], str | None]:
...
def _branch_sha(primary: Path, branch: str) -> str | None:
    proc = _common.run_subprocess(
        [
            "git",
            "-C",
...
def _enqueue_non_deleted_branch_outcome(
    primary: Path,
    target_branch: str,
    reason: str,
    *,
    ...
    if not task_ref or not lane_id or not target_branch or reason == "deleted":
        return
    # The queue's positive-candidate token is the durable outlet for a ref
    # whose row-side identity could not be written.  The receipt retains the
    # more specific task-finish status; the queue must not reject that new
    # status as an unknown ordinary deleter outcome and silently lose the ref.
    queue_reason = "candidate" if reason == "skipped_identity_unrecorded" else reason
    sha = _branch_sha(primary, target_branch)
    if sha is None:
        return
    try:
    ...
    except Exception:  # noqa: BLE001 — finish must not fail through queue persistence
        return
...
def _delete_merged_branch(
    ...
    *,
    ...
    """Run ``git branch -d`` and enqueue any non-deleted outcome."""
    status, warning = _delete_merged_branch_status(
        primary,
        target_branch,
    ...
    if status != "deleted":
        _enqueue_non_deleted_branch_outcome(
            primary,
            target_branch,
            status,
            task_ref=task_ref,
...
def _delete_merged_branch_status(
    primary: Path,
    target_branch: str,
    *,
    operator_force: bool = False,
...
# Provenance vocabulary for ``worktree_lanes.branch_tip_source``, shared with the
# handoff schema: ``branch`` (resolved from the live ref) | ``manifest`` | ``backfill``.
BRANCH_TIP_SOURCE_BRANCH = "branch"

_HEX40_ALPHABET = frozenset("0123456789abcdef")


def _resolve_branch_tip_sha(primary: Path, target_branch: str) -> str | None:
    """Resolve ``target_branch`` to a normalized 40-hex commit id.

    ``None`` means **unresolved** — no branch name was supplied, the ref does
    not exist, or git returned something that is not a full commit id. It never
    means "resolved to nothing", so a caller must not persist it over an
    identity that was recorded earlier.
    """
    if not target_branch:
        return None
    proc = _common.run_subprocess(
    ...
    if proc.returncode != 0:
        return None
    sha = (proc.stdout or "").strip().lower()
    if len(sha) != 40 or not set(sha) <= _HEX40_ALPHABET:
        return None
    return sha


def _record_branch_tip_identity(
    primary: Path,
    *,
    task_ref: str,
    target_branch: str,
    tip_sha: str | None,
    source: str = BRANCH_TIP_SOURCE_BRANCH,
) -> dict[str, Any]:
    """Persist the lane branch tip on ``worktree_lanes`` before any teardown.

    The ordering is the whole point. The failure this closes is a chain —
    ``lane branch tip -> branch ref -> row-side decision`` — in which deleting
    the middle link without first copying its identity onto the row makes the
    last link undecidable, permanently. A branch *name* stops resolving the
    moment the ref is collected; a recorded SHA does not.

    So this runs before the worktree removal **and** before any branch delete is
    attempted, and it runs unconditionally. All eight of
    :func:`_delete_merged_branch`'s outcome tokens leave the row bound, not just
    the one that deletes — a SHA captured only on the success path records
    exactly the case an operator never needs to investigate.

    Idempotent, and specifically never writes NULL over an existing
    ``branch_tip_sha``: an unresolvable tip is absence of new evidence, not
    evidence of absence, so a second finish over a ref that has already been
    collected preserves the first run's observation instead of erasing it.

    ``updated_at`` is deliberately **not** touched. It is load-bearing
    elsewhere — the reaper orders its FIFO by it, and a row whose
    ``created_at == updated_at`` is how a never-provisioned lane is told apart
    from a collected one. ``branch_tip_observed_at`` already carries this
    write's timestamp.

    Never raises. Returns a disposition dict whose ``disposition`` is one of
    ``recorded`` | ``unresolved`` | ``no_db`` | ``no_rows`` | ``failed``.
    """
    note: dict[str, Any] = {
    ...
    if not target_branch or tip_sha is None:
        return note
    canonical = resolver.canonical_workspace_root(primary) or primary
    db_path = canonical / ".task-state" / "handoff.db"
    if not db_path.is_file():
        note["disposition"] = "no_db"
        return note
    try:
        with _connect(db_path) as conn:
            # Not filtered by status: the auto-reap has already moved these rows
            # terminal by this point, and a terminal row is exactly the one that
            # most needs its identity bound.
            cursor = conn.execute(
                "UPDATE worktree_lanes "
                "SET branch_tip_sha = ?, "
                "    branch_tip_observed_at = datetime('now'), "
                "    branch_tip_source = ? "
                "WHERE task_ref = ? AND branch = ?",
                (tip_sha, source, task_ref, target_branch),
            )
            rows = cursor.rowcount
            conn.commit()
    except sqlite3.Error as exc:
        _log_sqlite_error(exc, site="_record_branch_tip_identity")
        note["disposition"] = "failed"
        note["error"] = str(exc)[:200]
        return note
    note["sha"] = tip_sha
    note["source"] = source
    note["rows"] = max(rows, 0)
    note["disposition"] = "recorded" if rows > 0 else "no_rows"
    return note


def _delete_branch_with_identity_guard(
    primary: Path,
    target_branch: str,
    branch_tip: dict[str, Any],
    *,
    operator_force: bool = False,
    self_pid: int | None = None,
    exclude_session_id: str = "",
    ...
    """Delete only a row-bound ref; record the guarded non-delete otherwise."""
    if branch_tip.get("disposition") == "recorded":
        return _delete_merged_branch(
            primary,
            target_branch,
            operator_force=operator_force,
            self_pid=self_pid,
    ...
    # The ref is the last recoverable source of branch identity. Worktree
    # removal may proceed because it is regenerable, but deleting the ref
    # without a durable row-side identity would make the lane undecidable.
    status = "skipped_identity_unrecorded"
    warning = (
        f"branch kept: identity unrecorded ({branch_tip.get('disposition', 'unknown')})"
    )
    # The fail-closed guard must still leave a durable outlet. Route the exact
    # refusal token through the same reclaim-outcome seam as every non-deleted
    # result from _delete_merged_branch; a later reaper can drain it without
    # this finish attempt ever deleting an unbound ref.
    _enqueue_non_deleted_branch_outcome(
        primary,
        target_branch,
        status,
...
def _request_codemap_reindex(repo: Path, sha: str) -> dict[str, Any] | None:
    ...
    ``WORKBAY_CODEMAP_REINDEX=0|off|false|no`` disables only the detached
    spawn. The request remains queued so a later manual runner can drain it.
    """
    if not sha:
    ...
    try:
        ...
        return {
            "ok": False,
            "note": f"codemap_import_failed: {type(exc).__name__}: {exc}",
        }
    ...
    try:
        ...
        if os.environ.get("WORKBAY_CODEMAP_REINDEX", "").strip().lower() in {
            "0",
            "off",
            "false",
            "no",
            ...
            return {
                ...
                "pending": pending,
                "spawned": False,
                "note": "spawn_skipped_disabled_by_env",
...
# lives on ``receipt["codemap_reindex"]`` for scanners. Intentional spawn skips
# are ok=True and never reach this path.
...
def _codemap_warning_token(codemap_note: dict[str, Any]) -> str | None:
    ...
    intentional skips (``spawn_skipped_inflight`` and
    ``spawn_skipped_disabled_by_env``, which are ok=True) do not.
...
def run(argv: list[str]) -> int:
    ...
    live_done = identity.get("source") == "live" and identity.get("status") == "done"
    ...
    # other GC and never runs in a daemon-less /wb-offload flow. Runs whenever the task
            ...
            if isinstance(orphan_reap, dict):
                ...
                for entry in orphan_reap.get("closed") or []:
                    ...
                    if not any(r.get("lane_id") == lane_id for r in reaped_lanes):
                        reaped_lanes.append(
                            {"lane_id": lane_id, "status": "closed_stale"}
                        )
                if reaped_now:
                    ...
                    if still_open != open_lanes:
                        ...
                        warnings[:] = [
                            w
                            for w in warnings
                            if not w.startswith("lane_close_skipped:")
                        ]
                        if open_lanes:
                            warnings.append(
                                "lane_close_skipped: open lanes detected "
    ...
    active_guard: dict[str, Any] = {
        "worktree": None,
        "branch": None,
        "forced": operator_force,
    }
    ...
    # C1 — copy the branch identity onto the lane row while the ref still
    # resolves. This MUST precede both the worktree removal and the branch
    # delete below: after either one runs, the tip may be unrecoverable and the
    # row's disposition becomes undecidable rather than merely unknown. It is
    # written on every path, so all eight branch-delete outcomes below (seven of
    # which leave the branch alive) still leave a bound row behind.
    branch_tip = _record_branch_tip_identity(
        primary,
        task_ref=task_ref,
        target_branch=target_branch,
        tip_sha=_resolve_branch_tip_sha(primary, target_branch),
    )
    ...
    branch_status, branch_err = _delete_branch_with_identity_guard(
        primary,
        target_branch,
        branch_tip,
        operator_force=operator_force,
    ...
    elif branch_status == "skipped_identity_unrecorded":
        warnings.append(branch_err or "branch kept: identity unrecorded")
    ...
    if not args.emit_json:
        ...
        sys.stderr.write(
            ...
            f"{sync_summary}"
            + (
                " branch kept: identity unrecorded"
                if branch_status == "skipped_identity_unrecorded"
                else ""
            )
