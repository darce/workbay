...
def _load_continuation_packet(repo: Path, task_ref: str | None) -> dict[str, Any] | None:
...
def run(argv: list[str]) -> int:
    ...
    # Never raise / never fail make context ([REF-20]/[AGT-10]). All three
    # share one warnings sink so an empty receipt list means clean [OBS-08].
    warnings: list[str] = []
    ...
    continuation_packet = _load_continuation_packet(repo, continuation_task_ref)
