...
def _reap_backstop(
    ...
    _record_envelope_warning(warnings, label="reap_backstop", warning=warning)
...
def _retention_backstop(
    ...
    _record_envelope_warning(warnings, label="retention_backstop", warning=warning)
    if payload is None or not isinstance(payload, dict):
        return None
    flat = _flatten_handoff_payload(payload)
    if not flat.get("ok", True):
...
def _load_continuation_packet(
    ...
    *,
    ...
    _record_envelope_warning(warnings, label="continuation", warning=warning)
    if payload is None or not isinstance(payload, dict):
        return None
    flat = _flatten_handoff_payload(payload)
    if not flat.get("ok", True):
