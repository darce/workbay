...
_ON_DEMAND_RENDER_UNTRACKED: frozenset[str] = frozenset(
    {"CURRENT_TASK.json", "DASHBOARD.txt"}
)
    ...
    *,
    ...
    if stored_plan:
        ...
        if paths and _changes_outside_paths([relative.as_posix()], paths):
            return {"ok": True, "applied": False, "skipped": "plan_outside_paths"}
        return _apply_validated_checklist_sync(
            repo, task_ref, plan, commit_resources=commit_resources
        )
    ...
    if paths and _changes_outside_paths([relative.as_posix()], paths):
        return {**preview, "applied": False, "skipped": "plan_outside_paths"}
    return _apply_validated_checklist_sync(
        repo, task_ref, repo.resolve() / relative, commit_resources=commit_resources
    )
...
def _apply_validated_checklist_sync(
    ...
    receipt = _apply_bound_checklist_sync(
        repo, task_ref, plan, commit_resources=commit_resources
    )
    ...
    try:
        ...
        if rewritten != text:
            ...
            try:
                ...
                if (current.st_dev, current.st_ino) != (written.st_dev, written.st_ino):
                    raise ValueError(
                        f"destination moved; original retained at {backup}"
                    )
...
def _git_lock_held(stderr: str) -> bool:
    return (".lock" in stderr and "File exists" in stderr) or (
        "Another git process" in stderr
    )
    ...
    for rel_path in staged_paths:
        ...
        if blob.returncode != 0:
            detail = (
                ...
                or "git staged blob read failed"
            )
            return None, f"{rel_path}: {detail}"
        lines = [
            line_number
            for line_number, line in enumerate(blob.stdout.splitlines(), start=1)
...
def _format_conflict_marker_detail(markers: dict[str, list[int]]) -> str:
    locations = ", ".join(
        f"{path}:{line}" for path, lines in markers.items() for line in lines
    )
    return (
...
def _derive_and_backfill_changed_files(
    repo: Path, task_ref: str, commit_sha: str
) -> dict[str, Any]:
...
def _emit_error(
    ...
    *,
    ...
    receipt: dict[str, Any] = {
        ...
        or {"staged": 0, "unstaged": 0, "untracked": 0, "total": 0},
...
def _run(argv: list[str], commit_resources: ExitStack) -> int:
    ...
    if not msg:
        return _emit_error(
            "msg_required", msg=msg, included_untracked=args.include_untracked
        )

    repo = _common.repo_root()
    if repo is None:
        return _emit_error(
            "not_in_git_repo", msg=msg, included_untracked=args.include_untracked
        )
    ...
    checklist_sync = _scoped_checklist_sync(
        repo, task_ref, paths, commit_resources=commit_resources
    )
    ...
    if sync_error in {
        ...
        return _emit_error(
            ...
            status=sync_error
            if sync_error in {"plan_locked", "lock_held"} and not args.emit_json
            else None,
        )
    ...
    if paths:
        ...
        sys.stderr.write(
            "WARNING slice-commit: unscoped staging sweeps: "
    ...
    changed_files_backfill = _derive_and_backfill_changed_files(
        repo, task_ref, commit_sha
    )
    projection_status, projection_warning = _project_commit_sha(repo, task_ref)
    stage_event = (
        "staged_paths"
        if paths
        else ("staged_all" if args.include_untracked else "staged_tracked")
    )
