...
def _derive_and_backfill_changed_files(
    ...
    try:
        from workbay_handoff_mcp.changed_files_derivation import derive_changed_files_from_commit
    ...
    with _get_db_connection() as conn:
        ...
        if ok:
            ...
            conn.rollback()
    if not ok:
        return {"ok": False, "warning": err or "backfill_failed", "derived_count": len(paths)}
