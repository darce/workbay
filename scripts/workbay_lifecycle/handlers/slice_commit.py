...
def _derive_and_backfill_changed_files(
    repo: Path, task_ref: str, commit_sha: str, *, workspace: Path
) -> dict[str, Any]:
    ...
    result = derive_changed_files_from_commit(repo, commit_sha)
...
def _checkout_refusal(
    ...
    *,
    ...
    return {
        ...
        "registered_owners": sorted(owners or ()),
        "candidate_checkout_paths": sorted(candidates or ())[:20],
...
def _resolved_git_common_dir(path: Path) -> Path | None:
    """Return the resolved git common dir that identifies ``path``'s repository."""
    proc = subprocess.run(
...
def _same_repository_registered_worktree(invoking: Path, stored: Path) -> bool:
    """True when ``stored`` is a registered worktree of ``invoking``'s repository."""
    invoking_common = _resolved_git_common_dir(invoking)
    stored_common = _resolved_git_common_dir(stored)
    if invoking_common is None or stored_common is None:
        return False
    if invoking_common != stored_common:
        return False
    try:
        stored_key = str(stored.resolve())
    except (OSError, RuntimeError, ValueError):
        return False
    registered: set[str] = set()
    for row in resolver.linked_worktrees(invoking):
        raw_path = row.get("path") if isinstance(row, dict) else None
        if not isinstance(raw_path, str) or not raw_path:
            continue
        try:
            registered.add(str(Path(raw_path).resolve()))
        except (OSError, RuntimeError, ValueError):
            continue
    return stored_key in registered


def _identity_active_mapping(envelope: object, task_ref: str) -> dict[str, Any] | None:
    """Return the selected task's identity mapping, or raise on malformed shape."""
    if not isinstance(envelope, dict):
        raise ValueError("identity envelope is not a mapping")
    if envelope.get("ok") is not True:
        raise ValueError("identity lookup not ok")
    data = envelope.get("data")
    if not isinstance(data, dict):
        raise ValueError("identity data is not a mapping")
    active = data.get("active")
    if not isinstance(active, dict):
        raise ValueError("identity active is not a mapping")
    if active.get("task_ref") != task_ref:
        return None
    return active


def _task_checkout_guard(repo: Path, task_ref: str, deadline: float | None = None) -> dict[str, Any] | None:
    ...
    def refusal(reason: str) -> dict[str, Any]:
        return _checkout_refusal(repo, task_ref, reason, owners=owners, candidates=candidates)
...
def _explicit_task_ref(args_task: str) -> str:
    """Return the operator-selected task, if any.

    ``--task`` wins over ``TASK=``. Either source is explicit intent: a
    cwd-derived fallback must not replace it (implementation note G2).
    """
    return (args_task or os.environ.get("TASK") or "").strip().upper()


def _stored_task_worktree(repo: Path, task_ref: str, deadline: float) -> Path | None:
    """Return the selected task's stored checkout when it is this repository.

    Identity is the selector for an explicit TASK invoked from another
    task's cwd. The stored path must share this repository's resolved git
    common dir and appear in ``git worktree list --porcelain``. Timeout,
    transport failure, or a malformed envelope raise so the bind path can
    emit a typed lookup refusal (OBS-08). A valid identity that names a
    different repository returns None so the original mismatch stays
    fail-closed (DBG-10).
    """
    remaining = deadline - time.monotonic()
    if remaining <= 0:
        raise TimeoutError("identity lookup budget exhausted")
    argv = _common.handoff_command_argv(repo, "state", "--sections", "identity", task_ref)
    proc = _common.run_handoff_subprocess(repo, argv, timeout=max(0.001, remaining))
    if proc.returncode != 0:
        raise ValueError("identity lookup failed")
    try:
        envelope = json.loads(proc.stdout)
    except json.JSONDecodeError as exc:
        raise ValueError("identity lookup failed") from exc
    active = _identity_active_mapping(envelope, task_ref)
    if active is None:
        return None
    raw = active.get("target_worktree_path")
    if not isinstance(raw, str) or not raw.strip():
        return None
    try:
        resolved = Path(raw).resolve()
    except (OSError, RuntimeError, ValueError) as exc:
        raise ValueError("unresolvable stored worktree") from exc
    if not resolved.is_dir():
        return None
    if not _same_repository_registered_worktree(repo, resolved):
        return None
    return resolved


def _bind_repo_to_explicit_task(repo: Path, task_ref: str) -> tuple[Path, dict[str, Any] | None]:
    """Authorize cwd, or retarget to the selected task's stored checkout.

    When cwd is already a registered checkout of ``task_ref``, keep it so a
    task that owns both a primary and a linked worktree can commit from
    either. On ``task_checkout_mismatch`` only, bind to the stored
    worktree and re-authorize there — that is the G2 path (explicit TASK
    from another task's cwd). Ambiguous or unavailable ownership still
    fail closed. Lookup and retry share one monotonic deadline so a hung
    identity CLI cannot stall slice-commit (OBS-08).
    """
    deadline = time.monotonic() + _common.handoff_read_timeout()
    refusal = _task_checkout_guard(repo, task_ref, deadline=deadline)
    if refusal is None or refusal.get("error") != "task_checkout_mismatch":
        return repo, refusal
    try:
        stored = _stored_task_worktree(repo, task_ref, deadline)
    except (
        TimeoutError,
        ValueError,
        TypeError,
        ...
        return repo, _checkout_refusal(repo, task_ref, "task_checkout_lookup_failed")
    if stored is None or stored == repo.resolve():
        return repo, refusal
    retry = _task_checkout_guard(stored, task_ref, deadline=deadline)
...
def _run(argv: list[str], commit_resources: ExitStack) -> int:
    ...
    explicit_task = _explicit_task_ref(args.task)
    if explicit_task:
        # Already-staged markers on the invoking checkout must refuse before
        # identity bind so an unavailable registry cannot mask conflict
        # content (WBLAND / g2afix). Scan errors fail closed even when the
        # escape hatch is set; the hatch authorizes marker content only and
        # still reaches checkout identity below.
        markers, scan_error = _staged_conflict_markers(repo)
        if scan_error is not None or (markers and os.environ.get(CONFLICT_MARKER_ESCAPE_HATCH_ENV, "").strip() != "1"):
        ...
        # Bind the write target before path validation, dirty measurement, or
        # staging so an explicit TASK from another cwd cannot project onto the
        # cwd-resolved task (implementation note G2).
        repo, refusal = _bind_repo_to_explicit_task(repo, explicit_task)
    ...
    git_facts = _common.gather_git_facts(repo)
    untracked_paths = _untracked_paths(repo)
    task_ref = explicit_task or git_facts.derived_task_ref
    if not task_ref:
        return _emit_error(
            "task_ref_required",
            branch=git_facts.branch,
    ...
    if explicit_task:
        ...
        # mutation. Checkout identity was bound above; this scan covers the
        # bound repo's already-staged blobs before checklist writes/staging,
        # and a second scan after staging covers newly added worktree content.
        markers, scan_error = _staged_conflict_markers(repo)
        if scan_error is not None or (markers and os.environ.get(CONFLICT_MARKER_ESCAPE_HATCH_ENV, "").strip() != "1"):
            return _emit_error(
                "conflict_marker_scan_failed" if scan_error is not None else "conflict_markers_staged",
    ...
    changed_files_backfill = _derive_and_backfill_changed_files(
        repo,
        task_ref,
        commit_sha,
