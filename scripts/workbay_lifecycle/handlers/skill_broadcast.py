...
agent to invoke the live ``/<command_id>`` from
``portable_commands.json`` (the non-tombstone row whose ``skill`` equals
the slug), and emit a ``workflow_intent`` event via
``mcp-workbay-handoff event record`` so the intent is replayable from
handoff. When MCP is offline, the intent is spooled to
``.task-state/pending-workflow-events.jsonl`` at repo root and the
receipt reports ``intent_event_id: null`` with
``handoff_projection: "spooled"`` (CLI ran and rejected) or
``"pending"`` (CLI unreachable). Exit 0 on success or spool; exit
non-zero if cwd is not a git repo or the slash-command lookup fail-closes.
"""
...
SKILL_BY_COMMAND: dict[str, str] = {
    ...
    "plan-analyze": "plan-draft",
}

_MANIFEST_RELATIVE = Path("config") / "agent-workflows" / "portable_commands.json"


def _portable_commands_manifest_path() -> Path:
    """Locate ``portable_commands.json`` from either lifecycle twin.

    The payload copy lives at ``<payload>/config/agent-workflows/...``.
    The root twin lives under ``scripts/workbay_lifecycle/`` and must find
    the same file at
    ``<repo>/packages/workbay-system/workbay_system/payload/config/agent-workflows/...``.
    Walk ancestors instead of hardcoding ``parents[N]`` so a byte-identical
    copy works in both layouts. Missing file is reported later as
    ``slash_command_manifest_unreadable``.
    """
    start = Path(__file__).resolve()
    for parent in start.parents:
        direct = parent / _MANIFEST_RELATIVE
        if direct.is_file():
            return direct
        nested = (
            parent
        ...
        if nested.is_file():
            return nested
    return start.parent / _MANIFEST_RELATIVE


class SlashCommandLookupError(LookupError):
    """Named fail-closed miss when a skill slug has no live slash command."""
...
def _slash_command_for_skill(skill: str) -> str:
    """Return the live command_id whose ``skill`` equals ``skill``.

    Fail-closed with a distinct named error when the only matches are
    tombstones or when nothing matches. Does not hardcode command ids.
    """
    manifest_path = _portable_commands_manifest_path()
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise SlashCommandLookupError(
            "slash_command_manifest_unreadable",
            f"slash_command_manifest_unreadable: cannot read {manifest_path}: {exc}",
        ) from exc
    if not isinstance(manifest, dict):
        raise SlashCommandLookupError(
            "slash_command_manifest_invalid",
            "slash_command_manifest_invalid: portable_commands.json must be an object",
        )
    commands = manifest.get("commands")
    if not isinstance(commands, list):
        raise SlashCommandLookupError(
            "slash_command_manifest_invalid",
            "slash_command_manifest_invalid: commands must be a list",
        )

    live_ids: list[str] = []
    tombstone_ids: list[str] = []
    for command in commands:
        if not isinstance(command, dict) or command.get("skill") != skill:
            continue
        command_id = command.get("command_id")
        if not isinstance(command_id, str) or not command_id.strip():
            continue
        if command.get("tombstone") is True:
            tombstone_ids.append(command_id)
        else:
            live_ids.append(command_id)

    if len(live_ids) == 1:
        return live_ids[0]
    if live_ids:
        raise SlashCommandLookupError(
            "slash_command_ambiguous",
            f"slash_command_ambiguous: skill {skill!r} matches live "
            f"command_ids {live_ids}",
        )
    if tombstone_ids:
        raise SlashCommandLookupError(
            "slash_command_tombstone",
            f"slash_command_tombstone: skill {skill!r} resolves only to "
            f"tombstone command_id(s) {tombstone_ids}",
        )
    raise SlashCommandLookupError(
        "slash_command_not_found",
        f"slash_command_not_found: skill {skill!r} has no live command_id "
        "in portable_commands.json",
    )
...
def _record_intent_event(
    decision_id: str, skill: str, doc: str, slash_command: str
    ...
    """
    rationale = f"invoke /{slash_command} for {doc}"
...
def run(command: str, argv: list[str]) -> int:
    ...
    try:
        slash_command = _slash_command_for_skill(skill)
    except SlashCommandLookupError as exc:
    ...
    intent_event_id, projection, returncode = _record_intent_event(
        decision_id, skill, args.doc, slash_command
    )
    ...
    if intent_event_id is None:
        ...
        if not spooled:
            ...
            projection = "dropped"
            sys.stderr.write(
                f"workflow_intent for /{slash_command} {args.doc} was SHED (dropped): the "
    ...
    if not args.emit_json:
        sys.stderr.write(
            ...
            f"is a skill-broadcast wrapper. Invoke `/{slash_command} {args.doc}` "
