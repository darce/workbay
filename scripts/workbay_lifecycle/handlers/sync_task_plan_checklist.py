...
@contextmanager
def checklist_write_lock(destination: Path | int):
    """Lock the stable parent inode across read, evidence lookup, and write.

    Contention refuses immediately so callers can retry from fresh bytes. A
    bound directory descriptor lets scoped callers retain their scope checks.
    """
    fd = (
        os.dup(destination)
        if isinstance(destination, int)
        else os.open(destination.resolve().parent, os.O_RDONLY | os.O_DIRECTORY)
    )
    try:
...
_SLICE_HEADING_RE = re.compile(r"^Checklist\s+for\s+Slice\s+(\d+)\b", re.IGNORECASE)
...
def _lookup_stored_plan_path(repo: Path, task_ref: str) -> str | None:
    ...
    try:
        ...
        return None
    active = (
        (payload.get("data") or {}).get("active") if isinstance(payload, dict) else None
    )
...
def run(argv: list[str]) -> int:
    ...
    try:
        with checklist_write_lock(plan_path) if args.apply_changes else nullcontext():
    ...
    _write_sync_sidecar(_resolve_state_dir(handoff_root), args.task_ref, sidecar_entry)
