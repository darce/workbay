...
def _projection_queue_summary(repo: Path) -> dict[str, object]:
    ...
    dead_letter_orphan_count, dead_letter_orphan_capped = (
        projection_queue.dead_letter_orphan_count(repo)
    )
    return {
        ...
        # Events stranded in orphan drain snapshots (a crash after a claim rename,
        # or a partial drain from an older build). The auto-drain reclaims these;
        # surfacing them keeps them visible until it does.
        "dead_letter_orphan_count": dead_letter_orphan_count,
        "dead_letter_orphan_count_capped": dead_letter_orphan_capped,
        "dead_letter_path": str(projection_queue.DEAD_LETTER_REL),
        "dead_letter_last_drain_at": projection_queue.dead_letter_last_drain_iso(repo),
    }
...
def _read_handoff_projection(
    ...
    *,
    ...
    if active is None:
        ...
        # tasks`` already sees. Still thread envelope warnings (ok=false)
        # so silence is not success [OBS-08].
        return None, False, warning
    ...
    return (
        ...
        True,
        # Thread ok=false / structural warnings into the receipt [OBS-08].
        warning,
    )
...
def _read_open_findings_count(
    ...
    *,
    ...
    return (
        ...
        # Keep parsed counts for reporting but surface envelope failure [OBS-08].
        _warning_for_field("review.open_findings_count", warning),
    )
...
def _read_blockers_count(
    ...
    *,
    ...
    return len(blockers), _warning_for_field("review.blockers_count", warning)
...
def _read_last_test_summary(
    ...
    *,
    ...
    field_warning = _warning_for_field("review.last_test_summary", warning)
    if not tests:
        return None, field_warning
...
def _worktree_tasks_dir(worktree: Path) -> Path:
    ...
    ``PACKAGE_ROOT`` is the *code's* location (``parents[3]`` of this file).
...
def run(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        type=float,
        default=_common.handoff_read_timeout(),
    )
