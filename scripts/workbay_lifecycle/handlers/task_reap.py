...
def run(argv: list[str]) -> int:
    ...
    if not flat.get("ok", True):
        # Surface envelope warning so ok=false is never silent [OBS-08].
        ...
        if warning is not None:
            failed_receipt["warning"] = warning.__dict__
    ...
    if warning is not None:
        # Structural degradation (e.g. not_ok) must reach the operator [OBS-08].
        receipt["warning"] = warning.__dict__
    if args.emit_json:
        _common.emit(receipt)
        return 0
