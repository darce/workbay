...
def _discover_active_plan_path(workspace_root: Path, task_ref: str) -> str | None:
    """Best-effort active-state lookup. ``None`` on any failure."""
    return sync_handler._lookup_stored_plan_path(workspace_root, task_ref)


def _discover_archived_plan_path(workspace_root: Path, task_ref: str) -> str | None:
...
def _discover_plan_paths_by_glob(workspace_root: Path, task_ref: str) -> list[str]:
