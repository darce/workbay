...
# Defensive: _common is imported by nearly every handler. A partial
# materialization that lands this module without interpreter_skew must not
# take down every lifecycle verb at import time [INT-01, OBS-08].
try:
    from interpreter_skew import _REEXEC_DEGRADED_ENV
except ImportError:  # cold-start / partial materialization; match entry point
...
# internal: see task_reap_timeout() for the measurement behind 600.
DEFAULT_TASK_REAP_TIMEOUT = 600.0
# Measured 2026-09-05: 214-row registry, 134.56s wall -> ~0.63 s/row.
TASK_REAP_MEASURED_ROWS = 214
TASK_REAP_MEASURED_WALL_SECONDS = 134.56
TASK_REAP_MEASURED_SECONDS_PER_ROW = 0.63
DEFAULT_LIFECYCLE_AGENT = "workbay-lifecycle"
DEFAULT_AGENT_ENV = "WORKBAY_HANDOFF_DEFAULT_AGENT"

# internal: agent-error spool reclaimer. Must match
# workbay_handoff_mcp.agent_errors.REPLAY_LIFECYCLE_DEFAULT_MAX_BATCH so one
# ``make context`` drains a ~137-line backlog. Shell-out only — do not import
# workbay_handoff_mcp symbols from lifecycle handlers.
AGENT_ERRORS_REPLAY_MAX_BATCH = 500
AGENT_ERRORS_REPLAY_TIMEOUT = 15.0
# internal: same reclaimer drains terminal_guard.jsonl (Planning-Review
# Resolution #2 — one reclaimer, both spools). Timeout matches agent-errors.
TERMINAL_GUARD_REPLAY_TIMEOUT = AGENT_ERRORS_REPLAY_TIMEOUT
# Subsystem spools drained by the reclaimers above — distinct from the
# projection-events queue at .task-state/pending-workflow-events.jsonl.
AGENT_ERRORS_SPOOL_REL = Path(".task-state") / "agent-errors-spool.jsonl"
TERMINAL_GUARD_SPOOL_REL = Path(".task-state") / "terminal_guard.jsonl"
# Bound the timeout-degrade depth scan the way projection_queue does; when the
# cap is hit the rendered label must say so (``N+``), not pretend exact.
SUBSYSTEM_SPOOL_DEPTH_LIMIT = 10_000
...
def live_active_statuses() -> tuple[str, ...]:
    ...
    """
    try:
        shared_primitives = importlib.import_module(
            "workbay_handoff_mcp.shared_primitives"
        )
    except Exception:
...
def snapshot_is_live_for_task(task_ref: str, workspace_root: Path) -> bool:
    ...
    if (
        not task_ref
        or task_ref.startswith(".")
        or any(sep in task_ref for sep in ("/", "\\", "\x00"))
    ):
        return False
...
_PROVISION_ENV_HINT = (
    "run `make provision-env` to provision the workspace .venv with mcp-workbay-handoff"
)


def resolve_handoff_cli_bin(start: Path | None = None) -> str:
    """Resolve the handoff CLI the spawn path will actually use.

    Prefer a start-scoped hermetic console script; when that does not yield an
    executable file (deleted worktree, non-git path), fall back to cwd-scoped
    resolution before returning the bare PATH name. Callers that only need the
    raw search can still call :func:`mcp_handoff_bin` directly.

    Host contract: never raise on a dead path. When the process cwd has already
    been removed (task-finish teardown of the worktree the operator is inside),
    the cwd fallback must not re-raise ``FileNotFoundError`` from ``Path.cwd()``
    — return the start-scoped result instead.
    """
    bin_path = mcp_handoff_bin(start)
    candidate = Path(bin_path)
    if candidate.is_file() and os.access(candidate, os.X_OK):
        return bin_path
    if start is not None:
        try:
            cwd_bin = mcp_handoff_bin()
        except OSError:
            # Deleted / unreachable process cwd — keep start-scoped result.
            return bin_path
        cwd_candidate = Path(cwd_bin)
        if cwd_candidate.is_file() and os.access(cwd_candidate, os.X_OK):
            return cwd_bin
    return bin_path


def handoff_cli_required_preflight_error(start: Path | None = None) -> str | None:
    """Return an actionable error when a required gate cannot spawn the handoff CLI.

    ``start`` must be the same repo the subsequent spawn will resolve against so
    the guard cannot green-light a different binary than the one that is gated.
    """
    bin_path = resolve_handoff_cli_bin(start)
    candidate = Path(bin_path)
    if candidate.is_file() and os.access(candidate, os.X_OK):
        return None
...
def handoff_command_argv(repo: Path, *argv: str) -> list[str]:
    ...
    # Prefer repo-scoped hermetic resolution; fall back to cwd-scoped when the
    # repo is a deleted / non-live worktree so healthy-cwd still hits .venv.
    return [resolve_handoff_cli_bin(repo), "--workspace-root", str(workspace), *argv]
...
def _harness_transcript_env_var_names(repo: Path | None) -> list[str]:
    ...
    try:
        ...
        return [rule.env_var for rule in contract.transcript_discovery.values()]
    except Exception:
...
def _subsystem_spool_depth_label(
    repo: Path,
    spool_rel: Path,
    *,
    limit: int | None = None,
) -> str:
    """Bounded non-empty-line count for a subsystem spool; never raises.

    Returns a plain number when the full count is known, ``N+`` when the scan
    hit the cap (same machine-keyed grammar as task-start ``spool_depth=N+``),
    or ``unknown`` when the file cannot be read.
    A missing spool is depth 0 (nothing undrained), not unknown.

    Depth is measured at the same root :func:`handoff_command_argv` passes as
    ``--workspace-root`` (canonical primary, else ``repo``), so a linked
    worktree reports the primary spool the drain will act on [OBS-08].
    """
    depth_limit = SUBSYSTEM_SPOOL_DEPTH_LIMIT if limit is None else limit
    try:
        workspace = resolver.canonical_workspace_root(repo) or repo
        spool = workspace / spool_rel
        ...
        return str(count)
    except Exception:  # noqa: BLE001 — never-raise host contract
        return "unknown"


def _timeout_replay_degrade_cause(
    repo: Path,
    *,
    base_cause: str,
    subsystem: str,
    ...
    """Enrich a timeout degrade cause with subsystem, undrained depth, drain cmd.

    Timeout (returncode 124) is the operator-hostile path: the bare
    ``exit=124 timed out`` line names neither the unavailable subsystem nor
    how much is left undrained nor what to re-run. Depth is measured from the
    subsystem's own spool file — not the projection-events queue [OBS-08][AGT-10].
    """
    depth = _subsystem_spool_depth_label(repo, spool_rel)
    return (
        f"{base_cause}; subsystem={subsystem}; "
        f"spool_depth={depth}; drain={drain_command}"
    )


def maybe_replay_agent_error_spool(
    repo: Path,
    *,
    warnings: list[str] | None = None,
) -> dict[str, Any] | None:
    """Best-effort agent-error spool drain ([RES-07]/[REF-20]/[AGT-10]).

    Shells out via :func:`handoff_command_argv` to
    ``errors-replay-spool --max-batch <AGENT_ERRORS_REPLAY_MAX_BATCH>``
    (resolved hermetic binary + ``--workspace-root``). On ANY failure
    (nonzero exit, timeout, exception, missing CLI) degrades to a stderr
    log and returns None — never raises, never blocks or fails the host
    lifecycle command. Prefer capturing the JSON count for the
    receipt/log when the subprocess succeeds.

    When ``warnings`` is provided, each degradation path appends the same
    cause string written to stderr so receipt consumers can see why the
    sub-step failed without reading stderr [OBS-08].

    A returned payload does **not** imply a successful drain: when the
    subprocess exits 0 but ``ok`` is not True, this helper still returns the
    payload for reporting while recording the degradation on the warnings
    sink (and stderr). Callers must consult the warnings sink or
    ``payload["ok"]`` rather than treating a non-None return as health
    [OBS-08][AGT-10].
    """

    def _degrade(cause: str) -> None:
        line = f"agent_errors_replay: degraded: {cause}"
    ...
    try:
        argv = handoff_command_argv(
            ...
            "errors-replay-spool",
            "--max-batch",
            str(AGENT_ERRORS_REPLAY_MAX_BATCH),
        )
        proc = run_handoff_subprocess(repo, argv, timeout=AGENT_ERRORS_REPLAY_TIMEOUT)
    except Exception as exc:  # noqa: BLE001 — never-raise host contract
        _degrade(f"{type(exc).__name__}: {exc}")
        return None
    if proc.returncode != 0:
        err_snip = (proc.stderr or proc.stdout or "").strip().replace("\n", " ")[:200]
        cause = f"exit={proc.returncode}" + (f" {err_snip}" if err_snip else "")
        if proc.returncode == 124:
            # Printed remedy is the argv that timed out (structural; no second
            # derivation that can disagree with the executed list).
            cause = _timeout_replay_degrade_cause(
                repo,
                base_cause=cause,
                subsystem="agent-error spool",
                drain_command=shlex.join(argv),
                spool_rel=AGENT_ERRORS_SPOOL_REL,
            )
        _degrade(cause)
        return None
    try:
        ...
        _degrade("non-json stdout")
        return None
    if not isinstance(payload, dict):
        _degrade("unexpected payload")
        return None
    # Surface counts for operators (even on ok=false with remaining backlog).
    sys.stderr.write(
        "agent_errors_replay:"
        f" ok={payload.get('ok')}"
        f" replayed={payload.get('replayed', 0)}"
        f" remaining={payload.get('remaining', 0)}"
        f" failed={payload.get('failed', 0)}"
        f" quarantined={payload.get('quarantined', 0)}\n"
    )
    if payload.get("ok") is not True:
        # A 0-exit subprocess whose payload says ok=false is a failed drain;
        # it must reach the warnings sink, not stderr alone [OBS-08].
        _degrade(
            f"ok={payload.get('ok')}"
            f" failed={payload.get('failed', 0)}"
            f" remaining={payload.get('remaining', 0)}"
...
def maybe_reap_stale_dev_temp(
    repo: Path | None = None,
    *,
    warnings: list[str] | None = None,
) -> dict[str, Any] | None:
    """Best-effort stale dev-tree age-out (internal / [RES-07]).

    Imports ``workbay_bootstrap.dev_temp.reap_stale_dev_temp`` and runs with
    ``apply=True``. On any import/runtime failure degrades to None — never
    raises, never fails the host lifecycle command. ``repo`` selects the
    second root (``<repo>/.workbay/scratch``) alongside the global ``/tmp``
    scan; without it only ``/tmp`` is reclaimed.

    When ``warnings`` is provided, each degradation path appends the same
    cause string written to stderr so receipt consumers can see why the
    sub-step failed without reading stderr [OBS-08][AGT-10].
    """

    def _degrade(cause: str) -> None:
        line = f"stale_dev_temp_reap: degraded: {cause}"
        sys.stderr.write(line + "\n")
        if warnings is not None:
            warnings.append(line)

    try:
        from workbay_bootstrap.dev_temp import reap_stale_dev_temp  # noqa: PLC0415
    except Exception as exc:  # noqa: BLE001 — never-raise host contract
        _degrade(f"import {type(exc).__name__}: {exc}")
        return None
    try:
        report = (
            reap_stale_dev_temp(apply=True, repo=repo)
            if repo is not None
            else reap_stale_dev_temp(apply=True)
        )
    except TypeError as exc:
        # scripts/ and the installed workbay_bootstrap are versioned
        # separately and this import is dynamic. An older package has no
        # ``repo`` parameter; losing the scratch arm is acceptable, losing
        # the /tmp arm that already worked is not.
        _degrade(f"repo-scoped scratch arm unavailable ({exc}); retrying /tmp only")
        try:
            report = reap_stale_dev_temp(apply=True)
        except Exception as retry_exc:  # noqa: BLE001 — never-raise
            _degrade(f"{type(retry_exc).__name__}: {retry_exc}")
            return None
    except Exception as exc:  # noqa: BLE001 — never-raise
        _degrade(f"{type(exc).__name__}: {exc}")
        return None
    if not isinstance(report, dict):
        _degrade(f"non-dict report type {type(report).__name__}")
        return None
    removed = report.get("removed") or []
    stale_count = report.get("stale_count", 0)
    try:
        n_removed = len(removed) if isinstance(removed, list) else int(removed or 0)
    except (TypeError, ValueError):
        n_removed = 0
    if n_removed or stale_count:
        sys.stderr.write(
            "stale_dev_temp_reap:"
            f" removed={n_removed}"
...
def maybe_replay_terminal_guard_spool(
    repo: Path,
    *,
    warnings: list[str] | None = None,
) -> dict[str, Any] | None:
    """Best-effort terminal_guard spool drain ([RES-07]; internal).

    Shells out via :func:`handoff_command_argv` to
    ``terminal-guard-replay-spool`` (resolved hermetic binary +
    ``--workspace-root``). No ``--max-batch``: terminal-guard dispatch
    currently discards it for whole-spool drain; making the bound real is
    ``internal``. Same never-raise posture as
    :func:`maybe_replay_agent_error_spool` — missing CLI / timeout /
    non-json degrades to a stderr log and returns None.
    ...
    def _degrade(cause: str) -> None:
        line = f"terminal_guard_replay: degraded: {cause}"
        sys.stderr.write(line + "\n")
        if warnings is not None:
            warnings.append(line)

    try:
        # Do not send --max-batch: terminal-guard dispatch currently discards
        # it (whole-spool drain; see internal).
        # Prefer an honest paste-ready remedy over an inert bound. The printed
        # timeout remedy is shlex.join(argv), so omitting the flag here is
        # what keeps all three surfaces (docstring, argv, print) honest.
        argv = handoff_command_argv(
            repo,
            "terminal-guard-replay-spool",
        )
        proc = run_handoff_subprocess(repo, argv, timeout=TERMINAL_GUARD_REPLAY_TIMEOUT)
    except Exception as exc:  # noqa: BLE001 — never-raise host contract
        _degrade(f"{type(exc).__name__}: {exc}")
        return None
    if proc.returncode != 0:
        ...
        if proc.returncode == 124:
            # Printed remedy is the argv that timed out (structural; no second
            # derivation that can disagree with the executed list). Bound is
            # absent because it is absent from argv (see above /
            # internal).
            cause = _timeout_replay_degrade_cause(
                repo,
                base_cause=cause,
                subsystem="terminal-guard spool",
                drain_command=shlex.join(argv),
                spool_rel=TERMINAL_GUARD_SPOOL_REL,
            )
        _degrade(cause)
        return None
    try:
    ...
    if not isinstance(payload, dict):
        _degrade("unexpected payload")
        return None
    # terminal_guard envelope uses processed/ingested/deduped/invalid.
    data = payload.get("data") if isinstance(payload.get("data"), dict) else payload
    sys.stderr.write(
        "terminal_guard_replay:"
        f" ok={payload.get('ok')}"
        f" processed={data.get('processed', payload.get('processed', 0))}"
        f" ingested={data.get('ingested', payload.get('ingested', 0))}"
        f" deduped={data.get('deduped', payload.get('deduped', 0))}"
        f" invalid={data.get('invalid', payload.get('invalid', 0))}\n"
    )
    if payload.get("ok") is not True:
        # A 0-exit subprocess whose payload says ok=false is a failed drain;
        # it must reach the warnings sink, not stderr alone [OBS-08].
...
def maybe_drain_lifecycle_reexec_degrades(
    repo: Path | None = None,
    *,
    warnings: list[str] | None = None,
) -> list[str] | None:
    """Pull re-exec heal degrades into the receipt warnings sink [OBS-08, AGT-10].

    Reads both the entry-module accumulator (same process: heal gave up without
    ``execve``) and ``WORKBAY_LIFECYCLE_REEXEC_DEGRADED`` (parent observations
    carried across re-exec). Consumes both sources exactly once — pops the env
    var and clears the accumulator — so a later lifecycle process (or a
    detached Popen child that inherits ambient env) cannot re-report the same
    cause [RES-03, ARCH-13]. Never raises. When ``warnings`` is provided,
    appends each distinct line so a receipt consumer need not read stderr.
    ``repo`` is unused but kept for call-site parity with sibling reclaimers.

    Each line must start with the typed prefix; at most 20 lines are kept and
    each is truncated to 200 characters so unvalidated ambient text cannot
    reach the receipt unbounded [OBS-08, DATA-01].
    """
    del repo  # signature parity with sibling maybe_* reclaimers

    _prefix = "lifecycle_reexec: degraded: "
    _max_lines = 20
    _max_len = 200
    ...
    def _add(line: str) -> None:
        ...
        if not text_line or text_line in seen:
            return
        if not text_line.startswith(_prefix):
            return
        if len(text_line) > _max_len:
            text_line = text_line[:_max_len]
        seen.add(text_line)
        lines.append(text_line)

    raw = os.environ.get(_REEXEC_DEGRADED_ENV, "") or ""
    if raw:
    ...
    if main_mod is not None:
        acc = getattr(main_mod, "_REEXEC_DEGRADED_LINES", None)
        if isinstance(acc, list):
            for item in acc:
                if isinstance(item, str):
                    _add(item)
            # Drain in place so a subsequent call (or a long-lived host that
            # reuses this module) cannot re-surface the same lines [RES-03].
            acc.clear()

    # Consumed exactly once, mirroring the re-exec sentinel pop [RES-03].
    os.environ.pop(_REEXEC_DEGRADED_ENV, None)
...
def run_subprocess(
    ...
    *,
    ...
    ``TimeoutExpired`` plus ``OverflowError``/``ValueError`` from an
    unusable ``timeout`` (``inf``, NaN, too-large) all surface as rc 124
    so a bad budget cannot traceback out of a lifecycle verb.
    """
    try:
    ...
    except (subprocess.TimeoutExpired, OverflowError, ValueError):
        return subprocess.CompletedProcess(
            args=argv, returncode=124, stdout="", stderr="timed out"
        )


def _parse_positive_finite_timeout(raw: str | None, default: float) -> float:
    """Parse an env timeout; invalid/non-finite/out-of-range values fall back.

    ``subprocess.run`` raises ``OverflowError`` on ``inf`` / too-large
    timeouts and ``ValueError`` on NaN; those must never reach the
    subprocess. Accepted values are ``0 < value < 1_000_000``.
    """
    if not raw:
        return default
    try:
...
def handoff_timeout() -> float:
    ...
    """
    return _parse_positive_finite_timeout(
        os.environ.get("WORKBAY_PROJECTION_HANDOFF_TIMEOUT"),
        DEFAULT_HANDOFF_TIMEOUT,
    )


def handoff_read_timeout() -> float:
    """Wall-clock budget for a read-only handoff-CLI query (e.g. ``handoff-rows``).

    Unlike the write/replay path (:func:`handoff_timeout`), a read query's
    *false-empty* result is a silent correctness hazard, not a mere pending
    write: ``_live_handoff_rows`` degrades a timed-out read to ``[]``, so the
    resolver loses the live task-ref registry and can misresolve a
    ``feature/<base>-fu-<slug>`` branch onto its base. The default stays at
    ``DEFAULT_HANDOFF_TIMEOUT`` so production still bounds a hung CLI, but the
    budget is env-overridable (``WORKBAY_HANDOFF_READ_TIMEOUT``, seconds) so a
    heavyweight console-script cold-start (~2-3s) under parallel test load or a
    warming backend cannot trip the timeout and manufacture a false-empty
    registry. Mirrors the write path's tunability rather than leaving the
    correctness-sensitive read hardcoded.
    """
    return _parse_positive_finite_timeout(
        os.environ.get("WORKBAY_HANDOFF_READ_TIMEOUT"),
        DEFAULT_HANDOFF_TIMEOUT,
    )


def task_reap_timeout() -> float:
    """Wall-clock budget for the whole-registry ``task-reap`` sweep.

    Unlike the write/replay path (:func:`handoff_timeout`) and the
    read-only query path (:func:`handoff_read_timeout`), the reap
    classifier walks every live registry row, so its cost scales with
    registry size. Evidence behind the default (internal):

    * Measured 2026-09-05 on this repository's 214-row registry:
      ``WORKBAY_TASK_REAP_TIMEOUT=600 make task-reap REAP_ARGS=--json``
      took 2m14.56s wall (rc=0; closeable=0, active=105, ambiguous=78,
      reaped=0), i.e. ~0.63 s/row.
    * With the previous 120s default every ``make task-reap`` on that
      registry returned ``{"ok": false, "error": "reap_unavailable",
      "warning": {"reason": "timeout", "exception_type":
      "TimeoutExpired"}}`` with rc 2, and the receipt did not say which
      budget had been used.

    The default is therefore 600s (``DEFAULT_TASK_REAP_TIMEOUT``): ~4.5x
    the measured sweep, so production still bounds a hung CLI without
    racing a healthy sweep on a registry of up to ~950 rows at the
    measured per-row cost. Env-overridable via ``WORKBAY_TASK_REAP_TIMEOUT``
    (seconds) for tests and operators; invalid, empty, non-finite,
    non-positive, or oversized (>= 1_000_000) values fall back to
    ``DEFAULT_TASK_REAP_TIMEOUT``. ``_parse_positive_finite_timeout``'s
    upper bound (1_000_000) does not clamp the default.
    """
    return _parse_positive_finite_timeout(
        os.environ.get("WORKBAY_TASK_REAP_TIMEOUT"),
        DEFAULT_TASK_REAP_TIMEOUT,
    )


def task_reap_timeout_hint(budget_seconds: float) -> str:
    """Operator hint for a timed-out reap sweep [AGT-10, OBS-08].

    Names the env knob and the measured per-row cost so the operator can
    size the budget from the registry row count instead of guessing.
    """
    return (
        f"task-reap sweep exceeded its {budget_seconds:g}s budget "
        f"(WORKBAY_TASK_REAP_TIMEOUT). Measured cost is "
        f"~{TASK_REAP_MEASURED_SECONDS_PER_ROW} s/row "
        f"({TASK_REAP_MEASURED_ROWS}-row registry took "
        f"{TASK_REAP_MEASURED_WALL_SECONDS}s on 2026-09-05); set "
        f"WORKBAY_TASK_REAP_TIMEOUT above rows * "
        f"{TASK_REAP_MEASURED_SECONDS_PER_ROW} seconds and retry."
    )


def handoff_envelope_warning(
    payload: Any,
    warning: ReceiptWarning | None,
    *,
    field: str,
) -> ReceiptWarning | None:
    """Return a warning unless a handoff payload passed the strict envelope gate.

    Callers must run this check before flattening a decoded payload. A valid
    envelope is a dict with the literal boolean ``ok is True`` and no warning;
    missing, truthy-non-boolean, non-dict, or otherwise warned payloads remain
    degraded [OBS-08][AGT-10].
    """
    ...
    if payload.get("ok") is not True:
        return ReceiptWarning(field=field, reason="not_ok")
    return None
...
def run_handoff_json(
    ...
    *,
    ...
    """Bounded handoff-CLI JSON probe with structural success gating.

    Zero-exit stdout is parsed as JSON. The parsed payload is always
    returned when decoding succeeds so reporting callers keep their data,
    but a non-None :class:`ReceiptWarning` is also returned when the
    payload is not a dict or its ``ok`` key is not True — callers treat
    ``warning is None`` as success, so the warning channel is the
    structural place to surface envelope degradation [OBS-08][ARCH-13].
    """
    ...
    # Hoist dict + ok gates into the shared helper so every call path
    # degrades loudly rather than reading ok=false as success [OBS-08][ARCH-13].
    if not isinstance(payload, dict):
        return payload, ReceiptWarning(
            field=field,
            reason="malformed",
            exception_type="UnexpectedPayload",
        )
    if payload.get("ok") is not True:
        # Distinct from decode/non-dict "malformed": CLI reported failure [OBS-08].
        return payload, ReceiptWarning(field=field, reason="not_ok")
    return payload, None
...
def run_orchestrator_json(
    ...
    *,
    ...
    JSON-malformed-as-warning, non-dict/ok-not-true-as-warning). Callers
    select the read-only orchestrator subcommand (e.g.
    ``["orchestrator-status"]``) — the helper does not pick one so a
    future cheaper surface stays trivially swappable.
    """
    proc = run_subprocess(
    ...
    # Same structural gate as run_handoff_json [OBS-08][ARCH-13].
    if not isinstance(payload, dict):
        return payload, ReceiptWarning(
            field=field,
            reason="malformed",
...
def _live_handoff_rows(repo: Path) -> list[dict[str, Any]]:
    ...
    proc = run_handoff_subprocess(repo, argv, timeout=handoff_read_timeout())
    if proc.returncode != 0:
        return []
    try:
        payload = json.loads(proc.stdout)
...
class WorkspaceSummaryRenderError(RuntimeError):
    """Raised when render-handoff returns an explicit failure outcome."""
...
class WorkspaceSummaryView:
    ...
    - ``resolved_task_ref`` / ``tiebreak_*``: resolution metadata from the
      render envelope. These fields let lifecycle readers distinguish a
      resolver-approved winner from an unresolved multi-task summary.
    """
...
def _coerce_v2(payload: dict[str, Any]) -> WorkspaceSummaryView:
    ...
    raise WorkspaceSummaryParseError(f"Unknown v2 workspace-summary shape: {shape!r}")
...
def derive_workspace_summary_view(repo: Path) -> WorkspaceSummaryView:
    ...
    Unavailable, unsuccessful, or malformed render output retains the
    historical ``shape="none"`` fallback, but typed process failures fail
    closed:

    - CLI unavailable / timeout without a typed result: ``shape="none"``.
    - Non-zero exit with a typed error: :class:`WorkspaceSummaryRenderError`.
    - Envelope JSON malformed: ``shape="none"``.
    - Envelope ``ok`` is not True after a zero exit: ``shape="none"``.
    ...
    proc = run_handoff_subprocess(
        ...
        handoff_command_argv(
            repo, "render-handoff", "--kind=current_task", "--no-write"
        ),
        timeout=handoff_read_timeout(),
    )
    try:
    ...
    if proc.returncode != 0:
        ...
        if isinstance(error, str) and error.strip():
            raise WorkspaceSummaryRenderError(error)
        return _NONE_VIEW
    ...
    if not isinstance(payload, dict):
        return _NONE_VIEW
    try:
        view = load_workspace_summary(payload)
    except WorkspaceSummaryParseError:
        return _NONE_VIEW
    resolved_task_ref = data.get("task_ref")
    if not isinstance(resolved_task_ref, str) or not resolved_task_ref:
        resolved_task_ref = None
    tiebreak_note = data.get("tiebreak_note")
    if not isinstance(tiebreak_note, str) or not tiebreak_note:
        tiebreak_note = None
    raw_candidates = data.get("tiebreak_candidates")
    if isinstance(raw_candidates, (list, tuple)) and all(
        isinstance(candidate, str) and candidate for candidate in raw_candidates
    ):
        tiebreak_candidates = tuple(raw_candidates)
    else:
...
def load_workspace_summary(
    ...
    if isinstance(source, Path):
        ...
        try:
            ...
            raise WorkspaceSummaryParseError(f"Failed to read {source}: {exc}") from exc
        try:
            payload = json.loads(text)
