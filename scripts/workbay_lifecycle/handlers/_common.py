...
# Resolution #2 — one reclaimer, both spools). Batch constant matches agent-errors.
TERMINAL_GUARD_REPLAY_MAX_BATCH = AGENT_ERRORS_REPLAY_MAX_BATCH
...
def maybe_replay_agent_error_spool(
    ...
    *,
    ...
    Shells out to ``mcp-workbay-handoff errors-replay-spool --max-batch
    <AGENT_ERRORS_REPLAY_MAX_BATCH>``. On ANY failure (nonzero exit, timeout,
    exception, missing CLI) degrades to a stderr log and returns None —
    never raises, never blocks or fails the host lifecycle command. Prefer
    capturing the JSON count for the receipt/log when the subprocess
    succeeds.
...
def maybe_replay_terminal_guard_spool(
    ...
    *,
    ...
    Shells out to ``mcp-workbay-handoff terminal-guard-replay-spool --max-batch
    <TERMINAL_GUARD_REPLAY_MAX_BATCH>``. Same never-raise posture as
    :func:`maybe_replay_agent_error_spool` — missing CLI / timeout / non-json
    degrades to a stderr log and returns None.
    ...
    try:
        argv = handoff_command_argv(
            ...
            "--max-batch",
            str(TERMINAL_GUARD_REPLAY_MAX_BATCH),
        )
        proc = run_handoff_subprocess(
