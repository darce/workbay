...
def maybe_replay_agent_error_spool(repo: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        sys.stderr.write(
            f"agent_errors_replay: degraded: {type(exc).__name__}: {exc}\n"
        )
        return None
    if proc.returncode != 0:
        err_snip = (proc.stderr or proc.stdout or "").strip().replace("\n", " ")[:200]
        sys.stderr.write(
            f"agent_errors_replay: exit={proc.returncode}"
    ...
    try:
        ...
        sys.stderr.write("agent_errors_replay: degraded: non-json stdout\n")
        return None
    if not isinstance(payload, dict):
        sys.stderr.write("agent_errors_replay: degraded: unexpected payload\n")
        return None
...
def maybe_reap_stale_dev_temp(repo: Path | None = None) -> dict[str, Any] | None:
    ...
    try:
        ...
        sys.stderr.write(
            f"stale_dev_temp_reap: degraded: import {type(exc).__name__}: {exc}\n"
        )
        return None
    try:
        ...
        sys.stderr.write(
            f"stale_dev_temp_reap: degraded: {type(exc).__name__}: {exc}\n"
        )
        return None
...
def maybe_replay_terminal_guard_spool(repo: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        sys.stderr.write(
            f"terminal_guard_replay: degraded: {type(exc).__name__}: {exc}\n"
        )
        return None
    if proc.returncode != 0:
        err_snip = (proc.stderr or proc.stdout or "").strip().replace("\n", " ")[:200]
        sys.stderr.write(
            f"terminal_guard_replay: exit={proc.returncode}"
            + (f" {err_snip}" if err_snip else "")
            + "\n"
        )
        return None
    try:
        ...
        sys.stderr.write("terminal_guard_replay: degraded: non-json stdout\n")
        return None
    if not isinstance(payload, dict):
        sys.stderr.write("terminal_guard_replay: degraded: unexpected payload\n")
        return None
