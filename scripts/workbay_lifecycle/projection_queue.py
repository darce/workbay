...
def draining_snapshot_is_orphan(path: Path) -> bool:
    """True when a ``.draining.<pid>-<ts>`` claim is dead or unparseable.

    Snapshot names embed the claimer pid. A live concurrent drainer must not
    be labeled orphaned (OBS-08 / REAPCONV-SPOOL-R3-ORPHAN-LABEL-DRIFT-...);
    only a dead pid (or a malformed filename) counts as reclaimable orphan
    noise. Shared by ``spool_health`` (status/doctor) and task-start's
    foreground drain gate so the surfaces answer the same question.

    The token after ``.draining.`` must be canonical decimal on both halves
    (non-empty, no leading zeros except the single digit ``0``). A
    ``.checkpoint`` sidecar is not a claim: it carries a borrowed claimer
    pid and is resolved via sibling-claim existence (never ``os.kill`` on
    the sidecar). A non-positive pid is rejected after parse. Any other
    shape is reclaimable noise and must not be probed with ``os.kill``
    (SECD-05): a reclaimer that cannot parse a name defaults to orphan,
    not to "someone else owns this".
    """
    name = path.name
    marker = ".draining."
    ...
    if idx < 0:
        return True
    # In-glob name producers:
    # 1. Claim writer: f"...draining.{os.getpid()}-{time.time_ns()}"
    # 2. Bounded-drain sidecar: claim_path.with_suffix(claim_path.suffix
    #    + ".checkpoint") in project_events_replay._bounded_checkpoint_path
    #    -> "...draining.PID-TS.checkpoint"
    # Both halves must be non-empty canonical ASCII decimal digits only.
    # Loose split/int parsing accepts shapes like ``1-``, ``1--2``, `` 1-2``,
    # ``+5-1`` and probes a pid that is not a claimer (pid 1 PermissionError
    # -> "live" wedges reclaim permanently). ``str.isdigit()`` alone is not
    # a safe pre-int guard: it accepts non-ASCII decimal digits that int()
    # also accepts, and superscripts that int() rejects — neither is a
    # writer shape. Malformed => orphan without os.kill.
    rest = name[idx + len(marker) :]
    # Sidecar from _bounded_checkpoint_path carries the *claimer's* pid, not
    # the sidecar writer's. The sidecar can outlive the claim; once the claim
    # is gone the embedded pid is not ownership evidence (and a recycled pid
    # would wedge reclaim as "live"). Resolve via sibling claim existence:
    # absent → orphan; present → answer the claim predicate. Never os.kill on
    # a .checkpoint name. Double .checkpoint is not a writer shape → orphan.
    if rest.endswith(".checkpoint"):
        body = rest[: -len(".checkpoint")]
        if body.endswith(".checkpoint"):
            return True
        sibling = path.with_name(name[: -len(".checkpoint")])
        if not sibling.is_file():
            return True
        return draining_snapshot_is_orphan(sibling)
    if "-" not in rest:
        return True
    pid_token, ts_token = rest.split("-", 1)
    if not (
        pid_token.isascii()
        and pid_token.isdigit()
        and ts_token.isascii()
        and ts_token.isdigit()
    ):
        return True
    # The writer emits str(os.getpid()) and str(time.time_ns()); neither ever
    # carries a leading zero. "01" parses to pid 1 and probes init
    # (PermissionError -> "live"), which wedges reclaim forever -- the exact
    # hazard the ASCII-digit gate was added to close. Canonical decimal only
    # (SECD-05).
    if (len(pid_token) != 1 and pid_token.startswith("0")) or (
        len(ts_token) != 1 and ts_token.startswith("0")
    ):
        return True
    try:
        pid = int(pid_token)
        ...
        return True
    # pid 0 probes the caller's process group (false "live"); overflow tokens
    # must not escape as OverflowError on the never-raise path.
    # With strict ASCII digits a leading "-" cannot reach int(); only pid == 0
    # remains reachable under this guard (``.draining.0-123``). Keep
    # ``pid <= 0`` so the process-group hazard stays pinned (REVF-SPOOL2-03).
    if pid <= 0:
        return True
    try:
        os.kill(pid, 0)
...
def has_orphaned_draining_claim(repo: Path) -> bool:
    """Pid-aware orphan probe used by spool_health and foreground drain gates."""
    state_dir = repo / ".task-state"
    if not state_dir.is_dir():
        return False
    for snapshot in state_dir.glob(DRAINING_GLOB):
        if snapshot.is_file() and draining_snapshot_is_orphan(snapshot):
            return True
    return False
...
def spool_health(
    ...
    *,
    ...
    state_dir = repo / ".task-state"
    # Pid-aware: a live concurrent claim is not an orphan (label drift fix).
    has_orphan_draining = has_orphaned_draining_claim(repo)
...
# ``mcp-workbay-handoff --help`` is not a cheap liveness ping: the CLI imports
# fastmcp, mcp, pydantic and workbay_protocol.bootstrap at module scope, so
# ~6.9s is spent before argparse runs at all. Measured wall clock on a loaded
# host spans 2.15-13.15s. The previous hardcoded 6s sat inside that spread, and
# because this probe is the *only* exit from an open breaker, a slow host
# latched it permanently -- every mutating command, including ``task-start``,
# stayed fail-closed with no path back. The bound is generous by design and
# tunable for hosts slower still [RES-02][RES-07].
PROBE_TIMEOUT_SECONDS_DEFAULT = 45.0

# ``timeout`` is the shell convention for "the command was killed on the clock",
# reused here so callers can branch on the outcome instead of parsing text
# [AGT-21].
PROBE_TIMEOUT_RETURNCODE = 124


def _float_env(name: str, default: float) -> float:
    """Read a float bound override from the environment, falling back to default."""
    raw = os.environ.get(name)
    if raw is None:
        return default
    try:
...
def _handoff_cli_reachable(repo: Path) -> tuple[bool, int | None]:
    ...
    try:
        proc = subprocess.run(
            ...
            text=True,
            timeout=_float_env(
                "WORKBAY_PROJECTION_PROBE_TIMEOUT_S", PROBE_TIMEOUT_SECONDS_DEFAULT
            ),
            check=False,
        )
        ...
        return False, PROBE_TIMEOUT_RETURNCODE
    except OSError:
        return False, 127
...
def projection_preflight(repo: Path, command: str) -> dict[str, Any] | None:
    ...
    if reachable and not over_hard_limit:
        ...
        return None
    probe_timed_out = returncode == PROBE_TIMEOUT_RETURNCODE
    if not reachable and not probe_timed_out:
        # A timeout is an absence of evidence, not evidence of absence: the
        # backend may be perfectly healthy behind a loaded host. Counting it
        # as a failure is what let the breaker deepen on every blocked
        # command while the probe -- its only exit -- kept timing out. Stay
        # fail-closed for this command, but do not deepen [RES-02][OBS-08].
    ...
    if over_hard_limit:
        ...
        recovery_command = (
            "handoff CLI probe timed out (host load, not an unreachable "
            "backend); retry, or raise WORKBAY_PROJECTION_PROBE_TIMEOUT_S "
            "above the default 45s"
        )
    else:
        recovery_command = (
    ...
    return {
        ...
        "projection_queue": {
            ...
            "handoff_cli_probe_timed_out": probe_timed_out,
        },
    }
