...
* ``--print-installed-dirs`` — ``uv pip install --reinstall <dirs>`` (pyenv sync).
...
def installed_dirs(
    ...
    """Package dirs of every *installed* workspace dist, for
    ``uv pip install --reinstall <dirs>`` (pyenv sync target)."""
    return [
        pkg_dir
