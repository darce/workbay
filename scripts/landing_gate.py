...
def _failure_data_bound_from_text(
    raw_text: str | None, ids_text: str | None
) -> tuple[list[str], str, list[str], str | None]:
    if raw_text is None:
        return [], "ids_not_measured", [], None
    extracted = extract_failure_ids(raw_text)
    errored = list(extracted.get("errored") or [])
    errored_reason = extracted.get("errored_reason")
    if not isinstance(errored_reason, str):
        errored_reason = None
    if extracted["outcome"] != "extracted":
        return [], "ids_not_measured", errored, errored_reason
    ...
    if normalize_failure_ids(extracted_ids) != normalize_failure_ids(supplied):
        return extracted_ids, "mismatch", errored, errored_reason
    return extracted_ids, "bound", errored, errored_reason


def _ids_bound_from_text(raw_text: str | None, ids_text: str | None) -> tuple[list[str], str]:
    ids, status, _errored, _reason = _failure_data_bound_from_text(raw_text, ids_text)
    return ids, status
...
def _run_gate_report(args: Any) -> int:
    ...
    baseline_ids, baseline_bind, baseline_errored, baseline_errored_reason = _failure_data_bound_from_text(
    ...
    subject_ids, subject_bind, subject_errored, subject_errored_reason = _failure_data_bound_from_text(
    ...
    report = gate_report(
        ...
        diff=failure_id_diff(
            ...
            subject_errored=subject_errored,
            baseline_errored_reason=baseline_errored_reason,
            subject_errored_reason=subject_errored_reason,
        ),
...
def _human_summary(report: dict[str, Any]) -> str:
    ...
    for rel in diff.get("errored", []):
        lines.append(f"  ERRORED {rel}")
    if diff.get("errored_reason"):
        lines.append(f"  ERRORED reason={diff['errored_reason']}")
    return "\n".join(lines)
