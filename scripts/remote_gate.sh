...
#      parsed, so env always wins). The file is never sourced.
#
...
# Layer 3 capture FIRST: caller env is snapshotted before the file is parsed
# so documented env-over-file precedence cannot invert.
...
# Layer 2: per-repo config file parsed as data (allowlisted keys only).
...
_remote_gate_env_load "$config_file" \
    REMOTE_GATE_HOST REMOTE_GATE_DIR REMOTE_GATE_WORKDIR \
    REMOTE_GATE_TARGETS REMOTE_GATE_ENV PYTEST_WORKERS
...
WORKERS="${_env_workers:-${PYTEST_WORKERS:-3}}"
...
# -n / StdinNull: ssh must not read the controlling TTY. A backgrounded gate
# (or any caller that loses the tty) otherwise stops on SIGTTIN and wedges
# forever (MAIN-REMOTEGATE-SIGTTIN-WEDGE-01). ConnectTimeout only bounds the
# TCP handshake — not the session.
SSH=(ssh -n -o BatchMode=yes -o ConnectTimeout=10
     -o ServerAliveInterval=30 -o ServerAliveCountMax=4 "$REMOTE_HOST")
# Wall-clock bound for the remote session when timeout(1) exists. 0 disables.
# macOS bash 3.2 hosts often lack timeout(1); -n still prevents the SIGTTIN wedge.
GATE_RUN_TIMEOUT_S="${WORKBAY_REMOTE_GATE_RUN_TIMEOUT_S:-7200}"
case "$GATE_RUN_TIMEOUT_S" in *[!0-9]*|"") GATE_RUN_TIMEOUT_S=7200 ;; esac
_ssh() {
    # Avoid "$@" empty-array pitfalls under bash 3.2 set -u by requiring args.
    if [ "$GATE_RUN_TIMEOUT_S" -gt 0 ] 2>/dev/null && command -v timeout >/dev/null 2>&1; then
        timeout -k 30 "$GATE_RUN_TIMEOUT_S" "${SSH[@]}" "$@"
    else
        "${SSH[@]}" "$@"
    fi
}
...
run)
    ...
    # Transport ref, deliberately OUTSIDE refs/heads/. The remote consumes the
    # commit by SHA (git checkout -qf ${sha} below) and never reads this name,
    # so the ref is a pure object anchor. Under refs/heads/ the branch-naming
    # pre-push gate classifies it as a published branch and hard-blocks every
    # gate run, because "remote-gate" cannot satisfy the feature/<task-ref>
    # grammar. The audited override exists for genuine nonconforming branches;
    # exporting it here would spam the audit log every run and mask real
    # refusals. collect_pre_push_branch_updates() only names refs/heads/*, so a
    # ref in another namespace is skipped rather than overridden.
