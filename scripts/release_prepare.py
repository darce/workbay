...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip the post-bump re-derive of installed dist metadata. The sync "
            "targets already self-skip when their env is absent, so this is an "
            "operator escape hatch for CI/dispatch, not a correctness flag."
        ),
    )
...
def main() -> int:
    ...
    # implementation note: re-derive installed dist metadata so the bump is transactional
    # over its derived state. Gated below the --dry-run early return above, so
    # a dry run never mutates installed envs.
