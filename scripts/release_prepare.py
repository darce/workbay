...
def rederive_install_state() -> None:
    ...
    root venv via ``sync-version-drift-venv``. The description-service pyenv
    reinstall band-aid (``sync-version-drift-pyenv``) was retired (implementation note
    Resolution #1); ``version_of`` self-heals from in-tree pyproject, and
    cross-interpreter skew remains a diagnostic via ``check-version-drift-pyenv``
    + doctor / src-provisioning — not a post-bump reinstall.
