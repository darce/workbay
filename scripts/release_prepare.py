...
def update_changelog(changelog_path: Path, new_version: str, release_date: str) -> str:
    ...
    if entry_heading in source:
        return source
    marker = re.search(r"(?m)^## Unreleased[ \t]*$", source)
    if marker is None:
        raise SystemExit(f"could not find '## Unreleased' in {changelog_path}")
    following = re.search(r"(?m)^## ", source[marker.end():])
    offset = marker.end() + following.start() if following else len(source)
    insertion = (
        f"{entry_heading}\n\n"
        "### Changed\n\n"
    ...
    return prefix + separator + insertion + source[offset:]
...
def rederive_install_state() -> None:
    ...
    installed root ``.venv`` dists keep the pre-bump version. Re-derive the
    root venv via ``sync-version-drift-venv``. No handoff-specific tail is
    needed: implementation note moved the handoff suite onto a dedicated ``.venv-handoff``
    env re-derived from src on every gated run (``ensure-handoff-env``
    sync-on-run), so it self-heals post-bump without a re-derive step — the
    description-service pyenv, its ``sync-version-drift-pyenv`` reinstall
    band-aid (retired implementation note Resolution #1), and its
    ``check-version-drift-pyenv`` gate (retired internal) are all gone.
    ``version_of`` self-heals from in-tree pyproject; residual skew stays a
    diagnostic via the doctor ``version_of_skew`` facet / src-provisioning.

    The venv target self-skips when the env is absent (fresh clone / CI); a
    genuine sync failure is a HARD error — leaving the SSOT and root install
    silently split-brain is the defect this removes.
...
def regenerate_uv_lock() -> None:
    """Regenerate root uv.lock after version-pin rewrites.

    Closes the direct-invocation hole: ``python scripts/release_prepare.py``
    rewrites pyproject floors without going through the make ``release-uv-lock``
    wrapper. Called only on the real write path (below ``main``'s ``--dry-run``
    early return) so dry runs never mutate the lock. Fail hard so a stale lock
    cannot ride along with a version bump.
    """
    result = subprocess.run(["uv", "lock"], cwd=REPO_ROOT)
    if result.returncode != 0:
        raise SystemExit(
            "release-prepare: uv lock failed with exit "
            f"{result.returncode}. Fix the lock and re-run so the version bump "
            "does not leave uv.lock stale."
        )
    print("release-prepare: regenerated uv.lock — commit it with the version bump")
...
def main() -> int:
    ...
    # Keep root uv.lock in sync with rewritten workspace pins. Below the
    # --dry-run early return so previews never rewrite the lock; also covers
    # direct python invocation that bypasses make release-uv-lock.
    regenerate_uv_lock()
