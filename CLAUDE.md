...
`packages/workbay-system/scripts/generate_agent_workflows.py` when the workflow
manifest changes.
...
- `/offload` (write) -> skill `offload` -> `(in-session grok offload skill; no standalone make target)`
